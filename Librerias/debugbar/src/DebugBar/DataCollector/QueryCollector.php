...
class QueryCollector extends PDOCollector
{
     ...
     */
    public function __construct(TimeDataCollector $timeCollector = null, ExceptionsCollector $exceptionCollector = null, $driver = 'MySQL')
    {
    ...
    public function setExceptions(ExceptionsCollector $exceptionCollector = null)
    {
        $this->exceptionCollector = $exceptionCollector;
    }
    ...
    public function addQuery($query, $other = array())
    {
        ...
        $driver = isset($other['driver']) ? $other['driver'] : ($this->driver ?: 'DB');
        ...
        if ($this->exceptionCollector && isset($other['is_success']) && ! $other['is_success']) {
            $this->exceptionCollector->addWarning(
                'DB_ERROR',
                $driver.'_Error '.$other['error_code'].': '.$other['error_message'],
                isset($source[0]['file']) ? $source[0]['file'] : '',
                isset($source[0]['line']) ? $source[0]['line'] : ''
            );
        }
        ...
        foreach ($this->connections as $name => $queries) {
            foreach ($queries as $query) {
                ...
                $statements[] = array_merge($query, array(
                    ...
                    'slow' => $this->slowThreshold && $this->slowThreshold <= $query['duration'],
                ));
            }
        }
