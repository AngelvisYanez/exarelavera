...
class ExceptionsCollector extends DataCollector implements Renderable
{
     ...
     */
    public function collectWarnings($preserveOriginalHandler = false) {
        $self = $this;

        $originalHandler = set_error_handler(function ($errno, $errstr, $errfile, $errline) use ($self, &$originalHandler, $preserveOriginalHandler) {
            $self->addWarning($errno, $errstr, $errfile, $errline);

            if ($preserveOriginalHandler && $originalHandler) {
    ...
    public function addWarning($errno, $errstr, $errfile = '', $errline = 0)
    {
        ...
        $warning = array(
            'count' => 1,
            'type' => (isset($errorTypes[$errno]) ? $errorTypes[$errno] : (!is_numeric($errno) ? $errno : 'UNKNOWN')),
            'message' => $errstr,
            'code' => $errno,
            'file' => $this->normalizeFilePath($errfile),
            'line' => $errline,
            'xdebug_link' => empty($errfile) ? null : $this->getXdebugLink($errfile, $errline)
        );
