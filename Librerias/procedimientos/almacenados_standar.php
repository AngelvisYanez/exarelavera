...
function is_empty($var,$key=null){
    if(is_null($var)||empty($var)) return true;
    if(!is_null($key)) return (!isset($var[$key])||empty($var[$key]));
    return true;
}
function set_val_or($var,$key=null,$def=''){ return is_empty($var,$key)?$def:(is_array($var)?$var[$key]:$var); }
...
function change_string_deep($procedure,&$input,$args1=null){
    if (is_string($input)) {
        if(!is_array($procedure)){ if(empty($args1))$input = call_user_func($procedure,$input); else $input = call_user_func($procedure,$input,$args1); }
        else foreach ($procedure as $fun) { $input = call_user_func($fun,$input); }
    } else if (is_array($input)) {
        foreach ($input as &$value) { change_string_deep($procedure,$value,$args1); } unset($value);
    } else if (is_object($input)) {
        $vars = array_keys(get_object_vars($input));
        foreach ($vars as $var) { change_string_deep($procedure,$input->$var,$args1); }
    }
}
...
function sql_conjunction($list, $prefix, $conjunction = 'OR', $init='AND') {
  ...
  if (is_array($list)&&count($list)>0) {
    foreach ($list as $k => &$v) $v=(is_numeric($k)?'':$k.(empty($prefix)?"=":'')).$prefix."'".$v."'"; unset($v);
    return $init.' ('.implode(' '.$conjunction.' ', $list).') ';
  }return '';
}
...
function endsWith($haystack, $needle) { // search forward starting from end minus needle length characters
...
function groupBy($array, $key) {
    ...
    foreach ($array as $element) {
        if (is_object($element)) {
            ...
            continue; // Si no es un array ni un objeto, lo ignoramos
        }
...
function pluck($array, $key, $implode = false, $default = 'NULL'){
    $result = array();
    if(is_array($array))
    foreach ($array as $element) {
        if (is_object($element)) {
            $result[$element->$key] = null;
        } elseif (is_array($element)) {
            $result[$element[$key]] = null;
        }
    }
...
function sortBy($array, $key) {
    ...
    $sortedArray = $array; // Clonamos el array original
    usort($sortedArray, function ($a, $b) use ($key) {
...
function error_alerta($mensaje, $tipo, $new_alert=false, $title=''){
	switch ($tipo){
		case 1: //Informacion
			$imagen = "admiracion2.jpg"; $clase = 'alertas2'; $alert = 'info'; $tooltip='Informaci&oacute;n'; $icon='info-circle';
			break;
		case 2://Advertencia
			$imagen = "advertencia.png"; $clase = 'alertas1'; $alert = 'warning'; $tooltip='Advertencia'; $icon='warning';
			break;
		case 3: //Grave
			$imagen = "error.png"; $clase = 'alertas3'; $alert = 'danger'; $tooltip='Error'; $icon='times-circle';
			break;
                case 4: //Tips notas de aviso
			$imagen = "aceptar.jpg"; $clase = 'alertas2'; $alert = 'success'; $tooltip='Tip de Ayuda'; $icon='lightbulb-o';
			break;
	} //$err ="<div align='center'><img src='../../mascaras/model1/imagenes/32x32/$imagen' height='32' width='32' />$mensaje</div>";
        $err ='<div '.($new_alert?'':'align="center"').' class="color-box '.$clase.'">
                    <div class="tip-box alert alert-'.$alert.'" '.(!$new_alert?'style="display:table;margin-top: 5px;"':'').'>'.(!$new_alert&&!empty($imagen)?"<div style='width: 50px; height:35px; display: table-cell;text-align: left; padding-top:2px;'><img src='../../mascaras/model1/imagenes/32x32/$imagen' height='32' width='32' style='position:absolute' /></div>":'').'
...
function num2letras($num, $fem = false, $dec = false) {
   ...
   $matuni[2]  = "dos";     $matdec[2] = "veint";       $matunisub[2] = "dos";
   $matuni[3]  = "tres";    $matdec[3] = "treinta";     $matunisub[3] = "tres";     $matsub[3]  = 'mill';
   $matuni[4]  = "cuatro";  $matdec[4] = "cuarenta";    $matunisub[4] = "cuatro";                           $matmil[4]  = 'millones';
   $matuni[5]  = "cinco";   $matdec[5] = "cincuenta";   $matunisub[5] = "quin";     $matsub[5]  = 'bill';
   $matuni[6]  = "seis";    $matdec[6] = "sesenta";     $matunisub[6] = "seis";                             $matmil[6]  = 'billones';
   $matuni[7]  = "siete";   $matdec[7] = "setenta";     $matunisub[7] = "sete";     $matsub[7]  = 'mill';   $matmil[7]  = 'de billones';
   $matuni[8]  = "ocho";    $matdec[8] = "ochenta";     $matunisub[8] = "ocho";                             $matmil[8]  = 'millones de billones';
   $matuni[9]  = "nueve";   $matdec[9] = "noventa";     $matunisub[9] = "nove";     $matsub[9]  = 'trill';
   $matuni[10] = "diez";                                                                                    $matmil[10] = 'trillones';
   $matuni[11] = "once";                                                            $matsub[11] = 'mill';   $matmil[11] = 'de trillones';
   $matuni[12] = "doce";                                                                                    $matmil[12] = 'millones de trillones';
   $matuni[13] = "trece";                                                           $matsub[13] = 'bill';   $matmil[13] = 'de trillones';
   $matuni[14] = "catorce";                                                                                 $matmil[14] = 'billones de trillones';
   $matuni[15] = "quince";                                                          $matsub[15] = 'mill';   $matmil[15] = 'de billones de trillones';
   $matuni[16] = "dieciseis";                                                                               $matmil[16] = 'millones de billones de trillones';
   ...
   for ($c = 0; $c < strlen($num); $c++) {
      ...
      if (! (strpos(".,'''", $n) === false)) {
         ...
         if ($punt) { if ($n != '0') $zeros = false; $fra .= $n; }else $ent .= $n;
      }else break;
   } $ent = '     ' . $ent;
   if ($dec and $fra and ! $zeros) {
      $fin = ' coma';
      for ($n = 0; $n < strlen($fra); $n++) { if (($s = $fra[$n]) == '0') $fin .= ' cero'; elseif ($s == '1') $fin .= $fem ? ' una' : ' un'; else $fin .= ' ' . $matuni[$s]; }
   }else{ $fin = ''.(" con ".(strlen($fra)<2?((empty($fra))||('0'.$fra)*1==0?"00":$fra."0"):$fra)."/100"); }
   if ((int)$ent === 0) return 'Cero ' . $fin;
   $tex = ''; $sub = 0; $mils = 0; $neutro = false;
   while ( ($num = substr($ent, -3)) != '   ') {
      $ent = substr($ent, 0, -3);
      if (++$sub < 3 and $fem) { $matuni[1] = 'una'; $subcent = 'as';
      }else{ $matuni[1] = $neutro ? 'un' : 'uno'; $subcent = 'os'; }
      $t = '';
      $n2 = substr($num, 1);
      if ($n2 == '00') {
      }elseif ($n2 < 21)
