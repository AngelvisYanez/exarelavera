...
if(isset($_SESSION['Ses_Prs_Cod']) && $_SESSION['Ses_Prs_Cod']==1 && \Env::get('DEBUGBAR_ENABLED', false)) {
    ...
    if (!class_exists('DebugBar', false)) { class DebugBar extends \DebugBar\DebugHelper{} }
    if (!class_exists('ChromePhp', false)) { class ChromePhp extends \DebugBar\DebugHelper{} }
    ...
    if (!class_exists('DebugBar', false)) { class DebugBar { public static function __callStatic($name, $arguments) { if($name == 'measure' && isset($arguments[1]) && is_callable($arguments[1])) $arguments[1](); return null; } } }
    if (!class_exists('ChromePhp', false)) { class ChromePhp extends DebugBar{} }
}
