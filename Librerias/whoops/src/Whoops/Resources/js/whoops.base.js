Zepto(function($) {
  ...
  $(document).delegate('.line-numbers-rows > span', 'click', function(e) {
    var $this = $(this),
      codeBlock = $this.parent().parent().parent(),
      linkTag = codeBlock.prev().find('a');

    if (! linkTag.length) return;

    var lineNumber = $this.index() + codeBlock.data('line-offset'),
      fileUrl = linkTag.attr('href').replace(new RegExp('([:=])' + codeBlock.data('line')), '$1' + lineNumber);

    if (linkTag.data('ajax')) {
      $.get(fileUrl);
      return;
    }

    $('<a href="' + fileUrl + '"></a>').trigger('click');
  });
