    ...
    if ($('#listsearch').length === 1) {
        ...
        listSearch.createGrid($.extend({
            ...
            colModel: [
                ...
                {
                    label: '&nbsp;', name: 'act1', width: 15, align: 'center', viewable: false, formatter: 'gridButton',
                    formatoptions: {
                        action: selectComp,
                        ...
                        // },
                        conditional: function (ro) {
                            // Pintar el background si el estado es 'I'
                            if (ro.Com_Est === 'I') {
                                setTimeout(function () {
                                    $("#listsearch").jqGrid('setRowData', ro.Com_Cod, false, { background: '#ffc3c3' });
                                }, 0);
                            }
                            if (alta) {
                                return (ro.Com_Est === 'I' || ro.Com_Est === 'B');
            ...
            loadComplete: function (data) {
                if ($.varValid(data.rows))
                    for (var i = 0, z = data.rows.length; i < z; i++) {
                        ...
                        if ((data.rows[i]['Com_Gen'] === 'A' || data.rows[i]['Doc'] !== '') && noEdit === true) $("#listsearch #" + data.rows[i].Com_Cod + ' td[aria-describedby=listsearch_act1] button').addClass('btn-warning').removeClass('btn-success');
                    }
            }
            ...
            colModel: [
                {
                    ...
                    formatter: function (cellvalue, options, rowObject) {
                        return '<span class="btn btn-success btn-xs" title="Cambiar" onclick="$(\'#cuenDialog\').dialog(\'open\');$(\'#Index\').val(\'' + rowObject.Index + '\');"><i class="glyphicon glyphicon-check"></i></span>';
                ...
                { label: 'Glosa', name: 'Glosa', width: 150, editable: true },
                {
                    ...
                    editoptions: {
                        dataInit: function (element) {
                            gridCompAsien.createInputDiario(element, "D", "Det_Tip");
                        }
                    }
                },
                {
                    ...
                    editoptions: {
                        dataInit: function (element) {
                            gridCompAsien.createInputDiario(element, "H", "Det_Tip");
                        }
                    }
                },
...
function validaGrid() {
    ...
    batch = $.arraySpliceFields(gridCompAsien.getGridBatch(), ['Index', 'act0', 'act1', 'Pld_Des', 'Pld_Cdc']);
    batch = $.jsonParser(batch);
    if (ban === false) { $.alert(msg); gridCompAsien.startGridEdit(); return ban; }
    return batch;
}
function validaComp() {
    if (tipo === 'Ingreso') {
    ...
    var data = $.extend($('#formCompConta').serializeObject(), { Com_Tip: (tipo === 'Ingreso' ? 'I' : (tipo === 'Egreso' ? 'E' : 'D')), saveForm: true, cuentas: batch, Tia_Abr: $('#Tia_Cod_Comp option:selected').data('Tia_Abr') }, $('#periodoForm').serializeObject());
...
function anularComp() {
    var dataSend = { 'anularComp': true, 'Com_Cod': Com_Cod, Com_Fec: $('#Com_Com_Fec').val() };
...
// FUNCION PARA CARGAR AUTOMATICAMENTE EL ASIENTO DE RETENCIONES
function cargar_comp_reten() {
    dataSend = { 'generar_asiento_ret': true };
    $.get("", dataSend, function (response) {
        if (response['success'] === true) {
            document.querySelector('a[href="#tabs-3"]').click();
            document.querySelector('a[href="#tabs-3"]').click();
            $(document).ready(function () {
                $('input[name=proveedor]').val("9999999999999");
                buscaProvee();
            });
            const fecha = new Date();
            const fec_actual = fecha.toISOString().split('T')[0];
            $(document).ready(function () {
                const valorAbr = "LI";
                const $opcion = $('#Tia_Cod_Comp option[data--tia_-abr="' + valorAbr + '"]');
                if ($opcion.length) {
                    $('#Tia_Cod_Comp').val($opcion.val()).trigger('change');
                }
            });
            const meses = ["ENERO", "FEBRERO", "MARZO", "ABRIL", "MAYO", "JUNIO", "JULIO", "AGOSTO", "SEPTIEMBRE", "OCTUBRE", "NOVIEMBRE", "DICIEMBRE"];
            const mesNumero = parseInt(response['compro']['mes']);
            const mesTexto = meses[mesNumero - 1];
            $('input[name=Com_Fec]').val(fec_actual);
            $('textArea[name=Com_Con]').val("PR/ LIQUIDACION DE RETENCIONES " + mesTexto + ' ' + response['compro']['anio']);
            editable = true;
            editing = true;
...
// FUNCION PARA CARGAR AUTOMATICAMENTE EL ASIENTO DE LIQUIDACION IVA FORMULARIO 104
function cargar_datos_formulario() {
    dataSend = { 'ajaxLiquidacionForm104': true };
    $.get("", dataSend, function (response) {
        if (response['success'] === true) {
            document.querySelector('a[href="#tabs-3"]').click();
            document.querySelector('a[href="#tabs-3"]').click();
            ...
            const mesTexto = meses[mesNumero - 1];
            $('input[name=Com_Fec]').val(fec_actual);
            $('textArea[name=Com_Con]').val("PR/ LIQUIDACION DE IVA " + mesTexto + ' ' + response['compro']['anio']);
            editable = true;
            editing = true;            
            console.log(response['diario']);
            gridCompAsien.setRowsByIndex(response['diario'], 'Index');
            gridCompAsien.startGridEdit().updateGridDiario();
            $('#main-panel').moveComp('#modificar-panel').updateGridsSizes();
        } else { $.alert(response['message']); }
    }, 'json').fail(function (error) { $.alert("El Servidor ha fallado en responder!"); });
