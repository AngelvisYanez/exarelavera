...
ORDER BY
  asientos.Asi_Deh";
		...
		case 240:
			$sql = "SELECT det_plan.Pla_Cod,det_plan.Pld_Cod,Pld_Cdc,Pld_Des,Pld_Est,
                    SUM(IF(Asi_Deh='D',ROUND(Asi_Val,2),0)) as debe,
					SUM(IF(Asi_Deh='H',ROUND(Asi_Val,2),0)) as haber, 
                    IF((SUM(IF(Asi_Deh='D',ROUND(Asi_Val,2),0))-SUM(IF(Asi_Deh='H',ROUND(Asi_Val,2),0)))>=0,(SUM(IF(Asi_Deh='D',ROUND(Asi_Val,2),0))-SUM(IF(Asi_Deh='H',ROUND(Asi_Val,2),0))),0) AS deudor,
                    IF((SUM(IF(Asi_Deh='D',ROUND(Asi_Val,2),0))-SUM(IF(Asi_Deh='H',ROUND(Asi_Val,2),0)))<0,ABS(SUM(IF(Asi_Deh='D',ROUND(Asi_Val,2),0))-SUM(IF(Asi_Deh='H',ROUND(Asi_Val,2),0))),0) AS acreedor
                    FROM asientos  
                    INNER JOIN det_plan ON asientos.Pld_Cod = det_plan.Pld_Cod                    
                    LEFT JOIN comprobantes ON comprobantes.Com_Cod = asientos.Com_Cod
                    WHERE  
                        comprobantes.Com_Est = 'A'	 		  
                        AND Pec_Cod = '$Par_Sql[0]'  $Par_Sql[3]
                        AND comprobantes.Com_Fec BETWEEN '$Par_Sql[1] 00:00:00' AND '$Par_Sql[2] 23:59:59'
                        GROUP BY det_plan.Pld_Cod  ORDER BY 
