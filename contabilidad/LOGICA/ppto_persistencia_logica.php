 ...
 * Capa de Persistencia y Lógica de Datos para el Módulo de Presupuestos (EXA PPTO).
 ...
 * @param array $p Parámetros requeridos para la consulta estructurados en un arreglo asociativo.
 * @return mixed Retorna ID insertado, valor escalar, array de resultados o null/false según el caso.
 */
function ppto_persistencia_consultar($mysqli, $caso, $p) {
    switch ($caso) {
        case 1:
            // Obtener el ID de la cabecera del presupuesto activo para una empresa y año específicos.
        ...
        case 2:
            // Obtener las reglas de asignación automáticas activas para un tipo de documento y empresa.
            $Emp_Cod = $mysqli->real_escape_string($p['Emp_Cod']);
        ...
        case 3:
            // Verificar si ya existe un registro de ejecución idéntico (evitar duplicados transaccionales).
        ...
        case 4:
            // Insertar una nueva transacción en el ledger de ejecución presupuestaria.
            $ppe_id = (int)$p['ppe_id'];
            $ppa_id = (int)$p['ppa_id'];
        ...
        case 6:
            // Calcular la ejecución acumulada de una partida hasta un mes del año activo.
            $ppe_id = (int)$p['ppe_id'];
            $ppa_id = (int)$p['ppa_id'];
        ...
        case 7:
            // Registrar una alerta presupuestaria por desvío/superación de umbrales.
            $ppe_id = (int)$p['ppe_id'];
            $ppa_id = (int)$p['ppa_id'];
        ...
        case 8:
            ...
            $sql = "SELECT 
                        ...
                        p.Ppa_Tip AS ppa_tipo, 
                        p.Ppa_Nat AS ppa_naturaleza,
                        COALESCE(NULLIF(p.Ppa_Clase, ''), 'D') AS ppa_clase,
                        p.Ppa_Niv AS ppa_nivel,
        ...
        case 9:
            // Obtener alertas presupuestarias activas (no leídas) de la versión activa de presupuesto.
            $Emp_Cod = (int)$p['Emp_Cod'];
            $ppe_id = (int)$p['ppe_id'];
        ...
        case 11:
            // Consultar la distribución nominal por cada uno de los 12 meses de una partida específica.
            $ppe_id = (int)$p['ppe_id'];
            $ppa_id = (int)$p['ppa_id'];
            $sql = "SELECT m.Mes AS pde_mes, IFNULL(d.Pde_Mon, 0.00) AS pde_monto, d.Pde_Cod AS pde_id
                    FROM (
        ...
        case 12:
            // Consultar el desglose de documentos que componen la ejecución real de una partida en particular.
            $ppe_id = (int)$p['ppe_id'];
            $ppa_id = (int)$p['ppa_id'];
            $mes_cond = "";
...
/**
 * Totales consolidados de lectura para tab Métricas Admin.
 *
 * @param mysqli $mysqli
 * @param int $Emp_Cod
 * @param int $ppe_id
