...
require_once("con_sql_compr.php");
	...
	*/
	...
	?>
		<table width="80%" border="0" cellpadding="0" cellspacing="0">
		  <tr align="center">
		    <td width="5%" rowspan="5" valign="top"><img src="<?php echo $row_institucion['Emp_Log']; ?>" width="50" height="50" /></td>
...
<?php
	...
	function codigoComprAuto2($Tia_Cod, $Pec_Cod, $mes, $obBD_conexion)
  	{			
		/* 
		* Codificación numerica en base al periodo contable y mensualmente 
		*/ 
	...
	/**
	 * Codigo de los comprobantes
	 * @param int $Tia_Ini Tipo de comprobante
	 * @param int $Pec_Cod periodo contable
     * @param int $mes mes
	 */		
	function codigoComprAutomatic($Tia_Cod, $Pec_Cod, $mes, $obBD_conexion)
  	{			
		...
		* Codificación numerica en base al periodo contable y mensualmente 
		*/
		$row_rs_numcom = $this->getRowConsulta(153, $Tia_Cod.'*'.$Pec_Cod.'*'.$mes, $obBD_conexion);
		// Revisar la condición (todo funciona correctamente pero con artificio)
		if ((count($row_rs_numcom) > 0) && ($row_rs_numcom['Com_Num'] != ''))
		{
			$Com_Num=$row_rs_numcom['Com_Num'];
