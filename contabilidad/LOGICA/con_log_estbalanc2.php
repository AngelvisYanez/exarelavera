...
require_once("con_sql_estbalances.php");
...
function getCtaChild($cuentas,$Pld_Rec){ 
    ...
    $plan=array();
    $target = isset($index) ? (isset($index[$Pld_Rec]) ? $index[$Pld_Rec] : array()) : array();
...
/* Helper interno para evitar re-indexar en cada nivel */
function getCtaChildInternal($index, $Pld_Rec) {
    $plan = array();
    if (isset($index[$Pld_Rec])) {
        foreach ($index[$Pld_Rec] as $value) {
            $plan[] = $value;
            $plan = array_merge($plan, $this->getCtaChildInternal($index, $value['Pld_Cod']));
        }
    }
...
function formatCuentaDiario($cuenta,$cant_cuent,$total,$format){ 
 ...
 function cargarNodosBalance($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format, $utilidadSiNo, $Pec_Cod2_completo = null, $Pec_Fei_periodo = null, $Pec_Fef_periodo = null, $sucursal = ''){
    ...
    /* consulto los valores del mayor (con filtro opcional de sucursal via SQL 240 $Par_Sql[3]) */
    ...
    foreach ($valores_raw as $v) {
        ...
        $current_cdc = "";
        for ($i = 0; $i < count($cdc_parts); $i++) {
            $current_cdc .= ($i == 0 ? "" : ".") . $cdc_parts[$i];
            if (!isset($valoresMap[$current_cdc])) {
                $valoresMap[$current_cdc] = array('debe' => 0, 'haber' => 0, 'saldo' => 0, 'asignar' => false);
            }
            $valoresMap[$current_cdc]['debe'] += (float)$v['debe'];
            $valoresMap[$current_cdc]['haber'] += (float)$v['haber'];
        }
    }
    /* Calcular saldos finales y flag asignar */
    foreach ($valoresMap as $cdc => &$total_cdc) {
        $total_cdc['saldo'] = $total_cdc['debe'] - $total_cdc['haber'];
        if (abs($total_cdc['saldo']) > 0.009) { $total_cdc['asignar'] = true; }
    }
    
    /* Wrapper para compatibilidad con getTotalNodo optimizado */
    $valores = array('__optimized_map' => $valoresMap, 'raw' => $valores_raw);
    
    /* calculo de utilidades - AHORA USA EL MAPA (Optimizado) */
    $utilidades=$this->cargarTotalEstados($cod, $np, $ini, $fin, $obBD_conexion, 2, $sucursal, $valoresMap);
	//
...
function cargarNodosBalance1($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format) {
		...
		if (count($rs_nodosrep) > 0) {
				...
				if ($tipo_grupo[0]==3 && $cod_util != "") {
					...
					*/
					...
					} else {//Fin del if ($asignar == 'si'  || $row_rs_nodosrep[$j]['Pld_Cdc']==3)
						/**
  ...
  function calculoBalance($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $total, $grupo, $valoresMap = null, $cdc = null)
  {
	$debe=0; $haber=0;
    ...
    if ($valoresMap !== null) {
        ...
        if ($cuenta_cdc === null) {
            ...
            $cuenta_cdc = is_array($row_cta) ? $row_cta['Pld_Cdc'] : null;
        }
        
        if ($cuenta_cdc !== null && isset($valoresMap[$cuenta_cdc])) {
            $v = $valoresMap[$cuenta_cdc];
            if ($grupo == 2 || $grupo == 3 || $grupo == 4) {
               $saldos = (float)$v['haber'] - (float)$v['debe'];
            } else {
               $saldos = (float)$v['debe'] - (float)$v['haber'];
            }
            return $total + $saldos;
        }
        return $total;
  ...
  function cargarTotalEstados($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $sucursal = '', $valoresMap = null)
  {
		$g = 0;
		if ($np == 0)
		{			
			...
			{	
				...
				*/
				$total=$this->calculoBalance($cod,$row_rs_nodosrep['Pld_Cod'], $ini, $fin, 
									$obBD_conexion, $row_rs_nodosrep['Pld_Tip'], 0, $tipo_grupo[0], $valoresMap, $row_rs_nodosrep['Pld_Cdc']); 		
				...
				* Recursividad del cargado de los nodos 
				*/
				$this->cargarTotalEstados($cod,$row_rs_nodosrep['Pld_Cod'], $ini, $fin, $obBD_conexion, $tipo, $sucursal, $valoresMap);				
			} 
		}//if ($total_rs_nodosrep > 0)			
