...
class Class_Log_Datos_Con extends MysqlDatos{
	...
	/* Variables para la optimizaciÃ³n de balances */
	private $lastValoresKey = null;
	private $valTreeCache = array();

	/**
	 * Construye un mapa jerÃ¡rquico de totales para optimizar la bÃºsqueda de saldos
	 */
	private function getValTree($valores) {
		$valTree = array();
		if (!is_array($valores)) return $valTree;
		foreach ($valores as $v) {
			$cdc = $v['Pld_Cdc'];
			$debe = (float)$v['debe'];
			$haber = (float)$v['haber'];
			
			// Registro directo para tipo de cuenta 'D' (detalle)
			if (!isset($valTree[$cdc])) {
				$valTree[$cdc] = array(
					'debe' => 0, 'haber' => 0, 'saldo' => 0, 'asignar' => false,
					'direct_debe' => 0, 'direct_haber' => 0, 'direct_saldo' => 0, 'direct_asignar' => false
				);
			}
			$valTree[$cdc]['direct_debe'] = $debe;
			$valTree[$cdc]['direct_haber'] = $haber;
			$valTree[$cdc]['direct_saldo'] = $debe - $haber;
			$valTree[$cdc]['direct_asignar'] = (abs($valTree[$cdc]['direct_saldo']) > 0.009);

			// AcumulaciÃ³n jerÃ¡rquica para tipo de cuenta 'G' (grupo)
			$parts = explode('.', $cdc);
			$current = '';
			foreach ($parts as $i => $part) {
				$current .= ($i === 0 ? '' : '.') . $part;
				if (!isset($valTree[$current])) {
					$valTree[$current] = array(
						'debe' => 0, 'haber' => 0, 'saldo' => 0, 'asignar' => false,
						'direct_debe' => 0, 'direct_haber' => 0, 'direct_saldo' => 0, 'direct_asignar' => false
					);
				}
				$valTree[$current]['debe'] += $debe;
				$valTree[$current]['haber'] += $haber;
				$valTree[$current]['saldo'] += ($debe - $haber);
				if (abs($valTree[$current]['saldo']) > 0.009) {
					$valTree[$current]['asignar'] = true;
				}
			}
		}
		return $valTree;
...
function getTotalNodo($cuenta,$valores,$tipo){ 
    /* OptimizaciÃ³n: Usar el mapa jerÃ¡rquico pre-calculado */
    $key = is_array($valores) ? count($valores) . (isset($valores[0]['Pld_Cdc']) ? $valores[0]['Pld_Cdc'] : '') : '';
    if ($this->lastValoresKey !== $key) {
        $this->lastValoresKey = $key;
        $this->valTreeCache = $this->getValTree($valores);
    }
    
    if (isset($this->valTreeCache[$cuenta])) {
        $v = $this->valTreeCache[$cuenta];
        if ($tipo == 'D') {
            return array(
                'debe' => $v['direct_debe'], 
                'haber' => $v['direct_haber'], 
                'saldo' => $v['direct_saldo'], 
                'asignar' => $v['direct_asignar']
            );
        } else {
            return array(
                'debe' => $v['debe'], 
    ...
    return array('debe'=>0,'haber'=>0,'saldo'=>0,'asignar'=>false);
}
...
function formatCuenta($cuenta,$cant_cuent,$total,$format){ 
    ...
    $cant_esp = 0;                 
    for ($i=1; $i<=count($cant_cuent)-1;$i++) {$cant_esp = $cant_esp + 2 ;}
...
function formatCuentaDiario($cuenta,$cant_cuent,$total,$format){ 
 ...
 function cargarNodosBalance($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format, $utilidadSiNo, $Pec_Cod2_completo = null, $Pec_Fei_periodo = null, $Pec_Fef_periodo = null){
    ...
    /* OptimizaciÃ³n: Pasar Pec_Cod a cargarTotalEstados para evitar consultas recursivas */
    $utilidades=$this->cargarTotalEstados($cod, $np, $ini, $fin, $obBD_conexion, 2, $Pec_Cod, $valores);
	...
	if($tipo==1){  // Aqui cargo la cuenta utilidad o perdida solo para balance general
		...
		for($i=0;$i<count($valores);$i++) 
			if($valores[$i]['Pld_Cod']==$utilidad['Pld_Cod']){									
				...
				$valores[$i]['debe']=$valores[$i]['debe']-$utilidades[1];$valores[$i]['haber']=$valores[$i]['haber']-($utilidades[2] + $utilidades[3] + $utilidades[4]);				
		...
		if($shouldAdd){	
			$utilidad['debe']=-$utilidades[1];$utilidad['haber']=-($utilidades[2] + $utilidades[3] + $utilidades[4]);//$utilidad['saldo']=$util*-1;
			array_push($valores,$utilidad);echo '<br>';
		}
    }
    ...
    for($i=0;$i<count($plan);$i++){
        ...
        if($total['asignar']&&count($cant_cuent)<=$max_nivel){
	        if (startsWith($plan[$i]['Pld_Cdc'],$ctaUtilidad) and $utilidadSiNo == "si"){ //Si la cuenta empieza con el secuencial de la cuenta participacion 
	        	$cuenta2=$this->formatCuenta($plan[$i], $cant_cuent, $total,$format);                 
	            $cuenta2['cant_cuent']=$cant_cuent;
	            $cuenta2['total']=$total;
	            if($bandera){
	            	$totalUtilidadesImpuestos = $total; //Tomo el total del grupo de la cuenta participacion
	            	$bandera =false;
	            }
	            array_push($balance2, $cuenta2);               
        	}
        	else{
    ...
    <?php       
            ...
            <tr>				  
                ...
                <?php
                	if(str_replace(array('<b>','</b>'), '',$cuenta['Pld_Cdc']) == $grupoUtilidad and $utilidadSiNo == 'si'){
						...
						$cuenta['Total'] = '<b>' . formato_numero($cuenta['Total'],2, $format) . '</b>';
					}
                    $tdTotal=''; $tdLast='';
                    for($j=1;$j<=$max_nivel;$j++){
                        if(count($cuenta['cant_cuent'])==$j){
                            ...
                            $tdTotal='<td></td>'.$tdTotal;  if($last) $tdLast='<td></td>'.$tdLast;
                        }
                    }
                    echo $tdTotal;
                ?>                              
            ...
            if(!isset($balance[($i+1)]) || $cuenta['cant_cuent'][0]!=$balance[($i+1)]['cant_cuent'][0]){  
                foreach ($grupos AS $raiz){
                    if($raiz['Pld_Cdc']==$cuenta['cant_cuent'][0]){
                        ...
                        if($raiz['Pld_Cdc'] == $grupoUtilidad and $utilidadSiNo == 'si'){
                        	$totalGrupo = formato_numero(($raiz['total']['debe']-$raiz['total']['haber']-$totalUtilidadesImpuestos['saldo']), 2, $format);
                        }
        ...
        switch($tipo) {
            ...
            case 2: 
               ...
               if($utilidadSiNo == "si"){
               	echo '<tr><td colspan="2" height="40" valign="top"><h3 style="margin-top:6px"><u><strong>TOTAL COSTOS + GASTOS =</strong></u></h3></td><td colspan="'.($max_nivel-1).'"></td><td style="border-top: 1px solid #000" align="right" valign="top"><h3 style="margin-top:6px">'.formato_numero($totales - $totalUtilidadesImpuestos['saldo'], 2, $format).'</h3></td></tr>';
               		if($util>0)
                   echo '<tr><td colspan="2" height="40" valign="top"><h2 style="margin-top:6px"><u><strong>UTILIDAD DEL EJERCICIO ANTES PARTICIPACION E IMPUESTOS =</strong></u></h2></td><td colspan="'.($max_nivel-1).'"></td><td align="right" valign="top"><h2 style="margin-top:6px">'.formato_numero($util + $totalUtilidadesImpuestos['saldo'], 2, $format).'</h2></td></tr>';
               		else
                   echo '<tr><td colspan="2" height="40" valign="top"><h2 style="margin-top:6px"><u><strong>PERDIDAD DEL EJERCICIO ANTES PARTICIPACION E IMPUESTOS=</strong></u></h2></td><td colspan="'.($max_nivel-1).'"></td><td style="border-top: 2px solid #000" align="right" valign="top"><h2 style="margin-top:6px;color:#FF0000">'.formato_numero($util + $totalUtilidadesImpuestos['saldo'], 2, $format).'</h2></td></tr>';
               } else{
        ...
        /* Aqui Armo la tabla de cuentas PARA LAS CUENTAS DE PARTICPACION E IMPUESTOS*/
       	 if($utilidadSiNo == "si"){
            for($i=0;$i<count($balance2);$i++){
                $cuenta2=$balance2[$i];
                $last=((!isset($balance2[($i+1)]) && $cuenta2['Pld_Tip']=='D' )|| isset($balance2[($i+1)]) && $cuenta2['Pld_Tip']=='D' && $balance2[($i+1)]['Pld_Tip']=='G');
                // Solo hacer clicables las cuentas de detalle (no los grupos)
                ...
                if ($esDetalle2 && $Pec_Cod2_completo !== null) {
                 ...
                 ?>        
            <tr>				  
                <td width="2%"><?php echo $codigoCuentaHtml2; ?></td>
                <td><?php echo $cuenta2['Pld_Des']; ?></td>                
                <?php
                    $tdTotal=''; $tdLast='';
                    for($j=1;$j<=$max_nivel;$j++){
                        if(count($cuenta2['cant_cuent'])==$j){
                            $tdTotal='<td align="right">'.$cuenta2['Total'].'</td>'.$tdTotal;
                            if($last) $tdLast='<td align="right" style="border-top: 1px solid #000">&nbsp;</td>'.$tdLast;
                        }else{
                            $tdTotal='<td></td>'.$tdTotal;  if($last) $tdLast='<td></td>'.$tdLast;
                        }
            ...
            <?php 
            /* Presenta los Totales de los grupos */
            if(!isset($balance2[($i+1)]) || $cuenta2['cant_cuent'][0]!=$balance2[($i+1)]['cant_cuent'][0]){  
                foreach ($grupos AS $raiz){
                    if($raiz['Pld_Cdc']==$cuenta2['cant_cuent'][0]){
                        $totalGrupo=formato_numero(($raiz['total']['debe']-$raiz['total']['haber']), 2, $format);
                        ...
                        if($raiz['total']['saldo']<0) $totalGrupo='<font color="#FF0000" >'. formato_numero($totalUtilidadesImpuestos['saldo'],2,$format) .'</font>';
                        echo '<tr><td colspan="2" height="40" valign="top"><h3 style="margin-top:2px"><u><strong>TOTAL PARTICIPACION, IMPUESTOS Y OTROS </strong></u></h3></td><td colspan="'.($max_nivel-1).'"></td><td style="border-top: 1px solid #000" align="right" valign="top"><h3 style="margin-top:2px">'. formato_numero($totalUtilidadesImpuestos['saldo'],2,$format) .'</h3></td></tr>';
                    }
                }    
            }
         }
         ...
         /* TOTALES DESPUES DE PARTICIPACION E IMPUESTOS. */
         switch($tipo)
			{
	            case 1: 
	                /* Sumatoria del Balance General */
	                $totales=0;
	                foreach ($grupos2 AS $raiz){
	                   if($raiz['Pld_Cdc']=='2'||$raiz['Pld_Cdc']=='3') {
	                        $totales=$totales+$raiz['total']['saldo'];
	                   }
	               } 
...
function cargarDiarioDesdeBalance($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format)
 {
    ...
    $util = $utilidades[1] - ($utilidades[2] + $utilidades[3]);
			...
			if($tipo==1){  // Aqui cargo la cuenta utilidad o perdida solo para balance general
				...
				if($shouldAdd){	
					$utilidad['debe']=-$utilidades[1];$utilidad['haber']=-($utilidades[2] + $utilidades[3]);//$utilidad['saldo']=$util*-1;
		...
		if (count($rs_nodosrep) > 0)		
			{
				 ...
				 */
				 ...
				 {					 
					...
					*/
					...
					{						
						$GLOBALS['asignar'] = $this->secuenciaCuenta($cod, $row_rs_nodosrep[$j]['Pld_Cod'], $obBD_conexion, $tipo, 
								$cod_util);
					}
					/**
					...
					*/					
					...
					{						
						$total=$this->calculoBalance($cod,$row_rs_nodosrep[$j]['Pld_Cod'], $ini, $fin, $obBD_conexion, 
											$row_rs_nodosrep[$j]['Pld_Tip'], 0, $tipo_grupo[0])+$util;
					...
					else
					{
						...
						*/
						$total=$this->calculoBalance($cod,$row_rs_nodosrep[$j]['Pld_Cod'], $ini, $fin, $obBD_conexion, 
											$row_rs_nodosrep[$j]['Pld_Tip'], 0, $tipo_grupo[0]);
					}
				...
				*/
				...
				<?Php
					...
					*/
					...
					{
					?>
						<td align="right" width="<?Php if ($valor_grupo >0){ echo "7"; }else{ echo "0"; }  ?>%"><?Php
					...
					<?Php
						...
						*/						
						$this->cargarNodosBalance($cod,$row_rs_nodosrep[$j]['Pld_Cod'], $ini, $fin, $obBD_conexion, 
											$tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format);
					?>
				</tr>
				<?Php				
				}//Fin if (($total != 0))
				...
				*/
				...
				<tr>
				  ...
				  {
				  ?>
					  <td align="right" width="<?Php if ($valor_grupo >0){ echo "7"; }else{ echo "0"; }  ?>%">&nbsp;</td>
				  <?Php
				...
				?>				
				<tr>
				  <td colspan="<?Php echo 4+$max_nivel; ?>" align="left"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      <td height="57">
                      <h3><u><strong>TOTAL &nbsp;<?Php echo $row_rs_nodosrep[$j]['Pld_Des']; ?></strong></u></h3></td>
                      <td align="right" valign="top">
					     <h3><font <?Php 					
									if ($grupo[$g] < 0){ echo "color='#FF0000'"; } 
										else { echo "class='TITULO_REPORTE'"; }?>><strong>__________<br />
			...
			<tr>
			  ...
			  */
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td><table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                      <td width="82%" class="TITULO_REPORTE"><div align="left"><h3>TOTAL PASIVO + PATRIMONIO = </h3></div></td>
                      <td width="18%" align="right"><font <?Php 					
									if ($grupo[2] + $patrimonio < 0){ echo "color='#FF0000'"; } 
										else { echo "class='TITULO_REPORTE'"; }?>><h2>__________<br /><?Php echo formato_numero($grupo[2] + $patrimonio, 2, $format); ?></h2></font></td>
                    </tr>
                    <?Php
				if ($cod_util == "")
				{
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="left">
                ...
                <tr>
                  <td class="TITULO_REPORTE"><h3><div align="left">
                   TOTAL DE COSTOS Y GASTOS =
                  </div></h3>                  
                  </td>
                  <td width="10%" class="" align="right"><h3><font <?Php 	
			                                 $egresos = $grupo[2]+$grupo[3];
								if ($egresos < 0){ echo "color='#FF0000'"; } 
								else { echo "class=''"; }?>><strong><?Php 
				  					
								echo formato_numero($egresos, 2, $format); ?></strong></font></h3>
				  </td>
                </tr>                
                <tr>
                  <td class="TITULO_REPORTE"><h3><div align="left">
                  ...
                  <td width="10%" class="" align="right"><h3><font <?Php 					
								if ($util < 0){ echo "color='#FF0000'"; } 
								else { echo "class=''"; }?>><strong><?Php echo formato_numero($util, 2, $format); ?></strong></font></h3>
				  <?Php //echo number_format($util,2); ?></td>
                </tr>
                <!--<tr>
                  <td class="TITULO_REPORTE"><div align="left">( - ) SUPERAVIT DE CAPITAL</div></td>
                  <td class="LetraNegra" align="right"><strong><?Php //$aporte=($util * 15)/100; echo formato_numero($aporte, 2, $format); ?></strong><br />__________</td>
                </tr>
                <tr>
                  <td class="TITULO_REPORTE"><div align="left">UTILIDADES DEL EJERCICIO = </div></td>
                  <td class="LetraNegra" align="right"><strong><?Php //echo formato_numero($util - $aporte, 2, $format); ?></strong></td>
                </tr>-->
              </table>			  
			  <?php
			  break;
   ...
   */
  function calculoBalance($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $total, $grupo)
  {
	$debe=0; $haber=0; //$i=$i+1;
	/**
	...
	*/ 
	...
	if (count($rs_nodosrep) > 0)
	{
		...
		{
			$total = $this->calculoBalance($cod, $row_rs_nodosrep['Pld_Cod'], $ini, $fin, $obBD_conexion, $tipo, $total, $grupo);
		}
	}	
	return $total;	
  }

  function cargarTotalEstados($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod = null, $valoresPrecalculados = null)
  {
		if ($np == 0 && ($Pec_Cod !== null || $valoresPrecalculados !== null))
		{			
			/* OPTIMIZACIÃ“N: Cargar balances una sola vez y usar el Ã¡rbol de valores */
			$rs_nodosrep = $this->getArrayConsulta(337, $cod.'*'.$np.'*'.$tipo, $obBD_conexion);
			
			// Si no tenemos los valores, los consultamos (evitar duplicados si ya vienen de fuera)
			$valores = $valoresPrecalculados;
			if ($valores === null) {
				$valores = $this->getArrayConsulta(240, $Pec_Cod.'*'.$ini.'*'.$fin, $obBD_conexion);
			}
			
			$valTree = $this->getValTree($valores);
			
			$grupo = array();
			$g = 0;
			foreach($rs_nodosrep as $row)
			{
				$cdc = $row['Pld_Cdc'];
				$v = isset($valTree[$cdc]) ? $valTree[$cdc] : array('saldo' => 0);
				
				$rootGroup = (int)$cdc[0];
				$total = $v['saldo'];
				
				// Ajuste de signo segÃºn el grupo
				if ($rootGroup == 2 || $rootGroup == 3 || $rootGroup == 4) {
					$total = -$total;
				}
		...
		if (count($rs_nodosrep) > 0)		
		{
			...
			{	
				...
				$total=$this->calculoBalance($cod,$row_rs_nodosrep['Pld_Cod'], $ini, $fin, 
									$obBD_conexion, $row_rs_nodosrep['Pld_Tip'], 0, $tipo_grupo[0]); 		
				if ($np == 0)
				{
					$g = isset($g) ? $g + 1 : 1;								
					$grupo[$g]=$total;
				}
				$this->cargarTotalEstados($cod,$row_rs_nodosrep['Pld_Cod'], $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod);				
			} 
		}			
			
		if ($np == 0)
		{	
			return isset($grupo) ? $grupo : array();
		}
  ...
  */
    ...
    function cargarNodosResumen($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format,$sql)
  {
    ...
    $debe=0;$haber=0;$deudor=0;$acreedor=0;$html="";
    foreach ($rs_ver as $fila){
        $deudor=$deudor+round($fila['deudor'],2);$acreedor=$acreedor+round($fila['acreedor'],2);
        $debe=$debe+round($fila['debe'],2);$haber=$haber+round($fila['haber'],2);
        $html=$html."<tr>
            <td style='font-size: 10px;'>$fila[Pld_Cdc]&nbsp;&nbsp;&nbsp;$fila[Pld_Des]</td>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="1" class="Texto_Reporte">       
        ...
        <?php echo $html; ?>
        <tr>
            <td colspan="3"><div align="left"><h3>TOTALES</h3></div></td>
            
            <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($deudor,2,$format); ?></strong></h3></td>
            <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($acreedor,2,$format); ?></strong></h3></td>
        </tr>
    </table>
    <?php
  }
  function cargarNodosComprobacion($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format)
  {
    $rs_ver = $this->getArrayConsulta(240, $Pec_Cod.'*'.$ini.'*'.$fin, $obBD_conexion);
    $debe=0;$haber=0;$deudor=0;$acreedor=0;$html="";
    foreach ($rs_ver as $fila){
        $deudor=$deudor+round($fila['deudor'],2);$acreedor=$acreedor+round($fila['acreedor'],2);
        $debe=$debe+round($fila['debe'],2);$haber=$haber+round($fila['haber'],2);
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="1" class="Texto_Reporte">
        ...
        <?php echo $html; ?>
        <tr>
            <td><div align="left"><h3>TOTALES</h3></div></td>
            <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($debe,2,$format); ?></strong></h3></td>
            <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($haber,2,$format); ?></strong></h3></td>
            <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($deudor,2,$format); ?></strong></h3></td>
            <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($acreedor,2,$format); ?></strong></h3></td>
        </tr>
    </table>
			...
			foreach($rs_nodosrep as $row_rs_nodosrep[$j])
			{
				...
				*/
				...
				{					 
					...
					*/
					/**************************************/				 
					$asignar = $this->secuenciaCuenta($cod, $row_rs_nodosrep[$j]['Pld_Cod'], $obBD_conexion, $tipo, 
							$cod_util);
					/**
					* Calculo de los totales 3=Patrimonio
					*/
					...
					{											 					 
						...
						* Calculo de los totales 
						*/
						$total_cadena= $this->calculoComprobacion($cod,$row_rs_nodosrep[$j]['Pld_Cod'], $ini, $fin, 
									$obBD_conexion, $row_rs_nodosrep[$j]['Pld_Tip'], 0, 0); 				 					
				...
				* Es muy importante poner este explode antes de movimiento del apuntador de la consulta 
				 ...
				 */				   
				   if ($saldos[0] == 0){ $debe = "&nbsp;"; }
				   else { $debe = formato_numero($saldos[0],2,$format); }
	
				   if ($saldos[1] == 0){ $haber = "&nbsp;"; }
				   else { $haber = formato_numero($saldos[1],2, $format); }
					...
					*/
                                        $saldo_deudor = 0;$saldo_acreedor=0;$deudor = 0;$acreedor = 0;	
                                        if($total>=0)	{	
                                            $total=abs($total);
...
//				}
				...
				<?Php
					...
					*/
					...
					?></td>
					<td align="right">
								<font <?Php if ($saldos[0] < 0){ echo "color='#FF0000'"; } else { echo "class='LetraNegra'"; }?>>
								<?Php	if ($row_rs_nodosrep[$j]['Pld_Tip']=='D')
										{
											echo $debe;
										}
					...
					<td align="right">
								<font <?Php if ($saldos[1] < 0){ echo "color='#FF0000'"; } else { echo "class='LetraNegra'"; }?>>	
								<?Php if ($row_rs_nodosrep[$j]['Pld_Tip']=='D')
									  {								
										echo $haber;
									  }
					...
					<td align="right">
					  <font <?Php  if ($total < 0){ echo "color='#FF0000'"; } else { echo "class='LetraNegra'"; } ?>>
					  
		  			  <?Php if ($saldo_deudor == 0){ $saldo_deudor = "&nbsp;"; }
					  			if ($row_rs_nodosrep[$j]['Pld_Tip']=='D')
									  {	
										  echo $saldo_deudor; 
									  }
					...
					<td align="right"><font <?Php  if ($total < 0){ echo "color='#FF0000'"; } else { echo "class='LetraNegra'"; } ?>>
					  <?Php 	if ($saldo_acreedor == 0){ $saldo_acreedor = "&nbsp;"; }
				...
				<?Php
						...
						* Recursividad del cargado de los nodos 
						*/
						$this->cargarNodosComprobacion($cod,$row_rs_nodosrep[$j]['Pld_Cod'], $ini, $fin, $obBD_conexion, 
											$tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format);
					}//Fin if (($total != 0))				
				}//Fin del foreach($rs_nodosrep as $row_rs_nodosrep[$j])
			}//if ($total_rs_nodosrep > 0)			
		...
		<tr>
              ...
              <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($grupo_d[1]+$grupo_d[2]+$grupo_d[3]+$grupo_d[4]+$grupo_d[5]+$grupo_d[6],2,$format); ?></strong></h3></td>
              <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($grupo_h[1]+$grupo_h[2]+$grupo_h[3]+$grupo_h[4]+$grupo_h[5]+$grupo_h[6],2,$format); ?></strong></h3></td>
              <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($saldos_grupo_d[1] + $saldos_grupo_d[2] + $saldos_grupo_d[3] +  $saldos_grupo_d[4] + $saldos_grupo_d[5]+ $saldos_grupo_d[6],2,$format); ?></strong></h3></td>
              <td align="right"><h3>&nbsp;<strong><?Php echo formato_numero($saldos_grupo_h[1] + $saldos_grupo_h[2] + $saldos_grupo_h[3] +  $saldos_grupo_h[4] + $saldos_grupo_h[5]+ $saldos_grupo_h[6],2,$format); ?></strong></h3></td>
		</tr>
</table>
		<?Php	
		}
  ...
  { 
		...
		{		
			...
			/**
				* Recursividad del cargado de los nodos 
				*/						
				$this->cargarNodosEfectivos($cod,$row_rs_nodosrep[$j]['Pld_Cod'], $ini, $fin, $obBD_conexion, 
									$tipo, $Pec_Cod, $nivel, $max_nivel, $format);
			} //Fin del foreach($rs_nodosrep as $row_rs_nodosrep[$j])
		}//if ($total_rs_nodosrep > 0)
	if ($np == 0)
	{	
  ...
  { 
		...
		{		
			...
			/**
				* Recursividad del cargado de los nodos 
				*/						
				$this->cargarNodosConversion($cod,$row_rs_nodosrep[$j]['Pld_Cod'], $ini, $fin, $obBD_conexion, 
									$tipo, $Pec_Cod, $format, $detalle);
			} //Fin del foreach($rs_nodosrep as $row_rs_nodosrep[$j])
		}//if ($total_rs_nodosrep > 0)
	if ($np == 0)
	{	
