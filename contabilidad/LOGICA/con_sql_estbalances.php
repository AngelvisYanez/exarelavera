...
function sentencias_con($id, $Par_Sql) {
	switch ($id) {
		...
		case 212:
			 ...
			 */
			$saldo_212 = "SELECT asientos.Asi_Deh, sum(round(asientos.Asi_Val,2)) as Asi_Val 
				FROM asientos 
				INNER JOIN comprobantes ON comprobantes.Com_Cod = asientos.Com_Cod
				WHERE comprobantes.Com_Fec BETWEEN '$Par_Sql[0]'
				AND  '$Par_Sql[1]'
  ...
  asientos.Asi_Deh";*/
			$sql = "SELECT 
					...
					WHERE
						...
						asientos.Com_Cod IN (SELECT DISTINCT comprobantes.Com_Cod
											FROM comprobantes
											...
											WHERE comprobantes.Com_Fec BETWEEN '$Par_Sql[0]' AND '$Par_Sql[1]' AND
												comprobantes.Com_Est = 'A' AND asientos.Pld_Cod = '$Par_Sql[2]')
					GROUP BY asientos.Asi_Deh
					ORDER BY asientos.Asi_Deh";
			return $sql;
			break;
		...
		case 215:
			$sql = "SELECT 
					...
					GROUP BY asientos.Asi_Deh, det_plan.Pld_Des
					ORDER BY asientos.Asi_Deh";
			return $sql;
			break;
		case 216:
			$sql = "SELECT 
						persona.Prs_Ape, persona.Prs_Nom
					FROM proveedore
		...
		case 219:
			...
			break;
		case 204:
			$sql = "SELECT Pld_Cdc FROM det_plan WHERE Pld_Cod = $Par_Sql[0]";
			return $sql;
			break;
		...
		case 220:
			...
			return $sql;
			break;
		/* Cargado de la raiz del Plan de Cuentas de cuantas activas */
		case 315:
			...
			return $sql;
			break;
		/* Cargado de la raiz del Plan de Cuentas de cuantas activas en base al tipo de balance financiero */
		case 337:
		...
		case 240:
			...
			$sql = "SELECT 
					...
					INNER JOIN det_plan ON asientos.Pld_Cod = det_plan.Pld_Cod                    
					INNER JOIN comprobantes ON comprobantes.Com_Cod = asientos.Com_Cod
					LEFT JOIN usuarios ON comprobantes.Usu_Cod = usuarios.Usu_Cod
					WHERE  
		...
		case 245: // Consulta de sucursales para el filtro
			$sql = "SELECT Suc_Cod, Suc_Des FROM sucursal WHERE Emp_Cod = $Par_Sql[0] AND Suc_Est = 'A' ORDER BY Suc_Des ASC";
			return $sql;
			break;
		case 246:
			/**
			 * Consulta que realiza el calculo del saldo establecido entre 2 fechas filtrado por sucursal
			 */
			$filtroAdicional = (isset($Par_Sql[3]) && $Par_Sql[3] != '') ? " AND usuarios.Suc_Cod = '$Par_Sql[3]'" : "";
			$sql = "SELECT asientos.Asi_Deh, ROUND(SUM(asientos.Asi_Val), 2) as Asi_Val 
				FROM asientos 
				INNER JOIN comprobantes ON comprobantes.Com_Cod = asientos.Com_Cod
				LEFT JOIN usuarios ON comprobantes.Usu_Cod = usuarios.Usu_Cod
				WHERE comprobantes.Com_Fec BETWEEN '$Par_Sql[0]' AND '$Par_Sql[1]'
					AND comprobantes.Com_Est = 'A'
					AND asientos.Pld_Cod = '$Par_Sql[2]' $filtroAdicional 
		...
		case 247:
			/**
			 * Consulta los valores del mayor filtrado por sucursal con redondeo optimizado
			 */
			$filtroAdicional = (isset($Par_Sql[3]) && $Par_Sql[3] != '') ? " AND " . $Par_Sql[3] : "";
			$sql = "SELECT 
						det_plan.Pla_Cod,
						...
						Pld_Est,
						det_plan.Pld_Tip,
						ROUND(SUM(IF(Asi_Deh='D', Asi_Val, 0)), 2) as debe,
						ROUND(SUM(IF(Asi_Deh='H', Asi_Val, 0)), 2) as haber,
						ROUND(SUM(IF(Asi_Deh='D', Asi_Val, -Asi_Val)), 2) as saldo
					FROM asientos  
					INNER JOIN det_plan ON asientos.Pld_Cod = det_plan.Pld_Cod                    
					INNER JOIN comprobantes ON comprobantes.Com_Cod = asientos.Com_Cod
					LEFT JOIN usuarios ON comprobantes.Usu_Cod = usuarios.Usu_Cod
