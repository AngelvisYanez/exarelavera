...
function sentencias_con($id, $Par_Sql) {
	switch ($id) {
		...
		case 126: /* Consulta la información la ciudada en base a la sucursal */
			$cargar_ciudad = "SELECT empresas.Emp_Nom, Emp_Ruc, ciudad.Ciu_Des, sucursal.Ciu_Cod,
									sucursal.Suc_Dir, sucursal.Suc_Te1, sucursal.Suc_Te2, sucursal.Suc_Fax,
									Emp_Con, Emp_Rco, Emp_Rep, Emp_Rre, sucursal.Suc_Cor, sucursal.Suc_Web,
		...
		case 201: /*  Consulta de la mayorizacion individual */
			switch ($Par_Sql[5]) {
				case 't':
					$Par_Sql[5] = " ";
					break;
			...
			$sql = "SELECT comprobantes.Com_Cod, comprobantes.Com_Num,  tipo_asien.Tia_Ini, 
					tipo_asien.Tia_Abr, comprobantes.Com_Fec, asientos.Asi_Cod, 
					comprobantes.Com_Con, asientos.Asi_Deh, asientos.Asi_Val AS Asi_Val, comprobantes.Com_Obs, 

						-- Obtener el número del cheque correcto
						(SELECT c.Che_Num 
						FROM cheques c 
						WHERE c.Asi_Cod = asientos.Asi_Cod 
						LIMIT 1) AS Che_Num,

					det_plan.Pld_Des, det_plan.Pld_Cdc, 
					det_plan.Pld_Rec, comprobantes.Com_Val, 
					comprobantes.Cli_Cod, comprobantes.Prv_Cod, 
					...
					WHERE comprobantes.Com_Cod = asientos.Com_Cod  AND 
		...
		case 202: /* Consulta que realiza el calculo del saldo anterior a una fecha establecida */
			$saldo_202 = "SELECT asientos.Asi_Deh, sum(asientos.Asi_Val) as Asi_Val
							FROM asientos, comprobantes, det_plan 
							WHERE comprobantes.Com_Cod = asientos.Com_Cod AND
									asientos.Pld_Cod = det_plan.Pld_Cod AND
									comprobantes.Com_Fec <= '$Par_Sql[0]' AND
									comprobantes.Com_Est = 'A' AND
									det_plan.Pld_Cod = '$Par_Sql[1]' AND
									comprobantes.Pec_Cod = $Par_Sql[2] 
		...
		case 204: /* Consulta la descripcion de la recusividad de una sub-cuenta */
			$consul_recur = "SELECT det_plan.Pld_Rec, det_plan.Pld_Cdc, Pld_Des FROM det_plan WHERE det_plan.Pld_Cod = '$Par_Sql[0]'";
			return $consul_recur;
			break;

		case 205: /* Consulta que realiza el calculo del saldo anterior a una fecha establecida, dentro de un mismo periodo */
			$sql = "SELECT asientos.Asi_Deh, sum(asientos.Asi_Val) as Asi_Val
						FROM asientos, comprobantes, det_plan 
						WHERE comprobantes.Com_Cod = asientos.Com_Cod AND
								asientos.Pld_Cod = det_plan.Pld_Cod AND
								comprobantes.Com_Fec <= '$Par_Sql[0]' AND
		...
		case 215:
			$consulta_plan_215 = "SELECT det_plan.Pla_Cod 
									FROM det_plan, plan_cuenta, comprobantes, asientos
									WHERE plan_cuenta.Pla_Cod = det_plan.Pla_Cod AND
											asientos.Pld_Cod = det_plan.Pld_Cod AND
											asientos.Com_Cod = comprobantes.Com_Cod AND
											comprobantes.Pec_Cod = $Par_Sql[0]
									GROUP BY det_plan.Pla_Cod ORDER BY det_plan.Pla_Cod DESC";
			return $consulta_plan_215;
			break;

		/*Consulta el codigo interno del plan de cuentas en base al codigo del plan de cuentas */
		case 216:
			$sql = "SELECT det_plan.Pld_Cod, Pld_Rec,det_plan.Pld_Cdc,det_plan.Pld_Des
						FROM det_plan, plan_cuenta
						WHERE plan_cuenta.Pla_Cod = det_plan.Pla_Cod  AND 
								Pld_Cdc = '$Par_Sql[0]' AND plan_cuenta.Pla_Cod = $Par_Sql[1] 
						GROUP BY det_plan.Pld_Cod, Pld_Rec";
			return $sql;
			break;
		...
		case 219:
			$cargar_per_219 = "SELECT Pec_Cod, Pec_Fei, Pec_Fef, Pec_Est,
										Year(Pec_Fei) as Periodo, perio_cont.Pla_Cod
								FROM perio_cont, plan_cuenta
								WHERE perio_cont.Pla_Cod = plan_cuenta.Pla_Cod AND
		...
		case 312:
			$sql = "SELECT det_plan.Pld_Cod, det_plan.Pld_Cdc, det_plan.Pld_Des,
							empresas.Emp_Nom, Pla_Obs, IF (Pld_Tip='G', 'Grupo', 'Detalle') as Pld_Tip,
							IF (Pld_Est='A', 'Activa', 'Inactiva') as Pld_Est, Pla_Obs, Pld_Rec
					FROM det_plan, plan_cuenta, empresas
					WHERE det_plan.Pla_Cod=plan_cuenta.Pla_Cod AND 
							plan_cuenta.Emp_Cod=empresas.Emp_Cod AND
							empresas.Emp_Cod=$Par_Sql[1] AND
							det_plan.Pld_Des LIKE '%$Par_Sql[0]%' AND
							plan_cuenta.Pla_Cod = $Par_Sql[3] $Par_Sql[2]
					Order by Pld_Cod";
			return $sql;
			break;
		...
		case 313:
			$bus_ctac =
				...
				"SELECT det_plan.Pld_Cod, det_plan.Pld_Cdc, 
						det_plan.Pld_Des, empresas.Emp_Nom, 
						Pla_Obs, 
				...
				WHERE empresas.Emp_Cod = $Par_Sql[1] AND
						REPLACE(det_plan.Pld_Cdc, '.', '') = REPLACE(TRIM('$Par_Sql[0]'), '.', '')
						AND plan_cuenta.Pla_Cod = $Par_Sql[3] $Par_Sql[2]";
			return $bus_ctac;
			break;
		...
		case 314:
			$sql = "SELECT det_plan.Pld_Cod, det_plan.Pld_Cdc, det_plan.Pld_Des,
						det_plan1.Pld_Des AS Pld_Des_Grupo, det_plan1.Pld_Cdc AS Pld_Cdc_Grupo
					FROM det_plan
		...
		case 338:
			$cargar_cuentas_338 = "SELECT comprobantes.Com_Val, Com_Con, Com_Obs, asientos.Asi_Cod, 
											asientos.Pld_Cod, det_plan.Pld_Cdc, det_plan.Pld_Des,
											asientos.Asi_Glo, Asi_Deh, ROUND(Asi_Val,2) as Asi_Val,
											Asi_Val AS Asi_Val2
									FROM asientos, det_plan, comprobantes
									WHERE asientos.Com_Cod=$Par_Sql[0] AND
									asientos.Pld_Cod=det_plan.Pld_Cod AND
									comprobantes.Com_Cod = asientos.Com_Cod";
			return $cargar_cuentas_338;
			break;

		case 339:
			$sql = "SELECT Pec_Cod,Year(Pec_Fei) AS Periodo,Pec_Fei,Pec_Fef,
							Ban_Cod,banco.Pld_Cod,Pld_Cdc,Pld_Des,Ban_Cue
					FROM banco
		...
		case 399:
			$sql = "SELECT Pec_Cod,Year(Pec_Fei) AS Periodo,Pec_Fei,Pec_Fef,
					...
					FROM banco
						...
						INNER JOIN plan_cuenta ON det_plan.Pla_Cod=plan_cuenta.Pla_Cod
						INNER JOIN perio_cont ON perio_cont.Pla_Cod=plan_cuenta.Pla_Cod
					WHERE Ban_Est='A' AND Ban_Tip='B' AND 
							plan_cuenta.Emp_Cod=$Par_Sql[0] AND
							Pec_Cod=$Par_Sql[1];";
			return $sql;

		case 340:
			$cargar_nodosrep = "SELECT det_plan.Pld_Cdc, Pld_Rec, Pld_Cod, Pld_Des
								FROM det_plan
								WHERE det_plan.Pld_Cdc LIKE '$Par_Sql[0]%' AND
										Pla_Cod='$Par_Sql[1]' AND Pld_Tip='D'
								ORDER BY det_plan.Pld_Cdc";
			return $cargar_nodosrep;

		case 341: /* Consulta de la mayorizacion individual */
			switch ($Par_Sql[5]) {
				case 't':
					$Par_Sql[5] = " ";
					break;
			...
			$sql = "SELECT comprobantes.Com_Cod, Com_Num, Tia_Ini, Tia_Abr, Com_Fec, asientos.Asi_Cod, comprobantes.Com_Con, 
                    ...
                    WHERE (comprobantes.Com_Fec BETWEEN '$Par_Sql[0]' AND '$Par_Sql[1]') AND
							det_plan.Pld_Cod = '$Par_Sql[2]' AND comprobantes.Com_Est = 'A' AND
							comprobantes.Pec_Cod = $Par_Sql[4] $Par_Sql[5] $Par_Sql[3]";
			return $sql;
			break;
	}
