 ...
 * Implementa el control y validaci�n de traspasos de fondos garantizando que el presupuesto inicial permanezca intacto.
 */
 ...
 * @param string|null $proy_id C�digo de proyecto (opcional).
...
function ppto_reajuste_validar_disponibilidad($mysqli, $ppe_id, $ppa_id, $proy_id, $rubro, $mes, $monto) {
    ...
    if (!empty($proy_id) && !empty($rubro)) {
        ...
        // Validaci�n a nivel de partidas de la empresa (sin proyectos)
...
/**
 * Registra y procesa un nuevo movimiento de reajuste presupuestario (transferencia, incremento, reducci�n).
 * Implementa las reglas cr�ticas de integridad transaccional del ERP.
 *
 * @param mysqli $mysqli Conexi�n activa.
 * @param array $datos Arreglo de par�metros del movimiento.
 * @return bool Retorna true si se proces� e insert� con �xito, false de lo contrario.
 */
function ppto_reajuste_registrar($mysqli, $datos) {
    ...
    // Definici�n de variables de origen y destino
    ...
    // 1. REGLA DE NEGOCIO CR�TICA: Validaciones espec�ficas de flujos
    ...
    // 2. Insertar movimiento en la bit�cora relacional pre_reajustes
    $sql = "INSERT INTO pre_reajustes (
                Emp_Cod, ppe_id, rea_tipo, 
                ppa_id_origen, proy_id_origen, pdp_rubro_origen, 
                ppa_id_destino, proy_id_destino, pdp_rubro_destino, 
                rea_mes, rea_monto, rea_justificacion, rea_fecha_registro, Usu_Cod
            ) VALUES (
                $Emp_Cod, $ppe_id, '$tipo',
                $ppa_id_origen, $proy_id_origen, $pdp_rubro_origen,
                $ppa_id_destino, $proy_id_destino, $pdp_rubro_destino,
    ...
    $res = $mysqli->query($sql);
    if ($res) {
        // 3. Forzar el rec�lculo din�mico de los balances mensuales en las tablas de proyectos (Fase 2)
        ...
        if ($tipo === 'transferencia') {
            if ($proy_id_origen !== "NULL" && $pdp_rubro_origen !== "NULL") {
                ...
                $sql_pdp_orig = "SELECT pdp_id FROM pre_proyecto_detalles WHERE ppe_id = $ppe_id AND ppa_id = $ppa_id_origen AND proy_id = $proy_id_origen AND pdp_rubro = $pdp_rubro_origen LIMIT 1";
        ...
        if ($proy_id_destino !== "NULL" && $pdp_rubro_destino !== "NULL") {
            $sql_pdp_dest = "SELECT pdp_id FROM pre_proyecto_detalles WHERE ppe_id = $ppe_id AND ppa_id = $ppa_id_destino AND proy_id = $proy_id_destino AND pdp_rubro = $pdp_rubro_destino LIMIT 1";
...
/**
 * Consolida la sumatoria neta de reajustes aplicados a una partida est�ndar hasta un mes espec�fico.
 *
 * @param mysqli $mysqli Conexi�n activa.
...
function ppto_reajuste_consolidar_partida($mysqli, $ppe_id, $ppa_id, $mes) {
    ...
    $sql_dest = "SELECT SUM(rea_monto) AS total_suma 
                 ...
                 WHERE ppe_id = $ppe_id 
                   ...
                   AND proy_id_destino IS NULL -- Filtramos que sea est�ndar, no proyecto
                   AND rea_mes <= $mes 
...
/**
 * Consolida la sumatoria neta de reajustes aplicados a un rubro anal�tico de un proyecto para un mes espec�fico.
 *
 * @param mysqli $mysqli Conexi�n activa.
 * @param int $ppe_id ID de cabecera presupuestaria.
 * @param int $ppa_id ID de la partida presupuestaria.
 * @param string $proy_id C�digo de proyecto.
