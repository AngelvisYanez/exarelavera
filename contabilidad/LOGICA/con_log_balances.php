...
?>
	...
	/**
	 * Función optimizada para retornar todos los totales calculados de una sola vez
	 * Evita iteraciones redundantes y recursividad costosa
	 */
	function precalculatePlanTotals($cod, $Pec_Cod, $ini, $fin, $obBD_conexion, $sucursal = '')
	{
		...
		/* 2. Cargar plan completo */
		$planCompleto = $this->getArrayConsulta(243, $cod . '*1=1', $obBD_conexion);

		/* 3. Mapear valores e inicializar totales */
		$totals = array();
		$planIndexPorPadre = array();
		$planIndex = array();

		foreach ($planCompleto as $row) {
			$id = $row['Pld_Cod'];
			$planIndex[$id] = $row;
			$planIndexPorPadre[$row['Pld_Rec']][] = $row;
			$totals[$id] = array(
				'debe' => 0,
				'haber' => 0,
				'saldo' => 0,
		...
		foreach ($valores as $val) {
			if (isset($totals[$val['Pld_Cod']])) {
				$totals[$val['Pld_Cod']]['debe'] = (float)$val['debe'];
				$totals[$val['Pld_Cod']]['haber'] = (float)$val['haber'];
			}
		}

		/* 4. Propagar totales de abajo hacia arriba */
		/* Ordenamos por profundidad del CDC para asegurar procesar hijos antes que padres */
		$planOrdenado = $planCompleto;
		usort($planOrdenado, function ($a, $b) {
			return substr_count($b['Pld_Cdc'], '.') - substr_count($a['Pld_Cdc'], '.');
		});

		foreach ($planOrdenado as $row) {
			$id = $row['Pld_Cod'];
			$pid = $row['Pld_Rec'];

			// Saldo según grupo (Activo, Pasivo, etc)
			$group = (int)$row['Pld_Cdc'][0];
			if ($group == 2 || $group == 3 || $group == 4) {
				$totals[$id]['saldo'] = $totals[$id]['haber'] - $totals[$id]['debe'];
			} else {
				$totals[$id]['saldo'] = $totals[$id]['debe'] - $totals[$id]['haber'];
			}
			$totals[$id]['asignar'] = (abs($totals[$id]['saldo']) > 0.009);

			// Propagar al padre
			if ($pid != 0 && isset($totals[$pid])) {
				$totals[$pid]['debe'] += $totals[$id]['debe'];
				$totals[$pid]['haber'] += $totals[$id]['haber'];
			}
		}
		...
		return array(
			'totals' => $totals,
			'planIndex' => $planIndex,
			'planIndexPorPadre' => $planIndexPorPadre,
			'valores' => $valores
		);
	}

	function getCtaChildOptimized($planIndexPorPadre, $Pld_Rec)
	{
		$plan = array();
		if (isset($planIndexPorPadre[$Pld_Rec])) {
			foreach ($planIndexPorPadre[$Pld_Rec] as $value) {
				$plan[] = $value;
				$res = $this->getCtaChildOptimized($planIndexPorPadre, $value['Pld_Cod']);
				if (count($res) > 0) $plan = array_merge($plan, $res);
			}
		}
		return $plan;
	}
	...
	{
		/* OPTIMIZACIÓN: Realizar el cálculo completo en una sola pasada */
		$data = $this->precalculatePlanTotals($cod, $Pec_Cod, $ini, $fin, $obBD_conexion, $sucursal);
		$totals = $data['totals'];
		$planIndex = $data['planIndex'];
		$planIndexPorPadre = $data['planIndexPorPadre'];
		$valores = $data['valores'];

		/* Cálculo de utilidades: Grupos 4, 5, 6, 7 */
		$u_ingresos = 0; $u_egresos = 0;
		foreach($totals as $t) {
			if($t['Pld_Rec'] == 0) {
				$g = (int)$t['Pld_Cdc'][0];
				if($g == 4) $u_ingresos += $t['saldo'];
				if($g >= 5 && $g <= 7) $u_egresos += $t['saldo'];
			}
		}
		$util_periodo = $u_ingresos - $u_egresos;

		/* Ajustar saldo de Pasivo + Patrimonio para el estado */
		$total_activo = 0; $total_pas_pat = 0;
		foreach($totals as $t) {
			if($t['Pld_Rec'] == 0) {
				$g = (int)$t['Pld_Cdc'][0];
				if($g == 1) $total_activo = $t['saldo'];
				if($g == 2 || $g == 3) $total_pas_pat += $t['saldo'];
			}
		}
		
		/* Sumar la utilidad calculada al Pasivo+Patrimonio para verificar cuadre */
		$total_pas_pat += $util_periodo;

		if ($tipo == 1) {
			$diferencia = abs($total_activo - $total_pas_pat);
			echo "Estado: <span style='color:" . ($diferencia > 0.01 ? "#FF0000'>Descuadrado" : "#0000FF'>Cuadrado") . "</span>";
			echo " | Diferencia: " . formato_numero($diferencia, 2, $format);
			echo "<hr>";
		}
	}
	 ...
	 * $max_nivel = Máximo nivel a presentar 
	 * $format = Formato de presentación de los números 
	 */
	function cargarNodosBalance($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format, $Pec_Cod2_completo = null, $Pec_Fei_periodo = null, $Pec_Fef_periodo = null, $sucursal = '')
	{
		...
		/* OPTIMIZACIÓN: Realizar el cálculo completo en una sola pasada usando la nueva lógica ultra-rápida */
		$data = $this->precalculatePlanTotals($cod, $Pec_Cod, $ini, $fin, $obBD_conexion, $sucursal);
		$totals = $data['totals'];
		$planIndex = $data['planIndex'];
		$planIndexPorPadre = $data['planIndexPorPadre'];
		...
		/* Cálculo de utilidad para balance general */
		$u_ingresos = 0; $u_egresos = 0;
		foreach($totals as $t) {
			if($t['Pld_Rec'] == 0) {
				$g = (int)$t['Pld_Cdc'][0];
		...
		$util_calc = $u_ingresos - $u_egresos;

		/* Identificar cuenta de utilidad/pérdida */
		$utilidad_row = $this->getRowConsulta(242, $cod . '*' . ($util_calc > 0 ? 'G' : 'P'), $obBD_conexion);
		$util_cod = $utilidad_row['Pld_Cod'];

		/* Inyectar utilidad en el árbol de totales */
		if (isset($totals[$util_cod])) {
			// Propagar la utilidad hacia arriba desde la cuenta de utilidad hasta la raíz
			$curr_id = $util_cod;
			while($curr_id != 0 && isset($totals[$curr_id])) {
				$totals[$curr_id]['saldo'] += $util_calc;
				$totals[$curr_id]['asignar'] = (abs($totals[$curr_id]['saldo']) > 0.009);
				$curr_id = $totals[$curr_id]['Pld_Rec'];
			}
		}

		/* Filtro de grupos según el tipo de balance */
		$grupos_query = $this->getArrayConsulta(337, $cod . '*' . $np . '*' . $tipo, $obBD_conexion);
		$cuentas_visibles = array();
		foreach($grupos_query as $g) {
			$hijos = $this->getCtaChildOptimized($planIndexPorPadre, $g['Pld_Cod']);
			$cuentas_visibles[] = $g;
			$cuentas_visibles = array_merge($cuentas_visibles, $hijos);
		}

		/* Filtrar por niveles y saldos */
		$balance = array();
		foreach ($cuentas_visibles as $cta) {
			$id = $cta['Pld_Cod'];
			$total_data = $totals[$id];
			$cant_cuent = explode('.', $cta['Pld_Cdc']);

			if ($total_data['asignar'] && count($cant_cuent) <= $max_nivel) {
				/* Formatear para visualización */
				$cuenta_fmt = $cta;
				$cuenta_fmt['Total'] = formato_numero($total_data['saldo'], 2, $format);
				if ($total_data['saldo'] < 0) $cuenta_fmt['Total'] = '<font color="#FF0000" >' . $cuenta_fmt['Total'] . '</font>';
				
				$cant_esp = (count($cant_cuent) - 1) * 2;
				if ($cta['Pld_Rec'] == 0) $cuenta_fmt['Pld_Des'] = '<u>' . $cta['Pld_Des'] . '</u>';
				if ($cta['Pld_Tip'] == 'G') {
					$cuenta_fmt['Pld_Des'] = '<b>' . $cta['Pld_Des'] . '</b>';
					$cuenta_fmt['Pld_Cdc'] = '<b>' . $cta['Pld_Cdc'] . '</b>';
					$cuenta_fmt['Total'] = '<b>' . $cuenta_fmt['Total'] . '</b>';
				}
				$cuenta_fmt['Pld_Des'] = str_repeat("&nbsp;", $cant_esp) . $cuenta_fmt['Pld_Des'];
				
				$cuenta_fmt['cant_cuent'] = $cant_cuent;
				$cuenta_fmt['calc_total'] = $total_data;
				$balance[] = $cuenta_fmt;
			}
		}
	?>
		<table width="100%" cellpadding="0" cellspacing="0" border="0" class="LetraNegra">
			<tbody>
				...
				for ($i = 0; $i < count($balance); $i++) {
					...
					if ($cuenta['Pld_Tip'] == 'D' && $Pec_Cod2_completo !== null) {
						$codigoCuentaLimpio = strip_tags($cuenta['Pld_Cdc']);
						$fechasPersonalizadas = ($ini != $Pec_Fei_periodo || $fin != $Pec_Fef_periodo);
						$urlMayorizacion = $mayorizacion_file . '?hdd_save2=1&txt_busqueda=' . urlencode($codigoCuentaLimpio) . '&Pec_Cod2=' . urlencode($Pec_Cod2_completo) . '&txt_fec_ini=' . urlencode($ini) . '&txt_fec_fin=' . urlencode($fin) . '&op=1&ordenar=' . urlencode('ORDER BY Com_Fec ASC') . ($fechasPersonalizadas ? '&Chk_Fec=1' : '');
						$codigoCuentaHtml = '<a href="' . htmlspecialchars($urlMayorizacion, ENT_QUOTES, 'UTF-8') . '" target="mayorizacion_window" style="color: inherit; text-decoration: none; cursor: pointer;" onmouseover="this.style.textDecoration=\'underline\'; this.style.color=\'#0066cc\';" onmouseout="this.style.textDecoration=\'none\'; this.style.color=\'inherit\';" title="Ver mayorización">' . $cuenta['Pld_Cdc'] . '</a>';
					}
				?>
					<tr>
					...
					<?php if ($last) echo '<tr><td colspan="2"></td>' . $tdLast . '</tr>'; ?>
				<?php
					/* Totales de grupos */
					if (!isset($balance[($i + 1)]) || $cuenta['cant_cuent'][0] != $balance[($i + 1)]['cant_cuent'][0]) {
						$raiz_cdc = $cuenta['cant_cuent'][0];
						foreach ($grupos_query as $rg) {
							if ($rg['Pld_Cdc'] == $raiz_cdc) {
								$v_grupo = $totals[$rg['Pld_Cod']]['saldo'];
								// Nota: la utilidad ya está incluida en el saldo por la propagación automática
								
								$v_fmt = formato_numero($v_grupo, 2, $format);
								if ($v_grupo < 0) $v_fmt = '<font color="#FF0000" >' . $v_fmt . '</font>';
								
								echo '<tr><td colspan="2" height="40" valign="top"><h3 style="margin-top:2px"><u><strong>TOTAL &nbsp;' . $rg['Pld_Des'] . '</strong></u></h3></td><td colspan="' . ($max_nivel - 1) . '"></td><td style="border-top: 1px solid #000" align="right" valign="top"><h3 style="margin-top:2px">' . $v_fmt . '</h3></td></tr>';
							}
						}
					}
				}
				/* Totales finales */
				if ($tipo == 1) {
					/* Pasivo (2) y Patrimonio (3) ya incluyen la utilidad por la inyección previa */
					$v_pasivo = isset($totals[$planIndexPorPadre[0][1]['Pld_Cod']]) ? $totals[$planIndexPorPadre[0][1]['Pld_Cod']]['saldo'] : 0; 
					$v_patrimonio = isset($totals[$planIndexPorPadre[0][2]['Pld_Cod']]) ? $totals[$planIndexPorPadre[0][2]['Pld_Cod']]['saldo'] : 0; 
					$total_final = $v_pasivo + $v_patrimonio;
					echo '<tr><td colspan="2" height="40" valign="top"><h2 style="margin-top:6px"><u><strong>TOTAL PASIVO + PATRIMONIO =</strong></u></h2></td><td colspan="' . ($max_nivel - 1) . '"></td><td style="border-top: 2px solid #000" align="right" valign="top"><h2 style="margin-top:6px">' . formato_numero($total_final, 2, $format) . '</h2></td></tr>';
				}
				?>
			</tbody>
		</table>
	...
	function cargarNodosBalance1($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $Pec_Cod, $util, $cod_util, $nivel, $max_nivel, $format)
	{
		...
		function cargarTotalEstados($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $sucursal = '')
		{
			//$j=$j+1;
			if ($np == 0) {
				/**
		...
		function cargarTotalEstadosOptimizado($cod, $np, $ini, $fin, $obBD_conexion, $tipo, $valoresIndex, $planIndex, $planIndexPorPadre)
		{
			...
			if (count($rs_nodosrep) > 0) {
				foreach ($rs_nodosrep as $row_rs_nodosrep) {
					...
					$total = $this->calculoBalanceOptimizado(
						$cod,
						$row_rs_nodosrep['Pld_Cod'],
						$ini,
						$fin,
