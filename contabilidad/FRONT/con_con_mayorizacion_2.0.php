...
require_once('../LOGICA/con_log_mayorizacion_2.0.php');
	...
	switch ($op) {
		case 1:
			...
			if ($txt_busqueda != "") {
				 ...
				 */
				list($annn, $mess, $dia) = explode('-', $fech_fut);
											...
											<div id="Exportar_a_Excel">
												...
												<table width="100%" border="1" cellpadding="0" cellspacing="0" class="fixedHeader03" style="table-layout:fixed;">
													...
													<tbody>
														...
														<?php
														// Optimizacion: Saldo inicial acumulado
														$saldo_acumulado_global = $saldos; 
														
														foreach ($result as $r) {
															$rs_cuenta = $obBD_con1->getArrayConsulta(201, $r['start'] . '*' . $r['end'] . '*' . $Pld_Cod . '*' . $ordenar . '*' . $Pec_Cod . '*' . $Com_Aut, $obBD_conexion);
															$total_rs_cuenta = count($rs_cuenta);
															
															// Ya no consultamos Case 202 en cada mes, usamos el saldo acumulado
															$saldos_mes = $saldo_acumulado_global;
															
															// Para el formato de fecha "Saldo al..."
															$fech_format = fechas_futuras($r['start'], -1);
															list($annn, $mess, $dia) = explode('-', $fech_format);
														?>
															<tr>
																<td align="center">&nbsp;</td>
																<td align="center">&nbsp;</td>
																...
																<td align="right">&nbsp;</td>
																<td align="right">&nbsp;</td>
																<td align="right" <?php if ($saldos_mes < 0) {
																						echo "style='color:#FF0000'";
																					} ?>><?php echo formato_numero($saldos_mes, 2, 2); ?></td>
																<td align="right" <?php if ($saldos_mes < 0) {
																						echo "style='color:#FF0000'";
																					} ?>>&nbsp;</td>
															</tr>
															<?php
															if ($total_rs_cuenta > 0) {
																$saldo_proceso = $saldos_mes;
																$total_debe = 0;
																$total_haber = 0;
																foreach ($rs_cuenta as $row) {
																	$numeroDocCompra = $row['Cop_Num'];
																	$numeroDocVenta = $row['Vet_Num'];
																	$cheque = $row['Che_Num'];
																	$tipo_documento_venta = $row['Tic_Des_Venta'];
																	$tipo_documento_compra = $row['Tic_Des_Compra'];

																	/* Datos cliente/proveedor (Query optimizada) */
																	$row_proveedore = array(
																		'Prs_Ced' => $row['Prs_Ced'],
																		'Prs_Ape' => $row['Prs_Ape'],
																		'Prs_Nom' => $row['Prs_Nom']
																	);
																	
																	if (empty($row_proveedore['Prs_Ape']) && $row['Tia_Abr'] == 'RL' && $row['Tia_Ini'] != 'D') {
															...
															?>
																	<tr>
																		...
																		<td align="right" <?php if ($saldo_proceso < 0) { 
																								echo "style='color:#FF0000'";
																							} ?>> <?php echo formato_numero($saldo_proceso, 2, 2); ?> </td>
																...
																$saldo_acumulado_global = $saldo_proceso;
																?>
																<tr>
																	<td>&nbsp;</td>
																	<td>&nbsp;</td>
														...
														<tbody>
															...
															?>
																...
																foreach ($rs_cuenta as $row) {
																	...
																	/* Datos del cliente o proveedor/empleado (Ya vienen en la query!) */
																	$row_proveedore = array(
																		'Prs_Ced' => $row['Prs_Ced'],
																		'Prs_Ape' => $row['Prs_Ape'],
																		'Prs_Nom' => $row['Prs_Nom']
																	...
																	// Fallback solo para casos especiales (Rol de pago si no vino el nombre)
																	if (empty($row_proveedore['Prs_Ape']) && $row['Tia_Abr'] == 'RL' && $row['Tia_Ini'] != 'D') {
																		$row_proveedore = $obBD_con1->getRowConsulta(2199, $row['Com_Cod'], $obBD_conexion);
																	}
																	...
																	$informacion_documentos = trim($documento_compra .' '.$documento_venta);
																?>
																	<tr>
																		<td align="center"><?php echo $row['Com_Cod']; ?></td>
																		<td align="center"><?php echo $row['Com_Gen']; ?></td>
														...
														<div id="Exportar_a_Excel">
															...
															if ($total_rs_rango > 0) {
																...
																foreach ($rs_rango as $row_rango) {
																	...
																	if ($tipo_grupo[0] == 2 || $tipo_grupo[0] == 3 || $tipo_grupo[0] == 4) {
																		$saldos_inicial = $haber - $debe;
																	} else {
																		$saldos_inicial = $debe - $haber;
																	}
																	
																	$saldo_acumulado_global = $saldos_inicial;

																	// Consultamos todos los movimientos del rango para esta cuenta una sola vez
																	$rs_cuenta_full = $obBD_con1->getArrayConsulta(201, $txt_fec_ini . '*' . $txt_fec_fin . '*' . $row_rango['Pld_Cod'] . '*' . $ordenar . '*' .
																		$Pec_Cod . '*' . $Com_Aut, $obBD_conexion);
																	$total_rs_cuenta_full = count($rs_cuenta_full);

																	if ($total_rs_cuenta_full > 0 or $total_rs_saldos > 0) {
																		...
																		<table width="100%" border="1" cellpadding="0" cellspacing="0" class="fixedHeader03" style="table-layout:fixed;">
																			...
																			<tbody>
																				...
																				foreach ($result as $r) {
																					...
																					// Filtrar registros del mes en memoria para evitar consultas redundantes
																					$registrosCuentaMerns = array();
																					foreach($rs_cuenta_full as $full_row) {
																						if ($full_row['Com_Fec'] >= $r['start'] && $full_row['Com_Fec'] <= $r['end']) {
																							$registrosCuentaMerns[] = $full_row;
																						}
																					}

																					$fech_format = fechas_futuras($r['start'], -1);
																					list($ann, $mes, $dia) = explode('-', $fech_format);
																				?>
																					<tr>
																						<td align="center">&nbsp;</td>
																						<td align="center">&nbsp;</td>
																					...
																					<?php
																					$saldo_proceso = $saldos_mes;
																					foreach ($registrosCuentaMerns as $row_rs_cuenta) {
																						$numeroDocCompra = $row_rs_cuenta['Cop_Num'];
																						$numeroDocVenta = $row_rs_cuenta['Vet_Num'];
																						$cheque = $row_rs_cuenta['Che_Num'];
																						
																						$row_proveedore = array(
																							'Prs_Ced' => $row_rs_cuenta['Prs_Ced'],
																							'Prs_Ape' => $row_rs_cuenta['Prs_Ape'],
																							'Prs_Nom' => $row_rs_cuenta['Prs_Nom']
																						);
																						
																						if (empty($row_proveedore['Prs_Ape']) && $row_rs_cuenta['Tia_Abr'] == 'RL' && $row_rs_cuenta['Tia_Ini'] != 'D') {
																							$row_proveedore = $obBD_con1->getRowConsulta(2199, $row_rs_cuenta['Com_Cod'], $obBD_conexion);
																						}
																						
																						list($ann, $mes, $dia) = explode('-', $row_rs_cuenta['Com_Fec']);
																						$i++;
																					?>
																						<tr>
																							...
																							<?php
																							// No necesitamos recalcular saldos_mes aquí, ya lo llevamos en saldo_proceso
																							?>
																							<td align="right" <?php if ($saldos < 0) {
																													echo "style='color:#FF0000'";
																												} ?>><?php
															...
															<div id="Exportar_a_Excel">
																...
																if ($total_rs_rango > 0) {
																	...
																	foreach ($rs_rango as $row_rango) {
																		 ...
																		 */
																		list($ann, $mes, $dia) = explode('-', $fech_fut);
