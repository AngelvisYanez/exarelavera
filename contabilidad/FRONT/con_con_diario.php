...
{	
		...
		{					
			...
			$rs_compr = $obBD_con1->getArrayConsulta(9,$txt_fec_ini.'*'.$txt_fec_fin.'*'.$txt_busqueda,$obBD_conexion); 
		}
		else
		{
			$txt_busqueda = "";
			/* Consulta de los comprobantes TODOS*/
			$rs_compr = $obBD_con1->getArrayConsulta(11,$txt_fec_ini.'*'.$txt_fec_fin,$obBD_conexion); 
		}		
...
else
{
	...
	else
	{
		...
		$rs_periodo = $obBD_con1->consulta(sentencias_con(214,''), $obBD_conexion->conexion);
...
<FIELDSET>
    ...
    <table width="225" border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		do{
		?>
			<option value="<?Php echo $row_rs_periodo['Pec_Cod'].'*'.$row_rs_periodo['Pec_Fei'].'*'.$row_rs_periodo['Pec_Fef']; ?>"><?Php echo $row_rs_periodo['Periodo']; ?></option>	
		<?php
        ...
        <button type="button" class="btn btn-success btn-mini" title="Buscar" onClick="validar_requeridos(this.form, 'Pec_Cod', 0)"> <i class="icon-search icon-white"></i> <span>Buscar</span> </button>
...
if (isset($hdd_save) or isset($hdd_save2))
{
	...
	<table width="100%" border="0">
      <tr>
        <td width="41%"><?php include("../COMPONENTES/con_con_anio_mes_fecha.php"); ?></td>
        <td width="31%">
        <FIELDSET>  
	<LEGEND>
	...
	<FIELDSET id="field_buscador">
    	...
    	<tr>
	      ...
	      <td width="90"><div align="center">
	        <button type="button" id="btn_libros" name="btn_libros" class="btn btn-success btn-mini" title="Cuentas" onClick="Muestra_Aparecer();"> <i class="icon-list-alt icon-white"></i> <span>Cuentas</span> </button>
	      </div>			</td>
    	</tr>
  		</table>
        <br>
...
<form action="con_pri_diario.php" method="post" name= "form2" id="form2" target="_blank">
<?Php
if (isset($detalle))
{
	
/* Consulta del detallete de la CUENTA */
$row_rs_recur = $obBD_con1->getRowConsulta(204,$codRec,$obBD_conexion);							
?>	
...
?>
  <table width="100%" border="1" cellpadding="1" cellspacing="0" class="fixedHeader01">
    ...
    <tr>
      ...
      <th width="8%" align="center">No. Com.</th>
