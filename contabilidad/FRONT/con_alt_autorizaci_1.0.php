...
<?php 
/**
* Descripci�n: Alta de las Autorizaciones
...
* Desarrollador: Lewis Chimarro
* Fecha de actualizaci�n: 2012-06-23
* Desarrollador: Lewis Chimarro
*/
...
{
	 /** 
	 * Da de baja una autorizacion
	 */
	 if (isset($cerrar))
	 {	 
		$obBD_con1->insertUpdateDelete(104, $codigo, $obBD_conexion); 
	 }
	...
	*/
	...
	{  
	  ...
	  $obBD_con1->insertUpdateDelete(103, $cadena[0].'*'.$Tic_Cod.'*'.$Aut_Sri.'*'.$Pun_Sri.'*'.$Aut_Fci.'*'.$Aut_Cad.'*'.$Num_Ini.'*'.$Num_Fin.'*'.$Alerta.'*'.$Alerta2.'*'.$Aut_Tem, $obBD_conexion);	
	}
}
?>
<HTML>
	<HEAD>
       ...
       <script>
		$(function() { 
			/* Campo 1 */
			$( "#Aut_Fci" ).datepicker({changeMonth: true,changeYear: true, dateFormat: 'yy-mm-dd',firstDay: 1});			
			$( "#Aut_Fci" ).change(function() {
			$( "#Aut_Fci" ).datepicker( "option", "dateFormat", "yy-mm-dd" );
			});			
			/* Campo 2 */
			$( "#Aut_Cad" ).datepicker({changeMonth: true,changeYear: true, dateFormat: 'yy-mm-dd',firstDay: 1});			
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
  ...
  <td height="10">&raquo;Registrar Autorizaci&oacute;n S.R.I. </td>   
  </tr>
	<tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="Azul">
    <tr>
      <td width="100%" height="59"><button type="button" class="btn btn-success fileinput-button" title="Buscar" onClick="validar_requeridos(this.form, 'Suc_Cod*Pun_Cod', 0)"> <i class="icon-search icon-white"></i> <span>Buscar</span> </button>        
...
<label class="Titulos2"> Resultado de la busqueda </label>
  ...
  <tr>
		...
		<th width="45%" >Tipo Documento</th>
		<th width="10%" >Autorizaci&oacute;n</th>
		<th width="10%" >Caducidad</th>
		<th width="9%" >Secuencia Inicio</th>
		<th width="9%" >Secuencia Final</th>
		<th width="12%" >&nbsp;</th>
  </tr>
  </thead>
  <tbody>
  ...
  <?php $thisPost->startPost(); ?>
    ...
    <td align="center">
	<input type="hidden" id="codigo" name="codigo" value="<?Php echo $row['Aut_Cod'];?>">
...
<div id="bgmodal"  class="bgmodal"   style="display:none">		
 ...
 <?Php
	...
	* Carga los tipos de documentos de un Punto de Impresion
	*/
	$row_rs_documento = $obBD_con1->getArrayConsulta(509, $cadena[0], $obBD_conexion); 
 ?>
<FIELDSET>
<LEGEND>
<label class="Titulos2">Datos a registrar</label>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <select name="Aut_Tem" id="Aut_Tem" style="width: 30%">
      ...
      <option value="E">Electr&oacute;nica</option>
    </select>
    </td>
  </tr>
  <tr>
    <td class="Etiqueta1"><span class="Asterisco">*</span> N&ordm;  Autorizaci&oacute;n:</td>
    <td colspan="2"><input id="Aut_Sri" name="Aut_Sri" type="text" size="9" maxlength="10" style="text-transform:uppercase; text-align:right" onKeyPress="return validar_numeric(event)" onBlur="if(this.value!=''){minimo(this,10)}"></td>
  </tr>
  <tr>
    <td class="Etiqueta1"><span class="Asterisco">*</span> Punto S.R.I.:</td>
    <td width="2%"><input name="Pun_Sri" id="Pun_Sri" type="text" size="3" maxlength="3" style="text-transform:uppercase; text-align:right" onKeyPress="return validar_numeric(event)"></td>
    <td width="81%" class="Texto_Reporte_Rojo">(Ejemplo: 001)</td>
  </tr>
  <tr>
    <td class="Etiqueta1"><span class="Asterisco">*</span> Fecha Inicio:</td>
    <td colspan="2"><input name="Aut_Fci" type="text" id="Aut_Fci" value="<?php echo date("Y-m-d"); ?>" size="8" onKeyUp="mascara(this,'-',patron,true)" onBlur="validar_fecha2(this)">
  </tr>
  <tr>
    <td class="Etiqueta1"><span class="Asterisco">*</span> Fecha Caducidad:</td>
    <td colspan="2"><input name="Aut_Cad" type="text" id="Aut_Cad" value="<?php echo date("Y-m-d"); ?>" size="8" onKeyUp="mascara(this,'-',patron,true)" onBlur="validar_fecha2(this)">
  </tr>
  <tr>
    <td class="Etiqueta1"><span class="Asterisco">*</span> Secuencia Inicial:</td>
    <td colspan="2"><input id="Num_Ini" name="Num_Ini" type="text" size="5" maxlength="7" style="text-transform:uppercase;text-align:right" onKeyPress="return validar_numeric(event)" onBlur="numeroMenor_Msn(this,document.getElementById('Num_Fin'),'No puede ingresar valores mayores que')">
    </td>
  </tr>
  <tr>
    <td class="Etiqueta1"><span class="Asterisco">*</span> Secuencia Final:</td>
    <td colspan="2"><input id="Num_Fin" name="Num_Fin" type="text" size="5" maxlength="7" style="text-transform:uppercase;text-align:right" onKeyPress="return validar_numeric(event)" onBlur="numeroMayor_Msn(this,document.getElementById('Num_Ini'),'No puede ingresar valores menores que')">
    </td>
  </tr>
  <tr>
    <td class="Etiqueta1"><span class="Asterisco">*</span> Alerta 1:</td>
    <td><input id="Alerta2" name="Alerta2" type="text" size="2" maxlength="2" style="text-transform:uppercase;text-align:right" onKeyPress="return validar_numeric(event)"></td>
    <td class="Texto_Reporte_Rojo">(Documentos m&iacute;nimos para mostrar una alerta antes que termine la secuencia)</td>
  </tr>
  <tr>
    <td class="Etiqueta1"><span class="Asterisco">*</span> Alerta 2:</td>
    <td><input id="Alerta" name="Alerta" type="text" size="2" maxlength="2" style="text-transform:uppercase;text-align:right" onKeyPress="return validar_numeric(event)"></td>
...
<table width="126" border="0">
  <tr>
	...
	<input type="hidden" id="hdd_avilita_1" name="hdd_avilita_1" value="1">
	<input type="hidden" id="hdd_avilita_2" name="hdd_avilita_2" value="1">
    <button type="button" class="btn btn-primary start" title="Guardar" onclick="document.getElementById('hdd_save').value=1; if(fecha_mayor(Aut_Fci,Aut_Cad)!=false){ validar_requeridos(this.form,'Pun_Cod*Aut_Sri*Pun_Sri*Num_Ini*Num_Fin*Alerta*Alerta2',1)}">
