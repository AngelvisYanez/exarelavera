...
$Max_Niv = (isset($_POST['Max_Niv']) ? $_POST['Max_Niv'] : (isset($_GET['Max_Niv']) ? $_GET['Max_Niv'] : 7));
$txt_fec_ini = (isset($_POST['txt_fec_ini']) ? $_POST['txt_fec_ini'] : (isset($_GET['txt_fec_ini']) ? $_GET['txt_fec_ini'] : ''));
$txt_fec_fin = (isset($_POST['txt_fec_fin']) ? $_POST['txt_fec_fin'] : (isset($_GET['txt_fec_fin']) ? $_GET['txt_fec_fin'] : ''));
$hdd_save = (isset($_POST['hdd_save']) ? $_POST['hdd_save'] : (isset($_GET['hdd_save']) ? $_GET['hdd_save'] : null));
$Pec_Cod = (isset($_POST['Pec_Cod']) ? $_POST['Pec_Cod'] : (isset($_GET['Pec_Cod']) ? $_GET['Pec_Cod'] : null));
...
<HEAD>
	...
	<style>
		...
		.balance-form-section { display: grid; grid-template-columns: 1.5fr 1fr 0.7fr; gap: 10px; margin-bottom: 8px; align-items: stretch; }
		@media (max-width: 1024px) { .balance-form-section { grid-template-columns: 1.2fr 1fr 0.8fr; } }
		@media (max-width: 768px) { .balance-form-section { grid-template-columns: 1fr; } }
		.balance-form-section fieldset { background: #DFE9F6; border: 1px solid #6c757d; border-radius: 6px; padding: 10px; height: 100%; box-sizing: border-box; }
		.balance-fieldset { background: #DFE9F6; border: 1px solid #6c757d; border-radius: 6px; padding: 12px; }
		/* .balance-form-section legend, .balance-fieldset legend { background: #DFE9F6; padding: 4px 12px; border-radius: 4px; border: 1px solid #6c757d; font-weight: 600; color: #495057; font-size: 13px; } */
		.balance-results-legend { border: none !important; background: transparent !important; padding: 0 !important; }
		/* Estilo específico para ajustar el legend de los componentes internos si es necesario */
		/* .balance-form-section fieldset legend { margin-left: 10px; } */
		...
		.balance-table-container table.LetraNegra td { font-size: 12px; text-align: left; padding: 3px 8px !important; line-height: 1.1; word-wrap: break-word; overflow-wrap: break-word; }
		...
		.balance-table-container table.LetraNegra td:first-child { font-size: 14px; font-weight: 400; color: #254463; text-align: left !important; padding-right: 8px !important; padding-left: 8px !important; white-space: nowrap; max-width: 80px; }
		/* Segunda columna (descripción) - más grande */
		.balance-table-container table.LetraNegra td:nth-child(2) { font-size: 12px; text-align: left; color: #212529; border-spacing: 0px; min-width: 0; max-width: none; word-wrap: break-word; overflow-wrap: break-word; }
		...
		/* Estado del balance */
		.balance-status { display: inline-block; padding: 8px 16px; border-radius: 20px; font-weight: 600; font-size: 13px; margin-top: 10px; }
		.balance-status.cuadrado { background: #d4edda; color: #155724; }
		.balance-status.descuadrado { background: #f8d7da; color: #721c24; }
					...
					<form name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
						...
						?>
							<div class="balance-form-section">
								...
								<div>
									<FIELDSET>
										...
										<table width="100%" border="0">
											<tr>
												<td class="Etiqueta1" style="width: 70px;">Sucursal:</td>
												<td>
													<select name="cmb_sucursal" id="cmb_sucursal" style="width: 70%; text-align: center;">
														<option value=""><< TODAS >></option>
														<?php
														$rs_sucursales = $obBD_con1->getArrayConsulta(245, $Ses_Emp_Cod, $obBD_conexion);
														foreach ($rs_sucursales as $suc) {
															$selected = (isset($cmb_sucursal) && $cmb_sucursal == $suc['Suc_Cod']) ? 'selected' : '';
															echo '<option value="' . $suc['Suc_Cod'] . '" ' . $selected . '>' . $suc['Suc_Des'] . '</option>';
														}
														?>
													</select>
												</td>
								...
								<div style="justify-self: end; width: 100%;"><?php include("../COMPONENTES/con_con_niveles_plan.php"); ?></div>
							</div>
							
							<div class="balance-actions" style="display: flex; gap: 10px; align-items: center;">
								<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="form_volver_top" id="form_volver_top" style="margin: 0;">
								...
								<input name="cmb_sucursal_val" id="cmb_sucursal_val" type="hidden" value="<?php echo isset($cmb_sucursal) ? $cmb_sucursal : ''; ?>">
							</div>
						<?php
						...
						<FIELDSET class="balance-fieldset">
							...
							<div class="balance-date-range">
								...
								<?php if (isset($cmb_sucursal) && $cmb_sucursal != '') { 
									...
									echo '<strong style="margin-left: 20px;">Sucursal:</strong> ' . $row_suc_info['Suc_Des'];
								} else {
									echo '<strong style="margin-left: 20px;">Sucursal:</strong> TODAS';
								} ?>
							</div>
									...
									<tr class="LetraNegra">
										<td colspan="4" style="padding: 0;">
											<?php
												...
												$obBD_con1->estadoBalance($Pla_Cod, 0, $txt_fec_ini, $txt_fec_fin, $obBD_conexion, 1, $arreglo[0], 0, $utilidad, 0, $Max_Niv, 2, $cmb_sucursal);
												$obBD_con1->cargarNodosBalance($Pla_Cod, 0, $txt_fec_ini, $txt_fec_fin, $obBD_conexion, 1, $arreglo[0], 0, $utilidad, 0, $Max_Niv, 2, $Pec_Cod2_completo, $Pec_Fei_periodo, $Pec_Fef_periodo, $cmb_sucursal);
											?>
										</td>
									</tr>
								</table>
							...
							<div class="balance-export-actions">
								...
								<form action="con_pri_balgeneral.php" method="post" name="form2" id="form2" target="_blank">
									...
									<input name="cmb_sucursal" type="hidden" id="cmb_sucursal" value="<?php echo isset($cmb_sucursal) ? $cmb_sucursal : ''; ?>">
								</form>
