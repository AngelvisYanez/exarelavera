...
{	
	...
	*/
	...
	{	
		...
		else 
		{
			...
			*/
			...
			$row_rs_cabcomp = $obBD_con1->getArrayConsulta(353, $tabla.'*'.trim($txt_busqueda).'*'.$op.'*'.$Pec_Cod.'*'.$campo.'*'.$Par_Fec, $obBD_conexion);
		}  			
	}
	else if (isset($codigo))
	{
		...
		*/
		$row_rs_codcompr = $obBD_con1->getRowConsulta(354, $tabla.'*'.$codigo.'*'.$op.'*'.$Pec_Cod.'*'.$campo, $obBD_conexion);
		/** 
...
if (!isset($txt_busqueda) && !isset($hdd_ver))
{	
	switch ($op)
	{	
		case "I":			
	...
	//$row_rs_cabcomp = $obBD_con1->getArrayConsulta(154, $tabla.'**'.$op.'*'.$Pec_Cod.'*'.$campo.'*'.$cmb_mes, $obBD_conexion);
}
?>
...
<FIELDSET>
	...
	?>
	    ...
	    <td width="113" align="center"><button type="button" class="btn btn-success fileinput-button" title="Buscar" onClick="validar_requeridos(this.form, 'Pec_Cod', 0)"> <i class="icon-ok icon-white"></i>
...
else
{
	...
	$pag4= $_SERVER['PHP_SELF']."?op=4&hdd_save&Pec_Cod=".$Pec_Cod;
		switch($op) {
			...
			case 4: $activo = 4; break; }									
	tabs(4,'Ingreso*Egreso*Diario*Totales', $pag1.'*'.$pag2.'*'.$pag3.'*'.$pag4, $activo); ?>
<div id="ContTabul">
<?php	
if (($op=="I" || $op=="E" || $op=="D")) 
{
		switch($op) {
		case "I": $etiqueta="Buscar Comprobante de ".(isset($array_asien[0])?$array_asien[0]:'Ingreso')." por: "; 
		break;
		case "E":	$etiqueta="Buscar Comprobante de ".(isset($array_asien[1])?$array_asien[1]:'Egreso')." por: "; 
		break; 
		case "D":	$etiqueta="Buscar Comprobante de ".(isset($array_asien[2])?$array_asien[2]:'Diario')." por: "; 
		break; 
		} ?>
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name= "form1">
	<FIELDSET>
	...
	<table width="595" height="38" border="0" cellpadding="0" cellspacing="0">
		<tr>
		  <td width="472" class="BarraBusqueda">Busqueda:
		  ...
		  <td width="123" align="center"><button type="button" class="btn btn-success fileinput-button" title="Buscar" onClick="validar_requeridos(this.form, 'txt_busqueda', 0)">
    ...
    <thead>
        <tr class="Cabecera1" height="27">
          ...
          <th width="18%" align="left">Proveedor/Cliente</th>
          <th width="13%" align="left">Tipo Comprobante</th>
          <th width="16%" align="left">Concepto</th>
          <th width="6%">Fecha</th>
          <th width="6%">Creac.</th>
		  <th width="6%">Valor</th>
		  <th width="4%">&nbsp;</th>
     ...
     <tbody>	    
		<?php 			
	if (isset($row_rs_cabcomp) && count($row_rs_cabcomp) > 0) 
	{	  		
		$i=0;
		...
		<tr class="Fondo">
		  ...
		  */		  
          ...
          <td align="center"><font color="<?php echo $rojo; ?>"><?php echo $row['Com_Sys']; ?></font></td>	          
		  <td align="right"><font color="<?php echo $rojo; ?>">&nbsp;<?php echo $row['Com_Val']; ?></font></td>
		  <td align="right"><button type="button" class="btn btn-info btn-mini" title="Detalle del registro" onClick="Muestra_Aparecer(); ajax_datos('<?php echo $_SERVER['PHP_SELF']; ?>?ajax_info=1&com_codigo=<?php echo $row['Com_Cod'];?>&Ses_Emp_Cod=<?php echo $Ses_Emp_Cod;?>','mostrar')"><i class="icon-info-sign icon-white"></i></button></td>
		  <td align="center"><button type="button" class="btn btn-success btn-mini" title="Ver Factura/Retenci�n" onClick="Muestra_Aparecer(); ajax_datos('<?php echo $_SERVER['PHP_SELF']; ?>?ajax_info2=1&com_codigo=<?php echo $row['Com_Cod'];?>&op=<?php echo $op;?>&Ses_Suc_Cod=<?php echo $Ses_Suc_Cod;?>','mostrar')"><i class="icon-info-sign icon-white"></i></button></td>   
		  <td align="center">
            ...
            <input name="hdd_ver" value="1" type="hidden">
		...
		<?php		
 		...
 		<?php echo barra_estado(isset($row_rs_cabcomp)?count($row_rs_cabcomp):0); ?>
		</FIELDSET>
    	<?php
  ...
  if (isset($codigo) && $codigo > 0 && isset($row_rs_codcompr) && count($row_rs_codcompr)>0) 
  { ?>
	<br>   
<FIELDSET>
<LEGEND>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td width="109" class="Etiqueta1">Tipo Comprobante:</td>
    <td width="498">&nbsp;<?php echo $row_rs_codcompr['Tia_Des']; ?></td>
    <td width="133" class="Etiqueta1">Fecha:</td>
    <td width="279" class="LetraNegra">&nbsp;<?php echo $row_rs_codcompr['Com_Fec']; 
	$m=explode("-",$row_rs_codcompr['Com_Fec']);	
	?></td>
    </tr>
  <tr>
    <td class="Etiqueta1">No. Compr: </td>
    <td class="LetraNegra"><div id="div_codigo">&nbsp;<span class="Etiqueta1"><?php echo $row_rs_codcompr['Tia_Abr'].'-'.$m[1].'-'.$row_rs_codcompr['Com_Num']; ?></span></div></td>
	...
	<table width="300" border="0" cellpadding="0" cellspacing="0">
    	  <tr>
            ...
            <form action="<?php echo $hdd_reportes; ?>" method="post" name= "form2" target="_blank"> 
                ...
                <input name="tipo" type="hidden" value="<?php echo $row_rs_codcompr['Tia_Cod']; ?>">
...
?>
	...
	<table width="449" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="167"><select name="Com_Aut" id="Com_Aut">
          <option <?php if (isset($Com_Au)&&$Com_Aut == "T"){ echo "selected"; } ?> value="T"><< TODOS >></option>
          <option <?php if (isset($Com_Au)&&$Com_Aut == "M"){ echo "selected"; } ?> value="M">Manual</option>
 	  <option <?php if (isset($Com_Au)&&$Com_Aut == "A"){ echo "selected"; } ?> value="A">Autom�ticos</option>
        </select>
        </td>
      </tr>
    </table>
      ...
      <tr>
        ...
        <td width="322">
          <select name="Com_Tip" id="Com_Tip">
            <?php
			$tip = explode("*", isset($Com_Tip)?$Com_Tip:'');
			
			foreach ($row_rs_tipo_asien as $row)
			{
			?>
        	    <option <?php if ($row['Tia_Cod'] == $tip[0]){ echo "selected"; } ?> 
                		value=<?php if ($row['Tia_Ini'] == "E" || $row['Tia_Ini'] == "D"){ ?>
	...
	<FIELDSET>
    <LEGEND>
    <label class="Titulos2">Opciones de presentaci�n</label>
    </LEGEND>
	...
	<table width="541" border="0" cellpadding="0" cellspacing="0">
      <tr>
		...
		<?php 
		list($ann, $mes, $dia) = explode('/', date("Y/m/d"));
		/** 
	...
	*/
    ...
    <thead>
          <tr>
            <?php if(isset($Niv_Cod[1])){ ?><th width="5%">C�d. Int. </th><?php } ?>
            <th width="8%">Tipo</th>            
            <th width="8%">No. Compr </th>
            <th width="12%">Fecha</th>
            <th width="30%">Proveedor/Cliente</th>
            <?php if(isset($Niv_Cod[2])){ ?><th width="20%">Concepto</th><?php } ?>
	...
	?>
		  ...
		  */	
		  ...
		  ?>
            ...
            <td align="center"><?php 
			list($ann, $mes, $dia) = explode('-', $row['Com_Fec']);
			echo $row['Tia_Abr']."-".$mes.'-'.$row['Com_Num']; ?></td>
            ...
            <td align="center">
                <form action="<?php echo $hdd_reportes; ?>" method="post" name= "form2" target="_blank"> 
		<button type="button" class="btn btn-primary  btn-mini" title="Imprimir Comprobante" onclick="this.form.submit()"> <i class="icon-print icon-white"></i></button> 
                <button type="button" class="btn btn-info btn-mini" title="Detalle del registro" onClick="Muestra_Aparecer(); ajax_datos('<?php echo $_SERVER['PHP_SELF']; ?>?ajax_info=1&com_codigo=<?php echo $row['Com_Cod'];?>&Ses_Emp_Cod=<?php echo $Ses_Emp_Cod;?>','mostrar')"><i class="icon-info-sign icon-white"></i></button></td>
                <input name="Com_Cod" type="hidden" value="<?php echo $row['Com_Cod']; ?>">
	...
	{ ?>		
		<table width="263" border="0" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <form name='formulario' method='post' action='con_pri_comprb_1.1.php' target='_blank' >
			...
			<input name="Pec_Fei" id="Pec_Fei" type="hidden" value="<?php echo $row_rs_periodo['Pec_Fei']; ?>">
			 ...
			 <input name="op" type="hidden" value="<?php echo $op; ?>" >
			<input name="Pec_Cod" id="Pec_Cod" type="hidden" value="<?php  echo $Pec_Cod; ?>">
		<input name="Niv_Cod" id="Niv_Cod" type="hidden" value="<?php  echo isset($Niv_Cod)&&$Niv_Cod; ?>">
			<input name="hdd_save" id="hdd_save" type="hidden" value="">
