<?php
/**
 * @abstract Permite realizar la activación/baja de usuarios
 * @author Cesar Bermeo
 * @version 1.0
...
require_once('../LOGICA/adm_log_usuario.php');
...
//Listo usuarios
if(isset($usrAjax)){
   $data = array_merge($_GET, array('setWhere'=>array('setSucCod', 'notIsAdmin', 'setEmpCod', 'notIsInterno')));
   $responce = $obBD_con1->getPageGridJson('usuarios.selectWhere', $data, $obBD_conexion);
   $obBD_con1->echoLog($responce);
}

//Busqueda Usuario por Usu_Cod
if(isset($usrPersona)){
      $resp=array('success'=>true,
      'usuario'=>$obBD_con1->getRowConsulta('usuarios.selectWhere', array('usuarios.Usu_Cod'=>$Usu_Cod),$obBD_conexion),
);
$obBD_con1->echoJson($resp);
}
// Activo/Desactivo Usuario
if(isset($delUsuario) || isset($upUsuario) ){
   $obBD_con1->inicio_transaccion($obBD_conexion);
   try{
      if($delUsuario){$obBD_con1->operacionobBD('usuarios.setInactive',array('Usu_Cod'=>$Usu_Cod),$obBD_conexion);}
      if($upUsuario){$obBD_con1->operacionobBD('usuarios.setActive',array('Usu_Cod'=>$Usu_Cod),$obBD_conexion);}
                  ...
                  <div id="allUsuarios" class = "row">  
                     <div class="col-sm-12">
                           <fieldset class="exa-fieldset">
                              <legend class="Titulos2">Usuarios registrados</legend>
                              <div>
