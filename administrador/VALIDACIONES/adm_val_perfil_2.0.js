...
function validaPermisos(type){
    ...
    switch(tab){
        ...
        case 'Grupales':
            ...
            break;
        case 'Globales':
            $.extend(data,$('#permisosGlobalesForm').getData());
            data['perfiles']=$.isString(data['perfiles'])?(data['perfiles'].trim()!==''?[data['perfiles']]:[]):data['perfiles'];
            if(data['perfiles'].length===0) return $.alert("Debe seleccionar al menos un perfil!");
            msg='¿Est&aacute; seguro que desea '+(type==='remove'?'<b class="red">Remover</b>':'<b class="green">Agregar</b>')+' los <b class="blue">Permisos Globales<b> de Usuario a los  <b class="blue">Procesos</b> seleccionados?<br/><br/><b class="red"><u>NOTA:</u></b> Esta accion afectara a todos los perfiles seleccionados de todas las empresas.';
            break;    
    }    
    console.log(data);
