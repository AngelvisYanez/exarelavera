 ...
 * Act�a como m�ltiples usuarios para probar el flujo de trabajo completo de adquisici�n de $6,000.
 */

// Configurar entorno de simulaci�n
...
require_once(__DIR__ . '/../LOGICA/adq_adquisiciones_log.php');
require_once(__DIR__ . '/../LOGICA/wf_manager_log.php');
...
echo "==================================================================\n";
echo " INICIANDO SIMULACI�N COMPLETA DE PROCESO DE ADQUISICI�N ($6,000.00)\n";
echo "==================================================================\n\n";

// Asegurar que los usuarios de prueba est�n asignados a los departamentos correctos
...
$_SESSION['Ses_Lis_Per'] = array(3); // Perfil com�n
...
$data_solicitud = array(
    ...
    'Sol_Val_Est' => '6000.00',
    'Cdc_Cod' => '1',
    'Sol_Jus' => 'Adquisici�n de servidores de desarrollo para el equipo de TI.',
    'Sol_Det' => 'Se requiere la compra de 2 servidores rackeables para virtualizaci�n de entornos.'
);
...
echo "? Solicitud #$sol_num (ID: $sol_cod) creada con �xito por $6,000.00.\n";

// Forzar commit/actualizaci�n de lectura en esta conexi�n
mysqli_commit($obBD_conexion->conexion);
...
// ==================================================================
// PASO 2: Kleber (Usu_Cod = 4) aprueba en la etapa 'Aprobaci�n Jefatura' (Nod_Cod = 2)
// ==================================================================
...
$res_aprob_jefe = $wf_mgr->avanzarSiguientePaso($ins_cod, 2, 'APROBAR', 'Aprobado por el Jefe de TI. Cumple con las especificaciones t�cnicas requeridas.');

if (!$res_aprob_jefe) {
    die("? Error en la aprobaci�n de Jefatura.\n");
}

// Forzar commit/actualizaci�n de lectura en esta conexi�n
mysqli_commit($obBD_conexion->conexion);

// Verificar que el flujo avanz� y evalu� el nodo Decisi�n
$instancia = $obBD_con1->getRowConsultaSql("SELECT * FROM wf_instancias WHERE Ins_Cod = $ins_cod;", $obBD_conexion);
$nodo_act_cod = $instancia['Nod_Act'];
$nodo_act = $obBD_con1->getRowConsultaSql("SELECT * FROM wf_nodos WHERE Nod_Cod = $nodo_act_cod;", $obBD_conexion);

echo "? Aprobaci�n de Jefatura procesada.\n";
echo "?? Evaluaci�n de Decisi�n: Como el monto es $6,000.00 (> $5,000.00), el motor de flujos debi� saltarse el atajo directo e ir a Aprobaci�n de Gerencia.\n";
echo "?? Nodo Actual: ID $nodo_act_cod - '" . $nodo_act['Nod_Nom'] . "' (" . $nodo_act['Nod_Tip'] . ").\n";
if ($nodo_act_cod == 4) {
    echo "?? ��XITO! El motor de flujos enrut� correctamente la solicitud a la etapa de Aprobaci�n de Gerencia.\n\n";
} else {
    die("? ERROR: El flujo se desvi� a un nodo incorrecto (ID: $nodo_act_cod).\n");
}

// ==================================================================
// PASO 3: Administrador (Usu_Cod = 1) aprueba en la etapa 'Aprobaci�n Gerencia' (Nod_Cod = 4)
// ==================================================================
...
$res_aprob_gerente = $wf_mgr->avanzarSiguientePaso($ins_cod, 4, 'APROBAR', 'Aprobado por Gerencia General. Proceder con la adquisici�n de inmediato.');

if (!$res_aprob_gerente) {
    die("? Error en la aprobaci�n de Gerencia.\n");
}

// Forzar commit/actualizaci�n de lectura en esta conexi�n
mysqli_commit($obBD_conexion->conexion);

// Verificar que avanz� a Facturaci�n
$instancia = $obBD_con1->getRowConsultaSql("SELECT * FROM wf_instancias WHERE Ins_Cod = $ins_cod;", $obBD_conexion);
$nodo_act_cod = $instancia['Nod_Act'];
$nodo_act = $obBD_con1->getRowConsultaSql("SELECT * FROM wf_nodos WHERE Nod_Cod = $nodo_act_cod;", $obBD_conexion);

echo "? Aprobaci�n de Gerencia procesada.\n";
echo "?? Nodo Actual: ID $nodo_act_cod - '" . $nodo_act['Nod_Nom'] . "' (" . $nodo_act['Nod_Tip'] . ").\n\n";
...
if (!$res_factura) {
    die("? Error en la validaci�n de Factura.\n");
}

// Forzar commit/actualizaci�n de lectura en esta conexi�n
mysqli_commit($obBD_conexion->conexion);

// Verificar que el flujo finaliz�
$instancia = $obBD_con1->getRowConsultaSql("SELECT * FROM wf_instancias WHERE Ins_Cod = $ins_cod;", $obBD_conexion);
...
echo "?? Estado de la Solicitud de Adquisici�n: " . ($solicitud['Sol_Est'] == 'A' ? "?? APROBADA / COMPLETADA" : "EN FLUJO") . "\n\n";

if ($instancia['Ins_Est'] == 'F' && $solicitud['Sol_Est'] == 'A') {
    echo "==================================================================\n";
    echo " ?? SIMULACI�N COMPLETADA CON �XITO ABSOLUTO ??\n";
    echo " El motor de flujos, la l�gica de decisiones por monto y los\n";
    echo " permisos de enrutamiento funcionan al 100% de forma correcta.\n";
    echo "==================================================================\n";
} else {
    die("? ERROR: El flujo no se complet� correctamente.\n");
}
