...
$version_nombre = "N/A";
if ($filtros['Ppe_Cod']) {
    $res_ver = $mysqli_conn->query("SELECT Ppe_Ver AS Ppe_Ver, Ppe_Des AS Ppe_Des FROM pre_presupuesto WHERE Ppe_Cod = " . $filtros['Ppe_Cod']);
    if ($res_ver && $v_row = $res_ver->fetch_assoc()) {
        $version_nombre = "V" . $v_row['Ppe_Ver'] . " (" . $v_row['Ppe_Des'] . ")";
    }
}

$proyecto_nombre = "Todos los Proyectos";
if ($filtros['Pro_Cod']) {
    $res_pr = $mysqli_conn->query("SELECT Pro_Nom AS Pro_Nom FROM pre_proyectos WHERE Pro_Cod = '" . $mysqli_conn->real_escape_string($filtros['Pro_Cod']) . "' LIMIT 1");
    if ($res_pr && $pr_row = $res_pr->fetch_assoc()) {
        $proyecto_nombre = $pr_row['Pro_Nom'] . " (" . $filtros['Pro_Cod'] . ")";
    }
}
