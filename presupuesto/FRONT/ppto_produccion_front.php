...
<div id="modalAprobarMesPreview" class="exa-pre-modal-overlay" style="display:none;">
    <div class="exa-pre-modal-box" style="width:680px;max-width:96%;">
        ...
        <div class="pub-kpi">
            <div class="item"><div class="lbl">Mes</div><div class="val" id="apMesNom">â€”</div></div>
...
function cargarConfigProyecto(){
  ...
  $.getJSON(API, {action:'get_config', Pro_Cod:proy}, function(r){
    if(r.config){
      var c=r.config;
...
function loadProyectos(){
  $.getJSON(API, {action:'proyectos'}, function(r){
    ...
    $.each(r.rows||[], function(i,p){
      var label = p.label || ((p.Pro_Codigo||p.proy_codigo||'') + ' - ' + p.Pro_Nom);
      opts+='<option value="'+p.Pro_Cod+'">'+label+'</option>';
    });
...
function renderD2(d2){
  ...
  $('#d2_rubro').text(
    (d2.partida_driver_codigo ? d2.partida_driver_codigo + ' â€” ' : '') + (d2.rubro_driver || 'Rubro driver')
  );
  $('#d2_plan').text(formatNumber(proyPer, 2));
  $('#d2_base').text(formatNumber(realPer, 2));
  $('#d2_base_mes').text(mesesReal+' mes(es) Â· base '+formatNumber(d2.ton_base_mensual||0, 2)+' Ton/mes');
...
function loadD2(){
  ...
  $.getJSON(API, {action:'divergencia_d2', Pro_Cod:proy, anio:anio}, function(r){
...
function loadCfg(){
  $.getJSON(API, {action:'list'}, function(r){
    ...
    $.each(r.rows||[], function(i,c){
      ...
      tb.append('<tr><td>'+c.Pro_Nom+'</td><td>'+(ORIGEN_LABEL[orig]||c.pco_origen)+'</td></tr>');
    });
  });
}
...
function construirTooltipAprob(mes){
  ...
  var dSig=delta>0?'+':'';
  var modoTxt=(a.modo==='proyectada_mes')?'Proyectada (mes)':(a.modo||'â€”');
  ...
  return ''
    ...
    +'<span class="tt-title">Aprobado Â· '+mesNombre(mes)+'</span>'
    +'<span class="tt-sub">'+fechaSoloDia(a.fecha)+'</span></div>'
    +'<div class="tt-amount"><div class="lbl">Presupuesto aprobado'+(veces>1?(' Â· vig. #'+veces):'')+'</div>'
    ...
    +'<span class="v">'+fechaSoloDia(a.fecha)+(fechaSoloHora(a.fecha)?(' Â· '+fechaSoloHora(a.fecha)):'')+'</span></div>'
    ...
    +vecesRow
    +'</div>'
    +'<div class="tt-foot"><span>Reg. #'+(a.pub_id||'â€”')+'</span>'+footHist+'</div>';
}
function abrirHistorialMes(mes){
  var proy=$('#plan_proy_id').val();
  if(!proy){ return; }
  ocultarTooltipAprob();
  $('#histMesTitulo').text('Historial de aprobaciones Â· '+mesNombre(mes));
  $('#histMesBody').html('<div class="text-center text-muted" style="padding:20px;">Cargandoâ€¦</div>');
  $('#modalHistMes').show();
  $.getJSON(API, {action:'hist_aprobaciones_mes', Pro_Cod:proy, anio:$('#prd_anio').val(), prd_mes:mes}, function(r){
    if(r.status!=='success'){ $('#histMesBody').html('<div class="alert alert-danger">'+(r.message||'Error')+'</div>'); return; }
    var rows=r.rows||[];
    if(!rows.length){ $('#histMesBody').html('<div class="alert alert-info" style="font-size:12px;">Este mes aÃºn no tiene aprobaciones registradas.</div>'); return; }
    var h='<div style="font-size:11px;color:#718096;margin-bottom:10px;">'+rows.length+' aprobaciÃ³n(es) registradas. La mÃ¡s reciente es la vigente.</div>';
    ...
    $.each(rows, function(i,x){
      ...
      var badge=x.es_actual
        ...
        : '<span class="label label-default">HistÃ³rico</span>';
...
function ejecutarAprobarMes(confirmarReaprobacion){
  ...
  $.post(API, postData, function(r){
    if(r.status==='confirm'){
      if(!confirm(r.message||'Â¿Reaprobar este mes?')){ return; }
...
function loadPlan(){
  $.getJSON(API, {action:'list_periodos', Pro_Cod:$('#plan_proy_id').val(), anio:$('#prd_anio').val()}, function(r){
    var map={};
...
$('#btnSaveCfg').click(function(){
  $.post(API, {action:'save_config', Pro_Cod:$('#cfg_proy_id').val(), pco_origen:$('#pco_origen').val(), pco_campo:$('#pco_campo').val(), pco_extra_config:$('#pco_extra_config').val()}, function(r){
    toast(r.message,r.status==='success');
    loadCfg();
    actualizarPanelRelavera();
  }, 'json');
...
$('#btnSync').click(function(){
  $.post(API, {action:'sync_relavera', Pro_Cod:$('#cfg_proy_id').val(), anio:$('#prd_anio').val()}, function(r){ toast(r.message,r.status==='success'); loadPlan(); }, 'json');
});
