...
$Pro_Nom = $Pro_Cod;
$r_proy = $mysqli->query("SELECT Pro_Nom AS Pro_Nom FROM pre_proyectos
    WHERE Pro_Cod='" . $mysqli->real_escape_string($Pro_Cod) . "' AND Emp_Cod=" . (int)$Emp_Cod . " LIMIT 1");
if ($r_proy && ($rp = $r_proy->fetch_assoc())) {
    $Pro_Nom = $rp['Pro_Nom'] . ' (' . $Pro_Cod . ')';
}

$ver_nombre = 'V' . $Ppe_Cod;
$r_ver = $mysqli->query("SELECT Ppe_Ani AS Ppe_Ani, Ppe_Ver AS Ppe_Ver FROM pre_presupuesto WHERE Ppe_Cod=" . (int)$Ppe_Cod . " LIMIT 1");
if ($r_ver && ($rv = $r_ver->fetch_assoc())) {
    $ver_nombre = (int)$rv['Ppe_Ani'] . ' V' . (int)$rv['Ppe_Ver'];
}
...
function ppto_cuadro_export_rubro_monto($row, $esc_key) {
    ...
    return round((float)$row['Pdp_PreAnual'], 2);
}
...
$safe_proy = preg_replace('/[^A-Za-z0-9_\-]/', '_', $Pro_Cod);
...
<p>
    Proyecto: <?php echo htmlspecialchars($Pro_Nom); ?> |
...
<?php
        foreach ($g['rows'] as $r):
            $monto = ppto_cuadro_export_rubro_monto($r, $esc_key);
            $ton_rubro = isset($r['Pdp_TonBase']) ? (float)$r['Pdp_TonBase'] : 0.0;
            if ($ton_rubro <= 0) {
                $ton_rubro = $ton_costo;
            }
            $fac_r = isset($r['Pdp_FacAnualTon']) ? (float)$r['Pdp_FacAnualTon'] : 0.0;
...
?>
        <tr>
            ...
            <td><?php echo htmlspecialchars(isset($r['Ppa_Cla']) ? $r['Ppa_Cla'] : ''); ?></td>
