...
// 2.1 Proyectos
$res_proyectos = $mysqli_conn->query("SELECT Pro_Cod AS Pro_Cod, Pro_Ide AS Pro_Codigo, CONCAT(Pro_Ide, ' - ', Pro_Nom) AS Pro_Nom FROM pre_proyectos WHERE Emp_Cod = " . $filtros['Emp_Cod'] . " AND Pro_Est = 'A' ORDER BY Pro_Nom");
$proyectos = array();
if ($res_proyectos) {
...
// Default alcance Proyecto: primer proyecto activo si no viene en la URL.
if ((!isset($filtros['alcance']) || $filtros['alcance'] === 'proyecto')
    && (empty($filtros['Pro_Cod']) || $filtros['Pro_Cod'] === null)
    && !empty($proyectos)
) {
    $filtros['alcance'] = 'proyecto';
    $filtros['Pro_Cod'] = (string)$proyectos[0]['Pro_Cod'];
}

$dash_inicial_relavera = false;
if (!empty($filtros['Pro_Cod']) && $mysqli_conn) {
...
$res_cabeceras = $mysqli_conn->query("SELECT Ppe_Cod AS Ppe_Cod, Ppe_Ver AS Ppe_Ver, Ppe_Des AS Ppe_Des, Ppe_Est AS Ppe_Est FROM pre_presupuesto WHERE Emp_Cod = " . $filtros['Emp_Cod'] . " AND Ppe_Ani = " . $filtros['anio'] . " ORDER BY Ppe_Ver DESC");
$versiones = array();
if ($res_cabeceras) {
...
// Siempre usar cabecera activa del anio (sin selector de version en UI).
$ver_activa = ppto_presupuesto_pick_activo_from_list($versiones);
if ($ver_activa) {
    $filtros['Ppe_Cod'] = (int)$ver_activa['Ppe_Cod'];
} elseif (!$filtros['Ppe_Cod']) {
    $act = ppto_presupuesto_activo($mysqli_conn, (int)$filtros['Emp_Cod'], (int)$filtros['anio']);
    if ($act) {
        $filtros['Ppe_Cod'] = (int)$act['Ppe_Cod'];
    }
}
...
<head>
    ...
    <style>
        ...
        .exa-dash-filter-row + .exa-dash-filter-row {
            margin-top: 12px;
            padding-top: 12px;
        ...
        .exa-dash-filter-field-anio { width: 88px; flex: 0 0 88px; }
        .exa-dash-filter-field-proyecto { flex: 1 1 220px; min-width: 180px; max-width: 360px; }
        .exa-dash-filter-field-refresh { width: 96px; flex: 0 0 96px; }
        .exa-dash-filter-btn {
            width: 36px;
            flex: 0 0 36px;
        }
        ...
        .exa-dash-filter-field-alcance {
            flex: 0 0 auto;
        }
        .exa-dash-filter-field-periodo {
            flex: 0 0 auto;
        }
        .exa-dash-filter-field-modo {
            flex: 0 0 auto;
        }
        ...
        .exa-dash-filter-field-alcance .exa-dash-segment,
        .exa-dash-filter-field-modo .exa-dash-segment {
            height: 32px;
            flex: 0 0 auto;
            display: inline-flex;
            align-items: stretch;
        ...
        .exa-dash-filter-field-alcance .exa-dash-segment-btn,
        .exa-dash-filter-field-modo .exa-dash-segment-btn {
        ...
        .exa-dash-filter-hint strong { color: #2d3748; }
        @media (max-width: 900px) {
            ...
            .exa-dash-filter-field-proyecto {
                max-width: none;
            }
        }
        .tab-navigation {
        ...
        button.exa-dash-segment-btn,
        a.exa-dash-segment-btn {
        ...
        button.exa-dash-segment-btn.active {
            background: #2b6cb0 !important;
            color: #fff !important;
            box-shadow: none !important;
        }
        .exa-dash-segment-btn.active:hover,
        button.exa-dash-segment-btn.active:hover {
            background: #2c5282 !important;
            color: #fff !important;
        }
        .exa-dash-segment-btn:hover:not(.active),
        button.exa-dash-segment-btn:hover:not(.active) {
...
<div class="panel panel-main exa-ui-panel exa-ui-fill-page">
    <div class="panel-heading exa-header exa-header-flex">
        <h3 class="panel-title"><i class="bi bi-speedometer2"></i> Dashboard presupuestario</h3>
        <div class="exa-header-actions" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            <div class="exa-dash-segment" role="navigation" aria-label="Ir a m&oacute;dulos de presupuesto">
                <a href="ppto_proyectos_front.php" class="exa-dash-segment-btn" title="Presupuesto proyectos"><i class="bi bi-folder2-open"></i> Proyectos</a>
                <a href="ppto_admin_front.php" class="exa-dash-segment-btn" title="Presupuesto general"><i class="bi bi-pie-chart"></i> General</a>
            </div>
    ...
    <div class="panel-body exa-body" style="padding: 20px;">
        <?php
        $alcance_ui = isset($filtros['alcance']) ? $filtros['alcance'] : 'proyecto';
        if ($alcance_ui !== 'general' && $alcance_ui !== 'proyecto' && $alcance_ui !== 'consolidado') {
            $alcance_ui = 'proyecto';
        }
        ?>
        <!-- SECCION DE FILTROS -->
        <div class="filter-section">
            <form id="form_filtros" class="exa-dash-filter-form">
                <input type="hidden" name="Emp_Cod" id="Emp_Cod" value="<?php echo (int)$filtros['Emp_Cod']; ?>" />
                <input type="hidden" name="alcance" id="alcance" value="<?php echo htmlspecialchars($alcance_ui); ?>" />
                <div class="exa-dash-filter-row">
                    <div class="exa-dash-filter-field exa-dash-filter-field-alcance">
                        <label>Alcance</label>
                        <div class="exa-dash-segment" id="seg_alcance" role="group" aria-label="Alcance del dashboard">
                            <button type="button" class="exa-dash-segment-btn<?php echo ($alcance_ui === 'proyecto' ? ' active' : ''); ?>" data-alcance="proyecto" title="Un proyecto (rubros Relavera/RCET)">Proyecto</button>
                            <button type="button" class="exa-dash-segment-btn<?php echo ($alcance_ui === 'general' ? ' active' : ''); ?>" data-alcance="general" title="Solo plan corporativo (Mensual/Admin)">General</button>
                            <button type="button" class="exa-dash-segment-btn<?php echo ($alcance_ui === 'consolidado' ? ' active' : ''); ?>" data-alcance="consolidado" title="General + todos los proyectos">Consolidado</button>
                        </div>
                    </div>
                    <div class="exa-dash-filter-field exa-dash-filter-field-anio">
                    ...
                    <div class="exa-dash-filter-field exa-dash-filter-field-proyecto" id="wrap_Pro_Cod">
                        <label for="Pro_Cod">Proyecto</label>
                        <select name="Pro_Cod" id="Pro_Cod" class="form-control input-sm" <?php echo ($alcance_ui !== 'proyecto' ? 'disabled="disabled"' : ''); ?>>
                            <option value="">Seleccione proyecto</option>
                            <?php foreach ($proyectos as $p): ?>
                                <option value="<?php echo htmlspecialchars($p['Pro_Cod']); ?>" <?php echo ((string)$p['Pro_Cod'] === (string)$filtros['Pro_Cod'] ? 'selected' : ''); ?>>
                                    <?php echo htmlspecialchars($p['Pro_Nom']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <input type="hidden" name="ver" id="ver" value="<?php echo (int)$filtros['Ppe_Cod']; ?>" />
                    <div class="exa-dash-filter-field exa-dash-filter-field-refresh">
                ...
                <div class="exa-dash-filter-row">
                    ...
                    <div class="exa-dash-filter-field exa-dash-filter-field-modo">
                        <label>Modo</label>
                    ...
                    <span class="exa-dash-filter-hint periodo-lbl" id="periodo_label_hint"></span>
                </div>
        ...
        <div id="ppto_dash_alcance_hint" class="alert alert-info" style="margin:0 0 14px;padding:10px 14px;font-size:12px;">
            <strong>Alcance:</strong>
            <span data-alcance-hint="general" style="<?php echo $alcance_ui === 'general' ? '' : 'display:none;'; ?>">solo presupuesto general (carga mensual corporativa).</span>
            <span data-alcance-hint="proyecto" style="<?php echo $alcance_ui === 'proyecto' ? '' : 'display:none;'; ?>">un proyecto (rubros Relavera/RCET).</span>
            <span data-alcance-hint="consolidado" style="<?php echo $alcance_ui === 'consolidado' ? '' : 'display:none;'; ?>">general + todos los proyectos (vista ejecutiva).</span>
            &nbsp;|&nbsp;
            <a href="ppto_admin_front.php">Presupuesto general</a> &middot;
            <a href="ppto_proyectos_front.php">Presupuesto proyectos</a>
        </div>

        <div id="ppto_dash_sin_proy_hint">
            <strong>Dashboard Relavera:</strong> seleccione alcance <em>Proyecto</em> y un proyecto con origen <em>Relavera</em>
...
<script>
    ...
    jQuery(document).ready(function($) {
        ...
        function pptoDashSyncAlcanceUi() {
            var a = $('#alcance').val() || 'proyecto';
            $('#seg_alcance .exa-dash-segment-btn').removeClass('active');
            $('#seg_alcance .exa-dash-segment-btn[data-alcance="' + a + '"]').addClass('active');
            var esProy = (a === 'proyecto');
            $('#Pro_Cod').prop('disabled', !esProy);
            $('#wrap_Pro_Cod').css('opacity', esProy ? 1 : 0.55);
            $('#ppto_dash_alcance_hint [data-alcance-hint]').hide();
            $('#ppto_dash_alcance_hint [data-alcance-hint="' + a + '"]').show();
        }

        function pptoDashAsegurarProyectoSeleccionado() {
            var $sel = $('#Pro_Cod');
            if (($sel.val() || '').trim()) return true;
        ...
        function pptoDashCambiarAlcance(alcance) {
            if (alcance !== 'general' && alcance !== 'proyecto' && alcance !== 'consolidado') {
                alcance = 'proyecto';
            }
            $('#alcance').val(alcance);
            pptoDashSyncAlcanceUi();
            if (alcance === 'proyecto') {
                if (!pptoDashAsegurarProyectoSeleccionado()) {
                    var colsWait = pptoDashColCountPartidas();
                    $('#tbody_partidas').html('<tr><td colspan="' + colsWait + '" class="text-center text-muted" style="padding:40px;">No hay proyectos activos. Cree uno en Presupuesto proyectos.</td></tr>');
                    return;
                }
            }
            cargarDashboard();
        ...
        $('#seg_alcance').on('click', '.exa-dash-segment-btn', function(e) {
            e.preventDefault();
            e.stopPropagation();
            pptoDashCambiarAlcance($(this).attr('data-alcance') || $(this).data('alcance'));
        });
        pptoDashSyncAlcanceUi();
        if (($('#alcance').val() || '') === 'proyecto') {
            pptoDashAsegurarProyectoSeleccionado();
        }

        $('#ani, #Pro_Cod, #mes').on('change', function() {
            var triggerId = $(this).attr('id');
            if (triggerId === 'ani') {
                actualizarCatalogosFiltros();
            } else if (triggerId === 'Pro_Cod') {
                if (($('#alcance').val() || '') !== 'proyecto') {
                    $('#alcance').val('proyecto');
                    pptoDashSyncAlcanceUi();
                }
                cargarDashboard();
            } else {
    ...
    function pptoDashCambiarVistaPartidas(vista) {
        // Gerente clÃ¡sico: capÃ­tulos. TÃ©cnico puede elegir CapÃ­tulos o Solo rubros.
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status === 'success') {
                    // Cabecera activa del anio (sin selector de version)
                    var ppeActivo = 0;
                    if (res.versiones && res.versiones.length) {
                        ...
                        if (!ppeActivo) {
                            ppeActivo = res.versiones[0].Ppe_Cod;
                        }
                    }
                    $('#ver').val(ppeActivo || '');

                    // Actualizar dropdown de proyectos
                    var selectProy = $('#Pro_Cod');
                    var prevProy = selectProy.val();
                    selectProy.empty();
                    selectProy.append('<option value="">Seleccione proyecto</option>');
                    $.each(res.proyectos, function(i, p) {
                        selectProy.append($('<option>', { value: p.Pro_Cod, text: p.Pro_Nom }));
                    });
                    if (prevProy) {
                        selectProy.val(prevProy);
                    }
                    pptoDashSyncAlcanceUi();
                }
    ...
    function cargarDashboard() {
        var alcance = $('#alcance').val() || 'proyecto';
        if (alcance === 'proyecto') {
            if (typeof pptoDashAsegurarProyectoSeleccionado === 'function') {
                pptoDashAsegurarProyectoSeleccionado();
            }
            if (!($('#Pro_Cod').val() || '').trim()) {
                var colsWait = pptoDashColCountPartidas();
        ...
        // Asegurar que Pro_Cod viaje en serialize cuando aplica
        var proyDisabled = $('#Pro_Cod').prop('disabled');
        if (alcance === 'proyecto' && proyDisabled) {
            $('#Pro_Cod').prop('disabled', false);
        }
        var data_filtros = $('#form_filtros').serialize() + '&action=fetch';
        if (alcance !== 'proyecto') {
            $('#Pro_Cod').prop('disabled', true);
        }
                ...
                if (res.status === 'success') {
                    ...
                    if (d2Proy !== undefined && d2Real !== undefined) {
                        ...
                        if (d2.meses_con_real) {
                            d2Det += ' Â· ' + d2.meses_con_real + ' mes(es)';
                        }
                        if (d2.rubro_driver) {
                            d2Det += ' Â· ' + d2.rubro_driver;
                        }
                    }
                    ...
                    if (d2.warning && d2.mensaje) {
                        ...
                        var d2Sum = d2.requiere_reajuste
                            ? 'Proyectada corta vs real (' + formatNumber(d2.pct_diferencia || 0, 2) + '%) â€” reajustar'
