 ...
 * Retorna respuestas estrictas en formato JSON {status, kpis, partidas, mensual} o catï¿½logos dinï¿½micos.
 */

// Iniciar almacenamiento en bï¿½fer de salida inmediatamente para prevenir cualquier fuga de texto/advertencias
if (!ob_get_level()) {
    @ob_start();
}

// Desactivar DebugBar si estï¿½ cargado para que no inyecte scripts/HTML en el JSON
...
// Funciï¿½n de respuesta JSON unificada que limpia el bï¿½fer de salida antes de enviar los datos
...
if ($action === 'catalogos') {
    ...
    $res_proy = $mysqli_conn->query("SELECT Pro_Cod AS Pro_Cod, Pro_Ide AS Pro_Codigo, CONCAT(Pro_Ide, ' - ', Pro_Nom) AS Pro_Nom FROM pre_proyectos WHERE Emp_Cod = $Emp_Cod AND Pro_Est = 'A' ORDER BY Pro_Nom");
    $proyectos = array();
    if ($res_proy) {
    ...
    // 2. Versiones presupuestarias para ese aï¿½o y empresa
    $res_ver = $mysqli_conn->query("SELECT Ppe_Cod AS Ppe_Cod, Ppe_Ver AS Ppe_Ver, Ppe_Des AS Ppe_Des, Ppe_Est AS Ppe_Est FROM pre_presupuesto WHERE Emp_Cod = $Emp_Cod AND Ppe_Ani = $anio ORDER BY Ppe_Ver DESC");
    $versiones = array();
    if ($res_ver) {
...
if ($action === 'fetch') {
    ...
    // Sin selector de version: forzar cabecera activa del anio.
    $actDash = ppto_presupuesto_activo($mysqli_conn, (int)$filtros['Emp_Cod'], (int)$filtros['anio']);
    if ($actDash) {
        $filtros['Ppe_Cod'] = (int)$actDash['Ppe_Cod'];
    }

    try {
        // Ejecutar consultas de KPIs, desglose por partidas y evoluciï¿½n mensual
        ...
        if ($filtros['Ppe_Cod'] && $filtros['Pro_Cod'] !== null && trim($filtros['Pro_Cod']) !== '') {
            ...
            if ($modo_reinversion) {
                ...
                // Saldo formal = techo PDF del periodo âˆ’ comprometido âˆ’ gastado
...
// Acciï¿½n no soportada
ppto_ajax_responder_json(array(
    'status' => 'error',
    'error'  => 'Acciï¿½n no definida o no permitida en este extremo.'
));
