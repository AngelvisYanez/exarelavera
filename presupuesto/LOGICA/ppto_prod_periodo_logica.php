 ...
 * Pel_MotorVer 'v2' = motor unico Fase 2B (ton_ref por Prd_Est).
 ...
 */
function ppto_prod_pf_snapshot($mysqli, $Pro_Cod, $Emp_Cod, $anio, $mes) {
    $Ppe_Cod = ppto_persistencia_consultar($mysqli, 1, array('Emp_Cod' => (int)$Emp_Cod, 'Ppe_Ani' => (int)$anio));
    $filtros = array(
        'Emp_Cod'  => (int)$Emp_Cod,
        'anio'    => (int)$anio,
        'mes'     => (int)$mes,
        'Ppe_Cod'  => $Ppe_Cod ? (int)$Ppe_Cod : null,
        'Pro_Cod' => $Pro_Cod,
        'Ppa_Cod'  => null,
    );
 ...
 * @return array|null
 */
function ppto_prod_periodo_obtener_fila($mysqli, $Pro_Cod, $Emp_Cod, $anio, $mes, $for_update = false) {
    $clean = $mysqli->real_escape_string(trim($Pro_Cod));
    ...
    $sql = "SELECT Prd_Cod AS prd_id, Pro_Cod AS proy_id, Emp_Cod, Prd_Anio AS prd_anio, Prd_Mes AS prd_mes,
            ...
            WHERE Pro_Cod='$clean' AND Emp_Cod=" . (int)$Emp_Cod . "
 ...
 * @return bool
 */
function ppto_prod_periodo_insert_evento_log($mysqli, $obDatos, $Pro_Cod, $Emp_Cod, $anio, $mes,
    $tipo, $origen, $Usu_Cod, $real_antes, $real_despues, $pf_antes, $pf_despues, $motivo, $motor_version) {

    $clean_proy = $mysqli->real_escape_string(trim($Pro_Cod));
    ...
    $sql = "INSERT INTO pre_prod_evento_log
            (Pro_Cod, Emp_Cod, Pel_Anio, Pel_Mes, Pel_Tipo, Pel_Origen, Usu_Cod, Pel_FecEvt,
             Pel_RealAntes, Pel_RealDespues, Pel_PfAntes, Pel_PfDespues, Pel_Motivo, Pel_MotorVer)
            VALUES
            ('$clean_proy', " . (int)$Emp_Cod . ", " . (int)$anio . ", " . (int)$mes . ",
 ...
 */
function ppto_prod_periodo_cerrar($mysqli, $obDatos, $Pro_Cod, $Emp_Cod, $anio, $mes, $Usu_Cod, $real_val, $Ses_Dat_Aut, $request_uri) {
    ...
    $fila = ppto_prod_periodo_obtener_fila($mysqli, $Pro_Cod, $Emp_Cod, $anio, $mes, true);
    ...
    $pf_antes = ppto_prod_pf_snapshot($mysqli, $Pro_Cod, $Emp_Cod, $anio, $mes);
    ...
    if ($fila) {
        $sql = "UPDATE pre_prod_periodos
                SET Prd_Real = $real_val,
                    ...
                    Prd_FecReg = NOW(),
                    Usu_Cod = " . (int)$Usu_Cod . "
                WHERE Pro_Cod='$clean_proy' AND Emp_Cod=" . (int)$Emp_Cod . "
                  AND Prd_Anio=" . (int)$anio . " AND Prd_Mes=" . (int)$mes;
    ...
    if (function_exists('ppto_integracion_variacion_calcular')) {
        ppto_integracion_variacion_calcular($mysqli, $Pro_Cod, $mes, $anio);
    }
 ...
 * @return array {ok: bool, message: string}
 */
function ppto_prod_periodo_reabrir($mysqli, $obDatos, $Pro_Cod, $Emp_Cod, $anio, $mes, $Usu_Cod, $motivo, $Ses_Dat_Aut, $request_uri) {
    $obDatos->sentencias = '';
    $obDatos->codigos = '';
    $obDatos->inicio_transaccion($mysqli);
    ...
    $pf_despues = ppto_prod_pf_snapshot($mysqli, $Pro_Cod, $Emp_Cod, $anio, $mes);
...
/**
 * Corrige real en periodo cerrado (sync tardio). No cambia Prd_Est.
 *
 * @return array {ok: bool, message: string}
 */
function ppto_prod_periodo_corregir_real_cerrado($mysqli, $obDatos, $Pro_Cod, $Emp_Cod, $anio, $mes, $nuevo_real, $origen, $Usu_Cod, $Ses_Dat_Aut, $request_uri) {
