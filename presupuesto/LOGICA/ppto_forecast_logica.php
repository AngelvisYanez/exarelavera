 ...
 * Motor unico de forecast presupuestario (Fase 2B â€” D1).
 */

require_once __DIR__ . '/ppto_proy_perfil_logica.php';

/**
 * Carga produccion mes a mes incluyendo Prd_Est para ton_ref.
 *
 * @param mysqli $mysqli
 * @param int $Emp_Cod
 * @param int $anio
 ...
 * @return array
 */
function ppto_forecast_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $Pro_Cod) {
    $meses = array();
    for ($m = 1; $m <= 12; $m++) {
        $meses[$m] = array(
            'esperada'   => 0.0,
    ...
    $res = $mysqli->query("SELECT Prd_Mes AS prd_mes, Prd_Esperada AS prd_esperada, Prd_Real AS prd_real,
            Prd_Proyectada AS prd_proyectada, COALESCE(Prd_Est, 'sin_dato') AS prd_estado
        FROM pre_prod_periodos
        WHERE Emp_Cod = " . (int)$Emp_Cod . " AND Prd_Anio = " . (int)$anio . " AND Pro_Cod = '$esc'");
    if ($res) {
...
/**
 * Cuenta meses con Prd_Est=cerrado.
 *
 * @param array $meses_prod
 * @return int
 */
...
/**
 * Semï¿½foro unificado (umbrales dashboard).
 *
 * @param float $vigente
 ...
 * @return string
 */
function ppto_forecast_metodo_proyecto($mysqli, $Pro_Cod, $Emp_Cod) {
    if ($Pro_Cod === null || trim($Pro_Cod) === '') {
        return 'produccion_proyectada';
    }
    $esc = $mysqli->real_escape_string(trim($Pro_Cod));
    $res = $mysqli->query("SELECT Pco_MetodoFc AS pco_metodo_forecast FROM pre_prod_config
        WHERE Pro_Cod='$esc' AND Emp_Cod=" . (int)$Emp_Cod . " LIMIT 1");
    if ($res && ($row = $res->fetch_assoc())) {
        $m = $row['pco_metodo_forecast'];
        if ($m === 'produccion_proyectada') {
            return $m;
        }
        // promedio_historico | manual: ENUM sin logica ï¿½ fallback produccion_proyectada
    }
    return 'produccion_proyectada';
}
 ...
 *
 * @param mysqli $mysqli
 * @param array $kpis Base KPIs dashboard (disponible = plan)
 * @param int $Ppe_Cod
 * @param int $Emp_Cod
 * @param int $anio
 * @param int|null $mes
 ...
 */
function ppto_forecast_calcular_global($mysqli, $kpis, $Ppe_Cod, $Emp_Cod, $anio, $mes, $Pro_Cod) {
    ...
    // disponible / semaforo legacy: plan (no se sobrescribe con forecast ï¿½ D4)
    ...
    $metodo = ppto_forecast_metodo_proyecto($mysqli, $Pro_Cod, $Emp_Cod);
    if ($metodo !== 'produccion_proyectada') {
        return $kpis;
    }

    $meses = ppto_forecast_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $Pro_Cod);
 ...
 * driver = rubro en pre_proyecto_detalles con factor; fijo = resto.
 *
 * @param int $Ppa_Cod
 ...
 */
function ppto_forecast_driver_tipo_partida($Ppa_Cod, $rubros_map, $usa_toneladas) {
    if ($usa_toneladas && isset($rubros_map[(int)$Ppa_Cod])) {
        return 'driver';
    }
    return 'fijo';
}
...
/**
 * Aplica reglas D7 al total de capÃ­tulo: PF fijo=VA; mixto=suma hijos; % sobre totales.
 *
 * @param array $sum
 * @return array
 */
...
function ppto_forecast_calcular_partida($row, $meses_prod, $mes, $rubros_map, $usa_toneladas, $modo_reinversion = false, $periodo_vista = null) {
    ...
    if ($usa_toneladas && isset($rubros_map[$ppa_key])) {
        ...
        if ($modo_reinversion) {
            ...
            // Proyectada = brÃºjula. Formalizar = solo toneladas reales del periodo.
