...
if (!function_exists('ppto_proy_es_modo_reinversion')) {
     ...
     * @return bool
     */
    function ppto_proy_es_modo_reinversion($mysqli, $Pro_Cod, $Emp_Cod) {
        if ($Pro_Cod === null || trim($Pro_Cod) === '') {
            return false;
        }
        $esc = $mysqli->real_escape_string(trim($Pro_Cod));
        $Emp_Cod = (int)$Emp_Cod;
        $res = $mysqli->query("SELECT Pco_Origen FROM pre_prod_config
            WHERE Pro_Cod='$esc' AND Emp_Cod=$Emp_Cod LIMIT 1");
