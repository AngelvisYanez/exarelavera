...
/**
 * Toneladas base PDF por mes por defecto (3500 x 22 = 77000) â€” egreso/costo Excel.
 *
 * @return float
 */
...
function ppto_pdf_siguiente_tiene_datos_rubro($raw, $idx, $total) {
    ...
    while ($k < $total) {
        ...
        if (preg_match('/^[A-Za-zÃ¡Ã©Ã­Ã³ÃºÃ±ÃÃ‰ÃÃ“ÃšÃ‘]/u', $cell) && !ppto_pdf_es_celda_monto($cell) && !preg_match('/^\d+$/', $cell)) {
            if (count($cells) === 0) {
                if (ppto_pdf_es_linea_componente_sin_datos($cell)) {
                    return false;
                }
...
function ppto_pdf_es_linea_componente_sin_datos($line) {
    ...
    if (preg_match('/^(Automatizaci|Adecuaci|Electricidad|Pintura|SeÃ±al|Senal|senal|chasis|herrajes|acoples|Trazabilidad|Guias|Volquetas|Control de)/i', $line)) {
        return true;
    }
    return false;
}
...
function ppto_pdf_es_linea_lista_catalogo($line, $raw = null, $idx = null, $total = 0) {
    ...
    if (preg_match('/^[A-Za-zÃ¡Ã©Ã­Ã³ÃºÃ±ÃÃ‰ÃÃ“ÃšÃ‘]/u', $line) && strlen($line) <= 120) {
        return true;
    }
    return false;
}
...
function ppto_pdf_es_linea_solo_totales($line) {
    ...
    if (!preg_match('/[A-Za-zÃ¡Ã©Ã­Ã³ÃºÃ±ÃÃ‰ÃÃ“ÃšÃ‘]/u', $line) && preg_match('/\d{1,3}\.\d{3},\d{2}/', $line)) {
        return true;
    }
    return false;
}
...
function ppto_pdf_es_continuacion_descripcion($line) {
    ...
    if (preg_match('/^[a-zÃ¡Ã©Ã­Ã³ÃºÃ±]/u', $line)) {
        return true;
    }
    return false;
}
...
function ppto_pdf_es_linea_cabecera_tabla($line) {
    ...
    $line_l = str_replace(array('ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½'), array('a', 'e', 'i', 'o', 'u', 'n'), $line_l);
    ...
    while ($i < $total) {
        ...
        if (preg_match('/^[A-D]$/i', $line) && ($i + 1) < $total) {
            ...
            if ($next !== '' && !ppto_pdf_es_linea_componente_sin_datos($next)
                && (preg_match('/^[A-ZÃÃ‰ÃÃ“ÃšÃ‘]/u', $next) || ppto_pdf_es_titulo_seccion($next))
                && !preg_match('/^\d/', $next)) {
        ...
        if (!preg_match('/^[A-Za-zÃ¡Ã©Ã­Ã³ÃºÃ±ÃÃ‰ÃÃ“ÃšÃ‘]/u', $line) && ppto_pdf_es_celda_monto($line)) {
            $i++;
            continue;
        }

        if (preg_match('/^[A-Za-zÃ¡Ã©Ã­Ã³ÃºÃ±ÃÃ‰ÃÃ“ÃšÃ‘]/u', $line)) {
            ...
            while ($j < $total) {
                ...
                if (preg_match('/^[A-Za-zÃ¡Ã©Ã­Ã³ÃºÃ±ÃÃ‰ÃÃ“ÃšÃ‘]/u', $cell) && !preg_match('/^\d/', $cell)) {
 ...
 * @return array
 */
function ppto_pdf_catalogo_por_codigos($mysqli, $Emp_Cod, $codigos) {
    $map = array();
    $Emp_Cod = (int)$Emp_Cod;
    if (!$mysqli || $Emp_Cod <= 0 || empty($codigos)) {
        return $map;
    }
    ...
    $sql = "SELECT Ppa_Cod AS Ppa_Cod, Ppa_Cla AS Ppa_Cla,
                   COALESCE(NULLIF(Ppa_Clase, ''), 'D') AS Ppa_Clase, Ppa_Des AS Ppa_Des
            FROM pre_partidas
            WHERE Emp_Cod = $Emp_Cod
              AND Ppa_Cla IN (" . implode(',', array_values($in_list)) . ")";
    $res = $mysqli->query($sql);
    while ($res && ($row = $res->fetch_assoc())) {
        $map[$row['Ppa_Cla']] = array(
            'Ppa_Cod' => (int)$row['Ppa_Cod'],
            'clase' => ppto_pdf_normalizar_clase($row['Ppa_Clase']),
            'descripcion' => $row['Ppa_Des'],
        );
    }
 ...
 * @return array
 */
function ppto_pdf_validar_contra_catalogo($mysqli, $Emp_Cod, $parsed) {
    ...
    $db_map = ppto_pdf_catalogo_por_codigos($mysqli, $Emp_Cod, $buscar);
    ...
    foreach ($items as $cod => $it) {
        ...
        $desc_db = ($existe && isset($db_map[$cod]['descripcion'])) ? $db_map[$cod]['descripcion'] : '';
        $desc_file = isset($it['descripcion']) ? trim($it['descripcion']) : '';
        $nombre_distinto = ($existe && $desc_db !== '' && $desc_file !== ''
            && strcasecmp($desc_db, $desc_file) !== 0);

        $catalogo[$cod] = array(
            'existe' => $existe,
            'clase_actual' => $actual,
            'clase_esperada' => $esperada,
            'descripcion_catalogo' => $desc_db,
            'descripcion_archivo' => $desc_file,
...
function ppto_pdf_partida_upsert($mysqli, $Emp_Cod, $codigo, $descripcion, $opts) {
    ...
    $Usu_Cod = (int)$opts['Usu_Cod'];

    $res = $mysqli->query("SELECT Ppa_Cod AS Ppa_Cod FROM pre_partidas WHERE Emp_Cod=$Emp_Cod AND Ppa_Cla='$codigo_esc' LIMIT 1");
    if ($res && $row = $res->fetch_assoc()) {
        $Ppa_Cod = (int)$row['Ppa_Cod'];
        // Solo renombra el catalogo compartido si el import lo pide explicitamente.
        if (!empty($opts['actualizar_nombre']) && $des_esc !== '') {
            $mysqli->query("UPDATE pre_partidas SET Ppa_Des='$des_esc' WHERE Ppa_Cod=$Ppa_Cod AND Emp_Cod=$Emp_Cod LIMIT 1");
        }
        return $Ppa_Cod;
    }
    ...
    $sql = "INSERT INTO pre_partidas
            ...
            VALUES ($Emp_Cod, '$codigo_esc', '$des_esc', '$tipo', '$nat', $padre_id, $nivel, '$clase', 'A', NOW(), $Usu_Cod)";
    if ($mysqli->query($sql)) {
...
function ppto_pdf_importar_presupuesto($mysqli, $Emp_Cod, $Usu_Cod, $Pro_Cod, $Ppe_Cod, $parsed, $ton_override = 0, $opts = null) {
    ...
    $actualizar_nombres = !empty($opts['actualizar_nombres']);
    $crear_nuevas = !isset($opts['crear_nuevas']) || !empty($opts['crear_nuevas']);

    $max_rubros = 2000;
    $n_rubros = isset($parsed['rubros']) && is_array($parsed['rubros']) ? count($parsed['rubros']) : 0;
    if ($n_rubros > $max_rubros) {
        return array(
            'ok' => false,
            'message' => 'El archivo tiene ' . $n_rubros . ' rubros (maximo ' . $max_rubros
                . '). Divida el Excel e importe por partes.',
        );
    }
    ...
    $stats = array(
        ...
        'ton_base' => 0,
        'ton_costo_mes' => 0,
        'crear_nuevas' => $crear_nuevas ? 1 : 0,
        'actualizar_nombres' => $actualizar_nombres ? 1 : 0,
    );
    ...
    // Precarga catalogo existente (1 query) — evita SELECT por fila.
    $codigos_all = array();
    foreach ($partidas as $p) {
        if (!empty($p['codigo'])) {
            $codigos_all[$p['codigo']] = true;
        }
        if (!empty($p['padre_codigo'])) {
            $codigos_all[$p['padre_codigo']] = true;
        }
    }
    foreach ($rubros as $r) {
        if (!empty($r['codigo'])) {
            $codigos_all[$r['codigo']] = true;
        }
        $padre_cod = ppto_partida_prefijo_padre_codigo($r['codigo']);
        if ($padre_cod) {
            $codigos_all[$padre_cod] = true;
        }
    }
    $db_cat = ppto_pdf_catalogo_por_codigos($mysqli, $Emp_Cod, array_keys($codigos_all));
    foreach ($db_cat as $cla => $info) {
        if (!empty($info['Ppa_Cod'])) {
            $cod_to_id[$cla] = (int)$info['Ppa_Cod'];
        }
    }

    $meses_grupo_ensure = false;
    foreach ($partidas as $p) {
        $cod = $p['codigo'];
        $exists = isset($cod_to_id[$cod]);
        $Ppa_Cod = 0;

        if ($exists) {
            $Ppa_Cod = (int)$cod_to_id[$cod];
            $stats['partidas_existentes']++;
            if ($actualizar_nombres && $Ppa_Cod > 0) {
                $before = ppto_pdf_partida_upsert($mysqli, $Emp_Cod, $cod, $p['descripcion'], array(
                    'tipo' => 'G',
                    'naturaleza' => 'OPE',
                    'clase' => $p['clase'],
                    'Usu_Cod' => $Usu_Cod,
                ...
                if ($before > 0) {
                    $stats['nombres_actualizados']++;
                }
            }
        } else {
            if (!$crear_nuevas) {
                $stats['partidas_omitidas']++;
                continue;
            }
            $padre_id = null;
            ...
            $Ppa_Cod = ppto_pdf_partida_upsert($mysqli, $Emp_Cod, $cod, $p['descripcion'], array(
                'tipo' => 'G',
                'naturaleza' => 'OPE',
                'clase' => $p['clase'],
                'padre_id' => $padre_id,
            ...
            if ($Ppa_Cod > 0) {
                $cod_to_id[$cod] = $Ppa_Cod;
                $stats['partidas_nuevas']++;
            }
        }

        if ($Ppa_Cod > 0 && ppto_pdf_normalizar_clase($p['clase']) === 'G' && isset($p['meses_prorrateo'])) {
            $meses_g = (int)$p['meses_prorrateo'];
            if ($meses_g > 0) {
                if (!$meses_grupo_ensure) {
                    ppto_schema_ensure_partida_meses_prorrateo($mysqli);
                    $meses_grupo_ensure = true;
                }
                $mysqli->query("UPDATE pre_partidas SET Ppa_Meses = $meses_g WHERE Ppa_Cod = $Ppa_Cod AND Emp_Cod = " . (int)$Emp_Cod);
            }
        }
    }
    ...
    $proy_esc = $mysqli->real_escape_string((string)$proy_id);
    $Ppe_Cod = (int)$Ppe_Cod;

    $sql_ton = "INSERT INTO pre_proyecto_version
            (Pro_Cod, Emp_Cod, Ppe_Cod, Ppv_TonBaseMes, Ppv_TonCostoMes, Ppv_FecReg, Usu_Cod)
            VALUES ('$proy_esc', " . (int)$Emp_Cod . ", $Ppe_Cod, $ton_ingreso, $ton_costo, NOW(), " . (int)$Usu_Cod . ")
            ON DUPLICATE KEY UPDATE Ppv_TonBaseMes=$ton_ingreso, Ppv_TonCostoMes=$ton_costo, Ppv_FecReg=NOW(), Usu_Cod=" . (int)$Usu_Cod;
    $mysqli->query($sql_ton);

    // Mapa Ppa_Cod=>Pdp_Cod (1 query). No incluir ppto_proyectos_logica.php aqui (es entrypoint AJAX).
    $ppa_to_pdp = array();
    if (function_exists('ppto_proy_rubros_map_por_ppa')) {
        $ppa_to_pdp = ppto_proy_rubros_map_por_ppa($mysqli, $proy_id, $Emp_Cod, $Ppe_Cod);
    } else {
        $res_map = $mysqli->query(
            "SELECT Ppa_Cod, MIN(Pdp_Cod) AS Pdp_Cod FROM pre_proyecto_detalles
             WHERE Pro_Cod='$proy_esc' AND Emp_Cod=" . (int)$Emp_Cod . " AND Ppe_Cod=$Ppe_Cod
             GROUP BY Ppa_Cod"
        );
        while ($res_map && ($rm = $res_map->fetch_assoc())) {
            $ppa_to_pdp[(int)$rm['Ppa_Cod']] = (int)$rm['Pdp_Cod'];
        }
    }
    $meses_batch = array();
    ...
    foreach ($rubros as $r) {
        ...
        } elseif ($actualizar_nombres) {
            ppto_pdf_partida_upsert($mysqli, $Emp_Cod, $cod, $r['descripcion'], array(
                'tipo' => 'G',
                'naturaleza' => 'OPE',
                'clase' => 'D',
                'Usu_Cod' => $Usu_Cod,
        ...
        $Ppa_Cod = isset($cod_to_id[$cod]) ? (int)$cod_to_id[$cod] : 0;
        if ($Ppa_Cod <= 0) {
            $stats['rubros_omitidos']++;
            continue;
        }
        ...
        if (isset($r['presupuesto_anual']) && (float)$r['presupuesto_anual'] > 0) {
            ...
            $factor = ($ton_rubro > 0.0001) ? round($presup_pdf / $ton_rubro, 6) : $factor;
        } else {
            $anual = round($ton_rubro * $factor, 2);
        }

        $Pdp_Cod = isset($ppa_to_pdp[$Ppa_Cod]) ? (int)$ppa_to_pdp[$Ppa_Cod] : 0;
        if ($Pdp_Cod > 0) {
            $sql = "UPDATE pre_proyecto_detalles
                SET Pdp_Rubro='$rubro', Pdp_TonBase=$ton_rubro, Pdp_FacAnualTon=$factor, Pdp_PreAnual=$anual
                WHERE Pdp_Cod=$Pdp_Cod AND Pro_Cod='$proy_esc' AND Emp_Cod=" . (int)$Emp_Cod . " AND Ppe_Cod=$Ppe_Cod";
            if ($mysqli->query($sql)) {
                $stats['rubros_actualizados']++;
                $stats['rubros_duplicados_eliminados'] += ppto_proy_rubro_purgar_duplicados_partida(
                    $mysqli, $Ppe_Cod, $Ppa_Cod, $proy_id, $Emp_Cod, $Pdp_Cod
                );
                $meses_batch[$Pdp_Cod] = $anual;
            }
        } else {
            $sql = "INSERT INTO pre_proyecto_detalles
                ...
                VALUES ($Ppe_Cod, $Ppa_Cod, '$proy_esc', " . (int)$Emp_Cod . ", '$rubro', $ton_rubro, $factor, $anual, NOW(), " . (int)$Usu_Cod . ")";
            if ($mysqli->query($sql)) {
                $Pdp_Cod = (int)$mysqli->insert_id;
                if ($Pdp_Cod <= 0) {
                    $Pdp_Cod = ppto_proy_rubro_id_por_partida($mysqli, $Ppe_Cod, $Ppa_Cod, $proy_id, $Emp_Cod);
                }
                if ($Pdp_Cod > 0) {
                    $stats['rubros_nuevos']++;
                    $ppa_to_pdp[$Ppa_Cod] = $Pdp_Cod;
                    $meses_batch[$Pdp_Cod] = $anual;
                }
            }
        }
    ...
    if (!empty($meses_batch)) {
        if (function_exists('ppto_proy_distribuir_meses_batch')) {
            ppto_proy_distribuir_meses_batch($mysqli, $meses_batch);
        } else {
            foreach ($meses_batch as $pid => $anual_b) {
                ppto_pdf_distribuir_meses($mysqli, $pid, $anual_b);
            }
        }
    }
    ...
    if ($stats['partidas_omitidas'] > 0 || $stats['rubros_omitidos'] > 0) {
        $msg .= ' Omitidos (sin crear nuevas): ' . (int)$stats['partidas_omitidas'] . ' partida(s), '
            . (int)$stats['rubros_omitidos'] . ' rubro(s).';
    }
    if ($stats['nombres_actualizados'] > 0) {
        $msg .= ' Nombres de catalogo actualizados: ' . (int)$stats['nombres_actualizados'] . '.';
    }
 ...
 * @param float $anual
 */
function ppto_pdf_distribuir_meses($mysqli, $Pdp_Cod, $anual) {
    if (function_exists('ppto_proy_distribuir_meses')) {
    ...
    for ($mes = 1; $mes <= 12; $mes++) {
        $vals[] = "($Pdp_Cod,$mes,22,0.0833,$mensual,0,0,$mensual)";
    }
    $mysqli->query(
