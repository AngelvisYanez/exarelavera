...
if (!function_exists('ppto_bridge_cargar')) {
     ...
     * @return string|null
     */
    function ppto_bridge_sync_orden_compra($mysqli, $ord_cod, $Emp_Cod, $Usu_Cod) {
        if (!$mysqli || $ord_cod <= 0) {
            return null;
        }
        ...
        $mysqli->query("INSERT INTO exa_compras_ordenes
            ...
            VALUES ('$id', $Emp_Cod, NULL, NULL, $monto, 'creada', $Usu_Cod, '$fecha')
    ...
    function ppto_bridge_compras_orden($mysqli, $ord_cod, $Emp_Cod, $Usu_Cod, $evento = 'crear') {
        return ppto_bridge_ejecutar_seguro(function () use ($mysqli, $ord_cod, $Emp_Cod, $Usu_Cod, $evento) {
            $conn = ppto_bridge_conn_desde_exa($mysqli);
            if (!$conn) {
                return null;
            }
            $id = ppto_bridge_sync_orden_compra($conn, (int)$ord_cod, (int)$Emp_Cod, (int)$Usu_Cod);
            if (!$id) {
                return null;
            }
    ...
    function ppto_bridge_relavera_manifiesto($mysqli, $man_cod, $Pro_Cod = 'RCET-01') {
        return ppto_bridge_ejecutar_seguro(function () use ($mysqli, $man_cod, $Pro_Cod) {
            ppto_bridge_cargar();
            $conn = ppto_bridge_conn_desde_exa($mysqli);
            if (!$conn || empty($man_cod) || !function_exists('ppto_sync_relavera_produccion')) {
                return null;
            }
            $Emp_Cod = function_exists('ppto_resolve_emp_id_proyecto')
                ? ppto_resolve_emp_id_proyecto($conn, $Pro_Cod, null)
                : null;
            ppto_sync_relavera_produccion($conn, $Pro_Cod, $Emp_Cod, (int)date('Y'));
