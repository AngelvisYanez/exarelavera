 ...
 * Capa de Lï¿½gica de Negocio para el Dashboard Presupuestario en EXA PPTO.
 * Procesa consultas consolidadas, semï¿½foros, KPI e integraciï¿½n de producciï¿½n.
 */
...
function ppto_dash_sql_solo_partidas_activas($alias_resumen = 'r') {
    return "EXISTS (
        ...
        WHERE (px.Ppa_Cod = {$alias_resumen}.Ppa_Cod OR px.Ppa_Cod = {$alias_resumen}.Ppa_Cod)
          AND px.Ppa_Est = 'A'
...
/**
 * Filtro SQL por alcance: general | proyecto | consolidado.
 *
 * @param mysqli $mysqli
 * @param string $alias
 * @param array $filtros
 * @return string
 */
function ppto_dash_sql_filtro_alcance($mysqli, $alias, $filtros) {
    $alcance = isset($filtros['alcance']) ? $filtros['alcance'] : 'proyecto';
    $Pro_Cod = isset($filtros['Pro_Cod']) ? $filtros['Pro_Cod'] : null;
    if ($alcance === 'general') {
        return "(" . $alias . ".Pro_Cod IS NULL OR TRIM(IFNULL(" . $alias . ".Pro_Cod,'')) = '' OR " . $alias . ".Pro_Cod = '0')";
    }
    if ($alcance === 'proyecto') {
        if ($Pro_Cod === null || trim((string)$Pro_Cod) === '') {
            return '1=0';
        }
        return $alias . ".Pro_Cod = '" . $mysqli->real_escape_string(trim((string)$Pro_Cod)) . "'";
    }
    // consolidado: sin filtro de proyecto
    return '1=1';
}
 ...
 * @return array
 */
function ppto_dash_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $Pro_Cod) {
    return ppto_forecast_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $Pro_Cod);
}
...
/**
 * Rubros tonelada-metrica del proyecto indexados por Ppa_Cod.
 *
 * @param mysqli $mysqli
 * @param int $Ppe_Cod
 * @param string|null $Pro_Cod
 ...
 * @return array
 */
function ppto_dash_cargar_rubros_proyecto($mysqli, $Ppe_Cod, $Pro_Cod, $Emp_Cod) {
    $rubros = array();
    if (!$Ppe_Cod || $Pro_Cod === null || trim($Pro_Cod) === '') {
        return $rubros;
    }
    $esc = $mysqli->real_escape_string(trim($Pro_Cod));
    $res = $mysqli->query("SELECT Ppa_Cod AS Ppa_Cod, Pdp_FacAnualTon AS Pdp_FacAnualTon, Pdp_TonBase AS Pdp_TonBase
        FROM pre_proyecto_detalles
        WHERE Emp_Cod = " . (int)$Emp_Cod . " AND Ppe_Cod = " . (int)$Ppe_Cod . " AND Pro_Cod = '$esc'
          AND Pdp_FacAnualTon > 0 AND Pdp_TonBase > 0");
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            $rubros[(int)$r['Ppa_Cod']] = array(
                'factor'   => (float)$r['Pdp_FacAnualTon'],
                'ton_base' => (float)$r['Pdp_TonBase']
            );
        }
    }
    return $rubros;
 ...
 * Base PDF = ton_costo Ã— factor_BD; factor_escenario = Base PDF / ton_ingresos_mes.
 * AsÃ­ Proyectada/Real del Dashboard coinciden con Gastos presup. del Cuadro.
 *
 * @param mysqli $mysqli
 ...
 * @return array
 */
function ppto_dash_rubros_map_factor_escenario_cuadro($mysqli, $rubros_map, $Ppe_Cod, $Pro_Cod, $Emp_Cod) {
    if (empty($rubros_map) || $Pro_Cod === null || trim($Pro_Cod) === '') {
        return $rubros_map;
    }
    require_once __DIR__ . '/ppto_format_helpers.php';
    require_once __DIR__ . '/ppto_proyecto_version_logica.php';

    $cfg = ppto_proy_version_config($mysqli, trim($Pro_Cod), (int)$Emp_Cod, (int)$Ppe_Cod);
    $ton_base_pdf = (float)$cfg['ton_mes'];
    if ($ton_base_pdf <= 0) {
        $ton_base_pdf = ppto_proy_version_ton_base($mysqli, trim($Pro_Cod), (int)$Emp_Cod, (int)$Ppe_Cod);
    }
    $ton_esc_gasto_mes = ppto_proy_ton_escenario_gasto_mes($ton_base_pdf);
    $ton_costo_mes = ppto_proy_version_ton_costo($mysqli, trim($Pro_Cod), (int)$Emp_Cod, (int)$Ppe_Cod);

    foreach ($rubros_map as $Ppa_Cod => $info) {
        $factor_bd = (float)$info['factor'];
        if ($factor_bd <= 0.0001) {
            continue;
        }
        $esc_esperada_anual = round($ton_costo_mes * $factor_bd, 2);
        $rubros_map[$Ppa_Cod]['factor_bd'] = $factor_bd;
        $rubros_map[$Ppa_Cod]['factor'] = ppto_proy_factor_escenario_gasto($esc_esperada_anual, $ton_esc_gasto_mes);
    }
    return $rubros_map;
}
 ...
 * @return float
 */
function ppto_dash_usd_por_ton_mes($mysqli, $Ppe_Cod, $Pro_Cod, $Emp_Cod) {
    if (!$Ppe_Cod || $Pro_Cod === null || trim($Pro_Cod) === '') {
        return 0.0;
    }
    $esc = $mysqli->real_escape_string(trim($Pro_Cod));
    $res = $mysqli->query("SELECT COALESCE(SUM(Pdp_FacAnualTon / 12), 0) AS usd_ton
        FROM pre_proyecto_detalles
        WHERE Emp_Cod = " . (int)$Emp_Cod . " AND Ppe_Cod = " . (int)$Ppe_Cod . " AND Pro_Cod = '$esc'");
...
/**
 * Obtiene el resumen de KPIs consolidados del presupuesto y la producciï¿½n fï¿½sica.
 *
 * @param mysqli $mysqli Conexiï¿½n activa a la BD.
 * @param array $filtros Filtros validados de entrada.
 * @return array Arreglo de KPIs consolidados con sus variaciones y semï¿½foros.
 */
function ppto_dash_kpis($mysqli, $filtros) {
    $Emp_Cod  = $filtros['Emp_Cod'];
    $anio    = $filtros['anio'];
    $mes     = $filtros['mes'];
    $Ppe_Cod  = $filtros['Ppe_Cod'];
    $Pro_Cod = $filtros['Pro_Cod'];
    $Ppa_Cod  = isset($filtros['Ppa_Cod']) ? $filtros['Ppa_Cod'] : null;

    // 1. Si no hay Ppe_Cod especificado, resolvemos el activo para ese aï¿½o
    if (!$Ppe_Cod) {
    ...
    $meses_prod_tmp = ppto_dash_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $Pro_Cod);
    $periodo = ppto_dash_periodo_resuelto($filtros, $meses_prod_tmp);
    $mes = $periodo['mes'];
    $periodo_vista = $periodo['periodo_vista'];

    $condiciones = array("r.Emp_Cod = $Emp_Cod", "r.Ppe_Cod = $Ppe_Cod");
    $condiciones[] = ppto_dash_sql_filtro_periodo('r', $periodo_vista, $mes);
    $condiciones[] = ppto_dash_sql_filtro_alcance($mysqli, 'r', $filtros);
    if ($Ppa_Cod !== null) {
        $condiciones[] = "r.Ppa_Cod = $Ppa_Cod";
    }
    ...
    $kpis = ppto_forecast_calcular_global($mysqli, $kpis, $Ppe_Cod, $Emp_Cod, $anio, ($periodo_vista === 'mes') ? $mes : null, $Pro_Cod);
 ...
 * @return array
 */
function ppto_dash_descendientes_detalle_ids($Ppa_Cod, $children_by_parent, $clase_by_id) {
    $ids = array();
    if (!isset($children_by_parent[$Ppa_Cod])) {
        return $ids;
    }
    foreach ($children_by_parent[$Ppa_Cod] as $child_id) {
...
function ppto_dash_emitir_filas_arbol($parent_id, $depth, $children_by_parent, $meta_by_id, $clase_by_id, $detalle_por_id, $roots) {
    ...
    foreach ($nodes as $Ppa_Cod) {
        $Ppa_Cod = (int)$Ppa_Cod;
        if (!isset($meta_by_id[$Ppa_Cod])) {
            continue;
        }
        $meta = $meta_by_id[$Ppa_Cod];
        $clase = isset($clase_by_id[$Ppa_Cod]) ? $clase_by_id[$Ppa_Cod] : 'D';

        if ($clase === 'G') {
            $hijos_det = ppto_dash_descendientes_detalle_ids($Ppa_Cod, $children_by_parent, $clase_by_id);
            ...
            if ($tiene_mov || !empty($children_by_parent[$Ppa_Cod])) {
            ...
            $out = array_merge(
                $out,
                ppto_dash_emitir_filas_arbol($Ppa_Cod, $depth + 1, $children_by_parent, $meta_by_id, $clase_by_id, $detalle_por_id, $roots)
            );
        } elseif (isset($detalle_por_id[$Ppa_Cod])) {
            $row = $detalle_por_id[$Ppa_Cod];
            $row['es_grupo'] = false;
 ...
 * @return array Colecciï¿½n de partidas con balances, variaciones y semï¿½foros.
 */
function ppto_dash_resumen_partidas($mysqli, $filtros) {
    ...
    if ($Pro_Cod !== null && trim($Pro_Cod) !== '') {
        $meses_prod = ppto_dash_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $Pro_Cod);
        $periodo = ppto_dash_periodo_resuelto($filtros, $meses_prod);
        $periodo_vista = $periodo['periodo_vista'];
        $mes = $periodo['mes'];
        $rubros_map = ppto_dash_cargar_rubros_proyecto($mysqli, $Ppe_Cod, $Pro_Cod, $Emp_Cod);
        $usa_toneladas = !empty($rubros_map);
        $modo_reinversion = ppto_proy_es_modo_reinversion($mysqli, $Pro_Cod, $Emp_Cod);
        if ($modo_reinversion && $usa_toneladas) {
            ...
            $rubros_map = ppto_dash_rubros_map_factor_escenario_cuadro(
                $mysqli, $rubros_map, $Ppe_Cod, $Pro_Cod, $Emp_Cod
            );
        }
    } else {
    ...
    $sql = "SELECT 
            ...
            FROM (" . ppto_sql_resumen_subquery() . ") r
            INNER JOIN pre_partidas p ON (r.Ppa_Cod = p.Ppa_Cod OR r.Ppa_Cod = p.Ppa_Cod)
            WHERE $where_sql
              AND p.Ppa_Est = 'A'
              AND COALESCE(NULLIF(p.Ppa_Clase, ''), 'D') = 'D'
            GROUP BY r.Ppa_Cod, p.Ppa_Cla, p.Ppa_Des
            HAVING (
    ...
    if ($res) {
        while ($row = $res->fetch_assoc()) {
            ...
            $detalle_por_id[(int)$row['Ppa_Cod']] = $row;
        }
    }
    ...
    $catalogo = ppto_partidas_listar($mysqli, array('Emp_Cod' => $Emp_Cod, 'solo_activas' => true));
    ...
    foreach ($catalogo as $p) {
        $pid = (int)$p['Ppa_Cod'];
        $pad = isset($p['Ppa_Pad']) && $p['Ppa_Pad'] !== '' && $p['Ppa_Pad'] !== null
            ? (int)$p['Ppa_Pad'] : 0;
        $clase = (isset($p['Ppa_Clase']) && $p['Ppa_Clase'] === 'G') ? 'G' : 'D';

        $meta_by_id[$pid] = array(
            'codigo' => $p['Ppa_Cla'],
            'descripcion' => $p['Ppa_Des']
        );
...
/**
 * Obtiene la evoluciï¿½n mensual del presupuesto y la producciï¿½n fï¿½sica (meses 1 a 12).
 *
 * @param mysqli $mysqli Conexiï¿½n activa.
 * @param array $filtros Filtros validados.
 * @return array Listado de evoluciï¿½n indexado por mes (1-12) con balances de presupuesto y producciï¿½n.
 */
function ppto_dash_evolucion_mensual($mysqli, $filtros) {
    ...
    $meses_prod = ppto_dash_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $Pro_Cod);
    $usd_por_ton = ppto_dash_usd_por_ton_mes($mysqli, $Ppe_Cod, $Pro_Cod, $Emp_Cod);
    $modo_reinversion = ($Pro_Cod !== null && trim($Pro_Cod) !== ''
        && ppto_proy_es_modo_reinversion($mysqli, $Pro_Cod, $Emp_Cod));

    // 1. Obtener la evolucion del presupuesto desde exa_ppto_resumen
    $condiciones = array("r.Emp_Cod = $Emp_Cod", "r.Ppe_Cod = $Ppe_Cod", "r.mes <= 12");
    $condiciones[] = ppto_dash_sql_filtro_alcance($mysqli, 'r', $filtros);
    if ($Ppa_Cod !== null) {
        $condiciones[] = "r.Ppa_Cod = $Ppa_Cod";
    }
