...
function sentencias_rrhh($id,$Par_Sql)
{
    switch ($id){
        ...
        case 5:
            $sql = "INSERT INTO personal (Prs_Cod,Emp_Cod,Per_Car,Per_Tit,Per_Obs,Per_Cfi) 
        ...
        case 50:            
            ...
            areas_rrhh.Are_Des
            FROM areas_rrhh
            WHERE areas_rrhh.Emp_Cod = '$Par_Sql[0]' AND areas_rrhh.Are_Est = 'A'";
        break;

        //DEDICACION
        case 51:            
            $sql = "SELECT Ded_Cod, Ded_Des FROM dedica_lab
        ...
        //CONSULTA DE EMPLEADOS 
         case 57:
            ...
            $complementoWhere = '';
            $area = $Par_Sql['Are_Cod'];
            $dedicacion = $Par_Sql['Ded_Cod'];
            $afiliacion = $Par_Sql['Afi_Cod'];

            if($area != 'T'){
                $complementoWhere .= " AND areas_rrhh.Are_Cod = $area";
            }
            if($dedicacion != 'T'){
                 $complementoWhere .= " AND dedica_lab.Ded_Cod = $dedicacion";
            }

            if($afiliacion == 'N'){
                 $complementoWhere .= " AND afiliacion.Afi_Est = 'I'";
            }
             if($afiliacion == 'A'){
                 $complementoWhere .= " AND afiliacion.Afi_Est = 'A'";
            }

            if(isset($Par_Sql["limits"])){
                    $Par_Sql["limits"]=" ORDER BY Prs_Ape $Par_Sql[limits]";
                    $campos="contratos_lab.Con_Cod,
                            contratos_lab.Con_Est,
                            contratos_lab.Con_Ini,
                            contratos_lab.Con_Fin,
                            persona.Prs_Ced,
                            personal.Per_Cod,
                            ...
                            Prs_Fec, 
                            CASE 
                                WHEN pago_contrato.Pag_Con_For = 'T' THEN 'Transferencia'
                                WHEN pago_contrato.Pag_Con_For = 'C' THEN 'Cheque'
                                WHEN pago_contrato.Pag_Con_For = 'E' THEN 'Efectivo'
                                ELSE 'Sin definir'
                            END AS Forma_Pago, Pag_Con_Cue, Are_Des ";
            }
            else{
                $campos="COUNT(contratos_lab.Con_Cod) as total";
                $Par_Sql["limits"]="";
            }
            ...
            $sql = "SELECT $campos 
                     FROM persona
                        ...
                        INNER JOIN areas_rrhh ON departamen.Are_Cod = areas_rrhh.Are_Cod
                        INNER JOIN ciudad ON persona.Ciu_Cod = ciudad.Ciu_Cod
                        LEFT JOIN afiliacion ON contratos_lab.Con_Cod = afiliacion.Con_Cod
                        LEFT JOIN pago_contrato ON contratos_lab.Con_Cod = pago_contrato.Con_Cod
                    WHERE $search
                    AND personal.Emp_Cod = $Par_Sql[Emp_Cod]
                    AND contratos_lab.Con_Est = 'A'
                    $complementoWhere
		    Group by contratos_lab.Con_Cod 
                    $Par_Sql[limits]";
