...
require_once("rhu_sql_roles_ing_egr_2.php");
...
//    function codigoComprAuto($Emp_Cod,$Tia_Cod,$Fecha, $obBD){ return $this->getComNumAuto($Emp_Cod, $Tia_Cod, $Fecha, $obBD);	}
//    function getPec_Cod($Emp_Cod,$Fecha, $obBD){ return $this->getPerioCont($Emp_Cod, $Fecha, $obBD);  }
...
//        }
//    }
    function getGridRolFilterAlt($Rubros, $Map_Cod=NULL,$obBD_conexion=NULL,$formulas=true){ 
        try{ 
    ...
    function getListRoles($datos=null,$obBD_conexion=null,$print=true,$total=false){ 
        try{ 
            ...
            //$datos['rubros'] = '';
            $campos = $this->getArrayConsulta(70,$datos, $obBD_conexion);
            foreach($campos as $c){ 
                if ($c['Cam_Tip'] === 'I') 
                {
                        $totalSum += $c['Rol_Val'];
                ...
                foreach($resp as &$r){
                    if($r['Con_Cod']==$c['Con_Cod']){
                        if ($c['Cam_Tip']='T') {
                            ...
                            if ($c['Cam_Var'] == 'total_ingr') 
                            {
                               //$r['total_ingr'] = $totalSum; //en caso de no terminar, pegar lo del else
                                 $r[$in.$c['Cam_Var'].$fn]=$c['Cam_Var']!='dias'?formato_numero($c['Total_Ingresos'],2,1):formato_numero($c['Rol_Val'],0,1);
                            }
                            else if ($c['Cam_Var'] == 'total_egr') 
                            {
                               //$r['total_ingr'] = $totalSum; //en caso de no terminar, pegar lo del else
                                 $r[$in.$c['Cam_Var'].$fn]=$c['Cam_Var']!='dias'?formato_numero($c['Total_Egresos'],2,1):formato_numero($c['Rol_Val'],0,1);
                            }
                            else if ($c['Cam_Var'] == 'total_rol') 
                            {
                               //$r['total_ingr'] = $totalSum; //en caso de no terminar, pegar lo del else
                                 $r[$in.$c['Cam_Var'].$fn]=$c['Cam_Var']!='dias'?formato_numero($c['Saldo'],2,1):formato_numero($c['Rol_Val'],0,1);
                            }
                            else
                            {
    ...
    function getListRolesFilter($rubros, $datos=null,$obBD_conexion=null,$print=true,$total=false){ 
        try{ 
            $DetRubros = "'" . implode("','", $rubros) . "'";
            if($print){ $in="{";$fn="}"; }else{ $in='';$fn=''; }
            $resp=array() ;
            if($total) $datos['totales']=true;
            $datos['rubros'] = $DetRubros;
            $campos = $this->getArrayConsulta(71,$datos, $obBD_conexion);
            foreach($campos as $c){ 
                if ($c['Cam_Tip'] === 'I') 
                {
                        $totalSum += $c['Rol_Val'];
