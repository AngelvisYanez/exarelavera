<?php	
/**
* @abstract Permite realizar el registro de una dedicaci�n laboral
* @author Jos� Ambulud�
* @version 1.0
* Fecha de creaci�n  2016-11-07
*/
require_once('../../administrador/LOGICA/seguridad.php');
require_once('../LOGICA/rhu_log_dedica_lab.php');
require_once('../../Librerias/procedimientos/almacenados_standar.php');
/**
* Creacion del Objeto de conexi�n
*/
$obBD_conexion = new Class_Log_Conexion_dedica($Ses_Dat_Dis);
/**
* Creacion del objeto mysql para las consultas 
*/
$obBD_con1 =  new Class_Log_Datos_dedica;
//Lista los dedica_lab registrados
if(isset($listarDedica)){ 
    $responce = $obBD_con1->getArrayConsulta(2,$Ses_Suc_Cod, $obBD_conexion);
    echo json_encode($responce);exit();
}
if(isset($saveDedicacion)){
    $response['success'] = false;
    $response['message'] = "No se ha logrado realizar la Transaccion";
    $obBD_con1->inicio_transaccion($obBD_conexion->conexion);
    foreach($campos as $valor){
        $existe=$obBD_con1->getRowConsulta(5,$valor['Ded_Cod'],$obBD_conexion);
        if(!empty($existe['Ded_Cod'])){
            $obBD_con1->operacionobBD(3,$valor['Ded_Cod'].'*'.$valor['Ded_Des'].'*'.$valor['Ded_Hrs'], $obBD_conexion);
        }else{
            $obBD_con1->operacionobBD(1,$Ses_Suc_Cod.'*'.$valor['Ded_Des'].'*'.$valor['Ded_Hrs'], $obBD_conexion);
        }
    }
...
if($delete){
    ...
    $response['message'] = "No se ha logrado realizar la Transaccion";
    $obBD_con1->inicio_transaccion($obBD_conexion->conexion);
    $obBD_con1->operacionobBD(4,$Ded_Cod, $obBD_conexion); 
    $obBD_con1->fin_transaccion_nomsn($obBD_conexion->conexion);
    if ($obBD_con1->Error == 0) { $response['success'] = true; }
    echo json_encode($response);
    exit();
...
<HTML>
    <HEAD>		
        ...
        <TITLE><?Php echo "Dedicación Laboral Registro [EXA]"; ?></TITLE>
        <meta charset="UTF-8">
...
<BODY>
    <div class="panel panel-main">
        <div class="panel-heading exa-header"><h3 class="panel-title">&raquo;  Registro Dedicaci&oacute;n Laboral</h3></div>
    ...
    <script type="text/javascript">
        $(function() {
            $("#list").jqGrid({
                ...
                postData:{listarDedica:true},
                cmTemplate: {sortable:false},
                colModel: [  
                    { label: 'Cod', key:true,hidden:true, name: 'Ded_Cod', width: 150 },
                    { label: 'Descripci&oacute;n', name: 'Ded_Des', width: 230,align:'center',editable:true}, 
                    { label: 'Horas Laborables', name: 'Ded_Hrs', width: 130,align:'right',editable:true}, 
