...
if(isset($getPlantilla)){ 
    ...
    $grid['grid']['colModel']=array_merge($grid['grid']['colModel'],array(        
        array('label'=>'Anticipo','name'=>'anticipo_val','width'=>75,'align'=>'right','formatter'=>'anticipos','formatoptions'=>array('defaultValue'=>''),'summaryRound'=>2,'summaryRoundType'=>"round"/*,'editable'=>false, 'editoptions'=>array('dataInit'=> "styleInput")*/),
        ...
        array('label'=>'<i class="glyphicon glyphicon-print"></i>','labelLong'=>'Imprimir','name'=>'imprimir','width'=>50,'align'=>'center','formatter'=>'gridButton','formatoptions'=>array('action'=>"printRecibo",'icon'=>'print','conditional'=>'issetRecibo'),'classes'=>'columnHighlight3','hidden'=>true,'vieawable'=>false),
    ));
...
if(isset($saveAnticipos)){ 
    ...
    try{        
        if(empty($contratos)||count($contratos)==0) throw new Exception('Debe existis al menos un anticipo!');
        ...
        $obBD_ins1->inicio_transaccion($obBD_conexionIns);
            ...
            foreach($contratos as $j=>&$c){                
                $prv=$obBD_con1->getRowConsulta(70, $c['Con_Cod'],$obBD_conexion);
				if(empty($prv['Prv_Cod'])){
					$obBD_ins1->operacionobBD(69, array('Prs_Cod'=>$prv['Prs_Cod'],'Emp_Cod'=>$Ses_Emp_Cod), $obBD_conexionIns); // relacion anticipos comprobante
					$Prv_Cod = $obBD_ins1->insercionid ($obBD_conexionIns);
				}else $Prv_Cod=$prv['Prv_Cod'];

                //$obBD_ins1->echoLog(array_merge($c,array('Ant_Fec'=>$Ant_Fec)));
                $obBD_ins1->operacionobBD(32,array_merge($c,array('Ant_Fec'=>$Ant_Fec)),$obBD_conexionIns); // inserta los valores de cada anticipo
                ...
                if($configs['Cof_Con']=='S'){                     
                    ...
                    $obBD_ins1->operacionobBD(24, $pec['Pec_Cod'].'*'.$Prv_Cod.'*'.$Com_Num.'*'.$Com_Fec.'*'.trim($c['Ant_Obs']).'*'.$Tia_Asi['Tia_Cod'].'*'.$c['Ant_Val'].'*'.'ANTICIPOS: '.$Ant_Obs.'*'.$campo, $obBD_conexionIns);
                    ...
                    if(!isset($cuenta['Pld_Cod'])||empty($cuenta['Pld_Cod'])) throw new Exception('Revisar la parametrizacion contable del campo: <u>Anticipos Rol</u>!');
                    $obBD_ins1->operacionobBD(27, $Com_Cod.'*'.'D'.'*'.$c['Ant_Val'].'*'.$cuenta['Pld_Des'].'*'.'Anticipos'.'*'.$cuenta['Pld_Cod'], $obBD_conexionIns);  // inserta asiento          
                    $c['cheque']=array();
                ...
                foreach($c['Ant_Det'] as $i=>$p){
                   $obBD_ins1->operacionobBD(38,array('Ant_Int'=>$i++,'Ant_Cod'=>$c['Ant_Cod'],'Ant_Val'=>$p['Pag_Val'],'Pag_Cod'=>$p['Pag_Cod'],'Asi_Cod'=>$p['Asi_Cod']),$obBD_conexionIns);
                } 
            } unset($c);
...
<HEAD>		
    ...
    <TITLE><?Php echo "Anticipo Registr/Cons [EXA]"; ?></TITLE>
    <meta charset="UTF-8">
		   ...
		   <div class="col-xs-12" style="margin-bottom: 5px;">
                        <div class="row">
                             ...
                             <div class="col-xs-2"> 
                              <div class="input-group">
                                <input name="valorPagar" id="valorPagar" class="form-control input-xs databind datatitle" placeholder="% Anticipos"/>
                                <span class="input-group-btn">
                                  <button type="button" onclick="asignarPago($('#valorPagar').val())" class="btn btn-success btn-xs" title="Calcular Valor">
                    ...
                    <div class="col-xs-12" >
                        <div class="row">
                            <div class="col-xs-3">
                              <fieldset class="exa-fieldset">                           
                                <legend class="Titulos2">Registrar Anticipos</legend>  
...
//    }
    ...
    function asignarPago(valor){
      ...
      var i = 0;
      $("#rol tbody tr[role='row']").each(function(i)
      {
        if(i!=0){

          var codFila = $(this).attr('id');  
          var suel = $(this).find("td[aria-describedby='rol_total_rol']").text().replace(',','');
          var sueldo = suel.substr(2,suel.lenght);
          var sueldoNumero = parseFloat(sueldo);
          var anticipo = sueldo * (valor/100);

          $("input[rowid='" + codFila + "'][name='anticipo_val']").val(anticipo.toFixed(2));             
        }
         i++;
      });
    }
