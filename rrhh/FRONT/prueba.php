...
if (isset($savePersonal)) {
    ...
    if ($Prs_Cod == 0) {
        $obBD_con1->operacionobBD(4, $Ciu_Cod . '*' . $Ide_Cod . '*' . $Prs_Ced . '*' . $Prs_Nom . '*' . $Prs_Ape . '*' . $Prs_Sex . '*' . $Prs_Esc . '*' . $Prs_Fec . '*' . $Prs_Tel . '*' . $Prs_Te2 . '*' . $Prs_Cel . '*' . $Prs_Cor . '*' . $Prs_Dir, $obBD_conexion);
    ...
    $obBD_con1->operacionobBD(5, $Prs_Cod . '*' . $Ses_Suc_Cod . '*' . $Per_Car . '*' . $Per_Tit . '*' . $Per_Obs, $obBD_conexion);
    ...
    $obBD_con1->operacionobBD(14, $tic_cod . '*' . $Per_Cod . '*' . $Ded_Cod . '*' . $Reb_Cod . '*' . $Con_Ini . '*' . $Con_Fin . '*' . $Con_Mot, $obBD_conexion);
    ...
    $obBD_con1->operacionobBD(15, $Con_Cod . '*' . $Sue_Fec . '*' . $Sue_Val, $obBD_conexion);
    //Secci�n para insertar un registro en la tabla afiliacion
    $obBD_con1->operacionobBD(16, $Con_Cod . '*' . $Afi_Fei . '*' . $Afi_Fnd . '*' . $Afi_Fef . '*' . $Afi_Mot, $obBD_conexion);
