...
AND puntos_imp.Pun_Cod = $Par_Sql[0]";
	   ...
	   case 82:
		...
		case 83:
		/*Consulta de los cliente por punto de impresi�n y por apellido */
...
//echo $my_cons_puntosfact_87;
		...
		case 88:
		/* Consulta los totales de las facturas agrupados por rubros por CARRERAS y dependiendo adem�s de Pun_Cod*/
...
FROM caja_aper WHERE caja_aper.Caj_Est ='A' AND caja_aper.Pun_Cod = '$Par_Sql[0]'";
		...
		case 94: 
		/*Busca las facturas registradas de acuerdo a los intervalos de fecha y por punto de impresi�n*/
...
/************************************* ROLES DE PAGO *********************************/		
		...
		case 111:
		/* Consultar tipo de rol de pagos en base al c�digo */
		$consultarol = "SELECT Tir_Des FROM tipos_rol WHERE Tir_Cod =	$Par_Sql[0]";
		return $consultarol;
		break;		
		
		case 112: 
		/* Consulta la informaci�n relacionada con el detalle del plan de cuentas */
		...
		case 114: 
		/* Consulta la informaci�n relacionada con el descripcion del campo rol */
		...
		case 117:
		/* Consulta por el c�digo de campo del rol */
		...
		case 130:
		/* Consultar tipos de rol de pagos en base a la descripci�n */
		$consultartirol = "SELECT Tir_Des FROM tipos_rol WHERE Tir_Des ='$Par_Sql[0]'";
		return $consultartirol;
		break;
		
		case 131:
		/* Modificar la informaci�n de tipos de rol de pagos*/		
		$mod_tirol="UPDATE tipos_rol SET Tir_Des= UPPER('$Par_Sql[0]') WHERE Tir_Cod = $Par_Sql[1]";
		return $mod_tirol;
		break;
		
		case 132: 
		/*Consulta la descripci�n del tipo de rol de pagos en base a un parametro*/
      ...
      AND comprobantes.Com_Cod = $Par_Sql[0] ORDER BY Che_Num";
		...
		case 145:
		/* Consulta por la descripci�n del Rol de Pagos */
...
AND rol_pagos.Rol_Des = '$Par_Sql[1]' group by Prs_Ape order by Cof_Or";
		...
		case 152:
		/* Consulta la la descripci�n del rol de pagos */
...
//		case 160:
//		/* Cargado cheques seg�n el apellido del proveedor de comprobante de egreso */
...
//		break;		
		...
		case 161: 
		/* Consulta la descripci�n del tipo de cargo */
		...
		case 162:	
		/* Consulta el cargo que tiene el personal seleeccionado en la configuraci�n*/	
...
/** Cargar retenci�n de una liquidacion de compras ***********/
		case 166:
...
AND renta_iva.Ren_Cod=det_retenc.Ren_Cod AND retencion.Ret_Cod=$Par_Sql[0] LIMIT 0,1"; 			
		...
		/* Carga todos los costos menores o iguales a partir de una fecha para su generaci�n */
		case 169:
...
//echo $detalle_xml_242;
		...
		case 246:
		/* Consulta los a�os de las retenciones generadas */
...
AND det_compra.Adq_Cod=adquisicio.Adq_Cod GROUP BY adquisicio.Adq_Cod ORDER BY adquisicio.Adq_Des ASC";
	...
	/** Consultar facturas sin retenci�n ********/
		case 326:
		...
		/* sentencias actualizacion de codigos de retenci�n */
      case 327:
...
det_retenc.Ren_Cod=renta_iva.Ren_Cod AND retencion.Ret_Fec >='2009-01-01' AND retencion.Ret_Fec <='2009-03-26' AND renta_iva.Ren_Cod='$Par_Sql[0]'";
		...
		/* consulto los a�os para la consulta de las retenciones */
		case 329:
...
GROUP BY YEAR(retencion.Ret_Fec) ORDER BY YEAR(retencion.Ret_Fec) DESC";
		...
		break;
		/* consulto los a�os de las retenciones por par�metros */
		case 330:
...
GROUP BY YEAR(retencion.Ret_Fec) ORDER BY YEAR(retencion.Ret_Fec) DESC";
		...
		break;
		/* consulta de codigos de formularios del SRI y los conceptos en la retenci�n en la fuente de impuesto a la renta (AIR) */
		case 331:
...
ORDER BY renta_iva.Ren_Sri";
		...
		/* consulta de retenciones activas e inactivas buscadas por n�mero de comprobante de retenci�n */
		case 332:
...
GROUP BY retencion.Ret_Cod ORDER BY persona.Prs_Ape";
	...
	/**
	* Carga los conceptos en la retenci�n en la fuente de impuesto a la renta (AIR)  
	*/
 // case 338:
...
//echo $delete_asiento_353;
 ...
 break;
 /* Actualizo los datos de la cabecera de la retenci�n */
 case 354:
...
/* Problema desde aqui */
	...
	/* Baja l�gica del comprobante en la base de datos*/
	case 359:
	...
	/* Carga los conceptos en la retenci�n en la fuente de impuesto a la renta (AIR) Ojoooooooooooooooooooooooooo 
...
//	echo $carg_rentaiva_adq_361;
	...
	break;
		/* Consulto el c�digo del detalle de la cuenta perteneciente al comprobante de compra */
	case 362:
...
AND comprobantes.Com_Cod=compr_auto.Com_Cod AND compr_auto.Cop_Cod='$Par_Sql[0]'";
	...
	break;
	/* Consulta que permite saber si la compra es autom�tica o manual */
...
/* Consulta si la cuenta se encuentra en la tabla Banco */
	...
	/* Consultar la cuenta del plan en base al c�digo*/
	case 371:
   ...
   /* Consultar los cheques que tiene asignado un asiento contable  */
	 ...
	 break;
	 /* Consulto la autorizaci�n para la retenci�n */
	 ...
	 break;
	 	 ...
	 	 /* Consultar si la retenci�n no se repite en la modificaci�n */
 ...
 /* Consultar si la retenci�n es autom�tica */
	 case 380:
	 ...
	 break;
	 /* Consultar el detalle de la retenci�n */
	 case 381:
...
//echo $baja_deuda_sinbeca_385;
	...
	/* B�squeda de un personal Receptor caja chica por apellido */
	case 395:
	$Sql_395="SELECT 
				  receptor.Rec_Cod,
				  personal.Per_Cod,
	...
	/* B�squeda de un personal Receptor caja chica por cedula */
	case 396:
	$Sql_396="SELECT 
				  receptor.Rec_Cod,
				  personal.Per_Cod,
...
break;
		/* Carga las facturas que se pueden anular que no se le ha generado la retencion buscando por n�mero de factura de compra */
				/* Carga las facturas que se pueden anular que no se le ha generado la retencion buscando por n�mero de factura de compra */
		case 469:
...
GROUP BY compras.Cop_Cod ORDER BY compras.Cop_Cod ASC ";
		...
		/* Dar de baja a las facturas de compra a las cuales no se les haya generado la retenci�n */
		case 471:
...
AND det_compra.Iva_Cod=iva.Iva_Cod AND ciudad.Ciu_Cod=compras.Ciu_Cod AND sustento.Tri_Cod= compras.Tri_Cod ";
	...
	/* Consulta de campos para el c�lculo de los totales de la factura */
		case 473:
...
$con_exi_fac_com="SELECT compras.Cop_Num FROM compras WHERE compras.Cop_Num='$Par_Sql[0]'  AND compras.Prv_Cod=$Par_Sql[1] AND compras.Cop_Est='A'";
		...
		/* Consulta las liquidaciones que se pueden modificar en base al tipo de comprobante, a�o, mes */
		case 475:
...
AND compras.Cop_Cod NOT IN(SELECT retencion.Cop_Cod FROM retencion WHERE Ret_Est='A' )";
		return $con_dat_ret;
		break;
		/** Consulta las facturas que se pueden modificar con estado Activo por n�mero de factura **********************/
		...
		/*** Modificaci�n de los datos de la cabecera de la Factura *********************************************/
		case 480:
...
";
		...
		/** Consulta las facturas de compra por n�mero de la factura de compra con estado activo e inactivo  ********************************************************************************************************************************/
    	case 484:
		$carg_fac_com_anu="SELECT persona.Prs_Ape, compras.Cop_Est, compras.Cop_Cod, compras.Cop_Fec, persona.Prs_Nom, compras.Cop_Num
FROM persona, compras, proveedore
WHERE
...
";
		return $carg_fac_com_anu;
		break;
		/** Consulta el total del importe de una factura de compra de acuerdo al c�digo interno
		*******************************************************************************************************************/
		case 486:
	...
	/* B�squeda de un proveedor por C�dula */
  	...
  	$bus_proc="SELECT Prv_Cod, Prs_Ced, Prs_Ape, Prs_Nom, Prv_Fax, IF (Prv_Est='A','Activo','Inactivo') as Prv_Est FROM proveedore, persona WHERE Prs_Ced = '$Par_Sql[0]' AND proveedore.Prs_Cod=persona.Prs_Cod ORDER BY persona.Prs_Ape ASC";
		...
		return $renta_iva;
		break;
		/* Cargado de los porcentajes RENTA IVA medidiante tomando el c�digo enviado atraves de AJAX */
		case 490:
		...
		/* Insercion del detalle de la retenci�n **/
		case 492:
		...
		/* Carga las facturas las retenciones que se deben modifcar producto de la actualizaci�n de una factura */
		case 493:
...
AND compras.Cop_Cod IN(SELECT retencion.Cop_Cod FROM retencion WHERE retencion.Ret_Est='A')";
		...
		break; 
	    /* Consulta las facturas con estado activo y sin generaci�n de la retenci�n o con la retencion dada de bajs ******/
		case 494:
		$carg_fac_com_anu="SELECT persona.Prs_Ape, compras.Cop_Est, compras.Cop_Cod, compras.Cop_Fec, persona.Prs_Nom, compras.Cop_Num
FROM persona, compras, proveedore
WHERE
...
AND compras.Cop_Cod NOT IN(SELECT retencion.Cop_Cod FROM retencion WHERE Ret_Est='A') ";
		...
		break;
		/** Consultar las retenciones a modificar por n�mero de comprobante de retencion ***/
		case 499:
...
AND retencion.Ret_Num='$Par_Sql[0]' GROUP BY retencion.Ret_Cod ORDER BY persona.Prs_Ape";
		...
		break;
		/** C�lculo del importe de la retenci�n ************/
		/** C�lculo del importe de la retenci�n ************/
		case 500:
...
ORDER BY det_retenc.Ret_Int ASC";
		...
		/** Actualiza la cabecera de la retenci�n *****************/
		case 502:
...
AND retencion.Ret_Num='$Par_Sql[0]' $Par_Sql[2] GROUP BY retencion.Ret_Cod ORDER BY persona.Prs_Ape";
		...
		/** Consulta de retenciones activas e inactivas buscadas por n�mero de factura *********************/
...
AND persona.Prs_Ape LIKE '%$Par_Sql[0]%'  $Par_Sql[2] GROUP BY retencion.Ret_Cod ORDER BY persona.Prs_Ape";
		...
		/** Consulta de retenciones activas e inactivas por fechas de registro de retenci�n *************/
		case 507:
...
AND retencion.Ret_Est='$Par_Sql[3]'
 GROUP BY retencion.Ret_Cod ORDER BY retencion.Ret_Cod ASC";
		...
		/** Consultar el codigo m�ximo en la base de datos *********************************************/
		case 511:
		...
		/* Carga los conceptos en la retenci�n en la fuente de impuesto a la renta (AIR) agrupados */
		case 513:
		...
		return $carg_ret_des_imp;
		break;
		/** Consulta las retenciones de acuerdo al porcentaje de retenci�n *******************************************/
		case 514:
...
AND renta_iva.Ren_Por=$Par_Sql[4]
 GROUP BY retencion.Ret_Cod ORDER BY retencion.Ret_Cod";
		...
		break;
		/** Calculo del valor de la retencion por porcentaje de retenci�n ********************/
		case 515:
...
/** Consultar el codigo de la retencion desde la autorizaci�n grabada en la base de datos *************************************/
 		case 517:
		...
		/** Modificaci�n de comprobante de liquidaci�n de compra  *******************/
		...
		return $con_mod_fac_compra;
		break;
		/** Consultar informaci�n de las liquidaciones de compra por codigo interno de la autorizaci�n *************/
		case 523:
...
//echo $con_inf_aut_liq;
		...
		break;
		/** Eliminaci�n de detalles de la retenci�n ******************/
		case 524:
...
/** Consultas de porcentajes del I.C.E. ***********************/
		...
		/** Consulta datos las facturas de compras que se identifiquen como adquisici�n de bienes o servicios  *************/
...
//echo $con_iva_tipo_comp;
 ...
 /** Consulta de facturas de compra que estan obligadas a generar el comprobante de retenci�n *****************/
		case 534:
...
ORDER BY compras.Cop_Fec, compras.Cop_Cod ASC";
	...
	break;
	/** Inserci�n del detalle de nota de compra ******************/
	case 538:
 ...
 VALUES($Par_Sql[0],$Par_Sql[1],UPPER('$Par_Sql[2]'),'$Par_Sql[3]' ,'$Par_Sql[4]')";
		...
		break;
   /** Consulta de notas de compra para la retenci�n *******/		
   case 539:
...
ORDER BY det_compra.Cop_Int";
   ...
   break;
   /** Actualizaci�n de los items de las notas de compras  ********************/
   case 541:
   ...
   break;
   /** Consultar el tipo de transacci�n **************************/
   case 542:
   $con_tipo_tran="SELECT transaccio.Tra_Cod, transaccio.Tra_Des  FROM transaccio";
   return $con_tipo_tran;
   break;
	
	/** Consulta las retenciones de acuerdo al codifo del formulario de retenci�n por fecha de compra *******************************************/
		case 543:
...
//echo $carg_reten_fechas_for;
		...
		break;
			/** Calculo del valor de la retencion por codifo de formulario de retenci�n ********************/
		case 544:
		$importe_porce_rete="SELECT det_retenc.Ret_Bas, renta_iva.Ren_Por FROM det_retenc, renta_iva
		...
		break;
		 /*** 21 DE JUNIO               ****************/
 /*** Consulta de comprobantes de compra por fecha de retenci�n *******************************************************************/
        case 545:
...
AND retencion.Ret_Est='$Par_Sql[3]'  AND  autorizaci.Aut_Cod= retencion.Aut_Cod 
 ...
 return $carg_reten_fechas_ret;
     ...
     /** Consulta las retenciones de acuerdo al porcentaje de retenci�n por fechas de retencion *******************************************/
		case 546:
		$carg_reten_fechas_por="SELECT persona.Prs_Nom,renta_iva.Ren_Por,persona.Prs_Ape, proveedore.Prv_Cod, compras.Cop_Cod,
compras.Cop_Num,compras.Cop_Fec, compras.Cop_Cad, retencion.Ret_Est, retencion.Ret_Num, 
retencion.Ret_Cod, retencion.Ret_Fec, det_retenc.Ret_Int, det_retenc.Ret_Bas, det_retenc.Ret_Cod, 
...
//echo $carg_reten_fechas_por;
 ...
 return $carg_reten_fechas_por;
		break;
/** Consulta las retenciones de acuerdo al codifo del formulario de retenci�n por fecha de retenci�n *******************************************/
case 547:
		$carg_reten_fechas_for="SELECT persona.Prs_Nom,renta_iva.Ren_Por ,renta_iva.Ren_Sri,persona.Prs_Ape, proveedore.Prv_Cod, compras.Cop_Cod,
compras.Cop_Num,compras.Cop_Fec, compras.Cop_Cad, retencion.Ret_Est, retencion.Ret_Num, 
retencion.Ret_Cod, retencion.Ret_Fec, det_retenc.Ret_Int, det_retenc.Ret_Bas, det_retenc.Ret_Cod, 
...
AND renta_iva.Ren_Cod='$Par_Sql[4]'
 GROUP BY retencion.Ret_Cod ORDER BY retencion.Ret_Num";
       //echo $carg_reten_fechas_for;
		...
		break;
/*** OJO LLEVAR UESMA  **********************************/
/** Consulta de totales por codigo de formulario de retenci�n y fecha de comprobante de compras ***************/
	   case 548:
...
//echo $consul_total_form_ret;
       ...
       break;
/** Consulta de totales por codigo de formulario de retenci�n y fecha de comprobante de retenci�n ***************/
    case 549:
...
AND tipo_compr.Tic_Cod=retencion.Tic_Cod AND persona.Prs_Cod=proveedore.Prs_Cod AND proveedore.Prv_Cod=compras.Prv_Cod";
    //    echo $consultar_reten_dup_551;
		...
		break;
		/***** Consulto si la factura de compra a duplicar ya tiene registrada una retenci�n con estado A=Activa *************************/
        	case 552:
...
GROUP BY renta_iva.Ren_Sri, renta_iva.Ren_Por ORDER BY det_retenc.Ret_Int ASC";
	...
	/* B�squeda de un personal por apellido */
		case 606:
...
WHERE Prs_Ape LIKE '%$Par_Sql[0]%' AND personal.Prs_Cod=persona.Prs_Cod";
	...
	/* B�squeda de un personal por C�dula */
		case 607:
...
//echo $rubros_deudas;
	   ...
	   break;
		...
		case 654:
		/* Consulta de estudiante por c�dula, mostrando las carreras en las que esta matriculado  */								
...
WHERE
  persona.Prs_Cod = $Par_Sql[0]";
		...
		break;		
		/**** Consulto si existe el cliente por c�digo de persona *************/
		case 659:
		/* Consulta para verificar si el cliente se encuentar registrado */
...
WHERE
  view_periodos_suc.Per_Int = $Par_Sql[0]";		
		...
		/* Caegado del N�mero de Comprobate que sigue */
		case 667:
...
IF(persona.Prs_Esc = 'V', 'Viudo/a', IF(persona.Prs_Esc = 'D', 'Divorciado/a', IF(persona.Prs_Esc = 'U', 'Uni�n Libre/a', ''))))) 
...
AND recibo.Rcb_Cod = liquidacio.Rcb_Cod AND liquidacio.Cja_Cod='NULL' GROUP BY recibo.Rcb_Cod";
	...
	/* CUENTAS POR PAGAR */
		...
		case 802:/*consulta de provedores que tiene pagos pendientes por c�dula*/
		...
		/* Inserci�n de detalle del pago de la factura de credito en la tabla det_ccpp_p */
		case 807:
		...
		break;
		 /* Inserci�n del anticipo en la tabla anticipos */
		case 829:
		...
		return $ins_anticipo;
		break;
		 /* Inserci�n de Com_Cod y Ant_Cod en l atabla compr_anti */
		case 830:
  ...
  /*sentencias roles de pago  */
   /* busqueda del empleado para obtener sueldo*/
	...
	//echo  $rol_detalle;
	 ...
	 /*Selcci�n de cuentas para insertar en el asiento contable*/		
	 case 864:
...
AND det_rpagos.Cam_Cod= campo_rol.Cam_Cod AND campo_rol.Cam_Tip= 'E' AND campo_rol.Cam_Vis= 'S' ";
	 ...
	 break;
	  /*inserci�n de ccpp_rol Almacena los comprobantes que estan como pagos pendientes*/
	 case 877:
 ...
 /* Inserci�n de detalle del pago de la factura de credito en la tabla det_ccpp_p ojojojoj*/
 case 884:
...
/* Consulta de la configuraci�n de rol */
 case 895:
