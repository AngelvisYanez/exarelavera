...
if (isset($searchMovimientos)) {
   ...
   try{
    ...
    if(trim($search)!=""){
      ...
      $resp = $obBD_con1->getPageGrid(20, $data, $obBD_conexion);
   }catch (Exception $exc){
      $obBD_con1->echoLog($exc->getTraceAsString());
   }
   $obBD_con1->echoJson($resp);
...
if(isset($getItems)){
   $data = $_GET;
    try{
      $resp['data'] = $obBD_con1->getArrayConsulta(21, $data, $obBD_conexion);
      $resp['success']=true;
    }catch (Exception $exc){
      $obBD_con1->echoLog($exc->getTraceAsString());
    }
...
if(isset($saveMovimiento)){
    ...
    try {
        ...
        // datos de estado de usuario permitido para administrar movimientos
        $data['Prv_Cod']=$proveedor['Prv_Cod'];
        $data['Vnd_Cod']=$vendedor['Vnd_Cod'];
        $data['Iva_Cod']=$iva['Iva_Cod'];
        // datos considerados para ajuste
        $data['Aju_Tip']='I';
...
<HTML>
	<HEAD>		
		...
		<script src="../VALIDACIONES/bod_rec_mov.js"></script>
		<style>                     
	...
	<BODY>
		<div class="panel panel-main">
			<div class="panel-heading exa-header"><h3 class="panel-title">&raquo;Confirmar Movimiento</h3></div>
			<div class="panel-body ui-widget-content ui-corner-bottom exa-body">
                <div id="documentoSearch">
                    <?php include("COMPONENTES/form_search_movimiento.html");?>
                </div>
