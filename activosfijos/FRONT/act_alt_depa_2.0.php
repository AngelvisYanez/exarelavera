...
if (isset($depaAjax)) {
    ...
    $contar = $obBD_con1->getRowConsulta(711, $data, $obBD_conexion);
    $pagination = pages($contar['total'], $page, $rows);
    $responce = $pagination['data'];
    $data["limits"] = $pagination['limits'];
    if ($contar['total'] > 0)
        $responce['rows'] = $obBD_con1->getArrayConsulta(711, $data, $obBD_conexion);
...
<HTML>
    ...
    <BODY>
        <div class="panel panel-main">
            <div class="panel-heading exa-header"><h3 class="panel-title">&raquo;  Asignar Departamento</h3></div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="col-md-6 col-sm-6">
                                    ...
                                    <div class="form-group">
                                        <label class="control-label col-md-3 col-sm-4 label-sm required">Departamento:</label>
                                        <div class="col-md-8 col-sm-8">
                                            <div class="input-group input-group-xs">
                                                <input type="text" id="departa" name="departa" class="form-control input-xs" readonly="" placeholder="Seleccione un departamento">
                                                <span class="input-group-btn">
                                                    <button type="button" id="bt_buscar" name="bt_buscar" class="btn btn-success" onclick="$('#depaDialog').dialog('open');"><span class="glyphicon glyphicon-search" title="Buscar departamento"></span></button>
                                                </span>
                                            </div>
                                        </div>
                                    </div>
        ...
        <div id="depaDialog" title="B&uacute;squeda de Departamentos">  
            <form class="form-horizontal normal"> 
                <fieldset class="exa-fieldset">
                    <legend class="Titulos2">Filtros</legend>
                    <div class="form-group">
                        ...
                        <div class="col-md-7" >                 
                            <div class="input-group">                        
                                <input name="search" onkeydown="if (event.keyCode === 13)
                                            this.form.submit()" type="text" size="50" maxlength="50" placeholder="Ingrese departamento a buscar..." autofocus class="form-control input-sm " />
                                <span class="input-group-btn"><button type="button" onclick="this.form.submit()" class="btn btn-success btn-sm" title="Buscar departamento" ><span class="glyphicon glyphicon-search"></span> <span> Buscar</span></button></span>
                            </div>                         
                        </div>                    
                    </div>
                </fieldset>  
        ...
        <script type="text/javascript">
            ...
            $(function () {
                $.createSearchDialog('#depaDialog', [
                    {label: 'Cod.Int.', name: 'Dep_Cod', key: true, width: 30},
                    {label: 'Departamento', name: 'Dep_Des', width: 145},
                    {label: 'Cuenta Contable', name: 'Pld_Des', width: 100},
                    {label: '<center><i class="ui-icon ui-icon-gear"></i></center>', name: 'act1', width: 18, align: 'center', viewable: false,
                        formatter: function (cellvalue, options, rowObject) {
                            var clic = '$("#departa").val("' + rowObject.Dep_Des + '");$("#Dep_Cod").val("' + rowObject.Dep_Cod + '");$("#depaDialog").dialog("close");';
            ...
            $(function(){
                $('#imprimir').click(function (){
                    window.open('./act_pri_custodio_3.0.php?Dep_Cod='+Dep_Cod,'_blank');
                });
            });
        </script>
    </BODY>
