		...
		{	
			...
			case 608:				
				$sql = "SELECT Tia_Cod, Tia_Des, Tia_Est, Tia_Rec, Tia_Cdc, Tia_Dep, Emp_Cod, Tia_Tip, Tia_Obs, Tia_Cod as id, 
						...
						IF(Tia_Rec=0,'fa fa-hand-o-right red bold',IF(Tia_Tip='G','glyphicon glyphicon-folder-open blue','fa fa-file-text green')) as icon, Apr_Cod 
						FROM tipo_activo 
			...
			/**
			* Permite listar los campos indistintamente al tipo de activo al cual pertenezcan
			*/
			case 616:
					$sql = "SELECT DISTINCT(campos_act.Cam_Cod),Cam_Lar,Cam_Cor,Cam_Tip,Cam_Bus,Cam_Obs,Cam_Ord,Cam_Req 
							FROM campos_act,campos_plan,tipo_activo
							WHERE campos_act.Cam_Cod=campos_plan.Cam_Cod AND campos_plan.Tia_Cod=tipo_activo.Tia_Cod AND tipo_activo.Emp_Cod='$Par_Sql[0]'";
					//echo $sql;
					return $sql;
			break;
			
			/**
			 * Eliminar campo de la tabla campos_act
			 */
			case 617:
					$sql = "UPDATE campos_act SET Cam_Est='I' WHERE Cam_Cod=$Par_Sql[0]";
					return $sql;
			break;	
			/**
			 * Verifica si el Cam_Cod y Tia_Cod existen en la tabla campos_plan
			 */
			case 618:
					$sql = "SELECT * FROM campos_plan WHERE Cam_Cod=$Par_Sql[0] AND Tia_Cod=$Par_Sql[1]";
					return $sql;
			break;

			case 619:
					$sql = "SELECT * FROM activo_porcent WHERE Suc_Cod=$Par_Sql[0]";
					return $sql;
			break;

			case 620:
				$sql = "UPDATE tipo_activo SET Apr_Cod='$Par_Sql[1]' WHERE Tia_Cod=$Par_Sql[0]";
				return $sql;
			break;
