<?Php
require_once('../../tesoreria/LOGICA/tes_log_compras.php');
...
{	
	if ($op_opciones=='d')
	{
		...
		$rs_buscta = $obBD_con1->consulta(sentencias_comf(11, $obBD_con1->parametros($ajax_buscod.'*'.$Ses_Emp_Cod.'*'.$Pla_Cod)), 
									$obBD_conexion->conexion);
	}
	if ($op_opciones=='c')
	{
		// Cargado de los resultados de la busqueda por codigo de la cuenta
		$rs_buscta = $obBD_con1->consulta(sentencias_comf(12, $obBD_con1->parametros($ajax_buscod.'*'.$Ses_Emp_Cod.'*'.$Pla_Cod)), 
									$obBD_conexion->conexion);
	}
	...
	<table width="100%" height="20" border="1" cellpadding="0" cellspacing="0" class="Fondo">
	  ...
	  do { 
		...
		$rs_recur = $obBD_con1->consulta(sentencias_comf(204, $obBD_con1->parametros($row_rs_buscta['Pld_Rec'])), 
		$obBD_conexion->conexion);
		$row_rs_recur = $obBD_con1->registros();	
		/* Consulta del detallete de la CUENTA (OTRO) */
		$rs_grupo = $obBD_con1->consulta(sentencias_comf(204, $obBD_con1->parametros($row_rs_recur['Pld_Rec'])), 
		$obBD_conexion->conexion);
	  ...
	  <tr class="Cuerpo_ajax">
		...
		<td align="left"><?php echo $row_rs_buscta['Pld_Des']; ?></td>
		...
		<td align="center"><img src="../../mascaras/model1/imagenes/forward.png" width="16" height="16" title="Agregar cuenta" style="	
		cursor:pointer" onClick=" 
							...
							document.getElementById(document.getElementById('Hdd_Pld_Des').value).value = '<?Php echo $row_rs_buscta['Pld_Des']; ?>';"></td>
	  </tr>
