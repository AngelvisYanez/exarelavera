...
?>				
        <FIELDSET>
          <LEGEND>
            <label class="Titulos2">B&uacute;squeda de cuenta contable: </label>
          </LEGEND>
				<table width="100%" border="0" cellspacing="0" cellpadding="0" id="Tbl_Cuentas">
				  <tr>
					<td>
					  ...
					  <table width="100%" height="36" border="0" cellpadding="0" cellspacing="0">
						<tbody id="tbusqueda">
						  <tr class="BarraBusqueda">
							<td width="72" height="28" ><div align="right"><strong>Cuenta:</strong></div></td>
							<td width="399" ><input name="<?Php echo $Nombre_Buscador; ?>" type="text" id="<?Php echo $Nombre_Buscador; ?>" size="50" maxlength="50" style="text-transform:uppercase" onKeyUp="parametro_injection(this)" onKeyPress="if(trim(document.getElementById('<?Php echo $Nombre_Buscador; ?>').value) != ''){ enter_ajax('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_buscador=<?Php echo $tipo_busc; ?>&ajax_buscod='+document.getElementById('<?Php echo $Nombre_Buscador; ?>').value+'&op_opciones='+document.getElementById('<?Php echo $Nombre_Opciones; ?>').value+'&Pec_Cod=<?Php echo $Com_Pec_Cod; ?>&Pla_Cod=<?Php echo $Pla_Cod; ?>','<?Php echo $Capa; ?>')} "></td>
							<td width="120" align="center">
                            <div id="set1">
                             <button type="button" class="btn btn-success fileinput-button" title="Buscar cuenta" id="btn_buscarcta" name="btn_buscarcta" onClick="if (trim(document.getElementById('<?Php echo $Nombre_Buscador; ?>').value) != ''){ ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_buscador=<?Php echo $tipo_busc; ?>&ajax_buscod='+document.getElementById('<?Php echo $Nombre_Buscador; ?>').value+'&op_opciones='+document.getElementById('<?Php echo $Nombre_Opciones; ?>').value+'&Pec_Cod=<?Php echo $Com_Pec_Cod; ?>&Pla_Cod=<?Php echo $Pla_Cod; ?>','<?Php echo $Capa; ?>') }"> <i class="icon-search icon-white"></i> <span>Buscar</span> </button>                              </div>                          
							</td>
						  </tr>
						</tbody>
					  </table>
