...
?>
        ...
        <input type="checkbox" name="checkbox" value="checkbox" onClick="if (this.checked){ document.getElementById('hdd_sql').value = 2; }
																else { document.getElementById('hdd_sql').value = 3; }; ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_mod_cod=1&sql='+document.getElementById('hdd_sql').value+'&Suc_Cod=<? echo $Suc_Cod;?>&Mod_Cod=' + document.getElementById('Mod_Cod').value,'div_etapa')">
      Otras Etapas
      <!-- 
2 = Etapas nivelacion
...
?>
        <select name="Eta_Cod" id="Eta_Cod" style="text-transform:uppercase"  onChange="ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_periodo=1&Mod_Cod=<?Php echo $Mod_Cod; ?>&Suc_Cod=<? echo $Suc_Cod;?>&Eta_Cod='+this.value,'div_periodo')">
            <option></option>
            <?Php do { ?>
