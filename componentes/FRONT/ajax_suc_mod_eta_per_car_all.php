...
{	/* Control para saber las carreras de cada etapa */
	...
	{
		do {
			$cadTodo=$cadTodo.'*'.$row_rs_carrera_etapa['Car_Int'];
		} while($row_rs_carrera_etapa=mysqli_fetch_assoc($rs_carreras_etapa));
		/* Retornamos el puntero del recorset al inicio */
...
if(isset($ajax_nivel))
{
	...
	$rs_semestres=$obBD_con1->consulta(sentencias_com(7, $obBD_con1->parametros($Car_Int.'*'.$Per_Int.'*'.$Mod_Cod)), $obBD_conexion->conexion);
