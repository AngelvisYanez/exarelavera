<?php 
/**
 * Componente para la busqueda de la modalidad-periodo[etapa]-carrera para inscripciones
 * Se utiliza el componentes Ajax ==> ajaxSucModPerActCar.php 
 * Sucursal= Todas
 * Modalidad = todas
 * Periodo = Los periodos antes o proximos a iniciar
 * Carreras = Filtra carreras que se habran en el periodo lectivo
 * Curso = Los abiertos en el periodo lectivo
 * 
 * @author car.87cod :)
 * @version 1.1
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	 ...
	 */
	$rs_periodos = $obBD_con11->getArrayConsulta(308, $Mod_Cod.'*'.$Com_Hoy.'*'.$Suc_Cod, $obBD_conexion11);
  ...
  <tr>
	...
	{
			?>
            <input name="Sem_Nom" id="Sem_Nom" type="text" value="<?Php echo $rs_semestres[0]['Sem_No2']; ?>" readonly="readonly" size="50" style="border:none; background:none" />
...
<?Php
	...
	?>
			...
			{  
			?>
			<option style="text-transform:uppercase" value="<?Php echo $row_rs_semestres['Sem_Cod'];  ?>"><?Php echo $row_rs_semestres['Sem_No2'];  ?></option>
			<?Php 
			} 
			?>
			</select>
