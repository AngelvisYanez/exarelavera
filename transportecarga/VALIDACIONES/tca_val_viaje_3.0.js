    ...
    if($('#reportForm').length>0){
        ...
        $("#viajesReport").createGrid({
            ...
            colModel: [
                ...
                {label: 'Fecha', name: 'Via_Fec', width: 30, align: "center"},
                {label: 'C.I.&nbsp;Chofer', name: 'Ruc_Chofer', width: 40, excel:'text',hidden:true },
		...
		{label: 'Cargamento', name: 'Car_Des', width: 25, align: "center", summaryType:'count',summaryTpl:$.fieldSummary()},
                ...
                {label: 'Origen', name: 'Via_Ded', width:30, formatter:'union', formatoptions:{sep:' - ',cols:['Ori_Zon']} },
                {label: 'Destino', name: 'Via_Has', width:30, formatter:'union', formatoptions:{sep:' - ',cols:['Des_Zon']} },
                {label: 'Booking', name: 'Via_Has', width: 25,hidden:true},
                {label: 'Conten.', name: 'Via_Con', width: 25, hidden:true},
                {label: 'Sello', name: 'Via_Sel', width: 25, hidden:true},
                {label: 'Km(s).', name: 'Via_Kil', width: 20, formatter:'number', hidden:true, summaryType:'sum', formatoptions:{decimalPlaces:0, defaultValue:''}, classes:'columnHighlight3'},
                ...
                {label: 'Obs.', name: 'Via_Des', width: 15, align: "center", hidden:true, formatter:'truefalse', formatoptions:{ yesMsg:function(o){ return o.Via_Des; }, noMsg:' ', yesIcon:'info-sign', noIcon:' ', yesColor:'blue', noText:false }, title:false },
...
function exportarReporte(){
    ...
    $.reportExa(grid, { excel:true, file:'Viajes',
        ...
        configs:$.extend({caption:true, unHideAll:true, removeCols:[0,3,4,7,10,13,14,15,16,22,24,25,26,27,28,29].concat(type!=='cliente'?[/*17,18,19,22*/20,21,24]:[1,2,/**/18,19]), colorGroup:true},type!=='cliente'?{footer:false,totalRows:false}:{})
    });
}
function imprimirReporte(){
    ...
    $.reportExa(grid, {
        subTitle:grid.getCaption()+' Desde '+ $("#Fec_Ini").val() +' Hasta '+$("#Fec_Fin").val(),
        configs:$.extend({generated:false, removeCols:[6].concat(type!=='cliente'?[/*9,10,11*/]:[])},type!=='cliente'?{footer:false,totalRows:false}:{})
    });
}
