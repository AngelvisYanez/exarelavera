...
//    $busqueda = array(
//        'success' => true,
//        'todosViajes'=>$obBD_con1->getArrayConsulta('viaje.selectWhere', array('viaje.Cli_Cod'=>$Cli_Cod, 'setWhere'=>array(/*'byChofer',*//*'byPersona',*//*'byCargamento',*//*'byModoTrabajo',*/'isActive', /*'byVehiculo',*/ 'notHasVetCod')),$obBD_conexion, true),
//		);
...
/*Secci�n para verificar si una persona ya se encuentra registrada como chofer*/
...
/*Secci�n para listar los viajes de un cliente*/
...
/*Secci�n para guardar datos de los agregar*/
if(isset($save)){
    ...
    if(isset($saveCargamento)){$case=4;$consultar=2;$datos=$Car_Des.'*'.$Pro_Cod;}              //Secci�n para registrar un cargamento
    if(isset($saveModo)){$case=5;$consultar=3;$datos=$Mot_Des.'*'.$Ses_Emp_Cod;}                                 //Secci�n para registrar un modo_trabajo
    if(isset($saveAutomotor)){$case=7;$consultar=8;$datos=$Ses_Emp_Cod.'*'.$Veh_Mar.'*'.$Veh_Pla.'*'.$Veh_Col;}  //Secci�n para registrar un vehiculo
    if(isset($saveChofer)){
        $longitud=strlen($Prs_Ced);
    ...
    }                                                                                           //Secci�n para registrar un chofer
...
/*Secci�n para registrar un viaje*/
if(isset($saveViaje)){
    $response['success'] = false;
    $response['message'] = "No se ha logrado realizar la Transaccion";
    $obBD_con1->inicio_transaccion($obBD_conexion->conexion);
    foreach ($campos as $valor){
        if($valor['Via_Aux']=='N'){
            $obBD_con1->operacionobBD(17,$valor['Cho_Cod'].'*'.$valor['Car_Cod'].'*'.$valor['Mot_Cod'].'*'.$Cli_Cod.'*'.$valor['Veh_Cod'].'*'.$valor['Via_Ded'].'*'.$valor['Via_Has'].'*'.$valor['Via_Fec'].'*'.$valor['Via_Can'].'*'.$valor['Via_Pru'].'*'.$Via_Des, $obBD_conexion,true);
        }else{
            $obBD_con1->operacionobBD(23,$valor['Via_Cod'].'*'.$valor['Cho_Cod'].'*'.$valor['Car_Cod'].'*'.$valor['Mot_Cod'].'*'.$Cli_Cod.'*'.$valor['Veh_Cod'].'*'.$valor['Via_Ded'].'*'.$valor['Via_Has'].'*'.$valor['Via_Fec'].'*'.$valor['Via_Can'].'*'.$valor['Via_Pru'].'*'.$valor['Via_Est'], $obBD_conexion,true);
        }
    }
    $obBD_con1->fin_transaccion_nomsn($obBD_conexion->conexion);
...
<HTML>
    ...
    <BODY>
        ...
        <!-- Inicio del di�logo para agregar un cargamento -->
        ...
        <!-- Inicio del di�logo para agregar un modo trabajo -->
        ...
        <!-- Inicio del di�logo para agregar un automotor -->
        ...
        <!-- Inicio del di�logo para agregar un chofer -->
        ...
        <!-- Inicio del di�logo para buscar una persona -->
