...
class Zend_Db_Select
{
    ...
    function setDataSelect($_parts) { $this->_parts = $_parts; }
    function getDataSelect() { return $this->_parts; }
    function addCols($table, $cols) { if (is_string($cols) || $cols instanceof Zend_Db_Expr) $cols = array($cols); $this->_tableCols($table, $cols, $table); return $this; }
    function unsetFrom() { unset($this->_parts['columns'][0]); list($firstKey) = array_keys($this->_parts['from']); unset($this->_parts['from'][$firstKey]); return $this; }
    function unsetWhere() { $this->_parts['where'] = array(); return $this; }
    function unsetCols($cols=null) {
        if ($cols == null) {
            $this->_parts['columns'] = array();
        } else {
            if (is_string($cols)) $cols = array($cols);
            if (isset($cols['tables']) && is_array($cols['tables'])) {
                foreach ($cols['tables'] as $t) $this->unsetColsbyTable($t);
                unset($cols['tables']);
            }
            foreach ($cols as $col) {
                $t = count($this->_parts['columns']) - 1;
                for ($i = $t; $i >= 0; $i--) {
                    if ((isset($this->_parts['columns'][$i][2]) && $this->_parts['columns'][$i][2] == $col) || $this->_parts['columns'][$i][1] == $col) {
                        unset($this->_parts['columns'][$i]);
                    }
                }
                $this->_parts['columns'] = array_values($this->_parts['columns']);
            }
        }
        return $this;
    }
    function unsetColsbyTable($table) {
        $t = count($this->_parts['columns']) - 1;
        for ($i = $t; $i >= 0; $i--) {
            if ($this->_parts['columns'][$i][0] == $table)
                unset($this->_parts['columns'][$i]);
        }
        $this->_parts['columns'] = array_values($this->_parts['columns']);
        return $this;
    }
    function hasTable($table) { return array_key_exists(trim($table), $this->_parts[self::FROM]); }
    /**
    ...
    public function union($select = array(), $type = self::SQL_UNION)
    {
        if (!is_array($select)) {
            throw new Exception(
                "union() only accepts an array of Zend_Db_Select instances or sql query strings."
            );
        }
    ...
    protected function _where($condition, $value = null, $type = null, $bool = true)
    {
        ...
        if ($value !== null) {
            if (is_array($value) && !empty($value)) {
                if (substr_count($condition, '?') > 1 && substr_count($condition, '?') == count($value)) {
                    foreach ($value as $i => $val) {
                        $condition = $this->_adapter->quoteInto($condition, $val, is_array($type) ? $type[$i] : $type, 1, is_numeric($i) ? null : "?$i");
                    }
                } else {
                    $or = array();
                    foreach ($value as $i => $val) {
                        $or[] = $this->_adapter->quoteInto($condition, $val, is_array($type) ? $type[$i] : $type, null, is_numeric($i) ? null : "?$i");
                    }
                    $condition = implode(" " . self::SQL_OR . " ", $or);
                }
            } else {
    ...
    protected function _renderLimitoffset($sql)
    {
         ...
         */
        if ($count > 0) {
            $sql = "\n" . trim($this->_adapter->limit($sql, $count, $offset));
        }
