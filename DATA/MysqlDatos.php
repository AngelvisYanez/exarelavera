...
class MysqlDatos
{
    ...
    public $lastNumSql = 0; // ultima sql
    /* @var string */
    public $beforeLastSqlQuery = '';
    public $lastSqlQuery = '';     // ultima sql
    public $MsgError = ''; // msg del error de MySql
    public $sentencias_sql; // string - funcion q arma las sql
    public $sentencias = ''; //audit //guardara las sql concatenadas con *
    public $codigos = ''; //audit //guarda los cods de insercion
    /* M E T O D O S */
    function __construct($con = null, $sqls = null)
    {
    ...
    function getDB($c = null)
    {
        ...
        if (is_object($c) && ($c instanceof MysqlConexion))
            return $c->getDB();
        if (is_object($c) && get_class($c) == 'mysqli' && !$this->conDB) {
    ...
    function setConnection($con)
    {
        ...
        } else if (!is_null($con)) $this->con = (is_object($con) && ($con instanceof MysqlConexion)) ? $con->conexion : (is_object($con) && get_class($con) == 'mysqli' ? $con : null);
    }
    ...
    {
        return is_null($c) || is_bool($c) ? (!is_null($this->con) ? $this->getMyCon($this->con) : null) : (is_object($c) && ($c instanceof MysqlConexion) ? $c->conexion : $c);
    }
    ...
    function consulta($sql, $conexion = null)
    {
        ...
        return $this->rs_cargar; /* Si hubo ï¿½xito devuelve el identificador de la conexiï¿½n, sino devuelve 0  */
    }
    ...
    function commit($conexion = null, $msge = 1)
    {
        ...
        if (!$msge) return; ?><script type="text/javascript">alert("La transacción se ha realizado con Ã‰xito!");</script><?php
    }
    ...
    function rollBack($conexion = null, $msge = 1)
    {
        ...
        if (!$msge) return; ?><script type="text/javascript">alert("< < < !!! A l e r ta !!!: NO se ha podido completar con Ã‰xito la transacción > > >");</script><?php
    }
    ...
    {
        if (!($rs_consulta instanceof mysqli_result)) return array();
...
class MysqlDatosContab extends MysqlDatos
{
    ...
    {
        $c = $this->getMyCon($obBD); /* Codificaciï¿½n numerica en base al periodo contable y mensualmente  */
