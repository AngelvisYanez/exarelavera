...
/**
 * @abstract Permite duplicar docuemntos de venta
 * @author Exa Contable
 * @version 1.0
...
if (isset($_POST['numeroSec'])) {
    $Tic_Cod = $_POST['Tic_Cod'];
    $Aut_Sri = $_POST['Aut_Sri'];
    $Pun_Sri = $_POST['Pun_Sri'];
    $Aut_Cod = $_POST['Aut_Cod'];
    $Aut_Ini_Front = isset($_POST['Aut_Ini']) ? $_POST['Aut_Ini'] : '';
    $Aut_Fin_Front = isset($_POST['Aut_Fin']) ? $_POST['Aut_Fin'] : '';

    $response = $obBD_con1->getRowConsulta(9, $Ses_Prs_Cod . '*' . $Ses_Suc_Cod . '*' . $Tic_Cod . '*' . $Aut_Cod, $obBD_conexion);

    if (!$response || !isset($response['Aut_Ini'])) {
        if (!empty($Aut_Ini_Front) && !empty($Aut_Fin_Front)) {
            $response = array(
                'Aut_Ini' => $Aut_Ini_Front,
                'Aut_Fin' => $Aut_Fin_Front,
                'Aut_Sri' => $Aut_Sri
            );
        } else {
            echo json_encode(array('error' => 'No se encontró autorización.'));
            exit;
        }
    }

    $param10 = $response['Aut_Ini'] . '*' . $response['Aut_Fin'] . '*' . $response['Aut_Sri'] . '*' . $Tic_Cod . '*' . $Ses_Suc_Cod . '*' . $Pun_Sri;
    $siguiente = $obBD_con1->getRowConsulta(10, $param10, $obBD_conexion);

    if (isset($siguiente['siguiente']) && $siguiente['siguiente'] != '') {
        $response['Vet_Num'] = $siguiente['siguiente'];
    } else {
        $response['Vet_Num'] = $response['Aut_Ini'];
    }

    if (isset($siguiente['contador'])) {
        $response['contador'] = $siguiente['contador'];
    } else {
        $response['contador'] = 0;
    }

    if (isset($siguiente['Aut_Tem'])) {
        $response['Aut_Tem'] = $siguiente['Aut_Tem'];
    } else {
        $response['Aut_Tem'] = 'F';
    }
...
$ivas = $obBD_con1->getArrayConsulta(16, "", $obBD_conexion); //Seccion para obtener los ivas de la tabla iva
...
if (isset($saveDocument)) {
    ...
    if ($obBD_conIns->Error == 0) {
        ...
        if (isset($rets)) {
            ...
            $alert = "No se han registrado retenciones para este documento";
        }
...
<HEAD>
    ...
    <TITLE><?php echo "Ventas Duplicar [EXA]"; ?></TITLE>
    <meta charset="UTF-8">
...
<BODY>
    ...
    <div class="panel panel-main">
        <div class="panel-heading exa-header">
            <h3 class="panel-title ">&raquo; Duplicar Documentos de Ventas</h3>
                        ...
                        <div class="col-xs-6">
                            <fieldset class="exa-fieldset">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-1">
                                        <input type="checkbox" value="S" offval="N" id="mis_ingresos" name="mis_ingresos" style="margin-left: -18px;" />
                                    </div>
                                </div>
                            </fieldset>
                        </div>
                                ...
                                <div class="col-md-7 col-xs-12">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="col-xs-10">
                                                <textarea name="com_con" id="com_con" class="form-control input-xs"
                                                    onfocus="this.value = this.value.replace(/REG\. VENTA\s+\d+\s*\/?/i, 'REG. VENTA ' + ($('#Vet_Num').val() || '') + ' /');"
                                                    oninput="if(this.value.match(/^REG\. VENTA\s+\d+\s*\/?/i)){ this.value = 'REG. VENTA ' + ($('#Vet_Num').val() || '') + ' /'; }"
                                                ></textarea>
                                            </div>
                                        </div>
                                    </div>
        ...
        function selectCliente(cliente) {
            ...
            if (Pec_Cod && Cli_Cod) {
                $.ajax({
                    ...
                    type: "POST",
                    dataType: "json",
                    data: { saldoCCxCC: true, Pec_Cod: Pec_Cod, Cli_Cod: Cli_Cod },
                    beforeSend: function() {
