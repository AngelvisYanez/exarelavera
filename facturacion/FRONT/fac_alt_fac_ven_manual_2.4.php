...
{   
	...
	*/
	...
	{ 
		$info="Comprobante Electr&oacute;nico";
	}
	else 
	{
	...
	if(count($row_rs_autorizaci) > 0)
	{
		...
		if($Vet_Num>=$row_rs_autorizaci['Aut_Ini'] && $Vet_Num<=$row_rs_autorizaci['Aut_Fin']){
		?>  <table width='85%' border='0' cellpadding='0' cellspacing='0'>
			 <tr>
			   ...
			   <td width='15%'><input name="Vet_Num" type="text" id="Vet_Num" size="7" maxlength="7" value="<?php echo $Vet_Num;?>" style="text-align:right" onKeyPress="return validar_numeric(event)" onBlur="ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_notif=1&Aut_Cod=<?php echo $row_rs_autorizaci['Aut_Cod']; ?>&SecIni=<?php echo $row_rs_autorizaci['Aut_Ini']; ?>&SecFin=<?php echo $row_rs_autorizaci['Aut_Fin']; ?>&AutoSri=<?php echo $row_rs_autorizaci['Aut_Sri']; ?>&NumFact='+ this.value,'div_notif')">
                                ...
                                <input name="Aut_Sri" type="hidden" id="Aut_Sri" value="<?Php echo $row_rs_autorizaci['Aut_Sri']; ?>">
			...
			echo "<span class='Alertas3'>&iexcl;El documento [".$Vet_Num."] esta fuera de rango, usted esta trabajando con el rango [".$row_rs_autorizaci['Aut_Ini']."] al [".$row_rs_autorizaci['Aut_Fin']."]!</span>";
		}
	}
	else
	{
		echo "<span class='Alertas3'>No existes registro de autorizaci&oacute;n para el tipo de comprobante</span>";
	}
	exit();
}
...
if(isset($ajax_notif))
{
	...
	$total_rs_buscaNumVenta=$row_rs_buscaNumVenta['Vet_Cod'] > 0? 1 : 0;
        $Vet_Num = $obBD_con1->codigoSiguiente($Aut_Cod, $SecIni, $obBD_conexion);	
...
$row_rs_apercaja = $obBD_con1->getRowConsulta(92, $row_rs_vendedor['Pun_Cod'],$obBD_conexion);
$total_rs_apercaja=$row_rs_apercaja['Pun_Cod'] > 0? 1 : 0;
if($total_rs_apercaja!=0)
{ 
	$row_rs_buscaAutori= $obBD_con1->getRowConsulta(1218, $row_rs_apercaja['Caj_Fec'].'*'.$row_rs_vendedor['Pun_Cod'],$obBD_conexion);
	$total_rs_buscaAutori=$row_rs_buscaAutori['Aut_Cod'] > 0? 1 : 0;
	if($total_rs_buscaAutori==1)
	{		
		$correcto=1;
		$row_rs_codPlaPeri= $obBD_con1->getRowConsulta(1224, $Ses_Emp_Cod.'*'.$row_rs_apercaja['Caj_Fec'],$obBD_conexion);
...
//Secci�n para verificar si el n�mero de factura existe
...
*/
//var_dump($_SESSION['postID']);var_dump($_POST['postID']);
	...
	*/
	...
	{ 
		...
		*/
		...
		if ($rs_infoEmpresa['Cof_Gce']=="S" && $rs_infoTipCom['Tic_Sri']!='00' && $rs_infoCliente['Aut_Tem']='E') /* Verifico si tiene autorizacion para generar F.E.*/
		{	
			$Vet_Aut="N"; //variable que nos indica que la factura electronica esta pendiente de envio al SRI
			for($i=strlen($Vet_Num); $i<=9-1; $i++)
			{ $ceroDoc=$ceroDoc."0";}
			 $cadena=date("dmY",strtotime($Caj_Fec)).str_pad($rs_infoTipCom['Tic_Sri'], 2, "0", STR_PAD_LEFT).$rs_infoEmpresa['Emp_Ruc'].$rs_infoEmpresa['Cof_Fac'].$rs_infoEmpresa['Suc_Sri'].$rs_infoCliente['Pun_Sri'].$ceroDoc.$Vet_Num."12345678"."1";			
			$factor = 2;
			$suma = 0;
		...
		*/
		...
		$Vet_Num.'*'.$Vet_Obs.'*'.$Aut_Cod.'*'.$Vet_Des.'*'.$hora.'*'.$url.'*'.$Vet_Aut.'*'.$Num_Ret.'*'.$Ret_Fec.'*'.$Num_Aut.'*'.$ForPagCodSri, $obBD_conexion);	
		...
		foreach ($datos as $puntero => $item)
		{
			...
			{                            
				...
				*/
				if (count($row_rs_adquisicio) <> 0)
				{
					...
					*/
					...
					if($rs_infoTipCom['Tic_Sri']!='00') 
                                        {
						$Kar_Int++;
						/** 
						* Actualiza el kardex 
						*/
						$obBD_ins1->operacionobBD(1072, $Vet_Cod.'*'.'0'.'*'.$Vnd_Cod.'*'.'0'.'*'.$param[0].'*'.$row_rs_apercaja['Caj_Fec'].'*'.$hora.'*'.'0'.'*'.(!$NCRED?$param[2]:($param[2]*-1)).'*'.$param[4].'*'.'0'.'*'.(!$NCRED?$param[5]:($param[5]*-1)).'*'.'0'.'*'.$desc_var.'*'.$param[8].'*'.'0'.'*'.$Kar_Int, $obBD_conexion);
						/** 
						* Consulta el Stock 
						*/
				...
				*/
                ...
                if($rs_infoEmpresa['Cof_Con']=="S" && $rs_infoTipCom['Tic_Sri']!='00')
                {		   
                    ...
                    foreach($productos as $fila){
                            ...
                            if($fila[15]!=""){
                                ...
                                $Asi_Val=(($fila[5]-$DsctoImp)*($fila[19]/100));$ban=true;
                                for($i=0;$i<count($retencion);$i++){
                                        if($retencion[$i]['Pld_Cod']==$row_rs_retcuen['Pld_Cod']){
                                            $retencion[$i]['Asi_Val']=$retencion[$i]['Asi_Val']+$Asi_Val;$ban=false;
                                        }
					...
					/* Agrego la cuenta de Descuentos*/
					if($t_descuento>0){
						//consultamos Pld_Cod con el parametro DV que significa Descuentos en Ventas
						$row_rs_ctaDsct = $obBD_con1->getRowConsulta(1317, 'DV'.'*'.$periodoCont['Pla_Cod'],$obBD_conexion);
						if($row_rs_ctaDsct['Pld_Cod']!=""){	
							$obBD_ins1->operacionobBD(1263, $Com_Cod.'*'.(!$NCRED?'D':'H').'*'.$t_descuento.'*Descuento*'.'Dscto. '.$t_descuento.'*'.$row_rs_ctaDsct['Pld_Cod'],$obBD_conexion);
						}
					}
		...
		*/
		if ($rs_infoEmpresa['Cof_Gce']=="S" && $rs_infoTipCom['Tic_Sri']!='00') /* Verifico si tiene autorizacion para generar F.E.*/
		{
		...
		*   GUARDAMOS AL CLIENTE COMO USUARIO DEL SISTEMA SOLO PARA FACTURAS ELECTRONICAS
		*/
		if ($rs_infoEmpresa['Cof_Gce']=="S" && ($rs_infoTipCom['Tic_Sri']!='00' || $rs_infoTipCom['Tic_Sri']!='0')) /* Verifico si tiene autorizacion para generar F.E.*/
		{					
			/*
			*  Conexion a la base Master
			*/
			$obBD_conexion_master = new Class_Log_Conexion_Tes('exa_master');
		...
		*/		
		...
		{   
			if ($rs_infoEmpresa['Cof_Gce']=="S" && $rs_infoTipCom['Tic_Sri']!='00') /* Verifico si tiene autorizacion para generar F.E.*/
			{																			
				if($rs_infoTipCom['Tic_Sri']=='1')
				{   
					/* Genera el Xml de la Fatura */
					include("../COMPONENTES/tesXmlFacturaElectronica_1.0.php");
				}
				if($rs_infoTipCom['Tic_Sri']=='4')
				{
				...
				$row_tipo_compr = $obBD_con1->getRowConsulta(1253, $Tic_Cod,$obBD_conexion);
				$fechaEmi = explode("-",$Caj_Fec);
				$copias = '';
				$msgHtml = '
				...
				<table width="597" border="0" cellpadding="0" cellspacing="0" align="center">
				  ...
				  <tr>
					<td valign="top" bgcolor="#D8E7C3"><table width="100%" border="0" cellspacing="0" cellpadding="0">
					  ...
					  <tr>
						<td height="47">&nbsp;</td>
						<td colspan="2" class="texto">Ha generado el siguente comprobante electr&oacute;nico a, '.$PrsNomCli.' con c&eacute;dula '.$PrsCedCli.'.<br><br>
						...
						<strong>Secuencia:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>'.$rs_infoEmpresa["Suc_Sri"].'-'.$rs_infoCliente["Pun_Sri"].'-'.$ceroDoc.$Vet_Num.'<br>
					...
					<td valign="top" bgcolor="#D8E7C3"><table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
						...
						<td width="96%" class="texto">Para descargar su Comprobante Electr&oacute;nico debe seguir los siguentes pasos:<br>
						  ...
						  <strong>Paso 3:</strong> Click en el bot&oacute;n <strong>Entrar</strong><br>
						  <strong>Paso 4:</strong> Cambiar la contrase?a
						  </td>
						<td width="2%">&nbsp;</td>
					  </tr>
					</table></td>
			...
			}// fin if ($rs_infoEmpresa['Cof_Gce']=="S" && $rs_infoTipCom['Tic_Sri']!='00')
		}// fin if ($obBD_conexion->Error==0)
...
/*Secci�n para obtener el cliente CONSUMIDOR FINAL = > Jos� Ambulud�*/
...
//Secci�n para buscar un cliente por c�dula por Jos� Ambulud�
if(isset($existeCliente)){
    /** 
    * Consulta del cliente en base de la cedula 
    */
    $response = $obBD_con1->getRowConsulta(22,$Prs_Ced.'*'.$Ses_Emp_Cod,$obBD_conexion);
    if(isset($response['Cli_Cod'])){
...
//Secci�n para guardar un cliente
if(isset($guardarCliente)){
    ...
    if($per_cod==''){
        if($ciu_cod=='')
        {
            $row_ciu_cod=$obBD_con1->getRowConsulta(1306,'',$obBD_conexion);
            $ciu_cod=$row_ciu_cod['Ciu_Cod'];
        }
        $obBD_con1->operacionobBD(1304, $prs_ced.'*'.$prs_nom.'*'.$prs_ape.'*'.$prs_sex.'*'.$prs_dir.'*'.$ciu_cod.'*'.$prs_cor, $obBD_conexion);
        $per_cod=$obBD_con1->insercionid($obBD_conexion->conexion);
    }
    $obBD_con1->operacionobBD(1305, $per_cod.'*'.$Ses_Emp_Cod.'*'.$cli_tic, $obBD_conexion);
    $cli_cod=$obBD_con1->insercionid($obBD_conexion->conexion);
    $response = $obBD_con1->getRowConsulta(22,$prs_ced.'*'.$Ses_Emp_Cod,$obBD_conexion);
    $response['Cli_Cod']=$cli_cod;
    $response['Persona']=true;
    echo json_encode($response);
    exit();
}

if (isset($ajax_cliente_registrar))
{	
?>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr class="BarraTitulo">
            <td width="45%" height="10">&raquo; Registrar Clientes </td>
        </tr>
        <tr style="background-color: #DFE9F6;"><td>&nbsp;</td></tr>
            ...
            <td>			
                <FIELDSET class="exa-fieldset">
                    ...
                    */
                    mensaje_requerido(); 
                    ?>
                    <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="formCliente" id="formCliente">
                    <table width="450" border="0">
                        <tr>
                            <td width="20%" class="Etiqueta1"><span class="Asterisco">* </span> C&eacute;dula/R.U.C.:</td>
                            <td width="80%" class="LetraNegra">
                                <input type="text" name="prs_ced" id="prs_ced" size="12" value="<?php echo $Prs_Ced; ?>"/>
                                <span id="mensaje_cedula" style="display: none;">C&eacute;dula Incorrecta</span>
                            </td>
                        </tr>
                        <tr>
                            <td width="12%" class="Etiqueta1"><span class="Asterisco">* </span> Nombre:</td>
                            <td width="88%" class="LetraNegra">
                                <input type="text" name="prs_nom" id="prs_nom" size="20"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="12%" class="Etiqueta1"><span class="Asterisco">* </span> Apellido:</td>
                            <td width="88%" class="LetraNegra">
                                <input type="text" name="prs_ape" id="prs_ape" size="20"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="12%" class="Etiqueta1">Sexo:</td>
                            <td width="88%" class="LetraNegra">
                        ...
                        <tr>
                            <td width="12%" class="Etiqueta1">Contribuyente:</td>
                            <td width="88%" class="LetraNegra">
                                <select name="cli_tic" id="cli_tic">
                        ...
                        <tr>
                            <td width="12%" class="Etiqueta1">Correo:</td>
                            <td width="88%" class="LetraNegra">
                                <input type="text" name="prs_cor" id="prs_cor" size="34"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="12%" class="Etiqueta1">Ciudad:</td>
                            <td width="88%" class="LetraNegra">
                                <?Php
                                /**
                                * Lista las ciudades
                                */
                                $row_ciudad = $obBD_con1->getArrayConsulta(1302, '', $obBD_conexion);	
                                ?>
                                <select name="ciu_Cod" id="ciu_cod">
                                  <option  value="">Seleccione...</option>
                                  <?Php
                                  foreach($row_ciudad as $row)
                                  { ?>
                        ...
                        <tr>
                            <td width="12%" class="Etiqueta1">Direcci&oacute;n:</td>
                            <td width="88%" class="LetraNegra">
                                <textarea name="prs_dir" cols="35" rows="6" id="prs_dir"></textarea>
                            </td>
                        </tr>
                        <tr>
                            <td width="12%"></td>
                            <td width="88%">
                                <button type="button" class="btn btn-primary btn-mini" onClick="GuardarCliente();"><span class="icon-book icon-white"></span> <span>Guardar</span> </button>
                            </td>
                        </tr>
                        <script type="text/javascript">
                            $('#prs_ced').bind('input',function(){ 
                                var value=$('#prs_ced').val();
                                if(value.length===10)
                                {
                            ...
                            function GuardarCliente()
                            {
                                if($('#prs_ced').val()==="")
                                {
                                    alert('Debe ingresar una C\u00E9dula o R.U.C.!!');
                                    $('#prs_ced').focus();
                                    return;
                                }
                                if($('#prs_nom').val()==="")
                                ...
                                if($('#prs_ape').val()==="")
                                {
                                    alert('Debe ingresar un apellido de cliente!!');
                                    $('#prs_ape').focus();
                                    return;
                                }
                                ...
                                if(respuesta==true){
                                    ...
                                    $.post("<?php echo htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'); ?>",data,function(response){
                                        if(response['Persona']===true)
                                        {
                                            ...
                                            $('#cliente').val(response['Prs_Ape']+' '+response['Prs_Nom']);
                                            $('#direccion').val(response['Prs_Dir']);
                                            closeModal();
                                        }
                                    },'json').fail(function(error){alert();});
                                }
...
/*Secci�n que permite buscar clientes por c�dula o por apellido mediante la presentaci�n de un DIALOG*/
                    ...
                    <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="form2" id="formSearch">
                        <table width="450" border="0">
                            <tr>
                                ...
                                <td width="266">
                                    <input id="op_opciones" name="op_opciones" type="radio" value="c" onClick="setfocus(this.form.buscta)">
...
/*Secci�n para efectuar la b�squeda dentro del Dialog en lo que respecta a b�squeda de clientes*/
if (isset($buscodcli))
{
    if ($op_opciones=='d')
    {
    ...
    <table width="100%" border="1" cellpadding="0" cellspacing="0">
        ...
        <tr <?php echo focus_row("resaltar_text", "resaltar_back", "undo_resaltar_text", "Fondo");?> class="Fondo">
            ...
            <td align="center">
                <button type="button" class="btn btn-success btn-mini" onClick="changeCliente('<?php echo $row['Cli_Cod'];?>','<?php echo $row['Prs_Ape'].' '. $row['Prs_Nom']; ?>','<?php echo $row['Prs_Ced']; ?>','<?php echo $row['Prs_Dir']; ?>')">
	...
	<tr><td colspan="4" align="center"><?Php echo error_alerta("No hay resultados que mostrar", 1); ?></td></tr>
	  <?php }?>
    </table>
...
<tr>
  ...
  <form action="javascript:preSave();" name="form22" id="form22" method="POST">
...
<table width="95%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        ...
        <td width="88%" class="LetraNegra">
            <input type="text" name="Prs_Ced" id="Prs_Ced" value="<?Php echo $row_rs_cliente['Prs_Ced'] ?>" size="11" maxlength="13" autofocus=""></input>
            ...
            <input name="PrsCedCli" id="PrsCedCli" type="hidden" value="<?Php echo $row_rs_cliente['Prs_Ced'];?>">
            <input name="PrsCorCli" id="PrsCorCli" type="hidden" value="<?Php echo $row_rs_cliente['Prs_Cor'] ?>">
            <input name="PrsNomCli" id="PrsNomCli" type="hidden" value="<?Php echo $row_rs_cliente['Prs_Ape'].' '.$row_rs_cliente['Prs_Nom'] ?>">
            <button type="button" name="button1" id="button1" class="btn btn-mini btn-primary fileinput-button" title="Buscar Cliente" onClick="document.getElementById('tipo_boton').value=1; ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_cliente_buscar=1&Vet_Cod=<?php echo $rs_cliente[0]['Vet_Cod']; ?>','ajax_modal')">
                <i class="icon-search icon-white"></i>                        
            </button>
            <button type="button" class="btn btn-primary btn-mini" onClick="registrarCliente();" title="Agregar Cliente"><span class="icon-plus icon-white"></span></button>
        </td>
    </tr>
    <tr>	
        <td width="12%" class="Etiqueta1">Cliente:</td>
        <td class="LetraNegra">
            <input type="text" name="cliente" id="cliente" value="<?Php echo $row_rs_cliente['Prs_Ape'].' '.$row_rs_cliente['Prs_Nom'] ?>" size="50" readonly=""/>
        </td>
    </tr>
    <tr>
        <td width="12%" class="Etiqueta1">Direcci&oacute;n:</td>
        <td colspan="3" class="LetraNegra">
            <input type="text" name="direccion" id="direccion" value="<?php echo $row_rs_cliente['Prs_Dir']?>" size="50" readonly=""/>
        </td>
	</tr>
	<?Php 
	...
	<?php } //Fin del if ($total_rs_cliente_esc > 0) ?>
</table>    
</FIELDSET>	
	<?Php
 ...
 <table width="95%" border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	?>
        ...
        <select name="Tic_Cod" id="Tic_Cod" style="width: 250px;" onChange="ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_mar_val=1&punto=<?php echo $row_rs_apercaja['Pun_Cod']; ?>&SucCod=<?php echo $Ses_Suc_Cod;?>&Tic_Cod='+ document.getElementById('Tic_Cod').value +'&fechaCja=<?php echo $row_rs_apercaja['Caj_Fec'] ; ?>','contenedormarca')">
            <option value="">Seleccione...</option>
        <?Php
  ...
  <tr>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="40%" class="LetraNegra"><?php echo $row_rs_autorizaci['Aut_Sri']?><input name="Aut_Sri" type="text" id="Aut_Sri" value="<?Php echo $row_rs_autorizaci['Aut_Sri']; ?>"></td>
        </tr>
      </table>
      </div>
     </td>
  ...
  <tr>
    ...
    <FIELDSET id="NotasCredito">
     ...
     <?php        
            $row_ivas = $obBD_con1->getArrayConsulta(1293, $row_rs_apercaja['Caj_Fec'], $obBD_conexion);             
        ?>
    <script>       
...
<label class="Titulos2">Detalle de la Factura</label>
  ...
  <thead>
	...
	<tr>
	  ...
	  <td>
	    <td align="right">&nbsp;</td>    
	    <td align="right">TARIFA <?php if(isset($row_ivas[0]['Iva_Por'])) echo $row_ivas[0]['Iva_Por'];?>%:</td>
	  <td>
	...
	<tr>
	  <td>&nbsp;</td>
		<td>
		  <td align="right">&nbsp;</td>
		  <td align="right"><?php if(isset($row_ivas[0]['Iva_Por'])) echo $row_ivas[0]['Iva_Por'];?>% I.V.A.:</td>
		<td>
		  <div align="center">
		    <label>
		      ...
		      <table width="100%" border="0" cellpadding="0" cellspacing="0" >
		        ...
		        <tr>
		          ...
		          <td class="Etiqueta1"><div align="left">
		            <input name="Num_Aut" id="Num_Aut" type="text"  size="40" maxlength="49"  align="right" />
		          </div></td>
		          </tr>
		        <tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	*/
	...
	?>
      <select name="For_Cod" id="For_Cod" onChange="if(this.value==='2') $('#Tbl_Cpp_Ven').show(); else $('#Tbl_Cpp_Ven').hide();   ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?cmb=1&For_Cod=' + this.value + '&div_banco=Ban_Cod&nom_pag=Pag_Cod&nom_ban=Ban_Cod&chk_ban=chk_bancos','Pag_Cod'); ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?cmb=1&For_Cod=' + this.value + '&div_banco=Ban_Cod2&nom_pag=Pag_Cod2&nom_ban=Ban_Cod2&chk_ban=chk_bancos2','Pag_Cod2');" onBlur="ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?cmb_tipo=1&Pag_Cod=' + document.getElementById('Pag_Cod').value + '&nom_ban=Ban_Cod', 'Ban_Cod'); ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?cmb_tipo=1&Pag_Cod=' + document.getElementById('Pag_Cod2').value + '&nom_ban=Ban_Cod2', 'Ban_Cod2')">
			<?Php 
    ...
    <tr>
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
		  ...
		  <select name="Pag_Cod" id="Pag_Cod" onChange="ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?cmb_tipo=1&Pag_Cod=' + this.value + '&nom_ban=Ban_Cod', 'Ban_Cod')">          
		  <?Php 
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td width="17%" class="LetraNegra"><div id="combo2">
              <select name="Pag_Cod2" id="Pag_Cod2" onChange="ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?cmb_tipo=1&Pag_Cod=' + this.value + '&nom_ban=Ban_Cod2', 'Ban_Cod2')">                 
                <?Php
				 foreach($rs_facttipo as $row_rs_facttipo)
				 { ?>
...
<FIELDSET>
		<LEGEND>
			<label class="Titulos2">PORCENTAJE DE RETENC�ON (338)</label>
		</LEGEND>
...
<table width="290" border="0" cellpadding="0" cellspacing="0" class="Azul">
    <tr>
        ...
        <td width="181">
            <button id="bt_guardar" type="button" class="btn btn-primary start" title="Guardar" onClick="validar_facturacion(this.form)">
            <i class="icon-book icon-white"></i>
            <span>Guardar</span>
        </button>
...
<?Php
 ...
 ?>
    <div id="bgtransparent1" class="bgtransparent" style="display:none" onClick="closeModal()"></div>
    <div id="bgmodal1"  class="bgmodal"  style="display:none">		
        <div id="ajax_modal"></div>
    </div>
 </form>
 </table>	   
...
<script>
    ...
    //Seccion realizada por Jose Ambuludi
    $(document).ready(function(){
        $('#Prs_Ced').keypress(function(){
            $('#cliente').val("");
            $('#direccion').val("");
        });
    }); 
    function preSave(){
        ...
        $.post("<?Php echo htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'); ?>",data,function(response){
            ...
            {
                alert('El n�mero de factura ya esta REGISTRADO..A continuaci�n el sistema proporcionar� uno NUEVO..!!');
                ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_mar_val=1&punto=<?php echo $row_rs_apercaja['Pun_Cod']; ?>&SucCod=<?php echo $Ses_Suc_Cod;?>&Tic_Cod='+ document.getElementById('Tic_Cod').value +'&fechaCja=<?php echo $row_rs_apercaja['Caj_Fec'] ; ?>','contenedormarca');
                $('#bt_guardar').attr('disabled',false);
            }else{
                $("#form22").attr('action',"<?Php $_SERVER['PHP_SELF']; ?>");
                $("#form22")[0].submit();
            }
        },'json').fail(function(error) {alert('El servidor fallo');$('#bt_guardar').attr('disabled',false);});
    }
    
    //Funci�n realizada, para buscar clientes por c�dula
...
<script>
    function changeCliente(Cli_Cod,Cliente,Ruc,Direcc)
    {
        $('#codigo').val(Cli_Cod);
        $('#Prs_Ced').val(Ruc);
        $('#cliente').val(Cliente);
    ...
    <?php if($oooooo){ ?>
    $(document).ready(function(){ $('#ocultar').hide(); $('#Prs_Ced').focus();});
    <?php } ?>
</script>
</BODY>
</HTML>
