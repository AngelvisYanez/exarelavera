...
if (isset($obtenerDerechosMineros)) {
    ...
    try {
        ...
        if ($result) {
            ...
            $resp['success'] = false;
            $resp['message'] = 'No se pudieron cargar los derechos mineros';
        }
    } catch (Exception $e) {
        $resp['success'] = false;
        $resp['message'] = 'Error: ' . $e->getMessage();
...
<BODY>
    <div class="panel panel-main">
        <div class="panel-heading exa-header">
            <h3 class="panel-title">&raquo; Consultar Guias de Remisión</h3>
        </div>
            ...
            <div id="documentoSearch">
                                    ...
                                    <div class="col-xs-3">
                                        <select name="Pec_Cod" class="form-control input-xs search_pec getData ins" onchange="if(this.value==='') $('#Cmb_Mes').attr('disabled','disabled'); else $('#Cmb_Mes').removeAttr('disabled'); ">
                                            <?php foreach ($rs_periodo as $row) {
                                                echo "<option value='$row[Pec_Cod]' data--Year='$row[Periodo]'>$row[Periodo]</option>";
                                            } ?>
                                            <option value="">
                                                << TODOS>>
                                            </option>
                ...
                <script>
                    function verDocument(doc) {
                        Aut_Tem = doc['Aut_Tem'];
                        window.current_der_min_id = doc['Der_Min_Id'];
                        $('#formDocumento').setData(doc);
                        $('#Der_Min_Id').val(doc['Der_Min_Id']);
    ...
    <div id="derechoMineroDialog" title="Registrar Nuevo Derecho Minero">
        <form class="form-horizontal normal" id="formDerechoMinero" action="javascript:guardarDerechoMinero();">
            <input type="hidden" name="Der_Min_Id" id="Der_Min_Id_Form">
            <fieldset class="exa-fieldset">
                <legend class="Titulos2">Información del Derecho Minero</legend>
                
                <div class="form-group">
                    <label class="col-xs-3 control-label label-xs required">Código:</label>
                    <div class="col-xs-4">
                        <input type="text" name="Der_Min_Codigo" id="Der_Min_Codigo" class="form-control input-xs" required placeholder="Ej: 506" maxlength="50">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-9">
                        <input type="text" name="Der_Min_Nombre" id="Der_Min_Nombre" class="form-control input-xs" required placeholder="Ej: SOROCHE UNIFICADO" maxlength="255">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-9">
                        <input type="text" name="Der_Min_Titular_Operador" id="Der_Min_Titular_Operador" class="form-control input-xs" required value="TITULAR" maxlength="255">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-9">
                        <select name="Der_Min_Tipo" id="Der_Min_Tipo" class="form-control input-xs" required>
                            ...
                            <option value="Concesión Minera">Concesión Minera</option>
                            <option value="Permiso de Exploración">Permiso de Exploración</option>
                            <option value="Licencia de Explotación">Licencia de Explotación</option>
                            <option value="Concesión de Beneficio">Concesión de Beneficio</option>
                            <option value="Permiso de Libre Aprovechamiento">Permiso de Libre Aprovechamiento</option>
                ...
                <div class="form-group">
                    <label class="col-xs-3 control-label label-xs">Recurso:</label>
                    <div class="col-xs-9">
                        <input type="text" name="Der_Min_Recurso" id="Der_Min_Recurso" class="form-control input-xs" placeholder="Ej: Oro, Cobre, etc." maxlength="100">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-9">
                        <input type="text" name="Der_Min_Ubicacion" id="Der_Min_Ubicacion" class="form-control input-xs" placeholder="Provincia / Cantón / Parroquia" maxlength="255">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-9">
                        <textarea name="Der_Min_Observaciones" id="Der_Min_Observaciones" class="form-control input-xs" rows="2"></textarea>
                    </div>
                </div>
            </fieldset>
            
            <div class="center" style="margin-top: 15px;">
                <button type="submit" class="btn btn-sm btn-success"><i class="glyphicon glyphicon-floppy-disk"></i> Guardar Derecho</button>
    ...
    <script type="text/javascript">
        ...
        function limpiarFormDerechoMinero() {
            ...
            $('#Der_Min_Id_Form').val('');
            $('#Der_Min_Titular_Operador').val('TITULAR');
        ...
        function guardarDerechoMinero() {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    alert('Error técnico al guardar el derecho minero');
                }
            });
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.derechos) {
                        ...
                        // Guardar valor actual si no se pasó uno específico
                        var currentVal = selectId || select.val();
                        
                        select.empty();
                        select.append('<option value="">Seleccione un derecho minero (opcional)...</option>');
                        
                        $.each(response.derechos, function(index, derecho) {
                            var selected = (derecho.Der_Min_Id == currentVal) ? 'selected' : '';
                            select.append(
                        ...
                        // Si se cargan por primera vez y tenemos un ID de la guía, seleccionarlo
                        if (!currentVal && window.current_der_min_id) {
                            currentVal = window.current_der_min_id;
                            select.val(currentVal);
                        }
                        
                        // Si tiene un valor seleccionado, disparar el cambio para actualizar observaciones
                        if (currentVal) {
                            select.val(currentVal);
                            // No siempre queremos sobreescribir las observaciones al cargar (si ya estaban editadas)
                            // Pero al CARGAR la guía por primera vez sí es bueno.
                        }
                    }
                }
            });
        ...
        // Actualizar automáticamente el campo observaciones
        function actualizarInfoDerecho(select) {
            if (!select.value) return;
            
            var opt = $(select).find('option:selected');
            var info = "DERECHO MINERO: " + opt.data('codigo') + " - " + opt.data('nombre') + " (" + opt.data('titular') + ") - TIPO: " + opt.data('tipo');
            if (opt.data('recurso')) {
                info += " - RECURSO: " + opt.data('recurso');
            }
            
            var obsField = $('textarea[name="Gui_Obs"]');
            var currentObs = obsField.val().trim();
            
            // Si ya tiene una línea de "DERECHO MINERO:", la reemplazamos
            if (currentObs.indexOf("DERECHO MINERO:") !== -1) {
                // Buscamos la línea que empieza con DERECHO MINERO y la reemplazamos
                var lines = currentObs.split('\n');
                var found = false;
                for(var i=0; i<lines.length; i++) {
                    if(lines[i].indexOf("DERECHO MINERO:") !== -1) {
                        lines[i] = info;
                        found = true;
                        break;
                    }
                ...
                // Si no tiene nada o tiene otra cosa, lo agregamos arriba
                obsField.val(info + (currentObs ? "\n" + currentObs : ""));
            }
        }
    </script>
</BODY>
