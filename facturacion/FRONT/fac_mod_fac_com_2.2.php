...
if(isset($changeCliente)){     
        ...
        $obBD_con1->grabarv_registros(sentencias_comf(1091,$obBD_con1->parametros($Vet_Cod.'*'.$Cli_Cod)), $obBD_conexion->conexion);
        if($llevarContabilidad['Cof_Con']=='S'){
            $rs_compr_auto=$obBD_con1->getRowConsulta(1111, $Vet_Cod, $obBD_conexion);
            $obBD_con1->operacionobBD(1112, $rs_compr_auto['Com_Cod'].'*'.$Cli_Cod, $obBD_conexion);
        }
...
if (isset($buscodcli))
{
		if ($op_opciones=='d')
		{
			...
			*/
			$rs_buspro = $obBD_con1->getArrayConsulta(701,trim($buscodcli).'*'.$Ses_Emp_Cod, $obBD_conexion);
		}
		elseif ($op_opciones=='c')
		{
			...
			*/
			$rs_buspro = $obBD_con1->getArrayConsulta(702,trim($buscodcli).'*'.$Ses_Emp_Cod, $obBD_conexion);
		}
                //cambio
...
<table width="100%" border="1" cellpadding="0" cellspacing="0">
      ...
      <tr <?php echo focus_row("resaltar_text", "resaltar_back", "undo_resaltar_text", "Fondo");?> class="Fondo">
        ...
        <td align="center">
            <button type="button" class="btn btn-success btn-mini" onClick="changeCliente(<?php echo $row['Prv_Cod']; ?>,<?php echo $Vet_Cod; ?>,'<?php echo $row['Prs_Ape'].' '. $row['Prs_Nom']; ?>','<?php echo $row['Prs_Ced']; ?>','<?php echo $row['Prs_Dir']; ?>')">
...
?>
      ...
      <tr>
		...
		<LEGEND>
                    ...
                    <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="form25" id="formSearch5">
		<table width="450" border="0">
			<tr>
			  <td width="205">
                              
                              <input id="op_opciones" name="op_opciones" type="radio" value="d" checked onClick="setfocus(this.form.buscta2)">
			  <span class="LetraNegra"><strong>Apellido</strong></span></td>
			  <td width="266"><input id="op_opciones" name="op_opciones" type="radio" value="c" onClick="setfocus(this.form.buscta2)">
		...
		<tbody id="tbusqueda">
		  <tr>
			...
			<td width="359" class="BarraBusqueda"><input name="buscta2" type="text" id="buscta2" size="50" maxlength="50" onKeyPress="if (trim(document.getElementById('buscta2').value) != ''&&event.keyCode == 13){var form = this.form;ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?buscodcli='+document.getElementById('buscta2').value+'&op_opciones='+form.elements['op_opciones'].value+'&Vet_Cod=<?php echo $Vet_Cod; ?>','busqueda2');}" /></td>
			<td width="110" align="center"><input type="text" style="display: none;" vlaue="" />
            <button type="button" class="btn btn-success" title="Buscar" onClick="var form = this.form;console.log(form);ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?buscodcli='+document.getElementById('buscta2').value+'&op_opciones='+form.elements['op_opciones'].value+'&Vet_Cod=<?php echo $Vet_Cod; ?>' ,'busqueda2')"> <i class="icon-search icon-white"></i> <span>Buscar</span> </button>
			</td>
		  </tr>
		</tbody>
		</table></form>   
  ...
  {   
	  ...
	  */
	  $obBD_ins1 =  new Class_Log_Datos_Comt;
          ...
          $row_cop_fec = $obBD_con1->getRowConsulta(1089, $Cop_Cod, $obBD_conexion);          
          $cop_fec_old=explode('-',$row_cop_fec['Cop_Fec']);
          $Cop_Sec=$row_cop_fec['Cop_Sec'];
          
		  /*Confic_fact de la empresa*/
		  $infoFactEmp=$obBD_ins1->getRowConsulta(1060,$Ses_Suc_Cod, $obBD_conexion);
	  ...
	  */
	  $obBD_ins1->inicio_transaccion($obBD_conexion->conexion);
          ...
          if($mese[1]!=$cop_fec_old[1]){
              $Cop_Sec= $obBD_ins1->codigoSecMensualAuto($Pec_Cod, $mese[1], $obBD_conexion); 
              $obBD_ins1->operacionobBD(1090,$Cop_Cod.'*'.$Cop_Sec, $obBD_conexion);
          }
	  /**
	  * Actualiza la cabecera de compras
	  */
	  $obBD_ins1->operacionobBD(719, $Tic_Cod.'*'.$codigo.'*'.$Ciu_Cod.'*'.trim($Cop_Num).'*'.$Cop_Aut.'*'.$Cop_Fec.'*'.$hoy.'*'.trim($Cop_Des).'*'.trim($Cop_Obs).'*'.$Cop_Cad.'*'.$Cop_Imp.'*'.$Tri_Cod.'*'.$hdd_TipoSri.'*'.$Cop_Ntd.'*'.$Cop_Nns.'*'.$Cop_Nna.'*'.$Cop_Cod, $obBD_conexion);
	...
	*/
	...
	{  
	  ...
	  {     //var_dump($param);
		...
		*/	
                 //var_dump($param[22]);
                if(isset($param[22])&&$param[22]==='S')
                    $obBD_ins1->operacionobBD(1110, $Cop_Cod.'*'.$param[0].'*'.$param[5].'*'.trim($param[1]).'*'.$param[2].'*'.$param[3].'*'.$param[4].'*'.$param[8].'*'.$param[6].'*'.$param[13].'*'.$param[12].'*'.$param[21].'*S', $obBD_conexion);
                else
				...
				if($llevarContabilidad['Cof_Con']=='S'){
					...
					*/
					$obBD_ins1->operacionobBD(256, $Com_Cod.'*'.(!$NCRED?'D':'H').'*'.$rubro_d.'*'.''.'*'.''.'*'.$param[12], $obBD_conexion);
                                        ...
                                        if(isset($param[22])&&$param[22]=='S'){
                                            $iva_al_costo=$iva_al_costo+round((($param[3] * $param[11])/100),2)*1;
		...
		if($llevarContabilidad['Cof_Con']=='S'){
			...
			*/
			for ($i=0;$i<=count($array_cnta_renta)-1;$i++)
			{
				...
				*/
				...
				$obBD_ins1->operacionobBD(256, $Com_Cod.'*'.(!$NCRED?'H':'D').'*'.$array_val_renta[$i].'*'.''.'*'.''.'*'.$array_cnta_renta[$i], $obBD_conexion);	 
			...
			*/
			...
			{			
				...
				* Cuentas del HABER del proveedor - Asignado automaticamente 
				*/
                             if($array_val_iva[$i]!=0) // Agregado x erik xq graba un Pld_Cod=0
				$obBD_ins1->operacionobBD(256, $Com_Cod.'*'.(!$NCRED?'H':'D').'*'.$array_val_iva[$i].'*'.''.'*'.''.'*'.$array_cnta_iva[$i], $obBD_conexion); 
			}//Fin del for ($i=0;$i<=count()-1;$i++)
                        /* en caso de retenciones asumidas */
                        if(!$NCRED && isset($Ret_Asu) && $Ret_Asu=='S' && $Ret_Pld_Cod!=='' && $Ret_Cod>0)
		...
		*/
		...
		{	
			...
			* Registra el Iva en caso que sea mayor a cero y el tipo de documento sea diferente de una nota de venta
			*/
			if (($t_iva-$iva_al_costo) > 0 and ($Tic_Cod != nota_venta and $Tic_Cod != tiquetes))
			{				
				/**
				* Cuentas del DEBE Iva Pagado - Asignado automaticamente 
				*/	
				$obBD_ins1->operacionobBD(256, $Com_Cod.'*'.(!$NCRED?'D':'H').'*'.($t_iva-$iva_al_costo).'*'.''.'*'.''.'*'.$iva_pagado, $obBD_conexion); 															
			}//Fin del if ($t_iva > 0)
			}// fin del if($llevarContabilidad['Cof_Con']=='S')
			...
			*/
			if ($For_Cod == 2 && $llevarContabilidad['Cof_Con']=='S')//Pago a C R E D I T O
			{
				...
				*/
				...
				$obBD_ins1->operacionobBD(256, $Com_Cod.'*'.(!$NCRED?'H':'D').'*'.$total_proveedores.'*'.''.'*'.''.'*'.$ccpp_prove,$obBD_conexion);	
        ...
        //var_dump($obBD_ins1);
        if($obBD_ins1->Error==0) $ban_aux=true;
		/**
		*  Actualizamos el xml de la Factura Electronica
		*/		
		if ($obBD_conexion->Error==0 && $infoFactEmp['Cof_Gce']=='S'){
...
?>
	...
	<table width="770" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td width="109" class="Etiqueta1"><span class="Asterisco">* </span>Tipo  documento:</td>
        <td colspan="4"><span class="LetraNegra">
                <select name="Tic_Cod" id="Tic_Cod2" style="width: 350px">            
      ...
      <tr>
        ...
        <td width="88"><input name="op_opciones" type="radio" value="ru" onClick="document.getElementById('cmb_mes').disabled=false; document.getElementById('Tic_Cod').disabled=false;  setfocus(form1.txt_busqueda)" style="cursor:pointer" >
            <span class="Etiqueta1">RUC</span></td>
        <td width="117"><input type="radio" name="op_opciones" value="r" onClick="document.getElementById('cmb_mes').disabled=true;  
	...
	{	
	  ...
	  { 	
		...
		*/ 
		...
		/*consultamos si la compra tiene retencion electronica y Autorizada */
		$rs_CopRetAutoriza=$obBD_con1->getRowConsulta(1109, $row_rs_buscar['Cop_Cod'], $obBD_conexion);
		...
		*/
		$row_rs_existe_pagos=$obBD_con1->getArrayConsulta(357, $row_rs_comprobante_compra['Com_Cod'], $obBD_conexion);
		 if(count($row_rs_existe_pagos)>0 || $rs_CopCajaChica['Cop_Cod']!='' || $rs_CopRetAutoriza['Ret_Aut']=='S')
		 {
			 $rojo = "#9CB8CF";
		 }
		?>
  ...
  <td width="5%" align="center">
	...
	//if(count($row_rs_existe_pagos)==0)			
	{
	    if($rs_CopCajaChica['Cop_Cod']==''){	
			  if($rs_CopRetAutoriza['Ret_Aut']!='S'){
				//if(1==1){
					...
					{ ?>
						...
						<input name="Tpc_Cod" id="Tpc_Cod" type="hidden" value="<?Php echo $row_rs_buscar['Tpc_Cod'];?>">    
						<input name="Cop_Ntd" id="Cop_Ntd" type="hidden" value="<?Php echo $row_rs_buscar['Cop_Ntd'];?>">
						<input name="Cop_Nns" id="Cop_Nns" type="hidden" value="<?Php echo $row_rs_buscar['Cop_Nns'];?>">
						<input name="Cop_Nna" id="Cop_Nna" type="hidden" value="<?Php echo $row_rs_buscar['Cop_Nna'];?>">    
						<input name="TicCod" id="TicCod" type="hidden" value="<?Php echo $row_rs_buscar['Tic_Cod'];?>">
	...
	?>
                  <img src="../../mascaras/model1/imagenes/finance_thumb_sm.png" /><!--Agregadp por erik para quitar boton-->
   <!-- <button type="button" class="btn btn-success btn-mini" title="Elegir" onclick="this.form.submit()">
      <i class=" icon-arrow-right icon-white"></i>
      </button>	-->
    <?php 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td class="LetraNegra">&nbsp;<span id="CliCli"><?Php echo $rs_proveed[0]['Prs_Nom']." ".$rs_proveed[0]['Prs_Ape']."&nbsp"; ?></span>
      <input type="hidden" id="Cha_Prv_Cod"  name="Prv_Cod" value="<?Php echo $Prv_Cod; ?>" ></td>
    </tr>
    <td class="Etiqueta1">Direcci&oacute;n: </td>
    <td class="LetraNegra">&nbsp;<span id="CliDir"><?php echo $rs_proveed[0]['Prs_Dir']?></span></td>
    </tr>
</table>
</FIELDSET>
        <?php  
        if(/*$row_rs_tip_compr['Tic_Cod']*1==4&&*/$rs_proveed[0]['Cop_Fec']>'2016-05-31')
            $row_ivas  = $obBD_con1->getArrayConsulta(1105, '', $obBD_conexion);
        else    
            $row_ivas = $obBD_con1->getArrayConsulta(1100, $rs_proveed[0]['Cop_Fec'].'*1', $obBD_conexion);
     ?>
     ...
     <script>
		...
		?>
            ...
            <td><input name="Cop_Fec" type="text" id="Cop_Fec" value="<?Php echo $rs_proveed[0]['Cop_Fec']; ?>" size="10"  onchange="checkFechaIva(this.value);">
...
<label class="Titulos2">Detalle de la Factura </label>
  ...
  <table  border="0" cellpadding="0" cellspacing="0">
    <tbody id="c_contenido">
		 <tr>
        <td width="34" height="30" class="Cabecera1">Cant.</td>
        ...
        <td width="190" class="Cabecera1">IVA</td>
        <td width="50" class="Cabecera1">ICE</td><td class="Cabecera1"></td>
        <td width="50" class="Cabecera1">Adq.</td>
        ...
        <td width="50" class="Cabecera1"></td>
        <td width="0"></td>
      </tr>
  ...
  value="<?Php echo $row_rs_proveed['Cop_Pru']; ?>" size="8" maxlength="8"  style="text-align:right" ></td>
	...
	onKeyUp="cal_importe_ice_com(document.getElementById('datos[<?Php echo $fila; ?>,1]'), document.getElementById('datos[<?Php echo $fila; ?>,3]'), document.getElementById('datos[<?Php echo $fila; ?>,4]')); valor_renta_compra();" value="<?Php echo $row_rs_proveed['Cop_Dec']?>" size="2" maxlength="2"  readonly=""<?Php if(isset($ocul)){ /* inicio if(isset($ocul)){ */ if ($row_rs_proveed['Cop_Des'] != 0) { echo "readonly='true'"; } } /* fin if(isset($ocul)){ */ ?>></td>
        <td align="right" class="LetraNegra">
            ...
            <input name="iva" id="iva" value="<?Php echo $row_rs_proveed['Iva_Por']; ?>" style="text-align:right;display: inline-block;" size="1" readonly="false" type="text">
            <?php if($llevarContabilidad['Cof_Con']=='S'){ ?>
            <input type="checkbox" style="display: inline-block;<?php if(!($row_rs_proveed['Adq_Cor']=='A'&&$row_rs_proveed['Iva_Por']*1>0)) echo 'visibility:hidden;';?>" <?php if($row_rs_proveed['Iva_Cos']=='S') echo 'checked="checked"'; ?> title='Iva al Costo' 
                   onchange="if($(this).attr('checked')) $('#datos\\[<?Php echo $fila; ?>\\,25\\]').val('S'); else $('#datos\\[<?Php echo $fila; ?>\\,25\\]').val('N');" />
            <?php } ?>
            
            		
			</td>
        <td width="109" class="LetraNegra">
...
value="<?Php echo $row_rs_cuenta['Pld_Cdc']; ?>" size="5" onKeyUp="cargar_cuenta('<?Php echo $_SERVER['PHP_SELF']; ?>?Pec_Cod=<?Php echo $row_rs_proveed['Pec_Cod']; ?>&ajax_cuenta=',document.getElementById('datos[<?Php echo $fila; ?>,13]'),document.getElementById('datos[<?Php echo $fila; ?>,14]'),document.getElementById('datos[<?Php echo $fila; ?>,15]'));  valor_renta_compra();"   >
        ...
        <td width="120" class="LetraNegra"><input name="datos[<?Php echo $fila; ?>,7]" type="hidden" id="datos[<?Php echo $fila; ?>,7]" 
...
<?Php 
	/* Consulta los cï¿½digos de retenciï¿½n  */
		...
		$base_renta=$base_renta+round($porcentaje_renta,2);	
...
onClick="busca_renta_quita_btn(form,this)">
		...
		$base_iva=$base_iva+round($cal_poriva,2);
...
<input id="Btn_IvaMas[<?Php echo $fila; ?>]" type="button" class="BotonEliminar" name="Btn_IvaMas[<?Php echo $fila; ?>]" value="+" onClick="busca_iva_btn(form,this)">
		<?Php // } /*  fin if(isset($ocul)){  */  ?>
</td>       <td width="120" class="LetraNegra">
		<?Php // if(isset($ocul)){ /* inicio if(isset($ocul)){  */ ?>
...
value="<?Php echo $row_rs_proveed['Cop_Int']?>" size="3" maxlength="10" ></td>
        <td width="131" class="LetraNegra"><input name="datos[<?Php echo $fila; ?>,23]" id="datos[<?Php echo $fila; ?>,23]" type="hidden" size="3" value="<?Php echo $row_rs_proveed['Pro_Cod']; ?>" >&nbsp;
           <?php  if($llevarContabilidad['Cof_Con']=='S'){ ?>
        <input name="datos[<?Php echo $fila; ?>,25]" id="datos[<?Php echo $fila; ?>,25]" type="hidden" size="3" value="<?Php echo $row_rs_proveed['Iva_Cos']; ?>" >
           <?php } ?> 
        </td>
      </tr>
       <?Php 
    ...
    <tr>
      <td class="LetraNegra">&nbsp;</td>
      <td colspan="2" class="Etiqueta1" align="right">TARIFA <span class='iva_por'>0</span>%:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
    ...
    <tr>
      <td class="LetraNegra">&nbsp;</td>
      <td colspan="2" class="Etiqueta1" align="right"><span class='iva_por'>0</span>% I.V.A.:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </td>
      ...
      <td colspan="8">
                   ...
                   <?php  if(/*$row_rs_tip_compr['Tic_Cod']*1==4&&*/$rs_proveed[0]['Cop_Fec']>'2016-05-31'){}else{ ?>                   
                    <script>$('#selectIvas').hide();</script>
                   <?php } ?>
                     <?php  if($llevarContabilidad['Cof_Con']=='S'){ ?>
                     <?php $row_rs_iva_asie = $obBD_con1->getRowConsulta(1087, $row_rs_comprobante_compra['Com_Cod'], $obBD_conexion);  ?>
                    ...
                    <select name="iva_pag_par">
                        <?php foreach($row_rs_iva_pag as $rowI){ ?>
                            <option value="<?php echo $rowI['Pld_Cod']; ?>" <?php if(isset($row_rs_iva_asie['Pld_Cod'])&&$row_rs_iva_asie['Pld_Cod']==$rowI['Pld_Cod']) echo 'selected';?>><?php echo $rowI['Pld_Des']; ?></option>
                        <?php } ?>
                    </select>
                     <?php } ?>
                </td>
...
<?Php 
	...
	*/
	...
	$row_rs_cuenta_pagar=$obBD_con1->registros();
        $FCod=1;
        if(isset($row_rs_cuenta_pagar['Cpp_Cod']))
            $FCod=2;                   
        //var_dump($FCod);
?>
<FIELDSET>
...
<table width="100%" border="0" cellpadding="2" cellspacing="2" >
  ...
  <tr>
    ...
    <td width="16%" valign="top" class="LetraNegra">
  	...
  	<select name="For_Cod_2" id="For_Cod_2"  onChange="compras_cheques();"  disabled >
      <?Php do { ?>
            <?php //var_dump($Fcod,$row_rs_pago['For_Cod']); ?>
      <option value="<?Php echo $row_rs_pago['For_Cod'];  ?>" 
	  <?Php if($FCod==$row_rs_pago['For_Cod']*1){ ?> selected="selected" <?Php } ?>><?Php echo $row_rs_pago['For_Des'];   ?></option>
...
<?Php /* Fin si se encuentra en la tabla cuentas por pagar Proveedores Locales */ 
  ...
  <tr>
	...
	<LEGEND>
	<label class="Titulos2">Datos de la Notas de Credito</label>
	</LEGEND>	
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
...
<?Php 	
		...
		*/
		...
		/*
		* Control para generar numero retencion cuando la compra aun no tiene asignado retencion  
		*/		
		if ($num_row_rs_retencion_modificar!=0)
		{	
		    
			/*
			* Consultamos la autorizacion segun codigo retencion
			*/		
			$row_autorizacion_sri=$obBD_con1->getRowConsulta(1058,$row_rs_retencion_modificar['Ret_Cod'],$obBD_conexion);  
			$num_autorizacion_sri=$row_autorizacion_sri['Aut_Cod'] > 0? 1 : 0;			
		}else{
		...
		if($num_autorizacion_sri==0)
		{ $rojo='#FF0000';}else{$rojo='#000000';}
		?>
<?Php //if($num_row_rs_retencion_modificar>0){ /* inicio if($num_row_rs_retencion_modificar>0){  */  ?>
	<input type="hidden" name="Hdd_AutCodigo" id="Hdd_AutCodigo" value="<?Php echo $row_autorizacion_sri['Aut_Cod']; ?>" >
...
<tr>
        ...
        <td class="LetraNegra">&nbsp;<font color="<? echo $rojo;?>"><?Php if($num_autorizacion_sri!=0){echo $row_autorizacion_sri['Aut_Sri'];}else{echo "ï¿½No se ha detectado Autorizaci&oacute;n para generar Retenciones!";} ?></font></td>
</tr>
</table>
      ...
      <tr>
        <td width="27%" class="Etiqueta1"><input name="Hdd_Ret" id="Hdd_Ret" type="hidden" value="N">
          ...
          <td align="left" class="Etiqueta1">
              <div align="left"><input id="Ret_Fec" name="Ret_Fec" type="text" size="15" value="<?php if($num_row_rs_retencion_modificar!=0) echo $row_rs_retencion_modificar['Ret_Fec']; ?>" /></div>
          </td>    
      </tr>
    </table> 
...
<?Php  		/* valor a pagar */
			$valor_retenciones=round($bases_ivas,3);//Agregado round 2010-05-05 antes $valor_retenciones=round($base_renta,3)+round($base_iva,3);
                        ...
                        $Asumida=$obBD_con1->getRowConsulta(1103,  $row_rs_retencion_modificar['Ret_Cod'],$obBD_conexion);
                        if($Asumida['Ret_Cod'] > 0 && $Asumida['Ret_Asu']=='S')
                            $valor_cheque=round($resultados[5],2);
...
<button name="btn_guardar" type="button" class="btn btn-primary start" id= "btn_guardar" 
 ...
 <br />	
<?php }else{//Fin del if ($codigo > 0 && !(isset($hdd_save))) ?>	
 </form>
     <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
  ...
  */
  if($llevarContabilidad['Cof_Con']=='S'){
       //$Com_Com_Cod = $row_rs_comprobante_forma['Com_Cod'];
  	if($For_Cod==1){ ?>
...
<script type="text/javascript" src="../../Librerias/textbox/main.js"></script>  	
  <br />  
<div id="bgtransparent2" class="bgtransparent" style="display:none" onClick="closeModal();"></div>
    <div id="bgmodal2"  class="bgmodal" style="display:none" >
       <div id="ajax_modal2">
        	 <div id="mostrar"></div>
       </div>
</div>
...
<script>$('#Cop_Fec').change(function (){$('#Com_Fec').val($('#Cop_Fec').val());$('#Ret_Fec').val($('#Cop_Fec').val());});</script>
 <script>
                $( document ).ready(function() {
                    ...
                    <?php if($rs_proveed[0]['Tic_Cod']*1!==1&&$rs_proveed[0]['Tic_Cod']*1!==3){ ?>
                    $("#Reten_Asum").hide();
                    <?php } ?>
                });                
</script>
