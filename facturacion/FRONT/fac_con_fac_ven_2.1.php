...
<?php
/*
* Descripción: Consulta las ventas individual, total y detallada, de cada vendedor y punto de impresi�n
...
switch ($op) {
		...
		*/
		if (isset($txt_busqueda) && $txt_busqueda != "") {
			...
			*/
				...
				$rs_buscar = $obBD_con1->consulta(sentencias_tes(1231, $obBD_con1->parametros(trim($txt_busqueda) . '*' . $Tic_Cod . '*' . $Ses_Suc_Cod . '*' . $cmb_anio . '*' . $cmb_mes)), $obBD_conexion->conexion);
			...
			*/
				...
				$rs_buscar = $obBD_con1->consulta(sentencias_tes(1233, $obBD_con1->parametros(trim($txt_busqueda) . '*' . $Tic_Cod . '*' . $Ses_Emp_Cod)), $obBD_conexion->conexion);
			} else {
				/*Buscar por cedula*/
				$rs_buscar = $obBD_con1->consulta(sentencias_tes(1277, $obBD_con1->parametros(trim($txt_busqueda) . '*' . $Tic_Cod . '*' . $Ses_Suc_Cod . '*' . $cmb_anio . '*' . $cmb_mes)), $obBD_conexion->conexion);
			}
			...
			*/
				...
				$rs_vetaTicSri = $obBD_con1->getRowConsulta(1290, $codigo, $obBD_conexion);
				$tipDocSri = $rs_vetaTicSri['Tic_Sri'];

				/*Factura de Ventas*/
				if ($tipDocSri == "1" || $tipDocSri == "01" || $tipDocSri == "41") {
					$hdd_documento = $reportes[1];
				}
				/*para imprimir un comprobante nota de credito*/
				if ($tipDocSri == '4' || $tipDocSri == '04') {
					$hdd_documento = $reportes[2];
				}
	...
	case 2:
		...
		*/
		//$puntos = "AND caja_aper.Pun_Cod = ".$Pun_Cod; // Trabaja con la sql 210

		$puntos = "AND puntos_imp.Suc_Cod = " . $Ses_Suc_Cod; // Trabaja con la sql 210
		$parametro = " Caj_Fec BETWEEN '" . (isset($txt_fec_ini) ? $txt_fec_ini : '') . "' AND '" . (isset($txt_fec_fin) ? $txt_fec_fin : '') . "' ";
		$parametro =  $parametro . (isset($Tic_Gen) ? ($Tic_Gen == 'E' ? " AND (Vet_Xml!='' AND Vet_Xml IS NOT NULL) " : ($Tic_Gen == 'N' ? " AND (Vet_Xml='' OR Vet_Xml IS NULL) " : '')) : '');
		if (isset($Tci) && $Tci != "") {
			...
			if (isset($rubros)) {
				...
				if (isset($rubros_cli)) {
					...
					*/
					...
					$rs_buscarcarrera = $obBD_con1->consulta(sentencias_tes(1238, $obBD_con1->parametros($parametro . '*' . (isset($optest) ? $optest : '') . '*' . $Tic_Cod . '*' . $puntos)), $obBD_conexion->conexion);
					$row_rs_buscarcarrera = $obBD_con1->registros();
					$total_rs_buscarcarrera = $obBD_con1->numregistros();
				}
	...
	case 4:
		...
		*/
		...
		if (!(isset($escu))) {
			...
			*/
			if (isset($rubros)) {
				$rs_buscarcarrera = $obBD_con1->consulta(
					sentencias_tes(210, $obBD_con1->parametros($txt_fec_ini . '*' . $txt_fec_fin . '*' . $optest . '*' . $Tic_Cod . '*' . $puntos)),
				...
				*/
				$rs_buscarcarrera = $obBD_con1->consulta(
					sentencias_tes(106, $obBD_con1->parametros($txt_fec_ini . '*' . $txt_fec_fin . '*' . $optest . '*' . $Tic_Cod . '*' . $puntos)),
					$obBD_conexion->conexion
				);
				$row_rs_buscarcarrera = $obBD_con1->registros();
				$total_rs_buscarcarrera = $obBD_con1->numregistros();
...
<BODY>
	...
	<div id="set1">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
						...
						?>
								<form name="form1" id="form1" action="<?Php echo $_SERVER['PHP_SELF']; ?>" method="post">
										...
										<table width="703" border="0" cellpadding="0" cellspacing="0">
											<tr>
												...
												<td width="587">
													...
													?>
													<select name="Tic_Cod" id="Tic_Cod" style="width: 200px">
														...
														?>
															<option <?Php if ($Tic_Cod == $row['Tic_Cod']) {
																		echo "selected";
																	} ?> value="<?Php echo $row['Tic_Cod']; ?>"><?Php echo $row['Tic_Des']; ?></option>
														<?Php
														}
														?>
													</select>
									...
									<FIELDSET>
                                        ...
                                        document.getElementById('cmb_anio').disabled=false; setfocus(this.form.txt_busqueda)" <?Php if ((isset($op_opciones) && $op_opciones == "d") or !isset($op_opciones)) {
																																	echo "checked";
																																} ?>>
													<span class="Etiqueta1">Apellidos</span>
												</td>
												<td width="129"><input type="radio" name="op_opciones" value="c" onClick="document.getElementById('cmb_mes').disabled=false;
                                        document.getElementById('cmb_anio').disabled=false; setfocus(this.form.txt_busqueda)" <?Php if (isset($op_opciones) && $op_opciones == "c") {
																																	echo "checked";
																																} ?>>
													<span class="Etiqueta1">C&eacute;dula/R.U.C. </span>
												</td>
												<td width="145"><input type="radio" name="op_opciones" value="r" onClick="document.getElementById('cmb_mes').disabled=true;
                                        document.getElementById('cmb_anio').disabled=true; setfocus(this.form.txt_busqueda)" <?Php if (isset($op_opciones) && $op_opciones == "r") {
																																	echo "checked";
																																} ?>>
													<span class="Etiqueta1">No. Documento </span>
												</td>
												<td width="134"><input type="radio" name="op_opciones" value="p" onclick="document.getElementById('cmb_mes').disabled=true;
                                        document.getElementById('cmb_anio').disabled=true; setfocus(this.form.txt_busqueda)" <?Php if (isset($op_opciones) && $op_opciones == "p") {
																																	echo "checked";
																																} ?> />
													<span class="Etiqueta1">Dep&oacute;sito </span>
												</td>
												<td width="444" class="LetraNegra"><?Php include("../COMPONENTES/tes_com_ann_mes_ven_1.0.php"); ?></td>
											</tr>
										</table>
										<table width="566" height="36" border="0" cellpadding="0" cellspacing="0">
											<tr>
												<td height="28" class="BarraBusqueda">
													<div align="left"><span class="Asterisco">* </span>Busqueda:
											...
											<tbody>
												...
												?>
														<tr>
															...
															<td align="center">
																<?php if ($row_rs_buscar['Vet_Aut'] == 'S') {
																	 ...
																	 * Consulta los tipos de comprobantes
																	 */
																	$row_tipoCompr = $obBD_con1->getRowConsulta(1253, $row_rs_buscar['Tic_Cod'], $obBD_conexion);
																	if ($row_tipoCompr['Tic_Sri'] == '01') {
																		$urlPdf = "../COMPONENTES/tesPdfFacturaElectronica_2.0.php";
																	}
																	if ($row_tipoCompr['Tic_Sri'] == '04') {
																...
																?>
																	<form name="frm_pdf" id="frm_pdf" action="<?php echo $urlPdf; ?>" method="post" target="_blank">
																		...
																		<!--  OBTENER EL LOGO DE LA SUCURSAL DONDE SE INICIA SESSION  -->
																		<?php
																		$data_suc = $obBD_con1->consulta(sentencias_tes(1323, $obBD_con1->parametros($Ses_Suc_Cod)), $obBD_conexion->conexion);
																		$row_rs_suc = $obBD_con1->registros();
																		//Obtener el logo de las sucursales
																		$logoUrl = empty($row_rs_suc['logo_sucursal']) ? $Ses_Emp_Log : $row_rs_suc['logo_sucursal'];
														...
														<tr id="detalle[<?Php echo $i; ?>]">
															...
															?>
															<td colspan="4"><?Php if (file_exists("../COMPONENTES/tes_com_detalle_ven.php")) include("../COMPONENTES/tes_com_detalle_ven.php");  ?></td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
														</tr>
							...
							case 2: ?>
									...
									<FIELDSET>
										...
										<table width="99%" border="0" cellpadding="0" cellspacing="0">
												...
												<td width="48%" valign="top">
													<FIELDSET>
														...
														<table width="100%" border="0" cellpadding="0" cellspacing="0">
															<tr>
																<td width="26%"><label class="Etiqueta1">
																		<input name="rubros" type="checkbox" id="rubros" value="checkbox" onclick="document.getElementById('rubros_cli').checked=false">
																		Agrupar Productos</label></td>
																<td width="42%"><label class="Etiqueta1">
																		<input name="rubros_cli" type="checkbox" id="rubros_cli" value="checkbox" onclick="document.getElementById('rubros').checked=false" />
																		Agrupar Clientes</label></td>
																<td width="32%"><span class="Etiqueta1">
											...
											<tr>
												...
												<td>
													<FIELDSET>
														<legend><label class="Titulos2">Tipo de Emision:</label></legend>
														<table width="565" border="0" cellpadding="0" cellspacing="0">
															<tr>
																<td width="120" class="Etiqueta1">Tipo documento:&nbsp;</td>
																<td width="445">
																	<select name="Tic_Gen" id="Tic_Gen" style="width: 200px">
																		<option value="">TODAS</option>
																		<option <?Php if (isset($Tic_Gen) && $Tic_Gen == 'E') {
																					echo "selected";
																				} ?> value="E">Electronicas</option>
																		<option <?Php if (isset($Tic_Gen) && $Tic_Gen == 'N') {
																					echo "selected";
																				} ?> value="N">Fisicas</option>
																	</select>
																</td>
															</tr>
														</table>
											...
											<tr>
												<td colspan="2">
													<FIELDSET>
														<legend>
															<label class="Titulos2">Buscar por Cliente:</label>
														</legend>
														<font class="Etiqueta1">C.I./RUC:</font> <input name="Tci" id="Tci" type="text" value="">
													</FIELDSET>
													<?php include("../../componentes/FRONT/com_con_fecha.php"); ?>
												</td>
											</tr>
										</table>
									</FIELDSET>
								...
								if (isset($hdd)) {
									...
									*/
									$rs_maximo_fac = $obBD_con1->consulta(
										sentencias_tes(1247, $obBD_con1->parametros($parametro . '*' . $optest . '*' . $Tic_Cod . '*' . $puntos)),
										$obBD_conexion->conexion
									);
									...
									*/
									$rs_minimo_fac = $obBD_con1->consulta(
										sentencias_tes(1248, $obBD_con1->parametros($parametro . '*' . $optest . '*' . $Tic_Cod . '*' . $puntos)),
										$obBD_conexion->conexion
									);
			...
			*/
												$rs_etapa = $obBD_con1->consulta(
													sentencias_tes(176, $obBD_con1->parametros($Eta_Cod)),
													$obBD_conexion->conexion
												);
			...
			*/
												$rs_modalidad = $obBD_con1->consulta(
													sentencias_tes(172, $obBD_con1->parametros($Mod_Cod)),
													$obBD_conexion->conexion
												);
			   ...
			   Consulta de las facturas totales agrupados por rubros en base a la carrera de todos los puntos de impresion */
														$rs_buscarcarrera = $obBD_con1->consulta(
															sentencias_tes(211, $obBD_con1->parametros($txt_fec_ini . '*' . $txt_fec_fin . '*' . $optest . '*' . $Tic_Cod . '*' . $carrera_cod[$x] . '*' . $puntos)),
															$obBD_conexion->conexion
														);
														$row_rs_buscarcarrera = $obBD_con1->registros();
														$total_rs_buscarcarrera = $obBD_con1->numregistros();
			   ...
			   * Consulta de las facturas totales en base a la carrera y el periodo actual*/
														$rs_buscarcarrera = $obBD_con1->consulta(
															sentencias_tes(110, $obBD_con1->parametros($txt_fec_ini . '*' . $txt_fec_fin . '*' . $carrera_cod[$x] . '*' . $optest . '*' . $Tic_Cod . '*' . $puntos)),
															$obBD_conexion->conexion
														);
														$row_rs_buscarcarrera = $obBD_con1->registros();
														$total_rs_buscarcarrera = $obBD_con1->numregistros();
		  ...
		  */
												...
												if ($total_rs_buscarcarrera != 0) {
													...
													?>
															...
															<tbody>
																...
																?>
																	<tr>
																		...
																		<td align="right"><?Php echo $row_rs_buscarcarrera['Vet_Can']; ?></td>
														...
														<table width="100%" border="0" cellpadding="0" cellspacing="0">
															...
															<tr class="LetraNegra">
																<td>&nbsp;</td>
																<td class="Etiqueta1">Tarifa diferente a 0: </td>
														...
														?>
															<table width="100%" border="1" cellpadding="0" cellspacing="0" class="fixedHeader01">
																...
																<tbody>
																	...
																	?>
																		<tr>
																			<td align="center"><?Php echo $row_rs_buscarcarrera['Cli_Cod']; ?></td>
																	...
																	<?Php
																		$subVetas = (isset($subVetas) ? $subVetas : 0) + $row_rs_buscarcarrera['Vet_Imp'];
																	} while ($row_rs_buscarcarrera = $obBD_con1->fetch_assoc($rs_buscarcarrera)); ?>
																</tbody>
															</table>
															<table width="100%" border="0" cellpadding="0" cellspacing="0">
															...
															<table width="100%" border="1" cellpadding="0" cellspacing="0" class="fixedHeader02" style="table-layout:fixed;">
																<thead>
																	<tr>
																		...
																		<th width="5%">No. Docto.</th>
																		<th width="5%">Fecha</th>
																		<th width="5%">C&eacute;dula/R.U.C.</th>
																		<th width="10%">Cliente</th>
																		<th width="4%">R.Num</th>
																		<th width="4%">R.Cod</th>
																		<th width="4%">R.Fec</th>
																		<th width="4%">R.Aut</th>
																		<th width="6%">Ret. Renta</th>
																		<th width="6%">Ret. Iva</th>
																		<th width="5%">Valor</th>
																		<th width="5%">Descto.</th>
					...
					*/
						...
						*/
																		...
																		if (strlen($row_rs_buscarcarrera['Prs_Ape'] . ' ' . $row_rs_buscarcarrera['Prs_Nom']) > 24) {
																			$CliCad = $row_rs_buscarcarrera['Prs_Ape'] . ' ' . $row_rs_buscarcarrera['Prs_Nom'];
																			$Cli_Nombre = substr($CliCad, 0, 24) . '...';
																		} else {
																			$CliCad = $row_rs_buscarcarrera['Prs_Ape'] . ' ' . $row_rs_buscarcarrera['Prs_Nom'];
																			$Cli_Nombre = $row_rs_buscarcarrera['Prs_Ape'] . ' ' . $row_rs_buscarcarrera['Prs_Nom'];
																		}
																	?>
																		<tr>
																			...
																			<!-- <td align="right"><?php echo formato_numero($row_rs_buscarcarrera['Vet_Pag'] + $row_rs_buscarcarrera['Iva'], 2, 3); ?></td> --> <!-- posible error -->
																			<td align="right"><?php echo formato_numero($row_rs_buscarcarrera['Vet_Pag'] , 2, 3); ?></td>
																		</tr>
																	<?Php
																		/**
																		 * Calculo de totales
																		...
																		// var_dump("linea1004: ",$row_retencion['r_iva']);
																	} while ($row_rs_buscarcarrera = $obBD_con1->fetch_assoc($rs_buscarcarrera)); ?>
																</tbody>
																<tfoot>
																	<tr>
																		...
																		<!-- <td align="right"><b><?Php echo formato_numero($total_tot + $total_iva, 2, 3); ?></b></td> <!--posible error -->
																		<td align="right"><b><?Php echo formato_numero($total_tot, 2, 3); ?></b></td>
																	</tr>
																</tfoot>
															</table>
															<?Php
															...
															<table width="100%" border="0" cellpadding="0" cellspacing="0">
																<tr class="LetraNegra">
																	<td width="73%">
																	<td width="19%" class="Etiqueta1">Subtotal:
																	<td width="5%" align="right"><?php echo formato_numero($resultados_total[0], 2, 3); ?></td>
																...
																<tr class="LetraNegra">
																	<td>&nbsp;</td>
																	<td class="Etiqueta1">Tarifa diferente a cero: </td>
										...
										<table border="0" cellpadding="0" cellspacing="0">
											<tr>
												<td width="106" align="left" class="LetraNegra">
													<form action="fac_pri_fac_total_2.1.php" method="post" name="form4" target="_blank">
														...
														<input name="Tci" id="Tci" type="hidden" value="<?php echo $Tci; ?>">
														...
														<input name="rubros" type="hidden" id="rubros" value="<?Php echo isset($rubros) ? $rubros : ''; ?>">
														<input name="rubros_cli" type="hidden" id="rubros_cli" value="<?Php echo isset($rubros_cli) ? $rubros_cli : ''; ?>">
														<input name="escu" type="hidden" id="escu" value="<?Php echo isset($escu) ? $escu : ''; ?>">
														...
														<input name="Eta_Des" type="hidden" id="Eta_Des" value="<?Php echo isset($row_rs_etapa['Eta_Des']) ? $row_rs_etapa['Eta_Des'] : ''; ?>">
														<input name="Mod_Des" type="hidden" id="Mod_Des" value="<?Php echo isset($row_rs_modalidad['Mod_Des']) ? $row_rs_modalidad['Mod_Des'] : ''; ?>">
														<?Php
...
*/
									if (isset($escu)) {
										/*
	* Consultas la inconsistencia de las facturas que no tienen relaci�n con matriculas
	*/
														...
														?>
															<tr>
																<form name="form1" method="post" action="<?Php echo $_SERVER['PHP_SELF']; ?>">
																	...
																	<td align="center">
																		<input name="imagen" type="image" src="../../mascaras/model1/imagenes/32x32/vista.jpg" title="Ver" width="22" height="22">
																	</td>
																	<input name="codigo" type="hidden" value="<?Php echo $row_rs_buscar['Vet_Cod']; ?>">
																</form>
															</tr>
	...
	*/
				...
				*/
					...
					*/
														?>
															<tr>
																<td width="5%" align="center" valign="top"><?php echo $row_rs_buscarcarrera['Vet_Cod']; ?></td>
																<td width="8%" align="center" valign="top"><?php echo $row_rs_buscarcarrera['Vet_Num']; ?></td>
							...
							*/
																		$rs_interes = $obBD_con1->consulta(
																			sentencias_tes(74, $obBD_con1->parametros(
																		...
																		echo formato_numero($row_rs_detalle['Vet_Can'], 0, 2);
							...
							*/
																		...
																		$row_rs_interes = $obBD_con1->registros();
																		$total_rs_interes = $obBD_con1->numregistros();
																		echo "&#8226; " . ((trim($row_rs_detalle['Ite_Cor']) != "") ? $row_rs_detalle['Ite_Cor'] : $row_rs_detalle['Ite_Lar']) . " " .
																			"[" . "Cant. " . formato_numero($row_rs_detalle['Vet_Can'], 0, 2) . " / Precio U. " . formato_numero($row_rs_detalle['Vet_Pru'], 2, 2) . "]<br>";
																		if ($total_rs_interes > 0) {
																			do { //Inicio del }while($row_rs_interes = mysqli_fetch_assoc($rs_interes));
																				echo "&#8226; " . $row_rs_interes['Ite_Cor'] .
																					"[" . formato_numero($row_rs_interes['Vet_Imp'], 2, 2) . "]<br>";
																...
																<td width="15%" align="right" valign="top"><?php echo formato_numero($row_rs_buscarcarrera['Vet_Imp'], 2, 2); ?></td>
															<?php
														} while ($row_rs_buscarcarrera = $obBD_con1->fetch_assoc($rs_buscarcarrera));
															?>
															</tr>
												...
												<table width="100%" border="0" cellpadding="0" cellspacing="0">
													<tr class="LetraNegra">
														...
														<td width="12%">
															<div align="right"><?php echo formato_numero($resultados_total[0], 2, 2); ?></div>
														</td>
													</tr>
													<tr class="LetraNegra">
														<td>&nbsp;</td>
														<td class="Etiqueta1">Tarifa 0%:</td>
														<td>
															<div align="right"><?Php echo formato_numero($resultados_total[1], 2, 2); ?></div>
														</td>
													</tr>
													<tr class="LetraNegra">
														<td>&nbsp;</td>
														...
														<td>
															<div align="right"><?Php echo formato_numero($resultados_total[2], 2, 2); ?></div>
														</td>
													</tr>
													<tr class="LetraNegra">
														<td>&nbsp;</td>
														...
														<td>
															<div align="right"><?Php echo formato_numero($resultados_total[3], 2, 2); ?></div>
														</td>
													</tr>
													<tr class="LetraNegra">
														<td>&nbsp;</td>
														<td class="Etiqueta1">Descuento:</td>
														<td>
															<div align="right"><?Php echo formato_numero($resultados_total[4], 2, 2); ?></div>
														</td>
													</tr>
													<tr class="LetraNegra">
														<td>&nbsp;</td>
														<td class="Etiqueta1">Total:</td>
														<td>
															<div align="right"><?php echo formato_numero($resultados_total[5], 2, 2); ?></div>
														</td>
													</tr>
												</table>
										</div>
														...
														<tr>
															...
															<td width="88%">
																<select name="Pun_Cod4" id="Pun_Cod4">
																	<?PHP
																	do { ?>
																		<option <?Php if ($row_rs_puntos['Pun_Cod'] == $Pun_Cod4) {
																					echo "selected";
																				} ?> value="<?Php echo $row_rs_puntos['Pun_Cod']; ?>"><?PHP echo $row_rs_puntos['Pun_Des']; ?></option>
	...
	*/
											$rs_maximo_fac = $obBD_con1->consulta(
												sentencias_tes(96, $obBD_con1->parametros($txt_fec_ini . '*' . $txt_fec_fin . '*' . $optest . '*' . $Tic_Cod . '*' . $puntos)),
												$obBD_conexion->conexion
											);
											$row_rs_maximo_fac = $obBD_con1->registros();
	...
	*/
											$rs_minimo_fac = $obBD_con1->consulta(
												sentencias_tes(97, $obBD_con1->parametros($txt_fec_ini . '*' . $txt_fec_fin . '*' . $optest . '*' . $Tic_Cod . '*' . $puntos)),
												$obBD_conexion->conexion
											);
											$row_rs_minimo_fac = $obBD_con1->registros();
...
*/
	...
	*/
					...
					<?Php
					if (isset($codigo) && $codigo > 0 && !(isset($txt_busqueda))) { ?>
						<br>
								...
								<tr>
									<td width="106">
										<form action="<?Php echo $_SERVER['PHP_SELF']; ?>" method="post" name="form2">
											<button type="button" class="btn btn-inverse fileinput-button" title="Atr&aacute;s" onclick="campos_hide(this.form, '<?Php echo "txt_busqueda*op_opciones*hdd_volver*cmb_anio*cmb_mes*Tic_Cod*op"; ?>', '<?Php echo $volver_busqueda . '*' . $volver_op . '*1*' . $volver_anio . '*' . $volver_mes . '*' . $volver_Tic_Cod . '*' . $op; ?>')"> <i class=" icon-arrow-left icon-white"></i> <span>&nbsp;&nbsp;Atr&aacute;s&nbsp;&nbsp;</span> </button>
										</form>
									</td>
									<td width="197">
										<?Php if ($Vet_Est == 'A') { ?>
											<form action="<?Php echo $hdd_documento; ?>" method="post" name="form2" target="_blank">
										...
										<?Php
											...
											echo error_alerta(" Documento ANULADO", 2);
										} ?>
									</td>
								</tr>
							</table>
	...
	*/
	if (isset($total_rs_buscar) && $total_rs_buscar != 0) {
