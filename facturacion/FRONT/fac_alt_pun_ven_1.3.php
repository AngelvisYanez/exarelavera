...
if(isset($saveDocument)){
    ...
    try{
        ...
        $Vet_Cod = $obBD_ins1->insercionid($obBD_conexionIns->conexion);
        /* REGISTRO PAGO VENTA */
        ...
        if($configs['Cof_Con']=='S'&&$Tic_Sri*1!=0){
            ...
            foreach ($pagos as $pag){
                ...
                if($pag['For_Cod']*1==2){
                    ...
                    $Cpc_Cod = $obBD_ins1->insercionid ($obBD_conexionIns->conexion);
                }
            }
      ...
      $kardex=array('IoE'=>'E', 'Kar_Fec'=>$Vet_Fec, 'Kar_Hor'=>date("H:i:s"), 'Vet_Cod'=>$Vet_Cod, 'Vnd_Cod'=>$Vnd_Cod);
    ...
    if($obBD_ins1->Error==0){ 
        ...
        if(!empty($Vet_Cod)){  
                 ...
                 $responce['Vet_Link_Cambio']="".(!empty($reportes[1])?"$reportes[1]?Vet_Cod":baseUrl("../../facturacion/FRONT/fac_pri_fac_ven.php?Vet_Cod"))."=$Vet_Cod&Cambio_Moneda=$Cambio_Moneda";

                 $responce['Vet_Link']="".(!empty($reportes[1])?"$reportes[1]?Vet_Cod":baseUrl("../../facturacion/FRONT/fac_pri_fac_ven.php?Vet_Cod"))."=$Vet_Cod";
...
<HTML>
    <HEAD>		
        ...
        <script> var cons_final=<?php if(!empty($cons_final)) echo json_encode($cons_final); else echo 'null'; ?>; </script>
    </HEAD>
<BODY>    
    <?php if(empty($error)){ ?>
                ...
                <div class="row">                   
                    <div class="col-xs-3">
                        <div id="accordion">
                            ...
                            <h3>Cambio Moneda</h3>
                            <div id="mon_form" class="form-horizontal normal">
                                <div class="form-group">
                                    <div class="col-xs-12">
                                    <div class="checkbox check-big input-xs"><label><input onchange="setCambio();" id="es_cambio" type="checkbox" value="S" offval="N" />Activar Cambio Moneda</label></div>
                                    </div>
                                </div>    
                                <!--
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-8">
                                        <input type="text" id="fec_cam" value="" class="form-control input-xs datepickers" />
                                    </div>                                                                                
                                </div>-->
                                <div class="form-group">
                                    <label class="col-xs-4 control-label label-xs">Valor&nbsp;Dolar:</label>  
                                    <div class="col-xs-8">
                                        <input type="text" id="rat_cam" value="" class="form-control input-xs" style="text-align: right;" onkeypress="return  validar_decimal(event)" />
                                    </div>                                                                                
                                </div>
                                <div class="form-group center"><br/>
                                    <a href="https://www.oanda.com/lang/es/currency/converter/" target="_BLANK">Consultar Cambio de Moneda</a>
                                </div> 
                            </div>
                        </div>  
                        ...
                        <div class="row">                            
                            ...
                            <div class="col-xs-7">
                                ...
                                <div>
                                    ...
                                    <button id="btnVetPrintCambio" onclick="$.imprimirUrl($(this).data('url'))" class="btn btn-sm btn-success" style="display:none;"><i class="glyphicon glyphicon-print"></i> Imprimir Moneda</button>
                                </div>                   
                            </div>
                            <div class="col-xs-5">
    ...
    <script type="text/javascript">
        ...
        $(function(){           
            ...
            docs.createGrid({
                ...
                colModel:[
                    ...
                    { label: 'Cód.Int.', name: 'Vet_Index', key: true, width: 15,align:"center", hidden:true },
            ...
            pagos.createGrid({
                ...
                colModel:[
                    { label: 'Cód.Int.', name: 'Vet_Num', key: true, width: 15, align:"center", hidden:true },
            ...
            items.createGrid({
                ...
                colModel:[
                    ...
                    {name:'Vet_Cam',label:'P/U Moneda', labelLong:'Precio Moneda', hidden:true, width:60, align:"right", title:false, editable:true, editoptions:{dataInit:stylePru}},                    
                    {name:'Vet_Pru',label:'P. Unitario', labelLong:'Precio Unitario', width:60, align:"right", title:false, editable:true, editoptions:{dataInit:stylePru}},                    
            ...
            items.jqGrid('footerData', 'set',{
                Vet_Cam:'<div style="margin-top:95px;" id="totalMoneda" class="footerFact"><label>Tot.Moneda</label><input id="t_rubros_c" name="t_rubros_c" type="text" class="total" readonly/></div>',
		...
		Vet_Pru:'<div class="footerFact" id="totalesLabel"><label>SUBTOTAL:</label><label>TARIFA 0%:</label><label>TARIFA <span class="iva_por"></span>%:</label><label><span class="iva_por"></span>% IVA:</label><label>ICE:</label><label>DESCUENTO:</label><label class="total">TOTAL:</label></div>',
        ...
        function stylePru(e,obj,opt){            
            ...
            if(e.name==='Vet_Pru'&&$('#es_cambio').is(':checked')) e.readOnly='readOnly';
            $(e).on('keyup',function (){
        ...
        function updateRowItem(obj){
            ...
            var es_cambio=$('#es_cambio').is(':checked');
            if(es_cambio){
		
                if(row['Vet_Cam']>0){
                    row['Vet_Pru']=row['Vet_Cam']/$('#rat_cam').val();
                }
                else{
                    row['Vet_Cam'] = row['Vet_Pru'] * $('#rat_cam').val();
                    row['Vet_Pru']=row['Vet_Cam']/$('#rat_cam').val();
                }

                items.find('tr#'+obj['rowId']+' td[aria-describedby=items_Vet_Pru] input').val($.toFixed(row['Vet_Pru'],8)).attr('readonly','readonly');
		items.find('tr#'+obj['rowId']+' td[aria-describedby=items_Vet_Cam] input').val(row['Vet_Cam']);
	    ...
	    if($('#es_cambio').is(':checked')){
	    	tot['t_rubros_c']= (tot['t_subtotal']+tot['t_iva']+tot['t_ice']-tot['t_descuento'])* $('#rat_cam').val();
	    }
        ...
        function validaDocument(){  
           ...
           docu['Cambio_Moneda']= $('#rat_cam').val();
        ...
        function saveDocument(data){ $.saveDataJson('',data,
            function (resp){
                ...
                $('#btnVetPrintCambio').removeAttr('disabled').show().data('url',resp['Vet_Link_Cambio']); 
                if($.varValid(resp['mail'])){
                    if(resp['mail']===false) $.alert('La transacción se realió con éxito!.<br> Surgio un problema al enviar el mail <u>Comprobante Electronico</u> al <i>Cliente</i>!');
        ...
        function getLocalStore(name){ var data=localStorage.getItem(name); if($.varValid(data)) return $.jsonParser(data); else return; }
        ...
        function setCambio(){
            items.jqGrid($('#es_cambio').is(':checked')?'showCol':'hideCol',["Vet_Cam"]);
            items.jqGrid('resizeGrid');
        }
