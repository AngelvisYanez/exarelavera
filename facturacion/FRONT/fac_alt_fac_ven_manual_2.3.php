...
<?php 
/**
* Descripcion: Alta de facturas de ventas
* Fecha de actualizacion:	2010-06-21 
* Desarrollador:	Jose Cumbicos
* Fecha de actualizacion: 2011-06-09
* Desarrollador: Nebil Oyola
* Fecha de actualizacion: 2012-02-17
...
*/
if(isset($ajax_mar_val)&& $ajax_mar_val==1)
{   
	/** 
...
/**
* Cargado AJAX de los resultados de la busqueda
*/
if (isset($deudas))
{ 	
...
/**
* Cargado Ajax de la proyeccion del Interes 
*/
if (isset($Ajax_Proye))
{
	/** 
...
/** 
* Cargado AJAX de los resultados de la busqueda del rubro 
*/
if (isset($buscod))
{
	$busqueda = $busqueda;
...
{	
	...
	$row_rs_codPlaPeri= $obBD_con1->getRowConsulta(1224, $Ses_Emp_Cod.'*'.$CajFec,$obBD_conexion);
	...
	if($total_rs_buscaAutori==1)
	{
		...
		$correcto=0;
		?><script type="text/javascript">alert("�No hay Autorizaci&oacute;n para la fecha ingresada!");</script> <?php
	}
}
...
//$row_rs_apercaja = $obBD_con1->getRowConsulta(1317, $Ses_Prs_Cod,$obBD_conexion);
/**
* Consulta de la autorizacion para las facturas 
*/
$row_rs_autorizaci = $obBD_con1->getRowConsulta(30, (isset($Tic_Cod)?$Tic_Cod:'').'*'.$row_rs_apercaja['Pun_Cod'].'*'.$hoy, $obBD_conexion);
	...
	*/
	...
	{ 
		...
		*/
		...
		if ($rs_infoEmpresa['Cof_Gce']=="S" && $rs_infoTipCom['Tic_Sri']*1!=0 && $rs_infoCliente['Aut_Tem']='E') /* Verifico si tiene autorizacion para generar F.E.*/
		{	
		...
		/**
		* Insercion de la cabecera de la factura 
		*/
		$obBD_ins1->operacionobBD(20, $Tic_Cod.'*'.$codigo.'*'.$Ciu_Cod.'*'.$Caj_Cod.'*'.$Vnd_Cod.'*'.	
		$Vet_Num.'*'.$Vet_Obs.'*'.$Aut_Cod.'*'.$Vet_Des.'*'.$hora.'*'.$url.'*'.$Vet_Aut.'*'.$Num_Ret.'*'.$Ret_Fec.'*'.$Num_Aut.'*'.$ForPagCodSri.'*'.$Vet_Ntd.'*'.$Vet_Fdm.'*'.$Vet_Nns, $obBD_conexion);	
		...
		foreach ($datos as $puntero => $item)
		{
			...
			{                            
				...
				*/
                ...
                if($rs_infoEmpresa['Cof_Con']=="S" && $rs_infoTipCom['Tic_Sri']*1!=0)
                {		   
		...
		*/
		if ($rs_infoEmpresa['Cof_Gce']=="S" && $rs_infoTipCom['Tic_Sri']*1!=0) /* Verifico si tiene autorizacion para generar F.E.*/
		{
		...
		*/		
		...
		{   
			...
			}// fin if ($rs_infoEmpresa['Cof_Gce']=="S" && $rs_infoTipCom['Tic_Sri']*1!=0)
...
{		
	...
	{
		/** 
		* Incremento del numero manual de la factura dependiendo del punto de impresion
		*/
		$Vet_Num = $obBD_con1->codigoSiguiente($row_rs_autorizaci['Aut_Cod'], $row_rs_autorizaci['Aut_Ini'], $obBD_conexion);		
		/**
		...
		*/
		...
		/**
		* Consulta de la ciudad de emision de la factura 
		*/
...
<HTML><HEAD>
    ...
    <BODY <?php if (isset($total_rs_vendedor)&&$total_rs_vendedor > 0){if ($total_rs_apercaja > 0){if ($total_rs_autorizaci > 0){if ($Vet_Num <= $row_rs_autorizaci['Aut_Fin']){ /*if ($codigo > 0){ */ ?> onLoad="setInterval('parpadeo(\'txt_blink\')',700);"<?php /*}*/}}}}?> >	
    <div id="set1">
<?Php //
/** 
...
if (count($row_rs_vendedor) > 0)
{
		...
		*/
		if (count($row_rs_autorizaci) > 0 or 1==1)
		{
			...
			*/	
                        //var_dump($Vet_Num); var_dump($row_rs_autorizaci['Aut_Fin']); 
...
<tr>
   ...
   <?php if($correcto==1){?>	
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name= "form1" id="form1">
     ...
     <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td align="center"><?php if(($Ses_Dat_Dis!='agronuevo')&& !isset($hdd_volver)  && isset($hdd_save)){echo "<br><span class='Texto_grande' style='color:blue;'>".mes($mese[1],1)."</span><br><span class='Texto_grande'><b style='color: darkgoldenrod;'>Cod:</b> ".$Vet_Cod."</span>";}?></td>
                </tr>
                </table>  
...
<?php
 ...
 <?Php if (isset($codigo) && $codigo > 0 && !(isset($hdd_save)))
 { 
	/**
...
<table width="95%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	if (isset($total_rs_cliente_esc) && $total_rs_cliente_esc > 0) 
	{?>
	<tr>	
...
<label class="Titulos2"> Generales </label>
 ...
 <table width="95%" border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	?>
      <select name="Tic_Cod" id="Tic_Cod" onChange="if(this.value=='4'){document.getElementById('NotasCredito').style.display = 'block';}else{document.getElementById('NotasCredito').style.display = 'none'} ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?ajax_mar_val=1&punto=<?php echo $row_rs_apercaja['Pun_Cod']; ?>&SucCod=<?php echo $Ses_Suc_Cod;?>&Tic_Cod='+ document.getElementById('Tic_Cod').value +'&fechaCja=<?php echo $row_rs_apercaja['Caj_Fec'] ; ?>','contenedormarca')">
        <option  value="">Seleccione...</option>
        <?Php
	foreach($row_tipo_compr as $row)
	{ ?>
        <option  <?Php if (isset($Tic_Cod) && $Tic_Cod == $row['Tic_Cod']){ echo "selected"; } ?> value="<?Php echo $row['Tic_Cod']; ?>"><?Php echo $row['Tic_Des']; ?></option>
        <?Php
	}
	?>
      </select></td>
  ...
  <tr>
    ...
    <FIELDSET id="NotasCredito">
         ...
         <table width="96%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td width="13%" align="right"><span class="Etiqueta1">Cod. documento:</span></td>
                <td width="5%" align="left">
                  <input name="Vet_Ntd" type="text" id="Vet_Ntd" size="3" maxlength="5" value="01" readonly="readonly"/>
                  </td>
                <td width="11%" class="Alertas">&nbsp;<em>Factura</em></td>
                <td width="14%" align="right" class="Alertas"><span class="Etiqueta1"><span class="Asterisco">*</span>&nbsp;Fecha Emisi&oacute;n:</span></td>
                <td width="8%" class="Alertas"><span class="LetraNegra">
                  <input name="Vet_Fdm" type="text" id="Vet_Fdm" size="10" onkeyup="mascara(this,'-',patron,true)"/>
                </span></td>
                <td width="17%" align="right" class="Alertas"><span class="Etiqueta1"><span class="Asterisco">*</span>&nbsp;Num. Secuencia:</span></td>
                <td width="32%" class="Alertas"><span class="LetraNegra">
                  <input name="Vet_Nns" type="text" id="Vet_Nns" onblur="" size="15" maxlength="20" />
                  </span></td>
              </tr>
              </table>
			  <script type="text/javascript">
	  ...
	  <td>&nbsp;</td>
          <td></td>
              ...
              <input name="activar1" type="checkbox" id="activar1" value="checkbox"  onclick="validar_text(); asignar_total_fac()" checked="checked" />
                      ...
                      <span><input name="Vet_Des" type="text" id="Vet_Des" size="2" maxlength="5" style="text-align:right" onblur="numerico(this)" onkeyup="validar_text(); asignar_total_fac()" value="<?Php echo isset($Vet_Des)?$Vet_Des:'';?>" />%</span></td>
          <td>
            <div align="center">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td class="LetraNegra"><input name="Vet_Cue" type="text" id="Vet_Cue" onKeyPress="return validar_numeric(event)" value="<?php echo isset($Vet_Cue)?$Vet_Cue:''; ?>" style="text-align:right" size="15" maxlength="15"></td>
          <td class="Etiqueta1"><span class="Asterisco">* </span>Cheque/Papeleta No: </td>
          <td width="12%" class="Titulos2"><input name="Vet_Che" type="text" id="Vet_Che" onKeyPress="return validar_numeric(event)" value="<?php echo (isset($Vet_Che)?$Vet_Che:0)+0; ?>" style=" text-align:right" size="10" maxlength="10" onBlur="ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?docBanco=1&num_Doc=' + this.value + '&Ban_Cod='+ document.getElementById('Ban_Cod').value,'div_numDoc')"></td>
    ...
    <tr>
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td class="Etiqueta1">Cuenta No:</td>
          <td class="LetraNegra"><input name="Vet_Cue2" type="text" id="Vet_Cue2" onKeyPress="return validar_numeric(event)" value="<?php echo isset($Vet_Cue)?$Vet_Cue:''; ?>" style="text-align:right" size="15" maxlength="15"></td>
          <td class="Etiqueta1"><span class="Asterisco">* </span>Cheque/Papeleta No: </td>
          <td width="13%" class="Titulos2"><input name="Vet_Che2" type="text" id="Vet_Che2" onKeyPress="return validar_numeric(event)" value="<?php echo isset($Vet_Che)?$Vet_Che:''; ?>" style="text-align:right" size="10" maxlength="10" onBlur="ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?docBanco=1&num_Doc=' + this.value + '&Ban_Cod='+ document.getElementById('Ban_Cod2').value,'div_numDoc2')"></td>
