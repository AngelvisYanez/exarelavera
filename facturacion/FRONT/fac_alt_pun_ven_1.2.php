...
if(isset($proAjax)){ 
    ...
    if($responce['records']>0){                
        foreach ($responce['rows'] AS &$r){
            $r['Precios']=$obBD_con1->getArrayConsulta(73, $Ses_Suc_Cod.'*'.$r['Pro_Cod'].'*'.'A', $obBD_conexion);
            $precio = $obBD_con1->getRowConsulta(73, $Ses_Suc_Cod.'*'.$r['Pro_Cod'].'*'.'A'.'*'.'D'.'*', $obBD_conexion);
...
if(isset($saveDocument)){
    ...
    $num_existe_gencod=$obBD_con1->getRowConsulta(50, $Ses_Suc_Cod.'*'.$Aut_Sri.'*'.$Vet_Num.'*'.(isset($Vet_Cod)?$Vet_Cod:'').'*'.$Pun_Sri, $obBD_conexion); //Consulto si ya existe un codigo generado en las retenciones basado en una autorizacion otorgada por el SRI 
