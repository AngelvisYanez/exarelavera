...
if (isset($proAjax)) {
	...
	$datos = array_merge($_GET, array('setWhere' => array('addStock')));
	$productos = $obBD_con1->getPageGridJson('producto.selectWhere', $datos, $obBD_conexion);
...
if (isset($saveDocumento)) {
	...
	try {
		...
		if ($data['Prf_is_editar'] == "E") {
			...
			$obBD_ins1->operacionobBD('proformas.update', array('Prf_Cod' => $prof_Cod['Prf_Cod'], 'Cli_Cod' =>  $data['Cli_Cod'], 'Prf_Fec' => $data['Prf_Fec'], 'Prf_Des' => $data['t_descuento'], 'Prf_Obs' => $data['Vet_Obs'], 'Prf_Ord' => $data['Prf_Num_Ext']), $obBD_conexionIns, true);
			...
			foreach ($items as $itemsito) {
				if ($itemsito['Pro_Cod'] != 0 && $itemsito['Iva_Por'] > 0) {
					...
					$obBD_ins1->operacionobBD('proformas_det.insert', array('Prf_Cod' => $prof_Cod['Prf_Cod'], 'Pfd_Int' => $conta + 1, 'Pro_Cod' => $itemsito['Pro_Cod'], 'Prf_Cant' => $itemsito['Vet_Can'], 'Prf_Uni' => $itemsito['Uni_Des'], 'Prf_Imp' => $itemsito['Vet_Imp'], 'Prf_Pru' => $itemsito['Vet_Pru'], 'Prf_Adi' => $itemsito['Prf_Adi'], 'Iva_Cod' => $data['Iva_Cod']), $obBD_conexionIns);
					$conta++;
				} else {
					if ($itemsito['Pro_Cod'] != 0) {
						//Guardo los nuevos items
						$obBD_ins1->operacionobBD('proformas_det.insert', array('Prf_Cod' => $prof_Cod['Prf_Cod'], 'Pfd_Int' => $conta + 1, 'Pro_Cod' => $itemsito['Pro_Cod'], 'Prf_Cant' => $itemsito['Vet_Can'], 'Prf_Uni' => $itemsito['Uni_Des'], 'Prf_Imp' => $itemsito['Vet_Imp'], 'Prf_Pru' => $itemsito['Vet_Pru'], 'Prf_Adi' => $itemsito['Prf_Adi'], 'Iva_Cod' => $itemsito['Iva_Cod']), $obBD_conexionIns);
						$conta++;
					}
				}
			}
...
<HEAD>
	<TITLE>
		<?php echo "[Exa] Proformas"; //echo $Ses_Sys_Nom; e 
		?>
...
<BODY>
	<div class="panel panel-main">
								...
								<div class="form-group">
									...
									<div class="col-xs-2">
										<button id="Prof_btn" type="button" onclick="$('#prfDialog').dialog('open');" class="btn btn-warning btn-xs" title="Modificar Proforma" tabindex="2">
											Buscar proforma <span class="glyphicon glyphicon-edit"> </span>
										</button>
									</div>
		...
		<script src="../VALIDACIONES/prf_val_proforma.js?a=101"></script>
