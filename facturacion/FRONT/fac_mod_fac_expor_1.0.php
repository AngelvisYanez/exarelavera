...
if(isset($cargarDoc)){
    $venta=$obBD_con1->getRowConsulta('ventas.selectWhere', array_merge($_GET, array('where'=>"ventas.Vet_Cod=".$Vet_Cod,'setWhere'=>array('setExportacion','setTotales'))), $obBD_conexion) ;
    $export=$obBD_con1->getRowConsulta('exporta_vent.selectWhere', array_merge($_GET, array('where'=>"exporta_vent.Vet_Cod=".$Vet_Cod)), $obBD_conexion,true) ;
    $resp= array_merge($venta,$export);
    $resp['success']=true;
    $obBD_con1->echoJson($resp);
}
if(isset($saveDocumento)){
    ...
    try{
        ...
        $obBD_ins1->operacionobBD('exporta_vent.update', array_merge($obBD_ins1->formatDataExp($_POST),array('where'=>array('Eve_Cod'=>$Eve_Cod))), $obBD_conexionIns); 
...
<BODY>
    <div class="panel panel-main">
        <div class="panel-heading exa-header"><h3 class="panel-title">&raquo;  Modificar Datos Exportacion</h3></div>
        <div class="panel-body ui-widget-content ui-corner-bottom exa-body">
            <script>                
                var busquedaButton=[{ label: '&nbsp;', name: 'act1', width: 20, align: 'center',viewable: false, formatter:'gridButton', formatoptions:{ action:'cargarDoc', data:'Vet_Cod', title:'Editar Exportacion' }, title:false }];
            </script>
            ...
            <div id="documentoEdit" style="display: none;">
                <div class="row">
                    ...
                    <input type="hidden" name="Eve_Cod" value=""  />    
                    <div class="col-sm-6">                        
                    ...
                    <div class="col-sm-6">    
                        ...
                        <fieldset class="exa-fieldset"  >
                            ...
                            <div class="form-group exporta tipo03">
                                ...
                                <div class="col-xs-2" ><input type="checkbox" id="Eve_Ren_Chk" name="Eve_Ren_Chk" value="S" offval="N" onchange="$('#Eve_Ren').prop('disabled',!$(this).is(':checked')).val('')" class="datatrigger veree" /></div>
                    ...
                    <div class="col-sm-12">       
                        <div class="center">
                            <button type="button" onclick="$('#documentoEdit') .moveComp('#documentoSearch').updateGridsSizes();" class="btn btn-sm btn-inverse"><i class="glyphicon glyphicon-arrow-left"></i> Atrás</button>
    ...
    function cargarDoc(Vet_Cod){    
        $.getDataJson('',{'cargarDoc':true,'Vet_Cod':Vet_Cod},function(resp){
            if(resp['Reg_Cod']==='01') resp['Pas_Cod_Gen']=resp['Pas_Cod'];
            if($.isNum(resp['Eve_Ren'])&&resp['Eve_Ren']*1>0) resp['Eve_Ren_Chk']='S';
            $('#formDocumento').setData(resp);
            $('#documentoSearch') .moveComp('#documentoEdit');
        }); 
    }
    ...
    function validaDocument(){
        ...
        $.createDialogConfirm('¿Esta seguro de modificar la Exportación?', data, saveDocument);
    }
    function saveDocument(data){ //console.log(data); console.log(data['rets']);
        $.saveDataJson('',data,
            function (resp){
                ...
                $('#documentoEdit').moveComp('#documentoSearch').updateGridsSizes();
                $.Search('venta');
            }
        ); 
    }
