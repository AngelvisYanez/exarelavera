...
if(isset($stocks_nd)){    
   $row_stocks = $obBD_con1->getArrayConsulta(142, $Ses_Emp_Cod, $obBD_conexion);
   $r=array('success'=>true, 'rows'=>$row_stocks, 'records'=>count($row_stocks), 'page'=>1);
...
if(isset($saveForm)){ 
    ...
    try{
        ...
        $num_existe_gencod=$obBD_con1->getRowConsulta(50, $Ses_Suc_Cod.'*'.$row_rs_autorizaci['Aut_Sri'].'*'.$Vet_Num.'*'.$Vet_Cod.'*'.$row_rs_autorizaci['Pun_Sri'], $obBD_conexion); //Consulto si ya existe un codigo generado en las retenciones basado en una autorizacion otorgada por el SRI
...
//         } unset($pag);
        ...
        if($configs['Cof_Con']=='S'&&($Tic_Sri*1!=0)){
            ...
            $obBD_conIns->operacionobBD(87, $Com_Cod.'*'.('D').'*'.$Vet_Tot.'*'.''.'*'.('Doc.'.$Vet_Num).'*'.$Pag_Pld, $obBD_conexionIns);  // inserta asiento // pago            
...
$row_rs_autorizaci = $obBD_con1->getRowConsulta(139, $Tic_Cod.'*'.$row_rs_vendedor['Pun_Cod'].'*'.$hoy, $obBD_conexion);      
    ...
    <div class="panel panel-main">
        ...
        <div class="panel-body ui-widget-content ui-corner-bottom exa-body">            
                <div class="row">
                    ...
                    <div class="col-sm-6">  
                        <fieldset class="exa-fieldset" style="min-height: 250px;">                           
                           <legend class="Titulos2">Listado Stocks no Regsistrados</legend> <!-- Form Name -->
                           <table id="list"></table>
                    ...
                    <div class="col-sm-6">  
                        <fieldset class="exa-fieldset" style="min-height: 250px;">                           
                           <legend class="Titulos2">Stock Seleccionado</legend> <!-- Form Name -->
                           <table id="list1"></table>
                           <div id="list1Pager"></div>
                        </fieldset>
                    </div>    
                    <script>
                    var compGrid;
                    $(document).ready(function () {                         
                        compGrid=$("#list").createGrid({  
                            postData: {stocks_nd:true},
                            rowNum: 10000000, rownumbers:true,
                            pgtext: ' ',  
                            colModel: [                               
                                { label: 'C�d.Int.', name: 'Pro_Cod', key: true, hidden:false,viewable:true, width:20 },
                                ...
                                { label: 'Importe', name: 'Pro_Imp',align:"right", width: 50  }
                            ],     
                    ...
                    function getValor(max){
                        var stocks=compGrid.getGridBatch();
                        var new_stocks=[];
                        var acum=0;
