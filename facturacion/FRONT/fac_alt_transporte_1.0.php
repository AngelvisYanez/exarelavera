...
if (isset($_POST['hdd_save']))
{
	if ($thisPost->postBlock($_POST['postID']))
	{
		...
		if($_POST['event'] == 1)
		{
			 ...
			 * @var string
			 */
			$Param_persona = $Prs_Ced.'*'.$Prs_Nom.'*'.$Prs_Ape.'*'.$Prs_Dir.'*'.$Ide_Cod;
				
			/**
			 * guardado en persona transporte
			 */
			$obBD_con1->InsetPersonaTransporte(7, $Param_persona, 8, $Param_transporte);
		}
		else
		{
			/**
			 ...
			 */
			$obBD_con1->insertUpdateDelete(8, $_POST['Prs_Cod'].'*'.$Ses_Emp_Cod, $obBD_conexion);
		}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
  <tr class="BarraTitulo">
    <td>&raquo; registrar Transportista</td>
  </tr> 
  <tr height="400">
    <td valign="top">
 <?php 
         ...
         <tr>
          ...
          <button type="button" class="btn btn-success fileinput-button" title="Comprobar" onclick="validar_requeridos(this.form, 'Prs_Ced', 0)">
		   ...
		   if (isset($_POST['hdd_comprobar']))
		   {
		    	if ($event == 0) 
		  		{ 
		  			echo "El Transporte con c&eacute;dula/R.U.C.: ".htmlspecialchars($_POST['Prs_Ced'], ENT_QUOTES, 'UTF-8').", ya se encuentra registrado";
				}	   
		   }
		   ?>
           </span> 
...
<label class='Titulos2'>Datos a registrar</label>
	...
	<FIELDSET>
	<LEGEND>
	<label class='Titulos2'>Datos del Transportista</label>
	</LEGEND>
  ...
  <tr>
    ...
    <td width="83%" class="LetraNegra">&nbsp;
      <?Php 
			...
			{
		?>
      <input name="Prs_Dir" type="text" style="text-transform:uppercase" id="Prs_Dir" value="" size="66" maxlength="60" >
      <?php 
			}
		?>
      </td>
