...
if(isset($hdd_save) && !isset($hdd_volver))
{
	...
	*/	
	...
	{	  
	  ...
	  */
	  $obBD_con1->operacionobBD(1038,$Adq_Cod.'*'.$Ite_Cod.'*'.$Mar_Cod.'*'.$Iva_Cod.'*'. $Pro_Obs.'*'.$Pro_Bar.'*'.$Ubi_Cod.'*'.$Lin_Cod.'*'.$Uni_Cod.'*'.$Pro_Cod.'*'.$Pro_Cdc.'*'.$Pro_Sec.'*'.$Pro_Uni,$obBD_conexion);	
	  /**
...
{	
	...
	*/
	...
	$total_rs_consulta = $row_rs_consulta['Ite_Cod'] > 0? 1 : 0;
        ...
        /**
	* Consulta el código de línea del producto
	*/
	$row_rs_consulta_lp = $obBD_con1->getRowConsulta(1209,$codigo,$obBD_conexion);
	...
	*/
	...
	$total_rs_ubicacion =  count($rs_ubicacion);
        /**
	* Consulta las líneas del producto
	*/
	$rs_lineas= $obBD_con1->getArrayConsulta(1208,$Ses_Emp_Cod,$obBD_conexion);
	$total_rs_lineas =  count($rs_lineas);
	/**
          ...
          <LEGEND>
			...
			{ 
			  foreach( $rs_buscar as $row_rs_buscar) { 
                ...
                <tr>
                  <td width="9%" align="center"><FONT COLOR="<? echo $rojo;?>"><?Php echo $row_rs_buscar['Pro_Cod']; ?></FONT></td>
                  ...
                  <td width="11%" align="center"><FONT COLOR="<? echo $rojo;?>"><?Php echo $row_rs_buscar['Mar_Des']; ?></FONT></td>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td colspan="5" class="LetraNegra">
      <input  border="0" name="Ite_Cor" type="text" id="Ite_Cor"  size="15" maxlength="50" value="<?Php echo $row_rs_consulta['Ite_Cor']?>" />      
  ...
  <tr>
    ...
    <td colspan="5" class="LetraNegra"><input   name="Ite_Lar" type="text" id="Ite_Lar" value="<?Php echo $row_rs_consulta['Ite_Lar']?>" size="25" maxlength="50" />  </td>
    </tr>
</table>
</fieldset>
<fieldset>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">    
  ...
  <tr>
      <td class="Etiqueta1"> L&iacute;nea: </td>
    <td colspan="5" class="LetraNegra"><select name="Lin_Cod" id="Lin_Cod">
        <option></option>
        <?php foreach($rs_lineas as $row_rs_lineas){ ?>
        <option  <?Php if ($row_rs_lineas['Lin_Cod'] ==$row_rs_consulta_lp['Lin_Cod']){ echo "selected"; }?> value="<?Php echo $row_rs_lineas['Lin_Cod']; ?>">
        <?php	echo $row_rs_lineas['Lin_Des'];  ?>
        </option>
        <?php }?>
    </select></td>
  </tr>
...
document.getElementById('Pro_Uni').value = ''; }">
    ...
    <tr>
      ...
      <td colspan="5" class="LetraNegra">          
          ...
          <input onkeypress="return validar_decimal(event)" border="0" name="Pre_Pvp" type="text" id="Pre_Pvp"  size="8" maxlength="8" style="text-align:right"  value="<?Php echo $row_rs_consulta['Pre_Pvp']; ?>" />
      </td>
      </tr>
    <tr>
      <td class="Etiqueta1">Descuento:</td>
      <td colspan="5" class="LetraNegra"><input name="Pro_Dsc" type="text" id="Pro_Dsc" style="text-align:right" onkeypress="return validar_decimal(event)" value="<?Php if($row_rs_consulta['Pro_Dsc']!=0){ echo $row_rs_consulta['Pro_Dsc'];}else{echo '0';} ?>"  size="8" maxlength="8" border="0" readonly="readonly" /></td>
    </tr>
    <tr>
      <td colspan="6" class="Etiqueta1"><div align="center"><? 
...
<input name="hdd_save" type="hidden" id="hdd_save" value="insertar">
  <table width="258" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <button type="button" class="btn btn-primary start" title="Guardar" onclick="validar_requeridos(this.form, 'Cat_Cod*Mar_Cod*Ite_Cor*Ite_Lar*Adq_Cod*Iva_Cod*Uni_Cod*Ubi_Cod*Ite_Cod', 1)"> <i class="icon-book icon-white"></i> <span>Guardar</span></button>
        <input  type="hidden" id="txt_busqueda" name="txt_busqueda" value="<? echo $volver_busqueda; ?>">
      </td>
    </tr>
  </table>
<?Php } ?>
