...
//Seccion para listar los clientes registrados en la empresa
...
//Seccion para obtener el numero de secuencia
if (isset($numeroSec)) {
...
if ($configs["Cof_NegCam"] == 'S') {
    $grupo_empresas = $obBD_con1->getRowConsulta(176, $Ses_Emp_Cod, $obBD_conexion); //Solo si tiene grupo de ecomar
    ...
    if ($responce['total'] > 0) {
        foreach ($responce['rows'] as &$row) {
            ...
            if (!empty($row['Cpc_Cod'])) {
                ...
                $row['Cpc_Ven'] = $row['Cpc_Ven'];
                $row['Cpc_Obs'] = $row['Cpc_Obs'];
            }
            if ($configs['Cof_Con'] == 'S' && !empty($row['Com_Cod'])) {
                ...
                $cnt_pg_vnta = $obBD_con1->getRowConsulta(91, $row["Vet_Cod"], $obBD_conexion);
                // $cnt_pg_vnta = $obBD_con1->getRowConsulta(171, $row["Vet_Cod"], $obBD_conexion);
                if (empty($row['Pld_Cod_Pag'])) {
                    $row["Pld_Cod_Pag"] = $cnt_pg_vnta["Pld_Cod"];
                }
                $row["aux_Pag_Cod"] = $cnt_pg_vnta["Pag_Cod"];
                $otras_comp = $obBD_con1->getRowConsulta(65, $row['Com_Cod'], $obBD_conexion);
                if ($otras_comp['total'] * 1 > 1) $row['Com_Edit'] = 'N';
                //Obtener negociacion si existe
                $row["Num_Neg"] = "";
                if (!empty($row['Cod_Neg'])) {
                    $num_nego = $obBD_con1->getRowConsulta(172, $row['Cod_Neg'], $obBD_conexion);
                    $row["Num_Neg"] = $num_nego["Num_Neg"];
                }
            }
        }
        unset($row);
...
//Seccion para extraer el Pun_Cod y Vnd_Cod del usuario sobre la tabla vendedor
...
// Inica pago
/* Cambiar Tipo de Pago A LA FACTURA DE VENTA */
if (isset($saveChangePago)) {
    // Validar clave de acceso si la empresa tiene claves activas
    $sql_clave = $obBD_con1->getArrayConsulta(182, '', $obBD_conexion);
    $clave_activa = (!empty($sql_clave) && is_array($sql_clave) && count($sql_clave) > 0);

    if ($clave_activa) {
        // Si hay clave activa, validar que se haya enviado y sea correcta
        $Cla_Cod = isset($_POST['empresaClaveValidada']) ? trim($_POST['empresaClaveValidada']) : '';
        $Cod_Psc = 1; // Código de proceso: Editar forma de pago de facturas autorizadas

        if (empty($Cla_Cod)) {
            $resp = array('success' => false, 'message' => 'Debe ingresar y validar la clave de acceso antes de guardar');
            $obBD_con1->echoJson($resp);
            exit();
        }

        // Validar la clave en el backend
        $row = $obBD_con1->getRowConsulta(183, array(
            'Emp_Cod' => $Ses_Emp_Cod,
            'Cla_Cod' => $Cla_Cod,
            'Cod_Psc' => $Cod_Psc
        ), $obBD_conexion);

        if (empty($row) || !isset($row['Cod_Cla'])) {
            $resp = array('success' => false, 'message' => 'Clave de acceso incorrecta o no válida');
            $obBD_con1->echoJson($resp);
            exit();
        }
    }

    $Pag_Cod = $aux_Pag_Cod; //El codigo del tipo de pago  ::CXC O ..
    $resp = array('success' => false);
    $obBD_con1->inicio_transaccion($obBD_conexion);
    $obBD_con1->validaCierrePeriodo('ventas', 'Caj_Fec', 'ventas.Vet_Cod', $Caj_Fec, $Vet_Cod, $obBD_conexion, 'S');
    try { //SE ENVIA CREDITO
        if ($For_Cod * 1 == 2) {
            //ESTABA A CREDITO
            if (isset($Cpc_Cod) && !empty($Cpc_Cod) && !is_null($Cpc_Cod)) {
                $obBD_con1->operacionobBD('ccpp_cobrar.update', array('Cpc_Ven' => $Cpc_Ven, 'Cpc_Obs' => $Cpc_Obs, 'where' => array('Vet_Cod' => $Vet_Cod, 'Cpc_Cod' => $Cpc_Cod, 'Com_Cod' => $Com_Cod)), $obBD_conexion, true);
            } else {  //ESTABA A CONTADO
                ...
                if (!empty($codigoRetencion) && !is_null($codigoRetencion)) {
                    ...
                    $obBD_con1->operacionobBD('ccpp_cobrar.insert', array('Cpc_Ven' => $Cpc_Ven, 'Cpc_Obs' => $Cpc_Obs, 'Vet_Cod' => $Vet_Cod, 'Com_Cod' => $Com_Cod), $obBD_conexion, true);
                }
            }
            ...
            if (!empty($retencionValue['total'])) { //Verifica si existe la retencion
            ...
            if ($onlyRetencion) {
                ...
                $tot_pagos = $obBD_con1->getRowConsulta(170, $Vet_Cod, $obBD_conexion);
                if ($tot_pagos["tot_pago"] == 0) {
                    $obBD_con1->operacionobBD('ccpp_cobrar.deleteWhere', array('where' => array('Vet_Cod' => $Vet_Cod, 'Cpc_Cod' => $Cpc_Cod, 'Com_Cod' => $Com_Cod)), $obBD_conexion);
                } else {
                    $resp = array('success' => false, 'message' => 'Esta factura tiene pagos activos, eliminalos para poder continuar.');
                    $obBD_con1->echoJson($resp);
                }
            }
        }
        $obBD_con1->operacionobBD('pago_venta.update', array('Pag_Cod' => $Pag_Cod, 'Pld_Cod' => $Pag_Pld,  'where' => array('Vet_Cod' => $Vet_Cod)),  $obBD_conexion,  true);
        $obBD_con1->operacionobBD('asientos.update', array('Pld_Cod' => $Pag_Pld, 'where' => array('Com_Cod' => $Com_Cod, 'Pld_Cod' => $Pld_Cod_Pag, 'Asi_Deh' => 'D')), $obBD_conexion, true);
    } catch (Exception $e) {
    ...
    try {
          if ($For_Cod * 1 == 2) {
            ...
            if ($onlyRetencion) {
                ...
                //Validar si existen pagos activos pedir que eliminen los pagos
                $tot_pagos = $obBD_con1->getRowConsulta(170, $Vet_Cod, $obBD_conexion);
                if ($tot_pagos["tot_pago"] == 0) {
                    $obBD_con1->operacionobBD('ccpp_cobrar.deleteWhere', array('where' => array('Vet_Cod' => $Vet_Cod, 'Cpc_Cod' => $Cpc_Cod, 'Com_Cod' => $Com_Cod)), $obBD_conexion);
                } else {
...
// Finalizar cambio de tipo de pago
...
/* Seccion para realizar el guardado */
if (isset($saveDocument)) {
    $obBD_con1->validaCierrePeriodo('ventas', 'Caj_Fec', 'ventas.Vet_Cod', $Caj_Fec, $editDoc['Vet_Cod'], $obBD_conexion, 'S');
    ...
    try {
        ...
        if (empty($rs_Caja['Caj_Cod'])) {
            //Seccion para aperturar la caja a traves de insert a la tabla caja_aper
            $obBD_conIns->operacionobBD(77, $rs_Punto['Pun_Cod'] . '*' . $Caj_Fec, $obBD_conexionIns);
            //Seccion para obtener el id ingresado en la tabla caja_aper
        ...
        //Codigo para la edicion del vendedor
        $Vnd_Cod_aux = (!empty($select_vendedores)) ? $select_vendedores : NULL;
        $cabeceraVenta = array('Tic_Cod' => $Tic_Cod, 'Cli_Cod' => $Cli_Cod, 'Ciu_Cod' => $Ciu_Cod, 'Caj_Cod' => $Caj_Cod, 'Vet_Ide' => $Vet_Ide, 'Vnd_Cod' => $rs_Punto['Vnd_Cod'], 'Vet_Num' => $Vet_Num, 'Vet_Obs' => $Vet_Obs, 'Aut_Cod' => $Aut_Cod, 'Vet_Des' => $Vet_Des, 'Vet_Hor' => $hora, 'Vet_Xml' => (isset($claveAcceso) ? $claveAcceso : ''), 'Vet_Aut' => (isset($Vet_Aut) ? $Vet_Aut : ''), 'Ret_Num' => $Ret_Num, 'Ret_Fec' => $Ret_Fec, 'Ret_Aut' => $Ret_Aut_Sri, 'Tpc_Cod' => $Tpc_Cod, 'Vet_Cod' => $Vet_Cod, 'Vet_Sri' => $Vet_Sri,  'Vnd_Cod_Aux' => $Vnd_Cod_aux, 'Vet_Prop' => $t_prop, 'Veh_Cod' => $Veh_Cod);
        ...
        if (isset($Cod_Neg) && !empty($Cod_Neg) && $Cod_Neg != 0) {
            $obBD_conIns->operacionobBD(169, $Cod_Neg . '*' . $Vet_Cod . '*' . 'VNT' . '*' . $Tip_Prod . '*' . $Cod_Nd, $obBD_conexionIns);
        }
        //SOLO REGISTRAR EL TIPO DE VENTA
        if ($configs["Cof_NegCam"] == 'S' &&  empty($Cod_Neg)) {
            $obBD_conIns->operacionobBD(169, 'NULL' . '*' . $Vet_Cod . '*' . 'VNT' . '*' . $Tip_Prod . '*' . $Cod_Nd, $obBD_conexionIns);
        }
        //Anular negociacion de la venta
        if (!empty($Cod_Nd) && empty($Cod_Neg) && empty($Num_Neg)  && empty($Tip_Prod)) {
            $obBD_conIns->operacionobBD(178, $Cod_Nd . '*' . 'VNT',  $obBD_conexionIns);
        }
        ...
        foreach ($items as $i => $item) {
            ...
            /* Item Documento */
            if ($item['Ret_Mod'] * 1 > 0) { // verificar si existe retencion
        ...
        if ($configs['Cof_Con'] == 'S' && (($Tic_Sri * 1 != 0) || $Check_Comprobante * 1 === 1)) {
            ...
            $Com_Con = $com_con; //registro de concepto contable
            $Com_Fec = $Caj_Fec;
    ...
    if ($obBD_conIns->Error == 0) {
        ...
        $response['Aut_Cod_Est']  = " <span style='color:red'>NO</span>";
...
if (isset($validarClaveAjax)) {
    $data = filter_input_array(INPUT_POST);
    $Cla_Cod = isset($data['clave']) ? trim($data['clave']) : '';
    $Cod_Psc = isset($data['Cod_Psc']) ? intval($data['Cod_Psc']) : 1; // Por defecto proceso 1
    $Emp_Cod = isset($data['Emp_Cod']) ? intval($data['Emp_Cod']) : $Ses_Emp_Cod;

    if (empty($Cla_Cod)) {
        $responce = array('success' => false, 'message' => 'Debe ingresar una clave');
    } else {
        $row = $obBD_con1->getRowConsulta(183, array(
            'Emp_Cod' => $Emp_Cod,
            'Cla_Cod' => $Cla_Cod,
        ...
        if (!empty($row) && isset($row['Cod_Cla'])) {
            $responce = array('success' => true, 'message' => 'Clave correcta');
        } else {
            $responce = array('success' => false, 'message' => 'Clave incorrecta o no válida');
        }
    }
    echo json_encode($responce);
    exit();
...
//Registra o editar el tipo de negociacion en las facturas
if (isset($saveChangeCamNeg)) {
    $ok = false;
    if (isset($Cod_Neg) && !empty($Cod_Neg) && $Cod_Neg != 0) {
        $obBD_con1->operacionobBD(169, $Cod_Neg . '*' . $Vet_Cod . '*' . 'VNT' . '*' . $Tip_Prod . '*' . $Cod_Nd, $obBD_conexion);
        $ok = true;
    }
    //SOLO REGISTRAR EL TIPO DE VENTA
    if ($configs["Cof_NegCam"] == 'S' &&  empty($Cod_Neg)) {
        $obBD_con1->operacionobBD(169, 'NULL' . '*' . $Vet_Cod . '*' . 'VNT' . '*' . $Tip_Prod . '*' . $Cod_Nd, $obBD_conexion);
        $ok = true;
    }
    //Anular negociacion de la venta
    if (!empty($Cod_Nd) && empty($Cod_Neg) && empty($Num_Neg) &&  empty($Tip_Prod)) {
        $obBD_con1->operacionobBD(178, $Cod_Nd . '*' . 'VNT',  $obBD_conexion);
        $ok = true;
    }
    if (($obBD_con1->Error == 0) && ($ok == true)) {
        $responce = array('success' => true, 'Actualizado con éxito');
    } else {
...
<HEAD>
    ...
    <script type="text/ecmascript" src="../VALIDACIONES/fac_val_factura2.js?x=105"></script>
    ...
    <style>
        ...
        .footrow td[aria-describedby="documento_Cop_Pru"] { padding: 0 !important; }
        .footerFact { text-align: right; width: 100%; }
        .footerFact input[type=text],
        .footerFact label,
        .footerFact textarea,
        .footerFact select { height: 19px; width: 100% !important; display: block; margin-bottom: 0px !important; margin-top: 0px !important; text-align: right; }
        ...
        .footerFact input.total { background-color: #254463; color: white; font-size: 14px; border: none; }
        ...
        .footrow td[aria-describedby="items_Vet_Imp"] { padding: 0 !important; }
    </style>
</HEAD>
...
<BODY>
            ...
            <div id="documentoSearch">
                        ...
                        <div class="col-xs-6">
                            <fieldset class="exa-fieldset">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-2">
                                        <select name="Pec_Cod" class="form-control input-xs search_pec" style="width: 120px; text-align: center;" onchange="if(this.value==='') $('#Cmb_Mes').attr('disabled','disabled'); else $('#Cmb_Mes').removeAttr('disabled');">
                                            <option value=""> << TODOS>> </option>
                                    ...
                                    <div class="col-xs-2">
                                        <select id="Cmb_Mes" name="Cmb_Mes" class="form-control input-xs search_pec" style="width: 120px; text-align: center;" disabled="disabled">
                                            <option value=""> << TODOS>> </option>
                                            <?php for ($i = 1; $i <= 12; $i++) { ?>
                                            <option <?php if ($i == $mes) {
                                                echo "selected=''";
                                    ...
                                    <label class="col-xs-3 control-label label-xs">Mis Ingresos</label>
                                    <div class="col-xs-1">
                ...
                <script>
                        ...
                        if (rows.length > 0) {
                            ...
                            Promise.all(promises).then(function() {
                                zip.generateAsync({
                                    ...
                                    link.download = "archivosPDF.zip"; // se le asigna un nombre junto a la extension del archivo comprimido
                                    link.click();
                    ...
                    function cargarDoc(doc) {
                        ...
                        TicCod = doc['Tic_Cod'];
                        $('#editDoc').setData({});
                        //$('#Pec_Cod').attr('disabled', true); //Bloquera el periodo contable
                        if (doc.Veh_Cod !== '')
                            $('#trans_carga').show();
                        else
                        ...
                        $('#select_vendedores').val(doc['Vnd_Cod_Aux'] * 1);
                        $('#com_con').val(doc['Com_Con']);

                        $('#formDocumento  #Num_Neg').val(doc['Num_Neg']);
                        $('#formDocumento input[name="Tip_Prod"]').prop('checked', false);
                        if (doc['Tip_Prod']) {
                            $('#formDocumento input[name="Tip_Prod"][value="' + doc['Tip_Prod'] + '"]').prop('checked', true);
                        }
                        $('#formDocumento  #Cod_Neg').val(doc['Cod_Neg']);
                        $('#formDocumento  #Cod_Nd').val(doc['Cod_Nd']);
                    ...
                    $('#searchGrid').createGrid({
                        ...
                        colModel: [
                            ...
                            { label: 'P. SRI', name: 'Tpc_Sri', width: 20, align: "center", formatter: 'title',
                                formatoptions: {
                                    title: function(o) { return o['Tpc_Des']; }
                                },
                                title: false
                            },
                            ...
                            { label: '&nbsp;', name: 'act2', width: 20, align: 'center', viewable: false, formatter: 'gridButton',
                                formatoptions: { action: ImpDoc, title: 'Imprimir Documento', icon: 'print', type: 'info' },
                                title: false
                            },
                            { label: '&nbsp;', name: 'act2', width: 20, align: 'center', viewable: false, formatter: 'gridButton',
                                formatoptions: { action: ImpCom, title: 'Imprimir Comprobante', icon: 'print', type: 'info' },
                                title: false
                            },
                            ...
                            { label: 'XML', name: 'act02', width: 20, align: 'center', viewable: false, formatter: 'gridButton',
                                formatoptions: { action: descargar, title: 'Ver XML', icon: 'file', type: 'info',
                            ...
                            { label: 'PDF', name: 'act02', width: 20, align: 'center', viewable: false, formatter: 'gridButton',
                                formatoptions: { action: viewPdfVenta, title: 'Ver PDF', icon: 'file', type: 'info',
                                    conditional: function(o) {
                                        return o.Vet_Est !== 'I' && !$.isEmpty(o.Vet_Xml);
                                    }
                                },
                ...
                <div class="row">
                    <div class="col-xs-12" id="panelVentas">
                                            ...
                                            <fieldset class="exa-fieldset">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-xs-10">
                                                        ...
                                                        <select id="select_vendedores" name="select_vendedores" defaultValue=1 class="form-control input-xs readOnly">
                                                            <option value="">Ninguno...</option>
                                                            <?php foreach ($data_personal as $value) {
                                                                echo  "<option value='$value[Vnd_Cod]'>$value[Vnd_Cod] - $value[Prs_Nom]  $value[Prs_Ape]</option>";
                                                            } ?>
                                                        </select>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group col-md-7">
                                                    ...
                                                    <div class="col-xs-8">
                                                        <div class="input-group input-group-xs">
                                                            ...
                                                            <span class="input-group-btn">
                                                                <button id="Prv_Btn_" type="button" onclick="$('#isUpdateNego').val('');$('#negDialog').dialog('open');" class="btn btn-success btn-xs" title="Buscar Negociación" tabindex="2"><span class="glyphicon glyphicon-search"></span></button>
                        ...
                        <div class="text-center">
                            <button class="black btn btn-md btn-inverse" onclick="clearDocument();$('#documentoMain').moveComp('#documentoSearch').updateGridsSizes();"><i class="glyphicon glyphicon-arrow-left"></i>Atr&aacute;s</button>
                            <button class="btn btn-md btn-primary" type="button" onclick="$('#formDocumento').formSubmit();"><i class="glyphicon glyphicon-floppy-disk"></i> Guardar</button>
                            <?php //if ($Ses_Emp_Cod == 96  || $Ses_Emp_Cod == 361 ||  $Ses_Emp_Cod == 427   ) { 
                            ?>
                            <button class="btn btn-md btn-success" id="save_aut" onclick="$('#save_aut').val('save_aut'); $('#formDocumento').formSubmit();"><i class="glyphicon glyphicon-floppy-disk"></i> Guardar y Autorizar</button>
                            <?php //} 
                            ?>
                        </div>
                    </div>
                    ...
                    <div class="col-xs-6" id="resultContent">
                        <fieldset class="exa-fieldset">
                            ...
                            <div>
                                ...
                                <p class="resp"><span>&raquo;Autoriz:</span><span class="databind" data-name="Aut_Cod_Est"></span></p>
    ...
    <script>
        ...
        //     if (cliente.op_ide === '06') $('#op_ide3').prop('checked', true).trigger('change');
        ...
        function selectCliente(cliente) {
            ...
            // Habilitar radios según tipo de identificación
            if ((cliente.op_ide === '04' || cliente.op_ide === '05') && cliente.Prs_Ced.substring(2, 3) * 1 !== 9) {
                $('input[name=Vet_Ide]').attr("disabled", true).trigger('change');
                $('input[name=Vet_Ide][value="4"]').attr("disabled", false).trigger('change');
                $('input[name=Vet_Ide][value="5"]').attr("disabled", false).trigger('change');
            } else if (cliente.op_ide === '06') {
                $('input[name=Vet_Ide]').attr("disabled", true).trigger('change');
                $('input[name=Vet_Ide][value="6"]').attr("disabled", false).trigger('change');
            } else if (cliente.op_ide === '07') {
                $('input[name=Vet_Ide]').attr("disabled", true).trigger('change');
                $('input[name=Vet_Ide][value="7"]').attr("disabled", false).trigger('change');
            } else {
                $('input[name=Vet_Ide]').attr("disabled", true).trigger('change');
            }
            $('#clieDialog').dialog('close');
    ...
    <script>
        ...
        $.createSearchDialog('imeiDialog', [
            ...
            { label: 'IMEI', name: 'Ime_Num', index: 'Ime_Num', width: 150, classes: 'highlightSearch' },
            ...
            { label: 'Estado', name: 'Ime_Est', index: 'Ime_Est', width: 100, 
                formatter: function(cellvalue) {
                    return cellvalue === 'A' ? 'Activo' : 'Inactivo';
                }
            },
            { label: '&nbsp;', name: 'act1', width: 60, align: 'right', viewable: false,
                formatter: 'gridButton',
                formatoptions: {
                    action: selectImei,
                    icon: 'arrow-right',
            ...
            beforeRequest: function() {
                ...
                $(this).jqGrid('setGridParam', {
                    postData: { 'Pro_Cod': Pro_Cod, 'Ime_Tip': Ime_Tip }
                });
                return true;
            }
        }, {
    ...
    <!--div id="changePagoDialog" title="Cambiar Forma de Pago" style="display:none;">
        <form class="form-horizontal normal" id="changePagoForm" action="javascript:saveChangePago();">
            ...
            <input type="text" name="Pld_Cod_Pag" id="Pld_Cod_Pag" class="hidden" />
            <fieldset class="exa-fieldset">
                <legend class="Titulos2">Forma de Pago</legend>
                <div class="form-group">
                    <label class="col-xs-2 control-label label-xs">Tipo:</label>
                ...
                <div class="form-group">
                    <label class="col-xs-2 control-label label-xs required">Tipos:</label>
                    <div class="col-xs-10">
                        <?php //$rs_tipo_ = $obBD_con1->getArrayConsulta(69, '', $obBD_conexion);
                        ?>
                        <select id="aux_Pag_Cod" name="aux_Pag_Cod" class="form-control input-xs readOnly" data-trigger="" required="">
                            <?php
                            /*  echo "<option value='' >Seleccione...</option>";
                            foreach ($rs_tipo_ as $row) { ?>
                                <option value="<?= $row["Pag_Cod"] ?>"> <?php echo  $row["Pag_Des"]  ?> </option>
                            <?php   } */ ?>
                        </select>
                    </div>
                </div>
    ...
    <div id="changePagoDialog" title="Cambiar Forma de Pago" style="display:none;">
        <input type="text" id="isUpdateNego" name="isUpdateNego" style="display:none;">
        <div class="tabs-container">
            <ul class="nav nav-tabs" style="margin-bottom: 8px;">
                <li class="active"><a data-toggle="tab" href="#tab-forma-pago" style="padding: 5px 10px;">Forma de Pago</a></li>
                <?php if ($configs["Cof_NegCam"] == 'S') { ?>
                    <li> <a data-toggle="tab" href="#tab-neg-camaron" style="padding: 5px 10px;">Negociación Camarón</a></li>
                <?php } ?>
            </ul>
            <div class="tab-content">
                <div id="tab-forma-pago" class="tab-pane active">
                    <form class="form-horizontal normal" id="changePagoForm" action="javascript:saveChangePago();">
                        <input type="text" name="Vet_Cod" class="hidden" />
                        <input type="text" name="Cpc_Cod" class="hidden" />
                        <input type="text" name="Com_Cod" class="hidden" />
                        ...
                        <fieldset class="exa-fieldset">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-10">
                                    <?php $rs_tipo_ = $obBD_con1->getArrayConsulta(69, '', $obBD_conexion); ?>
                                    <select id="aux_Pag_Cod" name="aux_Pag_Cod" class="form-control input-xs readOnly" data-trigger="" onchange="checkPago();" required="">
                                        <?php
                                        echo "<option value='' >Seleccione...</option>";
                                        foreach ($rs_tipo_ as $row) { ?>
                                            <option value="<?= $row["Pag_Cod"] ?>"> <?php echo  $row["Pag_Des"]  ?> </option>
                            ...
                            <!-- Seccion habilitada para formas de pago tipo Transferencia -->
                            <div class="form-group pagoTransf" style="display: none; margin-top: 5px;">
                                <label class="col-xs-2 control-label label-xs required">Banco:</label>
                                <div class="col-xs-5">
                                    <?php $rs_bancos = $obBD_con1->getArrayConsulta(18, '', $obBD_conexion); ?>
                                    <select id="aux_Bak_Cod" name="Bak_Cod" class="form-control input-xs readOnly">
                                ...
                                <label class="col-xs-1 control-label label-xs required">Cta-Banco:</label>
                                <div class="col-xs-4">
                                    <input type="text" id="aux_Vet_Cue" name="Vet_Cue" onchange="" class="form-control input-xs">
                                </div>
                            </div>

                            <div class="form-group pagoCheque" style="display: none;">
                                <label class="col-xs-2 control-label label-xs required">Número:</label>
                                <div class="col-xs-4">
                                    <div class="input-group input-group-xs">
                                        <input type="text" id="Vet_Che" name="Vet_Che" onchange="" class="form-control input-xs">
                                ...
                                <label class="col-xs-2 control-label label-xs required">Fecha del Cheque:</label>
                                <div class="col-xs-4">
                                    <input id="Fec_che" name="Fec_che" type="text" class="form-control input-xs datepickers">
                                </div>
                            </div>
                        </fieldset>
                        <!-- Validar si la clave de autorizacion esta activa -- -->
                        <?php
                        $clave_activa = false;
                        $sql_clave = $obBD_con1->getArrayConsulta(182, '', $obBD_conexion);
                        if (!empty($sql_clave) && is_array($sql_clave) && count($sql_clave) > 0) {
                            $clave_activa = true;
                        }
                        ?>
                        <div class="center">
                            <div class="separator"></div>
                            <?php if ($clave_activa): ?>
                                <!-- Si la empresa tiene clave activa, mostrar botón para habilitar input clave -->
                                <button id="showClaveInputBtn" type="button" class="btn btn-xs btn-warning"><i class="glyphicon glyphicon-lock"></i> Ingresar clave para habilitar acciones</button>
                                <!-- Campo input para la clave (inicialmente oculto) -->
                                <div id="claveInputSection" style="margin-top:15px;display:none;">
                                    <input type="password" id="empresaClaveInput" class="form-control input-xs" placeholder="Ingrese la clave de la empresa..." style="display: inline-block; width:220px;">
                                    <button type="button" id="validarClaveBtn" class="btn btn-xs btn-info">Validar clave</button>
                                    <span id="claveInputFeedback" class="help-block" style="display:inline;margin-left:10px;"></span>
                                </div>
                                <br>
                                <!-- Campo oculto para almacenar la clave validada -->
                                <input type="hidden" id="empresaClaveValidada" name="empresaClaveValidada" value="" />
                                <!-- Botón Guardar (inicialmente oculto, se habilita tras validación) -->
                                <button id="guardarBtn" type="submit" class="btn btn-sm btn-success no" style="display:none;"><i class="glyphicon glyphicon-floppy-disk"></i> Guardar</button>
                                <script type="text/javascript">
                                    $(function() {
                                        $('#showClaveInputBtn').click(function() {
                                            $('#claveInputSection').slideDown();
                                            $('#empresaClaveInput').focus();
                                        });
                                        $('#validarClaveBtn').click(function() {
                                            ...
                                            $.ajax({
                                                ...
                                                success: function(resp) {
                                                    if (resp.success) {
                                                        $('#claveInputFeedback').text('Accion habilitada.').css('color', 'green');
                                                        // Guardar la clave validada en el campo oculto
                                                        $('#empresaClaveValidada').val(valorClave);
                                                        $('#guardarBtn').show();
                                                        $('#empresaClaveInput, #validarClaveBtn').prop('disabled', true);
                                                    } else {
                                                        $('#claveInputFeedback').text(resp.message || 'Clave incorrecta').css('color', 'red');
                                                        $('#empresaClaveValidada').val(''); // Limpiar clave si es incorrecta
                                                        $('#guardarBtn').hide();
                                                    }
                                                },
                                                error: function() {
                                                    $('#claveInputFeedback').text('Error validando clave').css('color', 'red');
                                                    $('#guardarBtn').hide();
                                                }
                                            });
                                        });
                            ...
                            <?php else: ?>
                                <!-- Si NO hay clave activa, mostrar guardar directamente -->
                                <button type="submit" class="btn btn-sm btn-success no"><i class="glyphicon glyphicon-floppy-disk"></i> Guardar</button>
                            <?php endif; ?>
                        </div>
                        <!--div class="center">
                ...
                <?php if ($configs["Cof_NegCam"] == 'S') { ?>
                    <div id="tab-neg-camaron" class="tab-pane">
                        <form class="form-horizontal normal" id="asigNegCamForm" action="javascript:saveNegCamPago();">
                            <input type="text" name="Vet_Cod" class="hidden" />
                            <input type="text" name="Cod_Nd" class="hidden" />
                            <fieldset class="exa-fieldset">
                                <legend class="Titulos2">Negociación camarón</legend>
                                <div class="col-md-12">
                                    <div class="form-group col-md-12">
                                        <label class="col-md-4 control-label label-xs">Neg. camarón:</label>
                                        <div class="col-md-8">
                                            <div class="input-group input-group-xs">
                                                <input type="text" name="Num_Neg" id="Num_Neg" placeholder="Ingrese cod.Negociación..." class="form-control input-xs dialogSearch" tabindex="1" readonly />
                                                <input type="text" name="Cod_Neg" id="Cod_Neg" style="display:none;" />
                                                <span class="input-group-btn">
                                                    <button id="Prv_Btn_" type="button" onclick="$('#isUpdateNego').val('facIsAut'); $('#negDialog').dialog('open');" class="btn btn-success btn-xs" title="Buscar Negociación" tabindex="2"><span class="glyphicon glyphicon-search"></span></button>
                                    ...
                                    <div class="form-group col-md-12">
                                        <label class="col-md-4 control-label label-xs">Tipo de Venta:</label>
                                        <div class="col-md-8 d-flex align-items-center">
                                            <label class="radio-inline d-flex align-items-center">
                                                <input type="radio" name="Tip_Prod" id="Tip_Prod_N" value="" style="margin:0"> <span style="margin-left:20px">Ninguno</span>
                                            </label>
                                            <label class="radio-inline d-flex align-items-center">
                                                <input type="radio" name="Tip_Prod" id="Tip_Prod_B" value="B" style="margin:0"> <span style="margin-left:20px">Balanceado</span>
                                            </label>
                                            <label class="radio-inline d-flex align-items-center">
                                                <input type="radio" name="Tip_Prod" id="Tip_Prod_L" value="L" style="margin:0"> <span style="margin-left:20px">Larva</span>
                                            </label>
                                            <label class="radio-inline d-flex align-items-center">
                                                <input type="radio" name="Tip_Prod" id="Tip_Flet_F" value="F" style="margin:0"> <span style="margin-left:20px">Flete falso</span>
                                            </label>
                                            <label class="radio-inline d-flex align-items-center">
                                                <input type="radio" name="Tip_Prod" id="Tip_Ins_I" value="I" style="margin:0"> <span style="margin-left:20px">Insumos</span>
                                            </label>
                                            <label class="radio-inline d-flex align-items-center">
                                                <input type="radio" name="Tip_Prod" id="Tip_Otr_D" value="D" style="margin:0"><span style="margin-left:20px">Otros. Desc.</span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
