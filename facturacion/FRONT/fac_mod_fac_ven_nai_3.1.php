...
if(isset($guardaClieAjax)){
    ...
    if($obBD_con1->Error==0) {$responce=array('success'=>true,'clie'=>$data);} else {$responce=array('success'=>false,'message'=>'No se pudo realizar la transacci�n!','error'=>$obBD_con1->MsgError);}
...
if(isset($cargarDoc)){
    ...
    $responce['Iva_Por']=0;
    foreach ($responce['items'] as $r) if($r['Iva_Por']*1>0){ $responce['Iva_Por']=$r['Iva_Por']; break; }
    // para viajes
    ...
    foreach ($responce['items'] as &$r){
        ...
        $r['Viajes']=is_array($viajes)&&!empty($viajes)?array_map(function($e){ return $e['Via_Cod']; }, $viajes):'';
    }unset($r);
...
if(isset($saveDocument)){
    $obBD_con1->validaCierrePeriodo('ventas','Caj_Fec','Vet_Cod',$Caj_Fec,$editDoc['Vet_Cod'],$obBD_conexion);
...
//              verificar si existe retencion
     ...
     /* REGISTRO PAGO VENTA */
         foreach ($pagos as $i=>&$pag){
             ...
             $pag['Vet_Num']=$Vet_Num; $pag['Vet_Cod']=$Vet_Cod;
    ...
    if ($obBD_conIns->Error == 0) {
        ...
        if($Aut_Tem=='E' && $input_autorizacion==''){
                ...
                /*$response['xml']*/ $xml = $obBD_elect->createXmlFactura($Vet_Cod, $Aut_Cod, $claveAcceso, $obBD_conexion);
...
<HTML>
    <HEAD>
        ...
        <SCRIPT> //utilizacion de variable global que identifica el padre del archivo js validaciones
            ...
            var edicion_ventas=true, vet_num_ant=0,tic_cod_ant=0;
            var docs, items, pagos, data=[], Vet_Index=1, Vet_Selected, index, Cof_Con='<?php echo $configs['Cof_Con']; ?>';
            var array_vendedor=<?php echo json_encode($rs_Punto);?>;
        </script>

        <script type="text/ecmascript" src="../VALIDACIONES/fac_val_factura.js?v=35">
        </script>
        <script>
...
<BODY>
    <div class="panel panel-main">
        <div class="panel-heading exa-header"><h3 class="panel-title ">&raquo; Modificar Documentos de Ventas</h3><p id="cabeceraPuntoImp" class="text-right col-xs-12  " style="margin-top:-15px;">punto de impresion</p></div>
            ...
            <div id="documentoSearch">
                ...
                <script>
                    ...
                    function cargarDoc(doc){
                        ...
                        $.getDataJson('',{'cargarDoc':true,'vet_cod':doc['Vet_Cod'],'Aut_Cod':AutCod,'Tic_Cod':TicCod,'Cli_Cod':doc['Cli_Cod']},function(resp){
                                $('#editDoc').setData(doc);
                                $('#For_Cod').val(resp['For_Cod']).trigger('change');
                                $('#clieFormTemp,#viajesForm').setData({Prs_Ced:doc['Prs_Ced'],Cli_Cod:doc['Cli_Cod'],cliente:doc['cliente_per'],op_opciones:'c'});
                        ...
                        $('#searchGrid').createGrid({
                            caption:'Resultado de la B�squeda',height: 270, datatype: "local",caption:'Resultados <div class="pull-right"><b>ORDENAR POR:</b>&nbsp;<select id="OrderBy"><option value="">No ordenar</option><option value="order by caja_aper.Caj_Fec DESC ">Fecha Venta</option><option value="order by Vet_Num DESC ">Num. Documento</option></select>&nbsp;</div>',
                            colModel: [
                                ...
                                { label: 'Reten.', name: 'Ret_Exi', width: 20 ,align:"center", formatter:'truefalse', formatoptions:{yesMsg:'Tiene Retencion',noMsg:' '}, title:false},
								...
								{ label: 'XML', name: 'act02', width: 20, align: 'center',viewable: false, formatter:'gridButton', formatoptions:{ action:descargar, title:'Ver XML', icon:'file', type:'info', conditional:function(o){ return o.Vet_Est!=='I'&&!$.isEmpty(o.Vet_Xml); } }, title:false },
								{ label: 'PDF', name: 'act02', width: 20, align: 'center',viewable: false, formatter:'gridButton', formatoptions:{ action:viewPdfVenta, title:'Ver PDF', icon:'file', type:'info', conditional:function(o){ return o.Vet_Est!=='I'&&!$.isEmpty(o.Vet_Xml); } }, title:false },
                                ...
                                <form id="formDocumento" class="form-horizontal normal formDatos" action="javascript:validaDocument(true);">
                                    ...
                                    <div class="col-md-5 col-xs-12">
                                        <fieldset class="exa-fieldset" id="clieFormTemp">
                                            ...
                                            <div class="form-group">
                                                <label class="col-xs-2 control-label label-xs required">C�dula/RUC:</label>
                                                <div class="col-xs-7" >
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-xs-5">
                                                   <div class="addAutorizacion">
                                                      <button type=button id="btnAddAut" title="Autorizacion Externa" class="col-xs-1 input-xs btn btn-warning "><i class="glyphicon glyphicon-alert"></i></button><div><input maxlength="55" minlength="49"  title="minimo 49 caracteres" class="input-xs" name="input_autorizacion" size=""/></div>
                                                   </div>
                                                </div>
    ...
    <!-- Inicio del di�logo para buscar clientes -->
        <div id="clieDialog" title="B&uacute;squeda de Cliente" style="display: none;"><form class="form-horizontal normal"> </form></div>
        <script>
        ...
        <div id="changeReteDialog" title="Cambiar valor de Retenci�n" style="display:none;">
   ...
   <script type="text/javascript">
    ...
    <script>
           ...
           $.createSearchDialog('proDialog',[
            ...
            { label:'&nbsp;', name: 'act1', width: 20, align: 'center',viewable: false, formatter:'gridButton', formatoptions:{
                    action:selectItem,
    ...
    <div id="provDialog" title="B&uacute;squeda de Proveedor" style="display: none;"><form class="form-horizontal normal"> </form></div>
    <script>
    ...
    function selectProvee(provee){
            $('#clieFormTemp,#viajesForm').setData($.extend(provee,{op_opciones:'c'})).find('.dialogSearch').addClass('x');
    ...
    <script>
        ...
        $(function() {
            ...
            $('#reteresult').createGrid($.extend(opts,{caption:'Detalle Retenci�n'}),true);
    ...
    <div id="docDetaDialog" title="Documento" style="display: none;">
        <fieldset class="exa-fieldset" >
            ...
            <div class="form-group condensed">
                ...
                <div class="col-xs-12" style="text-align: right;font-size: 8px;padding-top: 2px;"><b>CREACI�N:</b> <span name="Vet_Sys" class="databind"></span> &nbsp;&nbsp;-&nbsp;&nbsp; <b>USUARIO:</b> <span name="vendedor_per" class="databind"></span></div>
            </div>
            </div>
        </fieldset>
