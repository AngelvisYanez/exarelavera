<?php 
/**
* Descripci�n: Permite registrar alta de vendedor
* Fecha de actualizaci�n:	2014-06-17
...
if ($txt_busqueda != "")
{
	if ($op_opciones == "d")
	{
		...
		*/
		$rs_buscar = $obBD_con1->getArrayConsulta(8, trim($txt_busqueda).'*'.$Ses_Suc_Cod,$obBD_conexion);
	}
	else 
	{
		/** 
		...
		*/
		$rs_buscar = $obBD_con1->getArrayConsulta(9, trim($txt_busqueda).'*'.$Ses_Suc_Cod,$obBD_conexion);
	}  
}  
...
if (isset($hdd_save) && !isset($hdd_volver)) 
{
	if ($thisPost->postBlock($_POST['postID'])) 
	{
		...
		$obBD_con1->operacionobBD(11,$Pun_Cod."*".$Prs_Cod.'*'.$Vnd_Cod,$obBD_conexion);
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table">
  <tr class="BarraTitulo">
	  <td height="10">&raquo; Modificar  vendedor </td>
  </tr>
  <tr>	  	
		...
		<label class="Titulos2">Resultados de la busqueda</label>
			...
			{	  
			  ...
			  <td align="center"><?Php echo $row_rs_buscar['Prs_Cod']; ?></td>
				...
				<td align="left">&nbsp;
				<?Php echo marcarCadenaColor($txt_busqueda,$row_rs_buscar['Prs_Ape'].' '.$row_rs_buscar['Prs_Nom'],'#FFFF00', '#000', 1); ?></td>
				<td align="left"><?Php echo $row_rs_buscar['Pun_Des']; ?></td>
				<form name="form1" id="form1" method="post" action="<?Php echo $_SERVER['PHP_SELF']; ?>">
                <td align="center">								
				<input name="codigo" id="codigo" type="hidden" value="<?Php echo $row_rs_buscar['Prs_Cod'];?>">
                <input name="punCod" id="punCod" type="hidden" value="<?Php echo $row_rs_buscar['Pun_Cod'];?>">
                <input name="Vnd_Cod" id="Vnd_Cod" type="hidden" value="<?Php echo $row_rs_buscar['Vnd_Cod'];?>">
