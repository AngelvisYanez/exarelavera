...
if (isset($searchAllVentas)) {
    ...
    if ($configs['Cof_Con'] === 'S') {
        ...
        $datos = array_merge($_GET, array('setWhere' => array(/*'byCcpp', 'byVentasCompr',*/ 'setSucCod'),
            'join'=>array(
...
if (isset($upVenta)) {
    ...
    $stado = true;
	$obBD_con1->validaCierrePeriodo('ventas','Caj_Fec','Vet_Cod',null,$Vet_Cod,$obBD_conexion);
    $obBD_ins1->inicio_transaccion($obBD_conexionIns);
    try {
        if ($configs['Cof_Con'] === 'S'){
            
            unset($vetD);
              $obBD_con1->operacionobBD('ventas.setInactive',array('Vet_Cod'=>$Vet_Cod),$obBD_conexion);
              $obBD_con1->operacionobBD('venta_reembolsos.deleteWhere',array('Vet_Cod'=>$Vet_Cod),$obBD_conexion);
        ...
        $obBD_con1->operacionobBD('viaje.update',array('Vet_Cod'=>null, 'Vet_Ite'=>null, 'where'=>array('Vet_Cod'=>$Vet_Cod)), $obBD_conexion);
    }
...
<HEAD>
    ...
    <BODY>
        <div class="panel panel-main" id="formFinal">
            <div class="panel-heading exa-header"><h3 class="panel-title">&raquo;  Anular de Ventas</h3></div>
	...
	<script src="../VALIDACIONES/fac_val_fac_ven_2.0.js?k=112"></script>
