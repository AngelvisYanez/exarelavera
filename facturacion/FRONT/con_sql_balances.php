 ...
 // echo $sql."<br>"; 
		...
		*/
		...
		$sql = "SELECT det_plan.Pld_Cod, Pld_Cdc, Pld_Des, Pld_Est, Pld_Rec, Pld_Tip FROM det_plan, det_estado WHERE det_plan.Pld_Cod = det_estado.Pld_Cod 
						AND Pla_Cod=$Par_Sql[0] AND Pld_Rec=$Par_Sql[1] AND Est_Cod = $Par_Sql[2]";
		//echo $sql."<br/>";
                    return $sql;
		break;
...
ORDER BY
  asientos.Asi_Deh";				
		...
		break;
                ...
                case 240:
                    $sql = "SELECT det_plan.Pld_Cod,Pld_Cdc,Pld_Des,
                    SUM(IF(Asi_Deh='D',ROUND(Asi_Val,3),0)) as debe,SUM(IF(Asi_Deh='H',ROUND(Asi_Val,3),0)) as haber, 
                    IF((SUM(IF(Asi_Deh='D',ROUND(Asi_Val,3),0))-SUM(IF(Asi_Deh='H',ROUND(Asi_Val,3),0)))>=0,(SUM(IF(Asi_Deh='D',ROUND(Asi_Val,3),0))-SUM(IF(Asi_Deh='H',ROUND(Asi_Val,3),0))),0) AS deudor,
                    IF((SUM(IF(Asi_Deh='D',ROUND(Asi_Val,3),0))-SUM(IF(Asi_Deh='H',ROUND(Asi_Val,3),0)))<0,ABS(SUM(IF(Asi_Deh='D',ROUND(Asi_Val,3),0))-SUM(IF(Asi_Deh='H',ROUND(Asi_Val,3),0))),0) AS acreedor
                    FROM asientos  
                    INNER JOIN det_plan ON asientos.Pld_Cod = det_plan.Pld_Cod                    
                    ...
                    WHERE  
                        ...
                        GROUP BY det_plan.Pld_Cod  ORDER BY Pld_Cdc ASC";
//echo $sql;
		return $sql;
