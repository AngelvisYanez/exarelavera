...
if (isset($profAjax)) {
   $responce = $obBD_con1->getPageGridJson('orden_compra.selectWhere', $_GET, $obBD_conexion,true);
...
if(isset($profAjaxTotales)){
    ...
    $respuesta = $obBD_con1->getPageGrid('orden_compra.selectWhere',$data,$obBD_conexion,true);
...
if(isset($profDetalleAjax)){
    $resProformas=array(
        'success' => true,
        'todasPrf'=>$obBD_con1->getArrayConsultaSql("select prfd.Ord_Cod, prfd.Pfd_Int, prfd.Pro_Cod, prfd.Ord_Imp,  prfd.Ord_Cant, prfd.Ord_Pru, prfd.Iva_Cod, prf.Ord_Cod, prf.Ord_Des,prf.Ord_Num, prf.Ord_Obs, prd.Pro_Cod, prd.Ite_Cod, iv.Iva_Cod, iv.Iva_Por, iv.Iva_Ini, iv.Iva_Fin, itm.Ite_Cod, CONCAT(itm.Ite_Lar,' ', prfd.Ord_Adi) AS Ite_Lar, prfd.Ord_Adi  from proformas_det prfd, proformas prf, producto prd, item itm, iva iv WHERE prf.Ord_cod=prfd.Ord_cod AND prfd.Pro_Cod=prd.pro_cod AND prd.ite_cod=itm.ite_cod and iv.Iva_Cod=prfd.Iva_Cod and prfd.Ord_Cod='$Ord_Cod';", $obBD_conexion),
		...
		'ventasAct'=>$obBD_con1->getArrayConsultaSql("SELECT ventas.* , caja_aper.Caj_Fec, CONCAT(Prs_Nom,' ',Prs_Ape) AS Cliente from ventas inner join cliente on proveedore.Cli_Cod=ventas.Cli_Cod inner join persona on persona.Prs_Cod=proveedore.Prs_Cod inner join caja_aper on ventas.Caj_Cod=caja_aper.Caj_Cod  where Ord_Cod= '$Ord_Cod';",$obBD_conexion),
		'numVentasAct'=>$obBD_con1->getArrayConsultaSql("SELECT  COUNT(*) AS total from ventas inner join cliente on proveedore.Cli_Cod=ventas.Cli_Cod inner join persona on persona.Prs_Cod=proveedore.Prs_Cod inner join caja_aper on ventas.Caj_Cod=caja_aper.Caj_Cod  where Ord_Cod= '$Ord_Cod';",$obBD_conexion),
        'ivas'=>!isset($Ord_Fec)?array():$obBD_con1->getArrayConsultaSql("SELECT * FROM iva WHERE  '$Ord_Fec'  BETWEEN Iva_ini AND Iva_fin AND Iva_Est = 'A' AND Iva_Por>0;", $obBD_conexion, true),
    );
    $obBD_con1->echoJson($resProformas);
}

$numActualProf=$obBD_con1->getRowConsultaSql("SELECT MAX(Ord_Num) AS total FROM orden_compra INNER JOIN proveedore ON proveedore.Prv_Cod=orden_compra.Prv_Cod INNER JOIN persona ON persona.Prs_Cod=proveedore.Prs_Cod INNER JOIN sucursal ON proveedore.Emp_Cod=sucursal.Emp_Cod WHERE (sucursal.Emp_Cod='$Ses_Emp_Cod');", $obBD_conexion);
...
<HEAD>
    ...
    <BODY>
        <div class="panel panel-main">
            <div class="panel-heading exa-header"><h3 class="panel-title">&raquo; Proformas</h3></div>
                            ...
                            <fieldset class="exa-fieldset">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-5 radioset opt_search">
                                        ...
                                        <input id="radsc2" name="op_opciones" type="radio" value="c" onclick="setfocus(this.form.search)" alt="" /><label for="radsc2">&nbsp;&nbsp;# Proforma&nbsp;&nbsp;</label>
					...
					<fieldset class="exa-fieldset">
						...
						<div class="form-group">
							...
							<div class="col-xs-5 radioset opt_search">
								...
								<input id="radsct2" name="op_opciones" type="radio" value="c" onclick="setfocus(this.form.search)" alt="" /><label for="radsct2">&nbsp;&nbsp;# Proforma&nbsp;&nbsp;</label>
							...
							<div class="col-xs-5">
								<div class="input-group input-group-sm">
									...
									<span class="input-group-btn">
										<button type="button" onclick="this.form.submit()" class="btn btn-success btn-xs" title="Buscar Proforma" tabindex="-1"><span class="glyphicon glyphicon-search"></span> <span>Buscar</span></button>
									</span>
								</div>
								<!-- /input-group -->
							</div>
	...
	<div class="container" id="documentoVista" style="display:none;">
		<div id="datosPrf" class="box-container">
			...
			echo $obBD_con1->getReportHeader($Ses_Suc_Cod, 'PROFORMA N� <span id="titleReporte"></span>', ' ', $obBD_conexion); 
			}
			?>
							...
							<table border="0" cellpadding="0" cellspacing="0" id="cabeceraTabla" style="width: 100%;border-collapse: collapse; font-family:Verdana, Geneva, sans-serif; font-size:12px "
							 class="rep">
								...
								<tr>
									...
									<td>
										<span name="Ord_Ord" style="font-size: 12px;" class="form-control input-xs databind datatitle">
									</td>
								</tr>
								<tr>
									<td class='bold' style="font-size: 12px;">
			...
			<div class="jqHeaderFirst jqFirst" style="min-height: 50px;">
				<table id="datosTabla" style="table-layout: fixed; width: 100%; word-wrap: break-word; border-collapse: collapse; font-family:Verdana, Geneva, sans-serif; font-size:12px"
				 align='center' cellpadding="5" border="1" class="noBorder">
					<thead>
						<tr>
							<th style="width:8%;font-size: 12px;">Cantidad</th>
							<th style="width:56%;font-size: 12px;">Descripci�n</th>
							<th style="width:12%;font-size: 12px;">P.Unitario</th>
							<th style="width:12%;font-size: 12px;">Importe</th>
							<th style="width:12%;font-size: 12px;" align='right'>Total</th>
						</tr>
					</thead>
			...
			<table style=" width: 700px;font-size: 12px;border-collapse: collapse;" border="1">
				<tr>
					<td class='bold' align='center' style="font-size: 12px;">
				...
				<tr style="height: 40px;">
					<td>
						<div id="Ord_Obs" align='left' style="padding: 5px; overflow: hidden; word-wrap: break-word; width: 600px;" name="Ord_Obs" class="form-control input-xs databind datatitle bold"></div>
					</td>
					<td>
							...
							<div class="col align-self-center">
								<fieldset class="exa-fieldset">
									...
									<table id="cabeceraTabla" style="width: 100%;border-collapse: collapse;" class="rep">
										...
										<tr>
											<td class='bold'>DIRECCI&Oacute;N:</td>
											<td colspan="4">
												<span name="Prs_Dir" style="font-size: 12px;" class="form-control input-xs databind datatitle">
											</td>
										</tr>
								...
								<table id="datosTablaDos" style="width: 100%;border-collapse: collapse; display:none;" align='center' cellpadding="5" border="1"
								 class="pull-right">
									...
									<tbody class='noBorder' style="border-collapse: collapse;">
										...
										<tr>
											<td colspan="1"></td>
											<td colspan="3" align="right" class="bold" style=" border-top: 1px solid;">TOTAL PROFORMA:</td>
											<td align='center' style=" border-top: 1px solid;">
					...
					<div class="col-sm-8">
						<div class="col align-self-center">
							<table style="width: 100%;font-size: 12px;border-collapse: collapse;" border="1">
								<tr>
									<td class='bold' align='center' style="width: 25%;">OBSERVACI&Oacute;N:</td>
	...
	<div id="docDetaDialog" title="Proforma - Venta">
		<fieldset class="exa-fieldset">
			<legend class="Titulos2">Proforma:</legend>
			<div class="form-horizontal normal" style="padding: 0 4px;">
				<div class="form-group">
					<label class="col-xs-2 control-label label-xs">Cod.Int:</label>
					<div class="col-xs-2">
						<span name="Ord_Cod" class="form-control input-xs"></span>
					</div>
					<label class="col-xs-1 control-label label-xs">Num:</label>
					<div class="col-xs-2">
						<span name="Ord_Num" class="form-control input-xs"></span>
					</div>
					<label class="col-xs-1 control-label label-xs">Ord:</label>
					<div class="col-xs-3" style="text-align: center;">
						<span name="Ord_Ord" class="form-control input-xs"></span>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-xs-4" style="text-align: center;">
						<span name="Ord_Fec" class="form-control input-xs"></span>
					</div>
				</div>
				<div class="form-group">
	...
	<script src="../VALIDACIONES/ord_val_con_orden_compra.js?x=109"></script>
