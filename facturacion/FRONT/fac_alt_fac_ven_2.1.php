...
{   
	...
	*/
	...
	if(count($row_rs_autorizaci) > 0)
	{
		...
		echo  "<table width='100%' border='0' cellpadding='0' cellspacing='0'>
			 <tr>
			   ...
			   <td><input name='Vet_Num' type='text' id='Vet_Num' size='7' maxlength='7' value='".$Vet_Num."' style='text-align:right' onKeyPress='return validar_numeric(event)'>
	...
	{
		echo "<span class='Alertas3'>No existe registro de autorizaci?n para el tipo de comprobante</span>";
	}
	exit();
}
/**
...
if (isset($cmb_tipo))
{
	...
	*/
    ...
    <select name="<?Php echo $nom_ban; ?>" id="<?Php echo $nom_ban; ?>">
           <?php if(count($row_rs_bancos)==0){ ?><option value="NULL">(Ninguno)</option><?php } ?>
	   <?php 
...
if (isset($buscod))
{
	...
	$codigo = $codigo;
	$Sem_Cod = $Sem_Cod;
	include("../COMPONENTES/tesComRubrosConsulta3.php");
	exit();	
}//if (isset($buscod))
	...
	*/
	...
	{ 
		...
		*/
                ...
                $Pec=explode('*', $Pec_Cod);
                $campo="Cli_Cod";
                $mese = explode('-', $row_rs_apercaja['Caj_Fec']);
				//var_dump($mese);
                $Com_Num= $obBD_ins1->codigoComprAuto($op, $Pec[0], $mese[1], $obBD_conexion);
		...
		*/
		...
		{	
			...
			{
				$cadena=date("dmY",strtotime($Caj_Fec)).$rs_infoTipCom['Tic_Sri'].$rs_infoEmpresa['Emp_Ruc'].$rs_infoEmpresa['Cof_Fac'].$rs_infoEmpresa['Suc_Sri'].$rs_infoCliente['Pun_Sri'].$ceroDoc.$Vet_Num."12345678".$rs_infoEmpresa['Cof_Fte'];
			}else{
				...
				{	
					...
					$cadena=date("dmY",strtotime($Caj_Fec)).$rs_infoTipCom['Tic_Sri'].$rs_infoEmpresa['Emp_Ruc'].$rs_infoEmpresa['Cof_Fac'].substr($file[0], 14, 23).$rs_infoEmpresa['Cof_Fte'];
				}else{					
					/*clave de acceso de tipo emision NORMAL*/
					$cadena=date("dmY",strtotime($Caj_Fec)).$rs_infoTipCom['Tic_Sri'].$rs_infoEmpresa['Emp_Ruc'].$rs_infoEmpresa['Cof_Fac'].$rs_infoEmpresa['Suc_Sri'].$rs_infoCliente['Pun_Sri'].$ceroDoc.$rs_infoCliente['Vet_Num']."12345678".$rs_infoEmpresa['Cof_Fte'];
		...
		if($Tic_Cod=='1')
		{
			$Vet_Ntd=''; $Vet_Fdm=''; $Vet_Nns='';
		}
						
		/**
		* Inserci?n de la cabecera de la factura 
		*/
		$obBD_ins1->operacionobBD(19, $Tic_Cod.'*'.$codigo.'*'.$Ciu_Cod.'*'.$Caj_Cod.'*'.$Vnd_Cod.'*'.	
		$Vet_Num.'*'.$Vet_Obs.'*'.$Aut_Cod.'*'.$Vet_Des.'*'.$hora.'*'.$url.'*'.$Vet_Aut.'*'.$Vet_Ntd.'*'.$Vet_Fdm.'*'.$Vet_Nns, $obBD_conexion);	
		...
		foreach ($datos as $puntero => $item)
		{
			...
			if ($cant==15)
			{
				...
				*/				
				...
				[4].'*'.$param[5].'*'.$param[6].'*'.$param[9].'*'.$param[11].'*'.$param[10].'*'.$param[12].'*'.$param[13].'*'.$param[14].'*null*null', $obBD_conexion);
				...
				*/
                ...
                if($rs_infoEmpresa['Cof_Con']=="S")
                {
                    ...
                    $obBD_ins1->operacionobBD(1260, $Pec[0].'*'.$codigo.'*'.$Com_Num.'*'.$row_rs_apercaja['Caj_Fec'].'*'.trim($Vet_Obs).'*'.$op.'*'.$t_rubros.'*'.trim($Vet_Obs).'*'.$campo, $obBD_conexion);
                    ...
                    if($For_Cod*1==2){ 
                       ...
                       $obBD_ins1->operacionobBD(1263, $Com_Cod.'*'.'D'.'*'.$t_rubros.'*'.trim($Vet_Obs).'*'.'Fact. No.'.$Vet_Num.'*'.$CC_Cod[0],$obBD_conexion);
                    ...
                    foreach($productos as $fila){
                            $row_rs_procuen = $obBD_con1->getRowConsulta(1264, $fila[0],$obBD_conexion);
                    ...
                    if(isset($t_iva)&&($t_iva*1)>0){
                        $row_rs_ivacuen = $obBD_con1->getRowConsulta(1265, $Pec[1],$obBD_conexion);
...
<HTML><HEAD>
		...
		*/
		jQuery(function($){
			$("#Vet_Nns").mask("999-999-999999999",{placeholder:"_"});			
		});	            			
		</script>
...
<table width="98%" border="0" cellpadding="0" cellspacing="0" class="table">
 <tr class="BarraTitulo">
  ...
  <td width="19%" align="right">&raquo; <strong>CAJA: </strong><?Php echo $row_rs_apercaja['Caj_Fec']; ?></td>
   ...
   <?php if (!isset($Pec_Cod) && !isset($hdd_volver))
{
?>
    <tr valign="top" height="400" ><td colspan="3"><form action="<?Php echo $_SERVER['PHP_SELF']?>" method="post" id="form1" name= "form1">	<FIELDSET>
	<LEGEND>
...
<?php
 ...
 ?>
               ...
               <label class="Titulos2">Datos del Cliente </label>
                    ...
                    <table width="95%" border="0" cellpadding="0" cellspacing="0">
                      ...
                      <tr>	
                        ...
                        <td class="LetraNegra">&nbsp;<?Php echo $row_rs_cliente['Prs_Ape'].' '.$row_rs_cliente['Prs_Nom'] ?>
	...
	<?php }//Fin del if ($total_rs_representante > 0) ?>	
            ...
            <label class="Titulos2"> Generales </label>
             ...
             <table width="95%" border="0" cellpadding="0" cellspacing="0">
              ...
              <tr>
                ...
                <td width="17%" class="LetraNegra">&nbsp;<?php echo $row_rs_apercaja['Caj_Fec'] ?>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
                <td width="10%"  valign="top" class="Etiqueta1"><span class="Asterisco">* </span>Forma:</td>
                <td width="12%" valign="top" class="LetraNegra">
                    <?Php 
                     /**
...
<br>	
            ...
            <button type="button" class="btn btn-primary start" title="Guardar" onclick="validar_facturacion(this.form)">
                ...
                <input name="PrsCodCli" id="PrsCodCli" type="hidden" value="<?Php echo $PrsCodCli;?>">  
              </td>
            </tr>
            </table>
	&nbsp;
