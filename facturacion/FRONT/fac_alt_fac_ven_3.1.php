...
if (isset($saveDocument)) {
    ...
    try {
        ...
        // $obBD_conIns->beginTrans();

        // try {

        //     $extra = array(
        //         'Emp_Cod'    => $Ses_Emp_Cod,
        //         'Ext_Placa'  => !empty($_POST['Ext_Placa']) ? trim($_POST['Ext_Placa']) : '',
        //         'Ext_PediPos'=> !empty($_POST['Ext_PediPos']) ? trim($_POST['Ext_PediPos']) : '',
        //         'Ext_NrAcep' => !empty($_POST['Ext_NrAcep']) ? trim($_POST['Ext_NrAcep']) : '',
        //         "Vet_Cod"    => $Vet_Cod
        //     );

        //     // Insertar en la consulta 2006 usando el Vet_Cod generado
        //     $obBD_conIns->operacionobBD(2006,$extra ,$obBD_conexionIns);            

        // } catch (Exception $e) {
        //     // Si hay error, revertir la transacción
        //     $obBD_conIns->revertTrans();
        //     // Mostrar error en la consola y en pantalla
        //     //ChromePhp::log("Error en la inserción: ", $e->getMessage());
...
//Cargar el proceso en caso de haber activado el modulo  vendedores
...
// if (isset($saveExtObsAjax)) {
//     $data = $_POST;
//     $data['Emp_Cod'] = $Ses_Emp_Cod;
//     $obBD_con1->operacionobBD(2006, $data, $obBD_conexion, true);
//     // //ChromePhp::log();
//     if ($obBD_con1->Error == 0) {
//         $responce = array('success' => true, 'Registrado con éxito');
//     } else {
...
<BODY>
                    ...
                    <div class="col-xs-12" id="panelVentas">
                        <div class="row">
                                ...
                                <div class="col-md-7 col-xs-12">
                                    ...
                                    <?php if (($is_vendedor_activo["total_rows"]) >= 1) {  ?>
                                        ...
                                        <fieldset class="exa-fieldset" id="clieFormTemp">
                                            ...
                                            <div class="form-group">
                                                <label class="col-xs-2 control-label label-xs required">Seleccionar:</label>
                                                <div class="col-xs-6">
                            ...
                            <div class="col-md-7 col-xs-12">
                                <form id="reteFormTemp" action="javascript:" class="formDatos form-horizontal normal">
                                    <fieldset class="exa-fieldset">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <label class="col-xs-1 control-label label-xs ">Autoriza:</label>
                                            <div class="col-xs-4">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class=" col-xs-3 input-group input-group-xs reteTot">
                                ...
                                <div class="row" id="anticipo" style="display: none;">
                                    <div class="form-group">
                                        <label class="col-sm-1 control-label label-xs">Anticipo:</label>
                                        <div class="col-sm-2">
                        ...
                        <div style="margin-left: 10px;">
                            <button class="btn btn-sm btn-primary" onclick="$('#formDocumento').formSubmit();">
    ...
    <!-- DIALOGO DE OBSERVACION ADICIONAL  -->
    <!-- <div id="ObsExtraCreateDialog" title="Observacion Adicional" style="display:none;">
        <form class="form-horizontal normal" id="ExtObsCreateForm" >
            <input name="Ext_Cod" id="Ext_Cod" type="text" class="hidden" />
            <fieldset class="exa-fieldset">
                <legend class="Titulos2"> Agregar Observacion Adicional </legend>
                <div class="form-group">
                    <label class="col-xs-2 control-label label-xs" style="text-align: justify">Placa:</label>
                    <div class="col-xs-8">
                        <input name="Ext_Placa" id="Ext_Placa" type="text" class="form-control input-xs" style="margin-bottom: 10px;"/>
                    </div>
                </div>

                <div class="form-group">
                    <label class="col-xs-2 control-label label-xs" style="text-align: justify">Pedido Pos:</label>
                    <div class="col-xs-8">
                        <input name="Ext_PediPos" id="Ext_PediPos" type="text" class="form-control input-xs" style="margin-bottom: 10px;"/>
                    </div>
                </div>

                <div class="form-group">
                    <label class="col-xs-2 control-label label-xs" style="text-align: justify">Nro. de Aceptacion:</label>
                    <div class="col-xs-8">
                        <input name="Ext_NrAcep" id="Ext_NrAcep" type="text" class="form-control input-xs" />
                    </div>
                </div>

                    <script>
                        $(document).ready(function() {
                            var datosExtras = {}; // Variable para guardar los datos temporalmente

                            // También guarda los datos cuando el usuario cierra el cuadro de diálogo manualmente
                            $("#ObsExtraCreateDialog").on("dialogclose", function() {
                                datosExtras = {
                                    Ext_Placa: $("#Ext_Placa").val().trim(),
                                    Ext_PediPos: $("#Ext_PediPos").val().trim(),
                                    Ext_NrAcep: $("#Ext_NrAcep").val().trim()
                                };
                                    //console.log("Datos guardados al cerrar:", datosExtras); // Verifica que los datos no se pierdan
                            });

                            // Antes de enviar el formulario principal, asegurarse de que los valores se mantengan
                            $("#formDocumento").on("submit", function(event) {
                                event.preventDefault(); // Evita el envío automático

                                // Inserta los valores guardados en los campos antes de enviar
                                $("#Ext_Placa").val(datosExtras.Ext_Placa);
                                $("#Ext_PediPos").val(datosExtras.Ext_PediPos);
                                $("#Ext_NrAcep").val(datosExtras.Ext_NrAcep);

                                // los valores del formulario emergente se copian en los inputs ocultos del formulario principal
                                $("<input>").attr({type: "hidden", name: "Ext_Placa", value: $("#Ext_Placa").val()}).appendTo(this);
                                $("<input>").attr({type: "hidden", name: "Ext_PediPos", value: $("#Ext_PediPos").val()}).appendTo(this);
                                $("<input>").attr({type: "hidden", name: "Ext_NrAcep", value: $("#Ext_NrAcep").val()}).appendTo(this);

                                // Enviar el formulario correctamente
                                event.target.submit(); // Aquí usamos event.target en lugar de this
                            });
                        });
                    </script>
            </fieldset>
            <div class="Titulos2">
                <hr><b>NOTA:</b> Al cerrar este cuadro de dialogo, los datos esperaran al proceso de guardado de la factura
            </div>
        </form>
    </div> -->
