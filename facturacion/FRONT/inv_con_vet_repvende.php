...
if(isset($ajaxKardex)){ 
    ...
    $responce['rows']=$obBD_con1->getArrayConsulta(7 ,$Ses_Emp_Cod.'*'.$Ses_Suc_Cod.'*'.$Vnd_Cod.'*'.$ini.'*'.$fin, $obBD_conexion);
...
<meta charset="iso-8859-1" />
                ...
                <div class="row">
                           ...
                           <legend class="Titulos2">Descripci�n Cliente:</legend> <!-- Form Name -->
                              <div class="row">                                  
                                        ...
                                        <div class="form-group">
                                          ...
                                          <div class="col-xs-9">  
                                            <div class="input-group input-group-xs">                                                
                                                ...
                                                <input id="Vnd_Ced"  type="text" class="form-control" placeholder="Seleccione un vendedor ..." required readonly />
                                                <span class="input-group-btn">
                                                    <button class="btn btn-success" onclick="$('#vndDialog').dialog('open');" type="button"><span class="glyphicon glyphicon-check" title="Buscar vendedor"></span></button>
                                                </span>
                                              </div><!-- /input-group -->                                 
                                          </div>  
                                  ...
                                  <div class="col-xs-4">
                                      <!-- static input-->
                                        <div class="form-group">
                                          <label class="col-xs-3 control-label label-xs ">Vendedor:</label>  
                                          <div class="col-xs-8">                                    
                                              <span  class="form-control input-xs" id="Vendedor"></span>                              
                                          </div>                                  
                                        </div>                                                                  
                                            ...
                                            <div class="row">
                                                <div class="col-xs-12">
                                                    <div class="form-group">
                                                        ...
                                                        <div class="col-xs-3">     
                                                            <input name="ini" type="text" id="iniVnd" class="form-control input-sm">                                                                                        
                                                        </div>
                                                        <label class="col-xs-2 control-label label-xs ">Hasta:</label>
                                                        <div class="col-xs-3">                                    
                                                            <input name="fin" type="text" id="finVnd" class="form-control input-sm">                              
                                                        </div>
                                                        <div class="col-xs-2">
                                                          <div class=""><button type="button"  onclick="/*if($('#Pro_Cod').val()!==''){*/this.form.submit();$('#listVnd').jqGrid('setCaption', 'Salidas de Mercaderia '+' - '+($('#producto').val()!==''?$('#producto').val()+' - ':'')+'Desde '+ $('#iniVnd').val()+' Hasta '+$('#finVnd').val());/*}else{$.alert('Seleccione el Producto');}*/" class="btn btn-sm btn-success" title="Ejecutar Búsqueda"><span class="glyphicon glyphicon-search"></span> &nbsp;Filtrar</button></div>
                                                        </div>
                                                      </div>
                                                </div>
                                            </div>
                    ...
                    <div class="col-xs-12" style="min-height: 350px;">
                        <table id="listVnd"></table>
                        <div id="listVndPager"></div>
                        <script>
                             $(document).ready(function () {
                                ...
                                kardexGrid.jqGrid({
                                    ...
                                    rowNum: 10000000, pager: "#listVndPager", gridview: true, rownumbers: true, viewrecords: true, pgbuttons: false,pgtext: null,                           
                                ...
                                kardexGrid.navGrid('#listVndPager',{ edit: false, add: false, del: false, search: false, refresh: true, view: false, position: "left", cloneToTop: false });
                    ...
                    <div class="col-xs-12" style="padding-top: 10px;">
                        <button class="btn btn-sm btn-primary" onclick="$('#listVnd').jqGrid('printGrid',{nombre:'Ventas',hoja:'Salidas',caption:true});" type="button" title="Imprimir    "><span class="glyphicon glyphicon-print"></span> Imprimir</button>
                        <button onclick="$('#listVnd').jqGrid('exportGridExcel',{nombre:'Ventas',hoja:'Salidas',caption:true});" class="btn btn-sm btn-primary start" title="Descargar archivo de Excel"> <i class="icon-share icon-white"></i> <span>Excel</span></button>
                    </div>
                </div> 

<!--INICIO DEL DIALOGO BUSCAR Vendedor--> 
    <div id="vndDialog" title="B�squeda de Vendedores">  
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-7" >                 
                      <div class="input-group">                        
                        <input name="search" onkeydown="if (event.keyCode === 13) this.form.submit()" type="text" size="50" maxlength="50" placeholder="Ingrese vendedor a buscar..." autofocus class="form-control input-sm " /><input type="text" style="display:none"/>
                        <span class="input-group-btn"><button type="button" onclick="this.form.submit()" class="btn btn-success btn-sm" title="Buscar Vendedor" ><span class="glyphicon glyphicon-search"></span> <span> Buscar</span></button></span>
                      </div><!-- /input-group -->                          
                    </div>                    
                </div>
        </fieldset>  
    ...
    <script type="text/javascript">
        $(document).ready(function() {               
                $.createSearchDialog('#vndDialog',[
                        { label: 'C�d.Int.', name: 'Vnd_Cod', key: true,hidden:true,viewable: true },                                
        ...
        function selectVnd(data){
            $('#Vnd_Cod').val(data['Vnd_Cod']);
            $('#Vnd_Ced').val(data['Prs_Ced']);
            $('#vnd_dir').html(data['Prs_Dir']);
            $('#Vendedor').html(data['Vendedor']);
            $('#vndDialog').dialog('close');
            $('#listVnd').jqGrid('setCaption', 'Salidas de Mercaderia '+' - '+data['Prs_Ced']+' - '+'Desde '+ $('#iniVnd').val()+' Hasta '+$('#finVnd').val());
            $('#listVnd').Search('#formVnd','ajaxKardex');
        }
    </script>
<!-- FIN DEL DIALOGO Vendedor-->
