...
* Fecha de actualizaci�n:	2012-02-19 
* Desarrollador:	Lewis Chimarro
* Fecha de actualizaci�n:	2012-05-12
* Desarrollador:	Lewis Chimarro
* Fecha de actualizaci�n:	2012-11-26
* Desarrollador:	Lewis Chimarro
* Fecha de actualizaci�n:	2015-02-06
* Desarrollador:	Lewis Chimarro
*/
...
if (isset($hdd_save) && !isset($hdd_volver))
{
	...
	*/
	$obBD_ins1->operacionobBD(35, $Vet_Num.'*'.$Vet_Obs.'*'.$Vet_Des.'*'.$Ret_Fec.'*'.$Num_Ret.'*'.$Num_Aut.'*'.'*'.$codigo, $obBD_conexion);	
	/** 
			...
			if (count($row_rs_adquisicio)<>0)
			{
				...
				*/					
				...
				if($rs_infoTipCom['Tic_Sri']!='00') 
                {
					...
					* Actualiza el kardex 
					*/	
					$obBD_ins1->operacionobBD(1035, $codigo.'*'.'0'.'*'.'0'.'*'.'0'.'*'.$param[0].'*'.$hoy.'*'.$hora.'*'.'0'.'*'.$param[2].'*'.$param[4].'*'.'0'.'*'.$param[5].'*'.'0'.'*'.$desc_var.'*'.$param[8].'*'.$Kar_Int, $obBD_conexion);
       ...
       <td><fieldset>
         ...
         <table border="1" cellpadding="0" cellspacing="0">
           <tr>
             ...
             <td width="229" class="Cuerpo_ajax" align="center"><strong>La Venta mantiene pagos vigentes </strong></td>
           </tr>
         </table>
       </fieldset></td>
     </tr>
	...
	*/
	...
	?>
		  ...
		  <td width="14%" class="Etiqueta1">Fecha:</td>
                  <td width="19%" class="LetraNegra"><input type="hidden" name="Fec_Fact" value="<?Php echo $rs_cliente[0]['Caj_Fec']; ?>" />&nbsp;<?Php echo $rs_cliente[0]['Caj_Fec']; ?></td>
	...
	*/
	...
	<tr>
	  ...
	  <td>
		  	...
		  	value="<?Php echo $row_rs_cliente['Ite_Lar']?>" size="45" maxlength="25" readonly="true">	  </td>
	  <td align="center">
	  <?Php 
	  /** 
...
<input name="datos[<?Php echo $fila; ?>,13]" type="hidden" id="datos[<?Php echo $fila; ?>,13]" value="<?Php 
      ...
      <td><span class="LetraNegra">
		...
		/* Consulta los c&oacute;digos de retenci�n  */	
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" >
        ...
        <tr>
          ...
          <td class="Etiqueta1"><div align="left">
            <input name="Num_Aut" id="Num_Aut" type="text"  size="35" maxlength="49" value="<?php echo $row_rs_cliente['Ret_Aut'];?>"  align="right" />
          </div></td>
        </tr>
        <tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
      <td width="8%" class="Etiqueta1" valign="top"><div align="right"><span class="Asterisco">* </span>Forma:</div></td>
    <td width="14%" class="LetraNegra" valign="top">
      <?Php 
	  /**
	...
	*/
    ...
    <td width="78%" align="left">
                  ...
                  <tr>
                    ...
                    <td><input readOnly name="Cpp_Ven" type="text" id="Cpp_Ven" size="10" onKeyUp="mascara(this,'-',patron,true)" onBlur="validar_fecha2(this)" value="<?php  if(count($rs_pago_fact)>0) echo $rs_pago_fact['Cpc_Ven']; ?>" /></td>
                  </tr>
                  <tr>
                    <td class="Etiqueta1">Observaci&oacute;n:&nbsp;</td>
                    <td><textarea readOnly name="Cpp_Obs" cols="28" rows="3" id="Cpp_Obs" style="text-transform:uppercase"><?php  if(count($rs_pago_fact)>0) echo $rs_pago_fact['Cpc_Obs']; ?></textarea></td>
                  </tr>
                </table>              
...
<table width="303" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td width="106">
    <button type="button" class="btn btn-inverse fileinput-button" title="Atr�s" onClick="campos_hide(this.form, '<?Php echo "txt_busqueda*op_opciones*hdd_volver*cmb_anio*cmb_mes*Tic_Cod"; ?>', '<?Php echo $volver_busqueda.'*'.$volver_op.'*1*'.$volver_anio.'*'.$volver_mes.'*'.$volver_Tic_Cod; ?>')">
