        ...
        if ($For_Cod * 1 == 2) {
            ...
            //ESTABA A CONTADO
                ...
                if (!empty($codigoRetencion) && !is_null($codigoRetencion) && !empty($codigoCuenta) && !is_null($codigoCuenta)) {
                    ...
                    $Ret_Fec_Tmp = $codigoRetencion[0]['Ret_Fec'];
                    // Mismo mes que la compra → fecha retención; mes distinto → fecha compra
                    $Com_Fec_Ret = (substr($Ret_Fec_Tmp, 0, 7) === substr($Cop_Fec, 0, 7)) ? $Ret_Fec_Tmp : $Cop_Fec;
                    $Com_Num_Ret = $obBD_con1->getComNumPecAuto($Tia_Asi_Ret['Tia_Cod'], $comprobanteCompra['Pec_Cod'], $Com_Fec_Ret, $obBD_conexion);
                    $campo = 'Prv_Cod';

                    /* Cabecera del Comprobante */
                    $obBD_con1->operacionobBD(14, $comprobanteCompra['Pec_Cod'] . '*' . $comprobanteCompra['Prv_Cod'] . '*' . $Com_Num_Ret . '*' . $Com_Fec_Ret . '*' . trim($Com_Con_Ret) . '*' . $Tia_Asi_Ret['Tia_Cod'] . '*' . '0' . '*' . 'RETENCION' . '*' . $campo, $obBD_conexion);
                    ...
                    //Crear detalle 
                    $obBD_con1->operacionobBD(255, array('Com_Cod' => $Com_Cod_Ret, 'Pag_Cod' => 50, 'Pag_Fec' => $Com_Fec_Ret, 'Pag_Val' => $total['totalRetencion'], 'Pag_Obs' => "ABONO POR RETENCION", 'Cpp_Cod' => $Cpp_Cod, 'Asi_Cod' => $Asi_Cod_Ret), $obBD_conexion);
                } else {
...
if (isset($docDetalle)) {
    ...
    if (!empty($Cop_Cod)) {
        ...
        $resp['ppa_general'] = fac_ppa_enriquecer_items_asiento($obBD_conexion->conexion, $resp['items'], $Cop_Cod);
            ...
            if (($For_Cod * 1 == 2) && $Ret_Asu != 'S') {
                if ($Retencion && $Ret_Num > 0) {
                    ...
                    if (!empty($Com_Cod_Ret_Ant)) {
                        ...
                        if (substr($Com_Fec_Ret, 0, 7) !== substr($Com_Fec_Ret_Old, 0, 7)) {
                    ...
                    /* Cabecera del Comprobante */
                    $obBD_ins1->operacionobBD(14, $Pec_Cod . '*' . $Prv_Cod . '*' . $Com_Num_Ret . '*' . $Com_Fec_Ret . '*' . trim($Com_Con_Ret) . '*' . $Tia_Asi_Ret['Tia_Cod'] . '*' . $Ren_Tot . '*' . 'RETENCION' . '*' . $campo . '*' . $Com_Cod_Ret, $obBD_conexionIns);
    ...
    if ($obBD_ins1->Error == 0) {
        ...
        if (!empty($Com_Cod_Ret)) {
            $responce['Com_Data_Ret'] = array(
                ...
                'Com_Fec_Ret' => (isset($Com_Fec_Ret) ? $Com_Fec_Ret : $Ret_Fec),
                'Com_Val_Ret' => $Ren_Tot
            );
...
<body>
                ...
                <script>
                    ...
                    function editDocument(doc) {
                        ...
                        if (!$.varValid(doc['Ret_Cod']) || doc['Ret_Cod'] === '') {
                            doc['Ret_Num'] = '';
                            doc['Ret_Fec'] = (typeof window.fecRetencionServidor === 'function' ? window.fecRetencionServidor() : '') || '<?php echo $hoy; ?>';
                            doc['Aut_Cod'] = '';
                        }
                        ...
                        $.getDataJson('', {
                            ...
                            if (typeof window.aplicarRubroPresupuestoGeneral === 'function') {
                                window.aplicarRubroPresupuestoGeneral(resp['ppa_general']);
                            }
                        ...
                        if (window.setAutCodTip) setAutCodTip(doc['Aut_Cod']); else $('#Aut_Cod').html(doc['Aut_Cod'] || '');
    ...
    <script>
        ...
        function selectAut(auto) {
            ...
            if (window.setAutCodTip) setAutCodTip(auto['Aut_Cod']); else $('#Aut_Cod').html(auto['Aut_Cod'] || '');
            validaRetNum(true);
