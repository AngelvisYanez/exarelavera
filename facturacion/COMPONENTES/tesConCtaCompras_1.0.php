  ...
  <tr>
    <td width="100%"  valign="top" colspan="2"><input type="hidden" name="Hdd_Pld_Cod" id="Hdd_Pld_Cod"/>
      ...
      <table width="96%" height="10"  border="0" cellpadding="0" cellspacing="0">
        <tbody id="tbusqueda">
          <tr>
            <td width="72%" class="BarraBusqueda"><strong>Producto:</strong>
              <input name="txtbuscadorcom" type="text" id="txtbuscadorcom" size="40" maxlength="40" onkeypress="
                         enter_ajax('../COMPONENTES/tesComRubrosConsultaCompras_1.0.php?iva_cod=<?Php echo  urlencode($iva_cod); ?>&amp;iva_por=<?Php echo  urlencode($iva_por); ?>&amp;ice_cod=<?Php echo  urlencode($ice_cod); ?>&amp;Ses_Suc_Cod=<?Php echo $Ses_Suc_Cod; ?>&amp;ice_por=<?Php echo  urlencode($ice_por); ?>&amp;Pec_Cod=<?Php echo  urlencode($Pec_Cod); ?>&amp;buscador='+document.getElementById('txtbuscadorcom').value,'con_resultado'); " /></td>
            <td width="28%" align="center">
              <button type="button" name="btn_buscarcta" id="btn_buscarcta" class="btn btn-success fileinput-button" title="Buscar producto" 
        onclick="if (trim(document.getElementById('txtbuscadorcom').value) != ''){   ajax_datos('../COMPONENTES/tesComRubrosConsultaCompras_1.0.php?iva_cod=<?Php echo  urlencode($iva_cod); ?>&amp;Ses_Suc_Cod=<?Php echo $Ses_Suc_Cod; ?>&amp;iva_por=<?Php echo  urlencode($iva_por); ?>&amp;ice_cod=<?Php echo  urlencode($ice_cod); ?>&amp;ice_por=<?Php echo  urlencode($ice_por); ?>&amp;Pec_Cod=<?Php echo  urlencode($Pec_Cod); ?>&amp;buscador='+ document.getElementById('txtbuscadorcom').value,'con_resultado'); }">
