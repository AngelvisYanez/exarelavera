...
{		
	...
	*/
	$rs_buscta = $obBD_con1->getArrayConsulta(1281, strtoupper(trim($busqueda)).'*'.$Ses_Emp_Cod.'*'.$Pec,$obBD_conexion);
...
<?php
	...
	if (count($rs_buscta) > 0) 
	{
		...
		{ 
			...
			nuevaFila_ventaManual('c_contenido','<?php echo $row_rs_buscta['Pro_Cod'];?>','<?php echo $row_rs_buscta['Ite_Lar'];?>','<?php if ($row_rs_buscta['Pre_Pvp'] > 0){ echo $row_rs_buscta['Pre_Pvp']; } ?>','<?php echo $row_rs_buscta['Iva_Por'];?>','<?php echo $row_rs_buscta['Iva_Cod'];?>','<?php echo $codigonota; ?>','','','<?php echo $bloc_cant; ?>','si','0','0','0','<?Php echo $row_rs_buscta['Pro_Uni']; ?>'); asignar_total_fac();">
