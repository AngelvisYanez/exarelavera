<?php
/** 
* Descripci�n: Componente que permite elegir la fecha en meses y tambien en un determinado 
				rango de fechas 
* Fecha de actualizaci�n:	2009-09-09
* Desarrollador:	Lewis Chimarro
* Fecha de actualizaci�n:	2012-08-20
* Desarrollador:	Lewis Chimarro
*/
/**
...
if (isset($For_Cod))
{
	...
	*/
	if (isset($Hdd_Valor))
	{
	  ...
	  <?Php 
	  		/**
			* Consulto la informacion del cheke Cop_Bus=> es el c�digo interno de la fatura de compra
			*/ 
		...
		*/
		...
		<?Php
			/**
			* Consultar la informaci�n del cheque 
			*/
			...
			/**
			* Fin consultar la informaci�n del cheque 
			*/	
...
<input name="quitar_fila" id="quitar_fila" type="button" class="BotonEliminar" onclick="quitar_fila_mod(this)" value="X"  />
	<?Php //} ?>
	</td> 
  ...
  ?>          
<?php
    if($num_row_rs_informacion_cheque==0 && isset($contado) && $contado=true ){            
        $row_rs_bancos = $obBD_con1->getArrayConsulta(1098, '1'.'*'.$Ses_Emp_Cod, $obBD_conexion);
        if(count($row_rs_bancos)>0){
            $num_row_rs_informacion_cheque=1;
    ?>    
            <tr>
                <td>
                    <select name="datos_ch[1,3]" id="datos_ch[1,3]" align="left" style="width:100%">
                        <?php if(count($row_rs_bancos)>1) echo '<option value="NULL">' ?>   
                         <?php 
                        foreach ($row_rs_bancos as $row)
                        {?>
                            <option value="*<?php echo $row['Pld_Cod']?>*C" ><?php echo $row['Pld_Des']?></option>
                        <?php 
                ...
                <td>
                    <input name="datos_ch[1,4]" id="datos_ch[1,4]" size="10" type="text" maxlength="10" class="" style="text-align: right;">
                </td>
                <td>
                    <input name="datos_ch[1,5]" id="datos_ch[1,5]" size="10" type="text" maxlength="10" class="pag_contado" style="text-align: right;" onblur="numerico(this);" onkeyup="cal_total_cheques(5, 'nfilas_ch', 'datos_ch')" value="">
                </td>
                <td>
                    <input name="datos_ch[1,6]" id="datos_ch[1,6]" size="7" type="text" maxlength="7" class="" style="text-align: left;" value="<?php echo $hoy; ?>">
                </td>
                <td>
                    <input name="datos_ch[1,8]" id="datos_ch[1,8]" size="35" type="text" maxlength="35" class="" style="text-align: left;" value="Pago al Contado">
                    <input name="datos_ch[1,9]" id="datos_ch[1,9]" size="10" type="hidden" maxlength="10" class="" value="1" style="text-align: left;">
                </td>
                <td>
                    <input name="quitar_fila" id="quitar_fila" value="X" type="button" class="BotonEliminar" onclick="quitar_fila_mod(this)" >
                </td>
            </tr>
    <?php
        }
...
?>
			  ...
			  <tr>
				...
				<td><input name="txt_total" type="text" id="txt_total" size="10" readonly="true" style="text-align:right" value="<?php echo $valor; ?>"></td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
			  </tr>
			...
			<table width="607" border="0" cellpadding="0" cellspacing="0">
			  <tr>
				<td width="111" align="left"><input id="nfilas_ch" name="nfilas_ch" type="hidden" value="<?php echo $num_row_rs_informacion_cheque; ?>" />
            ...
            <thead>
             ...
             <tbody>
			  ...
			  <tr class="Fondo">
				...
				<td align="center">
                <button type="button" class="btn btn-success btn-mini" title="Elegir" onclick="nueva_fila_cheque_com('Tbl_Cheques', <?php echo $ban_cod; ?>,<?php echo $ban_des; ?>, '<?Php echo $Hdd_Fecha; ?>', '<?Php echo $Hdd_Valor; ?>'); cal_total_cheques(5, 'nfilas_ch', 'datos_ch')">
...
<?Php
		...
		{
			 echo error_alerta("<< Error de componente: tes_com_cheques.php >> <br>Descripci�n: No se ha definido la Propiedad: Hdd_Fecha<br>
	...
	{
		 echo error_alerta("<< Error de componente: tes_com_cheques.php >> <br>Descripci�n: No se ha definido la Propiedad: Hdd_Valor<br>
								Hdd_Valor: Variable que contiene el nombre del texto que posse el valor del documento", 2); 
	}
}//Fin del if (isset($For_Cod))
else
{
	 echo error_alerta("<< Error de componente: tes_com_cheques.php >> <br>Descripci�n: No se ha definido la Propiedad: For_Cod<br>
