...
if(isset($Com_Pec_Cod))
{ 
	if(isset($Pla_Cod))
	{ 
?>
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="Tbl_Rentas">
  <tr>
    <td>
	<FIELDSET>
      <LEGEND>
        <label class="Titulos2">B&uacute;squeda de Renta-Iva: </label>
      </LEGEND>
      ...
      <table width="98%" height="36" border="0" cellpadding="0" cellspacing="0">
        ...
        <button type="button" name="btn_buscarcta" id="btn_buscarcta" class="btn btn-success fileinput-button" title="Buscar codigo S.R.I." 
        onClick="if(trim(document.getElementById('busrta').value) != ''){ ajax_datos('<?Php echo $_SERVER['PHP_SELF'];?>?ajax_renta_iva_buscar='+document.getElementById('busrta').value+'&Adq='+document.getElementById('datos['+  document.getElementById('Hdd_Txt_Ide').value +','+ 16 +']').value+'&Tipo_Rta='+document.getElementById('Hdd_Tip_Rta').value+'&Pec_Cod=<?Php echo $Com_Pec_Cod; ?>&Pla_Cod=<?Php echo $Pla_Cod; ?>&ajax_op='+document.getElementById('Hdd_Op').value+'&siCont=<?Php echo $llevarContabilidad['Cof_Con']; ?>','busqueda_renta') }else{
  ...
  <table width="100%" border="0">
    <tr>
      <td><br /><div id="busqueda_renta"></div></td>
    </tr>
  </table>
