...
$pdf->Text(128,25,mb_convert_encoding("NOTA DE CRÉDITO", 'ISO-8859-1', 'UTF-8'));
...
$pdf->SetFont('Arial','',8);
$pdf->Text(70,88,$datoXml->infoNotaCredito[0]->contribuyenteEspecial);

$pdf->SetFont('Arial','B',8);
$pdf->Text(13,93,mb_convert_encoding("OBLIGADO A LLEVAR CONTABILIDAD:", 'ISO-8859-1', 'UTF-8'));
$pdf->SetFont('Arial','',8);
$pdf->Text(70,93,$datoXml->infoNotaCredito[0]->obligadoContabilidad);
...
$pdf->SetFont('Arial','',8);
$pdf->Text(75,103,mb_convert_encoding($datoXml->infoNotaCredito[0]->razonSocialComprador, 'ISO-8859-1', 'UTF-8'));

$pdf->SetFont('Arial','B',8);
$pdf->Text(145,103,mb_convert_encoding("RUC / CI:", 'ISO-8859-1', 'UTF-8'));
$pdf->SetFont('Arial','',8);
$pdf->Text(160,103,$datoXml->infoNotaCredito[0]->identificacionComprador);

$pdf->SetFont('Arial','B',8);
$pdf->Text(13,108,mb_convert_encoding("FECHA DE EMISIÓN:", 'ISO-8859-1', 'UTF-8'));
$pdf->SetFont('Arial','',8);
$pdf->Text(75,108,$datoXml->infoNotaCredito[0]->fechaEmision);

//(inicio X, inicio Y,fin X, fin Y) 
$pdf->Line(13,110,200,110);

$pdf->SetFont('Arial','B',8);
$pdf->Text(13,115,mb_convert_encoding("COMPROBANTE QUE SE MODIFICA:", 'ISO-8859-1', 'UTF-8'));
$pdf->SetFont('Arial','',8);
$pdf->Text(80,115,'FACTURA   '.$datoXml->infoNotaCredito[0]->numDocModificado);

$pdf->SetFont('Arial','B',8);
$pdf->Text(13,120,mb_convert_encoding("FECHA DE EMISIÓN (Comprobante a modificar):", 'ISO-8859-1', 'UTF-8'));
$pdf->SetFont('Arial','',8);
$pdf->Text(80,120,$datoXml->infoNotaCredito[0]->fechaEmisionDocSustento);

$pdf->SetFont('Arial','B',8);
$pdf->Text(13,125,mb_convert_encoding("RAZON DE MODIFCACIÓN:", 'ISO-8859-1', 'UTF-8'));
$pdf->SetFont('Arial','',8);
$pdf->Text(80,125,$datoXml->infoNotaCredito[0]->motivo);
...
$pdf->AliasNbPages();
$pdf->SetY(130); // mueve hacia abajo la tabla de los productos
//Cabecera    	
$pdf->SetFont('Arial','B',8);
...
for($x=0;$x<=$totDet-1;$x++)
{
	...
	$pdf->Cell(15,5,$datoXml->detalles[0]->detalle[$x]->codigoInterno,1,0,'C');
...
$imp= count($datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto);
...
for($x=0;$x<=2;$x++)
{
	if ($datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->codigo=="2") /* IVA */
	{
		if ($datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->codigoPorcentaje=="0")  /* IVA 0%*/
		{
			$baseImp0=(float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->baseImponible;
			$valorImp0=(float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->valor;			
		}
		if ($datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->codigoPorcentaje=="2") /* IVA 12%*/
		{
			$baseImp12=(float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->baseImponible;
			$valorImp12=(float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->valor;			
		}
		if ($datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->codigoPorcentaje=="6") /* No Objeto de Impuesto */
		{
			$baseNoObjeto=(float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->baseImponible;
			$valorNoObjeto=(float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->valor;			
		}
		if ($datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->codigoPorcentaje=="7") /* Exento de IVA */
		{
			$baseExento=(float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->baseImponible;
			$valorExento=(float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->valor;			
		}	
	}
	if ($datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->codigo=="3") /* ICE */
	{
		$valorIce= $valorIce + (float)$datoXml->infoNotaCredito[0]->totalConImpuestos->totalImpuesto[$x]->valor; /* SUMA TODOS LOS ICE */
	}	
}
...
$pdf->SetFont('Arial','',8);
$pdf->Cell(25,5,number_format((float)$datoXml->infoNotaCredito[0]->totalSinImpuestos,2),1,0,'R');
$pdf->Ln();
$pdf->Cell(129);
$pdf->SetFont('Arial','B',8);
$pdf->Cell(40,5,"DESCUENTO",1);
$pdf->SetFont('Arial','',8);
$pdf->Cell(25,5,number_format((float)$datoXml->infoNotaCredito[0]->totalDescuento,2),1,0,'R');
$pdf->Ln();
$pdf->Cell(129);
$pdf->SetFont('Arial','B',8);
$pdf->Cell(40,5,"ICE",1);
...
$pdf->Ln();
$total=((float)$datoXml->infoNotaCredito[0]->totalSinImpuestos - (float)$datoXml->infoNotaCredito[0]->totalDescuento)+ (float)$valorIce + (float)$valorImp12;
$pdf->Cell(129);
$pdf->SetFont('Arial','B',8);
