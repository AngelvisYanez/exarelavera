...
* Fecha de actualizaci�n: 2012-11-21
* Desarrollador: Lewis Chimarro
*/
/**
...
if ($car > 0)
{
	...
	*/
	...
	<tr>
	  ...
	  ?>
	  <tr>
		<td colspan="3"><?Php echo error_alerta("�El estudiante no posee una Matr�cula Activa!", 1); ?></td>
	  </tr>	
	  <?Php
	  }
	  ?>
...
?>
	<table width="558" border="0" cellpadding="0" cellspacing="0">
        ...
        {  
        	ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?buscod&busqueda='+buscta.value+'&Sem_Cod=<?Php echo $row_rs_semestre['Sem_Cod']; ?>&codigo=<?Php echo $codigo; ?>&Per_Fea=<?Php echo $row_rs_semestre['Per_Fea']?>&Per_Fef=<?Php echo $row_rs_semestre['Per_Fef']?>&codigonota=<?Php echo $row_rs_semestre['Nge_Cod']?>&bloc_cant=<?Php echo $bloc_cant + 0; ?>&Pec=<?php echo $Pec; ?>', 'rubros');
		}">
			</td>			
			<td width="122" align="center" height="36" class="BarraBusqueda">
            <div id="set1">
            <button type="button" class="btn btn-success btn-mini" title="Buscar itemaaaaa" onClick="if (trim(document.getElementById('buscta').value) != ''){  ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?buscod&busqueda=' + buscta.value + '&Sem_Cod=<?Php echo $row_rs_semestre['Sem_Cod']; ?>&codigo=<?Php echo $codigo; ?>&Per_Fea=<?Php echo $row_rs_semestre['Per_Fea']?>&Per_Fef=<?Php echo $row_rs_semestre['Per_Fef']?>&codigonota=<?Php echo $row_rs_semestre['Nge_Cod']?>&bloc_cant=<?Php echo $bloc_cant + 0; ?>&Pec=<?php echo $Pec; ?>', 'rubros'); }else{ alert('El dato de este campo es requerido'); document.getElementById('buscta').focus(); } "> <i class="icon-search icon-white"></i> <span>Buscar</span> </button>
            </div>
			</td>
</tr>	  
	</tbody>
