...
include '../COMPONENTES/tesPdfGlobal.php';

$verRet=$Xml[key($Xml)]["@attributes"]['version'];
...
$pdf->setField(70,82, issetText($infoCompRet,'contribuyenteEspecial','NO') ,'',8);
$pdf->setField(70,86, issetText($infoCompRet,'obligadoContabilidad','NO') ,'',8);
...
if($verRet=='2.0.0'){
	foreach ($pdf->formatArray($datoXml['docsSustento']['docSustento']['retenciones']['retencion'])AS $i=>$impu){
		...
		$pdf->Row(array( 
			...
			selectDoc($datoXml['docsSustento']['docSustento']['codDocSustento']),isset($datoXml['docsSustento']['docSustento']['numDocSustento'])?$datoXml['docsSustento']['docSustento']['numDocSustento']:'-',isset($datoXml['docsSustento']['docSustento']['fechaEmisionDocSustento'])?$datoXml['docsSustento']['docSustento']['fechaEmisionDocSustento']:'-',$infoCompRet['periodoFiscal'],
			number_format((float)$impu['baseImponible'],2),
			($impu['codigo']=='1'?'RENTA'.'  '.$impu['codigoRetencion']:'IVA '.selectIvas($impu['porcentajeRetener'])),
...
if($verRet=='1.0.0'){
	foreach ($pdf->formatArray($datoXml['impuestos']['impuesto'])AS $i=>$impu){
		...
		$pdf->Row(array( 
			selectDoc($impu['codDocSustento']),isset($impu['numDocSustento'])?$impu['numDocSustento']:'-',isset($impu['fechaEmisionDocSustento'])?$impu['fechaEmisionDocSustento']:'-',$infoCompRet['periodoFiscal'],
			number_format((float)$impu['baseImponible'],2),
			($impu['codigo']=='1'?'RENTA'.'  '.$impu['codigoRetencion']:'IVA'),
			$impu['porcentajeRetener'],
			number_format((float)$impu['valorRetenido'],2)        
		) ,true,false);
	}
