...
<span style="top:<? echo $row?>px;left:10px;" class="flota"><? if ($row_rs_representante['Cli_Fac'] != ""){echo $row_rs_representante['Cli_Fac'];}else{ echo $row_rs_cliente['Prs_Ape'].' '.$row_rs_cliente['Prs_Nom'];}?></span>
<span style="top:<? echo $row+20?>px;left:10px;" class="flota"><? if ($row_rs_representante['Cli_Dir'] != ""){echo substr($row_rs_representante['Cli_Dir'],0,31);}else{echo substr($row_rs_cliente['Prs_Dir'],0,31);}?></span>
<span style="top:<? echo $row+41?>px;left:23px;" class="flota"><? if ($row_rs_representante['Cli_Fac'] != ""){echo $row_rs_representante['Cli_Ruf'];}else{echo $row_rs_cliente['Prs_Ced'];}?></span>
<span style="top:<? echo $row+68?>px;left:17px;" class="flota"><? echo $row_institucion['Ciu_Des'];?></span>
<span style="top:<? echo $row+53?>px;left:224px;" class="flota"><? echo $dia.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$mes.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$anio; ?></span>

<? $aux=230;
do{?>
	<span style="top:<? echo $aux; ?>px;left:0px;" class="flota"><? echo formato_numero($row_rs_cliente['Vet_Can'],1,1);?></span>
    <span style="top:<? echo $aux; ?>px;left:40px; width: 165px;overflow: hidden;white-space: nowrap;" class="flota"><? echo $row_rs_cliente['Ite_Lar'].' '.$row_rs_cliente['Pro_Obs'];?></span>
    <span style="top:<? echo $aux; ?>px;left:185px;" class="flota rigth"><? echo number_format($row_rs_cliente['Vet_Pru'], 2);?></span>
...
<? $posTot='410';?>
<span style="top:<? echo $posTot;?>px;left:255px;" class="flota rigth"><!--Subtotal&nbsp;--><?Php echo formato_numero($resultados[0], 2, 1); ?></span>
<span style="top:<? echo $posTot+23;?>px;left:255px;" class="flota rigth"><!--Descuento&nbsp;--><?Php echo formato_numero($resultados[4], 2, 1); ?></span>
<span style="top:<? echo $posTot+43;?>px;left:255px;" class="flota rigth"><!--Tarifa 0%--><?Php echo formato_numero($resultados[1]+0, 2, 1); ?></span>
<span style="top:<? echo $posTot+65;?>px;left:255px;" class="flota rigth"><!--IVA&nbsp;--><?Php echo formato_numero($resultados[3], 2, 1); ?></span>
<!--<span style="top:<? //echo $posTot+100;?>px;left:270px;" class="flota"><!--Tarifa 12%&nbsp;--><?Php //echo formato_numero($resultados[2]+0, 2, 1); ?><!--</span>-->
<span aling="rigth" style="top:<? echo $posTot+90;?>px;left:255px;" class="flota rigth"><!--TOTAL&nbsp;--><strong><?php echo number_format($resultados[5], 2); ?></strong></span>
