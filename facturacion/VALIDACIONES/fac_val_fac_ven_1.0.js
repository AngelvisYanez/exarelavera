			...
			if (entrada1==true)/* Cabecera de la factura */
		    {
				if (entrada3==true)/* Cantidad ded rubros registrados */
				{
					if (entrada2==true) /* Datos en la cantidad y precio unitario */
					{
						...
						total = document.getElementById ('t_rubros'); /* Total del tipo de pago 2 */		
...
function verificar_prod_deuda(prod, nota, asig, rec)
{
	...
	{		
		...
		if ((var_prod))
		{
			dato = var_prod.value + var_nota.value + var_asig.value + var_recu.value;			
		}
...
{	
	if (verificar_prod_deuda(codigo, codigonota, codigoasi, codigorec, codigocnt, indice)==true)
	{
		...
		*/
		var Ite_Lar=create_input('datos['+ total.value +','+ 4 +']', 60, 60,'text',true,'left');
		var etiqueta = "";
		if (unidad>1)
		...
		* Cuadro que muestra el precio unitario
		*/
		var Vet_Pru=create_input('datos['+ total.value +','+ 5 +']', 14,8,'text',solo_lect_pre,'right');
		Vet_Pru.value = precio;
		...
		*/		
		var Pro_Uni=create_input('datos['+ total.value +','+ 15 +']', 4, 10,'text',true,'right');		
