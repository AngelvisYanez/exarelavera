	...
	$(function(){
		...
		pagos.createGrid({
			...
			colModel:[
				...
				{ label: '<i class="glyphicon glyphicon-remove"></i>', name: 'btn_remover', width: 20, align: 'center', formatter:'gridButton',
					formatoptions:{ action:deletePago, data:function(o){ return o.Vet_Num; },
						icon:'remove', type:'danger' }
				}
		...
		items.createGrid({
			...
			colModel:[
				...
				{name:'Vet_Dec',label:'Descuen.', labelLong:'Descuento', align:'right', width:20, editable:(edit_reten?false:true)},
		...
		items.jqGrid('footerData', 'set',{
			...
			'<label>TARIFA <span class="iva_por"></span>%:</label><label><span class="iva_por"></span>% IVA:</label>'+
			'<label>ICE:</label><label>DESCUENTO:</label><label class="total">TOTAL:</label></div>',
...
$.fn.fmatter.edicion=function(cv,opts,cObjt){
	if($.varValid(edicion_ventas)){
		...
		if(cObjt['Vet_Est']==='I') return '<i title="Registro Anulados/Inactivo" class="glyphicon glyphicon-remove red"></i>';
		if(cObjt['Vet_Est']==='U') return '<i title="Registro facturado" class="fa fa-money"></i>';
...
function updateDocument(){
	...
	var rows = items.jqGrid('getRowData'),des_val=$('#t_descuento').val(), des_por=$('#Vet_Des').val(), tot={t_subtotal:0,t_iva0:0, t_iva5: 0,t_iva12:0,t_iva:0,t_ice:0,t_descuento:(isNaN(des_val)?0:des_val*1),Vet_Des:(isNaN(des_por)||des_por*1===0?0:des_por*1),t_rubros:0},
	...
	for (var i=0, z=rows.length; i<z ; i++){
		...
		if (row['Pro_Cod']!=='') {
			...
			if(row['Iva_Por']===0||rise){ 
				tot['t_iva0']=tot['t_iva0']+row['Vet_Imp'];
			}
			else if (row['Iva_Por'] === 5 || rise) {
				tot['t_iva5'] = tot['t_iva5'] + row['Vet_Imp'];//5%
			}
			else { 
				tot['t_iva12']=tot['t_iva12']+row['Vet_Imp'];
			}
...
function calculaRetencion(){
	...
	$.each(rets,function(i,v){
		...
		tot[(v['Ren_Ret']==='R'?'Ret':'Iva')+'_Ren_Tot']+=$.round(rets[i]['Ren_Val']);
	});
...
function showIntutAut(event){
	...
	input_aut.toggle({'effect':'slide','complete':function(){
...
$(function(){
    ...
    if(reembolsos.length>0){
        reembolsos.createGrid({
            ...
            null,{caption:'Agregar Compra', buttonicon:'glyphicon glyphicon-plus', onClickButton: function(){
...
function RemoveCompra(Cop_Cod){
	reembolsos.delRowData(Cop_Cod); reembolsos.gridUpdate();
}
...
$(function(){
    ...
    if(viajesCods.length===1){
        ...
        viajesCods.createSearchDialog({ postExtra:{
