...
function cal_retencionVenta()
{
	...
	{
		var dato=document.getElementById ('datos['+ x +',6]');  //Columna importe
		if ((dato) && (dato.value.length > 0))
		{
			var iva12=0;
			var imp = document.getElementById ('datos['+ x +',6]');  //Columna importe
			var iva = document.getElementById ('datos['+ x +',8]');  //Columna IVA
			var renta_por = document.getElementById ('datos['+ x +',20]');  // hidden contiene % de renta
			var iva_por = document.getElementById ('datos['+ x +',21]');  // hidden contiene % de iva
...
function nuevaFila_guiaRemi(contenido,codigo,descripcion,precio,codigonota,codigorec,codigoasi,ccxcc,botonelim,cantrecur,codigocnt,indice,marca)
{	
	if (verifica_repetidos(codigo, codigonota, codigoasi, codigorec, codigocnt, indice)==true)
	{
		/**
		* Definición de variables para columnas
		*/
		...
		var columna3 = document.createElement("td");//Cantidad
		var columna4 = document.createElement("td");//Descripcion	
		var columna5 = document.createElement("td");//Marca			
		var columna6 = document.createElement("td");//Boton Eliminar		
		/**
		* Definición de variable para fila
		*/	
		var fila = document.createElement("tr");
		...
		*/
		...
		document.getElementById('tr_total').className='Cabecera1';
		...
		* Cuadro oculto que guarda el codigo del producto
		*/
		var Cod_Pro=create_input('datos['+ total.value +','+ 1 +']', 2, 2,'text',true,'left');
		columna2.appendChild(Cod_Pro);
		Cod_Pro.value = codigo;
		/**
		* Cuadro que muestra codigo manual del producto
		*/
		var Pro_Ide=create_input('datos['+ total.value +','+ 2 +']', 4, 2,'hidden',true,'left')
		Pro_Ide.value = 0;
		columna2.appendChild(Pro_Ide);
		...
		* Cuadro que muestra cantidad del producto
		*/
		var Vet_Can=create_input('datos['+ total.value +','+ 3 +']', 10, 8,'text',false,'center')
		Vet_Can.onblur= function(){numerico(this); contar_pro_guia();};	
		columna3.appendChild(Vet_Can);  		   
		   	
		/**
		* Cuadro que muestra la descripcion del producto 
		*/
		var Ite_Lar=create_input('datos['+ total.value +','+ 4 +']', 80, 60,'text',true,'left');
		var etiqueta = "";		
		Ite_Lar.value = descripcion + etiqueta;
		columna4.appendChild(Ite_Lar);		
				
		/**
		* Cuadro que muestra marca del producto
		*/
		var Mar_Des=create_input('datos['+ total.value +','+ 6 +']', 30, 8,'text',true,'left')		
		Mar_Des.value=marca;
		columna5.appendChild(Mar_Des);
		
		/**
		* Creación del botón de eliminación
		*/
		columna6.appendChild(create_button('quitar_fila['+ total.value +']','X',function(){ quitar_fila(this,cantrecur);}));						
...
function contar_pro_guia()
{
	var totPro=0;
	var total = document.getElementById('nfilas');
	for (var i=1;i<=total.value;i++)
	{
		if(document.getElementById('datos['+ i +','+ 3 +']').value!="")
		{
			totPro = totPro + parseInt(document.getElementById('datos['+ i +','+ 3 +']').value);	
		}
	}
	document.getElementById('total_pro').value=totPro;
}

/*
* Funcion que permite verificar que un producto no se ingrese nuevamente en el detalle
*/
function verifica_repetidos(prod, nota, asig, rec)
{	
    var total = document.getElementById('nfilas');
	var dato=0;
	for (var i=1;i<=total.value;i++)
	{			
		...
		if ((var_prod) && var_prod.value==prod)
		{
			dato = 1;			
		}		
	}	
