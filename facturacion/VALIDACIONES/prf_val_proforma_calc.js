...
$(function () {
    ...
    gestionarItem.createGrid({
        ...
        colModel: [
            { name: 'select', label: '<i class="glyphicon glyphicon-check"></i>', width: 10, align: 'center', viewable: false, formatter: 'gridButton', formatoptions: { action: openItemSelector, icon: 'check', title: 'Seleccionar Item', data: function (o) { return o.index; } }, resizable: false },
            ...
            { name: 'Vet_Can', label: 'Cant.', labelLong: 'Cantidad', width: 20, align: 'right', title: false, editable: (true), editoptions: { dataInit: styleCant } },
            { name: 'Uni_Des', label: 'Uni.', labelLong: 'Unidad', width: 25, resizable: false },
            { name: 'Ite_Lar', label: 'Descripci&oacute;n', width: 80 },
            { name: 'Prf_Adi', label: 'Des. Adicional', width: 75, editable: true },
            { name: 'Cod_Cor', label: 'Cod.Corte', labelLong: 'corte', align: 'right', width: 20, hidden: true },
            // Nuevos campos 
            { name: 'select1', label: '<i class="glyphicon glyphicon-check"></i>', labelLong: 'Descuento', align: 'right', width: 10, formatter: 'gridButton', formatoptions: { action: openTiopCorte, icon: 'check', title: 'Seleccionar Item', data: function (o) { return o.index; } }, resizable: false },
            { name: 'Tip_Corte', label: 'Servicio', labelLong: 'Servicio', align: 'right', width: 20, hidden: true },
          
            { name: 'Espesor', label: 'Espesor', labelLong: 'Espesor', align: 'center', width: 20 },

            { name: 'Prec_cm', label: 'costo', labelLong: 'Espesor', align: 'right', width: 20, hidden: true },
            { name: 'largo', label: 'Largo(m)', labelLong: 'Largo', align: 'right', width: 20, editable: true, editoptions: { dataInit: stylePru } },
            { name: 'ancho', label: 'Ancho(m)', labelLong: 'Ancho', align: 'right', width: 20, editable: true, editoptions: { dataInit: stylePru } },
            //Fin
            { name: 'Vet_Pru', label: 'P. Unitario', labelLong: 'Precio Unitario', width: 30, align: 'right', title: false /*, summaryRound: 8,formatter:"currency",formatoptions: {prefix:'', thousandsSeparator:',',decimalSeparator:'.',decimalPlaces: 8, defaultValue: ''}*/, editable: (true), editoptions: { dataInit: stylePru } },
            { name: 'Vet_Imp', label: 'Importe', width: 30, align: 'right', summaryRound: 2, formatter: 'currency', formatoptions: { prefix: '', thousandsSeparator: ',', decimalSeparator: '.', defaultValue: '0.00' }, classes: 'columnHighlight1' },
    ...
    container.createGrid({
        ...
        height: 300,
        colModel: [
            { name: "Prf_Cod", hidden: true, key: true },
            { label: 'Fecha', name: 'Prf_Fec', align: "center", width: 10 },
            { label: 'Cliente', name: 'Cliente', align: "center", width: 10 },
            { label: 'Número Proforma', name: 'Prf_Num', align: "center", width: 20 },
...
function updateRowItem(obj) {
    ...
    costo_servicio = (parseFloat(row['ancho']) + parseFloat(row['largo'])) * 100 * parseFloat(row['Prec_cm']);
    console.log("costo servicio" + costo_servicio);
    console.log("Precio venta" + parseFloat(row['Prec_cm']));
    total_corte = ((parseFloat(row['ancho']) * parseFloat(row['largo']) * parseFloat(row['Espesor']) * parseFloat(row['Vet_Pru']) * constante) + parseFloat(costo_servicio)) * parseFloat(row['Vet_Can']);
    row['Vet_Imp'] = total_corte;
...
// Abrir 
function openTiopCorte(data) {
    index = data;
    console.log(data);
    $('#codiForm').setData(data).formSubmit();
    $('#tipoCorteDialog').dialog('open');
}
...
function styleCant(e, obj, opt) {
    ...
    $(e).on('keyup', function () {
        if (isNaN(this.value)) {
            ...
            var dec = String(this.value).split('.');
            if (typeof dec[1] !== 'undefined' && dec[1].length > 2) this.value = $.toFixed(this.value);
        }
        updateRowItem(obj);
    });
}
...
if ($('#tipoCorteDialog').length > 0)
    $.createSearchDialog('tipoCorteDialog', [
        { label: 'C&oacute;d.Int.', name: 'Cod_Cor', key: true, width: 25, align: "center" },
        {
            label: 'Tipo',
            name: 'Tip_Corte',
            width: 100,
            ...
            formatter: function (cellValue, options, rowObject) {
                if ((rowObject.Tip_Corte === 'L')) {
                    return 'corte lineal oxicorte';
                } else
                    if (rowObject.Tip_Corte === 'R') {
                        return 'Corte redondo oxicorte'
                    };
            },
        },
        { label: 'Espesor', name: 'Espesor', width: 40, align: "center" },
        { label: 'Precio (cm)', name: 'Prec_cm', align: "center", width: 30 },
        { label: 'Estado', name: 'Est_Corte', width: 25, align: "center" },
        { label: '&nbsp;', name: 'act1', width: 30, align: 'center', viewable: false, formatter: 'gridButton', formatoptions: { action: selectTipoCorte } }
    ], null, null, null, null, { title: 'B&uacute;squeda', options: [] });


function selectTipoCorte(item) {
    var lastId = gestionarItem.jqGrid('getCol', 'index', false, 'max'), close = false;
    console.log(lastId);
    if (index === 0) {
        index = lastId;
        close = false;
    }
    var new_item = gestionarItem.changeRow(index,
        $.extend(
            item,
            item['Iva_Por'] * 1 > 0 ? {
                Cop_Ice: null
    ...
    var rowIds = gestionarItem.jqGrid('getDataIDs');
    var total_medidas = 0, total_cm = 0, subotal_corte = 0, cont = 0;
    for (let i = 0; i < rowIds.length; i++) {
        cont++;
        let rowId = rowIds[i];
        let dataRow = gestionarItem.jqGrid('getRowData', rowId);
        console.log(dataRow);
        // console.log("valor de i :" + i);
        let ancho = $('#' + cont + '_ancho').val();
        let largo = $('#' + cont + '_largo').val();
        let precio = $('#' + cont + '_Vet_Pru').val();
        // let precio_cm = $('#' + cont + '_Prec_cm').val();
        let cantidad = $('#' + cont + '_Vet_Can').val();
        consto_servicio = (parseFloat(ancho) + parseFloat(largo)) * 100 * dataRow.Prec_cm;
         subotal_corte = ((parseFloat(ancho) * parseFloat(largo) * constante * parseFloat(dataRow.Espesor) * parseFloat(precio)) + parseFloat(consto_servicio)) * parseFloat(cantidad);
        console.log("Total:::::" + subotal_corte);
        // Actualizar solo la columna importe_corte sin afectar las demás
        dataRow['Vet_Imp'] = subotal_corte;
        gestionarItem.jqGrid('setRowData', rowId, { Vet_Imp: subotal_corte });
        updateDocument();
        $('#tipoCorteDialog').dialog('close');
    }
}
...
function selectItem(item) {
    ...
    var new_item = gestionarItem.changeRow(index, $.extend(item, item['Iva_Por'] * 1 > 0 ? {
        Iva_Cod: $('#Iva_Cod').val(),
    ...
    gestionarItem.changeRow(index, new_item, null, {
        Vet_Pru: item['Pre_Pvp']
    });
...
function selectCliente2(cliente) {
    ...
    // setFormData('#clieFormTemp', $.extend(cliente, { op_opciones: 'c' }));
    $('#Prf_Nva').val(es_Nueva);
...
function searchNumPrf(cliente) {
    var d = new Date();
    $.getDataJson("", { numProformaCliente: true, Cli_Cod: cliente.Cli_Cod, Prs_Cod: cliente.Prs_Cod, Prs_Ced: cliente.Prs_Ced, Emp_Cod: cliente.Emp_Cod }, function (busquedaNum) {
        var valor = parseInt(busquedaNum.numeroPrf.total) + 1;
        var is_editable = $('#Prf_is_editar').val();
        if (is_editable != 'E') {
            $('input:text[name=Prf_Num]').val(valor);
        }
...
function validarFormProforma(editaDocument = false) {
    ...
    data['Prf_is_editar'] = $('#Prf_is_editar').val();
    data['Prf_Cod_edit'] = $('#Prf_Cod_edit').val();

    console.log("PRUEBAXXXXX");
    console.log(data);
...
function mensajeConNumeroProf(data) {
    ...
    if (data['Num_Vtas'] > 0 && data['Es_Mod'] > 0) {
        $.getDataJson("", { numProformaCliente: true, Cli_Cod: data['Cli_Cod'] }, function (busquedaNum) {
            var valor = parseInt(busquedaNum.numeroPrf.total) + 1;
...
function imprimirProforma(reporteTabla, campos) {
    ...
    campos['items'].forEach(function (valor) {
        if (valor['Pro_Cod'] > 0) {
            ...
            //console.log(total);
            var fila = "<tr><td>" + valor['Vet_Can'] + "</td><td  align=left>" + valor['Ite_Lar'] + " " + valor['Prf_Adi'] + "</td>      <td>" + valor['largo'] + "</td>   <td>" + valor['ancho'] + "</td>              <td>" + valor['Vet_Pru'] + "</td><td>" + valor['Vet_Imp'] + "</td><td align=right>" + valor['Vet_Imp'] + "</td></tr>";
    ...
    $.saveDataJson('', data,
        function (resp) {
            if (resp['success']) {
                ...
                $('#Prs_Dir').text('');
                $('input:text[name=Prf_Fec]').val($.datepicker.formatDate(shortDateFormat, new Date()));
                addItem({});
                setDefaultIva();
...
function verProforma(row) {
    ...
    var resultado = (isNaN(parseInt(row['Prf_Ord'])) ? '' : parseInt(row['Prf_Ord']));
    $('#clieFormTemp').setData($.extend(row, { op_opciones: 'd' }));
    $('input:text[name=Prf_Fec]').val(row['Prf_Fec']);
    $('input:text[name=Prf_Num]').val(row['Prf_Num']);
    $('input:text[name=Prf_Mod_Cod]').val(row['Prf_Cod']);
    // Cli_Cod
    $('#Prf_Cod_edit').val(row['Prf_Cod']);
    $('#Prf_is_editar').val('E');
    /** Crear para editar */
    $('#Prf_Num_Ext').val(resultado);
...
function obtenerDetalle(prf) {
    ...
    $('#items').clearGrid();
    $('#Es_Mod').val(stado);
    $('#Vet_Obs').val(prf['Prf_Obs']);
    $.getDataJson("", { detalleProformas: true, Prf_Cod: prf.Prf_Cod }, function (resuladoDetalle) {
        resuladoDetalle.detPrfms.forEach(function (valor) {
            console.log(valor);
            var next = $("#items").jqGrid('getCol', 'index', false, 'max');
            next = (isNaN(next) ? 1 : next + 1);
            $("#items").jqGrid('addRowData', next, $.extend(valor, {
                index: next,
                Vet_Can: valor['Prf_Cant'],
                Ite_Lar: valor['Ite_Lar'],
                largo: valor['Prf_Largo'],
                ancho: valor['Prf_Ancho'],
                Cod_Cor: valor['Cod_Cor'],
                Vet_Pru: valor['Prf_Pru'],
                Vet_Imp: parseFloat(valor['Prf_Imp']).toFixed(2)
            }), 'last');
            gestionarItem.jqGrid('editRow', next);
        });
