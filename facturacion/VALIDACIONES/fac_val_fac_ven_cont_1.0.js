...
{	
	...
	if(true)	
	{
		...
		*/
		...
		var columna12New = document.createElement("td");//Editar
		...
		*/
		...
		{ solo_lect_pre = false; }
		if(typeof ban_pru_disabled!=='undefined' && ban_pru_disabled===true){
			solo_lect_pre = true;
		}
		/**
		* Cuadro oculto que guarda el codigo del producto
		...
		*/
		var Vet_Can=create_input('datos['+ total.value +','+ 3 +']', 4, 8,'text',solo_lect,'right')
		Vet_Can.onblur= function() {numerico(this);valor_renta_compra(); positivo(this);};	
		columna3.appendChild(Vet_Can);	
		/**
		* Cuadro que muestra la descripcion del producto 
		*/
		...
		* Cuadro que muestra el precio unitario
		*/
		var Vet_Pru=create_input('datos['+ total.value +','+ 5 +']', 10,20,'text',solo_lect_pre,'right');
		Vet_Pru.value = precio;
		...
		*/
		...
		/**
		* Creación del botón de Editar Porcentaje
		*/
		var btn = document.createElement("BUTTON");
		btn.type='button';
		btn.name='Btn_RentaEdit['+total.value+']';
		btn.id ='Btn_RentaEdit['+total.value+']';
		btn.className='btn btn-primary btn-small btn-edit';
		var icon = document.createElement("I"); 
		icon.className = 'icon-pencil icon-white';
		...
		btn.onclick = function()
				{
					...
					//alert(indicerm[0]);
					document.getElementById('Itx_Producto').value =indicerm[0];
					document.getElementById('Des_Producto').innerHTML = document.getElementById('datos['+indicerm[0]+',4]').value;
					document.getElementById('Can_Producto').value = document.getElementById('datos['+indicerm[0]+',3]').value;
					document.getElementById('Pre_Producto').value = document.getElementById('datos['+indicerm[0]+',5]').value;
					document.getElementById('Imp_Producto').value = document.getElementById('datos['+indicerm[0]+',6]').value;
					document.getElementById('Desc_Producto').value = document.getElementById('datos['+indicerm[0]+',7]').value;
					document.getElementById('Ret_Producto').value = '';
					document.getElementById('Por_Producto').value = '';
										
					multiple_capa('',400,275,'cont_fon_338','cont_cua_338','Calculo Retencion 338','cont_cua_338_titu'); 
					calc_338();
					document.getElementById('Por_Producto').value = document.getElementById('datos['+indicerm[0]+',20]').value;
				};
				
		
		columna12New.appendChild(btn);	
		/**
		* Creación del botón de busqueda de renta.
		*/
		...
		*/
		cal_importe(Vet_Pru, Vet_Can, Vet_Imp, Pro_Uni);	
		valor_renta_compra(); positivo(this);
	}
	else
	{
		alert ("El item ya se encuentra ingresado!");	
...
/*
	Registra valores erroreos en el codigo 338 
*/
function calc_338()
{
		document.getElementById('Imp_Producto').value=('0'+document.getElementById('Can_Producto').value) * ('0'+document.getElementById('Pre_Producto').value);
		document.getElementById('Sub_Producto').value=(document.getElementById('Imp_Producto').value)-('0'+document.getElementById('Desc_Producto').value)*(document.getElementById('Imp_Producto').value)/100;
		var porcentaje=('0'+document.getElementById('Ret_Producto').value)*100/('0'+document.getElementById('Sub_Producto').value);
		if(isNaN(porcentaje)||!isFinite(porcentaje)) document.getElementById('Por_Producto').value='1';
		else document.getElementById('Por_Producto').value=roundToFour(porcentaje);
}
function change_338()
{
		//if(('0'+document.getElementById('Por_Producto').value)*1>=1&&('0'+document.getElementById('Por_Producto').value)*1<=2){
			var itx_prod=document.getElementById('Itx_Producto').value;
			document.getElementById('datos['+itx_prod+',20]').value=document.getElementById('Por_Producto').value;
			document.getElementById('datos['+itx_prod+',3]').value=document.getElementById('Can_Producto').value;
			document.getElementById('datos['+itx_prod+',5]').value=document.getElementById('Pre_Producto').value;
			close_338Modal();
			cal_importe(document.getElementById('datos['+itx_prod+',5]'),document.getElementById('datos['+itx_prod+',3]'),document.getElementById('datos['+itx_prod+',6]'),document.getElementById('datos['+itx_prod+',15]'));			
			asignar_total_fac();
			valor_renta_compra();
		//}else alert('El porcentaje es incorrecto!. Debe estar entre 1% y 2%');
}
function close_338Modal()
{
	document.getElementById("cont_fon_338").style.display="none"; 
	document.getElementById("cont_cua_338").style.display="none";
}
  		...
  		for (var j = 1; j <= filas.value; j++)
		{
			...
			{	
				if(document.getElementById('Hdd_Tip_Rta').value=='R')  
				{
					...
					if(ren_sri===Cod_Banano) /* Para poder editar la retencion de la caja de banano  */
						document.getElementById('Btn_RentaEdit['+j+']').style.display='block';
					else
						document.getElementById('Btn_RentaEdit['+j+']').style.display='none';
					valor_renta_compra();					
					valor_renta_compra();
				}else
		  		{
...
*/
function btn_edit_338(form, campo)
{		   
...
{				
	...
	*/
	...
	document.getElementById("datos["+ indicerm[0] +",18]").value = ''; 
	document.getElementById("datos["+ indicerm[0] +",20]").value = ''; 
	document.getElementById('Btn_RentaEdit['+indicerm[0]+']').style.display='none';
	/**
	* llamado a la función de cálculo de valores retenidos
	*/
	valor_renta_compra();
...
{  
	var filas = document.getElementById ('nfilas');	
	var VetDes = document.getElementById ('Vet_Des');	
	var sub_total=0;
    var TotRenta=0;
	var TotIva=0;
    if (isNaN(parseFloat(VetDes.value))){VetDes.value=0;}   
	for (var x = 1; x <= filas.value; x++)
	{
		...
		//alert(imp.value+'*'+renta_por.value+'/100');
		impDsc=(imp.value * VetDes.value)/100;
		/* Calculo de la retencion(renta) */
		TotRenta=TotRenta+(((imp.value-impDsc)*renta_por.value)/100);
		
		/* Calculo de la retencion(iva) */
		if(iva.value!="0")
		{
			iva12=((imp.value-impDsc)*iva.value)/100;
			TotIva=TotIva+((iva12*iva_por.value)/100);
		}
                //alert(TotIva);
	}	
	...
	document.getElementById ('Val_Pcc').value=redondear(document.getElementById ('t_rubros').value - document.getElementById ('Riv_Tot').value,2);
	document.getElementById ('Vet_Tot').value=document.getElementById ('Val_Pcc').value;  
}