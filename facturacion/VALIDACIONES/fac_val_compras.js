	...
	*/
	if (document.getElementById('activar1').checked == true){	
		...
		for (var j = 1; j <= filas.value; j++)
		{
			...
			desc = (t_subtotal.value * redondear(des.value,2)) / 100 ; //Descuento total de la tarifa 12, antes estaba tarifa_12, se corrigio
...
function nueva_fila_cheque_com(contenido,cmb_cod,cmb_des,fecha,valor)
{
		...
		var columna2 = document.createElement("td"); // N潞 Cheque
		...
		*/
		...
		/**
		* Creaci贸n del Combo que va a almacenar los bancos
		*/
		...
		*/
		...
		/**
		* Cuadro que guarda alguna observaci贸n del cheque
		*/
...
function validar_cheques_com(form)
{
	...
	for (var j = 1; j <= filas.value; j++)
	{
		...
		dato4 = document.getElementById ('datos_ch['+ j +','+ 6 +']'); //Fecha Elaboraci贸n
		dato5 = document.getElementById ('datos_ch['+ j +','+ 7 +']'); //Fecha Cobro
		dato6 = document.getElementById ('datos_ch['+ j +','+ 8 +']'); //Observaci贸n
	...
	/**
	* Validaci贸n del Totales de Cheques 
	*/
	...
	{ 			
		alert("El valor del (los) Cheque (s) debe ser igual al valor Total a Pagar.");
		entrada5 = false;
		return false;
	}//Fin del if ((dato4) && dato4.value =='NULL')		
...
{			
		...
		var columna7 = document.createElement("td");//Adquisi贸n
		...
		*/
		...
		/**
		* Creaci贸n del bot贸n de eliminaci贸n
		*/
	...
	&& requerido(form.Cop_Fec) != false && requerido(form.Cop_Imp) != false && requerido(form.Cop_Cad) != false && requerido(form.Ciu_Cod) != false )
		{			
			...
			*/
			...
			{
				alert("La fecha de emisi贸n de la factura de compra debe estar dentro del periodo " + Pec_Ann.value);
				form.Cop_Fec.focus();
				return false;
			}
			...
			alert("La fecha de impresi贸n debe ser menor a la fecha de emisi贸n");
			return false};

			if (comparafecha(form.Cop_Cad.value,form.Cop_Imp.value)){
			alert("La fecha de caducidad no debe ser menor a la fecha de impresi贸n");
			return false};

			if(variable=='S')
			{	
				/**
				* Control para validar que la fecha del comprobante sea mayor o igual a la fecha de emisi贸n 
				*/
				if(comparafecha(form.Com_Fec.value, form.Cop_Fec.value))
				{	alert("La fecha de registro del comprobante contable debe ser mayor o igual a la fecha de emisi贸n");
					form.Com_Fec.focus();
					return false;
				}
			}
			...
			{	dato = document.getElementById ('datos['+ j +','+ 1 +']');//Cantidad
				...
				dato7=document.getElementById ('datos['+ j +','+ 16 +']'); //Codigo de la adquisici贸n
				/**
				* Cantidad 
				*/
				if ((dato) && (dato.value ==""))
			  ...
			  if (Fec_Ven)
				...
				*/
				...
				{	if (form.Cop_Fec.value >= form.Cpp_Ven.value)
					{
						alert("La Fecha de Vencimiento debe ser MAYOR a la Fecha de Emisi贸n en Compras a Cr茅dito");
						form.Cpp_Ven.focus();
						return false;											
			...
			*/
			...
			{	
				if(requerido(form.Ret_Int) != false){	
					/**
					* Control para validar cheques Solo valida cuando la forma de pago es Contado y no se genera Retenci贸n 
					*/
...
function quitar_fila_compra_ice(fila)
{
	...
	*/
	...
	cal_tarifas(8,4); //Calculo de las tarifas
	cal_iva_importe('Cop_Des',7,5,4);//Calculo del iva importe
	...
	*/
	if (document.getElementById('activar1').checked == true){	
		...
		for (var j = 1; j <= filas.value; j++)
		{
			...
			if ((ice) && (ice.value > 0)){ //Solo entra cuando el iva sea mayor a 0
			...
			if (isNaN(des.value)) { des.value=0; } 
			desc = (tarifa_ice * des.value)/100;// Descuento total de la tarifa 12	
		...
		for (var j = 1; j <= filas.value; j++)
		{
			...
			if ((ice) && (ice.value.length > 0)){
				if (isNaN(ice.value)) { ice.value=0; } 
...
/**
* C谩lculo de total compra con I.C.E 
*/
function cal_total_com_ice()
{
	...
	{ 	
	     document.getElementById('pagoSri').style.display='block'; 
             ...
             document.getElementById('TipoPag').options[1].disabled ='true';
	}
        else{
	    document.getElementById('pagoSri').style.display='none';
            document.getElementById('hdd_TipoSri').value="1";
            document.getElementById('TipoPag').options[1].disabled ='false';
	}
}
...
/**
* Nueva fila compras con I.C.E. (Impuesto a los consumos especiales 
*/
//function nueva_fila_com_ice(contenido,iva,codigoiva,ice,ice_por,dq,dq_cod, pagina, cuenta, descripcion)
...
//  var columna8 = document.createElement("td");//Adquisi贸n
//  var columna9 = document.createElement("td");//Codigo cuenta
//  var columna10 = document.createElement("td");//Boton buscador de cuenta contable
//  var columna11 = document.createElement("td"); //Descripci贸n de la cuenta contable
//  var columna12 = document.createElement("td"); //Renta
//  var columna13 = document.createElement("td"); //Boton busqueda renta
//  var columna14 = document.createElement("td"); //Boton borrar renta
//  var columna15 = document.createElement("td"); //I.V.A
//  var columna16 = document.createElement("td"); //Boton busqueda I.V.A.
//  var columna17 = document.createElement("td"); //Boton borrar I.V.A.
//  var columna18 = document.createElement("td"); //Boton borrar cuenta
// 
//  var fila = document.createElement("tr");
//  var cuerpo = document.getElementById (contenido);
//  
...
//  * Cuadro que muestra cantidad del producto
//  */
//  var Cop_Can=create_input('datos['+ total.value +','+ 1 +']', 3, 2,'text',false, 'right')
//  Cop_Can.onblur= function() {numerico(this); }; 
//  columna1.appendChild(Cop_Can);
//  /**
//  * Cuadro que muestra la descripcion del producto en base a la busqueda hecha con el codigo
//  */
//  var Ite_Lar=create_input('datos['+ total.value +','+ 2 +']', 23, 35,'text',false,'left');
//  columna2.appendChild(Ite_Lar);
//  /**
//  * Cuadro que muestra el precio unitario
//  */
//  var Cop_Pru=create_input('datos['+ total.value +','+ 3 +']', 8,8,'text',false,'right');
//  Cop_Pru.onblur= function() {numerico(this); valor_renta_compra();};
//  columna3.appendChild(Cop_Pru);
//  var Cop_Imp=create_input('datos['+ total.value +','+ 4 +']', 8, 8,'text',true,'right');
//  Cop_Imp.onblur= function() {numerico(this) };
//  columna4.appendChild(Cop_Imp);
//  /**
//  * Cuadro que muestra el porcentaje de descuento invidividual 
...
//  { bool = true; }
//  else { bool = false }
//  var Cop_Dec=create_input('datos['+ total.value +','+ 5 +']',3,2,'text',bool,'right');  
...
//    
//  /**
//  * Cuadro que muestra el porcentaje del Ice
//  */
//  var Cop_Ice=create_combo('datos['+ total.value +','+ 9 +']',ice,ice_por,'right');  
//  //Cop_Ice.value = ice;
//  columna7.appendChild(Cop_Ice);   
//  /**
//  * C贸digo porcenatje I.C.E 
//  */
...
//  */
//  var Adq=create_combo('datos['+ total.value +','+ 16 +']',dq,dq_cod,'right');  
//  //Cop_Iva.value = iva; 
//  columna8.appendChild(Adq); 
//  var Adq_Des=create_input('datos['+ total.value +','+ 12 +']', 5, 5,'hidden',false,'left');
//  columna8.appendChild(Adq_Des); 
//
//  /**
//  * Cuadro que muestra el codigo de la busqueda
//  */
//  var Pld_Cdc=create_input('datos['+ total.value +','+ 13 +']',7,50,'text',false,'left');
//  /**
...
//  columna9.appendChild(Pld_Cdc); 
//  
//  /**
//  * C贸digo porcenatje iva
//  */
//  alert("hola");
//  var Iva_Por=create_input('datos['+ total.value +','+ 7 +']', 5, 5,'text',false,'right');
//  //Cop_Iva.value = iva;
//  columna10.appendChild(Iva_Por);
//  
//  var Pld_Cod=create_input('datos['+ total.value +','+ 15 +']',20,100,'hidden',true,'left'); 
//  //Pld_Cod.value = descripcion;
//  columna10.appendChild(Pld_Cod);
//  var Cop_Int=create_input('datos['+ total.value +','+ 8 +']', 5, 5,'hidden',false,'left');
//  Cop_Int.value = total.value;
//  columna11.appendChild(Cop_Int);
// 
//  /**
//  * Cuadro que muestra la descripcion de la cuenta en base a la busqueda hecha con el codigo
//  */
//  var Pld_Des=create_input('datos['+ total.value +','+ 14 +']',18,100,'text',true,'left');
//  //Pld_Des.value = descripcion;
//  columna11.appendChild(Pld_Des);
//
//  var Ren_Con=create_input('datos['+ total.value +','+ 17 +']', 3, 2,'text',true,'left');
//  columna12.appendChild(Ren_Con);
//  /**
//  * Creaci贸n del bot贸n de busqueda de renta.
//  */
...
//    /**
//	* llamado a la funci贸n que permite ocultar la tabla en caso que no hayan resultados visibles 
//	*/
//    xDisplay('Tbl_Rencon', 'none');
//    }  
//  columna13.appendChild(boton_renta_mas);
...
//     /**
//	 * llamado a la funci贸n de c谩lculo de valores retenidos
//	 */
//     valor_renta_compra();
//     /**
//	 * Color de enfoque de la cuenta 
...
//    }
//    
//  columna14.appendChild(boton_renta_menos);
//  var Iva_Con=create_input('datos['+ total.value +','+ 18 +']', 3, 2,'text',true,'left');
//  /**
//  * Creo los 2 campos necesarios para el envio de c贸digos de retenci贸n 
//  */
//  var Int_Rer=create_input('datos['+ total.value +','+ 19 +']',20,100,'hidden',true,'left');  /* C贸digo interno RENTA */
//  var Int_Rei=create_input('datos['+ total.value +','+ 20 +']',20,100,'hidden',true,'left');  /* C贸digo interno IVA */
//  var Int_Rpc=create_input('datos['+ total.value +','+ 21 +']',20,100,'hidden',false,'right');  /*Porcentaje retenci贸n */
//  var Int_Riv=create_input('datos['+ total.value +','+ 22 +']',20,100,'hidden',true,'right');  /*Porcentaje retenci贸n IVA */
//  columna15.appendChild(Iva_Con);
//  columna15.appendChild(Int_Rer);
//  columna15.appendChild(Int_Rei);
//  columna15.appendChild(Int_Rpc);
//  columna15.appendChild(Int_Riv);
//  /**
//  * Creaci贸n del bot贸n IVA 
//  */
...
//    xDisplay('Tbl_Rencon', 'none');
//    } 
//  columna16.appendChild(boton_iva_mas);
...
//    lostfocus_compras('datos', '', false); 
//   }
//  columna17.appendChild(boton_iva_menos);
//  /**
//  * Asignacion del evento al cuadro de cantidad
//  */
...
//  * Creaci贸n del bot贸n de eliminaci贸n
//  */
//  columna18.appendChild(create_button('quitar_fila','X',function(){ quitar_fila_compr_x(this); valor_renta_compra()}));
// 
//  /**
//  * Creaci贸n del bot贸n de eliminaci贸n
//  */
...
//  Cop_Iva.options[0].value ='NULL';
//  Cop_Iva.options[0].text = '-';
//  Cop_Ice.options[0].value =null;
//  Cop_Ice.options[0].text = '-';
//  Cop_Ice.options[0].value =null;
//  Cop_Ice.options[0].text = '-';
//  Adq.options[0].value ='NULL';
//  Adq.options[0].text = '-';
//}//Fin del function nueva_fila_com_ice(contenido,iva,codigoiva,ice,ice_por,dq,dq_cod, pagina)
...
/**
* Checkear comprobante de retenci贸n  
*/
...
/**
* Habilita para buscar porcentajes en la retenci贸n 
*/
...
{
		/**
		* asigno el valor del c贸digo de rentencion 
		*/
		document.getElementById('datos['+  document.getElementById('nfilas').value +','+ 17 +']').value="";
		/**
		* asigno el valor del c贸digo interno de la retenci贸n 
		*/
	...
	*/
	...
	{	
		...
		*/
		...
		/**
		* Llamado a la funci锟絥 que permite ocultar la tabla en caso que no hayan resultados visibles 
		*/
...
/**
* funci贸n limpia campos cuando seleccione otros tipos de adquisici贸n 
*/
...
{  
	...
	{ 
			...
			*/
			...
			/* Dismunic贸n de descuento individual al importe */	
...
/**
* funcion para validar la modificaci贸n de los clientes 
*/
			...
			if((valor_total.value==100.00) )
			{
				for(var j = 1; j <= c.value; j++)
				{
					...
					*/
					...
					/**
					* objeto que almacena el contenido de Descripci贸n de la cuenta)
					*/
					...
					*/
					...
					/**
					* llamado al al funci贸n que agrega nueva fila
					*/
					nueva_fila_com_ice(c_contenido, iva_cod, iva_por, ice_cod, ice_por, ad_cod, ad_por, url, '', '')
					/**
					* asignaci贸n de los objetos para realizar respectivos calculos
					*/
					/**
					* cantidad
					*/
			...
			{	
				if(document.getElementById('Hdd_Tip_Rta').value=='R')  
				{
					document.getElementById('datos['+ j +','+ 17 +']').value=ren_sri; /* Ren_Con */
 					document.getElementById('datos['+ j +','+ 19 +']').value=ren_cod; /* C贸digo interno RENTA */
					document.getElementById('datos['+ j +','+ 21 +']').value=ren_por; /* Porcentaje retenci贸n  */
					valor_renta_compra();
				}else
		  		{
					document.getElementById('datos['+ j +','+ 18 +']').value=ren_sri; /* Iva_Con */
					document.getElementById('datos['+ j +','+ 20 +']').value=ren_cod;  /* C贸digo interno IVA */
					document.getElementById('datos['+ j +','+ 22 +']').value=ren_por; /*Porcentaje retenci贸n IVA */
					valor_renta_compra();
			   	}
		    }
       }
...
function busca_iva_btn(form, campo)
{ 
	multiple_capa('',600,300,'cont_fon_iva','cont_cua_iva','busqueda de retenci贸n','cont_cua_iva_titu');
	/**
	* Asigna el valor del la fila 
	*/
	var indicei = campo.name.split("[");
...
*/
function nueva_fila_com_ice(contenido,iva,codigoiva,ice,ice_por,dq_des,dq_cod, pagina, cuenta, descripcion,codigopro)
{   	
		...
		var columna6 = document.createElement("td");//I.V.A.
		var columna7 = document.createElement("td");//I.C.E
		var columna8 = document.createElement("td");//Adquisi髇
		var columna9 = document.createElement("td");//Codigo cuenta
		var columna10 = document.createElement("td");//Boton buscador de cuenta contable
		var columna11 = document.createElement("td"); //Descripci髇 de la cuenta contable
		var columna12 = document.createElement("td"); //Renta
		var columna13 = document.createElement("td"); //Boton busqueda renta
		var columna14 = document.createElement("td"); //Boton borrar renta
		var columna15 = document.createElement("td"); //I.V.A
		...
		* Cuadro que muestra cantidad del producto
		*/
		var Cop_Can=create_input('datos['+ total.value +','+ 1 +']', 4, 10,'text',false, 'right')
		Cop_Can.onblur= function() {numerico(this); };	
		columna1.appendChild(Cop_Can);
		/**
		* Codigo del producto
		...
		*/
		var Pld_Des=create_input('datos['+ total.value +','+ 14 +']',15,100,'text',true,'left');
		//Pld_Des.value = descripcion;
		columna11.appendChild(Pld_Des);
		...
		*/
		...
		columna12.appendChild(Ren_Con);
	    /**
		* Creaci髇 del bot髇 de busqueda de renta.
		*/
		var boton_renta_mas=create_button('Btn_RentaMas['+total.value+']','+',' ');
		boton_renta_mas.onclick = function()
			{   
				...
				*/
				...
				document.getElementById('Hdd_Ren_Ide').value="datos["+ indicer[0] +",19]";
				document.getElementById('Hdd_Ren_Por').value="datos["+ indicer[0] +",21]";				
				multiple_capa('',600,300,'cont_fon_iva','cont_cua_iva','busqueda de retenci髇','cont_cua_iva_titu');
				/**
				* Llamado a la funci髇 que permite ocultar la tabla en caso que no hayan resultados visibles 
				*/
				xDisplay('Tbl_Rencon', 'none');
			}	 
				columna13.appendChild(boton_renta_mas);
				...
				{  	
					...
					*/
					...
					document.getElementById("datos["+ indicerm[0] +",21]").value = ''; 
					/**
					* llamado a la funci髇 de c醠culo de valores retenidos
					*/
					valor_renta_compra();
					/**
					* Color de enfoque de la cuenta 
				...
				*/
				...
				/**
				* Creo los 2 campos necesarios para el envio de c骴igos de retenci髇 
				*/
				var Int_Rer=create_input('datos['+ total.value +','+ 19 +']',20,100,'hidden',true,'left');  /* C骴igo interno RENTA */
				var Int_Rei=create_input('datos['+ total.value +','+ 20 +']',20,100,'hidden',true,'left');  /* C骴igo interno IVA */
				var Int_Rpc=create_input('datos['+ total.value +','+ 21 +']',20,100,'hidden',false,'right');  /*Porcentaje retenci髇 */
				var Int_Riv=create_input('datos['+ total.value +','+ 22 +']',20,100,'hidden',true,'right');  /*Porcentaje retenci髇 IVA */
				columna15.appendChild(Iva_Con);
				columna15.appendChild(Int_Rer);
				columna15.appendChild(Int_Rei);
				columna15.appendChild(Int_Rpc);
				...
				*/
				...
				/**
				* Creaci髇 del bot髇 de eliminaci髇
				*/
				columna18.appendChild(create_button('quitar_fila','X',function(){ quitar_fila_compr_x(this); valor_renta_compra()}));
				/**
				* Creaci髇 del bot髇 de eliminaci髇
...
/**
* Funci贸n que permite validar la caducidad de una factura de compra
*/
