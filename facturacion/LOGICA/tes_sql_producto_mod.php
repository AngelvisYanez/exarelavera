...
function sentencias_pro($id, $Par_Sql)
{
    ...
    switch ($id) {
        ...
        case 3:
            ...
            break;
        case 4:
            $sql = "SELECT Iva_Por, Iva_Sri, Iva_Cod FROM iva WHERE Iva_Est='A' ORDER BY Iva_Por";
            //echo $sql;
            break;
        case 5:
        ...
        case 10:
            ...
            break;
        case 11:
            if (isset($Par_Sql['Pro_Tip'])) $Par_Sql['Pro_Tip'] = "'$Par_Sql[Pro_Tip]'";
            else $Par_Sql['Pro_Tip'] = 'NULL';
        ...
        case 18: //Busqueda de Productos            
            $sql = "SELECT producto.Cod_Const,precios.Pre_Cod as precio_cod, precios.Pre_Pvp,Pro_Dsc, producto.Pro_Cod,producto.Pro_Bar,producto.Pro_Gen,iva.Iva_Por,
            producto.Iva_Cod,iva.Iva_Por,producto.Pre_Cod,producto.Pro_Obs,producto.Uni_Cod,producto.Ubi_Cod,
            IF(producto.Lin_Cod IS NULL,'NULL',producto.Lin_Cod) AS Lin_Cod,
            categorias.Cat_Cod,adquisicio.Adq_Cod,adquisicio.Adq_Des,marca.Mar_Cod,CONCAT(Lin_Abr,SUBSTRING_INDEX(Cat_Cdc,'.',-1),LPAD(Pro_Ide,5,'0')) AS Cha_Cod,
            producto.Ite_Cod,Ite_Lar,Ite_Cor,Pro_Obs,Cat_Des,Mar_Des,Uni_Des,
            IF(presentaci.Pre_Des IS NULL ,'NINGUNA',presentaci.Pre_Des) AS Pre_Des,Pro_Uni,Stk_Can,Stk_Prp,Ubi_Des,
            IF(Lin_Des IS NULL ,'NINGUNA',Lin_Des) AS Lin_Des, Pre_Pvp as Precio1 
            FROM producto
                ...
                WHERE Pro_Est='A' AND categorias.Emp_Cod=$Par_Sql[Emp_Cod] AND $Par_Sql[Filtros] GROUP by producto.Pro_Cod $Par_Sql[Order] LIMIT 0, 5000";
            //ChromePhp::log($sql);
            break;
        ...
        case 27:
            $sql = "UPDATE producto
                ...
                Uni_Cod=$Par_Sql[Uni_Cod], Pro_Bar='$Par_Sql[Pro_Bar]',Pro_Dsc='$Par_Sql[Pro_Dsc]',Pro_Gen='$Par_Sql[Pro_Gen]',Pre_Cod=$Par_Sql[Pre_Cod], Pro_Uni=$Par_Sql[Pro_Uni], Lin_Cod=$Par_Sql[Lin_Cod], Cod_Const='$Par_Sql[Cod_Const]' 
		...
		INNER JOIN empresas ON categorias.Emp_Cod = empresas.Emp_Cod
            WHERE cat.Cat_Est='A' AND categorias.Emp_Cod=$Par_Sql[0] " . (empty($Par_Sql[1]) ? "AND Cat_Tip='D'" : "AND Cat_Tip='$Par_Sql[1]'");
            //ChromePhp::log($sql);
            break;
