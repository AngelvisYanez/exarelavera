<?php
/**
 * Logica de las paginas de facturación ventas
 *
 * @author Lewis Chimarro
 * @version 1.0
 * Fecha de actualización:	2012-05-07
 *
 * @package tesoreria.LOGICA
 */

require_once('../../auditoria/LOGICA/aud_log_auditoria.php');
require_once ("fac_sql_fac_ven.php");
...
<?php
	...
	*/
	...
	function calculos($Vet_Cod, $obBD_conexion) {
		...
		$subtotal0 = 0; //0 = SUBTOTAL
		$tarifa_0=0;    //1 = TARIFA 0
		$tarifa_12=0;   //2 = TARIFA 12
		$iva_12=0;      //3 = IVA
		$des_0=0; $des_12=0; //4 = DESCUENTO
		$subtotal=0;    //5 = TOTAL */
		$mon_ing=0; 	//6 = MONTO INGRESADO
		$mont_cam=0; 	//7 = MONTO CAMBIO
		...
		foreach ($row_rs_calculos as $row) {
			...
			if ($row['Iva_Por'] == 0) {
				...
				/*Descuento individual */
				$des_12 = $des_12 + ($row['Vet_Pru'] * $row['Vet_Dec'])/100;			
				$iva_12 = $row['Iva_Por'];
			}
			// nuevos parametros de monto ingresado y cambio
			$mon_ing = $row['Vet_Mon']; //Monto ingresado
			$mont_cam = $row['Vet_Cam']; //Monto cambio
		...
		return $subtotal.'*'.$tarifa_0.'*'.$tarifa_12.'*'.$iva.'*'.$des.'*'.$total.'*'.$iva_12.'*'.$mon_ing.'*'.$mont_cam;		
	}	
	...
	* Calcula el total de las ventas 
	*/
	function calculosConsultaVentas($parametro, $tipo,$tipo_compr, $Pun_Cod, $obBD_conexion) {
		/* Opciones para el retorno 
		0 = SUBTOTAL
		1 = TARIFA 0
		2 = TARIFA 12
		...
		/* Consulta del total de las ventas */
		$rs_ventas = $this->getArrayConsulta(1239, $parametro.'*'.$tipo.'*'.$tipo_compr.'*'.$Pun_Cod,$obBD_conexion);
		...
		$des = 0;
		$total = 0;
		foreach($rs_ventas as $datos){
			if ($datos['Iva_Sri']!=0)//Solo valores con iva 12%, 14%....etc
			{							
				$subtotal =$subtotal + $datos['Importe'];
				$tarifa_12 =$tarifa_12 + $datos['Importe'];
				$iva =$iva + $datos['Iva'];
				$des = $des + $datos['Descuento'];
				$total =$total + $datos['Total'] + $datos['Iva'];																			
				/* En caso de existir 2 registros de total de ventas se mueve el apuntador de la tabla */
				$mover = true;					
			}//Fin del if ($datos['Iva_Sri'] == 2)
					
			if ($datos['Iva_Sri'] == 0)//Valores 0% iva
			{				
				$subtotal = $subtotal + $datos['Importe']; //Suma los dos importes de la consulta
				$tarifa_0 =$tarifa_0+ $datos['Importe'];
				$des = $des + $datos['Descuento'];
				$total = $total + $datos['Total'];
			}//Fin del if ($datos['Iva_Sri'] == 0)
		}
		return $subtotal.'*'.$tarifa_0.'*'.$tarifa_12.'*'.$iva.'*'.$des.'*'.$total;
	}//Fin del function calculos_ventas($ini, $fin)
	 ...
	 * @param int $Pec_Cod periodo contable
         * @param int $mes mes
	 */		
	function codigoComprAuto($Tia_Cod, $PeCod, $mese, $obBD_conexion) {			
		/* Codificación numerica en base al periodo contable y mensualmente */
		$row_rs_numcom = $this->getRowConsulta(1271, $Tia_Cod.'*'.$PeCod.'*'.$mese, $obBD_conexion);
