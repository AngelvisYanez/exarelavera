...
class Class_Log_Datos_Factu extends MysqlDatosContab
{
    ...
    {
        echo $this->getReportHeader($sucursal, $titulo, $subtitulo, $obBD);
    }
    ...
    function retencionElectronica($Emp_Cod, $Suc_Cod, $Prs_Cod, $data, $obBD_conexion, $Ret_Xml = NULL)
    {
        ...
        try {
            ...
            $rs_retInfoFact = $this->getArrayConsultaSql("SELECT 
                    retencion.Ret_Cod,if(det_retenc.Ret_Imp = 'R','1','2')as ImpCod,
					...
					ROUND(sum(((det_retenc.Ret_Bas*renta_iva.Ren_Por)/100)),2) as ValRet,
                    renta_iva.Ren_Por,
					Sum(det_retenc.Ret_Bas)as Ret_Bas,
					sustento.Tri_Sri,tipo_compr.Tic_Sri,compras.Cop_Num,CAST(replace(Cop_Obs,'\n',' ') as char(200))as obs,date_format(compras.Cop_Fec,'%d/%m/%Y') as Cop_Fec
                FROM renta_iva
                ...
                WHERE retencion.Ret_Cod = '$data[Ret_Cod]' AND retencion.Ret_Est = 'A' GROUP BY Ren_Sri", $obBD_conexion);
            foreach ($rs_retInfoFact as $Ret_Info) {
                ...
                $armado_xml .= "<" . $Eti_infoImps[0] . ">" . $Ret_Info['ImpCod'] . "</" . $Eti_infoImps[0] . ">" .         //<codigo>
                    ...
                    "<" . $Eti_infoImps[3] . ">" . formato_numero($Ret_Info['Ren_Por'], 2, 1) . "</" . $Eti_infoImps[3] . ">" . //<porcentajeRetener>
            ...
            if ($rs_infoCliente['Prs_Dir'] != "" or $rs_infoCliente['Prs_Tel'] != '' or $rs_infoCliente['Prs_Cor'] != '') {
                ...
                if ($Ret_Info['obs'] != '') {
                    $armado_xml .= "<" . $Eti_infoAdicional[0] . " nombre='Observacion'>" . $Ret_Info['obs'] . "</" . $Eti_infoAdicional[0] . ">";
                }
                $armado_xml .= "</" . $Eti_raiz[3] . ">";  //</infoAdicional>
            }
    ...
    //Clave de liquidacion de compras
    ...
    //finalizar liquidacion de compras
    ...
    function sendMailRet($data, $body)
    {
        $ban = true;
        $this->echoLog($data);
        if (empty($data['{Prs_Cor}']) || empty($body) || strlen($data['{Prs_Cor}']) < 4) return 0;
        try {
