    ...
    switch($id){
        ...
        case 2://Busqueda de Proveedores
            ...
            if($Par_Sql[3]==""){$campos="COUNT(Prv_Cod) as total";}
                ...
                $campos=" Prv_Cod, persona.Prs_Cod, Prs_Ced, CONCAT(Prs_Ape,' ',Prs_Nom) as proveedor, Prv_Fax, Prs_Dir, Prs_Cor, IF (Prv_Est='A','Activo','Inactivo') as Prv_Est, Prv_Con, Prv_Esp";
            }
        ...
        case 11:
            ...
            break;
        case 12:
            $sql = "INSERT INTO det_compra (Cop_Cod, Cop_Can, Iva_Cod, Cop_Pro, Cop_Pru, Cop_Imp, Cop_Dec, Adq_Cod, Ice_Int, Cop_Int, Pld_Cod,Pro_Cod,Iva_Cos,Cop_Ice) 
            VALUES($Par_Sql[Cop_Cod],$Par_Sql[Cop_Can],$Par_Sql[Iva_Cod],UPPER('".(empty($Par_Sql['Cop_Pro'])?(empty($Par_Sql['Ite_Lar'])?'':$Par_Sql['Ite_Lar']):$Par_Sql['Cop_Pro'])."'),$Par_Sql[Cop_Pru],$Par_Sql[Cop_Imp],".(!empty($Par_Sql['Cop_Dec'])?$Par_Sql['Cop_Dec']:0).",$Par_Sql[Adq_Cod],".(!empty($Par_Sql['Ice_Int'])?$Par_Sql['Ice_Int']:'NULL').", '$Par_Sql[Cop_Int]',".(!empty($Par_Sql['Pld_Cod'])?$Par_Sql['Pld_Cod']:'NULL').",$Par_Sql[Pro_Cod],'".(isset($Par_Sql['Iva_Cos'])&&$Par_Sql['Iva_Cos']=='S'?'S':'N')."',".(empty($Par_Sql['Cop_Ice'])?'NULL':$Par_Sql['Cop_Ice']).")";  
            //echo $sql."<br>";           
            break;
        case 13:
        ...
        case 30://Busqueda de Proveedores
            $sql="SELECT persona.*,proveedore.Prv_Cod,CONCAT(Prs_Ape,' ',Prs_Nom) as proveedor FROM persona  
        ...
        case 34:  
            ...
            break;
        case 35:
            $sql="SELECT Cop_Int,Cop_Int AS 'index', det_compra.Pro_Cod,det_compra.Ice_Int,det_compra.Cop_Ice,det_compra.Iva_Cod,Iva_Por,Iva_Sri,Cop_Pro AS Ite_Lar,Cop_Can,Cop_Pru,Cop_Imp,Cop_Dec,det_compra.Adq_Cod,Adq_Des,Adq_Cor,det_plan.Pld_Cod, Pld_Cdc,Pld_Des,Uni_Des,Iva_Cos FROM det_compra 
                INNER JOIN producto ON producto.Pro_Cod=det_compra.Pro_Cod
                INNER JOIN unidad ON producto.Uni_Cod=unidad.Uni_Cod
        ...
        case 50:
            $sql="SELECT COUNT(Ret_Cod)AS total FROM retencion 
                    ...
                    WHERE autorizaci.Aut_Sri='$Par_Sql[1]' AND Suc_Cod='$Par_Sql[0]' AND Ret_Num='$Par_Sql[2]'".(!empty($Par_Sql[3])?"AND retencion.Ret_Cod<>$Par_Sql[3]":'').';';
            //echo $sql.'<br/>';
            break;
        case 51:
            $sql="SELECT 
                ...
                WHERE Suc_Cod=$Par_Sql[0] AND autorizaci.Aut_Sri='$Par_Sql[1]' AND autorizaci.Tic_Cod=$Par_Sql[4] AND Ret_Num BETWEEN $Par_Sql[2] AND $Par_Sql[3]";            
            //echo $sql.'<br/>';
            break;
        case 52:
