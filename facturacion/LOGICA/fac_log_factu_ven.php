...
require_once("fac_sql_factu_ven.php");
...
class Class_Log_Datos_Factu extends MysqlDatosContab{	
    function __construct() {
        $this->setSentencias('sentencias_doc');
    }
    function codigoComprAutomatic($Tia_Cod, $Pec_Cod, $mes, $obBD){ return $this->getComNumPecAuto($Tia_Cod, $Pec_Cod, $mes, $obBD); }
    ...
    function reportes($pagina, $empresa, $obBD_conexion){
        //$pag = explode("/", $pagina);
        $Pcs_Nom='fac_alt_fac_ven__._.php'; //str_replace("_mod_", "_alt_", $pag[count($pag)-1]);
...
//            /* Consultamos informacion del cliente */
            $Cliente = $this->getRowConsultaSql("SELECT 
                  ...
                  if(cliente.Cli_Con='NO','NO','SI')as Cli_Con,ventas.Vet_Cod,ventas.Vet_Num,identifica.Ide_Prv,date_format(caja_aper.Caj_Fec, '%d/%m/%Y') AS fecha,ventas.Vet_Des,autorizaci.Pun_Sri,tipo_compr.Tic_Sri,ventas.Vet_Ntd,date_format(ventas.Vet_Fdm, '%d/%m/%Y')as Vet_Fdm,ventas.Vet_Nns
                FROM
                  persona
            ...
            $armado_xml.="<".$Eti_infoFac[2].">".$rs_infoCliente['Cli_Con']."</".$Eti_infoFac[2].">".   //<obligadoContabilidad>
                "<".$Eti_infoFac[3].">".$rs_infoCliente['Ide_Prv']."</".$Eti_infoFac[3].">".            //<tipoIdentificacionComprador>
                "<".$Eti_infoFac[4].">".mb_convert_encoding(trim($rs_infoCliente['Prs_Nom']." ".$rs_infoCliente['Prs_Ape']), 'UTF-8', 'ISO-8859-1')."</".$Eti_infoFac[4].">".//<razonSocialComprador>
