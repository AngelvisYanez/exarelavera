...
function sentencias_doc($id, $Par_Sql)
{
    ...
    switch ($id) {
        ...
        case 9:
            ...
            break;
        case 10:
            $join = (isset($Par_Sql[2]) && !empty($Par_Sql[2]) ? " LEFT JOIN autorizaci ON autorizaci.Pun_Cod=puntos_imp.Pun_Cod AND Aut_Est='A' AND Tic_Cod=$Par_Sql[2]" : '');
            $sql = "SELECT * FROM vendedor
                INNER JOIN puntos_imp ON vendedor.Pun_Cod=puntos_imp.Pun_Cod
                $join
                WHERE Suc_Cod=$Par_Sql[0] AND Prs_Cod=$Par_Sql[1] 
                ORDER BY " . (!empty($join) ? "(autorizaci.Aut_Tem='E') DESC, autorizaci.Aut_Cod DESC" : "vendedor.Vnd_Cod DESC");
            //echo $sql."<br>";
            break;
        case 11:
            if (empty($Par_Sql[26])) {
            ...
            break;
        case 12:
            $codXmlPrin = empty($Par_Sql['Cod_Xml_Principal']) || $Par_Sql['Cod_Xml_Principal'] === 'S/N' ? 'NULL' : "'" . $Par_Sql['Cod_Xml_Principal'] . "'";
            $codXmlAux = empty($Par_Sql['Cod_Xml_Auxiliar']) ? 'NULL' : "'" . $Par_Sql['Cod_Xml_Auxiliar'] . "'";
            $asi_cod_sql = (isset($Par_Sql['Asi_Cod']) && $Par_Sql['Asi_Cod'] !== '' && $Par_Sql['Asi_Cod'] !== null && (int)$Par_Sql['Asi_Cod'] > 0) ? (int)$Par_Sql['Asi_Cod'] : 'NULL';
            $sql = "INSERT INTO det_compra (Cop_Cod, Cop_Can, Iva_Cod, Cop_Pro, Cop_Pru, Cop_Imp, Cop_Dec, Adq_Cod, Ice_Int, Cop_Int, Pld_Cod,Pro_Cod,Iva_Cos,Cop_Ice,Ret_Ren_Sri, Cod_Xml_Principal, Cod_Xml_Auxiliar, Asi_Cod)
            VALUES($Par_Sql[Cop_Cod],$Par_Sql[Cop_Can],$Par_Sql[Iva_Cod],UPPER('" . (empty($Par_Sql['Cop_Pro']) ? (empty($Par_Sql['Ite_Lar']) ? '' : $Par_Sql['Ite_Lar']) : $Par_Sql['Cop_Pro']) . "'),$Par_Sql[Cop_Pru],$Par_Sql[Cop_Imp]," . (!empty($Par_Sql['Cop_Dec']) ? $Par_Sql['Cop_Dec'] : 0) . ",$Par_Sql[Adq_Cod]," . (!empty($Par_Sql['Ice_Int']) ? $Par_Sql['Ice_Int'] : 'NULL') . ", '$Par_Sql[Cop_Int]'," . (!empty($Par_Sql['Pld_Cod']) ? $Par_Sql['Pld_Cod'] : 'NULL') . ",$Par_Sql[Pro_Cod],'" . (isset($Par_Sql['Iva_Cos']) && $Par_Sql['Iva_Cos'] == 'S' ? 'S' : 'N') . "'," . (empty($Par_Sql['Cop_Ice']) ? 'NULL' : $Par_Sql['Cop_Ice']) . ",'$Par_Sql[Ret_Ren_Sri]', $codXmlPrin, $codXmlAux, $asi_cod_sql)";
        ...
        case 34:
            ...
            if (!empty($Par_Sql['mis_ingresos']) && $Par_Sql['mis_ingresos'] == 'S') {
            ...
            break;
        case 35:
            $sql = "SELECT Cop_Int,Cop_Int AS 'index',det_compra.Asi_Cod,det_compra.Ret_Ren_Sri ,det_compra.Pro_Cod,det_compra.Ice_Int,det_compra.Cop_Ice,det_compra.Iva_Cod,Iva_Por,Iva_Sri,Cop_Pro AS Ite_Lar,Cop_Can,Cop_Pru,Cop_Imp,Cop_Dec,det_compra.Adq_Cod,IFNULL(Adq_Des,'') AS Adq_Des,IFNULL(Adq_Cor,'') AS Adq_Cor,det_plan.Pld_Cod, Pld_Cdc,Pld_Des,Uni_Des,Iva_Cos,Pro_Bar, IFNULL(det_compra.Cod_Xml_Principal, '') AS Cod_Xml_Principal, det_compra.Cod_Xml_Auxiliar FROM det_compra 
                INNER JOIN producto ON producto.Pro_Cod=det_compra.Pro_Cod
                INNER JOIN unidad ON producto.Uni_Cod=unidad.Uni_Cod
                INNER JOIN iva ON iva.Iva_Cod=det_compra.Iva_Cod
                LEFT JOIN adquisicio ON adquisicio.Adq_Cod=det_compra.Adq_Cod
        ...
        case 51:
            $sql = "SELECT 
                ...
                WHERE Suc_Cod=$Par_Sql[0] AND autorizaci.Aut_Sri='$Par_Sql[1]' AND autorizaci.Tic_Cod=$Par_Sql[4] AND Ret_Num BETWEEN $Par_Sql[2] AND $Par_Sql[3] " . (!empty($Par_Sql[5]) ? " AND autorizaci.Pun_Sri='$Par_Sql[5]'" : "");
            //echo $sql.'<br/>';
            //ChromePhp::log($sql);
            break;
        case 511:
            $sql = "SELECT IFNULL(MAX(Ret_Num),$Par_Sql[2]-1)+1 AS 'next'
                FROM retencion
                INNER JOIN autorizaci ON retencion.Aut_Cod=autorizaci.Aut_Cod
                INNER JOIN puntos_imp ON puntos_imp.Pun_Cod = autorizaci.Pun_Cod
                WHERE Suc_Cod=$Par_Sql[0] AND autorizaci.Aut_Sri='$Par_Sql[1]' AND autorizaci.Tic_Cod=$Par_Sql[4] " . (!empty($Par_Sql[5]) ? " AND autorizaci.Pun_Sri='$Par_Sql[5]'" : "");
            break;
        case 52:
        ...
        case 109:
            ...
            break;
        case 110:
            $sql = "SELECT Atp_Cod, Atp_Est, Atp_Val from anticipos_proveedores where Com_Cod= $Par_Sql[0]";
            break;
