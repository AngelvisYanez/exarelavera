...
// ARREGLADO PARA SEPARACION DE STOCK POR BODEGA
if(isset($proAjax)){
    $responce=$obBD_con1->getPageGrid(39, $search.'*'.$Ses_Emp_Cod.'*'.$op_opciones.'*'.$Suc_Cod, $obBD_conexion, $page, $rows,true);
    
    // Capturar bodega desde los filtros del diálogo de búsqueda
    $bodega_id = isset($_GET['bodega']) ? $_GET['bodega'] : '';
    $codigo_bodega = '';
    
    if (!empty($bodega_id) && $bodega_id != 't') {
        // Consultar el tipo de bodega (ID 29 suele ser para bodegas)
        $tipo_bod = $obBD_con1->getRowConsulta(29, $bodega_id, $obBD_conexion); 
        if(isset($tipo_bod['bod_tip']) && $tipo_bod['bod_tip'] == 'P'){
            $codigo_bodega = "(kardex_ie.bod_cod=$bodega_id OR kardex_ie.bod_cod is null)";
        } else {
            $codigo_bodega = "kardex_ie.bod_cod=$bodega_id";
        }
    }

    if (is_array($responce['rows'])) {
        foreach ($responce['rows'] as &$row) {
            // Si hay bodega seleccionada, calculamos el stock específico de esa bodega
            if (!empty($codigo_bodega)) {
                // El ID 200 suele ser para obtener stock filtrado por bodega en el kardex
                $stock_data = $obBD_con1->getRowConsulta(200, $Ses_Suc_Cod.'*'.$row['Pro_Cod'].'*'.$codigo_bodega, $obBD_conexion);
            } else {
                // Stock general de la sucursal (ID 2)
                $stock_data = $obBD_con1->getRowConsulta(2, $Ses_Suc_Cod.'*'.$row['Pro_Cod'], $obBD_conexion);
            }
            
            $row['Stk_Can'] = (empty($stock_data['Stk_Can']) ? 0 : $stock_data['Stk_Can']); 
            $row['Ite_Lar'] = $row['Ite_Lar'].' - '.$row['Ite_Cor'].' - '.$row['Pro_Obs'];         
            $row['Aju_Imp'] = $row['Aju_Pru'] = (empty($stock_data['Pro_Prp']) ? (empty($stock_data['Stk_Prp']) ? (empty($stock_data['Pre_Pvp']) ? NULL : $stock_data['Pre_Pvp']) : $stock_data['Stk_Prp']) : $stock_data['Pro_Prp']);          
        }
    }
    $obBD_con1->echoJson($responce);
    exit();
                    ...
                    <div class="col-xs-4">
                        ...
                        <fieldset class="exa-fieldset">                           
                           <legend class="Titulos2">Datos del Movimiento:</legend> <!-- Form Name -->
                            ...
                            <div class="form-group">
                              <label class="col-sm-3 control-label label-xs">Num. Docu.:</label>  
                              <div class="col-sm-9">   
                            ...
                            <!-- static input-->
                            <div class="form-group">
                              <label class="col-sm-3 control-label label-xs ">Descripci&oacuten:</label>  
                              <div class="col-sm-9"> 
                            ...
                            <!-- static input-->
                            <div class="form-group">
                              <label class="col-sm-3 control-label label-xs ">Observaci&oacuten:</label>  
                              <div class="col-sm-9"> 
