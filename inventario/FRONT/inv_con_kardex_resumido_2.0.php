...
require_once('../LOGICA/inv_log_inventario_resumido_2.0.php');
...
if (isset($productos)) {
	try {
		$param_prod = ($Suc_Cod != "t") ? "Suc_Cod=" . $Suc_Cod : "Emp_Cod=" . $Ses_Emp_Cod;
		if ($Cate_Cod != '' && $Sub_Cod == '') {
			$param_prod .= " AND categorias.Cat_Rec=$Cate_Cod";
		}
		if ($Cate_Cod != '' && $Sub_Cod != '') {
			$param_prod .= " AND item.Cat_Cod=$Sub_Cod";
		}
		if ($Ubi_Cod != '') {
			$param_prod .= " AND producto.Ubi_Cod=$Ubi_Cod";
		}
		
		// 1. Obtener lista de productos filtrada
		$array = $obBD_con1->getArrayConsulta(22, $param_prod, $obBD_conexion);

		// Filtros adicionales para Kardex masivo
		$bodega_filter = '';
		...
		if ($Bod_Cod != 't' && $tipo['Bod_Tip'] != 'P') {
			$bodega_filter = ' AND kardex_ie.Bod_Cod=' . $Bod_Cod;
		}
		if ($tipo['Bod_Tip'] == 'P') {
			$bodega_filter = ' AND (kardex_ie.Bod_Cod is null or kardex_ie.Bod_Cod=' . $Bod_Cod . ')';
		}
		
		$param_emp = "Emp_Cod=" . $Ses_Emp_Cod;

		// 2. Obtener saldos iniciales de forma masiva (Optimización)
		$saldos_iniciales_raw = $obBD_con1->getArrayConsulta(230, $ini . '*' . $param_emp . '*' . $bodega_filter, $obBD_conexion);
		...
		// 3. Obtener todos los movimientos del rango de forma masiva (Optimización)
		$movimientos_raw = $obBD_con1->getArrayConsulta(240, $ini . '*' . $fin . '*' . $param_emp . '*' . $bodega_filter, $obBD_conexion);
...
if (isset($productos)) {
	// Bloque inaccesible eliminado por redundancia
	exit();
}
					...
					<script>
						...
						$(document).ready(function() {
							...
							kardexGrid.createGrid({
								...
								colModel: [
									{ label: 'Cód. Int.', name: 'Pro_Cod', key: true, hidden: false, viewable: true, width: 30, align: 'center' },
									{ label: 'Detalle', name: 'Ite_Lar', width: 150,
										formatter: function(c, o, r) {
											return r.Ite_Lar + (r.Ite_Lar !== r.Pro_Obs && $.vv(r.Pro_Obs) ? ' ' + r.Pro_Obs : '');
										}
									},
									{ label: 'Det. Cor.', name: 'Ite_Cor', width: 40 },
									...
									{ label: 'P. Promedio', name: 'Ini_Prp', width: 40, classes: 'columnHighlight3', align: 'right', formatter: 'number',
										formatoptions: { decimalPlaces: 6, defaultValue: '0.000000' }
									},
									...
									{ label: 'P. Promedio', name: 'Sal_Prp', width: 40, classes: 'columnHighlight4', align: 'right', formatter: 'number',
										formatoptions: { decimalPlaces: 6, defaultValue: '0.000000' }
									},
									...
									{ label: 'P. Promedio', name: 'Kar_Prp', width: 40, classes: 'columnHighlight1', align: 'right', formatter: 'number',
										formatoptions: { decimalPlaces: 6, defaultValue: '0.000000' }
									},
