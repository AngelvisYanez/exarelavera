...
if (isset($saveForm)) {
    ...
    if (count($rs_vendedor) == 0) {
        $responce = array('success' => false, 'message' => " Ud. no esta autorizado para realizar ajustes ");
    ...
    foreach ($saveForm as $row) {
        ...
        $kardexie = $kardex;
        $kardexie['Pro_Cod'] = $row['Pro_Cod'];
        $kardexie['Iva_Cod'] = $row['Iva_Cod'];
        $kardexie['IoE'] = $Tia_IoE;
    ...
    if ($configuraciones['Cof_Con'] == 'S') {
        ...
        if (empty($Pec_Cod['Pec_Cod'])) {
            ...
            $responce['message'] = 'No Existe un periodo contable!';
            echo json_encode($responce);
            exit();
        }
        ...
        foreach ($saveForm as $row) {
            ...
            $cuentaDebe = $obBD_con1->getRowConsulta(12, $row['Pro_Cod'] . '*' . ($Tia_IoE == 'E' ? 'E' : 'N'), $obBD_conexion);
            $addDebe = true;
            $cuentaHaber = $obBD_con1->getRowConsulta(12, $row['Pro_Cod'] . '*' . 'C', $obBD_conexion);
            $addHaber = true;
            if (empty($cuentaDebe['Pld_Cod']) || empty($cuentaHaber['Pld_Cod'])) {
                mysqli_rollback($obBD_conexion->conexion);
                $responce['success'] = false;
                $responce['message'] = 'Revisar la parametrizacion contable de los productos';
                echo json_encode($responce);
                exit();
            }
            // cuenta debe
    ...
    if ($obBD_ins1->Error == 0) $responce['success'] = true;
    else {
        $responce['success'] = false;
        $responce['message'] = $obBD_ins1->MsgError;
    }
...
<HEAD>
    ...
    <TITLE><?Php echo "Inv. Ajuste Registro Contable [EXA]"; ?></TITLE>
    <meta charset="UTF-8">
...
<BODY>
                                    ...
                                    <div class="col-sm-9">
                                        ...
                                        <select name="Tia_Asi" id="Tia_Asi" class="form-control input-xs" required="">
                                            <?php foreach ($asien as $row) { ?>
                                                <option value="<?php echo $row['Tia_Cod']; ?>" <?php echo ($row['Tia_Cod'] == 6 ) ? 'selected' : ''; ?>>
    ...
    <script>
        function addFilaMat(data) { //console.log(data);
            ...
            if (!grid.existsId(data['Pro_Cod']) /*&& data['Pro_Cod']!==$('#Fin_Cod').val()*/ ) {
                ...
                grid.startGridEdit();
                kardexGrid.setGridSummary(['Aju_Imp'], {
                    Aju_Pru: '<div style="text-align:right;">TOTAL:</div>'
                });
            } else {
    ...
    <div id="successDialog" title="Mensaje del Sistema">
        <center>
            <h4>El Ajuste se registrado con exito!</h4>
        </center>
        <center>
