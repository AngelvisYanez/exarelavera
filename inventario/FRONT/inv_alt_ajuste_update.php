...
if(isset($saveStock)){
    $obBD_con1->inicio_transaccion($obBD_conexion->conexion); 
        foreach ($stocks AS $row){
            $stk_pro=array('Pro_Cod'=>$row['Pro_Cod'],'Pro_Stk'=>$row['Kar_Stk'],'Pro_Prp'=>$row['Kar_Prp']);
            $stk=array('Pro_Cod'=>$row['Pro_Cod'],'Stk_Can'=>$row['Kar_Stk'],'Stk_Prp'=>$row['Kar_Prp'],'Suc_Cod'=>$Ses_Suc_Cod);
            $obBD_con1->operacionobBD(15,$stk_pro, $obBD_conexion);
            $obBD_con1->operacionobBD(21,$stk, $obBD_conexion);
        }
                            ...
                            $(document).ready(function () {
                                ...
                                kardexGrid.jqGrid({
                                    ...
                                    colModel: [                               
										...
										{ label: 'Stock Dif2.',name: 'Dif2', width: 30,classes:'columnHighlight1',align:'center',formatter:'number'},
		...
		function styleInput(e,obj,opt){            
            ...
            $(e).on('change',function (){
               if(isNaN(this.value)||this.value===''){ $(this).val('0').focus(); return false; }
					...
					kardexGrid.changeLocalRow(obj['rowId'],{Dif:this.value*1-row['Kar_Stk']*1/*,Dif2:this.value*1-row['Stk_Can']*1*/});
               }
            });
        }
    </script>
