...
if(isset($proAjax)){
   ...
   //$tipo=$obBD_con1->getRowConsulta(29,$bodega, $obBD_conexion); 
        $codigo;
        if($tipo['bod_tip']=='P'){
        $codigo = '(kardex_ie.bod_cod='.$bodega.' OR kardex_ie.bod_cod is null)';
        }else{
        $codigo = 'kardex_ie.bod_cod='.$bodega;
        }
        if ($codigo == "kardex_ie.bod_cod=" ){
        $codigo='';
      ...
      foreach ($responce['rows'] as &$row) {
          $stock = $obBD_con1->getRowConsulta(2, $Ses_Suc_Cod.'*'.$row['Pro_Cod'].'*'.$codigo, $obBD_conexion);       
      ...
      foreach ($responce['rows'] as &$row) {
          $stock = $obBD_con1->getRowConsulta(200, $Ses_Suc_Cod.'*'.$row['Pro_Cod'].'*'.$codigo, $obBD_conexion);       
          $row['Stk_Can']=(empty($stock['Stk_Can'])?0:$stock['Stk_Can']); 
          $row['Ite_Lar']=$row['Ite_Lar'].' - '.$row['Ite_Cor'].' - '.$row['Pro_Obs'];         
          $row['Aju_Imp']=$row['Aju_Pru']=(empty($stock['Pro_Prp'])?(empty($stock['Stk_Prp'])?(empty($stock['Pre_Pvp'])?NULL:$stock['Pre_Pvp']):$stock['Stk_Prp']):$stock['Pro_Prp']);          
      }
    }
    //ChromePhp::log($codigo);
        ...
        $ajuste = array(
          ...
          foreach ($saveForm AS $row){
              ...
              if($Tia_IoE=='I'){
                  ...
                  $kardexie['Kar_Prs']=$row['Aju_Pru'];
                  $kardexie['Kar_Ims']=$row['Aju_Imp'];
                  $kardexie['Bod_Cod']=$bod_pri['bod_cod'];
              }else{
                  $kardexie['Kar_Sal']=$row['Aju_Can'];
                  $kardexie['Kar_Pre']=$row['Aju_Pru'];
                  $kardexie['Kar_Ime']=$row['Aju_Imp'];
