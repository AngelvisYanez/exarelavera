 ...
 */
var kardexGrid,Aco_Cod,Aco_Des;
$(function() {
    kardexGrid=$("#prods");
    $(document).ready(function () {
        ...
        kardexGrid.createGrid({
            ...
            colModel: [                               
                ...
                { label: 'Bodega',name: 'Aco_Cod', width: 50, formatter:'selectExa', formatoptions:{name:true,options:"setAcopios",attr:{disabled:'disabled'},classes:'readOnly',dataEvents:{change:"changeAcopio"} } }, 
                { label: 'Stock',name: 'Stock', width: 40,classes:'columnHighlight3', align:'center'/*, formatter:'stocks'*/ },
                { label: 'Detalle',name: 'Ite_Lar', width: 150},                
                { label: 'Cant.',name: 'Aju_Can', width: 40,classes:'columnHighlight2', formatter:'textboxExa', formatoptions:{type:'numeric',decimals:2,dataEvents:{keyup:"alertStock([$(this).attr('rowId')])"} }},
                ...
                { label:'&nbsp;', name: 'act1', width: 15, align: 'center',viewable: false, formatter:'gridButton',formatoptions:{action:eliminarFila, title:'Quitar', data:'Pro_Cod', type:'danger', icon:'trash'} }
    ...
    $('#Tia_IoE').on('change',function (){
        alertStock();
        var Ati_Tip=$('#Tia_IoE').val();
        $('#Ati_Cod').val('').trigger('change').find('option').hide();
        if(Ati_Tip!==''){
            $('#Ati_Cod').find('option[Ati_Tip='+Ati_Tip+']').show();
        }
    });
    // DIALOG BUSCAR PRODUCTOS    
    $('#proDialog').createSearchDialog({colModel:[
        { label: 'C&oacute;d.Int.', name: 'Pro_Cod', key: true, width: 15,align:"center",hidden:true },                                
        { label: 'Producto', name: 'Producto', width: 100, classes:'highlightSearch' }, 
    ...
    ]},{title:'Producto',options:[{label:' Descripción ',value:'d'},{label:' Código ',value:'c'}]},{title:'Busqueda de Producto'});
});
function setAcopios(el,op,row){ 
    var aco=$('#Aco_Cod');
    el.html(aco.html()); 
    if(aco.val()!=='')
        el.find("option[value='"+aco.val()+"']").attr("selected","selected");
    row['Stock']=0;
    $.each(row.Stocks,function(i,r){ var option=el.find("option[value='"+r['Aco_Cod']+"']"); if(option.length>0) $.extend(option[0].dataset,r); if($.vv(option.attr('selected'))) row['Stock']=r['Ast_Can']; });
}
function changeAcopio(opt,data,e){
    var t=$(this),d=t.find('option:selected').data();   
    kardexGrid.jqGrid('setCell', opt.rowId, 'Stock', $.vv(d['Ast_Can'])?d['Ast_Can']:0);
    alertStock([opt.rowId]);
}
...
function alertStock(id){
    var ids=id||kardexGrid.jqGrid('getDataIDs');
    kardexGrid.find('tr'+(ids.length===1?'#'+ids[0]:'')+' td[aria-describedby=prods_Stock]').removeClass("cellRed1 cellGreen1 cellBold");
    if($('#Tia_IoE').val()==='E'){                                        
        $.each(ids,function (i,v){
                if(!isNaN($('#'+v+'_Aju_Can').val())&&$('#'+v+'_Aju_Can').val()*1>kardexGrid.jqGrid("getCell", v, "Stock"))
                     kardexGrid.jqGrid('setCell',v,"Stock","",'cellRed1 cellBold'); 
                else kardexGrid.jqGrid('setCell',v,"Stock","",'cellGreen1 cellBold'); 
        });
    }   
}
...
function validarForm(){
    ...
    for(var i=0;i<data['saveForm'].length;i++){
        ...
        if(data['Tia_IoE']==='E' && ('0'+data['saveForm'][i]['Aju_Can'])*1>('0'+data['saveForm'][i]['Stock'])*1)
...
function guardar(data){
    ...
    $.saveDataJson("", data, function( response ) {
        ...
        $('.btn-frm').removeAttr('disabled');$.alert(response['message']); $("#prods").startGridEdit(); return false;
    },function(){
        $('.btn-frm').removeAttr('disabled'); $("#prods").startGridEdit(); 
    });
...
//$.fn.fmatter.stocks=function(cv,opts,cObjt){ if(cObjt.Aco_Cod===''||cObjt.Stk_Can.length===0) return 0; console.log(cv,opts,cObjt); };
