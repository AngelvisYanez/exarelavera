...
if(isset($saveDocumento)){
    ...
    $productore=$obBD_con1->getRowConsulta('productor_bana.selectWhere', array('Prv_Cod'=>$Prv_Cod), $obBD_conexion);
    ...
    try{
        ...
        $obBD_ins1->operacionobBD('productor_bana.insert', $_POST, $obBD_conexionIns);
        $Prd_Cod=$obBD_ins1->insercionid($obBD_conexionIns);
        // guardo las haciendas
        foreach($haciendas as $haci) {
            $haci['Prd_Cod']=$Prd_Cod;
            $obBD_ins1->operacionobBD('productor_haci.insert', $haci, $obBD_conexionIns,true);
        }
...
<!--                    <div class="jqHeaderFirst jqFirst">
    ...
    function deleteHacienda(data){ haciendas.delRowData(data.Index); }
    ...
    function validaDocument(){
        ...
        if(data['haciendas'].length===0) return $.alert("Debe ingresar al menos una hacienda!",null,'alert');
