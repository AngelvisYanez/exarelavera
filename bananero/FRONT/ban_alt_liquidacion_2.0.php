...
$precioOficial=$obBD_con1->getPrecioOficial();
$hoy = date("Y-m-d");

if(isset($provAjax)){
    $page=$obBD_con1->getPageGridJson('productor_bana.selectWhere', array_merge(array('setWhere'=>'isActive'),$_GET), $obBD_conexion);
}
if(isset($prodSearchAjax)){
    $page=$obBD_con1->getPageGridJson('producto.selectWhere', $_GET, $obBD_conexion);
}
if(isset($getDataTarjas)){
    $resp=array('success'=>true);
    $se=array('Prd_Cod'=>$Prd_Cod,'Prt_Ano'=>$Lib_Ano,'Prt_Sem'=>$Lib_Sem,'Prt_Est'=>'A','setWhere'=>array('notHasLiquidacion'));
    if(isset($Bam_Cod)&&!empty($Bam_Cod)) $se=array_merge($se,array('productor_tarja.Bam_Cod'=>$Bam_Cod));
    $resp['tarjas']=$obBD_con1->getArrayConsulta('productor_tarja.selectWhere', $se, $obBD_conexion);
    $resp['liquidaciones']=$obBD_con1->getArrayConsulta('liquidacion_bana.selectWhere', array('liquidacion_bana.Prd_Cod'=>$Prd_Cod,'liquidacion_bana.Lib_Ano'=>$Lib_Ano,'liquidacion_bana.Lib_Sem'=>$Lib_Sem,/*'liquidacion_bana.Bam_Cod'=>$Bam_Cod,*/'Lib_Est'=>'A'), $obBD_conexion );
    $obBD_con1->echoJson($resp);
}

if(isset($validaNum)){
    $next=$obBD_con1->getRowConsulta('liquidacion_bana.sql.getNext2', $Lib_Mag, $obBD_conexion);
    ...
    if(isset($_GET['Lib_Num'])&&!empty($Lib_Num)){
        ...
        $tarjas=$obBD_con1->getRowConsulta('liquidacion_bana.sql.getByLibNum2', array('Lib_Num'=>$Lib_Num,'Lib_Mag'=>$Lib_Mag), $obBD_conexion);
...
if(isset($getDetalleLiq)){
    $resp=array('success'=>true, 'ingresos'=>array('cartones'=>array(),'materiales'=>array(),'materiales2'=>array()), 'descuentos'=>array('cartones'=>array(),'materiales'=>array(),'materiales2'=>array()), 'ctaPagar'=>array(), 'cajas'=>array() );

    $resp['cajas']=$obBD_con1->getArrayConsulta('productor_tarja_det.selectWhere', array('Prt_Cod'=>$Tar_Cod,'where'=>'','setWhere'=>array('byTarjas')), $obBD_conexion);

    foreach ($resp['cajas'] as $car) {
        $carton=$obBD_con1->getArrayConsulta('mesclas_det.selectWhere', array('Pro_Cod'=>$car['Pro_Cod'],'where'=>array('Mes_Tip'=>'C'),'setWhere'=>array("setEmpCod",'byResProCod')), $obBD_conexion/*,true*/);
        foreach ($carton as $val) {
            // borrado porque quieren 0, es temporal
            //$val['Lid_Can']=$val['Mes_Can']*$car['Total'];
            $val['Lid_Can']=0;

            $stock=$obBD_con1->getRowConsulta('acopio_stk.selectWhere', array('Pro_Cod'=>$val['Pro_Cod'],'Prd_Cod'=>$Prd_Cod), $obBD_conexion);
            $val['Stock']=set_val_or($stock,'Ast_Can',0);
            $val['Stock_Prp']=set_val_or($stock,'Ast_Prp',0);

            /* codigo temporal */
            $precio=$obBD_con1->getRowConsulta('precios.selectWhere', array('precios.Pro_Cod'=>$val['Pro_Cod'], 'precios.Suc_Cod'=>$Ses_Suc_Cod, 'tipo_preci.Tpv_Des'=>'Standar'), $obBD_conexion);
            if(!is_empty($precio,'Pre_Cod')){
                $val['Lid_Pru']=$precio['Pre_Pvp'];
                $val['Lid_Imp']=$val['Lid_Can']*$val['Lid_Pru'];
            }else{
                $val['Lid_Pru']=0;
                $val['Lid_Imp']=0;
            }
            /* fin codigo tempporal */

            array_push($resp['ingresos']['cartones'], $val);
            array_push($resp['descuentos']['cartones'], $val);
        }// unset($val);
    }
    $material=$obBD_con1->getArrayConsulta('mesclas_det.selectWhere', array('Mes_Tip'=>'M','setWhere'=>array('setEmpCod')), $obBD_conexion);
    ...
    foreach ($material as &$mat) {
        ...
        $mat['Stock']=0;
        $mat['Lid_Can']=0;
        $precio=$obBD_con1->getRowConsulta('precios.selectWhere', array('precios.Pro_Cod'=>$mat['Pro_Cod'], 'precios.Suc_Cod'=>$Ses_Suc_Cod, 'tipo_preci.Tpv_Des'=>'Standar'), $obBD_conexion);
       
        if(!is_empty($precio,'Pre_Cod')){
            $mat['Lid_Pru']=$precio['Pre_Pvp']*1==0?'':$precio['Pre_Pvp'];
            $mat['Lid_Imp']=$mat['Lid_Can']*$mat['Lid_Pru'];
        }else{
            $mat['Lid_Pru']='';
            $mat['Lid_Imp']=0;
        }
    ...
    $resp['ingresos']['materiales']=$material;
    $resp['descuentos']['materiales']=$material;

    if($config['Cof_Con']=='S') $resp['ctaPagar']=$obBD_con1->getArrayConsulta('ccpp_prove.selectWhere', array('Year'=>$Lib_Ano,'where'=>array(),'setWhere'=>array('setEmpCod','isActive','getByYear')), $obBD_conexion);
    $resp['anticipos']=$obBD_con1->getAnticipos($Prv_Cod, $obBD_conexion/*,true*/)?:array('Total'=>0);
    $obBD_con1->echoJson($resp);
}
if(isset($saveDocument)){
    $resp=array('success'=>false);

    $liquid=$obBD_con1->getRowConsulta('liquidacion_bana.sql.getByLibNum2', array('Lib_Num'=>$form['Lib_Num'],'Lib_Mag'=>$form['Lib_Mag']), $obBD_conexion);
    ...
    try{
        $Com_Cod=null;
        if($config['Cof_Con']=='S'&&false){
            /* armo cuentas items, y busco la retencion 2 */
                        ...
                        foreach ($ti as $i => $v) {
                            if(($v['Lid_Imp']*1)>0){
                                ...
                                if($add){
                                    ...
                                    if(is_empty($cta,'Pld_Cod')) throw new Exception('Revisar la parametrizacion contable <i class="orange">'.($tiCta=='HABER'?'EGRESOS':'COSTOS').'</i> del Centro de Consumo <i class="green">PRODUCTORES</i> para el Producto: <u class="blue">'.$v['Producto'].'</u>!');
                         ...
                         foreach ($ti as $i => $v){
                            if(($v['Lid_Imp']*1)>0 && strpos(strtoupper($v['Producto']), 'RETENCI') !== false ){
                                $retencion2+=($v['Lid_Imp']*1);
                            }
                         }
                    }
                }
            ...
            $obBD_ins1->operacionobBD('comprobantes.insert', array('Pec_Cod'=>$PecCod['Pec_Cod'],'Tia_Cod'=>$TiaCod['Tia_Cod'],'Prv_Cod'=>$form['Prv_Cod'],'Usu_Cod'=>$Ses_Usu_Cod,'Com_Num'=>$ComNum,'Com_Fec'=>$form['Lib_Fec'],'Com_Tip'=>'D','Com_Con'=>'Liquidacion de Fruta','Com_Obs'=>$form['Lib_Obs'],'Com_Val'=>$totalDescuentos-$retencion2,'Com_Gen'=>'A'), $obBD_conexionIns);
            ...
            if($totalMatIngresos*1!=($totalMatDescuentos*1)){
                $ajusCtaTip=!($totalMatIngresos*1>($totalMatDescuentos*1));
                $ajusCtaVal=abs($totalMatDescuentos*1);
                ...
                $ctAjus=$obBD_con1->getRowConsulta('plan_param.selectWhere', array('Tpa_Abr'=>($ajusCtaTip?'OTE':'OTI'), 'where'=>'', 'setWhere'=>array('isActive','setEmpCod','getByAbr')), $obBD_conexion/*,true*/);
            ...
            if($totalCartIngresos*1!=$totalCartDescuentos*1){
                if($totalCartIngresos*1>$totalCartDescuentos*1){
                    ...
                    // Agrego Temporal Por Inventarios
                    /*$val=$totalCartIngresos*1-$totalCartDescuentos*1;
                    ...
                    $obBD_ins1->operacionobBD('asientos.insert', array('Com_Cod'=>$Com_Cod,'Asi_Deh'=>'D','Asi_Val'=>$val,'Asi_Con'=>$form['Productor'],'Asi_Glo'=>'REINGRESO CARTONES','Pld_Cod'=>$ctLiq['Pld_Cod']), $obBD_conexionIns);
                    $obBD_ins1->operacionobBD('asientos.insert', array('Com_Cod'=>$Com_Cod,'Asi_Deh'=>'H','Asi_Val'=>$val,'Asi_Con'=>$form['Productor'],'Asi_Glo'=>'REINGRESO CARTONES','Pld_Cod'=>$ctInv['Pld_Cod']), $obBD_conexionIns);                    */
                    // Fin Agrego Temporal Por Inventarios
                }
                if($totalCartIngresos*1<$totalCartDescuentos*1){
            ...
            foreach($ctas as $ct){ foreach($ct as $v){ $v['Com_Cod']=$Com_Cod; if($v['Asi_Val']>0) $obBD_ins1->operacionobBD('asientos.insert', $v, $obBD_conexionIns); } }
        ...
        $obBD_ins1->operacionobBD('liquidacion_bana.insert', $form, $obBD_conexionIns);
        $Lib_Cod=$obBD_ins1->insercionid($obBD_conexionIns);
        ...
        foreach ($tarjas as $t) {
            $obBD_ins1->operacionobBD('productor_tarja.update', array('Lib_Cod'=>$Lib_Cod, 'Prt_Cod'=>$t), $obBD_conexionIns);
        }
...
<HEAD>
    ...
    <script type="text/javascript" src="../VALIDACIONES/ban_val_liquidacion_2.0.js?x=2"></script>
</HEAD>
<BODY>
    <div class="panel panel-main">
        <div class="panel-heading exa-header"><h3 class="panel-title">&raquo;  Registrar Liquidacion de Compra de Fruta</h3></div>
        <div class="panel-body ui-widget-content ui-corner-bottom exa-body">
            <div class="row" id="main">
                <form id="formDocumento" class="form-horizontal normal formDatos" action="javascript:loadTarjas();">
                <div class="col-sm-4">
                    <fieldset class="exa-fieldset" id="provFormTemp">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-xs-7" >
                                <select id="Lib_Ano" name="Lib_Ano" class="form-control input-xs" required="" >
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-xs-9" ><select id="Prt_Sem" name="Lib_Sem" class="form-control input-xs" required=""></select></div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-xs-9" >
                                <select id="Bam_Cod" name="Bam_Cod" class="form-control input-xs getData ins" ><!-- antes required -->
                                    <?php if(count($marcas)!=1){ ?><option value="">Todas...</option><?php } ?>
                ...
                <div class="col-sm-4">
                    <div>
                        <table id="liquidacionesTemp"></table>
                        <div id="liquidacionesTempPager"></div>
                    </div>
                </div>
                <div class="col-sm-4">
                    <div class="jqHeaderFirst jqFirst">
                        <table id="tarjas"></table>
                        <div id="tarjasPager"></div>
                    </div><div class="help-block"></div>
                    <div class="center">
                        <button type="button" onclick="crearLiquidacion();" class="btn btn-sm btn-success"><i class="glyphicon glyphicon-floppy-disk"></i> Crear Liquidaci�n</button>
                    </div>
                </div>
            </div>
                ...
                <div class="col-sm-5">
                    <form id="formDocumentoMain" class="form-horizontal normal formDatos" action="javascript:validaDocument();">
                        <fieldset class="exa-fieldset">
                            <legend class="Titulos2">Datos de la Liquidaci�n</legend>
                            <div class="form-group">
                                <label class="col-xs-3 control-label label-xs required">N�mero:</label>
                                <div class="col-xs-4" >
                                    <div class="input-group input-group-xs">
                                ...
                                <div class="col-xs-3" ><span id="Magap" name="Lib_Mag" class="form-control input-xs databind datatitle"></span></div>
                            </div>
                            <div class="form-group">
                                <label class="col-xs-3 control-label label-xs required">Periodo:</label>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-4" ><input type="text" id="Lib_Fec" name="Lib_Fec" class="form-control input-xs " required="" /></div>
                                <label class="col-xs-2 control-label label-xs required">P. Caja:</label>
                                <div class="col-xs-3" ><input type="text" id="Lib_Pru" name="Lib_Pru" class="form-control input-xs txtRight" required="" value="<?php echo $precioOficial; ?>" onkeypress="return validar_decimal(event)" onchange="changePruCajas();" /></div>
                            </div>
                            <div class="form-group">
                                <label class="col-xs-3 control-label label-xs required">Marca:</label>
                                <div class="col-xs-9" ><span data-name="Bam_Cod_Txt" class="form-control input-xs databind datatitle"></span></div>
                            </div>
                            <div class="form-group">
                                <label class="col-xs-3 control-label label-xs required">Productor:</label>
                            ...
                            <div class="form-group">
                                <label class="col-xs-3 control-label label-xs required">Retenci�n:</label>
                                <div class="col-xs-9" >
                                    <select id="retencionValues" class="form-control input-xs readOnly" disabled=""></select>
                                    <script>$.each(valores_cal,function(i,v){ $('#retencionValues').append("<option value=\""+i+"\">"+tipos[i]+"</option>"); });</script>
                                </div>
                            </div>
                            <div class="form-group">
                        ...
                        <!--<div class="form-group" id="racimosForm">
                            <div class="col-xs-12" >
                                <div class="input-group input-group-xs">
                                    <span class="input-group-addon bold alert-warning">Racimos:</span>
                                    <span class="input-group-addon bold alert-info">Total:</span>
                                    <input id="Rac_Imp" type="text" class="form-control span txtRight" readonly="" tabindex="-1">
                                    <span class="input-group-addon bold alert-info">&divide;&nbsp;P. Uni:</span>
                                    <input id="Rac_Pru" name="Lib_Pra" type="text" class="form-control span txtRight" onkeypress="return validar_decimal(event);" onkeyup="updateRacimosCant();">
                                    <span class="input-group-addon bold alert-info">=&nbsp;Cant:</span>
                                    <input id="Rac_Can" name="Ren_Tot" type="text" class="form-control span txtRight" readonly="" tabindex="-1">
                                </div>
                            </div>
                        </div>-->
                    </form>
...
<!--                        <div class="form-horizontal normal">
                    ...
                    <div id="anticipos" class="form-horizontal normal" style="display:none;">
                        <div class="separator"></div>
                        <div class="form-group">
                            <div class="col-sm-6"></div>
                            <label class="col-sm-3 control-label label-xs">Anticipos a Favor:</label>
                            <div class="col-sm-3"><div class="input-group input-group-xs"><span class="input-group-addon alert-warning">$</span><span id="ant_msg" class="form-control input-xs txtRight alert alert-danger bold"></span><span id="infoAnt" class="input-group-addon alert-info"><i class="glyphicon glyphicon-info-sign"></i></span></div></div>
                        </div>
                    </div>
                </div>
                <div class="col-xs-12">
                    <button class="btn btn-sm btn-inverse" onclick="$('#liquidacion').moveComp('#main').updateGridsSizes();"><i class="glyphicon glyphicon-arrow-left"></i> Atr�s</button>
    ...
    <div id="successDialog" title="Mensaje del Sistema" style="display: none;">
        <center>
            <b style="font-size:14px;">Se registro la Liquidaci�n con Exito!</b>
