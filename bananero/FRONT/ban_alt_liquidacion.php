...
if (isset($prodSearchAjax)) {
  ...
  /*  $page = $obBD_con1->getArrayConsultaSql("SELECT * FROM producto  
     INNER JOIN  item ON item.Ite_Cod = producto.Ite_Cod     
     INNER JOIN categorias ON categorias.Cat_Cod = item.Cat_Cod  
      WHERE producto.Pro_Est='A' AND categorias.Emp_Cod=$Ses_Emp_Cod", $obBD_conexion);*/
    $obBD_con1->echoJson($page);
}

if (isset($getDataTarjas)) {
    $resp = array('success' => true);
    $resp['tarjas'] = $obBD_con1->getArrayConsulta('productor_tarja.selectWhere', array('Prd_Cod' => $Prd_Cod, 'Prt_Ano' => $Lib_Ano, 'Prt_Sem' => $Lib_Sem, 'productor_tarja.Bam_Cod' => $Bam_Cod, 'Prt_Est' => 'A', 'setWhere' => array('notHasLiquidacion')), $obBD_conexion);
    $resp['liquidaciones'] = $obBD_con1->getArrayConsulta('liquidacion_bana.selectWhere', array('Prd_Cod' => $Prd_Cod, 'Lib_Ano' => $Lib_Ano, 'Lib_Sem' => $Lib_Sem, 'liquidacion_bana.Bam_Cod' => $Bam_Cod, 'Lib_Est' => 'A'), $obBD_conexion);
...
if (isset($validaNum)) {
    ...
    if (isset($_GET['Lib_Num']) && !empty($Lib_Num)) {
        ...
        $tarjas = $obBD_con1->getRowConsulta('liquidacion_bana.sql.getByLibNum', array('Lib_Num' => $Lib_Num), $obBD_conexion);
...
if (isset($getDetalleLiq)) {
    ...
    //ChromePhp::log($resp['cajas']);
    foreach ($resp['cajas'] as $car) {
        //ChromePhp::log($car);
        $carton = $obBD_con1->getArrayConsulta('mesclas_det.selectWhere', array('Mes_Tip' => 'C', 'Pro_Cod' => $car['Pro_Cod'], 'where' => array(), 'setWhere' => array('byResProCod')), $obBD_conexion);
    ...
    // $material = $obBD_con1->getArrayConsulta('mesclas_det.selectWhere', array('Mes_Tip' => 'M'), $obBD_conexion);
...
if (isset($saveDocument)) {
    ...
    $liquid = $obBD_con1->getRowConsulta('liquidacion_bana.sql.getByLibNum', array('Lib_Num' => $form['Lib_Num']), $obBD_conexion);
    ...
    try {
        ...
        if ($config['Cof_Con'] == 'S') {
            ...
            $obBD_ins1->operacionobBD('comprobantes.insert', array('Pec_Cod' => $PecCod['Pec_Cod'], 'Tia_Cod' => $TiaCod['Tia_Cod'], 'Prv_Cod' => $form['Prv_Cod'], 'Usu_Cod' => $Ses_Usu_Cod, 'Com_Num' => $ComNum, 'Com_Fec' => $form['Lib_Fec'], 'Com_Tip' => 'D', 'Com_Con' => 'Liquidacion de Fruta', 'Com_Obs' => $form['Lib_Obs'], 'Com_Val' => $totalDescuentos, 'Com_Gen' => 'A'), $obBD_conexionIns);
            ...
            foreach ($ctas as $ct) {
                foreach ($ct as $v) {
                    $obBD_ins1->operacionobBD('asientos.insert', $v, $obBD_conexionIns);
                }
            }
        ...
        $obBD_ins1->rollBack_nomsn($obBD_conexionIns, $e->getMessage());
...
<BODY>
    <div class="panel panel-main">
        <div class="panel-heading exa-header">
            <h3 class="panel-title">&raquo; Registrar Liquidacion de Compra de Fruta</h3>
        </div>
        <div class="panel-body ui-widget-content ui-corner-bottom exa-body">
            <div class="row" id="main">
                    ...
                    <div class="col-sm-4">
                        <fieldset class="exa-fieldset" id="provFormTemp">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-9">
                                    <select id="Bam_Cod" name="Bam_Cod" class="form-control input-xs getData ins" required="">
                ...
                <div class="col-sm-4">
                    ...
                    <div class="center">
                        <button type="button" onclick="crearLiquidacion();" class="btn btn-sm btn-success"><i class="glyphicon glyphicon-floppy-disk"></i> Crear Liquidación</button>
                    </div>
                </div>
            </div>
                ...
                <div class="col-sm-7">
                    ...
                    <div id="tabsLiqui" class="ui-tab-fix ui-tabs">
                        <ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix">
                            ...
                            <li class="pull-right">
                                <div class="ui-tabs-anchor <?php echo ($config['Cof_Con'] == 'S' ? '' : 'hidden'); ?> " style="color:#333; text-shadow:0 0px 0px; padding: 0;">
                                    <div style="display: inline-table;">Cta. por Pagar:&nbsp;&nbsp;</div>
                                    <div style="display: inline-table;"><select id='Pld_Cod' name='Pld_Cod' class="form-control input-xs">
                                            <option>...</option>
                                        </select></div>
                                </div>
                            </li>
    ...
    <!--INICIO DEL DIALOGO BUSCAR PRODUCTOR-->
    <div id="prodSearchDialog" title="B&uacute;squeda de Materiales">
        <form id='prodSearchForm'><input type='hidden' name='ingresos' /><input type='hidden' name='grupo' /></form>
    </div>
    <!--INICIO DEL DIALOGO GUARDADO-->
    <div id="successDialog" title="Mensaje del Sistema" style="display: none;">
        <center>
            <b style="font-size:14px;">Se registro la Liquidación con Exito!</b>
            <h4 id="compCodGrp"><b class="blue">Asiento: </b><span class="orange" id="successCodigo">dd-55-55</span></h4>
            <button id="btnImpLiqui" type="button" class="btn btn-info" onclick="$.imprimirUrl($(this).data('url'))"><i class="glyphicon glyphicon-print"></i> Imprimir Liquidacion</button>
            <div class="help-block"></div>
            <button id="btnImpDetal" type="button" class="btn btn-info" onclick="$.imprimirUrl($(this).data('url'))"><i class="glyphicon glyphicon-print"></i> Imprimir Detalle</button>
            <div class="help-block"></div>
