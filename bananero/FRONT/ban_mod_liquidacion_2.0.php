...
if (isset($searchLiquid)) {
    $obBD_con1->getPageGridJson('liquidacion_bana.selectWhere', array_merge(array('setWhere' => array(/*'setProductor'*/)), $_GET), $obBD_conexion, true);
}
if (isset($validaNum)) {
    ...
    if (isset($_GET['Lib_Num']) && !empty($Lib_Num)) {
        ...
        $tarjas = $obBD_con1->getRowConsulta('liquidacion_bana.sql.getByLibNum2', array('Lib_Num' => $Lib_Num, 'Lib_Mag' => $Lib_Mag, 'Lib_Cod' => $Lib_Cod), $obBD_conexion);
...
if (isset($saveDocument)) {
    ...
    $liquid = $obBD_con1->getRowConsulta('liquidacion_bana.sql.getByLibNum2', array('Lib_Num' => $form['Lib_Num'], 'Lib_Cod' => $form['Lib_Cod'], 'Lib_Mag' => $form['Lib_Mag']), $obBD_conexion);
    ...
    try {
        if ($config['Cof_Con'] == 'S' && false) {
            ...
            $obBD_ins1->operacionobBD('comprobantes.update', array('where' => array('Com_Cod' => $Com_Cod), 'Usu_Cod' => $Ses_Usu_Cod,/*'Com_Num'=>$ComNum,'Com_Fec'=>$form['Lib_Fec'],*/ 'Com_Obs' => $form['Lib_Obs'], 'Com_Val' => $totalDescuentos - $retencion2), $obBD_conexionIns);
            ...
            if ($totalIngresos * 1 != ($totalDescuentos * 1 - $retencion2)) {
                $ajusCtaTip = !($totalIngresos * 1 > ($totalDescuentos * 1 - $retencion2));
                $ajusCtaVal = abs(($totalDescuentos * 1 - $retencion2));
        ...
        if (empty($Com_Cod)) $form['Com_Cod'] = null;
        /* falta control de inventarios aqui */


        $obBD_ins1->operacionobBD('liquidacion_bana.update', $form, $obBD_conexionIns);
        $obBD_ins1->operacionobBD('liquidacion_bana_det.deleteWhere', array('Lib_Cod' => $Lib_Cod), $obBD_conexionIns);
...
<HEAD>
    ...
    <script type="text/javascript">
        var buttonExtra = {
            ...
            formatoptions: {
                action: 'editarLiquidacion',
                data: 'Lib_Cod',
                title: 'Editar Liquidacion',
                conditional: function(o) {
                    return o.Lib_Est !== 'I' && o.Cop_Cod === null;
...
<BODY>
    <div class="panel panel-main">
        <div class="panel-heading exa-header">
            <h3 class="panel-title">&raquo; Modificar Liquidacion de Compra de Fruta</h3>
        </div>
                ...
                <div class="col-sm-5">
                    <form id="formDocumentoMain" class="form-horizontal normal formDatos" action="javascript:validaDocument();">
                        ...
                        <fieldset class="exa-fieldset">
                            ...
                            <div class="form-group">
                                <label class="col-xs-3 control-label label-xs required">Retención:</label>
                                <div class="col-xs-9">
                                    <select id="retencionValues" class="form-control input-xs readOnly" disabled=""></select>
                                    <script>
                                        $.each(valores_cal, function(i, v) {
                                            $('#retencionValues').append("<option value=\"" + i + "\">" + tipos[i] + "</option>");
                                        });
                                    </script>
                                </div>
                            </div>
    ...
    <div id="successDialog" title="Mensaje del Sistema" style="display: none;">
        <center>
            ...
            <div class="help-block"></div>
            <!--button style="display: none;" id="btnImpDetal" type="button" class="btn btn-info" onclick="$.imprimirUrl($(this).data('url'))"><i class="glyphicon glyphicon-print"></i> Imprimir Detalle</button>
            <div class="help-block"></div>
            <button style="display: none;" id="btnImpCompr" type="button" class="btn btn-info" onclick="$.imprimirUrl($(this).data('url'))"><i class="glyphicon glyphicon-print"></i> Imprimir Comprobante</button-->
        </center>
    </div>
