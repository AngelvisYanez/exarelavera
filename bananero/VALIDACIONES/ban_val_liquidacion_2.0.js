...
var tipos=["Exportador","Productor","1%"];
var valores_cal=[
    [
        ...
        {index:1, Desc:'De 1 en Adelante', Desd:1, Hast:Infinity, Porc:1.00}
    ]
];
var calculadora_bana=valores_cal[0];
...
$(function() {
    ...
    tarjas.createGrid({
        ...
        colModel: [
            ...
            { label: 'Magap', name: 'Prt_Mag', width: 10, formatter:'truefalse' }
        ]
    },true);
    ...
    resumen.createGrid({
        ...
        colModel: [
            ...
            { label: 'Descripción', name: 'Desc', width: 75, classes:'bgNoColor bgNoRight bgSumNoRight'},
    ...
    if($('#tabsLiqui').length>0){
        $('#tabsLiqui').tabs({cache:false,activate: function( event, ui ){
    ...
    $('#prodSearchDialog').createSearchDialog({colModel:[
        { label: 'C&oacute;d.Int.', name: 'Pro_Cod', key: true, width: 15,align:"center",hidden:false },
        { label: 'Producto', name: 'Producto', width: 100},
        { label:'&nbsp;', name: 'act1', width: 20, align: 'center',viewable: false, formatter:'gridButton', formatoptions:{action:addItemMaterial} }
    ],datatype:'local'},{title:'Materiales',options:[{label:'&nbsp;&nbsp;Producto&nbsp;&nbsp;',value:'d'},{label:'&nbsp;&nbsp;C&oacute;digo&nbsp;&nbsp;',value:'c'}]});
...
function crearLiquidacion(){
    ...
    var tipo, NoError=true;
    $.each(tarjasSelectedGrid, function(i,v){
        if(tipo===undefined) tipo=v['Prt_Mag'];
        else{
            if(v['Prt_Mag']!==tipo){
                NoError=false;
            }
    ...
    if(!NoError) return $.alert("No Puede Mesclar Tarjas con Magap Propio y Tarjas Con Magap Alterno!",null,'alert');

    $.each(tarjasSelectedGrid, function(i,v){ tarjasSelected.push(v['Prt_Cod']); });
    $.getDataJson('',{getDetalleLiq:true, Tar_Cod:tarjasSelected,Prv_Cod:$('input[name=Prv_Cod]').val(),Prd_Cod:$('input[name=Prd_Cod]').val(),Lib_Ano:form['Lib_Ano']},function (r){
...
//            $('#cajasLiquidaciones').html(cajasLiqAnteriores);
        ...
        updateAnticipos(r.anticipos);

        $('#main').moveComp('#liquidacion').updateGridsSizes();
    });
}
function updateAnticipos(Ant){
    $('#anticipos')[Ant.Total.toNum()===0?'hide':'show']();
    if(Ant.Total.toNum()>0){
        $('#ant_msg').html(Ant.Total);
        $('#infoAnt').attr('title',Ant.Obs);
    }
}
function setReteVal(cod){
    $('#retencionValues').val(cod);
    calculadora_bana=valores_cal[cod];
}
function updateRacimosCant(){
    var total=$('#cartones').jqGrid('getCol','Caj_Imp',false,'sum'), Pru=$('#Rac_Pru').val();
    $('#Rac_Imp').val($.toFixed(total));
    if(!$.isNum(Pru)){
        $('#Rac_Can').val('0.00');
    }else{
        $('#Rac_Can').val($.round(total/Pru,4));
    }
...
function validaDocument(){
    ...
    data['totalMatIngresos']=$.arraySumVal(data.ing['materiales'],'Lid_Imp');
    data['totalMatDescuentos']=$.arraySumVal(data.des['materiales'],'Lid_Imp');
    data['totalMat2Ingresos']=$.arraySumVal(data.ing['materiales2'],'Lid_Imp');
    data['totalMat2Descuentos']=$.arraySumVal(data.des['materiales2'],'Lid_Imp');
...
function saveDocument(data){ //console.log(data); console.log(data['rets']);
    $.saveDataJson('',data,
        function (r){
            ...
            $('#btnImpLiqui').data('url',r['linkLiqui']+(data['form']['Lib_Mag']==='S'?'':'&gerencial='));
            $('#btnImpDetal').data('url',r['linkLiquiDet']);
            $('#btnImpDetal')[data['form']['Lib_Mag']==='S'?'show':'hide']();
            if(liquidaciones.length>0)
...
function validatNum(){
    var Lib_Cod=$('#Lib_Cod') ;
   $('#Lib_Num').getValidationJson('',{validaNum:true, Lib_Num:$('#Lib_Num').val(), Lib_Mag:$('#Magap').text(), Lib_Cod:Lib_Cod.length>0?Lib_Cod.val():undefined},function(r){
        var rnum=$('#Lib_Num');
        if(!r['valid']){
...
function clearSearch(){
    $('#formDocumento').setData({Bam_Cod:''},false);
