...
$(function() {
    ...
    cajasban.createGrid({
        ...
        colModel: [
            { label: 'Cód. Int.', name: 'Index', width: 15 ,align:"center", key:true, hidden:true},
    ...
    setSemanas();
    if(!$('#Prt_Sem2').length)
    $('#Prt_Fec').on('change',function(){
        if(this.value.trim()==="") return;
        $('#Prt_Ano').val(this.value.substring(0,4));
        $('#Prt_Sem').val(moment(this.value).isoWeek());
        $('#Prt_Ano').trigger('change');
    });
});
...
function validaDocument() {
    ...
    if($.isEmpty(data.Nco_Cod)) return $.alert("Debe escoger un Container/Nave!",null,'alert');
    ...
    var Nco=$('#Exc_Cod option:selected').data();
    if(Nco.Nco_Can.toNum()<(cajas[1]['Can'].toNum()+Nco.Registrado.toNum())) return $.alert("La <u class='orange'>SUMATORIA TOTAL</u> excede la capacidad del contenedor <b class='green'>"+Nco.Nco_Nom+"</b>!",null,'alert');
    data['Prt_Mag']=$.isEmpty($('#Magap').html())?'N':'S';
    //console.log(data);
    $.createDialogConfirm('¿Esta seguro de guardar la <b class="green">Tarja(Comprobante de Recepción)</b>?', data, saveDocument);
}
...
function clearForm(next){
    $('#formDocumento').setData({Prt_Por:100,Prt_Fec:''/*, Prt_Fec:$.hoy()*/});
...
function setContainers(containers) {
    ...
    Exc_Cod.html('<option value="">Selecione Container..</option>');
    $.each(containers, function(i,v){
        console.log(v.Nco_Cod);
        var aux=$('<option value="'+v.Nco_Cod+'">'+v.Nco_Nom+'</option>');
        aux.data(v);
        Exc_Cod.append(aux);
    });
}
function loadContainers(){
    ...
    $.getDataJson('',{getContainers:true, where:{Vap_Sem:Prt_Sem,Vap_Ano:Prt_Ano}, setWhere:['setEmpCod','setVapor','isActive']},function(r){
