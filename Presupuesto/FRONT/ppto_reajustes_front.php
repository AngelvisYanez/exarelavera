...
if (isset($_POST['guardar_reajuste'])) {
    ...
    $res_ver = $mysqli->query("SELECT ppe_id FROM exa_ppto_cabeceras WHERE Emp_Cod=$emp_filtro AND ppe_id=$ppe_id LIMIT 1");
...
$versiones = array();
$partidas = array();
$res_v = $mysqli->query("SELECT ppe_id, ppe_anio, ppe_version, ppe_descripcion FROM exa_ppto_cabeceras WHERE Emp_Cod=$emp_filtro ORDER BY ppe_anio DESC, ppe_version DESC");
if ($res_v) {
...
$res_r = $mysqli->query("SELECT r.*, po.ppa_codigo_clasificacion AS cod_orig, pd.ppa_codigo_clasificacion AS cod_dest
    ...
    WHERE r.Emp_Cod=$emp_filtro ORDER BY r.rea_fecha_registro DESC LIMIT 50");
if ($res_r) {
