...
$proy_nombre = $proy_id;
$r_proy = $mysqli->query("SELECT proy_nombre FROM exa_ppto_proyectos
    WHERE proy_id='" . $mysqli->real_escape_string($proy_id) . "' AND emp_id=" . (int)$emp_id . " LIMIT 1");
if ($r_proy && ($rp = $r_proy->fetch_assoc())) {
    $proy_nombre = $rp['proy_nombre'] . ' (' . $proy_id . ')';
}

$ver_nombre = 'V' . $ppe_id;
$r_ver = $mysqli->query("SELECT ppe_anio, ppe_version FROM exa_ppto_cabeceras WHERE ppe_id=" . (int)$ppe_id . " LIMIT 1");
