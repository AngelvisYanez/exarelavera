...
if ($action === 'catalogos') {
    ...
    $res_proy = $mysqli_conn->query("SELECT proy_id, proy_nombre FROM exa_ppto_proyectos WHERE Emp_Cod = $Emp_Cod AND proy_estado = 'A' ORDER BY proy_nombre");
    $proyectos = array();
    if ($res_proy) {
    ...
    $res_ver = $mysqli_conn->query("SELECT ppe_id, ppe_version, ppe_descripcion, ppe_estado FROM exa_ppto_cabeceras WHERE Emp_Cod = $Emp_Cod AND ppe_anio = $anio ORDER BY ppe_version DESC");
    $versiones = array();
    if ($res_ver) {
