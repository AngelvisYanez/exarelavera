...
if ($action === 'catalogos') {
    $emp_id = isset($_GET['emp_id']) ? (int)$_GET['emp_id'] : 1;
    $anio   = isset($_GET['anio']) ? (int)$_GET['anio'] : (int)date('Y');

    // 1. Proyectos activos de la empresa
    $res_proy = $mysqli_conn->query("SELECT proy_id, proy_nombre FROM exa_ppto_proyectos WHERE emp_id = $emp_id AND proy_estado = 'A' ORDER BY proy_nombre");
    $proyectos = array();
    if ($res_proy) {
    ...
    $res_ver = $mysqli_conn->query("SELECT ppe_id, ppe_version, ppe_descripcion, ppe_estado FROM exa_ppto_cabeceras WHERE emp_id = $emp_id AND ppe_anio = $anio ORDER BY ppe_version DESC");
    $versiones = array();
    if ($res_ver) {
