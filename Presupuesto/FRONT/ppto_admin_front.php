...
if ($mysqli_conn) {
    @$mysqli_conn->query("CREATE TABLE IF NOT EXISTS `exa_ppto_reglas` (
      ...
      INDEX `idx_regla_emp_tipo` (`Emp_Cod`, `prg_tipo_documento`, `prg_prioridad`)
...
if (isset($_GET['ajax_check_partida'])) {
    ...
    $res_chk = $mysqli_conn->query("SELECT ppa_id FROM exa_ppto_partidas WHERE Emp_Cod = $emp_chk AND ppa_codigo_clasificacion = '$cla' $cond_exc LIMIT 1");
...
if (isset($_POST['guardar_partida'])) {
    ...
    if ($ppa_pad !== "NULL") {
        $res_pad = $mysqli_conn->query(
            "SELECT COALESCE(NULLIF(ppa_clase, ''), 'D') AS ppa_clase
             ...
             WHERE ppa_id = $ppa_pad AND Emp_Cod = $emp_cod AND ppa_estado = 'A'"
        );
    ...
    if ($ppa_cod) {
        $sql = "UPDATE exa_ppto_partidas SET 
                ...
                WHERE ppa_id = $ppa_cod AND Emp_Cod = $emp_cod";
    } else {
        $sql = "INSERT INTO exa_ppto_partidas (Emp_Cod, ppa_codigo_clasificacion, ppa_descripcion, ppa_tipo, ppa_naturaleza, ppa_padre_id, ppa_nivel, ppa_clase, ppa_porcentaje_tope, ppa_estado, ppa_fecha_registro, Usu_Cod)
    ...
    if ($ppa_cod && $mysqli_conn->affected_rows === 0) {
        $chk = $mysqli_conn->query("SELECT ppa_id FROM exa_ppto_partidas WHERE ppa_id = $ppa_cod AND Emp_Cod = $emp_cod LIMIT 1");
...
if (isset($_POST['guardar_regla'])) {
    ...
    if ($prg_cod) {
        $sql = "UPDATE exa_ppto_reglas SET 
                ...
                WHERE prg_id = $prg_cod AND Emp_Cod = $emp_cod";
    } else {
        $sql = "INSERT INTO exa_ppto_reglas (Emp_Cod, ppa_id, prg_tipo_documento, prg_campo_evaluacion, prg_valor_esperado, prg_signo, prg_campo_monto, prg_prioridad, prg_estado, prg_descripcion, Usu_Cod, prg_fecha_registro)
...
if (isset($_GET['prioridad_regla'])) {
    ...
    $dir = $_GET['dir'];
    $res = $mysqli_conn->query("SELECT prg_prioridad, prg_tipo_documento FROM exa_ppto_reglas WHERE prg_id = $prg_cod AND Emp_Cod = $Ses_Emp_Cod LIMIT 1");
    if ($res && $regla_actual = $res->fetch_assoc()) {
        $pri_actual = (int)$regla_actual['prg_prioridad'];
        $tip_doc = $regla_actual['prg_tipo_documento'];
        if ($dir === 'up') {
            $res_swap = $mysqli_conn->query("SELECT prg_id, prg_prioridad FROM exa_ppto_reglas WHERE Emp_Cod = $Ses_Emp_Cod AND prg_tipo_documento = '$tip_doc' AND prg_prioridad < $pri_actual ORDER BY prg_prioridad DESC LIMIT 1");
        } else {
            $res_swap = $mysqli_conn->query("SELECT prg_id, prg_prioridad FROM exa_ppto_reglas WHERE Emp_Cod = $Ses_Emp_Cod AND prg_tipo_documento = '$tip_doc' AND prg_prioridad > $pri_actual ORDER BY prg_prioridad ASC LIMIT 1");
        }
...
if (isset($_GET['estado_regla'])) {
    ...
    if ($prg_est_id > 0 && $mysqli_conn) {
        ...
        $mysqli_conn->query("UPDATE exa_ppto_reglas SET prg_estado = '$est_esc' WHERE prg_id = $prg_est_id AND Emp_Cod = $emp_est");
...
if (isset($_POST['guardar_cargar'])) {
    ...
    $mysqli_conn->query("INSERT INTO exa_ppto_cabeceras (Emp_Cod, ppe_anio, ppe_version, ppe_descripcion, ppe_estado, ppe_fecha_registro, Usu_Cod)
                         VALUES ($emp_cod, $ani, $ver, '$des', '$est', CURDATE(), $Ses_Usu_Cod)
                         ON DUPLICATE KEY UPDATE ppe_descripcion = '$des', ppe_estado = '$est'");
    
    $res_id = $mysqli_conn->query("SELECT ppe_id FROM exa_ppto_cabeceras WHERE Emp_Cod = $emp_cod AND ppe_anio = $ani AND ppe_version = $ver LIMIT 1");
    $intentos_bloqueados = 0;
    if ($res_id && $row_id = $res_id->fetch_assoc()) {
        ...
        if ($cargar_modo === 'copiar') {
            ...
            $res_origen = $mysqli_conn->query("SELECT pd.ppa_id, pd.pde_mes, pd.pde_monto 
                                               ...
                                               WHERE pp.Emp_Cod = $emp_cod AND pp.ppe_anio = $anio_origen AND pp.ppe_estado = 'A'");
            if ($res_origen) {
...
$res_anios = $mysqli_conn->query("SELECT DISTINCT ppe_anio FROM exa_ppto_cabeceras WHERE Emp_Cod = $emp_filtro ORDER BY ppe_anio DESC");
$anios = array();
if ($res_anios) {
...
$res_vers = $mysqli_conn->query("SELECT ppe_version, ppe_descripcion, ppe_estado, ppe_id FROM exa_ppto_cabeceras WHERE Emp_Cod = $emp_filtro AND ppe_anio = $ani_filtro ORDER BY ppe_version DESC");
$versiones = array();
if ($res_vers) {
