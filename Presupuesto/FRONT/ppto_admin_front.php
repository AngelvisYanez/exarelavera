...
if ($mysqli_conn) {
    @$mysqli_conn->query("CREATE TABLE IF NOT EXISTS `exa_ppto_reglas` (
      `prg_id` INT AUTO_INCREMENT PRIMARY KEY,
      `emp_id` INT NOT NULL,
      `ppa_id` INT NOT NULL,
      `prg_tipo_documento` VARCHAR(50) NOT NULL,
      `prg_campo_evaluacion` VARCHAR(100) NULL,
      `prg_valor_esperado` VARCHAR(100) NULL,
      `prg_signo` CHAR(1) NOT NULL DEFAULT '+',
      `prg_campo_monto` VARCHAR(100) NOT NULL,
      `prg_prioridad` INT NOT NULL DEFAULT 1,
      `prg_estado` CHAR(1) NOT NULL DEFAULT 'A',
      `prg_descripcion` VARCHAR(255) NULL,
      `usu_id` INT NULL,
      `prg_fecha_registro` DATE NULL,
      INDEX `idx_regla_emp_tipo` (`emp_id`, `prg_tipo_documento`, `prg_prioridad`)
...
if (isset($_GET['ajax_check_partida'])) {
    ...
    $res_chk = $mysqli_conn->query("SELECT ppa_id FROM exa_ppto_partidas WHERE emp_id = $emp_chk AND ppa_codigo_clasificacion = '$cla' $cond_exc LIMIT 1");
...
if (isset($_POST['guardar_partida'])) {
    ...
    if ($ppa_pad !== "NULL") {
        $res_pad = $mysqli_conn->query(
            "SELECT COALESCE(NULLIF(ppa_clase, ''), 'D') AS ppa_clase
             ...
             WHERE ppa_id = $ppa_pad AND emp_id = $emp_cod AND ppa_estado = 'A'"
        );
    ...
    if ($ppa_cod) {
        $sql = "UPDATE exa_ppto_partidas SET 
                    ppa_codigo_clasificacion = '$ppa_cla', ppa_descripcion = '$ppa_des', ppa_tipo = '$ppa_tip', 
                    ppa_naturaleza = '$ppa_nat', ppa_padre_id = $ppa_pad, ppa_nivel = $ppa_niv, ppa_clase = '$ppa_clase_sql', ppa_estado = '$ppa_est',
                    ppa_porcentaje_tope = " . ($ppa_clase === 'G' ? $ppa_pct_sql : 'NULL') . "
                WHERE ppa_id = $ppa_cod AND emp_id = $emp_cod";
    } else {
        $sql = "INSERT INTO exa_ppto_partidas (emp_id, ppa_codigo_clasificacion, ppa_descripcion, ppa_tipo, ppa_naturaleza, ppa_padre_id, ppa_nivel, ppa_clase, ppa_porcentaje_tope, ppa_estado, ppa_fecha_registro, usu_id)
    ...
    if ($ppa_cod && $mysqli_conn->affected_rows === 0) {
        $chk = $mysqli_conn->query("SELECT ppa_id FROM exa_ppto_partidas WHERE ppa_id = $ppa_cod AND emp_id = $emp_cod LIMIT 1");
...
if (isset($_POST['guardar_regla'])) {
    ...
    if ($prg_cod) {
        $sql = "UPDATE exa_ppto_reglas SET 
                    ppa_id = $ppa_cod, prg_tipo_documento = '$prg_tip_doc', prg_campo_evaluacion = $prg_campo, 
                    prg_valor_esperado = $prg_valor, prg_signo = '$prg_signo', prg_campo_monto = '$prg_cam_mon', 
                    prg_prioridad = $prg_pri, prg_descripcion = '$prg_des', prg_estado = '$prg_est'
                WHERE prg_id = $prg_cod AND emp_id = $emp_cod";
    } else {
        $sql = "INSERT INTO exa_ppto_reglas (emp_id, ppa_id, prg_tipo_documento, prg_campo_evaluacion, prg_valor_esperado, prg_signo, prg_campo_monto, prg_prioridad, prg_estado, prg_descripcion, usu_id, prg_fecha_registro)
...
if (isset($_GET['prioridad_regla'])) {
    ...
    $dir = $_GET['dir'];
    $res = $mysqli_conn->query("SELECT prg_prioridad, prg_tipo_documento FROM exa_ppto_reglas WHERE prg_id = $prg_cod AND emp_id = $Ses_Emp_Cod LIMIT 1");
    if ($res && $regla_actual = $res->fetch_assoc()) {
        $pri_actual = (int)$regla_actual['prg_prioridad'];
        $tip_doc = $regla_actual['prg_tipo_documento'];
        if ($dir === 'up') {
            $res_swap = $mysqli_conn->query("SELECT prg_id, prg_prioridad FROM exa_ppto_reglas WHERE emp_id = $Ses_Emp_Cod AND prg_tipo_documento = '$tip_doc' AND prg_prioridad < $pri_actual ORDER BY prg_prioridad DESC LIMIT 1");
        } else {
            $res_swap = $mysqli_conn->query("SELECT prg_id, prg_prioridad FROM exa_ppto_reglas WHERE emp_id = $Ses_Emp_Cod AND prg_tipo_documento = '$tip_doc' AND prg_prioridad > $pri_actual ORDER BY prg_prioridad ASC LIMIT 1");
        }
        if ($res_swap && $regla_swap = $res_swap->fetch_assoc()) {
            $pri_swap = (int)$regla_swap['prg_prioridad'];
            $cod_swap = (int)$regla_swap['prg_id'];
            $mysqli_conn->query("UPDATE exa_ppto_reglas SET prg_prioridad = $pri_swap WHERE prg_id = $prg_cod");
            $mysqli_conn->query("UPDATE exa_ppto_reglas SET prg_prioridad = $pri_actual WHERE prg_id = $cod_swap");
        }
    }
...
if (isset($_GET['estado_regla'])) {
    ...
    if ($prg_est_id > 0 && $mysqli_conn) {
        ...
        $mysqli_conn->query("UPDATE exa_ppto_reglas SET prg_estado = '$est_esc' WHERE prg_id = $prg_est_id AND emp_id = $emp_est");
...
if (isset($_GET['marcar_leida'])) {
    ...
    $mysqli_conn->query("UPDATE exa_ppto_alertas SET pal_leido = 'L' WHERE pal_id = $pal_cod");
...
if (isset($_POST['guardar_mensual'])) {
    ...
    foreach ($valores as $ppa_cod => $meses) {
        ...
        foreach ($meses as $mes => $monto) {
            $mes = (int)$mes;
            $monto = (float)$monto;
            $mysqli_conn->query("INSERT INTO exa_ppto_detalles (ppe_id, ppa_id, pde_mes, pde_monto) 
                                VALUES ($ppe_cod, $ppa_cod, $mes, $monto) 
                                ON DUPLICATE KEY UPDATE pde_monto = $monto");
        }
    }
...
if (isset($_POST['guardar_cargar'])) {
    ...
    $mysqli_conn->query("INSERT INTO exa_ppto_cabeceras (emp_id, ppe_anio, ppe_version, ppe_descripcion, ppe_estado, ppe_fecha_registro, usu_id)
                         VALUES ($emp_cod, $ani, $ver, '$des', '$est', CURDATE(), $Ses_Usu_Cod)
                         ON DUPLICATE KEY UPDATE ppe_descripcion = '$des', ppe_estado = '$est'");
    
    $res_id = $mysqli_conn->query("SELECT ppe_id FROM exa_ppto_cabeceras WHERE emp_id = $emp_cod AND ppe_anio = $ani AND ppe_version = $ver LIMIT 1");
    $intentos_bloqueados = 0;
    if ($res_id && $row_id = $res_id->fetch_assoc()) {
        ...
        if ($cargar_modo === 'copiar') {
            ...
            $res_origen = $mysqli_conn->query("SELECT pd.ppa_id, pd.pde_mes, pd.pde_monto 
                                               FROM exa_ppto_detalles pd 
                                               INNER JOIN exa_ppto_cabeceras pp ON pd.ppe_id = pp.ppe_id 
                                               WHERE pp.emp_id = $emp_cod AND pp.ppe_anio = $anio_origen AND pp.ppe_estado = 'A'");
            if ($res_origen) {
                while ($row_o = $res_origen->fetch_assoc()) {
                    ...
                    $mysqli_conn->query("INSERT INTO exa_ppto_detalles (ppe_id, ppa_id, pde_mes, pde_monto) 
                                        ...
                                        ON DUPLICATE KEY UPDATE pde_monto = $monto_nuevo");
                }
            }
            ...
            foreach ($valores_anual as $ppa_cod => $monto_anual) {
                ...
                for ($mes = 1; $mes <= 12; $mes++) {
                    $mysqli_conn->query("INSERT INTO exa_ppto_detalles (ppe_id, ppa_id, pde_mes, pde_monto) 
                                        ...
                                        ON DUPLICATE KEY UPDATE pde_monto = $monto_mensual");
                }
            }
        } else {
...
$res_anios = $mysqli_conn->query("SELECT DISTINCT ppe_anio FROM exa_ppto_cabeceras WHERE emp_id = $emp_filtro ORDER BY ppe_anio DESC");
$anios = array();
if ($res_anios) {
...
$res_vers = $mysqli_conn->query("SELECT ppe_version, ppe_descripcion, ppe_estado, ppe_id FROM exa_ppto_cabeceras WHERE emp_id = $emp_filtro AND ppe_anio = $ani_filtro ORDER BY ppe_version DESC");
$versiones = array();
if ($res_vers) {
...
$partidas_activas = ppto_partidas_listar($mysqli_conn, array('emp_id' => $emp_filtro, 'solo_activas' => true));
$partidas_grupo = ppto_partidas_listar($mysqli_conn, array('emp_id' => $emp_filtro, 'solo_activas' => true, 'clase' => 'G'));
$partidas_detalle = ppto_partidas_listar($mysqli_conn, array('emp_id' => $emp_filtro, 'solo_activas' => true, 'clase' => 'D'));
...
$res_reglas = $mysqli_conn->query("SELECT r.*, p.ppa_codigo_clasificacion, p.ppa_descripcion, p.ppa_estado, p.ppa_clase
                                   FROM exa_ppto_reglas r 
                                   INNER JOIN exa_ppto_partidas p ON r.ppa_id = p.ppa_id 
                                   WHERE r.emp_id = $emp_filtro "
                                   . ($ver_reglas_inactivas ? '' : " AND r.prg_estado = 'A' ")
                                   . "ORDER BY r.prg_tipo_documento, r.prg_prioridad ASC");
$reglas_todas = array();
if ($res_reglas) {
