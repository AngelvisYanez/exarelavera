...
if ($vista_periodo === 'anual') {
    ...
    $periodo_lbl = 'Acumulado ene–' . strtolower(ppto_nombre_mes($mes_consulta));
        ...
        <?php elseif ($active_tab === 2): ?>
            ...
            if (!empty($reporte_datos)) {
                foreach ($reporte_datos as $row) {
                    $clase_row = isset($row['ppa_clase']) ? $row['ppa_clase'] : 'D';
                    $match_tipo = ($filtro_tipo === 'All' || $row['ppa_tipo'] === $filtro_tipo);
                    $match_nat = ($filtro_nat === 'All' || $row['ppa_naturaleza'] === $filtro_nat);
                    ...
                    <tbody>
                        <?php if (!empty($filtered_partidas)): ?>
                            <?php foreach ($filtered_partidas as $p_row):
                                ...
                                $nivel_row = isset($p_row['ppa_nivel']) ? (int)$p_row['ppa_nivel'] : ppto_partida_nivel_desde_codigo($p_row['ppa_codigo_clasificacion']);
                                $indent = ppto_partida_indent_px($nivel_row);
                                $tr_style = ($clase_row === 'G') ? ' style="background:#f8fafc;font-weight:600;"' : '';
                            ?>
                                <tr<?php echo $tr_style; ?>>
                                    <td><strong><?php echo htmlspecialchars($p_row['ppa_codigo_clasificacion']); ?></strong></td>
                                    <td style="padding-left:<?php echo (int)$indent; ?>px;"><?php echo htmlspecialchars($p_row['ppa_descripcion']); ?></td>
                                    <td>
                                        <?php if ($clase_row === 'G'): ?>
                                    ...
                                    <td><?php echo htmlspecialchars(ppto_tipo_contable_etiqueta($p_row['ppa_tipo'])); ?></td>
                                    ...
                                    <td>
                                        <button class="btn btn-primary btn-xs" type="button" onclick="verDetallePartida(<?php echo $ppe_cod_filtro; ?>, <?php echo $p_row['ppa_id']; ?>, '<?php echo addslashes($p_row['ppa_descripcion']); ?>', <?php echo $mes_acumulado; ?>)">
            ...
            try {
                ...
                if (data && data.length > 0) {
                    data.forEach(function (row) {
                        ...
                        tr.innerHTML = '<td>' + row.pej_fecha_documento + '</td>' +
                                       '<td style="text-transform:capitalize;">' + row.pej_tipo_documento.replace('_', ' ') + '</td>' +
                                       '<td>' + row.pej_documento_codigo + '</td>' +
                                       '<td style="font-weight:600;">' + formatCurrency(row.pej_monto) + '</td>' +
                                       '<td style="font-weight:bold; color:' + (row.pej_signo === '+' ? '#38a169' : '#e53e3e') + ';">' + row.pej_signo + '</td>';
