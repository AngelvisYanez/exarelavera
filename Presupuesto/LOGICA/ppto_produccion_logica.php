...
if ($action === 'get_config') {
    ...
    $config = null;
    $res = $mysqli->query("SELECT * FROM exa_ppto_prod_config WHERE proy_id='$proy_id' AND Emp_Cod=$Emp_Cod LIMIT 1");
...
if ($action === 'list') {
    ...
    $res = $mysqli->query("SELECT c.*, p.proy_nombre FROM exa_ppto_prod_config c
        INNER JOIN exa_ppto_proyectos p ON c.proy_id = p.proy_id AND c.Emp_Cod = p.Emp_Cod
        WHERE c.Emp_Cod = $Emp_Cod ORDER BY p.proy_nombre");
    if ($res) {
...
if ($action === 'list_periodos') {
    ...
    $anio = (int)$_GET['anio'];
    $rows = array();
    $res = $mysqli->query("SELECT * FROM exa_ppto_prod_periodos WHERE proy_id='$proy_id' AND Emp_Cod=$Emp_Cod AND prd_anio=$anio ORDER BY prd_mes");
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            $rows[] = $r;
        }
...
if ($action === 'proyectos') {
    $rows = array();
    $res = $mysqli->query("SELECT proy_id, proy_nombre FROM exa_ppto_proyectos WHERE Emp_Cod=$Emp_Cod AND proy_estado='A' ORDER BY proy_nombre");
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            $rows[] = $r;
        }
