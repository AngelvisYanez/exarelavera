...
if ($action === 'get_config') {
    ...
    $config = null;
    $res = $mysqli->query("SELECT * FROM exa_ppto_prod_config WHERE proy_id='$proy_id' AND emp_id=$emp_id LIMIT 1");
...
if ($action === 'list') {
    $rows = array();
    $res = $mysqli->query("SELECT c.*, p.proy_nombre FROM exa_ppto_prod_config c
        INNER JOIN exa_ppto_proyectos p ON c.proy_id = p.proy_id AND c.emp_id = p.emp_id
        WHERE c.emp_id = $emp_id ORDER BY p.proy_nombre");
    if ($res) {
...
if ($action === 'save_config') {
    ...
    $sql = "INSERT INTO exa_ppto_prod_config
            (proy_id, emp_id, pco_origen, pco_campo, pco_frecuencia, pco_extra_config, pco_fecha_registro, usu_id)
            VALUES ('$proy_id', $emp_id, '$origen', '$campo', 'mensual', '$extra', NOW(), " . (int)$Ses_Usu_Cod . ")
...
if ($action === 'save_plan') {
    ...
    ppto_integracion_produccion_registrar($mysqli, $proy_id, $mes, $esperada, 'esperada', $anio, $emp_id);
    ...
    $d2 = ppto_divergencia_comparar_toneladas($mysqli, $proy_id, $emp_id, $anio, $ppe_id ? (int)$ppe_id : null);
...
if ($action === 'list_periodos') {
    ...
    $anio = (int)$_GET['anio'];
    $rows = array();
    $res = $mysqli->query("SELECT * FROM exa_ppto_prod_periodos WHERE proy_id='$proy_id' AND emp_id=$emp_id AND prd_anio=$anio ORDER BY prd_mes");
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            $rows[] = $r;
        }
    }
    $ppe_id = ppto_proy_version_ppe_id_por_anio($mysqli, $emp_id, $anio);
    $ton_pdf = ($ppe_id > 0) ? ppto_proy_version_ton_base($mysqli, $proy_id, $emp_id, $ppe_id) : 0.0;
    $aprobaciones = array();
    if ($ppe_id > 0) {
        require_once(__DIR__ . '/ppto_proy_publicar_logica.php');
        $map = ppto_proy_publicar_aprobaciones_mes($mysqli, trim($_GET['proy_id']), $emp_id, $ppe_id, $anio);
        $usu_ids = array();
        foreach ($map as $row) {
            $uid = (int)$row['usu_id'];
            if ($uid > 0) {
...
if ($action === 'hist_aprobaciones_mes') {
    ...
    $hist = ppto_proy_publicar_historial_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio, $mes);
    $usu_ids = array();
    foreach ($hist as $h) {
        $u = (int)$h['usu_id'];
        if ($u > 0) { $usu_ids[$u] = $u; }
...
if ($action === 'preview_aprobar_mes' || $action === 'aprobar_mes') {
    ...
    if ($action === 'preview_aprobar_mes') {
        $prev = ppto_proy_publicar_preview_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio, $mes, $ton_override);
        if (empty($prev['ok'])) {
    ...
    $res = ppto_proy_publicar_ejecutar_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio, $mes, (int)$Ses_Usu_Cod, $ton_override, $confirmar);
...
if ($action === 'sync_esperada_pdf') {
    ...
    $sync = ppto_prod_sync_esperada_desde_ton_base($mysqli, $proy_id, $emp_id, $anio, 0, array(
...
if ($action === 'sync_relavera') {
    $proy_id = $mysqli->real_escape_string(trim($_POST['proy_id']));
    $anio = isset($_POST['anio']) ? (int)$_POST['anio'] : (int)date('Y');
    ppto_sync_relavera_produccion($mysqli, $proy_id, $emp_id, $anio);
...
if ($action === 'save_cuadro') {
    ...
    foreach ($items as $item) {
        ...
        $ok1 = ppto_integracion_produccion_registrar($mysqli, $proy_id, $mes, $esperada, 'esperada', $anio, $emp_id);
        $ok2 = ppto_integracion_produccion_registrar($mysqli, $proy_id, $mes, $proyectada, 'proyectada', $anio, $emp_id);
        if ($ok1 && $ok2) {
            $guardados++;
        }
    }
...
if ($action === 'save_proyectadas') {
    ...
    foreach ($items as $item) {
        ...
        if (ppto_integracion_produccion_registrar($mysqli, $proy_id, $mes, $valor, 'proyectada', $anio, $emp_id)) {
            $guardados++;
        }
    }
    ppto_prod_json(array(
...
if ($action === 'save_proyectada') {
    ...
    $ok = ppto_integracion_produccion_registrar($mysqli, $proy_id, $mes, $valor, 'proyectada', $anio, $emp_id);
    ppto_prod_json(array(
...
if ($action === 'insert_proyectada') {
    ...
    $result = ppto_integracion_proyectar_promedio_siguiente_mes($mysqli, $proy_id, $anio, $mes_corte, $emp_id, $mes_destino);
    ppto_prod_json(array(
...
if ($action === 'proyectos') {
    $rows = array();
    $res = $mysqli->query("SELECT proy_id, proy_nombre FROM exa_ppto_proyectos WHERE emp_id=$emp_id AND proy_estado='A' ORDER BY proy_nombre");
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            $rows[] = $r;
        }
