...
function ppto_param_ejecutado_resolver_ppe($mysqli, $Emp_Cod, $anio, $ppe_id_preferido = 0) {
    ...
    if ($ppe_id_preferido > 0) {
        $res = $mysqli->query("SELECT ppe_id FROM exa_ppto_cabeceras
            WHERE ppe_id=$ppe_id_preferido AND Emp_Cod=$Emp_Cod LIMIT 1");
    ...
    $res = $mysqli->query("SELECT ppe_id FROM exa_ppto_cabeceras
        WHERE Emp_Cod=$Emp_Cod AND ppe_anio=$anio
...
function ppto_param_ejecutado_limpiar_rango($mysqli, $Emp_Cod, $ppe_id, $anio, $mes_desde, $mes_hasta) {
    ...
    $ok = $mysqli->query("DELETE FROM exa_ppto_ejecuciones
        WHERE Emp_Cod=$Emp_Cod AND ppe_id=$ppe_id AND pej_anio=$anio
