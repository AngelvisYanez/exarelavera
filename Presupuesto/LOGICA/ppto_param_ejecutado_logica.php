 ...
 * @return int
 */
function ppto_param_ejecutado_resolver_ppe($mysqli, $emp_id, $anio, $ppe_id_preferido = 0) {
    $emp_id = (int)$emp_id;
    $anio = (int)$anio;
    $ppe_id_preferido = (int)$ppe_id_preferido;
    if ($ppe_id_preferido > 0) {
        $res = $mysqli->query("SELECT ppe_id FROM exa_ppto_cabeceras
            WHERE ppe_id=$ppe_id_preferido AND emp_id=$emp_id LIMIT 1");
        if ($res && $res->fetch_assoc()) {
            return $ppe_id_preferido;
        }
    }
    $activo = ppto_persistencia_consultar($mysqli, 1, array('emp_id' => $emp_id, 'ppe_anio' => $anio));
    if ($activo) {
        return (int)$activo;
    }
    $res = $mysqli->query("SELECT ppe_id FROM exa_ppto_cabeceras
        WHERE emp_id=$emp_id AND ppe_anio=$anio
        ORDER BY ppe_version DESC LIMIT 1");
 ...
 */
function ppto_param_ejecutado_mapa_proy_por_ppa($mysqli, $emp_id, $ppe_id) {
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $map = array();
 ...
 */
function ppto_param_ejecutado_calcular_mes($mysqli, $emp_id, $pla_cod, $pec_cod, $anio, $mes) {
    $emp_id = (int)$emp_id;
    $pla_cod = (int)$pla_cod;
    $pec_cod = (int)$pec_cod;
    $anio = (int)$anio;
    $mes = (int)$mes;
    $out = array();
    if ($emp_id <= 0 || $pla_cod <= 0 || $pec_cod <= 0 || $mes < 1 || $mes > 12) {
        return $out;
    }
 ...
 * @return array
 */
function ppto_param_ejecutado_preview($mysqli, $emp_id, $anio, $mes_desde = 1, $mes_hasta = 12, $ppe_id = 0) {
    $emp_id = (int)$emp_id;
    $anio = (int)$anio;
    ...
    $ppe = ppto_param_ejecutado_resolver_ppe($mysqli, $emp_id, $anio, $ppe_id);
    if ($ppe <= 0) {
    ...
    $res = $mysqli->query("SELECT COUNT(*) AS n FROM exa_ppto_partida_cuenta
        WHERE emp_id=$emp_id AND pla_cod=" . (int)$plan['pla_cod'] . " AND ppc_estado='A'");
    if ($res && ($r = $res->fetch_assoc())) {
        $mapa_n = (int)$r['n'];
    }
    if ($mapa_n < 1) {
 ...
 */
function ppto_param_ejecutado_limpiar_rango($mysqli, $emp_id, $ppe_id, $anio, $mes_desde, $mes_hasta) {
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $anio = (int)$anio;
    ...
    // Vista updatable sobre pre_ejecucion
    $ok = $mysqli->query("DELETE FROM exa_ppto_ejecuciones
        WHERE emp_id=$emp_id AND ppe_id=$ppe_id AND pej_anio=$anio
          AND pej_mes BETWEEN $mes_desde AND $mes_hasta
          AND pej_tipo_documento='$tipo'");
    if (!$ok) {
        $ok = $mysqli->query("DELETE FROM pre_ejecucion
            WHERE Emp_Cod=$emp_id AND Ppe_Cod=$ppe_id AND Pej_Ani=$anio
              AND Pej_Mes BETWEEN $mes_desde AND $mes_hasta
              AND Pej_TipDoc='$tipo'");
    }
 ...
 * @return array
 */
function ppto_param_ejecutado_sincronizar($mysqli, $emp_id, $anio, $mes_desde = 1, $mes_hasta = 12, $usu_id = 0, $ppe_id = 0) {
    $prev = ppto_param_ejecutado_preview($mysqli, $emp_id, $anio, $mes_desde, $mes_hasta, $ppe_id);
    if (empty($prev['ok'])) {
        return $prev;
    }
    ...
    $usu_id = (int)$usu_id;
    $borrados = ppto_param_ejecutado_limpiar_rango($mysqli, $emp_id, $ppe, $anio, $mes_desde, $mes_hasta);
    $mapa_proy = ppto_param_ejecutado_mapa_proy_por_ppa($mysqli, $emp_id, $ppe);
    ...
    for ($m = $mes_desde; $m <= $mes_hasta; $m++) {
        $calc = ppto_param_ejecutado_calcular_mes($mysqli, $emp_id, $pla_cod, $pec_cod, $anio, $m);
        ...
        foreach ($calc as $row) {
            ...
            if ($id) {
                ...
                $sql = "INSERT INTO pre_ejecucion
                    ...
                    VALUES ($ppe, {$row['ppa_id']}, $emp_id, $proy_sql, $m, $anio, '" . PPTO_PEJ_TIPO_MAYOR . "',
                ...
                if (@$mysqli->query($sql)) {
                    ...
                    $sql2 = "INSERT INTO pre_ejecucion
                        ...
                        VALUES ($ppe, {$row['ppa_id']}, $emp_id, $m, $anio, '" . PPTO_PEJ_TIPO_MAYOR . "',
                            '" . $mysqli->real_escape_string($doc) . "', " . (float)$row['monto'] . ", '+', '$fec',
                            " . ($usu_id > 0 ? $usu_id : 1) . ", 'E')";
