...
function ppto_hook_compras($arg1, $arg2 = null, $arg3 = null) {
    ...
    // 2. VALIDACI�N 1: Existe id_rubro en la versi�n activa del proyecto
    if (!empty($id_proyecto) && !empty($rubro)) {
        ...
        $sql_chk = "SELECT pdp_id FROM pre_proyecto_detalles 
                    WHERE ppe_id = $ppe_id 
                      AND ppa_id = $ppa_id 
        ...
        if (!$res_chk || $res_chk->num_rows === 0) {
            return array(
                'status' => 'error',
                'message' => "El rubro '$rubro' no existe en la versi�n activa del proyecto '$id_proyecto'.",
                'id_movimiento' => null
            );
        }
    }

    // 3. VALIDACI�N 2: Hay presupuesto disponible (solo para comprometido)
    if ($tipo_mov === 'comprometido') {
        $disponible = 0.00;
        if (!empty($id_proyecto) && !empty($rubro)) {
            // Verificaci�n en la distribuci�n mensual del proyecto
            $clean_proy = $mysqli_obj->real_escape_string($id_proyecto);
            $clean_rubro = $mysqli_obj->real_escape_string($rubro);
            $sql_avail = "SELECT (pdm_presupuesto_mensual - pdm_ejecutado - pdm_comprometido) AS disponible 
                          FROM pre_proyecto_detalles_mes pdm
                          INNER JOIN pre_proyecto_detalles pd ON pdm.pdp_id = pd.pdp_id
    ...
    // 4. VALIDACI�N 3: No duplicar: mismo id_orden + mismo evento ya registrado
...
function ppto_vigente_obtener($mysqli, $ppe_id, $ppa_id, $proy_id = null, $rubro = null, $mes = null) {
    ...
    if (!empty($proy_id) && !empty($rubro)) {
        ...
        $sql = "SELECT pdm.pdm_presupuesto_mensual
                ...
                INNER JOIN pre_proyecto_detalles_mes pdm ON pd.pdp_id = pdm.pdp_id
                WHERE pd.ppe_id = $ppe_id 
                  AND pd.ppa_id = $ppa_id 
                  AND pd.proy_id = '$clean_proy' 
                  AND pd.pdp_rubro = '$clean_rubro'";
...
function ppto_hook_modulo($mysqli, $id, $evento) {
    ...
    // 2. VALIDACI�N A: Existe partida/rubro asignada en el proyecto (si aplica)
    if (!empty($proy_id) && !empty($rubro)) {
        $clean_proy = $mysqli->real_escape_string($proy_id);
        $clean_rubro = $mysqli->real_escape_string($rubro);
        ...
        if (!$res_chk || $res_chk->num_rows === 0) {
            return false; // Combinaci�n rubro/partida no pertenece al proyecto asignado
        }
    }

    // 3. VALIDACI�N B: Presupuesto disponible (solo para fase comprometido)
    if ($tipo_mov === 'comprometido') {
        $disponible = 0.00;
        if (!empty($proy_id) && !empty($rubro)) {
            // Verificaci�n en la distribuci�n mensual del proyecto
