...
function ppto_pdf_importar_presupuesto($mysqli, $Emp_Cod, $Usu_Cod, $proy_id, $ppe_id, $parsed, $ton_override = 0) {
    ...
    $sql_ton = "INSERT INTO pre_proyecto_version
            (proy_id, Emp_Cod, ppe_id, pv_toneladas_base_mes, pv_toneladas_costo_mes, pv_fecha_registro, Usu_Cod)
            VALUES ('$proy_esc', " . (int)$Emp_Cod . ", $ppe_id, $ton_ingreso, $ton_costo, NOW(), " . (int)$Usu_Cod . ")
            ON DUPLICATE KEY UPDATE pv_toneladas_base_mes=$ton_ingreso, pv_toneladas_costo_mes=$ton_costo, pv_fecha_registro=NOW(), Usu_Cod=" . (int)$Usu_Cod;
    ...
    foreach ($rubros as $r) {
        ...
        if ($pdp_id > 0) {
            $sql = "UPDATE pre_proyecto_detalles
                ...
                WHERE pdp_id=$pdp_id AND proy_id='$proy_esc' AND Emp_Cod=" . (int)$Emp_Cod . " AND ppe_id=$ppe_id";
            if ($mysqli->query($sql)) {
            ...
            $sql = "INSERT INTO pre_proyecto_detalles
                ...
                VALUES ($ppe_id, $ppa_id, '$proy_esc', " . (int)$Emp_Cod . ", '$rubro', $ton_rubro, $factor, $anual, NOW(), " . (int)$Usu_Cod . ")";
            if ($mysqli->query($sql)) {
