...
function ppto_pdf_partida_upsert($mysqli, $Emp_Cod, $codigo, $descripcion, $opts) {
    ...
    $res = $mysqli->query("SELECT ppa_id FROM exa_ppto_partidas WHERE Emp_Cod=$Emp_Cod AND ppa_codigo_clasificacion='$codigo_esc' LIMIT 1");
    if ($res && $row = $res->fetch_assoc()) {
        $ppa_id = (int)$row['ppa_id'];
        if ($des_esc !== '') {
            $mysqli->query("UPDATE exa_ppto_partidas SET ppa_descripcion='$des_esc' WHERE ppa_id=$ppa_id AND Emp_Cod=$Emp_Cod LIMIT 1");
        }
        return $ppa_id;
    }
...
function ppto_pdf_importar_presupuesto($mysqli, $Emp_Cod, $Usu_Cod, $proy_id, $ppe_id, $parsed, $ton_override = 0) {
    ...
    foreach ($partidas as $p) {
        $cod = $p['codigo'];
        $res_exist = $mysqli->query("SELECT ppa_id FROM exa_ppto_partidas WHERE Emp_Cod=" . (int)$Emp_Cod . " AND ppa_codigo_clasificacion='" . $mysqli->real_escape_string($cod) . "' LIMIT 1");
        ...
        if ($ppa_id > 0) {
            ...
            if (ppto_pdf_normalizar_clase($p['clase']) === 'G' && isset($p['meses_prorrateo'])) {
                ...
                if ($meses_g > 0) {
                    ...
                    $mysqli->query("UPDATE exa_ppto_partidas SET ppa_meses_prorrateo = $meses_g WHERE ppa_id = $ppa_id AND Emp_Cod = " . (int)$Emp_Cod);
                }
            }
        }
    }
