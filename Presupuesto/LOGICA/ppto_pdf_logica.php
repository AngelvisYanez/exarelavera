...
function ppto_pdf_catalogo_por_codigos($mysqli, $Emp_Cod, $codigos) {
    ...
    $sql = "SELECT Ppa_Cla AS ppa_codigo_clasificacion, COALESCE(NULLIF(Ppa_Clase, ''), 'D') AS ppa_clase, Ppa_Des AS ppa_descripcion
            FROM pre_partidas
...
function ppto_pdf_partida_upsert($mysqli, $Emp_Cod, $codigo, $descripcion, $opts) {
    ...
    $res = $mysqli->query("SELECT Ppa_Cod AS ppa_id FROM pre_partidas WHERE Emp_Cod=$Emp_Cod AND Ppa_Cla='$codigo_esc' LIMIT 1");
    if ($res && $row = $res->fetch_assoc()) {
        $ppa_id = (int)$row['ppa_id'];
        if ($des_esc !== '') {
            $mysqli->query("UPDATE pre_partidas SET Ppa_Des='$des_esc' WHERE Ppa_Cod=$ppa_id AND Emp_Cod=$Emp_Cod LIMIT 1");
        }
        return $ppa_id;
    }
 ...
 * @return array
 */
function ppto_pdf_importar_presupuesto($mysqli, $Emp_Cod, $Usu_Cod, $proy_id, $ppe_id, $parsed, $ton_override = 0) {
    ...
    foreach ($partidas as $p) {
        $cod = $p['codigo'];
        $res_exist = $mysqli->query("SELECT Ppa_Cod AS ppa_id FROM pre_partidas WHERE Emp_Cod=" . (int)$Emp_Cod . " AND Ppa_Cla='" . $mysqli->real_escape_string($cod) . "' LIMIT 1");
        ...
        $ppa_id = ppto_pdf_partida_upsert($mysqli, $Emp_Cod, $cod, $p['descripcion'], array(
            'tipo' => 'G',
            'naturaleza' => 'OPE',
            'clase' => $p['clase'],
            'padre_id' => $padre_id,
        ...
        if ($ppa_id > 0) {
            ...
            if (ppto_pdf_normalizar_clase($p['clase']) === 'G' && isset($p['meses_prorrateo'])) {
                ...
                if ($meses_g > 0) {
                    ...
                    $mysqli->query("UPDATE pre_partidas SET Ppa_Meses = $meses_g WHERE Ppa_Cod = $ppa_id AND Emp_Cod = " . (int)$Emp_Cod);
                }
            }
        }
    }
    ...
    $sql_ton = "INSERT INTO pre_proyecto_version
            ...
            VALUES (COALESCE((SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc' OR Pro_Cod='$proy_esc' LIMIT 1), 0), " . (int)$Emp_Cod . ", $ppe_id, $ton_ingreso, $ton_costo, NOW(), " . (int)$Usu_Cod . ")
    ...
    foreach ($rubros as $r) {
        ...
        if (!isset($cod_to_id[$cod])) {
            ...
            $ppa_id = ppto_pdf_partida_upsert($mysqli, $Emp_Cod, $cod, $r['descripcion'], array(
                'tipo' => 'G',
                'naturaleza' => 'OPE',
                'clase' => 'D',
                'padre_id' => $padre_id,
        ...
        if ($pdp_id > 0) {
            $sql = "UPDATE pre_proyecto_detalles
                ...
                WHERE Pdp_Cod=$pdp_id AND (Pro_Cod='$proy_esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc')) AND Emp_Cod=" . (int)$Emp_Cod . " AND Ppe_Cod=$ppe_id";
            if ($mysqli->query($sql)) {
                $stats['rubros_actualizados']++;
                $stats['rubros_duplicados_eliminados'] += ppto_proy_rubro_purgar_duplicados_partida(
                    $mysqli, $ppe_id, $ppa_id, $proy_id, $Emp_Cod, $pdp_id
                );
            ...
            $sql = "INSERT INTO pre_proyecto_detalles
                ...
                VALUES ($ppe_id, $ppa_id, COALESCE((SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc' OR Pro_Cod='$proy_esc' LIMIT 1), 0), " . (int)$Emp_Cod . ", '$rubro', $ton_rubro, $factor, $anual, NOW(), " . (int)$Usu_Cod . ")";
            if ($mysqli->query($sql)) {
