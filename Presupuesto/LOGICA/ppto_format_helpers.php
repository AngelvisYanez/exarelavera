 ...
 * @return bool
 */
function ppto_usuario_es_admin_db($mysqli, $usu_id, $emp_id) {
    if (!$mysqli || (int)$usu_id <= 0) {
        return false;
    }
    $usu_id = (int)$usu_id;
    $emp_id = (int)$emp_id;
    $sql = "SELECT p.Per_Des FROM usuarperfi up
            ...
            WHERE up.Usu_Cod = $usu_id";
    if ($emp_id > 0) {
        $sql .= " AND p.Emp_Cod = $emp_id";
    }
 ...
 * @return array int[]
 */
function ppto_consulta_ppa_ids_proyecto($mysqli, $emp_id, $ppe_id) {
    $out = array();
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
