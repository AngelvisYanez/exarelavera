...
function ppto_schema_ensure_ajuste_financiero($mysqli) {
    ...
    $cols = array(
        'pv_costo_capital_pct' => "ADD COLUMN pv_costo_capital_pct DECIMAL(8,4) NOT NULL DEFAULT 11.0000 COMMENT 'Porcentaje costo de capital sobre precio neto' AFTER pv_iva_divisor",
        'pv_gad_monto_objetivo' => "ADD COLUMN pv_gad_monto_objetivo DECIMAL(14,2) NOT NULL DEFAULT 2000000.00 COMMENT 'Monto objetivo recuperacion GAD' AFTER pv_costo_capital_pct",
        'pv_gad_factor_ton' => "ADD COLUMN pv_gad_factor_ton DECIMAL(12,6) NOT NULL DEFAULT 0.198400 COMMENT 'USD recuperacion GAD por tonelada' AFTER pv_gad_monto_objetivo",
        'pv_gad_recuperado_acum' => "ADD COLUMN pv_gad_recuperado_acum DECIMAL(14,2) NOT NULL DEFAULT 0.00 COMMENT 'Recuperacion GAD acumulada aplicada' AFTER pv_gad_factor_ton",
        'pv_ajuste_activo' => "ADD COLUMN pv_ajuste_activo TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1=usar partida final en cuadro' AFTER pv_gad_recuperado_acum",
    );
    $res_tbl = @$mysqli->query("SHOW TABLES LIKE 'exa_ppto_proyecto_version'");
    if ($res_tbl && $res_tbl->num_rows > 0) {
        foreach ($cols as $name => $ddl) {
            $c = @$mysqli->query("SHOW COLUMNS FROM exa_ppto_proyecto_version LIKE '" . $mysqli->real_escape_string($name) . "'");
            if (!$c || $c->num_rows === 0) {
                @$mysqli->query("ALTER TABLE exa_ppto_proyecto_version " . $ddl);
            }
        }
    }

    $res_p = @$mysqli->query("SHOW TABLES LIKE 'exa_ppto_proyecto_precio_anio'");
    if (!$res_p || $res_p->num_rows === 0) {
        @$mysqli->query("CREATE TABLE IF NOT EXISTS exa_ppto_proyecto_precio_anio (
    ...
    $res_c = @$mysqli->query("SHOW TABLES LIKE 'exa_ppto_ajuste_fin_cab'");
    if (!$res_c || $res_c->num_rows === 0) {
        @$mysqli->query("CREATE TABLE IF NOT EXISTS exa_ppto_ajuste_fin_cab (
    ...
    $res_d = @$mysqli->query("SHOW TABLES LIKE 'exa_ppto_ajuste_fin_det'");
    if (!$res_d || $res_d->num_rows === 0) {
        @$mysqli->query("CREATE TABLE IF NOT EXISTS exa_ppto_ajuste_fin_det (
 ...
 * @return array
 */
function ppto_ajuste_cfg_get($mysqli, $proy_id, $emp_id, $ppe_id) {
    ...
    $res = $mysqli->query("SELECT pv_costo_capital_pct, pv_gad_monto_objetivo, pv_gad_factor_ton,
            pv_gad_recuperado_acum, pv_ajuste_activo, pv_tarifa_ton_iva, pv_iva_divisor
        FROM exa_ppto_proyecto_version
        WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id LIMIT 1");
 ...
 * @return array
 */
function ppto_ajuste_cfg_save($mysqli, $proy_id, $emp_id, $ppe_id, $data, $usu_id) {
    ppto_schema_ensure_ajuste_financiero($mysqli);
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    ...
    $base = ppto_proy_version_config($mysqli, $proy_id, $emp_id, $ppe_id);
    ...
    $sql = "INSERT INTO exa_ppto_proyecto_version
            (proy_id, emp_id, ppe_id, pv_toneladas_base_mes, pv_toneladas_costo_mes, pv_tarifa_ton_iva, pv_iva_divisor,
             pv_costo_capital_pct, pv_gad_monto_objetivo, pv_gad_factor_ton, pv_ajuste_activo, pv_fecha_registro, usu_id)
            VALUES ('$esc', $emp_id, $ppe_id, $ton, $ton_costo, $tarifa, $iva,
             $pct, $obj, $fac, $activo, NOW(), $usu_id)
            ON DUPLICATE KEY UPDATE
             pv_costo_capital_pct=$pct,
             pv_gad_monto_objetivo=$obj,
             pv_gad_factor_ton=$fac,
             pv_ajuste_activo=$activo,
             pv_fecha_registro=NOW(),
             usu_id=$usu_id" . $acum_sql;
    if (!$mysqli->query($sql)) {
        return array('ok' => false, 'message' => $mysqli->error);
    }
    return array('ok' => true, 'message' => 'Configuracion de ajustes guardada.', 'cfg' => ppto_ajuste_cfg_get($mysqli, $proy_id, $emp_id, $ppe_id));
}
 ...
 * @return array
 */
function ppto_ajuste_precios_list($mysqli, $proy_id, $emp_id, $ppe_id) {
    ppto_schema_ensure_ajuste_financiero($mysqli);
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    ...
    $res = $mysqli->query("SELECT ppa_anio, ppa_tarifa_ton_iva
        FROM exa_ppto_proyecto_precio_anio
        WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id
 ...
 * @return array
 */
function ppto_ajuste_precios_save($mysqli, $proy_id, $emp_id, $ppe_id, $precios, $usu_id) {
    ppto_schema_ensure_ajuste_financiero($mysqli);
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    ...
    $mysqli->query("DELETE FROM exa_ppto_proyecto_precio_anio
        WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id");
    foreach ($precios as $p) {
        ...
        $mysqli->query("INSERT INTO exa_ppto_proyecto_precio_anio
            (proy_id, emp_id, ppe_id, ppa_anio, ppa_tarifa_ton_iva, ppa_fecha_registro, usu_id)
            VALUES ('$esc', $emp_id, $ppe_id, $anio, $tarifa, NOW(), $usu_id)");
    }
    return array('ok' => true, 'message' => 'Proyeccion de precios guardada.', 'precios' => ppto_ajuste_precios_list($mysqli, $proy_id, $emp_id, $ppe_id));
}
 ...
 * @return array
 */
function ppto_ajuste_precio_para_anio($mysqli, $proy_id, $emp_id, $ppe_id, $anio) {
    ...
    $ppe_id = (int)$ppe_id;
    if ($anio > 0) {
        $res = $mysqli->query("SELECT ppa_tarifa_ton_iva FROM exa_ppto_proyecto_precio_anio
            WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id AND ppa_anio=$anio LIMIT 1");
 ...
 * @return array
 */
function ppto_ajuste_simular($mysqli, $proy_id, $emp_id, $ppe_id, $cuadro, $opts = array()) {
    $cfg = ppto_ajuste_cfg_get($mysqli, $proy_id, $emp_id, $ppe_id);
    ...
    $precio = ppto_ajuste_precio_para_anio($mysqli, $proy_id, $emp_id, $ppe_id, $anio);
 ...
 * @return array
 */
function ppto_ajuste_aplicar($mysqli, $proy_id, $emp_id, $ppe_id, $sim, $usu_id, $obs = '') {
    if (empty($sim['ok'])) {
    ...
    $sql = "INSERT INTO exa_ppto_ajuste_fin_cab
        (proy_id, emp_id, ppe_id, ajc_anio, ajc_vista, ajc_mes, ajc_escenario, ajc_estado,
         ...
         ajc_observacion, ajc_fecha_registro, usu_id)
        VALUES (
         '$esc', $emp_id, $ppe_id, " . (int)$meta['anio'] . ",
         ...
         " . ($obs_esc !== '' ? "'$obs_esc'" : 'NULL') . ", NOW(), $usu_id
        )";
    if (!$mysqli->query($sql)) {
        return array('ok' => false, 'message' => $mysqli->error);
    }
    ...
    foreach ($sim['detalle'] as $d) {
        ...
        $mysqli->query("INSERT INTO exa_ppto_ajuste_fin_det
    ...
    $mysqli->query("UPDATE exa_ppto_proyecto_version
        SET pv_gad_recuperado_acum=$nuevo_acum, pv_ajuste_activo=1, pv_fecha_registro=NOW(), usu_id=$usu_id
        WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id");
 ...
 * @return array
 */
function ppto_ajuste_historial($mysqli, $proy_id, $emp_id, $ppe_id, $limit = 20) {
    ppto_schema_ensure_ajuste_financiero($mysqli);
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    ...
    $rows = array();
    $res = $mysqli->query("SELECT * FROM exa_ppto_ajuste_fin_cab
        WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id
 ...
 * @return array|null
 */
function ppto_ajuste_historial_detalle($mysqli, $ajc_id, $emp_id) {
    $ajc_id = (int)$ajc_id;
    $emp_id = (int)$emp_id;
    $cab = null;
    $res = $mysqli->query("SELECT * FROM exa_ppto_ajuste_fin_cab WHERE ajc_id=$ajc_id AND emp_id=$emp_id LIMIT 1");
    if ($res) {
        $cab = $res->fetch_assoc();
    }
    if (!$cab) {
    ...
    $det = array();
    $rd = $mysqli->query("SELECT * FROM exa_ppto_ajuste_fin_det WHERE ajc_id=$ajc_id ORDER BY grupo_cod");
    if ($rd) {
