...
function ppto_ajuste_cfg_save($mysqli, $proy_id, $Emp_Cod, $ppe_id, $data, $Usu_Cod) {
    ...
    $sql = "INSERT INTO exa_ppto_proyecto_version
            (proy_id, Emp_Cod, ppe_id, pv_toneladas_base_mes, pv_toneladas_costo_mes, pv_tarifa_ton_iva, pv_iva_divisor,
             pv_costo_capital_pct, pv_gad_monto_objetivo, pv_gad_factor_ton, pv_ajuste_activo, pv_fecha_registro, Usu_Cod)
            VALUES ('$esc', $Emp_Cod, $ppe_id, $ton, $ton_costo, $tarifa, $iva,
...
function ppto_ajuste_aplicar($mysqli, $proy_id, $Emp_Cod, $ppe_id, $sim, $Usu_Cod, $obs = '') {
    ...
    $mysqli->query("UPDATE exa_ppto_proyecto_version
        SET pv_gad_recuperado_acum=$nuevo_acum, pv_ajuste_activo=1, pv_fecha_registro=NOW(), Usu_Cod=$Usu_Cod
        WHERE proy_id='$esc' AND Emp_Cod=$Emp_Cod AND ppe_id=$ppe_id");
...
function ppto_ajuste_historial_detalle($mysqli, $ajc_id, $Emp_Cod) {
    ...
    $Emp_Cod = (int)$Emp_Cod;
    $cab = null;
    $res = $mysqli->query("SELECT * FROM exa_ppto_ajuste_fin_cab WHERE ajc_id=$ajc_id AND Emp_Cod=$Emp_Cod LIMIT 1");
    if ($res) {
        $cab = $res->fetch_assoc();
    }
    if (!$cab) {
