 ...
 * - exa_ppto_prod_periodos.prd_proyectada = toneladas proyectadas con las que se
 *   aprueba el presupuesto en $ del mes (proyectada x $/Ton).
 * - exa_ppto_prod_periodos.prd_real = toneladas reales al cerrar el mes.
 ...
 */
function ppto_divergencia_obtener_rubro_driver($mysqli, $proy_id, $emp_id, $ppe_id = null) {
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $sql = "SELECT d.pdp_id, d.pdp_rubro, d.pdp_toneladas_base, d.pdp_factor_anual_tonelada,
                   p.ppa_codigo_clasificacion, COALESCE(NULLIF(p.ppa_clase, ''), 'D') AS ppa_clase
            FROM exa_ppto_proyecto_detalles d
            INNER JOIN exa_ppto_partidas p ON p.ppa_id = d.ppa_id AND p.emp_id = d.emp_id
            WHERE d.proy_id = '$esc' AND d.emp_id = $emp_id
 ...
 * @return array
 */
function ppto_divergencia_comparar_toneladas($mysqli, $proy_id, $emp_id, $anio, $ppe_id = null, $umbral_pct = null) {
    if ($umbral_pct === null) {
    ...
    $driver = ppto_divergencia_obtener_rubro_driver($mysqli, $proy_id, $emp_id, $ppe_id);
    $ton_base_mes = 0.0;
    $rubro_ref = '';
    if ($ppe_id !== null && (int)$ppe_id > 0) {
        $ton_pv = ppto_proy_version_ton_base($mysqli, $proy_id, $emp_id, (int)$ppe_id);
        if ($ton_pv > 0.0001) {
            $ton_base_mes = $ton_pv;
    ...
    $res = $mysqli->query("SELECT
            ...
            WHERE proy_id = '$esc' AND emp_id = $emp_id AND prd_anio = $anio");
    $proy_periodo = 0.0;
    $real_periodo = 0.0;
    $meses_real = 0;
    $meses = 0;
