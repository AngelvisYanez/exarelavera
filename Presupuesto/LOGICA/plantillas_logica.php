...
function ppto_plt_contar_proyectos_activos($plt_id, $Emp_Cod) {
    ...
    $sql = "SELECT COUNT(*) AS total FROM pre_proyectos WHERE (Plt_Cod = $plt_id OR Plt_Cod = $plt_id) AND Emp_Cod = $Emp_Cod AND Pro_Est = 'A'";
...
function ppto_plantilla_nombre_existe($Emp_Cod, $nombre, $exclude_id = 0) {
    ...
    $sql = "SELECT plt_id FROM pre_plantillas WHERE Emp_Cod = $Emp_Cod AND plt_nombre = '$nombre'";
    if ($exclude_id > 0) {
