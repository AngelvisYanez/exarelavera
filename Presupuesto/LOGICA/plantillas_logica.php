...
if ($check_col && $check_col->num_rows === 0) {
    $mysqli_conn->query("ALTER TABLE `exa_ppto_proyectos` ADD COLUMN `plt_id` INT NULL AFTER `Usu_Cod`, ADD INDEX `idx_proy_plantilla` (`plt_id`)");
}
...
function ppto_plt_contar_proyectos_activos($plt_id, $Emp_Cod) {
    ...
    $sql = "SELECT COUNT(*) AS total FROM exa_ppto_proyectos WHERE plt_id = $plt_id AND Emp_Cod = $Emp_Cod";
...
function ppto_plantilla_nombre_existe($Emp_Cod, $nombre, $exclude_id = 0) {
    ...
    $sql = "SELECT plt_id FROM exa_ppto_plantillas WHERE Emp_Cod = $Emp_Cod AND plt_nombre = '$nombre'";
    if ($exclude_id > 0) {
...
function ppto_plantilla_crear($datos) {
    ...
    $sql = "INSERT INTO exa_ppto_plantillas (Emp_Cod, plt_nombre, plt_descripcion, plt_estado, plt_fecha_registro, Usu_Cod)
...
function ppto_plantilla_eliminar($id) {
    ...
    $sql_del = "DELETE FROM exa_ppto_plantillas WHERE plt_id = $id AND Emp_Cod = $Emp_Cod";
...
function ppto_plantilla_duplicar($id, $nuevo_nombre) {
    ...
    $sql_orig = "SELECT * FROM exa_ppto_plantillas WHERE plt_id = $id AND Emp_Cod = $Emp_Cod LIMIT 1";
    ...
    if ($res_orig && $row_orig = $res_orig->fetch_assoc()) {
        ...
        $sql_ins = "INSERT INTO exa_ppto_plantillas (Emp_Cod, plt_nombre, plt_descripcion, plt_estado, plt_fecha_registro, Usu_Cod)
                    VALUES ($Emp_Cod, '$nombre', $descripcion, '$estado', NOW(), $Usu_Cod)";
