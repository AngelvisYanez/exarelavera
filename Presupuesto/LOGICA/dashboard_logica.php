 ...
 * @return array
 */
function ppto_dash_cargar_rubros_proyecto($mysqli, $ppe_id, $proy_id, $Emp_Cod) {
    $rubros = array();
    if (!$ppe_id || $proy_id === null || trim($proy_id) === '') {
        return $rubros;
    }
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $res = $mysqli->query("SELECT Ppa_Cod AS ppa_id, Pdp_FacAnualTon AS pdp_factor_anual_tonelada, Pdp_TonBase AS pdp_toneladas_base
        FROM pre_proyecto_detalles
        WHERE Emp_Cod = " . (int)$Emp_Cod . " AND Ppe_Cod = " . (int)$ppe_id . " AND (Pro_Cod = '$esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$esc'))
 ...
 * @return array
 */
function ppto_dash_rubros_map_factor_escenario_cuadro($mysqli, $rubros_map, $ppe_id, $proy_id, $Emp_Cod) {
    ...
    $cfg = ppto_proy_version_config($mysqli, trim($proy_id), (int)$Emp_Cod, (int)$ppe_id);
    $ton_base_pdf = (float)$cfg['ton_mes'];
    if ($ton_base_pdf <= 0) {
        $ton_base_pdf = ppto_proy_version_ton_base($mysqli, trim($proy_id), (int)$Emp_Cod, (int)$ppe_id);
    }
    $ton_esc_gasto_mes = ppto_proy_ton_escenario_gasto_mes($ton_base_pdf);
    $ton_costo_mes = ppto_proy_version_ton_costo($mysqli, trim($proy_id), (int)$Emp_Cod, (int)$ppe_id);
 ...
 * @return float
 */
function ppto_dash_usd_por_ton_mes($mysqli, $ppe_id, $proy_id, $Emp_Cod) {
    if (!$ppe_id || $proy_id === null || trim($proy_id) === '') {
        return 0.0;
    }
    $esc = $mysqli->real_escape_string(trim($proy_id));
...
function ppto_dash_kpis($mysqli, $filtros) {
    ...
    $meses_prod_tmp = ppto_dash_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $proy_id);
    $periodo = ppto_dash_periodo_resuelto($filtros, $meses_prod_tmp);
    $mes = $periodo['mes'];
    $periodo_vista = $periodo['periodo_vista'];

    $condiciones = array("r.Emp_Cod = $Emp_Cod", "r.ppe_id = $ppe_id");
    ...
    $kpis = ppto_forecast_calcular_global($mysqli, $kpis, $ppe_id, $Emp_Cod, $anio, ($periodo_vista === 'mes') ? $mes : null, $proy_id);
...
function ppto_dash_resumen_partidas($mysqli, $filtros) {
    ...
    if ($proy_id !== null && trim($proy_id) !== '') {
        ...
        $periodo_vista = $periodo['periodo_vista'];
        $mes = $periodo['mes'];
        $rubros_map = ppto_dash_cargar_rubros_proyecto($mysqli, $ppe_id, $proy_id, $Emp_Cod);
        $usa_toneladas = !empty($rubros_map);
        $modo_reinversion = ppto_proy_es_modo_reinversion($mysqli, $proy_id, $Emp_Cod);
        if ($modo_reinversion && $usa_toneladas) {
            ...
            $rubros_map = ppto_dash_rubros_map_factor_escenario_cuadro(
                $mysqli, $rubros_map, $ppe_id, $proy_id, $Emp_Cod
            );
        }
    } else {
    ...
    $sql = "SELECT 
            ...
            GROUP BY r.ppa_id, p.Ppa_Cla, p.Ppa_Des
            HAVING (
...
function ppto_dash_evolucion_mensual($mysqli, $filtros) {
    ...
    $modo_reinversion = ($proy_id !== null && trim($proy_id) !== ''
        && ppto_proy_es_modo_reinversion($mysqli, $proy_id, $Emp_Cod));

    // 1. Obtener la evolucion del presupuesto desde exa_ppto_resumen
    $condiciones = array("r.Emp_Cod = $Emp_Cod", "r.ppe_id = $ppe_id", "r.mes <= 12");
    if ($proy_id !== null) {
        $condiciones[] = "r.proy_id = '" . $mysqli->real_escape_string($proy_id) . "'";
    } else {
        $condiciones[] = "(r.proy_id IS NULL OR r.proy_id = '')";
