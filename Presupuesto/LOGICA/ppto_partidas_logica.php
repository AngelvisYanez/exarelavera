...
/**
 * Recreación de vista desactivada (evitando vistas en BD).
 *
 * @param mysqli $mysqli
 */
...
function ppto_partidas_listar($mysqli, $opts = array()) {
    ...
    $sql = "SELECT 
        Ppa_Cod AS ppa_id, Ppa_Cod,
        Emp_Cod,
        Ppa_Cla AS ppa_codigo_clasificacion, Ppa_Cla,
        Ppa_Des AS ppa_descripcion, Ppa_Des,
        Ppa_Tip AS ppa_tipo, Ppa_Tip,
        Ppa_Nat AS ppa_naturaleza, Ppa_Nat,
        Ppa_Pad AS ppa_padre_id, Ppa_Pad,
        Ppa_Niv AS ppa_nivel, Ppa_Niv,
        COALESCE(NULLIF(Ppa_Clase, ''), 'D') AS ppa_clase, Ppa_Clase,
        Ppa_Pct AS ppa_porcentaje_tope, Ppa_Pct,
        Ppa_Meses AS ppa_meses_prorrateo, Ppa_Meses,
        Ppa_Est AS ppa_estado, Ppa_Est,
        Ppa_Fec AS ppa_fecha_registro, Ppa_Fec,
        Usu_Cod
...
function ppto_partida_contar_reglas_activas($mysqli, $ppa_id, $Emp_Cod) {
    ...
    $res = $mysqli->query("SELECT COUNT(*) AS cnt FROM pre_reglas WHERE (Ppa_Cod = $ppa_id OR ppa_id = $ppa_id) AND Emp_Cod = $Emp_Cod AND Prg_Est = 'A'");
    if ($res && $row = $res->fetch_assoc()) {
        return (int)$row['cnt'];
    }
    return 0;
...
function ppto_partida_cambiar_estado($mysqli, $ppa_id, $Emp_Cod, $nuevo_est) {
    ...
    $mysqli->query("UPDATE pre_partidas SET Ppa_Est = '$nuevo_est' WHERE Ppa_Cod = $ppa_id AND Emp_Cod = $Emp_Cod");

    if ($nuevo_est === 'I') {
        $mysqli->query("UPDATE pre_reglas SET Prg_Est = 'I' WHERE Ppa_Cod = $ppa_id AND Emp_Cod = $Emp_Cod AND Prg_Est = 'A'");
        $reglas_inactivadas = (int)$mysqli->affected_rows;
        // Evita montos fantasma en semáforo / plan mensual.
        $mysqli->query("DELETE FROM pre_detalle WHERE Ppa_Cod = $ppa_id");
    }
...
function ppto_partidas_pool_padre($mysqli, $Emp_Cod) {
    ...
    $sql = "SELECT p.Ppa_Cod AS ppa_id, p.Ppa_Cla AS ppa_codigo_clasificacion, p.Ppa_Des AS ppa_descripcion, p.Ppa_Niv AS ppa_nivel,
                   p.Ppa_Tip AS ppa_tipo, p.Ppa_Nat AS ppa_naturaleza,
...
function ppto_partidas_map_rubro_proyecto($mysqli, $Emp_Cod, $ppe_id = null) {
    ...
    $sql = "SELECT d.Ppa_Cod AS ppa_id, d.Pro_Cod AS proy_id, d.Pdp_Rubro AS pdp_rubro, pr.Pro_Nom AS proy_nombre
            FROM pre_proyecto_detalles d
            INNER JOIN pre_proyectos pr
                ON (pr.Pro_Cod = d.Pro_Cod OR pr.Pro_Ide = d.proy_id) AND pr.Emp_Cod = d.Emp_Cod
...
function ppto_proy_sumar_rubros_prefijo($mysqli, $proy_id, $Emp_Cod, $ppe_id, $grupo_cod, $excluir_pdp_id = 0) {
    ...
    $sql = "SELECT COALESCE(SUM(pdm.Pdm_PreAnual), 0) AS total
        FROM pre_proyecto_detalles d
        INNER JOIN pre_proyecto_detalles_mes pdm ON d.Pdp_Cod = pdm.Pdp_Cod
        INNER JOIN pre_partidas p ON d.Ppa_Cod = p.Ppa_Cod
        WHERE d.Pro_Cod='$proy_esc' AND d.Emp_Cod=$Emp_Cod AND d.Ppe_Cod=$ppe_id
...
function ppto_proy_grupos_resumen_tope($mysqli, $Emp_Cod, $proy_id, $ppe_id, $rows) {
    ...
    $cfg = ppto_proy_version_config($mysqli, $proy_id, $Emp_Cod, $ppe_id);
    $out = array();
    $res = $mysqli->query("SELECT Ppa_Cod AS ppa_id, Ppa_Cla AS ppa_codigo_clasificacion, Ppa_Des AS ppa_descripcion,
            COALESCE(Ppa_Pct, 0) AS ppa_porcentaje_tope,
            COALESCE(NULLIF(Ppa_Meses, 0), 12) AS ppa_meses_prorrateo
        FROM pre_partidas
