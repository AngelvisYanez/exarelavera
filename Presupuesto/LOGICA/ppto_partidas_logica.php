...
function ppto_partidas_listar($mysqli, $opts = array()) {
    ...
    $sql = "SELECT * FROM exa_ppto_partidas WHERE Emp_Cod = $Emp_Cod";
 ...
 * @return array|null
 */
function ppto_partida_obtener($mysqli, $ppa_id, $Emp_Cod) {
    $ppa_id = (int)$ppa_id;
    $Emp_Cod = (int)$Emp_Cod;
    if (!$mysqli || $ppa_id <= 0 || $Emp_Cod <= 0) {
        return null;
    }
    $res = $mysqli->query("SELECT * FROM exa_ppto_partidas WHERE ppa_id = $ppa_id AND Emp_Cod = $Emp_Cod LIMIT 1");
    if ($res && $row = $res->fetch_assoc()) {
        if (!isset($row['ppa_clase']) || $row['ppa_clase'] === '') {
            $row['ppa_clase'] = 'D';
        }
 ...
 * @return int
 */
function ppto_partida_contar_reglas_activas($mysqli, $ppa_id, $Emp_Cod) {
    $ppa_id = (int)$ppa_id;
    $Emp_Cod = (int)$Emp_Cod;
    if (!$mysqli || $ppa_id <= 0) {
        return 0;
    }
    $res = $mysqli->query("SELECT COUNT(*) AS cnt FROM exa_ppto_reglas WHERE ppa_id = $ppa_id AND Emp_Cod = $Emp_Cod AND prg_estado = 'A'");
    if ($res && $row = $res->fetch_assoc()) {
        return (int)$row['cnt'];
    }
    return 0;
 ...
 */
function ppto_partida_cambiar_estado($mysqli, $ppa_id, $Emp_Cod, $nuevo_est) {
    $ppa_id = (int)$ppa_id;
    $Emp_Cod = (int)$Emp_Cod;
    ...
    $mysqli->query("UPDATE exa_ppto_partidas SET ppa_estado = '$nuevo_est' WHERE ppa_id = $ppa_id AND Emp_Cod = $Emp_Cod");

    if ($nuevo_est === 'I') {
        $mysqli->query("UPDATE exa_ppto_reglas SET prg_estado = 'I' WHERE ppa_id = $ppa_id AND Emp_Cod = $Emp_Cod AND prg_estado = 'A'");
 ...
 * @return bool
 */
function ppto_partida_es_destino_regla($mysqli, $ppa_id, $Emp_Cod) {
    $part = ppto_partida_obtener($mysqli, $ppa_id, $Emp_Cod);
...
function ppto_partidas_pool_padre($mysqli, $Emp_Cod) {
    ...
    $sql = "SELECT p.ppa_id, p.ppa_codigo_clasificacion, p.ppa_descripcion, p.ppa_nivel,
                   ...
                   CASE WHEN EXISTS (
                       ...
                       WHERE h.ppa_padre_id = p.ppa_id AND h.Emp_Cod = p.Emp_Cod
 ...
 */
function ppto_partidas_map_rubro_proyecto($mysqli, $Emp_Cod, $ppe_id = null) {
    $map = array();
    $Emp_Cod = (int)$Emp_Cod;
    if (!$mysqli || $Emp_Cod <= 0) {
        return $map;
    ...
    $sql = "SELECT d.ppa_id, d.proy_id, d.pdp_rubro, pr.proy_nombre
            ...
            INNER JOIN exa_ppto_proyectos pr
                ON pr.proy_id = d.proy_id AND pr.Emp_Cod = d.Emp_Cod
...
function ppto_partida_obtener_por_codigo($mysqli, $Emp_Cod, $codigo) {
    ...
    $res = $mysqli->query("SELECT * FROM exa_ppto_partidas WHERE Emp_Cod=$Emp_Cod AND ppa_codigo_clasificacion='$esc' LIMIT 1");
    if ($res && ($row = $res->fetch_assoc())) {
        if (!isset($row['ppa_clase']) || $row['ppa_clase'] === '') {
            $row['ppa_clase'] = 'D';
        }
...
function ppto_partida_guardar($mysqli, $Emp_Cod, $Usu_Cod, $data) {
    ...
    $sql = "INSERT INTO exa_ppto_partidas
        (Emp_Cod, ppa_codigo_clasificacion, ppa_descripcion, ppa_tipo, ppa_naturaleza, ppa_padre_id, ppa_nivel, ppa_clase, ppa_estado, ppa_fecha_registro, Usu_Cod)
 ...
 * @return float
 */
function ppto_proy_sumar_rubros_prefijo($mysqli, $proy_id, $Emp_Cod, $ppe_id, $grupo_cod, $excluir_pdp_id = 0) {
 ...
 * @return float
 */
function ppto_proy_total_presupuesto_anual($mysqli, $proy_id, $Emp_Cod, $ppe_id, $excluir_pdp_id = 0) {
    $proy_esc = $mysqli->real_escape_string(trim($proy_id));
    $Emp_Cod = (int)$Emp_Cod;
    $ppe_id = (int)$ppe_id;
    $excluir_pdp_id = (int)$excluir_pdp_id;
    ...
    $res = $mysqli->query("SELECT COALESCE(SUM(pdp_presupuesto_anual), 0) AS total
        ...
        WHERE proy_id='$proy_esc' AND Emp_Cod=$Emp_Cod AND ppe_id=$ppe_id$excl");
    if ($res && ($row = $res->fetch_assoc())) {
        return (float)$row['total'];
    }
    return 0.0;
 ...
 */
function ppto_proy_validar_tope_grupo_rubro($mysqli, $proy_id, $Emp_Cod, $ppe_id, $ppa_codigo, $nuevo_anual, $excluir_pdp_id = 0) {
    ...
    $sum_grupo = ppto_proy_sumar_rubros_prefijo($mysqli, $proy_id, $Emp_Cod, $ppe_id, $grupo_cod, $excluir_pdp_id);
 ...
 */
function ppto_proy_grupos_resumen_tope($mysqli, $Emp_Cod, $proy_id, $ppe_id, $rows) {
    require_once __DIR__ . '/ppto_proyecto_version_logica.php';
    $Emp_Cod = (int)$Emp_Cod;
    $ppe_id = (int)$ppe_id;
    $cfg = ppto_proy_version_config($mysqli, $proy_id, $Emp_Cod, $ppe_id);
    ...
    $res = $mysqli->query("SELECT ppa_id, ppa_codigo_clasificacion, ppa_descripcion,
        ...
        FROM exa_ppto_partidas
        WHERE Emp_Cod = $Emp_Cod AND ppa_estado = 'A' AND COALESCE(NULLIF(ppa_clase, ''), 'D') = 'G'");
    if (!$res) {
        return $out;
    }
    while ($g = $res->fetch_assoc()) {
        ...
        $sum = ppto_proy_sumar_rubros_prefijo($mysqli, $proy_id, $Emp_Cod, $ppe_id, $cod);
        $tope = ppto_proy_grupo_tope_anual($pct, $cfg);
