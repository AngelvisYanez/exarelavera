 ...
 * @return array
 */
function ppto_proy_cuadro_cargar($mysqli, $emp_id, $proy_id, $ppe_id, $cuadro_vista = 'anual', $cuadro_mes = null, $anio_precio = null) {
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    ...
    $sql_rub = "SELECT d.*, p.ppa_codigo_clasificacion, p.ppa_descripcion,
        ...
        LEFT JOIN exa_ppto_partidas pg ON pg.emp_id = d.emp_id
            AND pg.ppa_codigo_clasificacion = SUBSTRING_INDEX(p.ppa_codigo_clasificacion, '.', 1)
        LEFT JOIN exa_ppto_partidas ps ON ps.emp_id = d.emp_id
            AND ps.ppa_codigo_clasificacion = CASE
                WHEN (LENGTH(p.ppa_codigo_clasificacion) - LENGTH(REPLACE(p.ppa_codigo_clasificacion, '.', ''))) >= 2
                THEN SUBSTRING_INDEX(p.ppa_codigo_clasificacion, '.', 2)
                ELSE ''
            END
        WHERE d.proy_id='$proy_esc' AND d.emp_id=$emp_id";
    if ($ppe_id > 0) {
    ...
    $cfg_ver = ppto_proy_version_config($mysqli, $proy_id, $emp_id, $ppe_id);
    ...
    $precio_anio = ppto_ajuste_precio_para_anio($mysqli, $proy_id, $emp_id, $ppe_id, $anio_proy);
    ...
    $precio_base = ppto_ajuste_precio_para_anio($mysqli, $proy_id, $emp_id, $ppe_id, $anio_esc);
    ...
    if ($ton_base_pdf <= 0) {
        $ton_base_pdf = ppto_proy_version_ton_base($mysqli, $proy_id, $emp_id, $ppe_id);
    }
    $ton_esc_gasto_mes = ppto_proy_ton_escenario_gasto_mes($ton_base_pdf);
    $ton_costo_mes = ppto_proy_version_ton_costo($mysqli, $proy_id, $emp_id, $ppe_id);

    $meses_prod_esc = ppto_forecast_cargar_produccion_meses($mysqli, $emp_id, $anio_esc, $proy_id);
    $meses_con_real = 0;
    for ($mm = 1; $mm <= 12; $mm++) {
    ...
    return array(
        ...
        'precios_proyeccion' => ppto_ajuste_precios_list($mysqli, $proy_id, $emp_id, $ppe_id),
        'grupos_tope' => ppto_proy_grupos_resumen_tope($mysqli, $emp_id, $proy_id, $ppe_id, $rows),
