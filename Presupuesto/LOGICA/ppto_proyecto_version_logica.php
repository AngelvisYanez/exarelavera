...
if (!function_exists('ppto_proy_version_ton_base')) {
    function ppto_proy_version_ton_base($mysqli, $proy_id, $Emp_Cod, $ppe_id) {
        ...
        $res = $mysqli->query("SELECT Ppv_TonBaseMes AS pv_toneladas_base_mes FROM pre_proyecto_version
        ...
        $res2 = $mysqli->query("SELECT MAX(d.Pdp_TonBase) AS ton
            FROM pre_proyecto_detalles d
            WHERE (d.Pro_Cod='$esc' OR d.Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$esc')) AND d.Emp_Cod=$Emp_Cod AND d.Ppe_Cod=$ppe_id AND d.Pdp_TonBase > 0");
...
if (!function_exists('ppto_proy_version_ton_costo')) {
    ...
    function ppto_proy_version_ton_costo($mysqli, $proy_id, $Emp_Cod, $ppe_id) {
        ...
        $res = $mysqli->query("SELECT Ppv_TonCostoMes AS pv_toneladas_costo_mes FROM pre_proyecto_version
            WHERE (Pro_Cod='$esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$esc')) AND Emp_Cod=$Emp_Cod AND Ppe_Cod=$ppe_id LIMIT 1");
        if ($res && ($row = $res->fetch_assoc()) && (float)$row['pv_toneladas_costo_mes'] > 0) {
            return (float)$row['pv_toneladas_costo_mes'];
        }
        $res2 = $mysqli->query("SELECT d.Pdp_TonBase AS ton, COUNT(*) AS c
            FROM pre_proyecto_detalles d
...
function ppto_proy_version_config($mysqli, $proy_id, $Emp_Cod, $ppe_id) {
    ...
    $res = $mysqli->query("SELECT Ppv_TonBaseMes AS pv_toneladas_base_mes, Ppv_TarifaTonIva AS pv_tarifa_ton_iva, Ppv_IvaDivisor AS pv_iva_divisor
...
function ppto_prod_sync_esperada_desde_ton_base($mysqli, $proy_id, $Emp_Cod, $anio, $ppe_id = 0, $opts = array()) {
    ...
    for ($mes = 1; $mes <= 12; $mes++) {
        $res = $mysqli->query("SELECT Prd_Esperada AS prd_esperada, Prd_Est AS prd_estado FROM pre_prod_periodos
            WHERE (Pro_Cod='$esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$esc')) AND Emp_Cod=$Emp_Cod AND Prd_Anio=$anio AND Prd_Mes=$mes LIMIT 1");
        if ($res && ($row = $res->fetch_assoc())) {
