<?php
/**
 * Toneladas base PDF por proyecto/version (exa_ppto_proyecto_version).
 */

if (!function_exists('ppto_proy_version_ton_base')) {
    function ppto_proy_version_ton_base($mysqli, $proy_id, $emp_id, $ppe_id) {
        ...
        $res = $mysqli->query("SELECT pv_toneladas_base_mes FROM exa_ppto_proyecto_version
        ...
        $res2 = $mysqli->query("SELECT MAX(d.pdp_toneladas_base) AS ton
            ...
            WHERE d.proy_id='$esc' AND d.emp_id=$emp_id AND d.ppe_id=$ppe_id AND d.pdp_toneladas_base > 0");
...
if (!function_exists('ppto_proy_version_ton_costo')) {
     ...
     * @return float
     */
    function ppto_proy_version_ton_costo($mysqli, $proy_id, $emp_id, $ppe_id) {
        $esc = $mysqli->real_escape_string(trim($proy_id));
        $emp_id = (int)$emp_id;
        $ppe_id = (int)$ppe_id;
        if ($ppe_id <= 0) {
        ...
        $res = $mysqli->query("SELECT pv_toneladas_costo_mes FROM exa_ppto_proyecto_version
            WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id LIMIT 1");
 ...
 * @return array
 */
function ppto_proy_version_config($mysqli, $proy_id, $emp_id, $ppe_id) {
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $cfg = array(
    ...
    if ($res && ($row = $res->fetch_assoc())) {
        ...
        $cfg['ton_mes'] = ppto_proy_version_ton_base($mysqli, $proy_id, $emp_id, $ppe_id);
    }
 ...
 * @return int
 */
function ppto_proy_version_ppe_id_por_anio($mysqli, $emp_id, $anio) {
    $emp_id = (int)$emp_id;
    $anio = (int)$anio;
    if (!$mysqli || $emp_id <= 0 || $anio <= 0) {
        return 0;
    }
 ...
 * @return array
 */
function ppto_prod_sync_esperada_desde_ton_base($mysqli, $proy_id, $emp_id, $anio, $ppe_id = 0, $opts = array()) {
    ...
    if (!$mysqli || trim($proy_id) === '' || $emp_id <= 0 || $anio <= 0) {
    ...
    $ton = ppto_proy_version_ton_base($mysqli, $proy_id, $emp_id, $ppe_id);
    if ($ton <= 0.0001) {
    ...
    for ($mes = 1; $mes <= 12; $mes++) {
        $res = $mysqli->query("SELECT prd_esperada, prd_estado FROM exa_ppto_prod_periodos
            WHERE proy_id='$esc' AND emp_id=$emp_id AND prd_anio=$anio AND prd_mes=$mes LIMIT 1");
        if ($res && ($row = $res->fetch_assoc())) {
        ...
        if (ppto_integracion_produccion_registrar($mysqli, $proy_id, $mes, $ton, 'esperada', $anio, $emp_id)) {
            $meses_ok++;
        }
    }
