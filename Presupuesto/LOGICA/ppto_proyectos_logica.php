...
function ppto_proy_clonar_plantilla($mysqli, $proy_id, $plt_id, $Emp_Cod, $Usu_Cod) {
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        if ($pdp_id <= 0) {
            $r2 = $mysqli->query("SELECT pdp_id FROM pre_proyecto_detalles WHERE ppe_id=$ppe_id AND ppa_id=$ppa_id AND proy_id='$proy_esc' AND pdp_rubro='$rubro' LIMIT 1");
...
function ppto_proy_rubro_id_por_partida($mysqli, $ppe_id, $ppa_id, $proy_id, $Emp_Cod) {
    ...
    $r = $mysqli->query("SELECT pdp_id FROM pre_proyecto_detalles
...
function ppto_proy_rubro_purgar_duplicados_partida($mysqli, $ppe_id, $ppa_id, $proy_id, $Emp_Cod, $pdp_id_keep) {
    ...
    $eliminados = 0;
    $res = $mysqli->query("SELECT pdp_id FROM pre_proyecto_detalles
...
function ppto_proy_rubro_eliminar($mysqli, $pdp_id, $proy_id, $Emp_Cod, $ppe_id) {
    ...
    $chk = $mysqli->query("SELECT pdp_id, ppa_id FROM pre_proyecto_detalles
    ...
    $mov = $mysqli->query("SELECT SUM(pdm_ejecutado + pdm_comprometido) AS mov
        FROM pre_proyecto_detalles_mes WHERE pdp_id=$pdp_id");
    ...
    $mysqli->query("DELETE FROM pre_proyecto_detalles_mes WHERE pdp_id=$pdp_id");
...
if ($action === 'save_rubro') {
    ...
    if ($presup_fijo > 0.0001) {
        ...
        $anual = 0;
        if ($pdp_edit > 0) {
            $r0 = $mysqli->query('SELECT pdp_presupuesto_anual FROM pre_proyecto_detalles WHERE pdp_id=' . $pdp_edit . ' LIMIT 1');
...
if ($action === 'aplicar_escenario') {
    ...
    foreach ($pendientes as $p) {
        ...
        if ($mysqli->query("UPDATE pre_proyecto_detalles SET pdp_presupuesto_anual=$anual WHERE pdp_id=$pdp_id")) {
            $actualizados++;
        }
    }
