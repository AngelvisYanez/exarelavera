...
function ppto_proy_recalcular_rubros_preservar_anual($mysqli, $proy_id, $Emp_Cod, $ppe_id, $ton_mes, $ton_origen = 0) {
    ...
    $actualizados = 0;
    $res = $mysqli->query("SELECT d.Pdp_Cod AS pdp_id, d.Pdp_FacAnualTon AS pdp_factor_anual_tonelada
        FROM pre_proyecto_detalles d
        WHERE (d.Pro_Cod='$esc' OR d.Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$esc')) AND d.Emp_Cod=$Emp_Cod AND d.Ppe_Cod=$ppe_id
...
function ppto_proy_version_sync_prod_esperada($mysqli, $proy_id, $Emp_Cod, $ppe_id) {
    ...
    $res = $mysqli->query("SELECT Ppe_Ani AS ppe_anio FROM pre_presupuesto WHERE Ppe_Cod=$ppe_id LIMIT 1");
...
function ppto_proy_clonar_plantilla($mysqli, $proy_id, $plt_id, $Emp_Cod, $Usu_Cod) {
    ...
    $anio = (int)date('Y');
    $res_ppe = $mysqli->query("SELECT Ppe_Cod AS ppe_id FROM pre_presupuesto WHERE Emp_Cod=$Emp_Cod AND Ppe_Ani=$anio AND Ppe_Est='A' ORDER BY Ppe_Ver DESC LIMIT 1");
    if (!$res_ppe || !($v = $res_ppe->fetch_assoc())) {
        return;
    }
    $ppe_id = (int)$v['ppe_id'];
    $proy_esc = $mysqli->real_escape_string($proy_id);
    $res = $mysqli->query("SELECT pp.Ppa_Cod AS ppa_id, p.Ppa_Des AS ppa_descripcion
        FROM pre_plantilla_partidas pp
        INNER JOIN pre_partidas p ON pp.Ppa_Cod = p.Ppa_Cod
        WHERE pp.Plt_Cod = $plt_id");
    if (!$res) {
        return;
    }
    while ($row = $res->fetch_assoc()) {
        ...
        $mysqli->query("INSERT IGNORE INTO pre_proyecto_detalles
            ...
            VALUES ($ppe_id, $ppa_id, COALESCE((SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc' OR Pro_Cod='$proy_esc' LIMIT 1), 0), $Emp_Cod, '$rubro', 0, 0, 0, NOW(), " . (int)$Usu_Cod . ")");
        $pdp_id = (int)$mysqli->insert_id;
        if ($pdp_id <= 0) {
            $r2 = $mysqli->query("SELECT Pdp_Cod AS pdp_id FROM pre_proyecto_detalles WHERE Ppe_Cod=$ppe_id AND Ppa_Cod=$ppa_id AND (Pro_Cod='$proy_esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc')) AND Pdp_Rubro='$rubro' LIMIT 1");
...
function ppto_proy_rubro_id_por_partida($mysqli, $ppe_id, $ppa_id, $proy_id, $Emp_Cod) {
    ...
    $r = $mysqli->query("SELECT Pdp_Cod AS pdp_id FROM pre_proyecto_detalles
        WHERE Ppe_Cod=$ppe_id AND Ppa_Cod=$ppa_id AND (Pro_Cod='$proy_esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc')) AND Emp_Cod=$Emp_Cod
...
function ppto_proy_rubros_map_por_codigo($mysqli, $proy_id, $Emp_Cod, $ppe_id) {
    ...
    $sql = "SELECT p.Ppa_Cla AS ppa_codigo_clasificacion, d.Pdp_Cod AS pdp_id, d.Pdp_Rubro AS pdp_rubro
        FROM pre_proyecto_detalles d
        INNER JOIN pre_partidas p ON p.Ppa_Cod = d.Ppa_Cod AND p.Emp_Cod = d.Emp_Cod
        WHERE (d.Pro_Cod='$proy_esc' OR d.Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc')) AND d.Emp_Cod=$Emp_Cod AND d.Ppe_Cod=$ppe_id
...
function ppto_proy_rubro_purgar_duplicados_partida($mysqli, $ppe_id, $ppa_id, $proy_id, $Emp_Cod, $pdp_id_keep) {
    ...
    $eliminados = 0;
    $res = $mysqli->query("SELECT Pdp_Cod AS pdp_id FROM pre_proyecto_detalles
        WHERE Ppe_Cod=$ppe_id AND Ppa_Cod=$ppa_id AND (Pro_Cod='$proy_esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc')) AND Emp_Cod=$Emp_Cod AND Pdp_Cod<>$pdp_id_keep");
    if (!$res) {
        return 0;
    }
    while ($row = $res->fetch_assoc()) {
...
function ppto_proy_distribuir_meses($mysqli, $pdp_id, $anual) {
    ...
    for ($mes = 1; $mes <= 12; $mes++) {
        $mysqli->query("INSERT INTO pre_proyecto_detalles_mes
            ...
            ON DUPLICATE KEY UPDATE Pdm_PreMensual=$mensual, Pdm_Disponible=GREATEST(0, $mensual - Pdm_Ejecutado - Pdm_Comprometido)");
    }
}
...
function ppto_proy_rubro_eliminar($mysqli, $pdp_id, $proy_id, $Emp_Cod, $ppe_id) {
    ...
    $chk = $mysqli->query("SELECT Pdp_Cod AS pdp_id, Ppa_Cod AS ppa_id FROM pre_proyecto_detalles
        WHERE Pdp_Cod=$pdp_id AND (Pro_Cod='$proy_esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_esc')) AND Emp_Cod=$Emp_Cod AND Ppe_Cod=$ppe_id LIMIT 1");
    ...
    $mov = $mysqli->query("SELECT SUM(Pdm_Ejecutado + Pdm_Comprometido) AS mov
        FROM pre_proyecto_detalles_mes WHERE Pdp_Cod=$pdp_id");
    ...
    if ($ppa_id_rubro > 0) {
        $otros = $mysqli->query("SELECT COUNT(*) AS cnt FROM pre_proyecto_detalles
            WHERE Ppa_Cod=$ppa_id_rubro AND Emp_Cod=$Emp_Cod AND Ppe_Cod=$ppe_id LIMIT 1");
        $sin_otros = ($otros && ($o = $otros->fetch_assoc()) && (int)$o['cnt'] === 0);
        if ($sin_otros) {
            $mysqli->query("DELETE FROM pre_detalle WHERE Ppa_Cod=$ppa_id_rubro AND Ppe_Cod=$ppe_id");
        }
    }
...
if ($action === 'list') {
    $rows = array();
    $res = $mysqli->query("SELECT p.Pro_Cod AS proy_id, p.Pro_Cod, p.Pro_Ide, p.Pro_Ide AS proy_codigo, p.Pro_Nom AS proy_nombre, p.Pro_Des AS proy_descripcion, p.Pro_Est AS proy_estado, p.Pro_FecReg AS proy_fecha_registro, p.Plt_Cod AS plt_id, pl.Plt_Nom AS plt_nombre
...
if ($action === 'save') {
    ...
    if ($is_edit) {
        $sql = "UPDATE pre_proyectos SET Pro_Nom='$proy_nombre', Pro_Est='$proy_estado', Plt_Cod=$plt_id
                WHERE (Pro_Cod='$proy_id' OR Pro_Ide='$proy_id') AND Emp_Cod=$Emp_Cod";
    } else {
        $sql = "INSERT INTO pre_proyectos (Pro_Ide, Emp_Cod, Pro_Nom, Pro_Est, Pro_FecReg, Usu_Cod, Plt_Cod)
                ...
                ON DUPLICATE KEY UPDATE Pro_Nom='$proy_nombre', Pro_Est='$proy_estado', Plt_Cod=$plt_id";
    }
    $ok = $mysqli->query($sql);
...
if ($action === 'save_rubro') {
    ...
    if ($rubro === '' && $ppa_id > 0) {
        $r_desc = $mysqli->query("SELECT Ppa_Des AS ppa_descripcion FROM pre_partidas WHERE Ppa_Cod=$ppa_id AND Emp_Cod=$Emp_Cod LIMIT 1");
    ...
    $r_cod = $mysqli->query("SELECT Ppa_Cla AS ppa_codigo_clasificacion FROM pre_partidas WHERE Ppa_Cod=$ppa_id AND Emp_Cod=$Emp_Cod LIMIT 1");
    ...
    if ($presup_fijo > 0.0001) {
        ...
        $anual = 0;
        if ($pdp_edit > 0) {
            $r0 = $mysqli->query('SELECT Pdp_PreAnual AS pdp_presupuesto_anual FROM pre_proyecto_detalles WHERE Pdp_Cod=' . $pdp_edit . ' LIMIT 1');
    ...
    if ($pdp_edit > 0) {
        $sql = "UPDATE pre_proyecto_detalles
                ...
                WHERE Pdp_Cod=$pdp_edit AND (Pro_Cod='$proy_id' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_id')) AND Emp_Cod=$Emp_Cod AND Ppe_Cod=$ppe_id";
    } else {
        $sql = "INSERT INTO pre_proyecto_detalles
                (Ppe_Cod, Ppa_Cod, Pro_Cod, Emp_Cod, Pdp_Rubro, Pdp_TonBase, Pdp_FacAnualTon, Pdp_PreAnual, Pdp_FecReg, Usu_Cod)
                VALUES ($ppe_id, $ppa_id, COALESCE((SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_id' OR Pro_Cod='$proy_id' LIMIT 1), 0), $Emp_Cod, '$rubro', $ton, $factor, $anual, NOW(), " . (int)$Ses_Usu_Cod . ")";
    }
    $ok = $mysqli->query($sql);
    ...
    if ($ok) {
        ...
        $anio_row = $mysqli->query("SELECT Ppe_Ani AS ppe_anio FROM pre_presupuesto WHERE Ppe_Cod=$ppe_id LIMIT 1");
...
if ($action === 'aplicar_escenario') {
    ...
    $res = $mysqli->query("SELECT Pdp_Cod AS pdp_id, Pdp_FacAnualTon AS pdp_factor_anual_tonelada, Pdp_PreAnual AS pdp_presupuesto_anual
        FROM pre_proyecto_detalles
        WHERE (Pro_Cod='$proy_id' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$proy_id')) AND Emp_Cod=$Emp_Cod" . ($ppe_id > 0 ? " AND Ppe_Cod=$ppe_id" : ""));
    $actualizados = 0;
    $total_nuevo = 0.0;
    $pendientes = array();
    if ($res) {
    ...
    foreach ($pendientes as $p) {
        ...
        if ($mysqli->query("UPDATE pre_proyecto_detalles SET Pdp_PreAnual=$anual WHERE Pdp_Cod=$pdp_id")) {
            $actualizados++;
        }
    }
...
if ($action === 'save_grupo_meses') {
    ...
    $chk = $mysqli->query("SELECT Ppa_Cod AS ppa_id, Ppa_Cla AS ppa_codigo_clasificacion, COALESCE(NULLIF(Ppa_Clase,''),'D') AS ppa_clase
        FROM pre_partidas WHERE Ppa_Cod=$ppa_id AND Emp_Cod=$Emp_Cod LIMIT 1");
    ...
    if ($raw_meses === '') {
        $ok = $mysqli->query("UPDATE pre_partidas SET Ppa_Meses = NULL WHERE Ppa_Cod=$ppa_id AND Emp_Cod=$Emp_Cod");
    ...
    $ok = $mysqli->query("UPDATE pre_partidas SET Ppa_Meses = $meses WHERE Ppa_Cod=$ppa_id AND Emp_Cod=$Emp_Cod");
    ppto_json(array(
...
if ($action === 'save_grupo_pct') {
    ...
    if ($raw_pct === '') {
        $ok = $mysqli->query("UPDATE pre_partidas SET Ppa_Pct = NULL WHERE Ppa_Cod=$ppa_id AND Emp_Cod=$Emp_Cod");
    ...
    $ok = $mysqli->query("UPDATE pre_partidas SET Ppa_Pct = $pct_sql WHERE Ppa_Cod=$ppa_id AND Emp_Cod=$Emp_Cod");
    ppto_json(array(
        'status' => $ok ? 'success' : 'error',
...
if ($action === 'catalogos') {
    ...
    $partidas = array();
    $versiones = array();
    $r1 = $mysqli->query("SELECT Plt_Cod AS plt_id, Plt_Nom AS plt_nombre FROM pre_plantillas WHERE Emp_Cod=$Emp_Cod AND Plt_Est='A'");
    if ($r1) {
