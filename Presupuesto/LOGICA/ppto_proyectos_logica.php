 ...
 * @return array
 */
function ppto_proy_recalcular_rubros_preservar_anual($mysqli, $proy_id, $emp_id, $ppe_id, $ton_mes, $ton_origen = 0) {
    ...
    $res = $mysqli->query("SELECT d.pdp_id, d.pdp_factor_anual_tonelada
        ...
        WHERE d.proy_id='$esc' AND d.emp_id=$emp_id AND d.ppe_id=$ppe_id
 ...
 * @return array
 */
function ppto_proy_version_guardar_ton($mysqli, $proy_id, $emp_id, $ppe_id, $ton_mes, $usu_id, $aplicar_rubros = false, $tarifa_iva = 3.0, $iva_div = 1.15, $ton_costo_mes = 0) {
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    ...
    $sql = "INSERT INTO exa_ppto_proyecto_version
            (proy_id, emp_id, ppe_id, pv_toneladas_base_mes, pv_toneladas_costo_mes, pv_tarifa_ton_iva, pv_iva_divisor, pv_fecha_registro, usu_id)
            VALUES ('$esc', $emp_id, $ppe_id, $ton_mes, $ton_costo_mes, $tarifa_iva, $iva_div, NOW(), $usu_id)
            ON DUPLICATE KEY UPDATE pv_toneladas_base_mes=$ton_mes, pv_toneladas_costo_mes=$ton_costo_mes, pv_tarifa_ton_iva=$tarifa_iva,
                pv_iva_divisor=$iva_div, pv_fecha_registro=NOW(), usu_id=$usu_id";
    $ok = $mysqli->query($sql);
    if (!$ok) {
    ...
    if ($aplicar_rubros && $ton_costo_mes > 0) {
        $rec = ppto_proy_recalcular_rubros_preservar_anual($mysqli, $proy_id, $emp_id, $ppe_id, $ton_costo_mes);
 ...
 * @return array|null
 */
function ppto_proy_version_sync_prod_esperada($mysqli, $proy_id, $emp_id, $ppe_id) {
    $ppe_id = (int)$ppe_id;
    if ($ppe_id <= 0) {
        return null;
    }
    ...
    return ppto_prod_sync_esperada_desde_ton_base($mysqli, $proy_id, $emp_id, (int)$row['ppe_anio'], $ppe_id, array(
 ...
 */
function ppto_proy_clonar_plantilla($mysqli, $proy_id, $plt_id, $emp_id, $usu_id) {
    $plt_id = (int)$plt_id;
    if ($plt_id <= 0) {
        return;
    }
    $anio = (int)date('Y');
    $res_ppe = $mysqli->query("SELECT ppe_id FROM exa_ppto_cabeceras WHERE emp_id=$emp_id AND ppe_anio=$anio AND ppe_estado='A' ORDER BY ppe_version DESC LIMIT 1");
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        $mysqli->query("INSERT IGNORE INTO exa_ppto_proyecto_detalles
            ...
            VALUES ($ppe_id, $ppa_id, '$proy_esc', $emp_id, '$rubro', 0, 0, 0, NOW(), " . (int)$usu_id . ")");
 ...
 * @return int
 */
function ppto_proy_rubro_id_por_partida($mysqli, $ppe_id, $ppa_id, $proy_id, $emp_id) {
    $ppe_id = (int)$ppe_id;
    $ppa_id = (int)$ppa_id;
    $emp_id = (int)$emp_id;
    ...
    $r = $mysqli->query("SELECT pdp_id FROM exa_ppto_proyecto_detalles
        WHERE ppe_id=$ppe_id AND ppa_id=$ppa_id AND proy_id='$proy_esc' AND emp_id=$emp_id
 ...
 * @return array
 */
function ppto_proy_rubros_map_por_codigo($mysqli, $proy_id, $emp_id, $ppe_id) {
    $proy_esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $map = array();
    ...
    $sql = "SELECT p.ppa_codigo_clasificacion, d.pdp_id, d.pdp_rubro
        FROM exa_ppto_proyecto_detalles d
        INNER JOIN exa_ppto_partidas p ON p.ppa_id = d.ppa_id AND p.emp_id = d.emp_id
 ...
 * @return int
 */
function ppto_proy_rubro_purgar_duplicados_partida($mysqli, $ppe_id, $ppa_id, $proy_id, $emp_id, $pdp_id_keep) {
    $ppe_id = (int)$ppe_id;
    $ppa_id = (int)$ppa_id;
    $emp_id = (int)$emp_id;
    ...
    $res = $mysqli->query("SELECT pdp_id FROM exa_ppto_proyecto_detalles
        WHERE ppe_id=$ppe_id AND ppa_id=$ppa_id AND proy_id='$proy_esc' AND emp_id=$emp_id AND pdp_id<>$pdp_id_keep");
    if (!$res) {
        return 0;
    }
    while ($row = $res->fetch_assoc()) {
        $del = ppto_proy_rubro_eliminar($mysqli, (int)$row['pdp_id'], $proy_id, $emp_id, $ppe_id);
        if (!empty($del['ok'])) {
            $eliminados++;
        }
    }
 ...
 * @return array
 */
function ppto_proy_rubro_eliminar($mysqli, $pdp_id, $proy_id, $emp_id, $ppe_id) {
    $pdp_id = (int)$pdp_id;
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $proy_esc = $mysqli->real_escape_string(trim($proy_id));
    ...
    $chk = $mysqli->query("SELECT pdp_id, ppa_id FROM exa_ppto_proyecto_detalles
        WHERE pdp_id=$pdp_id AND proy_id='$proy_esc' AND emp_id=$emp_id AND ppe_id=$ppe_id LIMIT 1");
    ...
    if ($ppa_id_rubro > 0) {
        $otros = $mysqli->query("SELECT COUNT(*) AS cnt FROM exa_ppto_proyecto_detalles
            WHERE ppa_id=$ppa_id_rubro AND emp_id=$emp_id AND ppe_id=$ppe_id LIMIT 1");
...
if ($action === 'list') {
    ...
    $res = $mysqli->query("SELECT p.*, pl.plt_nombre FROM exa_ppto_proyectos p
        ...
        WHERE p.emp_id = $emp_id ORDER BY p.proy_nombre");
    if ($res) {
...
if ($action === 'save') {
    ...
    if ($is_edit) {
        $sql = "UPDATE exa_ppto_proyectos SET proy_nombre='$proy_nombre', proy_estado='$proy_estado', plt_id=$plt_id
                WHERE proy_id='$proy_id' AND emp_id=$emp_id";
    } else {
        $sql = "INSERT INTO exa_ppto_proyectos (proy_id, emp_id, proy_nombre, proy_estado, proy_fecha_registro, usu_id, plt_id)
                VALUES ('$proy_id', $emp_id, '$proy_nombre', '$proy_estado', CURDATE(), " . (int)$Ses_Usu_Cod . ", $plt_id)
                ON DUPLICATE KEY UPDATE proy_nombre='$proy_nombre', proy_estado='$proy_estado', plt_id=$plt_id";
    }
    $ok = $mysqli->query($sql);
    if ($ok && !$is_edit && $plt_id !== 'NULL') {
        ppto_proy_clonar_plantilla($mysqli, trim($_POST['proy_id']), (int)$_POST['plt_id'], $emp_id, (int)$Ses_Usu_Cod);
    }
...
if ($action === 'save_rubro') {
    ...
    if (!ppto_partida_es_destino_regla($mysqli, $ppa_id, $emp_id)) {
    ...
    if ($rubro === '' && $ppa_id > 0) {
        $r_desc = $mysqli->query("SELECT ppa_descripcion FROM exa_ppto_partidas WHERE ppa_id=$ppa_id AND emp_id=$emp_id LIMIT 1");
    ...
    if ($pdp_edit <= 0) {
        $pdp_edit = ppto_proy_rubro_id_por_partida($mysqli, $ppe_id, $ppa_id, $proy_id, $emp_id);
    }
    $r_cod = $mysqli->query("SELECT ppa_codigo_clasificacion FROM exa_ppto_partidas WHERE ppa_id=$ppa_id AND emp_id=$emp_id LIMIT 1");
    ...
    if ($ppa_codigo !== '') {
        $val_tope = ppto_proy_validar_tope_grupo_rubro($mysqli, trim($_POST['proy_id']), $emp_id, $ppe_id, $ppa_codigo, $anual, $pdp_edit);
        if (!$val_tope['ok']) {
    ...
    if ($pdp_edit > 0) {
        $sql = "UPDATE exa_ppto_proyecto_detalles
                ...
                WHERE pdp_id=$pdp_edit AND proy_id='$proy_id' AND emp_id=$emp_id AND ppe_id=$ppe_id";
    } else {
        $sql = "INSERT INTO exa_ppto_proyecto_detalles
                (ppe_id, ppa_id, proy_id, emp_id, pdp_rubro, pdp_toneladas_base, pdp_factor_anual_tonelada, pdp_presupuesto_anual, pdp_fecha_registro, usu_id)
                VALUES ($ppe_id, $ppa_id, '$proy_id', $emp_id, '$rubro', $ton, $factor, $anual, NOW(), " . (int)$Ses_Usu_Cod . ")";
    }
    $ok = $mysqli->query($sql);
    ...
    if ($ok) {
        ...
        if ($pdp_id > 0) {
            $dup = ppto_proy_rubro_purgar_duplicados_partida($mysqli, $ppe_id, $ppa_id, $proy_id, $emp_id, $pdp_id);
            if ($dup > 0) {
...
if ($action === 'delete_rubro') {
    ...
    $res = ppto_proy_rubro_eliminar($mysqli, $pdp_id, $proy_id, $emp_id, $ppe_id);
    if (!$res['ok']) {
...
if ($action === 'aplicar_escenario') {
    ...
    $meses_prod_esc = ppto_forecast_cargar_produccion_meses($mysqli, $emp_id, $anio_esc, $proy_id_raw);
    $ton_base_pdf = ppto_proy_version_ton_base($mysqli, $proy_id_raw, $emp_id, $ppe_id);
    $ton_esc_gasto_mes = ppto_proy_ton_escenario_gasto_mes($ton_base_pdf);
    $ton_costo_mes = ppto_proy_version_ton_costo($mysqli, $proy_id_raw, $emp_id, $ppe_id);

    $res = $mysqli->query("SELECT pdp_id, pdp_factor_anual_tonelada, pdp_presupuesto_anual
        FROM exa_ppto_proyecto_detalles
        WHERE proy_id='$proy_id' AND emp_id=$emp_id" . ($ppe_id > 0 ? " AND ppe_id=$ppe_id" : ""));
    $actualizados = 0;
    $total_nuevo = 0.0;
    $pendientes = array();
    if ($res) {
...
if ($action === 'list_rubros') {
    ...
    $data = ppto_proy_cuadro_cargar($mysqli, $emp_id, $proy_id, $ppe_id, $cuadro_vista, $cuadro_mes, $anio_precio);
    ...
    $ajuste = ppto_ajuste_simular($mysqli, $proy_id, $emp_id, $ppe_id, $data, array(
        'escenario' => $esc_sim,
        'anio' => $anio_sim,
    ));
    $data['ajuste_financiero'] = $ajuste;
    $data['ajuste_cfg'] = ppto_ajuste_cfg_get($mysqli, $proy_id, $emp_id, $ppe_id);
    ppto_json($data);
}
...
if ($action === 'save_grupo_meses') {
    ...
    $chk = $mysqli->query("SELECT ppa_id, ppa_codigo_clasificacion, COALESCE(NULLIF(ppa_clase,''),'D') AS ppa_clase
        FROM exa_ppto_partidas WHERE ppa_id=$ppa_id AND emp_id=$emp_id LIMIT 1");
    ...
    if ($raw_meses === '') {
        $ok = $mysqli->query("UPDATE exa_ppto_partidas SET ppa_meses_prorrateo = NULL WHERE ppa_id=$ppa_id AND emp_id=$emp_id");
    ...
    $ok = $mysqli->query("UPDATE exa_ppto_partidas SET ppa_meses_prorrateo = $meses WHERE ppa_id=$ppa_id AND emp_id=$emp_id");
    ppto_json(array(
...
if ($action === 'save_grupo_pct') {
    ...
    if ($raw_pct === '') {
        $ok = $mysqli->query("UPDATE exa_ppto_partidas SET ppa_porcentaje_tope = NULL WHERE ppa_id=$ppa_id AND emp_id=$emp_id");
    ...
    $ok = $mysqli->query("UPDATE exa_ppto_partidas SET ppa_porcentaje_tope = $pct_sql WHERE ppa_id=$ppa_id AND emp_id=$emp_id");
    ppto_json(array(
        'status' => $ok ? 'success' : 'error',
...
if ($action === 'catalogos') {
    ...
    $partidas = array();
    $versiones = array();
    $r1 = $mysqli->query("SELECT plt_id, plt_nombre FROM exa_ppto_plantillas WHERE emp_id=$emp_id AND plt_estado='A'");
    if ($r1) {
        while ($x = $r1->fetch_assoc()) {
            $plantillas[] = $x;
        }
    }
    $r2_list = ppto_partidas_listar($mysqli, array('emp_id' => $emp_id, 'solo_activas' => true, 'clase' => 'D'));
    ...
    $r2g = ppto_partidas_listar($mysqli, array('emp_id' => $emp_id, 'solo_activas' => true, 'clase' => 'G'));
    foreach ($r2g as $x) {
        $partidas_grupo[] = array(
            'ppa_id' => $x['ppa_id'],
            'ppa_codigo_clasificacion' => $x['ppa_codigo_clasificacion'],
    ...
    $r3 = $mysqli->query("SELECT ppe_id, ppe_anio, ppe_version, ppe_descripcion FROM exa_ppto_cabeceras WHERE emp_id=$emp_id ORDER BY ppe_anio DESC, ppe_version DESC");
    if ($r3) {
...
if ($action === 'sugerir_codigo_partida') {
    ...
    $cod = ppto_partida_sugerir_codigo($mysqli, $emp_id, $padre_id);
...
if ($action === 'save_partida_catalogo') {
    ...
    $res = ppto_partida_guardar($mysqli, $emp_id, (int)$Ses_Usu_Cod, array(
...
if ($action === 'get_version_config') {
    ...
    $ton = ppto_version_ton_base_sanitize(ppto_proy_version_ton_base($mysqli, $proy_id, $emp_id, $ppe_id));
    $ton_costo = ppto_proy_version_ton_costo($mysqli, $proy_id, $emp_id, $ppe_id);
...
if ($action === 'save_version_ton') {
    ...
    $res = ppto_proy_version_guardar_ton($mysqli, $proy_id, $emp_id, $ppe_id, $ton, (int)$Ses_Usu_Cod, $aplicar, $tarifa, $iva_div, $ton_costo);
    if (!$res['ok']) {
        ppto_json(array('status' => 'error', 'message' => $res['message']));
    }
    $sync = ppto_proy_version_sync_prod_esperada($mysqli, $proy_id, $emp_id, $ppe_id);
    $msg = $res['message'];
...
if ($action === 'parse_pdf') {
    ...
    if ($proy_parse !== '' && $ppe_parse > 0) {
        $rubros_proyecto = ppto_proy_rubros_map_por_codigo($mysqli, $proy_parse, $emp_id, $ppe_parse);
    ...
    if ($proy_parse !== '' && $ppe_parse > 0) {
        $ton_proy_cfg = ppto_proy_version_ton_base($mysqli, $proy_parse, $emp_id, $ppe_parse);
        if ($ton_proy_cfg > 0) {
...
if ($action === 'import_pdf') {
    ...
    $res = ppto_pdf_importar_presupuesto($mysqli, $emp_id, (int)$Ses_Usu_Cod, $proy_id, $ppe_id, $parsed, $ton_override);
    if (!$res['ok']) {
        ppto_json(array(
            'status' => 'error',
            'message' => $res['message'],
...
if ($action === 'preview_publicar' || $action === 'publish_aprobado' || $action === 'ultima_publicacion') {
    ...
    if ($action === 'ultima_publicacion') {
        $ult = ppto_proy_publicar_ultima($mysqli, $proy_id, $emp_id, $ppe_id);
        ppto_json(array('status' => 'success', 'ultima' => $ult));
    }
    if ($action === 'preview_publicar') {
        $prev = ppto_proy_publicar_preview($mysqli, $proy_id, $emp_id, $ppe_id, $anio);
        if (empty($prev['ok'])) {
    ...
    $res = ppto_proy_publicar_ejecutar($mysqli, $proy_id, $emp_id, $ppe_id, $anio, (int)$Ses_Usu_Cod, $forzar, true);
...
if ($action === 'ajuste_cfg_get' || $action === 'ajuste_cfg_save'
    ...
    if ($action === 'ajuste_cfg_get') {
        ppto_json(array(
            'status' => 'success',
            'cfg' => ppto_ajuste_cfg_get($mysqli, $proy_id, $emp_id, $ppe_id),
            'precios' => ppto_ajuste_precios_list($mysqli, $proy_id, $emp_id, $ppe_id),
        ));
    }
    ...
    if ($action === 'ajuste_cfg_save') {
        ...
        $res = ppto_ajuste_cfg_save($mysqli, $proy_id, $emp_id, $ppe_id, $data, (int)$Ses_Usu_Cod);
        ppto_json(array(
    ...
    if ($action === 'ajuste_precios_save') {
        ...
        $res = ppto_ajuste_precios_save($mysqli, $proy_id, $emp_id, $ppe_id, $precios, (int)$Ses_Usu_Cod);
        ppto_json(array(
            'status' => $res['ok'] ? 'success' : 'error',
            'message' => $res['message'],
    ...
    if ($action === 'ajuste_simular' || $action === 'ajuste_aplicar') {
        ...
        $cuadro = ppto_proy_cuadro_cargar($mysqli, $emp_id, $proy_id, $ppe_id, $cuadro_vista, $cuadro_mes, $anio_precio);
        ...
        $sim = ppto_ajuste_simular($mysqli, $proy_id, $emp_id, $ppe_id, $cuadro, $opts);
        if ($action === 'ajuste_simular') {
            ppto_json(array('status' => 'success', 'sim' => $sim));
        }
        $obs = isset($_POST['observacion']) ? $_POST['observacion'] : '';
        $res = ppto_ajuste_aplicar($mysqli, $proy_id, $emp_id, $ppe_id, $sim, (int)$Ses_Usu_Cod, $obs);
        ppto_json(array(
            'status' => $res['ok'] ? 'success' : 'error',
            'message' => $res['message'],
    ...
    if ($action === 'ajuste_historial') {
        ppto_json(array(
            'status' => 'success',
            'rows' => ppto_ajuste_historial($mysqli, $proy_id, $emp_id, $ppe_id, 30),
        ));
    }

    if ($action === 'ajuste_historial_detalle') {
        $ajc_id = isset($_REQUEST['ajc_id']) ? (int)$_REQUEST['ajc_id'] : 0;
        $det = ppto_ajuste_historial_detalle($mysqli, $ajc_id, $emp_id);
        if (!$det) {
