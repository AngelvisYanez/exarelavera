 ...
 */
function ppto_umbral_pf_resolver($mysqli, $emp_id, $ppa_id = null) {
    $emp_id = (int)$emp_id;
    $pct = PPTO_UMBRAL_PF_DEFAULT;
    $origen = 'global_default';

    $res_g = $mysqli->query("SELECT ubp_umbral_pct FROM exa_ppto_umbral_pf
        WHERE emp_id = $emp_id AND ppa_id IS NULL LIMIT 1");
    ...
    if ($ppa_id !== null && (int)$ppa_id > 0) {
        $ppa_id = (int)$ppa_id;
        $res_p = $mysqli->query("SELECT ubp_umbral_pct FROM exa_ppto_umbral_pf
            WHERE emp_id = $emp_id AND ppa_id = $ppa_id LIMIT 1");
 ...
 * @return string
 */
function ppto_alerta_pf_url_reajuste($emp_id, $ppe_id, $ppa_id, $proy_id = null, $monto = null, $origen = 'd8', $rea_mes = null) {
    $q = array(
        'emp_cod' => (int)$emp_id,
        'ppe_id' => (int)$ppe_id,
 ...
 * @return array
 */
function ppto_alerta_pf_procesar_partidas($mysqli, $emp_id, $partidas, $ppe_id, $proy_id = null) {
    $alertas = array();
    if (empty($partidas)) {
        return $alertas;
    }
    ...
    foreach ($partidas as $row) {
        ...
        $umb = ppto_umbral_pf_resolver($mysqli, $emp_id, (int)$row['ppa_id']);
 ...
 * @return array
 */
function ppto_alerta_reinversion_procesar_partidas($mysqli, $emp_id, $partidas, $ppe_id, $proy_id = null, $rea_mes = null) {
    $alertas = array();
    if (empty($partidas)) {
        return $alertas;
    }
