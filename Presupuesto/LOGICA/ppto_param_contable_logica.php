 ...
 */
function ppto_param_contable_plan_empresa($mysqli, $Emp_Cod, $anio = 0) {
    $Emp_Cod = (int)$Emp_Cod;
    $anio = (int)$anio;
    if ($Emp_Cod <= 0) {
        return null;
    ...
    if ($anio > 0) {
        $sql = "SELECT pc.Pla_Cod AS Pla_Cod, pe.Pec_Cod AS pec_cod, YEAR(pe.Pec_Fei) AS anio
            ...
            WHERE pc.Emp_Cod = $Emp_Cod AND pc.Pla_Est = 'A'
 ...
 * @return array
 */
function ppto_param_contable_kpis($mysqli, $Emp_Cod, $Pla_Cod) {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    $out = array(
        'rubros_detalle' => 0,
    ...
    $res = $mysqli->query("SELECT COUNT(*) AS n FROM pre_partidas
    ...
    $res = $mysqli->query("SELECT COUNT(DISTINCT ppc.Ppa_Cod) AS n
        FROM pre_partida_cuenta ppc
        INNER JOIN pre_partidas p ON (p.Ppa_Cod=ppc.ppa_id OR p.Ppa_Cod=ppc.Ppa_Cod) AND p.Emp_Cod=ppc.Emp_Cod
        WHERE ppc.Emp_Cod=$Emp_Cod AND ppc.Pla_Cod=$Pla_Cod AND ppc.Ppc_Est='A'
          AND p.Ppa_Est='A' AND COALESCE(NULLIF(p.Ppa_Clase,''),'D')='D'");
    if ($res && ($r = $res->fetch_assoc())) {
        $out['rubros_parametrizados'] = (int)$r['n'];
    }
    $out['rubros_pendientes'] = max(0, $out['rubros_detalle'] - $out['rubros_parametrizados']);

    $res = $mysqli->query("SELECT COUNT(*) AS n FROM pre_partida_cuenta
        WHERE Emp_Cod=$Emp_Cod AND Pla_Cod=$Pla_Cod AND ppc_estado='A'");
    if ($res && ($r = $res->fetch_assoc())) {
    ...
    $res = $mysqli->query("SELECT COUNT(*) AS n FROM det_plan d
        WHERE d.Pla_Cod=$Pla_Cod AND d.Pld_Tip='D' AND d.Pld_Est='A'
          AND NOT EXISTS (
            SELECT 1 FROM pre_partida_cuenta ppc
            WHERE ppc.Pld_Cod=d.Pld_Cod AND ppc.Emp_Cod=$Emp_Cod
 ...
 */
function ppto_param_contable_conteo_por_rubro($mysqli, $Emp_Cod, $Pla_Cod) {
    $map = array();
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    $res = $mysqli->query("SELECT ppa_id, COUNT(*) AS n FROM pre_partida_cuenta
        WHERE Emp_Cod=$Emp_Cod AND Pla_Cod=$Pla_Cod AND ppc_estado='A'
        GROUP BY ppa_id");
    if ($res) {
        while ($r = $res->fetch_assoc()) {
 ...
 * @return array
 */
function ppto_param_contable_arbol_partidas($mysqli, $Emp_Cod, $Pla_Cod, $filtro = 'todos') {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    ...
    $partidas = ppto_partidas_listar($mysqli, array('Emp_Cod' => $Emp_Cod, 'solo_activas' => true));
    $conteo = ppto_param_contable_conteo_por_rubro($mysqli, $Emp_Cod, $Pla_Cod);
 ...
 * @return array
 */
function ppto_param_contable_cuentas_rubro($mysqli, $Emp_Cod, $Pla_Cod, $ppa_id) {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    $ppa_id = (int)$ppa_id;
    $rows = array();
    $sql = "SELECT ppc.ppc_id, ppc.Pld_Cod, ppc.ppc_estado, ppc.ppc_fecha_registro,
        ...
        WHERE ppc.Emp_Cod=$Emp_Cod AND ppc.Pla_Cod=$Pla_Cod AND ppc.ppa_id=$ppa_id
 ...
 * @return array
 */
function ppto_param_contable_rubro_detalle($mysqli, $Emp_Cod, $Pla_Cod, $ppa_id) {
    $p = ppto_partida_obtener($mysqli, (int)$ppa_id, (int)$Emp_Cod);
    if (!$p) {
    ...
    $cuentas = ppto_param_contable_cuentas_rubro($mysqli, $Emp_Cod, $Pla_Cod, $ppa_id);
    return array(
        'ok' => true,
        'rubro' => array(
            'ppa_id' => (int)$p['ppa_id'],
 ...
 * @return array
 */
function ppto_param_contable_buscar_cuentas($mysqli, $Emp_Cod, $Pla_Cod, $q = '', $limit = 80, $grupo = 'todas', $filtro = 'todas') {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    ...
    $sql = "SELECT d.Pld_Cod, d.Pld_Cdc, d.Pld_Des, d.Pld_Tip, d.Pld_Est, d.Pld_Deb, d.Pld_Cre,
        ...
        FROM det_plan d
        LEFT JOIN pre_partida_cuenta ppc
            ON ppc.Pld_Cod = d.Pld_Cod AND ppc.Emp_Cod = $Emp_Cod
           AND ppc.Pla_Cod = $Pla_Cod AND ppc.Ppc_Est = 'A'
        LEFT JOIN pre_partidas p ON (p.Ppa_Cod = ppc.ppa_id OR p.Ppa_Cod = ppc.Ppa_Cod) AND p.Emp_Cod = $Emp_Cod
 ...
 * @return array
 */
function ppto_param_contable_cuentas_pendientes($mysqli, $Emp_Cod, $Pla_Cod, $grupo_filtro = 'todas', $q = '', $limit = 0) {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
 ...
 * @return array
 */
function ppto_param_contable_asignar($mysqli, $Emp_Cod, $Pla_Cod, $ppa_id, $Pld_Cod, $Usu_Cod) {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    $ppa_id = (int)$ppa_id;
    $Pld_Cod = (int)$Pld_Cod;
    ...
    $p = ppto_partida_obtener($mysqli, $ppa_id, $Emp_Cod);
    if (!$p || $p['ppa_estado'] !== 'A') {
    ...
    $res = $mysqli->query("SELECT ppc.ppa_id, p.Ppa_Cla AS ppa_codigo_clasificacion, p.Ppa_Des AS ppa_descripcion
        FROM pre_partida_cuenta ppc
        INNER JOIN pre_partidas p ON (p.Ppa_Cod=ppc.ppa_id OR p.Ppa_Cod=ppc.Ppa_Cod) AND p.Emp_Cod=ppc.Emp_Cod
        WHERE ppc.Emp_Cod=$Emp_Cod AND ppc.Pla_Cod=$Pla_Cod AND ppc.Pld_Cod=$Pld_Cod AND ppc.Ppc_Est='A'
        LIMIT 1");
    ...
    $ok = $mysqli->query("INSERT INTO pre_partida_cuenta
        (Emp_Cod, Pla_Cod, ppa_id, Pld_Cod, ppc_estado, ppc_fecha_registro, Usu_Cod)
        VALUES ($Emp_Cod, $Pla_Cod, $ppa_id, $Pld_Cod, 'A', NOW(), $Usu_Cod)");
    if (!$ok) {
 ...
 * @return array
 */
function ppto_param_contable_quitar($mysqli, $Emp_Cod, $ppc_id) {
    $Emp_Cod = (int)$Emp_Cod;
    $ppc_id = (int)$ppc_id;
    $ok = $mysqli->query("DELETE FROM pre_partida_cuenta
        WHERE ppc_id=$ppc_id AND Emp_Cod=$Emp_Cod LIMIT 1");
 ...
 * @return array
 */
function ppto_param_contable_auditar($mysqli, $Emp_Cod, $Pla_Cod, $pec_cod = 0) {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    $pec_cod = (int)$pec_cod;
    $hallazgos = array();

    $res = $mysqli->query("SELECT p.Ppa_Cod AS ppa_id, p.Ppa_Cla AS ppa_codigo_clasificacion, p.Ppa_Des AS ppa_descripcion
        FROM pre_partidas p
        WHERE p.Emp_Cod=$Emp_Cod AND p.Ppa_Est='A'
          AND COALESCE(NULLIF(p.Ppa_Clase,''),'D')='D'
          AND NOT EXISTS (
            SELECT 1 FROM pre_partida_cuenta ppc
            WHERE (ppc.ppa_id=p.Ppa_Cod OR ppc.Ppa_Cod=p.Ppa_Cod) AND ppc.Emp_Cod=$Emp_Cod
              AND ppc.Pla_Cod=$Pla_Cod AND ppc.Ppc_Est='A'
          )
        ORDER BY p.Ppa_Cla
        LIMIT 100");
    ...
    $pend = ppto_param_contable_cuentas_pendientes($mysqli, $Emp_Cod, $Pla_Cod, 'todas', '', 100);
    $items = array();
    foreach ($pend as $c) {
    ...
    $res = $mysqli->query("SELECT d.Pld_Cdc, d.Pld_Des, p.Ppa_Cla AS ppa_codigo_clasificacion
        FROM pre_partida_cuenta ppc
        INNER JOIN det_plan d ON d.Pld_Cod=ppc.Pld_Cod
        INNER JOIN pre_partidas p ON (p.Ppa_Cod=ppc.ppa_id OR p.Ppa_Cod=ppc.Ppa_Cod) AND p.Emp_Cod=ppc.Emp_Cod
        WHERE ppc.Emp_Cod=$Emp_Cod AND ppc.Pla_Cod=$Pla_Cod AND ppc.Ppc_Est='A'
    ...
    $res = $mysqli->query("SELECT Pld_Cod, COUNT(*) AS n FROM pre_partida_cuenta
        WHERE Emp_Cod=$Emp_Cod AND Pla_Cod=$Pla_Cod AND ppc_estado='A'
 ...
 * @return array
 */
function ppto_param_contable_copiar($mysqli, $Emp_Cod, $anio_origen, $anio_destino, $Usu_Cod, $sobreescribir = false) {
    $Emp_Cod = (int)$Emp_Cod;
    ...
    $src = ppto_param_contable_plan_empresa($mysqli, $Emp_Cod, $anio_origen);
    $dst = ppto_param_contable_plan_empresa($mysqli, $Emp_Cod, $anio_destino);
    ...
    $res = $mysqli->query("SELECT ppc.ppa_id, ppc.Pld_Cod, d.Pld_Cdc
        FROM pre_partida_cuenta ppc
        INNER JOIN det_plan d ON d.Pld_Cod = ppc.Pld_Cod
        WHERE ppc.Emp_Cod=$Emp_Cod AND ppc.Pla_Cod=$pla_src AND ppc.ppc_estado='A'");
    if (!$res) {
    ...
    foreach ($origen_rows as $row) {
        ...
        $existe = $mysqli->query("SELECT ppc_id, ppa_id FROM pre_partida_cuenta
            WHERE Emp_Cod=$Emp_Cod AND Pla_Cod=$pla_dst AND Pld_Cod=$pld_dst AND ppc_estado='A' LIMIT 1");
        if ($existe && ($ex = $existe->fetch_assoc())) {
            ...
            $mysqli->query("DELETE FROM pre_partida_cuenta WHERE ppc_id=" . (int)$ex['ppc_id']);
        }

        $r = ppto_param_contable_asignar($mysqli, $Emp_Cod, $pla_dst, $ppa_id, $pld_dst, $Usu_Cod);
        if (!empty($r['ok'])) {
            $copiados++;
        } else {
            $errores++;
 ...
 * @return array
 */
function ppto_param_contable_sugerir($mysqli, $Emp_Cod, $Pla_Cod, $ppa_id, $top = 12) {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    $ppa_id = (int)$ppa_id;
 ...
 * @return array
 */
function ppto_param_contable_mapa($mysqli, $Emp_Cod, $Pla_Cod, $pec_cod = 0, $con_movimientos = false, $filtro = 'todos') {
    $Emp_Cod = (int)$Emp_Cod;
    $Pla_Cod = (int)$Pla_Cod;
    $pec_cod = (int)$pec_cod;
    $filtro = strtolower(trim($filtro));
    ...
    $sql = "SELECT p.Ppa_Cod AS ppa_id, p.Ppa_Cla AS ppa_codigo_clasificacion, p.Ppa_Des AS ppa_descripcion, p.Ppa_Tip AS ppa_tipo, p.Ppa_Nat AS ppa_naturaleza,
            COALESCE(NULLIF(p.Ppa_Clase,''),'D') AS ppa_clase,
            ppc.ppc_id, ppc.Pld_Cod, ppc.Ppc_Est AS ppc_estado, ppc.Ppc_FecReg AS ppc_fecha_registro,
            d.Pld_Cdc, d.Pld_Des, d.Pld_Tip, d.Pld_Est, d.Pld_Deb, d.Pld_Cre
        FROM pre_partidas p
        LEFT JOIN pre_partida_cuenta ppc
            ON (ppc.ppa_id = p.Ppa_Cod OR ppc.Ppa_Cod = p.Ppa_Cod) AND ppc.Emp_Cod = p.Emp_Cod
           AND ppc.Pla_Cod = $Pla_Cod AND ppc.Ppc_Est = 'A'
        LEFT JOIN det_plan d ON d.Pld_Cod = ppc.Pld_Cod
        WHERE p.Emp_Cod = $Emp_Cod AND p.Ppa_Est = 'A'
          AND COALESCE(NULLIF(p.Ppa_Clase,''),'D') = 'D'
        ORDER BY p.Ppa_Cla ASC, d.Pld_Cdc ASC";
    $res = $mysqli->query($sql);
    $map = array();
    $pld_ids = array();
