...
function ppto_param_contable_kpis($mysqli, $Emp_Cod, $Pla_Cod) {
    ...
    $res = $mysqli->query("SELECT COUNT(DISTINCT ppc.ppa_id) AS n
        ...
        INNER JOIN exa_ppto_partidas p ON p.ppa_id=ppc.ppa_id AND p.Emp_Cod=ppc.Emp_Cod
        WHERE ppc.Emp_Cod=$Emp_Cod AND ppc.Pla_Cod=$Pla_Cod AND ppc.ppc_estado='A'
...
function ppto_param_contable_buscar_cuentas($mysqli, $Emp_Cod, $Pla_Cod, $q = '', $limit = 80, $grupo = 'todas', $filtro = 'todas') {
    ...
    $sql = "SELECT d.Pld_Cod, d.Pld_Cdc, d.Pld_Des, d.Pld_Tip, d.Pld_Est, d.Pld_Deb, d.Pld_Cre,
        ...
        LEFT JOIN exa_ppto_partida_cuenta ppc
           ...
           AND ppc.Pla_Cod = $Pla_Cod AND ppc.ppc_estado = 'A'
        LEFT JOIN exa_ppto_partidas p ON p.ppa_id = ppc.ppa_id AND p.Emp_Cod = $Emp_Cod
...
function ppto_param_contable_asignar($mysqli, $Emp_Cod, $Pla_Cod, $ppa_id, $Pld_Cod, $Usu_Cod) {
    ...
    $res = $mysqli->query("SELECT ppc.ppa_id, p.ppa_codigo_clasificacion, p.ppa_descripcion
        FROM exa_ppto_partida_cuenta ppc
        INNER JOIN exa_ppto_partidas p ON p.ppa_id=ppc.ppa_id AND p.Emp_Cod=ppc.Emp_Cod
        WHERE ppc.Emp_Cod=$Emp_Cod AND ppc.Pla_Cod=$Pla_Cod AND ppc.Pld_Cod=$Pld_Cod AND ppc.ppc_estado='A'
        LIMIT 1");
...
function ppto_param_contable_auditar($mysqli, $Emp_Cod, $Pla_Cod, $pec_cod = 0) {
    ...
    $res = $mysqli->query("SELECT p.ppa_id, p.ppa_codigo_clasificacion, p.ppa_descripcion
        ...
        WHERE p.Emp_Cod=$Emp_Cod AND p.ppa_estado='A'
          AND COALESCE(NULLIF(p.ppa_clase,''),'D')='D'
          AND NOT EXISTS (
            SELECT 1 FROM exa_ppto_partida_cuenta ppc
            WHERE ppc.ppa_id=p.ppa_id AND ppc.Emp_Cod=$Emp_Cod
              AND ppc.Pla_Cod=$Pla_Cod AND ppc.ppc_estado='A'
          )
...
function ppto_param_contable_mapa($mysqli, $Emp_Cod, $Pla_Cod, $pec_cod = 0, $con_movimientos = false, $filtro = 'todos') {
    ...
    $sql = "SELECT p.ppa_id, p.ppa_codigo_clasificacion, p.ppa_descripcion, p.ppa_tipo, p.ppa_naturaleza,
        ...
        FROM exa_ppto_partidas p
        LEFT JOIN exa_ppto_partida_cuenta ppc
            ON ppc.ppa_id = p.ppa_id AND ppc.Emp_Cod = p.Emp_Cod
           AND ppc.Pla_Cod = $Pla_Cod AND ppc.ppc_estado = 'A'
