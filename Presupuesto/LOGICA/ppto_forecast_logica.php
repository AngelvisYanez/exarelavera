...
function ppto_forecast_cargar_produccion_meses($mysqli, $Emp_Cod, $anio, $proy_id) {
    ...
    $res = $mysqli->query("SELECT Prd_Mes AS prd_mes, Prd_Esperada AS prd_esperada, Prd_Real AS prd_real, Prd_Proyectada AS prd_proyectada,
            COALESCE(Prd_Est, 'sin_dato') AS prd_estado
        FROM pre_prod_periodos
        WHERE Emp_Cod = " . (int)$Emp_Cod . " AND Prd_Anio = " . (int)$anio . " AND (Pro_Cod = '$esc' OR Pro_Cod IN (SELECT Pro_Cod FROM pre_proyectos WHERE Pro_Ide='$esc'))");
    if ($res) {
...
function ppto_forecast_metodo_proyecto($mysqli, $proy_id, $Emp_Cod) {
    ...
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $res = $mysqli->query("SELECT pco_metodo_forecast FROM pre_prod_config
