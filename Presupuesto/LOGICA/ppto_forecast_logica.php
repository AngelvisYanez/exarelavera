 ...
 * @return array
 */
function ppto_forecast_cargar_produccion_meses($mysqli, $emp_id, $anio, $proy_id) {
    $meses = array();
    for ($m = 1; $m <= 12; $m++) {
        $meses[$m] = array(
            'esperada'   => 0.0,
    ...
    $res = $mysqli->query("SELECT prd_mes, prd_esperada, prd_real, prd_proyectada,
        ...
        WHERE emp_id = " . (int)$emp_id . " AND prd_anio = " . (int)$anio . " AND proy_id = '$esc'");
    if ($res) {
 ...
 * @return string
 */
function ppto_forecast_metodo_proyecto($mysqli, $proy_id, $emp_id) {
    if ($proy_id === null || trim($proy_id) === '') {
        return 'produccion_proyectada';
    }
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $res = $mysqli->query("SELECT pco_metodo_forecast FROM exa_ppto_prod_config
        WHERE proy_id='$esc' AND emp_id=" . (int)$emp_id . " LIMIT 1");
 ...
 */
function ppto_forecast_calcular_global($mysqli, $kpis, $ppe_id, $emp_id, $anio, $mes, $proy_id) {
    ...
    $metodo = ppto_forecast_metodo_proyecto($mysqli, $proy_id, $emp_id);
    if ($metodo !== 'produccion_proyectada') {
        return $kpis;
    }

    $meses = ppto_forecast_cargar_produccion_meses($mysqli, $emp_id, $anio, $proy_id);
