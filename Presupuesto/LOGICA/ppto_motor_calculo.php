...
function ppto_motor_calcular_rubro($mysqli, $id_rubro, $periodo = null, $params = array()) {
    ...
    $mysqli->query("UPDATE pre_proyecto_detalles SET pdp_presupuesto_anual = $presupuesto_anual WHERE pdp_id = $id_rubro");

    // 4. Procesar el desglose y prorrateo mensual en pre_proyecto_detalles_mes
    $sql_meses = "SELECT * FROM pre_proyecto_detalles_mes WHERE pdp_id = $id_rubro ORDER BY pdm_mes ASC";
...
function ppto_motor_calcular_partida($mysqli, $id_partida, $periodo = null, $params = array()) {
    ...
    $sql_det = "SELECT d.Ppe_Cod AS ppe_id, SUM(d.Pde_Mon) AS total_base 
                FROM pre_detalle d
    ...
    $sql_proy = "SELECT pd.Pdp_Cod AS pdp_id 
...
function ppto_motor_calcular_version($mysqli, $id_version, $periodo = null) {
    ...
    $sql_parts = "SELECT Ppa_Cod AS ppa_id FROM pre_partidas WHERE Emp_Cod = $Emp_Cod AND Ppa_Est = 'A'";
