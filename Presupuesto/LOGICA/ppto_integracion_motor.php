...
function ppto_integracion_produccion_registrar($mysqli, $id_proyecto, $periodo, $valor, $tipo, $anio = null, $emp_override = null) {
    ...
    $emp_id = ppto_integracion_emp_id($mysqli, $id_proyecto, $emp_override);
    $usu_id = isset($_SESSION['Ses_Usu_Cod']) ? (int)$_SESSION['Ses_Usu_Cod'] : 1;
    ...
    if ($tipo === 'real') {
        $res_est = $mysqli->query("SELECT prd_estado, prd_real FROM exa_ppto_prod_periodos
            WHERE proy_id='$clean_proy' AND emp_id=$emp_id AND prd_anio=$anio AND prd_mes=$periodo LIMIT 1");
        if ($res_est && ($row_est = $res_est->fetch_assoc()) && isset($row_est['prd_estado']) && $row_est['prd_estado'] === 'cerrado') {
            ...
            $result = ppto_prod_periodo_corregir_real_cerrado(
                $mysqli, $obDatos, $id_proyecto, $emp_id, $anio, $periodo,
                $valor, 'sync', $usu_id, $Ses_Dat_Aut, $request_uri
            );
    ...
    // Ejecuta la inserci?n at?mica con ON DUPLICATE KEY para evitar duplicaci?n de periodos
    $sql = "INSERT INTO exa_ppto_prod_periodos 
                (proy_id, emp_id, prd_anio, prd_mes, $col_campo, prd_fecha_registro, usu_id)
            VALUES 
                ('$clean_proy', $emp_id, $anio, $periodo, $valor, NOW(), $usu_id)
 ...
 * Guarda los resultados en exa_ppto_prod_variaciones.
 *
...
function ppto_integracion_variacion_calcular($mysqli, $id_proyecto, $periodo, $anio = null) {
    ...
    $anio = ($anio !== null) ? (int)$anio : (int)date('Y');
    $clean_proy = $mysqli->real_escape_string(trim($id_proyecto));
    $emp_id = ppto_integracion_emp_id($mysqli, $id_proyecto);
    $var_porcentual = 0.00;

    // 1. Consultar el hist?rico cargado para el periodo
    $sql = "SELECT prd_esperada, prd_real 
            FROM exa_ppto_prod_periodos 
            WHERE proy_id = '$clean_proy' AND emp_id = $emp_id AND prd_anio = $anio AND prd_mes = $periodo 
            LIMIT 1";
    $res = $mysqli->query($sql);
    ...
    // 2. Insertar o actualizar la variaci?n en la tabla relacional
    $mysqli->query("INSERT INTO exa_ppto_prod_variaciones 
                        (proy_id, emp_id, var_anio, var_mes, var_absoluta, var_porcentual, var_fecha_calculo)
                    VALUES 
                        ('$clean_proy', $emp_id, $anio, $periodo, $var_absoluta, $var_porcentual, NOW())
                    ON DUPLICATE KEY UPDATE 
 ...
 * @return array
 */
function ppto_integracion_proyectar_esperada_mes($mysqli, $id_proyecto, $anio, $mes_destino, $emp_id, $clean_proy) {
    $res_esp = $mysqli->query("SELECT prd_esperada FROM exa_ppto_prod_periodos
        WHERE proy_id='$clean_proy' AND emp_id=$emp_id AND prd_anio=$anio AND prd_mes=$mes_destino LIMIT 1");
    $valor = 0.0;
...
function ppto_integracion_proyectar_ultima_calculada($mysqli, $id_proyecto, $anio, $mes_destino, $emp_id, $clean_proy, $mes_ult_real) {
    $res_last = $mysqli->query("SELECT prd_proyectada FROM exa_ppto_prod_periodos
        WHERE proy_id='$clean_proy' AND emp_id=$emp_id AND prd_anio=$anio
    ...
    if ($mes_ult_real > 0) {
        $res_fb = $mysqli->query("SELECT SUM(prd_real) AS acum FROM exa_ppto_prod_periodos
            WHERE proy_id='$clean_proy' AND emp_id=$emp_id AND prd_anio=$anio AND prd_mes <= $mes_ult_real");
...
function ppto_integracion_proyectar_promedio_siguiente_mes($mysqli, $id_proyecto, $anio = null, $periodo_corte = null, $emp_override = null, $mes_destino = null) {
    ...
    $res_m = $mysqli->query("SELECT MAX(prd_mes) AS mes FROM exa_ppto_prod_periodos
        WHERE proy_id='$clean_proy' AND emp_id=$emp_id AND prd_anio=$anio AND prd_real > 0");
    $mes_corte_global = 0;
    ...
    if ($mes_destino === 1) {
        $valor = ppto_integracion_proyectar_esperada_mes($mysqli, $id_proyecto, $anio, 1, $emp_id, $clean_proy);
        if ($valor <= 0) {
    ...
    $res_cut = $mysqli->query("SELECT MAX(prd_mes) AS mes FROM exa_ppto_prod_periodos
        WHERE proy_id='$clean_proy' AND emp_id=$emp_id AND prd_anio=$anio AND prd_real > 0 AND prd_mes <= $mes_ref");
    $mes_ult_real = 0;
    ...
    if ($mes_ref > $mes_ult_real) {
        $valor = ppto_integracion_proyectar_ultima_calculada($mysqli, $id_proyecto, $anio, $mes_destino, $emp_id, $clean_proy, $mes_ult_real);
        $modo = 'mantenida';
        if ($valor <= 0) {
            return array('ok' => false, 'message' => 'No hay proyectada previa ni real suficiente para el mes ' . $mes_destino . '.');
        }
    } else {
        $res_acum = $mysqli->query("SELECT SUM(prd_real) AS acum FROM exa_ppto_prod_periodos
            WHERE proy_id='$clean_proy' AND emp_id=$emp_id AND prd_anio=$anio
...
function ppto_integracion_recalcular($mysqli, $id_version, $periodo, $proyecto_id = null) {
    ...
    $periodo = (int)$periodo;
    $emp_id = ppto_integracion_emp_id($mysqli, $proyecto_id);
    ...
    $sql_rubros = "SELECT r.*, pp.ppe_anio 
                   FROM exa_ppto_proyecto_detalles r
                   INNER JOIN exa_ppto_cabeceras pp ON r.ppe_id = pp.ppe_id
                   WHERE r.ppe_id = $id_version AND r.emp_id = $emp_id $cond_proy";
    ...
    while ($rubro = $res_rubros->fetch_assoc()) {
        ...
        $ext_prod = ppto_prod_obtener($mysqli, $proy_id, $periodo, array('emp_id' => $emp_id, 'anio' => $anio));
