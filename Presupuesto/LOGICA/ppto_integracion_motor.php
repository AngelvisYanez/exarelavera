...
function ppto_integracion_proyectar_esperada_mes($mysqli, $id_proyecto, $anio, $mes_destino, $Emp_Cod, $clean_proy) {
    $res_esp = $mysqli->query("SELECT prd_esperada FROM exa_ppto_prod_periodos
        WHERE proy_id='$clean_proy' AND Emp_Cod=$Emp_Cod AND prd_anio=$anio AND prd_mes=$mes_destino LIMIT 1");
    $valor = 0.0;
...
function ppto_integracion_proyectar_ultima_calculada($mysqli, $id_proyecto, $anio, $mes_destino, $Emp_Cod, $clean_proy, $mes_ult_real) {
    ...
    if ($mes_ult_real > 0) {
        $res_fb = $mysqli->query("SELECT SUM(prd_real) AS acum FROM exa_ppto_prod_periodos
            WHERE proy_id='$clean_proy' AND Emp_Cod=$Emp_Cod AND prd_anio=$anio AND prd_mes <= $mes_ult_real");
...
function ppto_integracion_proyectar_promedio_siguiente_mes($mysqli, $id_proyecto, $anio = null, $periodo_corte = null, $emp_override = null, $mes_destino = null) {
    ...
    $res_cut = $mysqli->query("SELECT MAX(prd_mes) AS mes FROM exa_ppto_prod_periodos
        WHERE proy_id='$clean_proy' AND Emp_Cod=$Emp_Cod AND prd_anio=$anio AND prd_real > 0 AND prd_mes <= $mes_ref");
    $mes_ult_real = 0;
...
function ppto_integracion_recalcular($mysqli, $id_version, $periodo, $proyecto_id = null) {
    ...
    $sql_rubros = "SELECT r.*, pp.ppe_anio 
                   ...
                   WHERE r.ppe_id = $id_version AND r.Emp_Cod = $Emp_Cod $cond_proy";
