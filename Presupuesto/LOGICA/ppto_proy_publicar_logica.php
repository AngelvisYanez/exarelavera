 ...
 * @return array|null
 */
function ppto_proy_publicar_ultima($mysqli, $proy_id, $emp_id, $ppe_id) {
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $res = $mysqli->query("SELECT pub_id, pub_total_nuevo, pub_fecha_registro, pub_anio, usu_id
 ...
 * @return array
 */
function ppto_proy_publicar_listar_rubros($mysqli, $proy_id, $emp_id, $ppe_id) {
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $rows = array();
 ...
 * @return array
 */
function ppto_proy_publicar_preview($mysqli, $proy_id, $emp_id, $ppe_id, $anio) {
    $rubros = ppto_proy_publicar_listar_rubros($mysqli, $proy_id, $emp_id, $ppe_id);
    if (empty($rubros)) {
        return array('ok' => false, 'message' => 'No hay rubros en este proyecto y version.');
    }

    $meses_prod = ppto_forecast_cargar_produccion_meses($mysqli, $emp_id, $anio, $proy_id);
    $total_vigente = 0.0;
    $total_publicar = 0.0;
    $driver = 0;
    $fijo = 0;
    ...
    $ultima = ppto_proy_publicar_ultima($mysqli, $proy_id, $emp_id, $ppe_id);
 ...
 * @return array
 */
function ppto_proy_limpiar_detalles_contaminados_proyecto($mysqli, $emp_id, $ppe_id) {
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    if ($emp_id <= 0 || $ppe_id <= 0) {
        return array('ok' => false, 'message' => 'Parametros invalidos.', 'eliminados' => 0);
    }
    $sql = "DELETE d FROM exa_ppto_detalles d
        INNER JOIN (
            SELECT DISTINCT ppa_id
            FROM exa_ppto_proyecto_detalles
            WHERE emp_id=$emp_id AND ppe_id=$ppe_id
              AND proy_id IS NOT NULL AND proy_id != ''
        ) px ON px.ppa_id = d.ppa_id
        WHERE d.ppe_id=$ppe_id";
    $ok = $mysqli->query($sql);
 ...
 * @return array
 */
function ppto_proy_publicar_ejecutar($mysqli, $proy_id, $emp_id, $ppe_id, $anio, $usu_id, $forzar_republicacion = false, $sync_detalles = false) {
    $preview = ppto_proy_publicar_preview($mysqli, $proy_id, $emp_id, $ppe_id, $anio);
    if (empty($preview['ok'])) {
        return $preview;
    }
    ...
    $mysqli->query("INSERT INTO exa_ppto_proyecto_publicacion
        (proy_id, emp_id, ppe_id, pub_anio, pub_total_anterior, pub_total_nuevo,
         pub_rubros_driver, pub_rubros_fijo, pub_modo, pub_notas, pub_fecha_registro, usu_id)
        VALUES ('$esc', $emp_id, $ppe_id, $anio, $tot_ant, $tot_new, $drv, $fij, 'proyectada', '$notas', NOW(), $usu_id)");
 ...
 */
function ppto_proy_publicar_aprobaciones_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio) {
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $anio = (int)$anio;
    $out = array();
    $res = $mysqli->query("SELECT pub_id, pub_mes, pub_total_nuevo, pub_total_anterior, pub_notas, pub_modo, pub_fecha_registro, usu_id
        FROM exa_ppto_proyecto_publicacion
        WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id AND pub_anio=$anio
 ...
 */
function ppto_proy_publicar_historial_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio, $mes) {
    $esc = $mysqli->real_escape_string(trim($proy_id));
    $emp_id = (int)$emp_id;
    $ppe_id = (int)$ppe_id;
    $anio = (int)$anio;
    ...
    $res = $mysqli->query("SELECT pub_id, pub_mes, pub_total_nuevo, pub_total_anterior, pub_notas, pub_modo, pub_fecha_registro, usu_id
        FROM exa_ppto_proyecto_publicacion
        WHERE proy_id='$esc' AND emp_id=$emp_id AND ppe_id=$ppe_id AND pub_anio=$anio AND pub_mes=$mes
 ...
 * @return array
 */
function ppto_proy_publicar_preview_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio, $mes, $ton_proyectada_override = null) {
    $mes = (int)$mes;
    if ($mes < 1 || $mes > 12) {
    ...
    $aprob = ppto_proy_publicar_aprobaciones_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio);
 ...
 * @return array
 */
function ppto_proy_publicar_ejecutar_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio, $mes, $usu_id, $ton_proyectada_override = null, $confirmar_reaprobacion = false) {
    $preview = ppto_proy_publicar_preview_mes($mysqli, $proy_id, $emp_id, $ppe_id, $anio, $mes, $ton_proyectada_override);
    if (empty($preview['ok'])) {
        return $preview;
    }
    ...
    $ins = $mysqli->query("INSERT INTO exa_ppto_proyecto_publicacion
        (proy_id, emp_id, ppe_id, pub_anio, pub_mes, pub_total_anterior, pub_total_nuevo,
         pub_rubros_driver, pub_rubros_fijo, pub_modo, pub_notas, pub_fecha_registro, usu_id)
        VALUES ('$esc', " . (int)$emp_id . ", " . (int)$ppe_id . ", " . (int)$anio . ", $mes,
         $tot_ant, $tot_new, 0, 0, 'proyectada_mes', '$notas', NOW(), " . (int)$usu_id . ")");
    if (!$ins) {
        return array(
            'ok' => false,
