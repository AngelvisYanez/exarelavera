...
function ppto_proy_publicar_listar_rubros($mysqli, $proy_id, $Emp_Cod, $ppe_id) {
    ...
    $ppe_id = (int)$ppe_id;
    $rows = array();
    $sql = "SELECT d.Pdp_Cod AS pdp_id, d.Ppa_Cod AS ppa_id, d.Pdp_Rubro AS pdp_rubro, d.Pdp_FacAnualTon AS pdp_factor_anual_tonelada,
            d.Pdp_TonBase AS pdp_toneladas_base, d.Pdp_PreAnual AS pdp_presupuesto_anual, p.Ppa_Cla AS ppa_codigo_clasificacion
...
function ppto_proy_publicar_validar_piso_movimiento($mysqli, $pdp_id, $montos_mes) {
    $pdp_id = (int)$pdp_id;
    $res = $mysqli->query("SELECT Pdm_Mes AS pdm_mes, Pdm_Comprometido AS pdm_comprometido, Pdm_Ejecutado AS pdm_ejecutado
        FROM pre_proyecto_detalles_mes WHERE Pdp_Cod=$pdp_id");
    if (!$res) {
        return null;
    }
 ...
 * El presupuesto de proyecto vive en pre_proyecto_detalles / _mes.
 *
 * @param mysqli $mysqli
 * @param int $ppe_id
...
function ppto_proy_publicar_vigente_rubro_mes($mysqli, $pdp_id, $mes) {
    $pdp_id = (int)$pdp_id;
    $mes = (int)$mes;
    $res = $mysqli->query("SELECT pdm_presupuesto_mensual FROM pre_proyecto_detalles_mes
...
function ppto_proy_publicar_recalc_anual_rubro($mysqli, $pdp_id) {
    ...
    $mysqli->query("UPDATE pre_proyecto_detalles SET Pdp_PreAnual=$anual WHERE Pdp_Cod=$pdp_id");
}
