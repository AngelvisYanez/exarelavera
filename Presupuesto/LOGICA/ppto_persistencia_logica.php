...
function ppto_persistencia_consultar($mysqli, $caso, $p) {
    switch ($caso) {
        case 1:
            ...
            $sql = "SELECT ppe_id FROM exa_ppto_cabeceras WHERE Emp_Cod = '$Emp_Cod' AND ppe_anio = '$ani' AND ppe_estado = 'A' LIMIT 1";
        ...
        case 2:
            ...
            $sql = "SELECT * FROM exa_ppto_reglas WHERE Emp_Cod = '$Emp_Cod' AND prg_tipo_documento = '$tip_doc' AND prg_estado = 'A' ORDER BY prg_prioridad ASC";
            $res = $mysqli->query($sql);
            $data = array();
            if ($res) {
        ...
        case 4:
            ...
            $sql = "INSERT INTO exa_ppto_ejecuciones (
                        ...
                        pej_fecha_documento, pej_fecha_registro, Usu_Cod, prg_id, pej_fase, pej_rubro
                    ) VALUES (
                        $ppe_id, $ppa_id, $Emp_Cod, $Suc_Cod, $Dep_Cod, $proy_id,
        ...
        case 7:
            ...
            $sql = "INSERT IGNORE INTO exa_ppto_alertas (ppe_id, ppa_id, pal_umbral, pal_porcentaje_actual, pej_id, pal_leido, pal_fecha_registro, Usu_Cod)
        ...
        case 8:
            ...
            if ($ppe_id_f > 0) {
                ...
                $where_pp_proy = "pd.Emp_Cod = $Emp_Cod AND pd.ppe_id IN (
                    SELECT ppe_id FROM exa_ppto_cabeceras
                    WHERE Emp_Cod = $Emp_Cod AND ppe_anio = $ani AND ppe_estado = 'A'
                )";
            }
            ...
            $sql = "SELECT 
                    ...
                    LEFT JOIN (
                        ...
                        INNER JOIN exa_ppto_partidas px ON px.ppa_id = pd.ppa_id
                            AND px.Emp_Cod = $Emp_Cod AND px.ppa_estado = 'A'
                        WHERE $where_pp_det
                          AND $where_mes_det
                        GROUP BY pd.ppa_id
...
function ppto_admin_metricas_consolidado($mysqli, $Emp_Cod, $ppe_id, $mes_hasta, $vista = 'acumulado') {
    ...
    $sql_std = "SELECT
        ...
        WHERE $base_where
          ...
          AND r.ppa_id NOT IN (
              ...
              FROM exa_ppto_proyecto_detalles pd
              WHERE pd.Emp_Cod = $Emp_Cod AND pd.ppe_id = $ppe_id
                AND pd.proy_id IS NOT NULL AND pd.proy_id != ''
          )";
    ...
    $res_np = $mysqli->query("SELECT proy_id, proy_nombre FROM exa_ppto_proyectos
        WHERE Emp_Cod = $Emp_Cod AND proy_estado = 'A'");
    if ($res_np) {
