 ...
 * @param int|null $anio
 */
function ppto_sync_relavera_produccion($mysqli, $proy_id, $emp_id = null, $anio = null) {
    ppto_hooks_cargar();
    ...
    $emp_id = ($emp_id !== null) ? (int)$emp_id : (function_exists('ppto_resolve_emp_id') ? ppto_resolve_emp_id() : 1);
    for ($mes = 1; $mes <= 12; $mes++) {
        $ext = ppto_prod_obtener($mysqli, $proy_id, $mes, array('emp_id' => $emp_id, 'anio' => $anio));
        if (!empty($ext['valor'])) {
            ppto_integracion_produccion_registrar($mysqli, $proy_id, $mes, (float)$ext['valor'], 'real', $anio, $emp_id);
        }
    }
    if (function_exists('ppto_integracion_proyectar_promedio_siguiente_mes')) {
        ppto_integracion_proyectar_promedio_siguiente_mes($mysqli, $proy_id, $anio, null, $emp_id);
    } elseif (function_exists('ppto_integracion_proyectar_cierre')) {
        ppto_integracion_proyectar_cierre($mysqli, $proy_id, (int)date('n'), $anio);
    }
}
