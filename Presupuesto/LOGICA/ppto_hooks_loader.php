 ...
 * @param int|null $anio
 */
function ppto_sync_relavera_produccion($mysqli, $proy_id, $Emp_Cod = null, $anio = null) {
    ppto_hooks_cargar();
    ...
    for ($mes = 1; $mes <= 12; $mes++) {
        $ext = ppto_prod_obtener($mysqli, $proy_id, $mes, array('Emp_Cod' => $Emp_Cod, 'anio' => $anio));
        if (!empty($ext['valor'])) {
            ppto_integracion_produccion_registrar($mysqli, $proy_id, $mes, (float)$ext['valor'], 'real', $anio, $Emp_Cod);
        }
    }
    if (function_exists('ppto_integracion_proyectar_promedio_siguiente_mes')) {
        ppto_integracion_proyectar_promedio_siguiente_mes($mysqli, $proy_id, $anio, null, $Emp_Cod);
