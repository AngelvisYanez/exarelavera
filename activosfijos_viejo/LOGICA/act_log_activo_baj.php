<?
	...
	require_once ("act_sql_activo_baj.php");
...
class Class_Log_Datos_Con extends MysqlDatos{
	...
	*/
	...
	/**
	 * Ejecuta consulta a la base de datos para verificar si existen datos en persona y personal
	 * @param int $Sen_Sql_Per numero de sentencia de la persona a buscar
	 * @param int $Sen_Sql_Cli numero de sentencia del personal a buscar
	...
	{	
	    /**
		 * Verificvar en la tabla personal si existe la persona.
		 */
		...
		/**
		 * Verifico si existe en la tabla Encargado de mantenimiento
		 */
		...
		*/
		...
		{
			/**
			 * registrar persona - encargado de mantenimiento
			 */
			$event = 1;
		}
	...
	function InsetPersonaMantenim($sen_Prs,$paramPrs,$sen_Mant,$paramMant, $pag, $Ses_Usu_Cod)
	{
		/**
		 * Objeto de coneccion
		 * @var Class_Log_Conexion_Rhu
		 */
		$obBD = new Class_Log_Conexion_Con;
		 ...
		 */
		 $this->operacionobBD($sen_Prs,$paramPrs,$obBD);
		//$this->grabarv_registros(sentencias_rhu($sen_Prs,$this->parametros($paramPrs)),$obBD->conexion);
		...
		$paramMant = $Id.'*'.$paramMant;
		/**
		 * Grabar registro en encargado mantenimeinto
		 */
		 $this->operacionobBD($sen_Mant,$paramMant,$obBD);
		//$this->grabarv_registros(sentencias_rhu($sen_Cli,$this->parametros($paramCli)),$obBD->conexion);
	
		//$this->grabarAuditoria($pag, $Ses_Usu_Cod, $obBD);
