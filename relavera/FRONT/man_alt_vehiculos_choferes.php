...
if (isset($_POST['saveChoferAjax'])) {
    ...
    try {
        ...
        // Buscar chofer
        $chofer = $obBD_con1->getArrayConsultaSql("SELECT Cho_Cod, IFNULL(Cho_Tip, '') as Cho_Tip FROM chofer WHERE Prs_Cod = $Prs_Cod AND Emp_Cod = $Ses_Emp_Cod LIMIT 1", $obBD_conexion);
        if (!empty($chofer)) {
...
if (isset($_POST['saveVehiculoAjax'])) {
    ...
    try {
        ...
        // Buscar vehículo
        $veh_exist = $obBD_con1->getArrayConsultaSql("SELECT Veh_Cod, IFNULL(Veh_Tip, '') as Veh_Tip FROM vehiculo WHERE Veh_Pla = '$Veh_Pla' AND Emp_Cod = $Ses_Emp_Cod AND Veh_Est = 'A' LIMIT 1", $obBD_conexion);
        if (!empty($veh_exist)) {
