...
require_once('../LOGICA/log_man_busq_doc_2.0.php');
...
// Listar manifiestos por estado (Grid)
if(isset($_GET['listarManifiestosPorEstadoAjax']) || isset($listarManifiestosPorEstadoAjax)){
    $resp = array('success' => false, 'message' => '', 'data' => null);
    ...
    // Usamos getConsulta para traer múltiples registros (Case 5)
    $data = $obBD_con->getArrayConsulta(5, $valores, $obBD_conexion);
    
    if ($obBD_con->Error == 0) {
        if ($data && !empty($data)) {
            $resp['success'] = true;
            $resp['data'] = $data;
            $resp['message'] = 'Registros encontrados';
            $resp['tipo_mensaje'] = 'success';
        } else {
            $resp['success'] = true; // Éxito pero sin datos, para limpiar grid
            $resp['data'] = array();
            $resp['message'] = 'No se encontraron documentos con ese estado';
            $resp['tipo_mensaje'] = 'info';
        }
    } else {
        $resp['message'] = 'Error en la consulta: ' . $obBD_con->MsgError;
...
if(isset($_GET['aprobarManifiestoAjax']) || isset($aprobarManifiestoAjax)){
    ...
    if ($estado_actual == 'P') {
        // Validación de fecha: No permitir aprobar si es de una fecha anterior
        // $fecha_man = isset($data['Man_Fec']) ? substr($data['Man_Fec'], 0, 10) : '';
        // $fecha_actual = date('Y-m-d');

        // Validación de FECHA: No permitir aprobar si ya pasó el día
        $fecha_man_dia = isset($data['Man_Fec']) ? substr($data['Man_Fec'], 0, 10) : '';
        $fecha_actual_dia = date('Y-m-d');
        $fecha_actual_full = date('Y-m-d H:i:s');
        
        // if ($fecha_man < $fecha_actual) {
        //     $resp['message'] = 'El documento ha expirado. No se puede aprobar un manifiesto de una fecha anterior.';
        if (!empty($fecha_man_dia) && $fecha_man_dia < $fecha_actual_dia) {
            $resp['message'] = 'El documento ha expirado. El manifiesto pertenece a una fecha anterior.';
            $obBD_con->echoJson($resp);
            exit();
        }

        // Nueva validación: Man_Fea debe ser comparado con la hora actual
        $man_fea = isset($data['Man_Fea']) ? $data['Man_Fea'] : '';

        if (!empty($man_fea)) {
            // Si la hora actual ya superó la hora de Man_Fea, está expirado
            if ($man_fea < $fecha_actual_full) {
                $resp['message'] = 'El documento ha expirado. El tiempo de llegada/atención ha caducado.';
                $obBD_con->echoJson($resp);
                exit();
            }

            // Validar que el día de Man_Fea no sea posterior al día de Man_Fec
            $man_fea_dia = substr($man_fea, 0, 10);
            if (!empty($fecha_man_dia) && $man_fea_dia > $fecha_man_dia) {
                $resp['message'] = 'El documento ha expirado. La fecha de llegada es posterior a la del documento.';
                $obBD_con->echoJson($resp);
                exit();
            }
        }
...
<HTML>
    <HEAD>
        <TITLE><?php echo "Control Garita"; ?></TITLE>
        <meta charset="UTF-8">
        ...
        <style>
            ...
            @media (max-width: 992px) {
                ...
                /* Fila de inputs unificada y horizontal */
                .row-mobile-inputs {
                    display: flex;
                    flex-direction: row;
                    align-items: stretch;
                ...
                .row-mobile-inputs .input-numero {
                    ...
                    min-width: 100px;
                    border-right: none; /* Quitar borde derecho ya que es el último */
                }
                
                /* Display manifiesto eliminado */

                /* Botón Buscar y Limpiar en Móvil */
                .btn-group-mobile {
                    display: flex;
                    width: 100%;
    ...
    <BODY>
        <div class="panel panel-main">
            <div class="panel-heading exa-header">
                <h3 class="panel-title">&raquo; Control de Garita</h3>
            </div>
                ...
                <div class="search-container">
                    <form name="searchForm" id="searchForm" class="form-horizontal normal">
                        ...
                        <fieldset class="exa-fieldset">
                            ...
                            <div class="form-group">
                                <label class="col-xs-2 control-label">Filtrado Por:</label>
                                <div class="col-xs-6">
                                    <span class="radioset">
                                ...
                                <div class="col-xs-4" style="padding-left: 0; padding-right: 0;">
                                    <div class="row" style="margin: 0;">
                                        <div class="col-xs-6 col-sm-4" style="display: flex; align-items: center; justify-content: flex-end; padding-right: 5px;">
                                            <label class="control-label" style="font-size: 16px;">Estado:</label>
                                        </div>
                                        <div class="col-xs-6 col-sm-8">
                                            <select name="Man_Tip" id="Man_Tip" class="form-control" style="text-align:center; height: 40px; font-size: 16px;">
                                ...
                                <div class="col-xs-6">
                                    ...
                                    <div class="input-group manifiesto-fields">
                                        ...
                                        <span class="input-group-btn">
                                            ...
                                            <button type="button" id="btnCleanSearchManifiesto" class="btn btn-danger" title="Limpiar Búsqueda" style="margin-left: 5px;">
                                    ...
                                    <div class="manifiesto-fields-mobile">
                                        <!-- Fila única de inputs horizontal -->
                                        ...
                                        <div class="btn-group-mobile">
                                            <button type="button" id="btnSearchManifiestoMobile" class="btn btn-success btn-lg" title="Buscar Documento">
                                                <span class="glyphicon glyphicon-search"></span>
                                                Buscar
                                            </button>
                                            <button type="button" id="btnCleanSearchManifiestoMobile" class="btn btn-danger btn-lg" title="Limpiar Búsqueda">
                                                <span class="glyphicon glyphicon-erase"></span>
                                                Limpiar
                                            </button>
                                        </div>
                                ...
                                <div class="col-xs-10">
                                    <div class="input-group">
                                        ...
                                        <span class="input-group-btn">
                                            ...
                                            <button type="button" id="btnCleanSearchPlaca" class="btn btn-danger" title="Limpiar Búsqueda" style="margin-left: 5px;">
                                                <span class="glyphicon glyphicon-erase"></span>
                                                Limpiar
                                            </button>
                                        </span>
                    ...
                    <div id="resultContainer" class="result-container">
                        ...
                        <div id="resultData" class="result-data" style="display: none;">
                            <div class="row">
                                <span class="label" style="color: black; font-size: 16px;">CHOFER:</span>
                                <span class="value" id="choferValue" style="font-size: 16px;"></span>
                            </div>
                            <div class="row">
                                <span class="label" style="color: black; font-size: 16px;">PLACA:</span>
                                <span class="value" id="placaValue" style="font-size: 16px;"></span>
                            </div>
                            <div class="row">
                                <span class="label" style="color: black; font-size: 16px;">No. MANIFIESTO:</span>
                                <span class="value" id="manifiestoValue" style="font-size: 16px;"></span>
                            </div>
                            <div class="row">
                                <span class="label" style="color: black; font-size: 16px;">FECHA:</span>
                                <span class="value" id="fechaValue" style="font-size: 16px;"></span>
                            </div>
                            <div class="row">
                                <span class="label" style="color: black; font-size: 15px;">HORA DE ARRIBO:</span>
                                <span class="value" id="fechaArriboValue" style="font-size: 16px;"></span>
                            </div>
                            <div class="row" id="notaProceso" style="margin-top: 15px; padding-top: 15px; border-top: 2px solid #ddd; display: none;">
                                <div style="text-align: center; width: 100%; padding: 10px;">
                                    <span style="color: #31708f; font-size: 16px; font-weight: bold; font-style: italic;" id="notaTexto"></span>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Contenedor Grid para resultados múltiples -->
                    <div id="gridContainer" class="row" style="margin-top: 20px;"></div>
                    ...
                    <div class="form-group" id="rescanButtonGroup" style="display: none; margin-top: 15px;">
                        <div class="col-xs-12 text-center">
                            <button type="button" id="btnRescanFromResult" class="btn btn-info">
                                <span class="glyphicon glyphicon-qrcode"></span>
        ...
        <script src="../VALIDACIONES/man_val_busq_doc_2.0.js?e=6"></script>
