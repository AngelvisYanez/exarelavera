...
if (isset($LoadManifAjax)) {
    ...
    if (count($origenRows) > 0) {
        foreach ($origenRows as $row) {
            $abonoRaw = isset($row['Abono']) ? (string)$row['Abono'] : '0';
            $amaValRaw = isset($row['Ama_Val']) ? (string)$row['Ama_Val'] : '0';
            $abono = floatval(str_replace(array(',', ' '), '', $abonoRaw));
            $amaVal = floatval(str_replace(array(',', ' '), '', $amaValRaw));
            $saldo = $amaVal - $abono;
            ...
            if ($abono > 0) {
                $consumos = $obBD_con1->getArrayConsulta(37, array(
                    'Ama_Cod' => $amaCod,
                    'Fec_IniM' => isset($parms['Fec_IniM']) ? $parms['Fec_IniM'] : '',
                    'Fec_FinM' => isset($parms['Fec_FinM']) ? $parms['Fec_FinM'] : '',
                ), $obBD_conexion);
                ...
                foreach ($consumos as $c) {
                    ...
                    $tc = floatval(str_replace(array(',', ' '), '', $tcRaw));
                    if ($tc <= 0) {
                        continue;
                    }
                    ...
                    $rowConsumo['row_id'] = 'C_' . $amaCod . '_' . $comCodC;
                    $rowConsumo['Ama_Tip'] = 'C';
                    $rowConsumo['_fec_mov'] = ($fecCons !== '' ? $fecCons : (isset($row['Ama_Fec']) ? $row['Ama_Fec'] : ''));
        ...
        /* Un solo comprobante de consumo = una fila (como estado de cuenta anticipo clientes por Com_Cod) */
        $filasA = array();
        $consumoPorCom = array();
        $ordenComCons = array();
        foreach ($rowsKardex as $r) {
            $tipo = isset($r['_tipo_linea']) ? strtoupper(trim((string) $r['_tipo_linea'])) : '';
            if ($tipo !== 'C') {
                $filasA[] = $r;
                continue;
            }
            $cc = '';
            if (isset($r['Com_Cod_Consumo']) && trim((string) $r['Com_Cod_Consumo']) !== '') {
                $cc = trim((string) $r['Com_Cod_Consumo']);
            } elseif (isset($r['Com_Cod']) && trim((string) $r['Com_Cod']) !== '') {
                $cc = trim((string) $r['Com_Cod']);
            }
            if ($cc === '') {
                $filasA[] = $r;
                continue;
            }
            if (!isset($consumoPorCom[$cc])) {
                $consumoPorCom[$cc] = $r;
                $ordenComCons[] = $cc;
            } else {
                $m = &$consumoPorCom[$cc];
                $m['Abono'] = floatval($m['Abono']) + floatval(isset($r['Abono']) ? $r['Abono'] : 0);
                $fN = isset($r['_fec_mov']) ? trim((string) $r['_fec_mov']) : (isset($r['Ama_Fec']) ? trim((string) $r['Ama_Fec']) : '');
                $fO = isset($m['_fec_mov']) ? trim((string) $m['_fec_mov']) : (isset($m['Ama_Fec']) ? trim((string) $m['Ama_Fec']) : '');
                if ($fN !== '' && ($fO === '' || strcmp($fN, $fO) < 0)) {
                    $m['_fec_mov'] = $fN;
                    $m['Ama_Fec'] = $fN;
                }
                $cliO = isset($m['cliente']) ? trim((string) $m['cliente']) : '';
                $cliN = isset($r['cliente']) ? trim((string) $r['cliente']) : '';
                if ($cliN !== '' && $cliO !== '' && strcasecmp($cliN, $cliO) !== 0 && stripos($cliO . ' / ', $cliN . ' / ') === false) {
                    $m['cliente'] = $cliO . ' / ' . $cliN;
                }
                $amO = isset($m['Ama_Cod']) ? intval($m['Ama_Cod']) : 0;
                $amN = isset($r['Ama_Cod']) ? intval($r['Ama_Cod']) : 0;
                if ($amN > 0 && ($amO === 0 || $amN < $amO)) {
                    $m['Ama_Cod'] = (string) $amN;
                }
                unset($m);
            }
        }
        $consumosUnicos = array();
        foreach ($ordenComCons as $ccKey) {
            $consumosUnicos[] = $consumoPorCom[$ccKey];
        }
        foreach ($consumosUnicos as &$cu) {
            $ccKey = isset($cu['Com_Cod_Consumo']) ? trim((string) $cu['Com_Cod_Consumo']) : trim((string) $cu['Com_Cod']);
            $cu['row_id'] = 'C_M_' . $ccKey;
        }
        unset($cu);
        $rowsKardex = array_merge($filasA, $consumosUnicos);
    }

    $saldoIniDb = $obBD_con1->getRowConsulta(40, $parms, $obBD_conexion);
    $saldoIniVal = 0;
    if (is_array($saldoIniDb) && isset($saldoIniDb['saldo_ini'])) {
        $saldoIniVal = floatval($saldoIniDb['saldo_ini']);
    }
    $cliSaldoIni = '';
