<?php
/**
 * Certificado B.07.01 por factura agrupado por placa de vehículo, con firma electrónica (TCPDF).
 */
...
/* Agrupar manifiestos por placa de vehículo */
...
$cert_base_widths = array(4, 6.4, 69.2, 20, 34.4, 22, 22); // Fecha -20%; Guía +5% de Fecha; Chofer +15% de Fecha
...
foreach ($grupos_vehiculo as $placa => $items) {
    ...
    $pdf->Cell($cert_table_w, 6, 'VEHICULO: ' . $placa . '  -  ' . $cant_placa . ' manifiesto(s)', 1, 1, 'L', true);
    $pdf->SetFont('helvetica', '', 7);
    ...
    $pdf->SetX($cert_margins['left']);
    $pdf->Cell(array_sum(array_slice($cert_col_widths, 0, 5)), 6, 'Subtotal (Placa: ' . $placa . ')', 1, 0, 'R', true);
