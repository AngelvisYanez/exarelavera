...
function cargarPlantasRango() {
  $.get(
    "",
    { getPlantasDisponiblesAjax: true },
    function (response) {
      if (response.success && response.plantas) {
        ...
        response.plantas.forEach(function (pla) {
          $select.append(
            $("<option>", { value: pla.Pla_Cod, text: pla.Pla_Nom }),
          );
        });
        if ($.fn.chosen) {
          try {
...
function cargarDashboard() {
  ...
  $.get(
    ...
    $("#dashboardContent").html(
      '<div class="alert alert-danger">Error al cargar el dashboard. Por favor, intente nuevamente.</div>',
    );
  });
}
...
function construirTendenciaDesdeTurnos(turnosDetalle) {
  ...
  turnosDetalle.forEach(function (t) {
    ...
    if (fechaKey.indexOf("/") >= 0) {
      ...
      if (p.length === 3)
        fechaKey =
          p[2] + "-" + ("0" + p[1]).slice(-2) + "-" + ("0" + p[0]).slice(-2);
    }
...
function cambiarVistaGlobalConfig(vista) {
  ...
  $("#btnVistaTablaConfig")
    .toggleClass("btn-warning", vista === "tabla")
    .toggleClass("btn-default", vista !== "tabla");
  $("#btnVistaBarrasConfig")
    .toggleClass("btn-warning", vista === "barras")
    .toggleClass("btn-default", vista !== "barras");
...
function generarVistaDiasConTresOpciones(turnosDetalle, forPrint) {
  ...
  fechasOrdenadas.forEach(function (fecha) {
    ...
    var iconoToggle =
      forPrint || esHoy || expandirTodo ? "fa-chevron-up" : "fa-chevron-down";
...
function generarBarrasParaDia(turnosFecha, forPrint) {
  ...
  turnosFecha.forEach(function (turno) {
    ...
    var ocupadosH =
      totalMostrado > 0 ? (ocupados / totalMostrado) * barHeightPx : 0;
    var libresH =
...
function verManifiestos(Tud_Cod) {
  ...
  $.get(
    "",
    { getManifiestosDashboardAjax: true, Tud_Cod: Tud_Cod },
    function (response) {
      if (response.success && response.manifiestos) {
        mostrarManifiestosModal(
          response.manifiestos,
          Tud_Cod,
          response.turnoInfo,
        );
      } else {
...
function verManifiestosConfiguracion(Tur_Cod, Tur_Fei, Tur_Fef) {
  ...
  $.get(
    "",
    { getManifiestosConfiguracionAjax: true, Tur_Cod: Tur_Cod },
    function (response) {
      if (response.success && response.manifiestos) {
...
function verManifiestosDiaDirecto(Tud_Cod) {
  ...
  $.get(
    "",
    { getManifiestosDiaAjax: true, Tud_Cod: Tud_Cod },
    function (response) {
      if (response.success && response.manifiestos) {
        mostrarManifiestosModal(
          response.manifiestos,
    ...
    .fail(function () {
      alert(
        "Error al cargar los manifiestos del día. Por favor, intente nuevamente.",
      );
    })
    .always(function () {
      cargandoManifiestos = false;
  ...
  if (!rawFec && man.Man_Usu) {
    ...
    if (!rawFec) {
      ...
      if (m && m[1]) {
        ...
        var m2 = raw.match(
          /["']Man_Tip["']\s*:\s*["']GE["'].*?["']Fecha["']\s*:\s*["']([^"']+)["']/i,
        );
        if (m2 && m2[1]) {
          rawFec = m2[1];
        } else {
          var m3 = raw.match(
            /["']Fecha["']\s*:\s*["']([^"']+)["'].*?["']Man_Tip["']\s*:\s*["']GE["']/i,
          );
          if (m3 && m3[1]) {
            rawFec = m3[1];
          }
...
function mostrarManifiestosModal(manifiestos, Tud_Cod, turnoInfo) {
  ...
  if (manifiestos.length === 0) {
    ...
    var mostrarColumnaFechaDia =
      manifiestos.length > 0 && manifiestos[0].fecha_dia;
    var manifiestosOrdenados = ordenarManifiestosParaTabla(
      manifiestos,
      "fecha",
    );
    html +=
...
function abrirModalManifiestosChoferPlaca(manifiestos, turnoInfo, tipoVista) {
  ...
  if (!manifiestos || manifiestos.length === 0) {
    ...
    if (tipoVista === "planta") {
      ...
      manifiestos.forEach(function (man) {
        var chofer =
          man.chofer_nombre && man.chofer_nombre.trim() !== ""
            ? man.chofer_nombre.trim()
            : "(Sin asignar)";
        var placa =
          man.Veh_Pla && man.Veh_Pla.trim() !== ""
            ? man.Veh_Pla.trim()
            : "(Sin placa)";
...
function verManifiestosChoferPlaca(idx) {
  ...
  abrirModalManifiestosChoferPlaca(
    grupo.manifiestos || [],
    turnoInfo,
    tipoVista,
  );
...
function imprimirModalManifiestosChoferPlaca() {
  ...
  var contenido = $activo.length
    ? $activo.clone()
    : $modal.find(".modal-body").clone();
  contenido.find(".no-print").remove();
  abrirVentanaImpresion(
    encabezado + contenido.prop("outerHTML"),
...
function exportarTablaAExcel(htmlTabla, nombreArchivo, nombreHoja) {
  ...
  form.append(
    $("<input>", { type: "hidden", name: "datos_a_enviar", value: htmlTabla }),
  );
  form.append(
    $("<input>", { type: "hidden", name: "nombre", value: nombreArchivo }),
  );
  form.append(
    $("<input>", {
      type: "hidden",
...
function imprimirReporte() {
  if (!datosDashboardActual || datosDashboardActual.length === 0) {
    alert(
      "No hay datos para imprimir. Por favor, cargue el dashboard primero.",
    );
    return;
  }
  ...
  datosDashboardActual.forEach(function (config) {
    ...
    if (modoVistaConfig === "tendencia" && Object.keys(tendenciaImgs).length > 0) {
      ...
      var turnosPorFechaT = {};
      (turnosPrint || []).forEach(function (t) {
        var fec = t.Tud_Fec || "";
...
function exportarExcel() {
  if (!datosDashboardActual || datosDashboardActual.length === 0) {
    alert(
      "No hay datos para exportar. Por favor, cargue el dashboard primero.",
    );
    return;
  }
  ...
  htmlExcel +=
    'table.xl-dashboard { border-collapse: collapse; table-layout: fixed; width: 600px; mso-displayed-decimal-separator: "\\,"; mso-displayed-thousand-separator: "\\."; }';
  htmlExcel +=
    'table.xl-dashboard td, table.xl-dashboard th { border: 1px solid #2C5D94; padding: 4px 8px; mso-number-format: "\\@"; }';
  htmlExcel +=
    ".xl-header { background-color: #2C5D94 !important; color: white !important; font-weight: bold !important; }";
  htmlExcel +=
    ".xl-stats { background-color: #f0f0f0 !important; font-weight: bold !important; }";
  htmlExcel +=
    ".xl-fecha { background-color: #d1ecf1 !important; color: #0c5460 !important; font-weight: bold !important; }";
  htmlExcel +=
    ".xl-col-header { background-color: #e0e0e0 !important; font-weight: bold !important; }";
  ...
  datosDashboardActual.forEach(function (config) {
    ...
    fechasOrdenadas.forEach(function (fecha) {
      ...
      htmlExcel +=
        '<td bgcolor="#e0e0e0" style="padding: 5px; text-align: center;">Horario</td>';
      htmlExcel +=
        '<td bgcolor="#e0e0e0" style="padding: 5px; text-align: center;">Cupos Program.</td>';
      htmlExcel +=
        '<td bgcolor="#e0e0e0" style="padding: 5px; text-align: center;">Manif. Creados</td>';
      htmlExcel +=
        '<td bgcolor="#e0e0e0" style="padding: 5px; text-align: center;">Inactivos</td>';
      htmlExcel +=
        '<td bgcolor="#e0e0e0" style="padding: 5px; text-align: center;">Cupos Libres</td>';
      htmlExcel +=
        '<td bgcolor="#e0e0e0" style="padding: 5px; text-align: center;">% Ocupación</td>';
      htmlExcel +=
...
function cargarDashboardRango() {
  ...
  $.get(
    ...
    $("#dashboardContentRango").html(
      '<div class="alert alert-danger">Error al cargar el reporte. Por favor, intente nuevamente.</div>',
    );
    datosDashboardRango = null;
  });
}
  ...
  return diasConManifiestos.map(function (d) {
    ...
    return {
      ...
      porcentaje_ocupacion_general:
        totalCupos > 0
          ? Math.round((totalOcupados / totalCupos) * 10000) / 100
          : 0,
    };
  });
}
...
function imprimirReporteRango() {
  ...
  if (!datosDashboardRango || dias.length === 0) {
    alert(
      "No hay datos para imprimir. Por favor, ejecute una búsqueda primero.",
    );
    return;
  }
...
function exportarExcelRango() {
  ...
  if (!datosDashboardRango || dias.length === 0) {
    alert(
      "No hay datos para exportar. Por favor, ejecute una búsqueda primero.",
    );
    return;
  }
  ...
  form.append(
    $("<input>", { type: "hidden", name: "datos_a_enviar", value: htmlExcel }),
  );
  form.append(
    $("<input>", { type: "hidden", name: "nombre", value: nombreArchivo }),
  );
  form.append(
    $("<input>", { type: "hidden", name: "hoja", value: "Reporte Rango" }),
  );
  $("body").append(form);
  form.submit();
  form.remove();
...
function calcularKPIsEstrategicos(agrupado, tipoVista, plantasOptional) {
  var src =
    agrupado && agrupado.length > 0
      ? agrupado
      : tipoVista === "planta" && plantasOptional && plantasOptional.length > 0
        ? plantasOptional
        : [];
  if (tipoVista === "chofer") {
  ...
  if (tipoVista === "chofer") {
    var sumaCuadrados = valores.reduce(function (s, v) {
      return s + (v - promedio) * (v - promedio);
    }, 0);
  ...
  out.indiceConcentracion =
    totalManifiestos > 0 ? (sumaTop20 / totalManifiestos) * 100 : 0;
  var umbralBajo = promedio * 0.7;
  out.pctBajaActividad =
    n > 0
...
function generarAlertaRiesgoPlanta(pctTop2) {
  ...
  var clase =
    pctTop2 >= 65
      ? "dashboard-alert-riesgo dashboard-alert-roja"
      : "dashboard-alert-riesgo dashboard-alert-amarilla";
  var texto =
    pctTop2 >= 65
...
function prepararDatosPareto(agrupado, tipoVista, plantasOptional) {
  ...
  var getNombre = function (g) {
    if (tipoVista === "chofer")
      return (g.chofer_nombre || "").trim() || "Sin asignar";
    if (tipoVista === "planta")
      return (g.Pla_Nom || "").trim() || "(Sin planta)";
  ...
  if (items.length <= TOP_N) {
    ...
    var otrosCant = items.slice(TOP_N).reduce(function (s, i) {
      return s + i.cantidad;
    }, 0);
    chartItems.push({ nombre: "Otros", cantidad: otrosCant, esOtros: true });
  }
  var totalChart = chartItems.reduce(function (s, i) {
    return s + i.cantidad;
  }, 0);
  acu = 0;
...
function initParetoChart(config, mode) {
  ...
  if (mode === "hasta70" && chartItems.length > 0) {
    ...
    if (pctAcumFull[pctAcumFull.length - 1] < 70)
      pctAcumFull[pctAcumFull.length - 1] = 70;
...
function initParetoYTarjetasChoferPlaca(agrupado, tipoVista, plantasOptional) {
  ...
  if (datos.itemsCompletos) {
    ...
    var maxCant =
      Math.max.apply(
        null,
        itemsCompletos.slice(0, 5).map(function (i) {
          return i.cantidad;
        }),
      ) || 1;
    for (var i = 0; i < 5; i++) {
      ...
      if (pctEl)
        pctEl.textContent = top5[i]
          ? top5[i].pct.toFixed(1) + "% del total"
          : "0%";
    ...
    if (switchEl) {
      switchEl.querySelectorAll(".pareto-mode-btn").forEach(function (btn) {
        btn.addEventListener("click", function () {
          ...
          switchEl.querySelectorAll(".pareto-mode-btn").forEach(function (b) {
...
function generarGraficoChoferPlaca(tipoVista, agrupado, plantas) {
  ...
  if (tipoVista === "planta") {
    ...
    src.forEach(function (g) {
      items.push({
        ...
        value: g.total_manifiestos != null ? g.total_manifiestos : g.total || 0,
      });
    });
  } else if (tipoVista === "chofer") {
    ...
    agruparDatosChoferParaGraficos(agrupado).forEach(function (g) {
      items.push({
        label: (g.chofer_nombre || "").trim() || "Sin asignar",
    ...
    (agrupado || []).forEach(function (g) {
      items.push({
        label: (g.Veh_Pla || "").trim() || "Sin placa",
        value: g.total_manifiestos || 0,
      });
    });
  }
  ...
  showItems.forEach(function (item) {
    ...
    if (tipoVista === "chofer") {
      ...
      else if (item.value < promedio * 0.95)
        rowClass += " chart-cp-bajo-promedio";
...
function cargarDashboardChoferPlaca() {
  ...
  $("#dashboardContentChoferPlaca").html(
    '<div class="loading"><i class="fa fa-spinner fa-spin"></i><p>Cargando...</p></div>',
  );
  ...
  $.get(
    ...
    function (resPlantas) {
      ...
      $.ajax({
        ...
        success: function (resMan) {
          ...
          datosDashboardChoferPlaca = {
            agrupado: resMan.agrupado || [],
            tipo_vista: resMan.tipo_vista || tipoVista,
          };
          var tipoVistaRender = ($("#tipo_vista_chofer_placa").val() || "")
            .toString()
            .toLowerCase()
            .trim();
          if (tipoVistaRender !== "planta" && tipoVistaRender !== "placa")
            tipoVistaRender = "chofer";
          renderDashboardChoferPlaca(
    ...
    $("#dashboardContentChoferPlaca").html(
      '<div class="alert alert-danger">Error al cargar ranking de plantas.</div>',
    );
  });
}
...
function renderDashboardChoferPlaca(plantas, agrupado, tipoVista) {
  ...
  if (tieneDatos) {
    ...
    setTimeout(function () {
      initParetoYTarjetasChoferPlaca(datosParaPareto, tipoVista, plantasParam);
    }, 0);
  }
}
...
function imprimirReporteChoferPlaca() {
  if (!datosRankingPlantasChoferPlaca && !datosDashboardChoferPlaca) {
    alert(
      "No hay datos para imprimir. Ejecute una búsqueda en el tab Por Chofer/Placa primero.",
    );
    return;
  }
  var tituloRango =
...
function exportarExcelChoferPlaca() {
  if (!datosRankingPlantasChoferPlaca && !datosDashboardChoferPlaca) {
    alert(
      "No hay datos para exportar. Ejecute una búsqueda en el tab Por Chofer/Placa primero.",
    );
    return;
  }
  var tituloRango =
  ...
  if (tipoVista === "planta") {
    ...
    if (
      ...
      if (tipoVista === "chofer")
        ...
        htmlExcel +=
          '<tr class="xl-cp-header"><td>#</td><td>Placa</td><td>Total manifiestos</td><td>Plantas</td><td>Tiempo Prom.</td></tr>';
      datosDashboardChoferPlaca.agrupado.forEach(function (g, i) {
  ...
  form.append(
    $("<input>", { type: "hidden", name: "hoja", value: "Chofer Placa" }),
  );
  $("body").append(form);
  form.submit();
  form.remove();
  ...
  var estilosImpresion = `
        <style>
            ...
            * { margin-top: 0; box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            ...
            .chart-bar-spacer { flex: 1 1 0; min-height: 0; }
            .chart-bar-porcentaje { font-size: 9px; font-weight: 600; color: #2C5D94; margin: 0; text-align: center; }
            .chart-bar-columna { width: 100%; max-width: 45px; flex-shrink: 0; display: flex; flex-direction: column-reverse; border-radius: 4px 4px 0 0; overflow: hidden; background: #e9ecef !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
            .chart-segmento { display: flex; align-items: center; justify-content: center; min-height: 0; flex-shrink: 0; width: 100%; text-align: center; }
            .chart-segmento-ocupados { background: #2C5D94 !important; background-color: #2C5D94 !important; color: white !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
            .chart-segmento-libres { background: #fd7e14 !important; background-color: #fd7e14 !important; color: white !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
            .chart-segmento-valor { font-size: 10px; font-weight: 700; line-height: 1; width: 100%; text-align: center; display: block; }
            .chart-segmento-pequeno { min-height: 18px !important; }
            .chart-x-label { font-size: 10px; white-space: nowrap; text-align: center; width: 100%; }
...
function imprimirReporteFechaRango(fecha) {
  ...
  if (!datosDashboardRango || dias.length === 0) {
    alert(
      "No hay datos para imprimir. Por favor, ejecute una búsqueda en Por Rango primero.",
    );
    return;
  }
  var diaFiltrado = dias.filter(function (d) {
  ...
  (diaFiltrado.turnos_detalle || []).forEach(function (t) {
    ...
    tCopy.Tud_Fec =
      tCopy.Tud_Fec || diaFiltrado.Tud_Fec_SQL || diaFiltrado.Tud_Fec;
...
function exportarExcelFechaRango(fecha) {
  ...
  if (!datosDashboardRango || dias.length === 0) {
    alert(
      "No hay datos para exportar. Por favor, ejecute una búsqueda en Por Rango primero.",
    );
    return;
  }
  var diaFiltrado = dias.filter(function (d) {
  ...
  htmlExcel +=
    "table.xl-dashboard-fecha td, table.xl-dashboard-fecha th { border: 1px solid #2C5D94; padding: 4px 8px; }";
  htmlExcel += ".xl-data { background-color: #ffffff !important; }";
  htmlExcel += "</style>";
  htmlExcel +=
  ...
  form.append(
    $("<input>", { type: "hidden", name: "hoja", value: "Reporte Fecha" }),
  );
  $("body").append(form);
  form.submit();
  form.remove();
...
function exportModalManifiestosExcel(tipo) {
  ...
  htmlExcel +=
    "table.xl-modal td, table.xl-modal th { border: 1px solid #2C5D94; padding: 4px 8px; }";
  htmlExcel +=
    ".xl-modal-header { background-color: #2C5D94 !important; color: white !important; font-weight: bold !important; }";
  htmlExcel +=
    ".xl-modal-total { background-color: #2C5D94 !important; color: white !important; font-weight: bold !important; }";
  htmlExcel += "</style>";
  htmlExcel +=
  ...
  if (tipo === "detallado") {
    ...
    manifiestosParaExport.forEach(function (man, index) {
      ...
      if (mostrarColumnaFechaDia)
        htmlExcel += "<td>" + (man.fecha_dia || "") + "</td>";
      if (mostrarColumnaHorario)
        htmlExcel += "<td>" + (man.horario_turno || "") + "</td>";
    ...
    filas.forEach(function (item, index) {
      var pct =
        totalManifiestos > 0
          ? (((item.cantidad || 0) / totalManifiestos) * 100).toFixed(2)
          : "0.00";
      htmlExcel += "<tr>";
      htmlExcel += "<td>" + (index + 1) + "</td>";
...
function cargarDashboardEjecutivo() {
  ...
  $("#dashboardContentEjecutivo").html(
    '<div class="loading"><i class="fa fa-spinner fa-spin"></i><p>Generando tablero ejecutivo...</p></div>',
  );
  $.ajax({
    url: "",
...
function imprimirReporteEjecutivo() {
  if (!datosDashboardEjecutivo) {
    alert(
      "No hay datos para imprimir. Genere el tablero ejecutivo primero (seleccione fechas y pulse Generar tablero).",
    );
    return;
  }
  ...
  if (!contenidoConGraficos) {
    alert(
      "No se pudo obtener el contenido del tablero. Asegúrese de haber generado el tablero y que los gráficos estén visibles.",
    );
    return;
  }
...
function renderDashboardEjecutivo(data) {
  ...
  var tonFacturadas = Number(
    data.kpis.total_toneladas_facturadas || 0,
  ).toLocaleString("es-ES");
  var tonPorFacturar = Number(
    data.kpis.total_toneladas_por_facturar || 0,
  ).toLocaleString("es-ES");
  ...
  if (totalInac === 0 && totalAct === 0) {
    ...
    if (topPlantasInac.length > 0) {
      ...
      topPlantasInac.forEach(function (p) {
        sumTopInac += p.total_inactivos || 0;
      });
      var pctConc =
      ...
      topPlantasInac.forEach(function (p) {
        var tasaPlanta =
          p.tasa_pct != null
            ? p.tasa_pct
            : p.total_activos > 0
              ? (((p.total_inactivos || 0) / p.total_activos) * 100).toFixed(1)
              : p.total_inactivos > 0
                ? "100"
                : "0";
        html +=
  ...
  $(document)
    ...
    .on("click.ceoPlantaInactivos", ".ceo-planta-inactivos-link", function (e) {
      e.preventDefault();
      var plaCod = $(this).data("pla-cod");
      var plaNom = $(this).data("pla-nom");
      if (!plaCod) return;
      $("#modalInactivosDetallePlantaTitulo").text(
        "Inactivos: " + (plaNom || "Planta"),
      );
...
function exportarExcelInactivos() {
  ...
  if (!fi || !ff) {
    alert(
      "Indique el rango de fechas en la Vista Ejecutiva (Desde / Hasta) y genere el tablero primero.",
    );
    return;
  }
  $.ajax({
    ...
    success: function (res) {
      ...
      form.append(
        $("<input>", { type: "hidden", name: "datos_a_enviar", value: html }),
      );
      form.append(
...
function cargarPlantasTiempos() {
  ...
  $.ajax({
    url: "",
    data: { getPlantasDisponiblesAjax: true },
    dataType: "json",
    success: function (res) {
      if (res.success && res.plantas) {
        var $sel = $("#select_planta_tiempos");
        $sel.html('<option value="">Todas las plantas</option>');
        res.plantas.forEach(function (p) {
          $sel.append(
            '<option value="' + p.Pla_Cod + '">' + p.Pla_Nom + "</option>",
          );
        });
      }
    },
