...
$(function () {
    ...
    man_ant.createGrid({
        ...
        loadComplete: function () {
            ...
            rowsChrono.sort(function (x, y) {
                return manAntCompareRowsChrono(x.r, y.r);
            });
            var totalVal = 0, totalAbono = 0, saldoAcumulado = 0;
            rowsChrono.forEach(function (item) {
                var id = item.id;
                var r = item.r;
                var tipoL = String(r._tipo_linea || '').toUpperCase();
                if (String(r.row_id || '') === 'SALDO_INI' || tipoL === 'S') {
                    saldoAcumulado = parseFloat(String(r.Saldo != null ? r.Saldo : r._saldo_inicial).replace(/[^\d.-]/g, '')) || 0;
                    man_ant.jqGrid('setCell', id, 'Saldo', saldoAcumulado);
                ...
                saldoAcumulado = parseFloat((saldoAcumulado + val - abono).toFixed(2));
