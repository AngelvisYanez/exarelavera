...
function guardarAjuste(btn) {
    ...
    if (!Dis_Cod || !Did_Tip || !Did_Fec || isNaN(Did_Can) || Did_Can <= 0 || !Did_Obs) {
        $.alert('Por favor, complete todos los campos obligatorios (*) y aseg�rese de que la cantidad sea mayor a 0. El motivo es indispensable.');
        return;
    }
...
function anularAjuste(didCod) {
    if (confirm('�Est� seguro de anular este ajuste? La existencia ser� recalculada.')) {
        if (typeof $.carga === 'function') { $.carga('show'); }
        $.ajax({
            url: window.location.href,
            type: 'POST',
...
function imprimirKardex() {
    ...
    var header = "<div style=\"text-align:center; margin-bottom: 20px;\"><h2>CONTROL DE COMBUSTIBLE</h2><h3>KARDEX DE DISPENSADOR</h3><p><strong>Per�odo:</strong> " + fec_ini + " a " + fec_fin + "</p><p><strong>Dispensador:</strong> " + dis_nom + "</p><p><strong>Fecha generaci�n:</strong> " + now + "</p></div>";
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.success && res.data) {
                ...
                $.alert(res.message || 'Error al obtener c�lculo.');
                $('#boxCalculoTeorico').hide();
                $('#btnGuardarCierre').prop('disabled', true);
            }
        },
...
function anularCierre(cieCod) {
    if (confirm('�Est� seguro de anular este Cierre Diario?')) {
        if (typeof $.carga === 'function') { $.carga('show'); }
        $.ajax({
            url: window.location.href,
            type: 'POST',
...
function loadDashboard() {
    ...
    $.ajax({
        ...
        error: function() {
            $.alert('Error de conexión al cargar Dashboard');
        }
    });
}
...
// Cargar dashboard automáticamente al entrar a la pestaña si no se ha cargado
$('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
    var target = $(e.target).attr("href");
