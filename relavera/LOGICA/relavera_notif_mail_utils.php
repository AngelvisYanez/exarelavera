...
/**
 * Env�o de correo para notificaciones Relavera (SMTP dedicado en relavera_notif_mail_config.php).
 */
...
if (!function_exists('relavera_notif_enviar_correo_notif')) {
     ...
     * @param string $asunto
     * @param string $mensajePlano Mismo texto plano que se env�a por WhatsApp
    ...
    function relavera_notif_enviar_correo_notif($para, $nombreDestinatario, $asunto, $mensajePlano, $imagenAdj = null)
    {
        ...
        if (defined('RELAVERA_NOTIF_SMTP_ENABLED') && RELAVERA_NOTIF_SMTP_ENABLED) {
            ...
            $mail->isHTML(true);
            $mail->Subject = ($asuntoLinea !== '') ? $asuntoLinea : (defined('RELAVERA_NOTIF_SMTP_SUBJECT_DEFAULT') ? RELAVERA_NOTIF_SMTP_SUBJECT_DEFAULT : 'Notificaci�n Relavera');
            $mail->Body = $html;
