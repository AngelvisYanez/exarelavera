 ...
 */
function sentencias_actualizacion($id, $Par_Sql)
{
    switch ($id) {
        ...
        case 2:
            ...
            break;
            //Select para extraer la informacin de una persona de la tabla del mismo nombre
        case 3:
            $sql = "SELECT persona.Prs_Cod,Prs_Ced,Prs_Nom,Prs_Ape,ciudad.Ciu_Cod,Prs_Sex,Prs_Esc,Prs_Cor,identifica.Ide_Cod,Ide_Des,
                           Per_Cod,Per_Tit,Per_Obs,Per_Car,Per_Req,Per_Cfi,Prs_San,Prs_Dir,Prs_Tel,Prs_Cel,Prs_Fec
                    FROM persona
					LEFT JOIN ciudad ON persona.Ciu_Cod=ciudad.Ciu_Cod
					LEFT JOIN identifica ON persona.Ide_Cod=identifica.Ide_Cod
                    LEFT JOIN personal ON persona.Prs_Cod=personal.Prs_Cod
        ...
        case 8:
            $Prs_Sex_S = !empty($Par_Sql[3]) ? $Par_Sql[3] : 'M';
            $Prs_Esc_S = !empty($Par_Sql[4]) ? $Par_Sql[4] : 'S';
            $Ciu_Cod_S = !empty($Par_Sql[6]) ? $Par_Sql[6] : '0';

            $sql = "UPDATE persona 
                    SET Prs_Nom='$Par_Sql[1]', 
                        Prs_Ape='$Par_Sql[2]', 
                        Prs_Sex='$Prs_Sex_S', 
                        Prs_Esc='$Prs_Esc_S', 
                        Prs_Fec='$Par_Sql[5]', 
        ...
        // Caso 18: Verificar si la campaña de actualización está activa para la empresa
        case 18:
            $sql = "SELECT COUNT(*) as activa FROM manifiesto_plantas mp 
                    LEFT JOIN cliente c ON c.Cli_Cod = mp.Cli_Cod 
                    WHERE mp.Pla_Est = 'A' AND mp.Pla_Act = 'S' 
                    AND (c.Emp_Cod = $Par_Sql[0] OR mp.Cli_Cod IS NULL)";
            break;

        // Caso 19: Listado de personal para el Grid filtrado por planta
        case 19:
            $sql = "SELECT p.Prs_Cod, p.Prs_Ced, p.Prs_Nom, p.Prs_Ape, ciu.Ciu_Des, p.Prs_Tel, p.Prs_Cel, 
                           c.Cho_Tli, c.Cho_Cli, per.Per_Cod, p.Prs_Sex, p.Prs_Esc, p.Prs_Cor, p.Prs_Dir, p.Prs_San,
                           per.Per_Car, per.Per_Tit, per.Per_Obs, per.Per_Fot, p.Prs_Fec, p.Ciu_Cod
                    FROM persona p
                    LEFT JOIN personal per ON p.Prs_Cod = per.Prs_Cod
                    LEFT JOIN ciudad ciu ON p.Ciu_Cod = ciu.Ciu_Cod
                    LEFT JOIN chofer c ON p.Prs_Cod = c.Prs_Cod
                    INNER JOIN manifiesto_chofer mc ON c.Cho_Cod = mc.Cho_Cod
                    WHERE p.Prs_Est = 'A' AND mc.Pla_Cod = $Par_Sql[Pla_Cod] $Par_Sql[searchQuery]
                    GROUP BY p.Prs_Cod
                    ORDER BY $Par_Sql[sidx] $Par_Sql[sord] LIMIT $Par_Sql[start], $Par_Sql[limit]";
            break;

        // Caso 20: Conteo de personal para el Grid
        case 20:
            $sql = "SELECT COUNT(DISTINCT p.Prs_Cod) as count 
                    FROM persona p 
                    LEFT JOIN personal per ON p.Prs_Cod = per.Prs_Cod
                    INNER JOIN chofer c ON p.Prs_Cod = c.Prs_Cod
                    INNER JOIN manifiesto_chofer mc ON c.Cho_Cod = mc.Cho_Cod
                    WHERE p.Prs_Est = 'A' AND mc.Pla_Cod = $Par_Sql[Pla_Cod] $Par_Sql[searchQuery]";
            break;

        // Caso 21: Obtener planta por defecto si Pla_Cod es 0
        case 21:
            $sql = "SELECT Pla_Cod FROM manifiesto_plantas mp 
                    LEFT JOIN cliente c ON c.Cli_Cod = mp.Cli_Cod 
                    WHERE mp.Pla_Est = 'A' AND c.Emp_Cod = $Par_Sql[0] LIMIT 1";
            break;

        // Caso 22: Actualizar datos de Chofer (Licencia) usando Prs_Cod
        case 22:
            $sql = "UPDATE chofer SET Cho_Tli = '$Par_Sql[0]', Cho_Cli = '$Par_Sql[1]' 
                    WHERE Prs_Cod = '$Par_Sql[2]'";
            break;

        // Caso 23: Obtener datos del usuario logeado
        case 23:
            $sql = "SELECT persona.Prs_Cod, Prs_Ced 
                    FROM persona, usuarios 
                    WHERE persona.Prs_Cod = usuarios.Prs_Cod AND usuarios.Usu_Cod = $Par_Sql[0]";
            break;

        // Caso 24: Obtener Per_Cod
        case 24:
            $sql = "SELECT Per_Cod FROM personal 
                    WHERE Prs_Cod = '$Par_Sql[0]' AND Emp_Cod = $Par_Sql[1]";
            break;

        // Caso 25: Actualizar datos en la tabla PERSONAL
        case 25:
            if (empty($Par_Sql[16]) || $Par_Sql[16] == "NULL") {
                $foto_sql = "NULL";
            } else {
                $foto_sql = "'$Par_Sql[16]'";
            }
            $sql = "UPDATE personal 
                    SET Per_Car='$Par_Sql[13]', 
                        Per_Tit='$Par_Sql[14]', 
                        Per_Obs='$Par_Sql[15]', 
                        Per_Fot=$foto_sql, 
                        Per_Cfi='$Par_Sql[17]', 
