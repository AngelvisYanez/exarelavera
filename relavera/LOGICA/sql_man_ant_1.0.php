    ...
    switch ($id) {
        ...
        case 37:
            ...
            if (!empty($Par_Sql['Fec_IniM']) && !empty($Par_Sql['Fec_FinM'])) {
                ...
                $whereFechaCons = " AND DATE(com.Com_Fec) BETWEEN DATE('$fi') AND DATE('$ff') ";
            }
            $sql = "SELECT dacc.Com_Cod AS Com_Cod_Consumo,
                           MAX(com.Com_Fec) AS Com_Fec_Consumo,
                           MAX(com.Com_Num) AS Com_Num_Consumo,
                           MAX(CONCAT(tp.Tia_Abr,'-',IF(CHAR_LENGTH(MONTH(com.Com_Fec))=1,CONCAT('0',CAST(MONTH(com.Com_Fec) AS CHAR)),CAST(MONTH(com.Com_Fec) AS CHAR)),'-',CAST(com.Com_Num AS CHAR))) AS Codigo_Compr_Consumo,
                           MAX(com.Com_Con) AS Com_Con_Consumo,
                           SUM(dacc.Ddc_Val) AS total_consumo
                    FROM det_ant_cccc dacc
                    INNER JOIN anticipos_clientes ac ON dacc.Ant_Cod = ac.Ant_Cod
                    INNER JOIN comprobantes com ON dacc.Com_Cod = com.Com_Cod
                    INNER JOIN tipo_asien tp ON tp.Tia_Cod = com.Tia_Cod
                    WHERE ac.Ama_Cod = '$ama'
                      AND ac.Ant_Est IN ('A','U','C')
                      AND IFNULL(com.Com_Est,'A') <> 'I'
                      AND dacc.Com_Cod <> ac.Com_Cod
                      $whereFechaCons
                    GROUP BY dacc.Com_Cod
                    ORDER BY MAX(com.Com_Fec) ASC, dacc.Com_Cod ASC";
            return $sql;
            // break;
