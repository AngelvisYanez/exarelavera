 ...
 * @author angeloni
 * @version 1.0
 * Fecha de actualizaci�n:	2021-04-28
 *
 * @package classes
 */
...
class CategoriaClass {
    ...
    public function getAllDetalles($body){
        $data=$body;
        $consulta = $this->datos->getArrayConsulta( 1051,$data, $this->conexion);
        $response['data'] = $consulta;
        if ($this->datos->Error == 0) {
            $response['status'] = true;
            $response['message'] = "Consulta exitosa";
    ...
    public function updateCategoria($body){
        $data=$body;
        $this->datos->inicio_transaccion($this->conexion);
