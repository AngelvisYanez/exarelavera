	...
	AND compras.Cop_Est = 'A' AND tipo_compr.Tic_Est='A' AND (Tic_Sri = 1 OR Tic_Sri = 2 OR Tic_Sri = 5 OR Tic_Sri = 3 OR Tic_Sri = 4 OR Tic_Sri = 11 OR Tic_Sri = 15 OR  Tic_Sri = 19 OR Tic_Sri = 41 OR Tic_Sri = 26 OR Tic_Sri = 21)  AND compras.Tri_Cod != 1 AND proveedore.Emp_Cod = $Par_Sql[2]";
		...
		case 387:
			$sql = "	SELECT  
					...
					WHERE
					  ventas.Vet_Cod not in(select exp.Vet_Cod from exporta_vent as exp) AND
		...
		case 389:
			...
			$sql = "	SELECT 
					  ...
					  CAST( SUM( IF(Iva_Sri=6, CAST( ((Vet_Pru * Vet_Can)-((Vet_Pru * Vet_Can) * Vet_Dec/100)) AS decimal(20,2) ) - ( CAST( ((Vet_Pru * Vet_Can)-((Vet_Pru * Vet_Can) * Vet_Dec/100)) AS decimal(20,2) ) * ventas.Vet_Des/100 ),0)) AS decimal(20,2) ) AS NgIva,
					  CAST( SUM( IF(Iva_Sri !=6, CAST( ((Vet_Pru * Vet_Can)-((Vet_Pru * Vet_Can) * Vet_Dec/100)) AS decimal(20,2) ) - ( CAST( ((Vet_Pru * Vet_Can)-((Vet_Pru * Vet_Can) * Vet_Dec/100)) AS decimal(20,2) ) * ventas.Vet_Des/100 ),0)) AS decimal(20,2) ) AS Total,
				...
				SELECT sucursal.Suc_Cod,sucursal.Suc_Sri,sucursal.Suc_Des,COALESCE(SUM(totales.Total),0)AS Total 
					FROM sucursal
						...
						(
						 ...
						 INNER JOIN autorizaci ON ventas.Aut_Cod=autorizaci.Aut_Cod 
						 INNER JOIN tipo_compr ON (autorizaci.Tic_Cod = tipo_compr.Tic_Cod)                 
						WHERE ventas.Vet_Cod not in(select exp.Vet_Cod from exporta_vent as exp) AND 
						ventas.Vet_Cod not in(select reemb.Vet_Cod from venta_reembolsos as reemb) AND
