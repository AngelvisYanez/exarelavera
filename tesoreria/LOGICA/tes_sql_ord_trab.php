...
function sentencias_facturaVenta($id,$Par_Sql)
{
    switch($id)
    {
        ...
        return $sql;
        break;
        case 5://Insertar datos en la tabla medicamento
            $sql = "INSERT INTO medicamento(Med_Nom, Med_Est, Med_Can) VALUES('$Par_Sql[Med_Nom]','A','$Par_Sql[Med_Can]')";
            break;
        ...
        case 11://Select para comprobar si el numero de secuencia ya se encuentra registrado
            $sql = "SELECT ficha_medicamento.Fic_Cod, paciente.Pac_Cod, Emp_Ruc, Suc_Te1, Suc_Log, Suc_Dir, Emp_Rre, Emp_Rep, Emp_Nom, Emp_Log, ficha_paciente.Pac_Cod, med_nom, med_dur, med_dos, fic_obs, fic_mot, fic_hea FROM ficha_medicamento
                    INNER JOIN medicamento ON ficha_medicamento.Med_Cod = medicamento.Med_Cod
                    INNER JOIN ficha_paciente ON ficha_medicamento.Fic_Cod = ficha_paciente.Fic_Cod
                    INNER JOIN paciente ON ficha_paciente.Pac_Cod = paciente.Pac_Cod
                    INNER JOIN empresas ON paciente.Emp_Cod = empresas.Emp_Cod
                    INNER JOIN sucursal ON empresas.Emp_Cod = sucursal.Emp_Cod
                WHERE ficha_medicamento.Fic_Cod=$Par_Sql[Fic_Cod]";
            break;
        case 12:
             $sql = "SELECT Prs_Ced, Emp_Nom, Emp_Ruc, Suc_Dir, Suc_Des, Suc_Te1, Suc_Cor, Ord_Cod, Ord_Num, Ord_Est, CONCAT(Prs_Ape,' ',Prs_Nom) as cliente, Ord_Fec, Ord_Pla, Ord_Mod, Ord_Obs, Prs_Ced, Prs_Cor, Ord_Cli, Prs_Dir, Ord_Ter, Prs_Tel, Ord_Mod, Ord_Ani, Ord_Col, Ord_Pla, Ord_Vin, Ord_Mot, Ord_Kms, Ord_Obs, Ord_Ant, Ord_Rad, Ord_Plu, Ord_Ext, Ord_Con, Ord_Seg, Ord_Sig, Ord_Enc, Ord_Moq, Ord_Esp, Ord_Com, Ord_Tri, Ord_Cds, Ord_Tap, Ord_Lla, Ord_Gat, Ord_Her, Ord_Llr, Ord_Bot, Ord_Tpg from orden_trabajo 
            		INNER JOIN cliente ON orden_trabajo.Cli_Cod = cliente.Cli_Cod
            		INNER JOIN persona ON cliente.Prs_Cod = persona.Prs_Cod
            		INNER JOIN empresas ON cliente.Emp_Cod = empresas.Emp_Cod
            		INNER JOIN sucursal ON empresas.Emp_Cod = sucursal.Emp_Cod
            		WHERE orden_trabajo.Ord_Cod=$Par_Sql[Ord_Cod] AND sucursal.Suc_Cod = '$Par_Sql[Suc_Cod]' GROUP BY cliente.Cli_Cod";
            //ChromePhp::log($sql);
            break;
        case 13://Con esta sentencia consulto producto y stock
            $sql= "SELECT Med_Cod, Med_Nom, Med_Can, Med_Est FROM medicamento";
            break;        
        case 14://Select para obtener el precio de los productos
            $sql="SELECT MAX(Ord_Num) as numero FROM orden_trabajo";
            //ChromePhp::log($sql);
            break;
        case 15://fichas registradas
        if($Par_Sql[2]=="c") {$search="(Prs_Ape LIKE '%$Par_Sql[search]%' OR Prs_Nom LIKE '%$Par_Sql[search]%')";}
            else {$search="Ord_Pla LIKE '%$Par_Sql[search]%'";}
            $sql = "SELECT Prs_Ced, Ord_Cod, CONCAT('OT-000', Ord_Num) as Ord_Num, Ord_Est, CONCAT(Prs_Ape,' ',Prs_Nom) as cliente, Ord_Fec, Ord_Pla, Ord_Mod, Ord_Obs from orden_trabajo 
            		INNER JOIN cliente ON orden_trabajo.Cli_Cod = cliente.Cli_Cod
            		INNER JOIN persona ON cliente.Prs_Cod = persona.Prs_Cod
            		INNER JOIN empresas ON cliente.Emp_Cod = empresas.Emp_Cod
            		WHERE Ord_Est = 'A' AND $search AND empresas.Emp_Cod = '$Par_Sql[Emp_Cod]'";
            //ChromePhp::log($sql);
            break;
        case 16://Select para obtener los datos de la tabla iva, cuyos porcentajes sean mayor a cero y se encuentren activos
            $sql = "SELECT ficha_medicamento.Fic_Cod, paciente.Pac_Cod, Emp_Ruc, Suc_Te1, Suc_Log, Suc_Dir, Emp_Rre, Emp_Rep, Emp_Nom, Emp_Log, ficha_paciente.Pac_Cod, med_nom, med_dur, med_dos, fic_obs, fic_mot, fic_hea, Ciu_Des, Pro_Nom, Pas_Nom FROM ficha_medicamento
                    INNER JOIN medicamento ON ficha_medicamento.Med_Cod = medicamento.Med_Cod
                    INNER JOIN ficha_paciente ON ficha_medicamento.Fic_Cod = ficha_paciente.Fic_Cod
                    INNER JOIN paciente ON ficha_paciente.Pac_Cod = paciente.Pac_Cod
                    INNER JOIN empresas ON paciente.Emp_Cod = empresas.Emp_Cod
                    INNER JOIN sucursal ON empresas.Emp_Cod = sucursal.Emp_Cod
                    INNER JOIN ciudad ON sucursal.Ciu_Cod = ciudad.Ciu_Cod
                    INNER JOIN provincia ON ciudad.Pro_Cod = provincia.Pro_Cod
    ...
    case 72: // usado
      ...
      case 79:
        ...
        break;
        case 154://guardar orden de trabajo
            $sql="INSERT INTO orden_trabajo (Cli_Cod, Emp_Cod, Ord_Cli, Ord_Num, Ord_Evi_Pro, Ord_Fec, Ord_Ter, Ord_Mod, Ord_Vin, Ord_Ani, Ord_Col, Ord_Pla, Ord_Mot, Ord_Kms, Ord_Ant, Ord_Rad, Ord_Plu, Ord_Ext, Ord_Con, Ord_Seg, Ord_Sig, Ord_Enc, Ord_Moq, Ord_Esp, Ord_Com, Ord_Tri, Ord_Cds, Ord_Tap, Ord_Lla, Ord_Gat, Ord_Her, Ord_Llr, Ord_Bot, Ord_Tpg, Ord_Obs) 
            VALUES ($Par_Sql[Cli_Cod], $Par_Sql[Emp_Cod], '$Par_Sql[Ord_Cli]', '$Par_Sql[Ord_Num]', '$Par_Sql[Mod_Tic_Evi_Pro]', '$Par_Sql[Ord_Fec]', '$Par_Sql[Ord_Ter]', '$Par_Sql[Ord_Mod]', '$Par_Sql[Ord_Vin]', '$Par_Sql[Ord_Ani]', '$Par_Sql[Ord_Col]', '$Par_Sql[Ord_Pla]', '$Par_Sql[Ord_Mot]', '$Par_Sql[Ord_Kms]', '$Par_Sql[Ord_Ant]', '$Par_Sql[Ord_Rad]', '$Par_Sql[Ord_Plu]', '$Par_Sql[Ord_Ext]', '$Par_Sql[Ord_Con]', '$Par_Sql[Ord_Seg]', '$Par_Sql[Ord_Sig]', '$Par_Sql[Ord_Enc]', '$Par_Sql[Ord_Moq]', '$Par_Sql[Ord_Esp]', '$Par_Sql[Ord_Com]', '$Par_Sql[Ord_Tri]', '$Par_Sql[Ord_Cds]', '$Par_Sql[Ord_Tap]', '$Par_Sql[Ord_Lla]', '$Par_Sql[Ord_Gat]', '$Par_Sql[Ord_Her]', '$Par_Sql[Ord_Llr]', '$Par_Sql[Ord_Bot]', '$Par_Sql[Ord_Tpg]', '$Par_Sql[Ord_Obs]');";
                //ChromePhp::log($sql);
        break;
    }
    return $sql;
