...
function sentencias_che($id, $Par_Sql) {
    ...
    switch ($id) {
        ...
        case 18:
            ...
            $fecha_campo = !empty($Par_Sql['TipFecha']) && $Par_Sql['TipFecha']=='CHE' ? 'Che_Fec':'Com_Fec';
            $search_fechas = "and $fecha_campo between '$Par_Sql[txt_fec_ini]' and '$Par_Sql[txt_fec_fin]'";
            $banco_search = "";
            ...
            if (empty($Par_Sql['limits'])) {
                ...
                $campos = "bancos.Bak_Des,cheques_ext.*, 
                            ...
                            comprobantes.Com_Fec,CONCAT(Tia_Abr,'-',IF(CHAR_LENGTH(MONTH(Com_Fec))=1,CONCAT('0',CAST(MONTH(Com_Fec) AS char)),CAST(MONTH(Com_Fec) AS char)),'-',CAST(Com_Num AS char)) as Com_Num,
                            (SELECT COUNT(*) FROM cheq_det_ccpp cdc
                                INNER JOIN det_ccpp_c dcc ON cdc.Dcc_Cod = dcc.Dcc_Cod 
                                INNER JOIN comprobantes cmp ON dcc.Com_Cod = cmp.Com_Cod 
                            WHERE cdc.Che_Cod = cheques_ext.Che_Cod AND cmp.Com_Est='A') as used_in_payment";
            }
...
// echo $sql."<br/>";
        ...
        case 27:
            ...
            if (isset($Par_Sql['Che_Num'])) {
                $che_cob .= ", Che_Num='$Par_Sql[Che_Num]'";
            }
        ...
        case 352:
            ...
            $bus_xmld_331 = "SELECT $campos
                                ...
                                AND det_plan.Pld_Tip = 'D' 
                                /* clausula adicional - 04-07-25 */
        ...
        case 408:
            ...
            break;
        case 409:
            $sql="UPDATE asientos SET Asi_Glo='$Par_Sql[Asi_Glo]' WHERE Asi_Cod=$Par_Sql[Asi_Cod]";  
            break;  
    }
    //echo $sql."<br/>";
    return $sql;
