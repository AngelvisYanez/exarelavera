...
function sentencias_cli($id, $Par_Sql)
{
	switch ($id) {
		...
		case 20: //Insert en la tabla cliente
			$sql = "INSERT INTO cliente(Prs_Cod,Emp_Cod,Cli_Tic,Cli_Con,Cli_Cor, Dia_Cred, Mon_Max) VALUES('$Par_Sql[Prs_Cod]','$Par_Sql[Emp_Cod]','$Par_Sql[Cli_Tic]','$Par_Sql[Cli_Con]'," . (empty($Par_Sql['Cli_Cor']) ? 'NULL' : "'$Par_Sql[Cli_Cor]'") . ", '$Par_Sql[Dia_Cred]', '$Par_Sql[Mon_Max]');";
			return $sql;
			break;
		...
		case 26: //Update sobre la tabla cliente
			$sql = "UPDATE cliente SET Prs_Cod='$Par_Sql[0]', Cli_Tic='$Par_Sql[1]', Cli_Con='$Par_Sql[2]', Cli_Cor=" . (!empty($Par_Sql[4]) ? "'$Par_Sql[4]'" : 'NULL') . " , Cli_Fac='$Par_Sql[5]', Dia_Cred='$Par_Sql[6]', Mon_Max='$Par_Sql[7]'  WHERE Cli_Cod='$Par_Sql[3]'";
			return $sql;
			break;
		...
		case 27: //Select para listar los clientes registrados en la empresa
			...
			if (isset($Par_Sql["limits"])) {
				...
				$campos = "Cli_Cod,cliente.Prs_Cod, cliente.Dia_Cred, cliente.Mon_Max, Prs_Ced,Prs_Ape,Cli_Con,persona.Ide_Cod,Ide_Sri,Prs_Ape,Prs_Nom,Cli_Tic,CONCAT(Prs_Ape,' ',Prs_Nom) AS cliente,Prs_Sex,Ciu_Cod,Prs_Dir,Prs_Tel,Prs_Te2,Prs_Cel,IF(Cli_Cor IS NULL OR TRIM(Cli_Cor)='',Prs_Cor,Cli_Cor)AS Prs_Cor,Cli_Fac";
			} else {
		...
		case 28:
			if ($Par_Sql['op_opciones'] == "d") {
				$search = "(persona.Prs_Ape LIKE '%$Par_Sql[search]%' OR persona.Prs_Nom)";
			} else {
				$search = "persona.Prs_Ced LIKE '$Par_Sql[search]%'";
			}
			if ($Par_Sql['est_opciones'] == "a") {
				$estado = "cliente.Cli_Est = 'A'";
			} else {
				$estado = "cliente.Cli_Est = 'I'";
			}
			$campos = empty($Par_Sql['limits']) ? " COUNT(cliente.Cli_Cod) AS total" : "cliente.Prs_Cod, Cli_Cod, cliente.Emp_Cod, persona.Prs_Ced, persona.Prs_Tel, persona.Prs_Cor, persona.Prs_Dir, persona.Prs_Nom, persona.Prs_Ape, CONCAT(persona.Prs_Ape,' ',persona.Prs_Nom)  AS cliente";
			$sql = "SELECT $campos
					FROM cliente
					INNER JOIN persona ON (cliente.Prs_Cod = persona.Prs_Cod)
					WHERE (cliente.Emp_Cod = '$_SESSION[Ses_Emp_Cod]') AND $search AND $estado $Par_Sql[limits];";
			return $sql;
			break;
		...
		// 
		//	CONSULTAS PARA APP MOVIL
		//
		case 1000:
			$sql = "SELECT cliente.Prs_Cod, Cli_Cod, cliente.Emp_Cod, persona.Prs_Ced, persona.Prs_Nom, persona.Prs_Ape, persona.Prs_Dir
			FROM cliente
			INNER JOIN persona ON (cliente.Prs_Cod = persona.Prs_Cod)
			WHERE (cliente.Emp_Cod = '$Par_Sql[Emp_Cod]')";
			return $sql;
			break;

		// OPERACIONES SQL PARA CLIENTES QUE TIENEN CREDITO (GASOLINERA)
		case 30:
			if (empty($Par_Sql["Cod_Cre"])) {
				$sql = "INSERT INTO clientes_credito(Nom_Cli,Tim_Cre,Ced_Cre,Cre_Est,Emp_Cod) 
				VALUES('$Par_Sql[Nom_Cli]',$Par_Sql[Tim_Cre] ,'$Par_Sql[Ced_Cre]' ,'$Par_Sql[Cre_Est]','$Par_Sql[Emp_Cod]');";
			} else {

				$sql = "UPDATE clientes_credito  SET Nom_Cli = TRIM('$Par_Sql[Nom_Cli]'),  Tim_Cre = '$Par_Sql[Tim_Cre]',  Ced_Cre = '$Par_Sql[Ced_Cre]', Cre_Est = '$Par_Sql[Cre_Est]' 
       				 WHERE Cod_Cre = '$Par_Sql[Cod_Cre]'";
			}
			return $sql;
			break;

		case 31: //Select para listar los clientes registrados en la empresa
			if ($Par_Sql['op_opciones'] == "d") {
				$search = " (Nom_Cli LIKE '%$Par_Sql[search]%' OR Ced_Cre LIKE '%$Par_Sql[search]%')";
			} else {
				$search = " Ced_Cre LIKE '$Par_Sql[search]%'";
			}
			if (isset($Par_Sql["limits"])) {
				$Par_Sql["limits"] = "ORDER BY Nom_Cli $Par_Sql[limits]";
				$campos = "Cod_Cre,Nom_Cli,Tim_Cre,Ced_Cre,Cre_Est";
			} else {
				$campos = "COUNT(Cod_Cre) as total";
				$Par_Sql["limits"] = "";
			}
			$sql = "SELECT $campos FROM clientes_credito
						WHERE $search  AND Emp_Cod=$Par_Sql[Emp_Cod] $Par_Sql[limits]";
			return $sql;
			break;

		case 32:
			$sql = "SELECT * FROM clientes_credito WHERE Ced_Cre='$Par_Sql[0]' AND Emp_Cod='$Par_Sql[1]'";
			return $sql;
			break;

		//SOCIOS CHOFERES
		case 33: //Insert chofer
			$sql = "INSERT INTO rutas_fact_extra(Ext_Nom,Ext_Ciu,Ext_Fec,
			Emp_Cod,Ext_Telf, Ext_Placa,Ext_Email, Ext_Ruc, Ext_Reg_Sri, Ext_Dir) 
			VALUES('$Par_Sql[Ext_Nom]','$Par_Sql[Ext_Ciu]','$Par_Sql[Ext_Fec]','$Par_Sql[Emp_Cod]',
			'$Par_Sql[Ext_Telf]','$Par_Sql[Ext_Placa]','$Par_Sql[Ext_Email]','$Par_Sql[Ext_Ruc]',
			'$Par_Sql[Ext_Reg_Sri]','$Par_Sql[Ext_Dir]');";
			return $sql;

		case 34:
			$sql = "SELECT Prs_Nom, Prs_Ape,  Pun_Sri , autorizaci.Aut_Cod FROM autorizaci 
			INNER JOIN puntos_imp ON puntos_imp.Pun_Cod = autorizaci.Pun_Cod
			INNER JOIN sucursal ON sucursal.Suc_Cod = puntos_imp.Suc_Cod
			INNER JOIN vendedor ON vendedor.Pun_Cod = autorizaci.Pun_Cod
			INNER JOIN persona ON vendedor.Prs_Cod = persona.Prs_Cod
			WHERE Aut_Tpt='$Par_Sql[0]' AND  sucursal.Emp_Cod='$Par_Sql[1]'";
			return $sql;
			break;

		case 35: //Select para listar los clientes registrados en la empresa
			if ($Par_Sql['op_opciones'] == "d") {
				$search = "(Ext_Nom LIKE '%$Par_Sql[search]%' OR Ext_Ruc LIKE '%$Par_Sql[search]%')";
			} else {
				$search = "Ext_Ruc LIKE '$Par_Sql[search]%'";
			}
			if (isset($Par_Sql["limits"])) {
				$Par_Sql["limits"] = "ORDER BY Ext_Nom $Par_Sql[limits]";
				$campos = " rutas_fact_extra.* ";
			} else {
				$campos = "COUNT(Ext_Cod) as total";
				$Par_Sql["limits"] = "";
			}
			$sql = "SELECT $campos FROM rutas_fact_extra
					WHERE $search AND rutas_fact_extra.Emp_Cod=$Par_Sql[Emp_Cod]  AND Ext_Ruc IS NOT NULL   $Par_Sql[limits]";
			return $sql;
			break;

		case 36: // Update chofer
			$sql = "UPDATE rutas_fact_extra  SET Ext_Nom = '$Par_Sql[Ext_Nom]', Ext_Ciu = '$Par_Sql[Ext_Ciu]',
                Ext_Telf = '$Par_Sql[Ext_Telf]', Ext_Placa = '$Par_Sql[Ext_Placa]',
                Ext_Email = '$Par_Sql[Ext_Email]', Ext_Ruc = '$Par_Sql[Ext_Ruc]',
                Ext_Reg_Sri = '$Par_Sql[Ext_Reg_Sri]', Ext_Dir = '$Par_Sql[Ext_Dir]'  WHERE Ext_Cod = '$Par_Sql[Ext_Cod]';";
			return $sql;

		case 37:
			$sql = " ";
			if (!empty($Par_Sql[0])) {
				if ($Par_Sql[1] == 'p') {$sql = " AND  Veh_Pla =" . "$Par_Sql[0]";}
				if ($Par_Sql[1] == 'd') {$sql = " AND  Ext_Nom  Like " . "'%$Par_Sql[0]%'";}
				if ($Par_Sql[1] == 'c') {$sql = " AND  Ext_Ruc =" . "$Par_Sql[0]";}
			}
			$sql = "SELECT vehiculo.*, Ext_Nom, Ext_Ruc, Ext_Email, Ext_Dir FROM vehiculo   
			INNER JOIN rutas_fact_extra ON vehiculo.Ext_Cod = rutas_fact_extra.Ext_Cod
			WHERE  vehiculo.Emp_Cod='$Par_Sql[2]' AND Veh_Est='A'  $sql   ";
			return $sql;
			break;
		case 38: // Obtener clientes de otra empresa para copiar
			if ($Par_Sql['op_opciones'] == "d") {
			...
			$campos = empty($Par_Sql['limits']) ? " COUNT(cliente.Cli_Cod) AS total" : "cliente.Prs_Cod, Cli_Cod, cliente.Emp_Cod, persona.Prs_Ced, persona.Prs_Tel, persona.Prs_Cor, persona.Prs_Dir, persona.Prs_Nom, persona.Prs_Ape, CONCAT(persona.Prs_Ape,' ',persona.Prs_Nom) AS cliente, cliente.Cli_Tic, cliente.Cli_Con, cliente.Cli_Tip";
			$sql = "SELECT $campos
					FROM cliente
					INNER JOIN persona ON (cliente.Prs_Cod = persona.Prs_Cod)
					WHERE (cliente.Emp_Cod = '$Par_Sql[Emp_Cod_Origen]') AND $search AND $estado $Par_Sql[limits];";
			return $sql;
			break;

		case 39: // Verificar si un cliente ya existe en la empresa destino
			$sql = "SELECT Cli_Cod FROM cliente WHERE Prs_Cod='$Par_Sql[0]' AND Emp_Cod='$Par_Sql[1]'";
			return $sql;
			break;
	}
