 ...
 * @version 1.0
 * Fecha de actualizaciÃ³n:	2012-11-27
 *
 * @param int $id
 * @param array $Par_Sql
 * @return string $sql
	...
	switch ($id) {
		...
		case 277:
			$sql = "SELECT det_plan.Pld_Cod, det_plan.Pld_Cdc, det_plan.Pld_Des
				  ...
				  WHERE  det_plan.Pld_Cod=plan_param.Pld_Cod
							AND plan_param.Tpa_Cod=tipo_param.Tpa_Cod
							AND tipo_param.Tpa_Abr='RC'
							AND det_plan.Pld_Est='A' AND
				  det_plan.Pla_Cod = (select max(plan_cuenta.Pla_Cod)
				  from plan_cuenta WHERE (plan_cuenta.Emp_Cod = '$_SESSION[Ses_Emp_Cod]'));";
			return $sql;
		case 28:
		...
		case 44:
			...
			// bloque de consulta para las negociaciones de camaron
			$filtroCxp = "";
			if( $Par_Sql['isnego'] == "S" ){
				$filtroCxp = " LEFT JOIN nego_documentos ON nego_documentos.Cod_Doc = compras.Cop_Cod
								LEFT JOIN nego_camaron AS negocam ON nego_documentos.Cod_Neg = negocam.Cod_Neg ";
			}
			// clausula de filtrado por negociaciones
			$filtroWhereCxp = "";
			if(!empty($Par_Sql['filtroCCxPP'])){
				$filtroWhereCxp = "AND nego_documentos.Tip_Prod='$Par_Sql[filtroCCxPP]'";
            }
		...
		case 557:
			...
			$rangosSQL = "";
			$numRangos = isset($Par_Sql['numRangos']) ? intval($Par_Sql['numRangos']) : 4;
			...
			for ($i = 1; $i <= $numRangos; $i++) {
				...
				if ($rangoIni > 0 && $rangoFin > 0) {
					$rangosSQL .= ",IF(DATEDIFF(CURDATE(),Cpp_Ven)>=$rangoIni && DATEDIFF(CURDATE(),Cpp_Ven)<=$rangoFin, (asientos.Asi_Val - IF(SUM(IF(comp2.Com_Est='A',ROUND(ROUND(Pag_Val,2),2),0)) IS NULL,0,SUM(IF(comp2.Com_Est='A',ROUND(ROUND(Pag_Val,2),2),0)))), '0.00') AS Rango$i\n\t\t\t\t\t";
				}
			}
			...
			if ($ultimoRangoFin > 0) {
				$rangosSQL .= ",IF(DATEDIFF(CURDATE(),Cpp_Ven)>$ultimoRangoFin, (asientos.Asi_Val - IF(SUM(IF(comp2.Com_Est='A',ROUND(ROUND(Pag_Val,2),2),0)) IS NULL,0,SUM(IF(comp2.Com_Est='A',ROUND(ROUND(Pag_Val,2),2),0)))), '0.00') AS RangoUltimo\n\t\t\t\t\t";
			}
  ...
  FROM
	compras
	  ...
	  GROUP BY compras.Cop_Cod";
		...
		case 128:
			$sql = "SELECT Cop_Cod FROM ccpp_pagar WHERE Cpp_Cod=$Par_Sql[Cpp_Cod]";
			break;

		case 129:
			$sql = "SELECT COUNT(Cop_Cod) AS cant_cop FROM det_reposicion WHERE Cop_Cod=$Par_Sql[0]";
			break;

		case 130:
			$sql = "SELECT IFNULL(SUM(t.total), 0) AS total_g_caja FROM ( SELECT  IF( ( SELECT SUM(dcpp.Pag_Val) 
					FROM ccpp_pagar cpp 
						INNER JOIN det_ccpp_p dcpp ON dcpp.Cpp_Cod = cpp.Cpp_Cod  INNER JOIN tipos_pago ON tipos_pago.Pag_Cod = dcpp.Pag_Cod 
						INNER JOIN comprobantes ON comprobantes.Com_Cod = dcpp.Com_Cod
					WHERE cpp.Cop_Cod = compras.Cop_Cod AND comprobantes.Com_Est = 'A'  AND Pag_Abr = 'RC') > 0,
						( SELECT SUM(dcpp.Pag_Val) FROM ccpp_pagar cpp  INNER JOIN det_ccpp_p dcpp ON dcpp.Cpp_Cod = cpp.Cpp_Cod  
						INNER JOIN tipos_pago ON tipos_pago.Pag_Cod = dcpp.Pag_Cod  INNER JOIN comprobantes ON comprobantes.Com_Cod = dcpp.Com_Cod
					WHERE cpp.Cop_Cod = compras.Cop_Cod  AND comprobantes.Com_Est = 'A'  AND Pag_Abr = 'RC'),
						SUM(( (det_compra.Cop_Imp - (((det_compra.Cop_Imp * compras.Cop_Des) / 100) + ((det_compra.Cop_Imp * det_compra.Cop_Dec) / 100))) + 
						(det_compra.Cop_Imp - (((det_compra.Cop_Imp * compras.Cop_Des) / 100) + ((det_compra.Cop_Imp * det_compra.Cop_Dec) / 100))) * 
						(IF(ice.Ice_Por IS NOT NULL, 1 + ice.Ice_Por / 100, 0)) ) * (1 + iva.Iva_Por / 100))) AS total
					FROM compras
						INNER JOIN det_reposicion ON compras.Cop_Cod = det_reposicion.Cop_Cod INNER JOIN proveedore ON compras.Prv_Cod = proveedore.Prv_Cod
						INNER JOIN det_compra ON compras.Cop_Cod = det_compra.Cop_Cod  
						INNER JOIN iva ON det_compra.Iva_Cod = iva.Iva_Cod LEFT JOIN ice ON ice.Ice_int = det_compra.Ice_Int
					WHERE Dre_Tip = 'P'   AND Cop_Est = 'A' AND Emp_Cod = '$Par_Sql[0]'
						AND EXISTS (  SELECT 1  FROM ccpp_pagar cpp  INNER JOIN det_ccpp_p dcpp ON dcpp.Cpp_Cod = cpp.Cpp_Cod  INNER JOIN comprobantes c ON c.Com_Cod = dcpp.Com_Cod
					WHERE cpp.Cop_Cod = compras.Cop_Cod AND c.Com_Est = 'A') GROUP BY compras.Cop_Cod) AS t;";
			break;
		case 131:
			$sql = "SELECT Cch_Val AS val_caja FROM caja_chica WHERE Emp_Cod=$Par_Sql[0]  AND Cch_Est='A'";
			break;


		//Anular el pago det_reposicion
		case 132:
			$sql = "DELETE det_reposicion
					FROM det_reposicion
					INNER JOIN compras ON det_reposicion.Cop_Cod = compras.Cop_Cod
					INNER JOIN ccpp_pagar ON ccpp_pagar.Cop_Cod = compras.Cop_Cod
					WHERE ccpp_pagar.Cpp_Cod = '$Par_Sql[Cpp_Cod]' AND det_reposicion.Dre_Tip = 'P'";
			break;
		case 133:	
			//var_dump($Par_Sql);		
			$sql = "INSERT INTO det_reposicion(Cop_Cod,Dre_Int,Dre_Tip,Com_Cod,Dre_Val)VALUES($Par_Sql[Cop_Cod],$Par_Sql[Dre_Int],'P',$Par_Sql[Com_Cod],$Par_Sql[Dre_Val])";
			//ChromePhp::log($sql);			 
			break;
		case 134:
			$sql = "DELETE det_reposicion FROM det_reposicion WHERE Com_Cod = '$Par_Sql[Com_Cod]' AND det_reposicion.Dre_Tip = 'P'";
			break;
		...
		case 135:
            ...
            break;
		case 136:
			$sql="SELECT conciliacion_banc_asientos.Asi_Cod
			FROM conciliacion_banc_asientos 
				LEFT JOIN asientos ON conciliacion_banc_asientos.Asi_Cod = asientos.Asi_Cod
				LEFT JOIN comprobantes ON asientos.Com_Cod = comprobantes.Com_Cod
			WHERE conciliacion_banc_asientos.Asi_Cod is not null AND comprobantes.Com_Cod=$Par_Sql[Com_Cod]";
			ChromePhp::log($sql);
			break;
	}
	return $sql;
