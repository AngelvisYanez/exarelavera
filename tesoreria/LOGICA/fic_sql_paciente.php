...
function sentencias_facturaVenta($id,$Par_Sql)
{
    switch($id)
    {
        case 1://Listado de clientes
            ...
            if($Par_Sql[3]==""){$campos="COUNT(Pac_Cod) as total";}
            else{
                $Par_Sql[3]="ORDER BY Prs_Ape ".$Par_Sql[3];
                $campos=" Pac_Cod, persona.Prs_Cod, Prs_Ced, persona.Prs_Sex, persona.Prs_Tel, ciudad.Ciu_Des, CONCAT(Prs_Ape,' ',Prs_Nom) as paciente, Pac_Emp, Pac_Fna,  Pac_Dir, Prs_Dir, IF(Pac_Cor IS NULL OR TRIM(Pac_Cor)='',Prs_Cor,Pac_Cor)AS Prs_Cor, IF(Pac_Est='A','Activo','Inactivo') as Pac_Est";
            }
            $sql="SELECT $campos FROM paciente
                ...
                INNER JOIN ciudad ON ciudad.Ciu_Cod = persona.Ciu_Cod 
                    WHERE Prs_Ced!='0' 
                    AND $search 
                    AND paciente.Prs_Cod=persona.Prs_Cod 
                    AND Pac_Est='A' 
                    AND paciente.Emp_Cod = $Par_Sql[1] $Par_Sql[3]";
	//ChromePhp::log($sql);

            break;
        case 2://Busqueda de clientes
            $sql="SELECT persona.*,paciente.Pac_Cod,CONCAT(Prs_Ape,' ',Prs_Nom) as paciente FROM persona
                    LEFT JOIN paciente ON paciente.Prs_Cod=persona.Prs_Cod AND paciente.Emp_Cod = $Par_Sql[1]
                    WHERE Prs_Ced LIKE '$Par_Sql[0]%'  LIMIT 2;";
		//ChromePhp::log($sql);
            break;
        case 3://Insert en la tabla persona
            $sql="INSERT INTO persona(Prs_Ced,Prs_Ape,Prs_Nom,Prs_Dir,Prs_Cor,Prs_Sex,Ciu_Cod,Ide_Cod,Prs_Tel) VALUES('$Par_Sql[Prs_Ced]','$Par_Sql[Prs_Ape]','$Par_Sql[Prs_Nom]','$Par_Sql[Prs_Dir]','$Par_Sql[Prs_Cor]','$Par_Sql[Prs_Sex]',$Par_Sql[Ciu_Cod],2,$Par_Sql[Prs_Tel]);";
            //ChromePhp::log($sql);
            break;
        case 4://Insert en la tabla cliente
            $sql="INSERT INTO paciente(Prs_Cod,Pac_Emp,Pac_Dir,Pac_Fna,Pac_Cor,Emp_Cod) VALUES($Par_Sql[Prs_Cod],'$Par_Sql[Pac_Emp]','".(empty($Par_Sql['Pac_Dir'])?'':$Par_Sql['Pac_Dir'])."','$Par_Sql[Cli_Fna]',".(empty($Par_Sql['Pac_Cor'])?'NULL':"'$Par_Sql[Pac_Cor]'").",$Par_Sql[Emp_Cod]);";
            //ChromePhp::log($sql);
            break;
        ...
        case 11://Select para comprobar si el numero de secuencia ya se encuentra registrado
            ...
            break;
        case 12:
            $sql = "SELECT paciente.Pac_Cod, CONCAT(persona.Prs_Nom,' ',persona.Prs_Ape) AS paciente, paciente.Pac_Dir, Pac_Emp, Pac_Cor, Prs_Ced, Prs_Sex, Pac_Fna, Fic_Num, Fic_Fec FROM ficha_medicamento
                    INNER JOIN medicamento ON ficha_medicamento.Med_Cod = medicamento.Med_Cod
                    INNER JOIN ficha_paciente ON ficha_medicamento.Fic_Cod = ficha_paciente.Fic_Cod
                    INNER JOIN paciente ON ficha_paciente.Pac_Cod = paciente.Pac_Cod
                    INNER JOIN persona ON paciente.Prs_Cod = persona.Prs_Cod
                WHERE ficha_medicamento.Fic_Cod=$Par_Sql[Fic_Cod] GROUP BY paciente.Pac_Cod";
            //ChromePhp::log($sql);
            break;
        ...
        case 14://Select para obtener el precio de los productos
            $sql="SELECT MAX(fic_num) as numero FROM ficha_paciente";
            break;
        case 15://fichas registradas
            $sql = "SELECT Fic_Cod, ficha_paciente.Pac_Cod, ficha_paciente.Fic_Fec, ficha_paciente.Fic_Num, Fic_Fec, Fic_Mot, Fic_Hea, Fic_Est, Fic_Obs, paciente.Pac_Dir, paciente.Pac_Fna, paciente.Pac_Emp, persona.Prs_Dir, persona.Prs_Ced, ciudad.Ciu_Des, persona.Prs_Cor, persona.Prs_Sex, persona.Prs_Tel, CONCAT(persona.Prs_Ape,' ',persona.Prs_Nom) AS paciente
            FROM ficha_paciente
            INNER JOIN paciente ON ficha_paciente.Pac_Cod = paciente.Pac_Cod
            INNER JOIN persona ON paciente.Prs_Cod = persona.Prs_Cod
            INNER JOIN ciudad ON ciudad.Ciu_Cod = persona.Ciu_Cod WHERE paciente.Pac_Cod=$Par_Sql[Pac_Cod]
            ORDER BY ficha_paciente.Fic_Num DESC";
            //ChromePhp::log($sql);
            break;
