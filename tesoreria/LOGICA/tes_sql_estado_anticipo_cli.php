...
function sentencias_anticipo_cli($id, $Par_Sql)
{
	switch ($id) {
		...
		/* Kardex anticipos + consumos clientes (orden por fecha); saldo acumulado en PHP */
		case 201:
			...
			if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $fec_ini)) $fec_ini = date('Y-01-01');
			if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $fec_fin)) $fec_fin = date('Y-m-d');
			$estadoFiltro = isset($Par_Sql['letra']) ? trim($Par_Sql['letra']) : 'AUC';
			$estIn = "('A','U','C')";
			switch ($estadoFiltro) {
				case 'AUC': $estIn = "('A','U','C')"; break;
				case 'AUCI': $estIn = "('A','U','C','I')"; break;
				case 'AU': $estIn = "('A','U')"; break;
				case 'C': $estIn = "('C')"; break;
				case 'I': $estIn = "('I')"; break;
			}
			...
			$cliSql = '';
			if (isset($Par_Sql['Cli_Cod']) && $Par_Sql['Cli_Cod'] !== '' && ctype_digit((string) $Par_Sql['Cli_Cod']) && intval($Par_Sql['Cli_Cod']) > 0) {
				$cliSql = ' AND ant.Cli_Cod = ' . intval($Par_Sql['Cli_Cod']) . ' ';
			}
			...
			if ($ordenKardex === 'A') $orderSql = " ORDER BY Ant_Cod ASC, Fecha_Mov ASC, tipo_ord ASC, row_id ASC";

			$sqlBase = "SELECT * FROM (
				SELECT
					CONCAT('A', ant.Ant_Cod) AS row_id,
					'Anticipo' AS Tipo_Linea,
					ant.Ant_Cod AS Ant_Cod,
					ant.Ant_Est AS Ant_Est,
					ant.Cli_Cod AS Cli_Cod,
					DATE(ant.Ant_Fec) AS Fecha_Mov,
					ant.Ant_Fec AS Ant_Fec,
					...
					prs.Prs_Ced AS cedCli,
					prs.Prs_Ced AS Prs_Ced,
					prs.Prs_Cod AS Prs_Cod,
					IFNULL(ant.Ant_Obs,'') AS Ant_Obs,
					...
					CAST(ant.Ant_Val AS DECIMAL(14,4)) AS TOTAL,
					CAST(0 AS DECIMAL(14,4)) AS CONSUMO,
					1 AS tipo_ord,
					ant.Com_Cod AS Com_Cod,
					ant.Com_Cod AS Com_Cod_in,
					IFNULL(c.Com_Con,'') AS Glosa,
					'' AS Pag_Des,
					'' AS Pac_Es2,
					cli.Cli_Cod AS cliCod,
					IFNULL(c.Prv_Cod,'') AS Prv_Cod,
					'' AS Asi_Cod,
					'' AS Pag_Cod,
					'' AS Pac_Ctd,
					'' AS Pac_Obs,
					CAST(ant.Ant_Val AS DECIMAL(14,4)) AS Com_Val,
					c.Com_Fec AS Com_Fec
				FROM anticipos_clientes AS ant
				INNER JOIN cliente AS cli ON cli.Cli_Cod = ant.Cli_Cod
				INNER JOIN persona AS prs ON prs.Prs_Cod = cli.Prs_Cod
				...
				WHERE cli.Emp_Cod = " . $emp . "
					AND ant.Ant_Est IN " . $estIn . "
					AND ant.Ant_Fec BETWEEN '" . $fec_ini . " 00:00:00' AND '" . $fec_fin . " 23:59:59'
					" . $searchSqlPerson . $searchSqlNumAnt . $cliSql . "
					" . ($onlyConsumo ? " AND 1=0 " : "") . "
				UNION ALL
				SELECT
					CONCAT('C', ddc.Ddc_Cod) AS row_id,
					'Consumo' AS Tipo_Linea,
					ant.Ant_Cod AS Ant_Cod,
					ant.Ant_Est AS Ant_Est,
					ant.Cli_Cod AS Cli_Cod,
					DATE(ccon.Com_Fec) AS Fecha_Mov,
					ccon.Com_Fec AS Ant_Fec,
					CONCAT(tp2.Tia_Abr,'-',IF(CHAR_LENGTH(MONTH(ccon.Com_Fec))=1,CONCAT('0',CAST(MONTH(ccon.Com_Fec) AS CHAR)),CAST(MONTH(ccon.Com_Fec) AS CHAR)),'-',CAST(ccon.Com_Num AS CHAR)) AS codigoCompra,
					TRIM(CONCAT(IFNULL(prs.Prs_Nom,''),' ',IFNULL(prs.Prs_Ape,''))) AS nombre,
					prs.Prs_Ced AS cedCli,
					prs.Prs_Ced AS Prs_Ced,
					prs.Prs_Cod AS Prs_Cod,
					...
					CAST(ddc.Ddc_Val AS DECIMAL(14,4)) AS CONSUMO,
					2 AS tipo_ord,
					ccon.Com_Cod AS Com_Cod,
					ant.Com_Cod AS Com_Cod_in,
					IFNULL(ccon.Com_Con,'') AS Glosa,
					IFNULL(tpp.Pag_Des,'') AS Pag_Des,
					IFNULL(pac.Pac_Est,'') AS Pac_Es2,
					cli.Cli_Cod AS cliCod,
					IFNULL(ccon.Prv_Cod,'') AS Prv_Cod,
					IFNULL(pac.Asi_Cod,'') AS Asi_Cod,
					IFNULL(pac.Pag_Cod,'') AS Pag_Cod,
					IFNULL(pac.Pac_Ctd,'') AS Pac_Ctd,
					IFNULL(pac.Pac_Obs,'') AS Pac_Obs,
				...
				INNER JOIN cliente AS cli ON cli.Cli_Cod = ant.Cli_Cod
				INNER JOIN persona AS prs ON prs.Prs_Cod = cli.Prs_Cod
				INNER JOIN comprobantes AS ccon ON ccon.Com_Cod = ddc.Com_Cod
				...
				LEFT JOIN pag_anticipo_cli AS pac ON pac.Pac_Cod = ddc.Pac_Cod
				LEFT JOIN tipos_pago AS tpp ON tpp.Pag_Cod = pac.Pag_Cod
				WHERE cli.Emp_Cod = " . $emp . "
					AND ant.Ant_Est IN " . $estIn . "
					AND IFNULL(ccon.Com_Est,'A') <> 'I'
					AND ccon.Com_Fec BETWEEN '" . $fec_ini . "' AND '" . $fec_fin . "'
					" . $searchSqlPerson . $searchSqlNumCon . $cliSql . "
			...
			if ($ordenKardex === 'C') {
				$sql = "SELECT
						...
						Tipo_Linea,
						MIN(Ant_Cod) AS Ant_Cod,
						CASE WHEN SUM(CASE WHEN Ant_Est = 'I' THEN 1 ELSE 0 END) > 0 THEN 'I' ELSE MIN(Ant_Est) END AS Ant_Est,
						MIN(Cli_Cod) AS Cli_Cod,
						MIN(Fecha_Mov) AS Fecha_Mov,
						MIN(Ant_Fec) AS Ant_Fec,
						MIN(codigoCompra) AS codigoCompra,
						MIN(nombre) AS nombre,
						MIN(cedCli) AS cedCli,
						MIN(Prs_Ced) AS Prs_Ced,
						MIN(Prs_Cod) AS Prs_Cod,
						MIN(Ant_Obs) AS Ant_Obs,
						...
						Com_Cod,
						MIN(Com_Cod_in) AS Com_Cod_in,
						MIN(Glosa) AS Glosa,
						'' AS Pag_Des,
						'' AS Pac_Es2,
						MIN(cliCod) AS cliCod,
		...
		case 202:
			...
			$op = isset($Par_Sql['op_opciones']) ? $Par_Sql['op_opciones'] : 'p';
			$search = isset($Par_Sql['search']) ? trim($Par_Sql['search']) : '';
			if ($op === 'n' && $search !== '') return "SELECT 0 AS saldo_ini";
			$searchSqlPerson = '';
			if ($search !== '') {
				$esc = str_replace(array("\\", "'", '"'), array("\\\\", "''", ''), $search);
				if ($op === 'c') $searchSqlPerson = " AND prs.Prs_Ced LIKE '%" . $esc . "%' ";
				elseif ($op !== 'n') $searchSqlPerson = " AND ((UPPER(prs.Prs_Nom) LIKE UPPER('%" . $esc . "%')) OR (UPPER(prs.Prs_Ape) LIKE UPPER('%" . $esc . "%'))) ";
			}
			$cliSql = '';
			if (isset($Par_Sql['Cli_Cod']) && $Par_Sql['Cli_Cod'] !== '' && ctype_digit((string) $Par_Sql['Cli_Cod']) && intval($Par_Sql['Cli_Cod']) > 0) {
				$cliSql = ' AND ant.Cli_Cod = ' . intval($Par_Sql['Cli_Cod']) . ' ';
			...
			$sql = "SELECT (
				COALESCE((SELECT SUM(ant.Ant_Val) FROM anticipos_clientes AS ant
					INNER JOIN cliente AS cli ON cli.Cli_Cod = ant.Cli_Cod
					INNER JOIN persona AS prs ON prs.Prs_Cod = cli.Prs_Cod
					WHERE cli.Emp_Cod = " . $emp . " AND ant.Ant_Est IN " . $estInSaldoIni . "
					AND ant.Ant_Fec < '" . $fec_ini . " 00:00:00'
					" . $searchSqlPerson . $cliSql . "
				), 0) - COALESCE((SELECT SUM(ddc.Ddc_Val) FROM det_ant_cccc AS ddc
					INNER JOIN anticipos_clientes AS ant ON ddc.Ant_Cod = ant.Ant_Cod
					INNER JOIN cliente AS cli ON cli.Cli_Cod = ant.Cli_Cod
					INNER JOIN persona AS prs ON prs.Prs_Cod = cli.Prs_Cod
					INNER JOIN comprobantes AS ccon ON ddc.Com_Cod = ccon.Com_Cod
					WHERE cli.Emp_Cod = " . $emp . " AND ant.Ant_Est IN " . $estInSaldoIni . "
		...
		case 203:
			$antCod = isset($Par_Sql['Ant_Cod']) ? intval($Par_Sql['Ant_Cod']) : 0;
			if ($antCod <= 0) return "SELECT '' AS codigo_consumo, '' AS fecha_consumo, '' AS glosa_consumo, 0 AS valor_anticipo, 0 AS valor_consumo, 0 AS saldo_anticipo LIMIT 0";
			$sql = "SELECT
					...
					CAST((ant.Ant_Val - IFNULL((SELECT SUM(d2.Ddc_Val) FROM det_ant_cccc d2 WHERE d2.Ant_Cod = ant.Ant_Cod),0)) AS DECIMAL(14,4)) AS saldo_anticipo
				FROM det_ant_cccc AS ddc
				INNER JOIN anticipos_clientes AS ant ON ant.Ant_Cod = ddc.Ant_Cod
				INNER JOIN comprobantes AS c ON c.Com_Cod = ddc.Com_Cod
				INNER JOIN tipo_asien AS tp ON tp.Tia_Cod = c.Tia_Cod
				WHERE ddc.Ant_Cod = " . $antCod . "
		...
		case 204:
			...
			if ($comCod <= 0) return "SELECT 0 AS Ddc_Cod, 0 AS Ant_Cod, '' AS asiento_anticipo, 0 AS valor_anticipo, 0 AS valor_consumido, 0 AS saldo_momento, 0 AS saldo_final_hoy LIMIT 0";
			$sql = "SELECT
					ddc.Ddc_Cod AS Ddc_Cod,
					ant.Ant_Cod AS Ant_Cod,
					...
					CAST(ant.Ant_Val AS DECIMAL(14,4)) AS valor_anticipo,
					CAST(ddc.Ddc_Val AS DECIMAL(14,4)) AS valor_consumido,
					CAST((ant.Ant_Val - IFNULL((
						...
						WHERE d2.Ant_Cod = ant.Ant_Cod
							...
							AND (c2.Com_Fec < ccons.Com_Fec OR (c2.Com_Fec = ccons.Com_Fec AND d2.Com_Cod <= ddc.Com_Cod))
					),0)) AS DECIMAL(14,4)) AS saldo_momento,
					CAST((ant.Ant_Val - IFNULL((
						SELECT SUM(d3.Ddc_Val)
						FROM det_ant_cccc d3
						INNER JOIN comprobantes c3 ON c3.Com_Cod = d3.Com_Cod
						WHERE d3.Ant_Cod = ant.Ant_Cod AND IFNULL(c3.Com_Est,'A') <> 'I'
					),0)) AS DECIMAL(14,4)) AS saldo_final_hoy
				FROM det_ant_cccc AS ddc
				INNER JOIN comprobantes AS ccons ON ccons.Com_Cod = ddc.Com_Cod
				INNER JOIN anticipos_clientes AS ant ON ant.Ant_Cod = ddc.Ant_Cod
				...
				ORDER BY ant.Ant_Cod ASC";
			return $sql;

		/**
		 * Caso 44: Cuenta anticipos con descuadre entre ddc_total y ddc_activo
		 * (det_ant_cccc total vs solo con comprobantes activos)
		 */
		case 44:
			$sql = "SELECT COUNT(*) AS count
				FROM (
					SELECT
						ant.Ant_Cod,
						COALESCE(SUM(ddc.Ddc_Val), 0) AS ddc_total,
						COALESCE(SUM(CASE WHEN com.Com_Est = 'A' THEN ddc.Ddc_Val ELSE 0 END), 0) AS ddc_activo
					FROM anticipos_clientes AS ant
						INNER JOIN cliente AS cli ON cli.Cli_Cod = ant.Cli_Cod
						LEFT JOIN det_ant_cccc AS ddc ON ddc.Ant_Cod = ant.Ant_Cod
						LEFT JOIN comprobantes AS com ON com.Com_Cod = ddc.Com_Cod
					WHERE
						(ant.Ant_Est = 'A' OR ant.Ant_Est = 'U')
						AND cli.Emp_Cod = '$_SESSION[Ses_Emp_Cod]'
					GROUP BY ant.Ant_Cod
					HAVING ABS(ddc_total - ddc_activo) >= 0.01
				) AS descuadres";
			return $sql;

		/**
		 * Caso 45: Lista detallada de anticipos con descuadre para el modal
		 * Limit 500 filas
		 */
		case 45:
			$sql = "SELECT
					ant.Ant_Cod,
					ant.Ant_Est,
					COALESCE(ant.Ama_Cod, '') AS Ama_Cod,
					CONCAT(prs.Prs_Nom, ' ', prs.Prs_Ape) AS nombre,
					prs.Prs_Ced,
					COALESCE(SUM(ddc.Ddc_Val), 0) AS ddc_total,
					COALESCE(SUM(CASE WHEN com.Com_Est = 'A' THEN ddc.Ddc_Val ELSE 0 END), 0) AS ddc_activo,
					ABS(COALESCE(SUM(ddc.Ddc_Val), 0) - COALESCE(SUM(CASE WHEN com.Com_Est = 'A' THEN ddc.Ddc_Val ELSE 0 END), 0)) AS diferencia
				FROM anticipos_clientes AS ant
					INNER JOIN cliente AS cli ON cli.Cli_Cod = ant.Cli_Cod
					INNER JOIN persona AS prs ON prs.Prs_Cod = cli.Prs_Cod
					LEFT JOIN det_ant_cccc AS ddc ON ddc.Ant_Cod = ant.Ant_Cod
				...
				WHERE
					(ant.Ant_Est = 'A' OR ant.Ant_Est = 'U')
					AND cli.Emp_Cod = '$_SESSION[Ses_Emp_Cod]'
				GROUP BY ant.Ant_Cod, ant.Ama_Cod, prs.Prs_Nom, prs.Prs_Ape, prs.Prs_Ced
				HAVING ABS(ddc_total - ddc_activo) >= 0.01
		...
		/**
		 * Caso 46: Líneas de det_ant_cccc para un Com_Cod dado (subgrid modal)
		 */
		case 46:
			$sql = "SELECT
					ddc.Ddc_Cod,
					...
					ddc.Ddc_Val,
					ddc.Ddc_Obs,
					com_ant.Com_Num AS num_comprobante_anticipo,
					CONCAT(tas_ant.Tia_Abr, '-', MONTH(com_ant.Com_Fec), '-', com_ant.Com_Num) AS codigo_anticipo,
					CONCAT(prs.Prs_Nom, ' ', prs.Prs_Ape) AS cliente_nombre,
					prs.Prs_Ced
				FROM det_ant_cccc AS ddc
					INNER JOIN anticipos_clientes AS ant ON ant.Ant_Cod = ddc.Ant_Cod
					INNER JOIN comprobantes AS com_ant ON com_ant.Com_Cod = ant.Com_Cod
					INNER JOIN tipo_asien AS tas_ant ON tas_ant.Tia_Cod = com_ant.Tia_Cod
					INNER JOIN cliente AS cli ON cli.Cli_Cod = ant.Cli_Cod
					INNER JOIN persona AS prs ON prs.Prs_Cod = cli.Prs_Cod
				WHERE ddc.Com_Cod = $Par_Sql[Com_Cod]
				ORDER BY ddc.Ddc_Cod";
			return $sql;
	}
	return $sql;
}
