...
if(isset($ajax_suc_cod))
{
	...
	$rs_modalidad = $obBD_con1->consulta(sentencias_tes(261, ''), $obBD_conexion->conexion);
	...
	?>
        ...
        <input type="checkbox" name="checkbox" value="checkbox" onClick="if (this.checked){ document.getElementById('hdd_sql').value = 268; }
																else { document.getElementById('hdd_sql').value = 173; }; 
...
3 = Etapas semestrales -->
      <input name="hdd_sql" type="hidden" id="hdd_sql" value="173">
<?Php
...
{   /* Cargado de etapas */
	...
	$rs_etapas = $obBD_con1->consulta(sentencias_tes($sql, ''), $obBD_conexion->conexion);
...
/* Cargado de los periodo en base a las deudas */
if(isset($ajax_periodo))
{	 	 
     $Eta_Arr = explode('*',$Eta_Cod);
	 $rs_periodos = $obBD_con1->consulta(sentencias_tes(262, $obBD_con1->parametros($Suc_Cod.'*'.$Mod_Cod.'*'.$Eta_Arr[0])), $obBD_conexion->conexion);
	 ...
	 if ($Eta_Cod != "")
	 {
		 ...
		 {
	 		echo "<span class='Texto_Reporte_Rojo'>&iexcl;No existen deudas para estos periodos!</span>";
...
{	/* Control para saber las carreras de cada etapa */
	...
	/* Consultar las carreras por periodo */
	$rs_carreras_etapa=$obBD_con1->consulta(sentencias_tes(267, $obBD_con1->parametros($Per_Int)), $obBD_conexion->conexion);
