...
<th style="width: 200px;" align="left" ><input name="txt_colum2" type="text" value="Personal."size="60"  readonly="readonly" style="  <?Php echo estilo_head; ?>" id="txt_colum2"></th>
    ...
    <input type="hidden" size="30" id="hdd_cod_camp" name="hdd_cod_camp" value="<? echo $cadena; ?>"/>
	  <input type="hidden" size="30" id="hdd_cod_ing" name="hdd_cod_ing" value="<? echo $cading; ?>"/>
	  <input type="hidden" size="30" id="hdd_cod_egr" name="hdd_cod_egr" value="<? echo $cadegr; ?>"/>
...
<td style="width: 10px;" align="right" <? if ($vis[$cont_caja_ingre] == "N"){?> id="<? echo "det[".$td_cont."]"; ?>"<? }?>>
		...
		<input  type="text" name="hdd_ing_egr[<? echo $cont;?>,<? echo $cod;?>]" style=" <?Php echo estilo_body_right; ?>" id="hdd_ing_egr[<? echo $cont;?>,<? echo $cod; ?>]" size="6" <?php if(($hdd_cam_ing_egr_cal[$cont_caja_ingre] == "S") || ($Com_Tipo == 'C')){ echo "readonly='true'";}?> 
            ...
            <? if(($Com_Tipo == 'M') || ($Com_Tipo == 'A')){  ?> onkeyup="<? if($sum=='S'){?> 
<? echo formula_rol($cont, $cod, $modulo_rol['Event'], $req, $obBD_con1, $obBD_conexion); /*}*/ ?> <?php  
if($total_campoformula>0){ ?>  <? do{?>
...
SumaColumnas(this.form, <?php echo $total_rs_per_rol;?>) <?php }// fin de  if($sum=='S') ?>" <?  }// fin de if(($Com_Tipo == 'M') || ($Com_Tipo == 'A')){?> tabindex="<? if ($vis[$cont_caja_ingre] != "N"){ 
			echo $cont;}?>" <?Php echo focus_row_select_id('tbody_fila['.$cont.']',"resaltar_text", "resaltar_back", "undo_resaltar_text", "Fondo"); ?> />
		<input type="hidden" name="hdd_ingreso_egreso[<? echo $dep;?>,<? echo $cod; ?>]" id="hdd_ingreso_egreso[<? echo $cont;?>,<? echo $cod; ?>]" value="<? echo $cod;?>"/>		</td>
...
<? } while($row_rs_per_rol = $obBD_con1->fetch_assoc($rs_per_rol));  //Fin del IF $rs_per_rol ?>
 ...
 <tr class="Cabecera1">
   ...
   <td style="width: 10px;">
   <?php 
   if( ($row_rs_campos['Cam_Cod']==2) || ($row_rs_campos['Cam_Cod']==34) ){?><input type="text" name="campo_vac" id="campo_vac" value="<?php echo  $val;  ?>" size="6" style=" <?Php echo estilo_head; ?>" /><?php } else{ ?><input type="text" name="hdd_ingre[<? echo $row_rs_campos['Cam_Cod'];?>]" id="hdd_ingre[<? echo $row_rs_campos['Cam_Cod'];?>]" value="<?Php echo round($suma_colum_i[$row_rs_campos['Cam_Cod']],2); ?>" size="6" style=" <?Php echo estilo_head; ?>"  title=" <?php echo $row_rs_campos['Cam_Cod']; ?>" />
    <?php } ?>   </td>
   <?php 
   ...
   ?>
   <td style="width: 10px;" id="Det[<? echo $cont?>]"><input type="text" name="txt_total_ingres" id="txt_total_ingres" value="<?Php echo round($total_colum_i,2); ?>" size="10" readonly="readonly" style=" <?Php echo estilo_head_right; ?>" /></td>
    <?php
   /* Consulta campos ingresos del rol de pagos*/
	...
	$ke++;?>
   <td style="width: 10px;">
   <input type="text" name="hdd_ingre[<? echo $row_rs_campose['Cam_Cod'];?>]" id="hdd_ingre[<? echo $row_rs_campose['Cam_Cod'];?>]" value="<?Php echo $suma_colum_e[$row_rs_campose['Cam_Cod']]; ?>" size="6" style=" <?Php echo estilo_head; ?>" title=" <?php echo $row_rs_campose['Cam_Cod']; ?>"></td>
    <?php
