...
?>
	<table width="100%" height="36" border="0" cellpadding="0" cellspacing="0">
        ...
        {  
        	ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?buscod&busqueda=' + buscta.value + '&Sem_Cod=<?Php echo $row_rs_semestre['Sem_Cod']; ?>&codigo=<?Php echo $codigo; ?>&Per_Fea=<?Php echo $row_rs_semestre['Per_Fea']?>&Per_Fef=<?Php echo $row_rs_semestre['Per_Fef']?>&codigonota=<?Php echo $row_rs_semestre['Nge_Cod']?>&bloc_cant=<?Php echo $bloc_cant; ?>', 'rubros');
		}"><!--&Car_Nom=<?php //echo trim(cortar_cadena_param(' ', $row_rs_semestre['Car_Nom'])); ?>&Sem_Nom=<?Php //echo $row_rs_semestre['Niv_Des']." &#8220;".$row_rs_semestre['Sem_Par']."&#8221; ".$row_rs_semestre['Sem_Sec']; ?>-->
			</td>			
			<td width="110" align="center">
            <button type="button" class="btn btn-success btn-mini" title="Buscar" onClick="if (trim(document.getElementById('buscta').value) != ''){  ajax_datos('<?Php echo $_SERVER['PHP_SELF']; ?>?buscod&busqueda=' + buscta.value + '&Sem_Cod=<?Php echo $row_rs_semestre['Sem_Cod']; ?>&codigo=<?Php echo $codigo; ?>&Per_Fea=<?Php echo $row_rs_semestre['Per_Fea']?>&Per_Fef=<?Php echo $row_rs_semestre['Per_Fef']?>&codigonota=<?Php echo $row_rs_semestre['Nge_Cod']?>&bloc_cant=<?Php echo $bloc_cant; ?>', 'rubros'); } "> <i class="icon-search icon-white"></i> <span>Buscar</span> </button>
			</td>
