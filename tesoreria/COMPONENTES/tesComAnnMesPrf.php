...
* Carga los años 
*/
$row_rs_anios = $obBD_con1->getArrayConsulta(245, $Ses_Emp_Cod, $obBD_conexion);	
  ...
  <tr>
    ...
    <td width="10%" class="LetraNegra">
      <select name="cmb_anio" id="cmb_anio">
		...
		{
		?>
        <option <?php if ($anio == $row['Anio']){ echo "selected"; } ?> value="<?php echo $row['Anio']; ?>"><?php echo $row['Anio']; ?></option>
        <?Php
    ...
    <td width="10%" class="LetraNegra">
      <select name="cmb_mes" id="cmb_mes">
        ...
        <?Php
				  for ($i=1;$i<=12;$i++)
				  { ?>
        <option <?php if ($i == $mes){ echo "selected"; } ?> value="<?Php echo "AND MONTH(Prf_Fec)=$i"; ?>"><?php echo mes($i, 1) ?></option>
        <?Php
				  } ?>
      </select>
    </td>
