...
?>
	...
	function set_dia_mes()
	{
		...
		else //En caso de elegir el mes, se toma el primer dia del mes y el ultimo dia del mes 
		{
			...
			document.getElementById('txt_fec_ini').value= ann[0]+"-01-01"; //Antes document.getElementById('cmb_mes').value+"-01"
	...
	<tr>
	  <td width="6%" class="Etiqueta1"><div align="left"><span id="capa_fecha">
		Desde: <span class="LetraNegra">Enero</span> &nbsp;&nbsp;&nbsp;Hasta: 
		...
		<span id="capa_rango_fec">
			Desde:
			<input name="txt_fec_ini" type="text" id="txt_fec_ini" value="<?php if (isset($Chk_Fec)){ echo $txt_fec_ini;  }else{ echo $ann[0]."-01-01"; } //Antes $mes."-01" ?>" size="10"  onkeyup="mascara();" onBlur="validar_fecha2(this)" />
			...
			&nbsp;Hasta:
			<input name="txt_fec_fin" type="text" id="txt_fec_fin" value="<?php if (isset($Chk_Fec)){ echo $txt_fec_fin; } else{ echo $ann[0].'-'.$ann_act[1].'-'.$ann_act[2]; } ?>" size="10" onkeyup="mascara();" onBlur="validar_fecha2(this)" />
...
{
	echo error_alerta("<< Error de componente: con_con_anio_mes_fecha.php >> <br>Descripci�n: No se ha definido la Propiedad: Pec_Fei<br>
