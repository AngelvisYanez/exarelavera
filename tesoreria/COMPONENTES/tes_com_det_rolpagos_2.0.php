...
<td style="width: 10px;" align="right" <?php if ($vis[$cont_caja_ingre] == "N"){?> id="<?php echo "det[".$td_cont."]"; ?>"<?php }?>>
		...
		<input  type="text" name="hdd_ing_egr[<?php echo $cont;?>,<?php echo $cod;?>]" style=" <?Php echo estilo_body_right; ?>" id="hdd_ing_egr[<?php echo $cont;?>,<?php echo $cod; ?>]" size="6" <?php if(($hdd_cam_ing_egr_cal[$cont_caja_ingre] == "S") || ($Com_Tipo == 'C')){ echo "readonly='true'";}?> 
            ...
            <?php if(($Com_Tipo == 'M') || ($Com_Tipo == 'A')){  ?> onkeyup="<?php if($sum=='S'){ echo formula_rol($cont, $cod, $modulo_rol['Event'], $req, $obBD_con1, $obBD_conexion); /*}*/ ?> <?php  if($total_campoformula>0){ do{?>SumaCamposRol(this.form,  <?php echo $total_rs_per_rol;?>, <?php echo $row_rs_campoformula['Cam_Cod'];?>,<?php echo $cont;?>);<?php }while($row_rs_campoformula = $obBD_con1->fetch_assoc($rs_campoformula));} ?> SumaCamposRol(this.form,  <?php echo $total_rs_per_rol;?>, <?php echo $cod;?>,<?php echo $cont;?>);SumaColumnas(this.form, <?php echo $total_rs_per_rol;?>) <?php }// fin de  if($sum=='S') ?>" <?php  }// fin de if(($Com_Tipo == 'M') || ($Com_Tipo == 'A')){?> tabindex="<?php if ($vis[$cont_caja_ingre] != "N"){ 
...
<?php } while($row_rs_per_rol = $obBD_con1->fetch_assoc($rs_per_rol));  //Fin del IF $rs_per_rol ?>
 ...
 <tr class="Cabecera1">
   ...
   /* Consulta campos ingresos del rol de pagos*/
	...
	$k++;?>
   <td style="width: 10px;">
   <?php if(  ($row_rs_campos['Cam_Cod']==2) || ($row_rs_campos['Cam_Cod']==34) ){?><input type="text" name="campo_vac" id="campo_vac" value="0" size="6" style=" <?Php echo estilo_head; ?>" /><?php } else{ ?><input type="text" name="hdd_ingre[<?php echo $row_rs_campos['Cam_Cod'];?>]" id="hdd_ingre[<?php echo $row_rs_campos['Cam_Cod'];?>]" value="<?Php echo round($suma_colum_i[$row_rs_campos['Cam_Cod']],2); ?>" size="6" style=" <?Php echo estilo_head; ?>" />
    <?php } ?>
   </td>
   <?php 
