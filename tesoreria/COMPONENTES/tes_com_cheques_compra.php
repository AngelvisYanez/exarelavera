...
if (isset($For_Cod))
{
	...
	if (isset($Hdd_Valor))
	{
		...
		{	
			...
			$rs_saldo_anticipos = $obBD_con1->consulta(sentencias_tes(192, $obBD_con1->parametros($codigo)), $obBD_conexion->conexion);
	  ...
	  <?Php //Consulto la informacion del cheke Cop_Bus=> es el c�digo interno de la fatura de compra 
	        //echo "Codigo ===>".$Cop_Bus;
...
<input name="quitar_fila" id="quitar_fila" type="button" class="BotonEliminar" onclick="quitar_fila_mod_pre(this)" value="X"  />
  ...
  <?Php }while($row_rs_informacion_cheque=$obBD_con1->fetch_assoc($rs_informacion_cheque)); ?>              
			...
			<table width="607" border="0" cellpadding="0" cellspacing="0">
			  <tr>
				<td width="50" align="left"><input id="nfilas_ch" name="nfilas_ch" type="hidden" value="0" />
					<input name="Btn_Pagos" type="button" class="Boton_Dinero" id="Btn_Pagos" title="Agregar tipo de pago" onclick="botones_opcion_compra(1, 'Tbl_Pagos*Tbl_BusCtas*Tbl_Anticipos')" value="Pagos" /></td>
				<td width="62" align="left"><input name="Btn_BusCta" type="button" class="Boton_Libros" id="Btn_BusCta" title="Buscar cuenta contable" onclick="botones_opcion_compra(2, 'Tbl_Pagos*Tbl_BusCtas*Tbl_Anticipos')" value="Cuentas" /></td>
			    <td width="67" align="left"><input name="Btn_Anticipo" type="button" class="Boton_Dinero2" id="Btn_Anticipo" title="Cancelar anticipo" onclick="botones_opcion_compra(3, 'Tbl_Pagos*Tbl_BusCtas*Tbl_Anticipos')" value="Anticipos" /></td>
			    <td width="428" align="left"><?php if ($total_rs_saldo_anticipos > 0){  blink("El proveedor posee Anticipos", "txt_blink", "#FFFF00", "#FF0000"); } ?>
			...
			$i=0;
			  do{
					...
					$rs_cruzado_anticipos = $obBD_con1->consulta(sentencias_tes(194, $obBD_con1->parametros($row_rs_saldo_anticipos['Ant_Cod'])), $obBD_conexion->conexion);
