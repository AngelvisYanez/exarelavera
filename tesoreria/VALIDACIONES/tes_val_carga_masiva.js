        ...
        'tabs-1':function(){
            jgridBusqSri=$("#gridBusSri").createGrid({
                ...
                colModel: [
                    ...
                    { label: 'Tarifa dif.0%', name: 'TARIFA12', width: 25, align: 'right', formatter:'currency' },
        ...
        'tabs-2':function(){
            jgridBusPre=$("#gridPreCar").createGrid({
                ...
                colModel: [
                    ...
                    { label: 'Tarifa dif.0%', name: 'Carm_Tard', width: 25, align: 'right', formatter:'currency' },
                    { label: 'Tarifa 0%', name: 'Carm_Tarc', width: 25, align: 'right', formatter:'currency' },
...
function fetchData()
{
    ...
    console.log(opcion);
    $.getDataJson('',{'searchDocument':true, 'num_doc':numero, 'opc':opcion},
        function(res){
...
function fetchDataOmi()
{
    ...
    console.log(opcion);
    $.getDataJson('',{'searchDocumentOmi':true, 'num_doc':numero, 'opc':opcion},
        function(res){
...
function fetchDataExi()
{
    ...
    $.getDataJson('',{'searchDocumentExi':true, 'num_doc':numero, 'opc':opcion},
        function(res){
...
function saveXml(){
    ...
    function colores(subtotalesPeticion){
        ...
        $("#gridBusSri tbody tr[role='row']").each(function(i){
            ...
            if(numero){
            $.getDataJson("", {colores: true, clave:claveAcceso, cop_num:numero, prs_ced:cedula, subtotalesPeti:subtotalesPeticion}, function(responce) { 
		    ...
		    $('#process').css('display', 'none');
                ...
                if(subtotalesPeticion){
                    //SETEAR LOS SUBTOTALES
                    var subtotales = responce.subts;
                    var rowData = $("#gridBusSri").jqGrid('getRowData', codFila);
                    $("#gridBusSri").jqGrid('setCell', rowData['id'], 'TARIFA12', subtotales[0]);
                    $("#gridBusSri").jqGrid('setCell', rowData['id'], 'TARIFA0', subtotales[1]);
                    $("#gridBusSri").jqGrid('setCell', rowData['id'], 'IVA', subtotales[2]);
                    $("#gridBusSri").jqGrid('setCell', rowData['id'], 'IMPORTE_TOTAL', subtotales[3]);

                    //TOTALES
                    tarifa12total = tarifa12total + parseFloat(subtotales[0]);
                    tarifa0total = tarifa0total + parseFloat(subtotales[1]);
                    ivatotal = ivatotal + parseFloat(subtotales[2]);
                    total = total + parseFloat(subtotales[3]);
                }
                ...
                if(subtotalesPeticion){
                    //SETEAR LOS TOTALES
                    $(".footrow").find("td[aria-describedby='gridBusSri_TARIFA12']").text(tarifa12total.toFixed(2).toString());
                    $(".footrow").find("td[aria-describedby='gridBusSri_TARIFA0']").text(tarifa0total.toFixed(2).toString());
                    $(".footrow").find("td[aria-describedby='gridBusSri_IVA']").text(ivatotal.toFixed(2).toString());
                    $(".footrow").find("td[aria-describedby='gridBusSri_IMPORTE_TOTAL']").text(total.toFixed(2).toString());
                }
            },function(r){
		$("#" + codFila).removeClass('myAltRowClass');
                porcen = porcen + increase;
