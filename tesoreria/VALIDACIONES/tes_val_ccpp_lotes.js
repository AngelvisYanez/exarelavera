...
$(function () {
  ...
  $( "#verFactsDialog" ).createDialog({width:700,height:350,icon:'info-sign'});

  $("#searchGrid").createGrid({
    postData: $("#searchCcpp").getData("ajaxComprobante"), height: 300,
    ...
    colModel: [
      ...
      { label:'<i class="ui-icon ui-icon-circle-check"></i>', name: 'act', width: 10, align: 'center',viewable: false,
        formatter: function (cellvalue, options, rowObject) {
      ...
      { classes:'columnDisabled no_padding',label: 'A Pagar', name: 'Pago', width: 45, align: 'right',
        formatter: function (cellvalue, options, rowObject) {
          if (rowObject.vencimiento==='Pagado') {
            return "";
          }else {
        ...
        decimalPlaces: '2', summaryRound: 2,
    ...
    loadComplete:function(data){
      ...
      $('#searchGrid tr').each(function () {
        if($(this).find("td").eq(12).text()==="Vencido"){
					$(this).addClass("cellRed2");
					$(this).addClass("myAltRowClass");
				}
        if($(this).find("td").eq(12).text()==="Pagado"){
    ...
    {caption:'Marcar todos',buttonicon:'glyphicon glyphicon-check',class:'btn-sg-pg',id:"mrc_btn_sg",
      onClickButton: function(){
        $("[id^='sg_act_']:not(:checked)").trigger("click");
      }
    },
    {caption:'Desmarcar todos',buttonicon:'glyphicon glyphicon-unchecked',class:'btn-sg-pg',
      onClickButton: function(){
  ...
  $('#showPagosChe').createGrid({viewrecords:false,
    ...
    data:[], rowNum: 100, height: 80, width: 650, footerrow:true, responsive:false,
    ...
    colModel:[
      ...
      {label:'<center><i class="ui-icon ui-icon-gear"></i></center>', name: 'btns_anti', width: 10, align: 'center',viewable: false,
				formatter:function (cellvalue, options, rowObject) {
					if(rowObject.Che_Est==='P' || rowObject.Che_Est==='I' || rowObject.Che_Est==='C'){
						return "-";
					} else{
...
function cargarFactsGrid(){
  $('#factsGrid').createGrid({viewrecords:false,
    ...
    colModel:[
      ...
      { label: 'Fecha Emis.', name: 'Cop_Fec', width: 15, align:"left"},
      { label: 'Fecha Venc.', name: 'Cpp_Ven', width: 15, align:"left"},
      { label: 'No. Doc.', name: 'Cop_Num', width: 30, align:"left"},
      { label: 'Total', name: 'total', width: 13, align: 'right', formatter:'currency', editable:true,
        formatoptions: {
          prefix:'$ ', thousandsSeparator:',',decimalSeparator:'.',defaultValue:''
        }
      },
      { label: 'Pagado', name: 'Abono', width: 13, align: 'right', formatter:'currency', editable:true,
        formatoptions: {
          prefix:'$ ', thousandsSeparator:',',decimalSeparator:'.',defaultValue:''
        }
      },
      { label:'<i class="ui-icon ui-icon-circle-check"></i>', name: 'fctact', width: 6, align: 'center',viewable: false,
        formatter: function (cellvalue, options, rowObject) {
...
function migrarData(){
  ...
  setTimeout(function(){
    //******************************buscamos empresas*******************************
    $.post( "", {a_migrar:true}, function( responce ) {
      if(responce['success']===true){
        datos_mig = responce['dat'];
      }else{
    ...
    setTimeout(function(){
      $.each(datos_mig, function(em, item_d) {
        //********************************buscamos proveedores de una empresa****************
        $.post( "", {migrar:true, Asi_Cod:item_d.Asi_Cod, Cpp_Cod:item_d.Cpp_Cod, Pag_Cod:item_d.Pag_Cod, Com_Cod:item_d.Com_Cod, Pag_Fec:item_d.Pag_Fec, Pag_Val:item_d.Pag_Val, Pag_Obs:item_d.Pag_Obs}, function( responce ) {
          if(responce['success']===true){
            //
          }else{
            $.alert(responce['message']);
          }
        },'json').fail(function(error) {$.alert("El Servidor ha fallado en responder! 2");});
      ...
      setTimeout(function(){
        $.post( "", {migrado:true}, function( responce ) {
          if(responce['success']===true){
            $.alert("migracion exitosa...");
          }else{
            $.alert(responce['message']);
          }
        },'json').fail(function(error) {$.alert("El Servidor ha fallado en responder! 3");});
    ...
    if(responce['success']===true){
      for(let i=0; i<responce['data'].length; i++){
        $('#factsGrid').jqGrid('addRowData', responce['data'][i].Cpp_Cod,
        {
          ...
          Com_Cod:responce['data'][i].Com_Cod,
          flag:"s",
          Com_Codigo:responce['data'][i].Com_Codigo,
          Cop_Fec:responce['data'][i].Cop_Fec,
          Cpp_Ven:responce['data'][i].Cpp_Ven,
          Cop_Num:responce['data'][i].Cop_Num,
          total:responce['data'][i].total,
  ...
  $('#pagosGrid').createGrid({viewrecords:false,
    ...
    colModel:[
      ...
      { label: '<center><i class="ui-icon ui-icon-gear"></i></center>',
        name: 'Pag_Item', width: 5, align: 'center', viewable: false,
        formatter:function (cellvalue, options, rowObject) {
...
function anularAbono(row){
  $.post( "", {delAbono:true,Com_Cod:row[0].Com_Cod}, function( responce ) {
    if(responce['success']===true){
...
function enableDisableCampos(){
  ...
  }else if($("#Pag_Cod option:selected").attr("data-abr") === 'CCC'){
...
function addPagoIni(){
  $.post( "", {getPagoIniAjax:true}, function( responce ) {
		if(responce['success']===true){
      let prm_array = ["D","inicial","","","","","","","","","","",responce['data'].Pld_Cod,responce['data'].Pld_Cdc,responce['data'].Pld_Des,$("#Com_Val").val(),"",$("#Com_Con").val(),"first",""];
      addPago(prm_array);
		}else{
			$.alert(responce['message']);
		}
...
function preAddPago(){
  ...
  if($("#Pag_Cod option:selected").attr("data-abr") === "EFE" || $("#Pag_Cod option:selected").attr("data-abr") === "OTR" || $("#Pag_Cod option:selected").attr("data-abr") === "DEP" || $("#Pag_Cod option:selected").attr("data-abr") === "ANT" || $("#Pag_Cod option:selected").attr("data-abr") === "CCC"|| $("#Pag_Cod option:selected").attr("data-abr") === "RC"){
    $.post( "", {getPagoCtaAjax:true,tipo:""+$("#Pag_Cod option:selected").attr("data-abr")}, function( responce ) {
        ...
        if($("#pagosGrid td:contains('CCC')").text().search("CCC")==-1 && $("#Pag_Cod option:selected").attr("data-abr") === "CCC"){
          if($("#lim_val_pago_cc").val() === "none"){
  ...
  if($("#Pag_Cod option:selected").attr("data-abr") === "TDC" || $("#Pag_Cod option:selected").attr("data-abr") === "TRF"){
    prm_array = [
      "H","pago","","",
      $("#Pag_Cod").val(),
      $("#Pag_Cod option:selected").attr("data-abr"),
...
function guardarPago(){
  ...
  if(parseFloat(tots.debe).toFixed(2) !== parseFloat(tots.haber).toFixed(2)){ return $.alert("El valor total de pagos debe ser de "+$("#Com_Val").val()); }
      ...
      if($("#tip_trans").val() === "mod"){
        ...
        for(let i=0; i< ids.length; i++){
          ...
          if(reg_pagofc.flag === 's'){
            obj_ccpp_mod[contador_fcsm]={Cpp_Cod:reg_pagofc.Cpp_Cod,Pag_Val:reg_pagofc.Abono};
            contador_fcsm++;
          }
        }
  ...
  $.post( "", {getPagsAbono:true,Com_Cod:row[0].Com_Cod,Prv_Cod:$("#agg_Prv_Cod").val()}, function( responce ) {
    if(responce['success']===true){
      for(let i=0; i < responce['data'].length ;i++){
        ...
        }else if(responce['data'][i].Pag_Abr === "CCC"){
...
$.fn.createInputDiario3=function(element,tipo){
    let jgrid=this, rowId=$(element).closest('tr.jqgrow').attr('id'), tip=jgrid.jqGrid('getCell',rowId,'Det_Tip');
    ...
    if(tip===tipo){
        $(element).on('change', function(){
          if($(this).val() != ''){
            ...
            $(this).val($.toFixed("0.00")); jgrid.updateGridDiario();
          }
