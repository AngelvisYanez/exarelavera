...
function validaNoIdentif(number) {
   ...
   if (dto === 0) {
      ...
      for (var i = 0; i < dto; i++) if (!isNaN(digitos[i])) {
         digitos[i] = digitos[i] * 1; acu = acu + 1;
      }
      if (acu === dto) {//CANTIDAD DE DIGITOS  10   13  
         var tipo = digitos[2];
         ...
         if (tipo === 7 || tipo === 8) {
            ...
            resp['tipo_abrev'] = tipo;//Se guarda el tipo de abr
         ...
         if (number.substring(0, 2) * 1 > 24) {
            resp['message'] = 'Los dos primeros d\u00EDgitos no pueden ser mayores a 24.'
         };
         if (dto === 13) {
            if (number.substring(10, 13) !== '001') {
               resp['message'] = 'Los tres \u00faltimos d\u00EDgitos no tienen el c\u00F3digo del RUC 001.'
            };
            if (tipo === 'PU' && number.substring(9, 13) !== '0001') {
               resp['message'] = 'El R.U.C. de la empresa del sector p\u00fablico debe terminar con 0001'
            };

         } else if ((tipo === 'PU' || tipo === 'PR')) {
            resp['message'] = 'El R.U.C. de las empresas ' + resp['tipo'] + 's deben tener 13 digitos!'
         };
...
function crearVariosIngresos() {
   ...
   var nuevoCliente = new Cliente('1111111111', 1, 'N', 'VARIOS INGRESOS', 'VARIOS INGRESOS', 'M', 0, '(NINGUNA)', '(NINGUNA)', '(NINGUNA)');
