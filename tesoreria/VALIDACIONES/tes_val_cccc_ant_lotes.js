...
function generarColumnas() {
    ...
    columnas.push({ name: "Cli_Cod", hidden: true });
    ...
    $("#searchGrid_mod").createGrid({
        ...
        subGridRowExpanded: function(subgrid_id, row_id) {
            ...
            $("#" + subgrid_table_id).createGrid({
                url: "?abonosDetAjax=" + ccc_data.Cli_Cod,
                datatype: "json",
                regional: 'es',
                height: 'auto',
                colModel: [
                    { label: '', name: 'indx', key: true, hidden: true },
                    { label: 'No. Venta', name: 'Vet_Num', width: 30, align: "left" },
...
function actualizarTotalesSGmod() {
    ...
    // Inicializar objeto para rangos dinámicos
    let rangos = {};
    ...
    for (let i = 0; i < ids.length; i++) {
        ...
        for (var k = 1; k <= numRangos; k++) {
            rangos['Rango' + k] += parseFloat(reg_pago['Rango' + k] || 0);
        }
    ...
    $('#searchGrid_mod').jqGrid('footerData', 'set', { Cliente: "TOTALES:", Asi_Val: $('#searchGrid_mod').jqGrid('getCol', 'Asi_Val', false, 'sum') });
    ...
    // Establecer totales de rangos dinámicos
    for (var k = 1; k <= numRangos; k++) {
...
// Reconstruir el grid cuando se agregan o eliminan rangos
