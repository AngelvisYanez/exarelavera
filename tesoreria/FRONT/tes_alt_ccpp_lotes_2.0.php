...
if (isset($savePago)) {
	...
	// Evitar comprobantes duplicados por doble guardado (mismo cheque Ban+Num)
	if (is_array($save_p)) {
		$chequesLote = array();
		foreach ($save_p as $pagoChk) {
			if (!isset($pagoChk['grid_tipp']) || $pagoChk['grid_tipp'] !== 'pago') {
				continue;
			}
			if (!isset($pagoChk['Pag_Abr']) || $pagoChk['Pag_Abr'] !== 'CHE') {
				continue;
			}
			$banChk = isset($pagoChk['Ban_Cod']) ? trim((string) $pagoChk['Ban_Cod']) : '';
			$numChk = isset($pagoChk['Che_Num']) ? trim((string) $pagoChk['Che_Num']) : '';
			if ($banChk === '' || $numChk === '') {
				$response['message'] = "Hay un cheque sin banco o n&uacute;mero. Verifique los pagos.";
				$obBD_con1->echoJson($response);
				exit();
			}
			$keyChk = $banChk . '|' . $numChk;
			if (isset($chequesLote[$keyChk])) {
				$response['message'] = "El cheque No. $numChk est&aacute; repetido en el lote. No se guard&oacute;.";
				$obBD_con1->echoJson($response);
				exit();
			}
			$chequesLote[$keyChk] = true;
			$num_Ches = $obBD_con1->getArrayConsulta(10, $banChk, $obBD_conexion);
			if (is_array($num_Ches)) {
				foreach ($num_Ches as $nch) {
					if ((string) $nch['Che_Num'] === $numChk) {
						$response['message'] = "El cheque No. $numChk ya fue emitido. No se guard&oacute; para evitar duplicar el pago.";
						$obBD_con1->echoJson($response);
						exit();
					}
				}
...
<head>
	...
	<script src="../VALIDACIONES/tes_val_alt_ccpp_lotes.js?a=5166"></script>
