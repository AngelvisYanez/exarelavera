                ...
                if ($op=="I"){					
                    if(isset($CtaPagar))
                        ...
                        foreach ($CtaPagar as $row2){
                            ...
                            $obBD_con1->grabarv_registros(sentencias_cccc(19,$obBD_con1->parametros($row2['Cpc_Cod'].'*'.$Pag_Cod.'*'.$ultimo.'*'.$Com_Fec.'*'.$row2['Pago'].'*'.$Com_Obs)),$obBD_conexion->conexion);
                            if($Pag_Cod*1==3){
                            ...
                            if(isset($detener)&&$detener==true) break;
                        }    
...
<div id="set1">
	...
	<tr class="BarraTitulo">
            <td colspan="2" height="10">&raquo; Cobro Por Lotes a Clientes</td>
        </tr>
        <tr>
            <td colspan="2" >
                <div id="comp1">
		...
		<LEGEND>
                    ...
                    <form id="provFormTemp">
                       ...
                       <div class="datasegmento">
                               ...
                               <input type="hidden" name="Cli_Cod" id="CliCodBus" value="" />
		...
		<LEGEND>
                  ...
                  <form id="formCompTemp" action="javascript:if($('#CliCodBus').val()===''){$.alert('Seleccione Cliente');} else{$('#list').Search('#formCompTemp','ajaxComprobante');}">
          ...
          <td colspan="2" align="left" valign="top"> 
            <div id="grillaComp" style="padding:10px;">   
                        ...
                        <div style="padding-top:10px;">                            
                            <button type="button" class="btn btn-primary start" onclick="$('#list').stopGridEdit(); if($('#list').jqGrid('getCol', 'Pago', false, 'sum')===0){$.alert('El valor del Pago es Inválido');updateSaldos($('#list'));}else{SelectFact();}" title="Gestionar el Pago Seleccionado"> <i class="icon-book icon-white"></i> <span>Cobrar</span></button>
                        </div>
                </div> 
                 ...
                 <script type="text/javascript"> 
                        ...
                        function setPagoCell(grid,row){
                            if(row.act==="Yes"){ 
                                ...
                                if(!(pago>0&&pago<=saldo))
                                    grid.jqGrid("setCell", row.Cpc_Cod, "Pago", row.Saldo);                                 
                                grid.jqGrid('editRow',row.Cpc_Cod);
                            } else grid.jqGrid("setCell", row.Cpc_Cod, "Pago", null);   
                        }
                        ...
                        function updateTotals(grid){ 
                            ...
                            grid.jqGrid("footerData", "set", {proveedor: "<div style='text-align:right;'>TOTAL A COBRAR:</div>",Pago:grid.jqGrid('getCol', 'Pago', false, 'sum')});        
                        }
                    ...
                    $(document).ready(function () {  
                        ...
                        compGrid.jqGrid({
                            ...
                            colModel: [                               
                                { label: 'C�d.Int.', name: 'Cpc_Cod', key: true, hidden:true,viewable:true },  
                                ...
                                { classes:'columnDisabled no_padding',label: 'A Cobrar', name: 'Pago', width: 45, align: 'right', formatter:'currency', decimalPlaces: '2', summaryRound: 2,
                        ...
                        compGrid.navGrid('#listPager',{ edit: false, add: false, del: false, search: false, refresh: true, view: false, position: "left", cloneToTop: false })
                            ...
                            .jqGrid('navButtonAdd',"#listPager",{ caption: "Exportar &nbsp;",buttonicon: "ui-icon-arrowthickstop-1-s",title:"Exportar Excel",
                                onClickButton: function() {
                                    compGrid.jqGrid('exportGridExcel',{nombre:"PorCobrarLOTES",hoja:"CCxCC"});	
                                },position: "last"
                            });
                    ...
                    function resetComp(){ 
                        ...
                        $('#ConEgreso').val(glosa);       
                        $('#ObsEgreso').val(glosa); 
                        $('#Cpc_Cod').val(fact[0]['Cpc_Cod']);
                        $('#lblProvee').val(fact[0]['proveedor']);
                        $('#cod_pvr').val(fact[0]['Cli_Cod']);  
                       
                        $('#lblMsg').html('Maximo: $ '+valor+'. Saldo  de '+fact.length+' Factura(s)');  
                        resetPago();
                        //addBanco();
                    }
                    function addBanco(){ 
                        ...
                        if(bancos.length>0&&(($('#pagos').val()*1)===2||($('#pagos').val()*1)===3))
                    ...
                    function addFilaCuenta(cuenta,tipo){                         
                        ...
                        cuenta['Haber']=valor; 
                        if(tipo==='S')
                        {cuenta['Haber']=cuenta['ValPago'];cuenta['Det_Tip']='H';}
                        var grid=$("#comp");
