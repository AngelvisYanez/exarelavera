...
/*
if(isset($anula)) {         
		$Che=$obBD_con1->getRowConsulta(52, $Dcc_Cod, $obBD_conexion); 
        $obBD_con1->inicio_transaccion($obBD_conexion->conexion);
        //	var_dump($Che['Che_Cod']);
        
        $obBD_con1->grabarv_registros(sentencias_cccc(18,$obBD_con1->parametros($Com)), $obBD_conexion->conexion);
		$obBD_con1->grabarv_registros(sentencias_cccc(50,$obBD_con1->parametros($Dcc_Cod)), $obBD_conexion->conexion);
		if(isset($Che['Che_Cod'])&&!empty($Che['Che_Cod']))
			$obBD_con1->grabarv_registros(sentencias_cccc(51,$obBD_con1->parametros($Che['Che_Cod'])), $obBD_conexion->conexion);
...
if (isset($anula)) {
    /*
    var clic2 = '$.createDialogConfirm("¿Est&aacute; seguro que desea anular el <b>cobro</b>?",
    {anula:true,Dcc_Cod:' + rowObject.Dcc_Cod + ',Cpc:' + rowObject.Cpc_Cod + ',Com:' + rowObject.Com_Cod + ',Che_Cod:' + rowObject.Che_Cod + '},anulaCobro);';                                                               
*/
    $response['success'] = false;
    // $obBD_con1->validaCierrePeriodo('det_ccpp_c', 'Cpc_Fec', 'Dcc_Cod', $Com_Fec, $Dcc_Cod, $obBD_conexion);
    ...
    $codigos_anticipos = $obBD_con1->getArrayConsulta(54, array('Dcc_Cod' => $Dcc_Cod), $obBD_conexion);
    if (count($codigos_anticipos) > 0) {
        foreach ($codigos_anticipos as $cod) {
          ...
          el estado es 'U' caso contrario 'A' */
            $anticipos_diferentes = $obBD_con1->getArrayConsulta(55, array('Ant_Cod' => $cod['Ant_Cod'], 'Dcc_Cod' => $Dcc_Cod), $obBD_conexion);
            if (count($anticipos_diferentes) > 0) {
                //Estado U en anticipo_clientes y pag_ant_cli
                $obBD_con1->operacionobBD(56, array('Ant_Cod' => $cod['Ant_Cod'], 'Ant_Est' => "U"), $obBD_conexion);
                $obBD_con1->operacionobBD(57, array('Ant_Cod' => $cod['Ant_Cod'], 'Pac_Est' => "U"), $obBD_conexion);
            } else {
                //Estado A en anticipo_clientes y pag_ant_cli
                $obBD_con1->operacionobBD(56, array('Ant_Cod' => $cod['Ant_Cod'], 'Ant_Est' => "A"), $obBD_conexion);
                $obBD_con1->operacionobBD(57, array('Ant_Cod' => $cod['Ant_Cod'], 'Pac_Est' => "A"), $obBD_conexion);
            }
            //Elimino el detalle de det_ant_cliente
            $obBD_con1->operacionobBD(58, array('Ant_Cod' => $cod['Ant_Cod'], 'Dcc_Cod' => $Dcc_Cod), $obBD_conexion);
        }
    }
    try {
        //cambiamos a estado 'I' los cheques del pago
        $obBD_con1->operacionobBD(59, array('Com_Cod' => $Com), $obBD_conexion);
        //cambiamos a estado 'I' el comprobante del pago
        $obBD_con1->operacionobBD(60, array('Com_Cod' => $Com), $obBD_conexion);
...
if (isset($ajaxComprobante)) {
    $responce['rows'] = $obBD_con1->getArrayConsulta(17, $Ses_Emp_Cod . '*' . $Cli_Cod . '*' . $Pec_Cod . '*' . $txt_fec_ini . '*' . $txt_fec_fin . '*', $obBD_conexion);
    if ($op_opciones != 'T')
...
<HEAD>
    ...
    <TITLE><?Php echo "Ccxcc Anular [EXA]"; ?></TITLE>
    <meta charset="UTF-8">
    ...
    <div id="set1">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table" style="table-layout:fixed;">
            <tr class="BarraTitulo">
                <td colspan="2" height="10">&raquo; Anulación de Cobros </td>
            </tr>
            <tr>
                <td colspan="2">
                    <div id="comp1">
                                        ...
                                        <script type="text/javascript">
                                            ...
                                            $(document).ready(function() {
                                                ...
                                                compGrid.jqGrid({
                                                    ...
                                                    //                                var rowIds = compGrid.getDataIDs();
                                                    //                                $.each(rowIds, function (index, rowId) {
                                                    //                                  compGrid.expandSubGridRow(rowId); 
                                                    //                                });
                                                    //                             },
                                                    subGridOptions: {
                                                            ...
                                                            colModel: [{
                                                                ...
                                                                },
                                                                {
                                                                    ...
                                                                    formatter: function(cellvalue, options, rowObject) {
                                                                        ...
                                                                        var clic2 = '$.createDialogConfirm("¿Est&aacute; seguro que desea anular el <b>cobro</b>?",{anula:true,Dcc_Cod:' + rowObject.Dcc_Cod + ',Cpc:' + rowObject.Cpc_Cod + ',Com:' + rowObject.Com_Cod + ',Che_Cod:' + rowObject.Che_Cod + '},anulaCobro);';
                                                                        return '<span class="btn btn-info btn-mini" title="Seleccionar" onclick=\'' + clic + '\'><i class="icon-info-sign icon-white"></i></span>&nbsp;<span class="btn btn-danger btn-mini" title="Anular Cobro" onclick=\'' + clic2 + '\'><i class="icon-trash icon-white"></i></span>';
                                                                    }
                                                                }
                                                            ],
                                                ...
                                                compGrid.navGrid('#listPager', {
                                                    ...
                                                    .jqGrid('navButtonAdd', "#listPager", {
                                                        caption: "Exportar &nbsp;",
                                                        buttonicon: "ui-icon-arrowthickstop-1-s",
                                                        title: "Exportar Excel",
                                                        onClickButton: function() {
                                                            compGrid.jqGrid('exportGridExcel', {
                                                                nombre: "CreditosOtorgados",
                                                                hoja: "CCCC"
                                                            });
                                                        },
                                                        position: "last"
