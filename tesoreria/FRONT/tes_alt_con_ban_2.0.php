    ...
    if (count($rs_bancos) > 0){ 
        foreach ($rs_bancos as $row){  
             ?><option value="<?php echo $row['Ban_Cod']; ?>"><?php echo $row['Pld_Des']." (Cta.#: ".$row['Ban_Cue'].")"; ?></option><?php
                                     ...
                                     <div class="col-xs-3" >
                                        <button type="button" class="btn btn-success btn-sm" title="Generar Concilici�n Bancaria" onclick="Search()"> <i class="glyphicon glyphicon-eye-open"></i> <span> Visualizar</span> </button>
                                     </div>   
                                </div>
                            </fieldset>   
                        </div>
