<?php
/**
 * Estado de Cuenta de Clientes (CCxCC)
 * Kardex financiero: facturas, pagos aplicados y saldo acumulado cronológico.
 * Filtro por cliente y rango de fechas. Exportación Excel. Totales al final.
 *
 * @package ccxcc
 */
require_once(__DIR__ . '/../../administrador/LOGICA/seguridad.php');
require_once(__DIR__ . '/../LOGICA/ccxcc_log_estado_cuenta.php');
require_once(__DIR__ . '/../../Librerias/procedimientos/almacenados_standar.php');

$obBD_conexion = new Class_Log_Conexion_Estado_Cuenta_Cliente($Ses_Dat_Dis);
$obBD_con1    = new Class_Log_Datos_Estado_Cuenta_Cliente();

// ----- AJAX: Listado de clientes (para selector / búsqueda) -----
if (isset($_REQUEST['clientesAjax'])) {
    $params = array('searchCli' => isset($_GET['searchCli']) ? $_GET['searchCli'] : '');
...
if (isset($_REQUEST['getEstadoCuentaCliente'])) {
    $Cli_Cod     = isset($_POST['Cli_Cod']) ? intval($_POST['Cli_Cod']) : 0;
    ...
    if ($Cli_Cod <= 0) {
        $response['message'] = 'Seleccione un cliente.';
...
if (isset($_REQUEST['exportExcelEstadoCuenta'])) {
    ...
    $txt_fec_ini = isset($_POST['txt_fec_ini']) ? $_POST['txt_fec_ini'] : date('Y-01-01');
    $txt_fec_fin = isset($_POST['txt_fec_fin']) ? $_POST['txt_fec_fin'] : date('Y-m-d');
    $nombre_cli = isset($_POST['nombre_cliente']) ? $_POST['nombre_cliente'] : '';
    ...
    $params = array('Cli_Cod' => $Cli_Cod, 'txt_fec_ini' => $txt_fec_ini, 'txt_fec_fin' => $txt_fec_fin);
    $rows = $obBD_con1->getArrayConsulta(2, $params, $obBD_conexion);
    if ($obBD_con1->Error != 0) {
        $response['message'] = $obBD_con1->MsgError;
        $obBD_con1->echoJson($response);
    ...
    $response['success'] = true;
    $response['html']    = $body;
    $response['titulo']  = 'Estado de Cuenta - ' . $nombre_cli . ' (' . $txt_fec_ini . ' / ' . $txt_fec_fin . ')';
    $obBD_con1->echoJson($response);
    exit;
}
...
<head>
    <meta charset="UTF-8">
    <title>Estado de Cuenta de Clientes - CCxCC</title>
    ...
    <script src="../VALIDACIONES/ccxcc_val_estado_cuenta.js"></script>
    <style>
...
<body>
    <div class="panel panel-main">
        <div class="panel-heading exa-header"><h3 class="panel-title">&raquo; Estado de Cuenta de Clientes</h3></div>
                            ...
                            <div class="form-group form-group-compact">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    <div class="input-group input-group-xs">
                                        <input name="Prs_Ced" id="Prs_Ced" type="text" placeholder="Seleccione cliente..." class="form-control input-xs" readonly />
                                        <span class="input-group-btn">
                                            <button type="button" class="btn btn-success btn-xs" onclick="$('#clientesDialog').dialog('open');" title="Seleccionar"><span class="glyphicon glyphicon-search"></span></button>
                                            <button type="button" class="btn btn-danger btn-xs" onclick="limpiarCliente();" title="Quitar"><span class="glyphicon glyphicon-remove"></span></button>
                                        </span>
                                    </div>
                                </div>
                            </div>
    ...
    <div id="clientesDialog" title="Búsqueda de Clientes"><form class="form-horizontal normal"></form></div>

    <!-- Contenedor oculto para exportar Excel -->
    <div id="exportar" style="display:none">
        <?php echo $obBD_con1->getReportHeader($Ses_Suc_Cod, 'Estado de Cuenta de Clientes', 'Kardex por cliente', $obBD_conexion, false, 10); ?>
