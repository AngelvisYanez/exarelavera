...
//                        }else 
                            $ultimo4=$Bene_Id;
                        $obBD_con1->grabarv_registros(sentencias_ccpp(25,$obBD_con1->parametros($ultimo4.'*'.$Ban_Cod.'*'.$ultimo2.'*'.$Num_Doc.'* *'.$Com_Val.'*'.$Com_Obs.'*'.$Che_Fec.'*1*'."$apellido $nombre")),$obBD_conexion->conexion);
                        $responce['cheque']="?codigo2=1&asi=$ultimo2&ban=$Ban_Cod&pro=$ultimo4";
                    }
                    $obBD_con1->grabarv_registros(sentencias_ccpp(19,$obBD_con1->parametros($Cpp_Cod.'*'.$Pag_Cod.'*'.$ultimo.'*'.$Com_Fec.'*'.$Com_Val.'*'.$Com_Obs)),$obBD_conexion->conexion);
                }
		/**
...
<div id="set1">
	...
	<tr class="BarraTitulo">
            <td colspan="2" height="10">&raquo; Pago de Abonos a Proveedores </td>
        </tr>
        <tr>
            <td colspan="2" >
                <div id="comp1">
          ...
          <td colspan="2" align="left" valign="top"> 
            <div id="grillaComp" style="padding:10px;">   
                 ...
                 <script type="text/javascript"> 
                            ...
                            colModel: [                               
                                ...
                                { label: 'Proveedor', name: 'proveedor', width: 75},
                                    { label:'&nbsp;', name: 'act1', width: 30, align: 'center',viewable: false,
                                        formatter:function (cellvalue, options, rowObject) {   
                                            var selectBut='<span class="btn btn-success btn-mini" title="Seleccionar" type="button" onclick="SelectFact($(\'#list\').jqGrid (\'getRowData\', \''+rowObject.Cpp_Cod+'\'))"><i class="icon-arrow-right icon-white"></i></span>';
                        ...
                        compGrid.navGrid('#listPager',{ edit: false, add: false, del: false, search: false, refresh: true, view: false, position: "left", cloneToTop: false })
                            .jqGrid('navButtonAdd',"#listPager",{ caption: "Exportar &nbsp;",buttonicon: "ui-icon-arrowthickstop-1-s",title:"Exportar Excel",
                                onClickButton: function() {
                                    compGrid.jqGrid('exportGridExcel',{nombre:"PorPagarABONOS",hoja:"CCPP"});	
                                },position: "last"
                            });
                    ...
                    function resetComp(){ 
                        $("#comp").clearGrid();                        
                        glosa='Abono Fact. '+fact['Cop_Num'];
                        valor=fact['Saldo'];
                        $('#Com_Val_Egre').val(valor);
                        $('#Com_Sal_Egre').val('$ 0.00');
                        $('#ConEgreso').val(glosa);       
                        $('#ObsEgreso').val(fact['Cop_Num']); 
                        $('#Cpp_Cod').val(fact['Cpp_Cod']);
                        $('#lblProvee').val(fact['proveedor']);
                        $('#cod_pvr').val(fact['Prv_Cod']);
                        $('#apellido').val(fact['Prs_Ape']);
                        $('#nombre').val(fact['Prs_Nom']);
                        $('#Bene_Id').val(fact['Prv_Cod']);
                        ...
                        <tr><td valign="top">
                            <div class="row">
                                ...
                                <div  class="datasegmento">
                                     <select name="Pec_Cod" id="periodos" onchange="loadBancos();" class="text medium ui-corner-all" required>   
                                        ...
                                        <?Php 
                                        $rs_periodos = $obBD_con1->getArrayConsulta(5,array(0=>$Ses_Emp_Cod), $obBD_conexion);                               
                            ...
                            <div class="row">
                                ...
                                <div  class="datasegmento" id="Benediv"><input id="Bene_Id" name="Bene_Id" type="hidden" />
                                    ...
                                    <button id="btnBene" onclick="$('#beneDialog').dialog('open');return false;" title="Seleccionar Beneficiario" style="display:none" class="btn btn-success btn-mini"><i class=" icon-check icon-white"></i></button>
                                </div>
                            </div>
                            <div class="row">
