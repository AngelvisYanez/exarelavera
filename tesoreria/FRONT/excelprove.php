    ...
    for ($i = 0; $i < 1; $i++) {
        ...
        foreach ($Reader as $Row) {
            ...
            if ($contador > 1) {
                ...
                $Prv_Esp = "";
                if (isset($Row[12])) {
                    $Prv_Esp = mysqli_real_escape_string($conn, $Row[12]);
                }
                ...
                if (!empty($cedula) && !empty($apellido)) {
                    ...
                    $persona = NULL;
                    $sqlExisteProveedore = " SELECT Prs_Cod FROM proveedore where Prs_Cod = " . $id . "  AND Emp_Cod=" . $_SESSION['Ses_Emp_Cod'];
                    $resultExisteProveedore = mysqli_query($conn, $sqlExisteProveedore);
                    while ($row = mysqli_fetch_array($resultExisteProveedore)) {
                        $persona = $row['Prs_Cod'];
                    }

                    $ingresoProveedore = false;
                    if ($persona == null) {
                        //Se ingresa el proveedore relacionado con la persona
                        $queryProveedore = "insert into proveedore(Prs_Cod,Prv_Tic,Prv_Con,Prv_Esp,Emp_Cod) values(" . $id . ",'" . $esp_contribuyente . "','" . $contabilidad . "', '" . $Prv_Esp . "'," . $_SESSION['Ses_Emp_Cod'] . ")";
                        $resultProveedore = mysqli_query($conn, $queryProveedore);
                        $ingresoProveedore = true;
                    } else {
                        $filas2 = 2;
                        $Ingresadas = $Ingresadas . $contador . ",";
                    }

                    if ($ingresoProveedore == false) {
                        $filas = 1;
...
<body>
    <div class="panel panel-main">
        <div class="panel-heading exa-header">
            <h3 class="panel-title">&raquo; Importar proveedore</h3>
        </div>
        ...
        <div class="row">
            <div class=" col-xs-6" style="margin-left: 10px; margin-top: 10px;">
                ...
                <div class="text-center mt-2" style="margin-bottom: 10px;">
                    <a href="formato.xlsm" download="formatoProveedor.xlsm">
