	...
	if (isset($data['Pec_Cod']) && $data['Pec_Cod'] === 'Corte') {
		...
		if (trim($data['letra']) == 'Activos') {
			if ($data['Pec_Cod'] === 'Corte') {
				$datos = array_merge($_GET, array('setWhere' => array('Corte', 'pagos-corte', 'pagoAnticipo', 'getUsuario', 'getDetAntCCCC')));
			} else {
		...
		if (trim($data['letra']) == 'Anulados') {
			$datos = array_merge($_GET, array('setWhere' => array('isInactive', 'pagos-corte', 'pagoAnticipo', 'getUsuario', 'getDetAntCCCC')));
		}
	} else {
		if (trim($data['letra']) == 'Activos') {
			if ($data['Pec_Cod'] === 'Corte') {
...
<HEAD>
	...
	<TITLE><?php echo "Fecha Corte Ant.Cliente [EXA]"; ?></TITLE>
	<meta charset="UTF-8">
...
<BODY>
	<div class="panel panel-main">
		<div class="panel-heading exa-header">
			<h3 class="panel-title">&raquo;Anticipos a Clientes Fecha Corte</h3>
		</div>
	...
	<script src="../VALIDACIONES/tes_val_anticipo_cli_corte.js?k=1"></script>
