...
if (isset($deudor)) {
    $responce['rows'] =  $obBD_con1->getArrayConsulta(45, $Pla_Cod, $obBD_conexion);
    utf8_encode_deep($responce['rows']);
    echo json_encode($responce);
    exit();
}
if (isset($acreedor)) {
    $responce['rows'] =  $obBD_con1->getArrayConsulta(46, $Pla_Cod, $obBD_conexion);
    utf8_encode_deep($responce['rows']);
    echo json_encode($responce);
    exit();
}
if (isset($addCuenta)) {
    ...
    if ($addCuenta === 'Deudor')
        $obBD_con1->operacionobBD(47, $Pld_Cod, $obBD_conexion);
    else
        $obBD_con1->operacionobBD(48, $Pld_Cod, $obBD_conexion);
    $obBD_con1->fin_transaccion_nomsn($obBD_conexion->conexion);
    if ($obBD_con1->Error == 0) {
        $responce['success'] = true;
    } else {
        $responce = array(success => false, message => 'No se pudo guardar el parametro!', error => $obBD_con1->MsgError);
    }
    echo json_encode($responce);
    exit();
}
$periodos = $obBD_con1->getArrayConsulta(39, $Ses_Emp_Cod, $obBD_conexion);
$periodo = $periodos[0];
?>
<!DOCTYPE html>
<HTML>
...
<HEAD>
    ...
    <TITLE><?Php echo "Parametrización Prov. Cli. [EXA]"; ?></TITLE>
    <meta charset="UTF-8">
...
<BODY>
    ...
    <div class="panel panel-main">
        <div class="panel-heading exa-header">
            <h3 class="panel-title">&raquo; Parametrización de Cuentas Proveedores/Clientes (Periodo <span id="anio"><?php echo $periodo['Pla_Fec'] ?></span>)</h3>
        </div>
            ...
            <div class="row">
                            ...
                            <div class="col-sm-7">
                                <select id="selectPecCod" class="form-control input-sm">
                                    <?php foreach ($periodos as $row) {
                                        echo "<option value='$row[Pec_Cod]' data-placod='$row[Pla_Cod]' data-peccod='$row[Pec_Cod]' data-anio='$row[Pla_Fec]'>Periodo $row[Pla_Fec]</option>";
                                    } ?>
                                </select>
                            </div>
                        </div>
                ...
                <div class="col-sm-6">
                    <div style="min-height: 250px;">
                        ...
                        <button onclick="tipo='Deudor';$('#cuenDialog').dialog('open');" title="Buscar Cuentas" type="button" class="btn btn-success btn-sm" style="margin-top: 10px;"><i class="glyphicon glyphicon-check"></i><span> Agregar Cuenta</span></button>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div style="min-height: 250px;">
                        ...
                        <button onclick="tipo='Acreedor';$('#cuenDialog').dialog('open');" title="Buscar Cuentas" type="button" class="btn btn-success btn-sm" style="margin-top: 10px;"><i class="glyphicon glyphicon-check"></i><span> Agregar Cuenta</span></button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        function addCuenta(a2) {
            ...
            if ((tipo === 'Deudor' && $("#list_1").existsId(a2)) || (tipo === 'Acreedor' && $("#list_2").existsId(a2))) {
            ...
            $.saveDataJson("<?Php echo htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'); ?>", {
                ...
                function(r) { // success
                    $("#list_" + (r['tipo'] === 'Deudor' ? '1' : '2')).jqGrid().trigger("reloadGrid", [{
                        page: 1
                    }]);
                }
            );
        ...
        $(document).ready(function() {
            $("#list_1").createGrid({
                postData: $("#formDeudas").getData("deudor"),
                height: 200,
                rowNum: 10000000,
                pgbuttons: false,
                pgtext: null,
                ...
                colModel: [{
                    ...
                    },
                    {
                        ...
                        viewable: false,
                        formatter: function(cellvalue, options, rowObject) {
                            return $.getGridButton(deleteCuenta, {
                                Pld_Cod: rowObject.Pld_Cod,
                                tipo: 'Deudor'
                            }, 'Eliminar', 'remove', null, 'danger');
                        }
                    }
                ]
            }, false, "#listPager_1", {
            ...
            $("#list_2").createGrid({
                postData: $("#formDeudas").getData("acreedor"),
                height: 200,
                rowNum: 10000000,
                pgbuttons: false,
                pgtext: null,
            ...
            $('#selectPecCod').on('change', function() {
                ...
                $('#anio').html(data['anio']);
                $("#list_1").Search("#formDeudas", "deudor");
                $("#list_2").Search("#formDeudas", "acreedor");
                $.Search('cuen');
            });
        });
    </script>
