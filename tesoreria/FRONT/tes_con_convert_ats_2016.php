                ...
                if($explode_name[1] == 'xml'||$explode_name[1] == 'XML'){
                    ...
                    {   $z=$z+1;                                           
                        ...
                        if(count($datosCom->detalleCompras[$x]->baseImpExe)==0){
						 ...
						 if($anio>='2016' && $mes>='01' && count($datosCom->detalleCompras[$x]->valRetServ50)==0){
                            $insert = new SimpleXMLElement("<valRetServ50>0.00</valRetServ50>");    
                            $target = $datosCom->detalleCompras[$x]->valorRetBienes;
                            if(count($target)==1)
                                simplexml_insert_after($insert, $target);
                        }
                    ...
                    {   $z=$z+1;                       
                        ...
                        if(($datosVen->detalleVentas[$x]->tpIdCliente.'') != '07'){
                            if($fecha>='2016-01'){
								if(count($datosVen->detalleVentas[$x]->parteRelVtas)==0){                                									  
									if($fecha>='2016-05' && ($datosVen->detalleVentas[$x]->tpIdCliente.'') == '06' && count($datosVen->detalleVentas[$x]->tipoCliente)==0){
										$insert = new SimpleXMLElement("<parteRelVtas>NO</parteRelVtas>");
										$target = $datosVen->detalleVentas[$x]->tpIdCliente;									
										simplexml_insert_after($insert, $target);
									
										$insert = new SimpleXMLElement("<tipoCliente>01</tipoCliente>");
										$target = $datosVen->detalleVentas[$x]->parteRelVtas;									
										simplexml_insert_after($insert, $target);
																				
										$insert = new SimpleXMLElement("<denoCli>NINGUNO</denoCli>");
										$target = $datosVen->detalleVentas[$x]->tipoCliente;									
										simplexml_insert_after($insert, $target);
									}else{
										$insert = new SimpleXMLElement("<parteRelVtas>NO</parteRelVtas>");  
                        ...
                        if(count($datosVen->detalleVentas[$x]->tipoEmision)==0){
                            $insert = new SimpleXMLElement("<tipoEmision>F</tipoEmision>");    
                            $target = $datosVen->detalleVentas[$x]->tipoComprobante;
                            if(count($target)==1)
                                simplexml_insert_after($insert, $target);
                        }  
						if($fecha>='2016-06' && count($datosVen->detalleVentas[$x]->formasDePago)==0){
                            $insert = new SimpleXMLElement("<formasDePago><formaPago>01</formaPago></formasDePago>");    
                            $target = $datosVen->detalleVentas[$x]->valorRetRenta;
                            if(count($target)=='1' && $datosVen->detalleVentas[$x]->tipoComprobante!='04')
                                simplexml_insert_after($insert, $target);
                        }  												
						if($fecha>='2015-03' && count($datosVen->detalleVentas[$x]->montoIce)==0){
                            $insert = new SimpleXMLElement("<montoIce>0.00</montoIce>");    
                            $target = $datosVen->detalleVentas[$x]->montoIva;
                            if(count($target)==1)
                                simplexml_insert_after($insert, $target);
                        }  
...
<tr class="BarraTitulo">
	  <td height="10">&raquo; Aplicar formato ATS 2016</td>
</tr>
<tr>
