...
if (isset($anula)) {
    ...
    $obBD_con1->validaCierrePeriodo('det_ccpp_p', 'Pag_Fec', 'Cpp_Cod', $Fec, null, $obBD_conexion);

    $obBD_con1->inicio_transaccion($obBD_conexion->conexion);
    //$obBD_con1->grabarv_registros(sentencias_cccc(341,$obBD_con1->parametros($Cpc.'*'.$Com)), $obBD_conexion->conexion);
    /*  $obBD_con1->grabarv_registros(sentencias_ccpp(31, $obBD_con1->parametros($Com)), $obBD_conexion->conexion); //Anular comprobante
    $obBD_con1->grabarv_registros(sentencias_ccpp(33, $obBD_con1->parametros($Com)), $obBD_conexion->conexion); //Anular cheques*/
    $obBD_con1->operacionobBD(67, array('Com_Cod' => $Com), $obBD_conexion);

    $anticipos_vinculados = $obBD_con1->getArrayConsulta(62, array('Com_Cod' => $Com), $obBD_conexion);
    ...
    if (count($anticipos_vinculados) > 0) {
        foreach ($anticipos_vinculados as $cod) {
            ...
            $anticipos_diferentes = $obBD_con1->getArrayConsulta(63, array('Atp_Cod' => $cod['Atp_Cod'], 'Com_Cod' => $Com), $obBD_conexion);
            ...
            if (count($anticipos_diferentes) > 0) {
                ...
                $obBD_con1->operacionobBD(65, array('Atp_Cod' => $cod['Atp_Cod'], 'Atp_Est' => 'U'), $obBD_conexion);
                $obBD_con1->operacionobBD(64, array('Atp_Cod' => $cod['Atp_Cod'], 'Pap_Est' => 'U'), $obBD_conexion);
            } else {
                //Estado A en anticipo_proveedores y pag_ant_prov
                $obBD_con1->operacionobBD(65, array('Atp_Cod' => $cod['Atp_Cod'], 'Atp_Est' => 'A'), $obBD_conexion);
                $obBD_con1->operacionobBD(64, array('Atp_Cod' => $cod['Atp_Cod'], 'Pap_Est' => 'A'), $obBD_conexion);
            }
        }
    }
    //3) Elimino el det_ant_ccpp
    $obBD_con1->operacionobBD(66, array('Com_Cod' => $Com), $obBD_conexion);
...
if (isset($ajaxComprobante)) {
    $responce['rows'] = $obBD_con1->getArrayConsulta(61, $Ses_Emp_Cod . '*' . $Prv_Cod . '*' . $Pec_Cod . '*' . $txt_fec_ini . '*' . $txt_fec_fin . '*', $obBD_conexion);
    if ($op_opciones != 'T')
...
<HEAD>
    ...
    <TITLE><?Php echo "Ccxpp Anular [EXA] "; ?></TITLE>
    <meta charset="UTF-8">
    ...
    <div id="set1">
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table" style="table-layout:fixed;">
            <tr class="BarraTitulo">
                <td colspan="2" height="10">&raquo; Pagos Realizados </td>
            </tr>
            <tr>
                <td colspan="2">
                    <div id="comp1">
                                ...
                                <td colspan="2" height="389" align="left" valign="top">
                                    <FIELDSET>
                                        ...
                                        <script type="text/javascript">
                                            ...
                                            function setCaption() {
                                                var caption = '';
                                                caption = "Historial de Pagos Realizados - ";
                                            ...
                                            $(document).ready(function() {
                                                ...
                                                compGrid.jqGrid({
                                                    ...
                                                    mtype: "GET",
                                                    datatype: "local",
                                                    regional: 'es', //ajaxRowOptions: { async: true },//postData: $("#form1").getData("ajaxGrid"),
                                                    autowidth: true,
                                                    shrinkToFit: true,
                                                    height: 270,
                                                    caption: ' ',
                                                            ...
                                                            colModel: [{
                                                                ...
                                                                },
                                                                {
                                                                    ...
                                                                    formatter: function(cellvalue, options, rowObject) {
                                                                        ...
                                                                        var clic2 = '$.createDialogConfirm("¿Est&aacute; seguro que desea anular el <b>pago</b>?",{anula:true,Cpp:' + rowObject.Cpp_Cod + ',Com:' + rowObject.Com_Cod + ',Fec:"' + rowObject.Pag_Fec + '"},anulaPago);';
                                                                        return '<span class="btn btn-info btn-mini" title="Seleccionar" onclick=\'' + clic + '\'><i class="icon-info-sign icon-white"></i></span>&nbsp;<span class="btn btn-danger btn-mini" title="Anular Pago" onclick=\'' + clic2 + '\'><i class="icon-trash icon-white"></i></span>';
                                                                    }
                                                                }
                                                            ],
                                            ...
                                            function anulaPago(data) {
                                                $.post("<?Php echo filter_input(INPUT_SERVER, 'PHP_SELF', FILTER_SANITIZE_STRING); ?>", data, function(response) {
                                                    if (response['success'] === true) {
                                                        $.alert("El Pago se ha Anulado con Exito!");
