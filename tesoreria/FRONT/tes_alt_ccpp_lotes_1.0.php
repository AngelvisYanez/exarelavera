...
//echo '<a href="'.baseUrl("../../contabilidad/FRONT/con_pri_compr_1.1.php?codigo=$ultimo&tabla=$tabla&campo=$campo&tipo=$Tia_Cod&Pec_Cod=$Pec_Cod").'">ver</a>';
...
//}
if(isset($cuenAjax)){ 
    $obBD_con1->getPageGridJson(12, $search.'*'.$Ses_Emp_Cod.'*'.$Pec_Cod.'*'.$op_opciones, $obBD_conexion, $page, $rows);
}
if(isset($bancosAjax)){ 
...
if(isset($ajaxComprobante)){   
    ...
    if(!isset($txt_fec_ini)) $txt_fec_ini='';    if(!isset($txt_fec_fin)) $txt_fec_fin='';    
...
if(isset($save)){	
    ...
    $rs_buscar =  $obBD_con1->getArrayConsulta(20, (isset($Num_Doc)?$Num_Doc:'')."*".$Ses_Emp_Cod.'*'.(isset($Pld_Cod)?$Pld_Cod:'').'*'.$var, $obBD_conexion);
...
//		$acum=$Com_Val;
		...
		*/
                $Num_Doc2=isset($Num_Doc)?$Num_Doc:'';
                if ($op=="E"){$Num_Doc2='';}
		...
		$obBD_con1->grabarv_registros(sentencias_ccpp(21,$obBD_con1->parametros($Pec_Cod.'*'.$Codigo.'*'.$Com_Num.'*'.$Com_Fec.'*'.$Com_Con.'*'.$Tia_Cod
											.'*'.$Com_Val.'*'.$Com_Obs.'*'.$op.'*'.$campo.'*'.$Num_Doc2)),$obBD_conexion);
		...
		*/
                foreach ($save as $row)
                {
                    ...
                    $obBD_con1->grabarv_registros(sentencias_ccpp(22,$obBD_con1->parametros($ultimo.'*'.$row['Det_Tip'].'*'.$valor.'*'.$row['Pld_Des'].'*'.$row['Glosa'].'*'.
											$row['Pld_Cod'])),$obBD_conexion);
                    if(isset($Pld_Cod)&&$row['Pld_Cod']==$Pld_Cod){
                        $ultimo2 = $obBD_con1->insercionid ($obBD_conexion);
                    }
                }
                if ($op=="E"){
...
//                        }else 
                            $ultimo4=$Bene_Id;
                        $obBD_con1->grabarv_registros(sentencias_ccpp(25,$obBD_con1->parametros($ultimo4.'*'.$Ban_Cod.'*'.$ultimo2.'*'.$Num_Doc.'* *'.$Com_Val.'*'.$Com_Obs.'*'.$Che_Fec.'*1*'."$apellido $nombre")),$obBD_conexion);
...
//                            else {$row2['Pago']=$acum;$detener=true;}
                                $obBD_con1->grabarv_registros(sentencias_ccpp(19,$obBD_con1->parametros($row2['Cpp_Cod'].'*'.$Pag_Cod.'*'.$ultimo.'*'.$Com_Fec.'*'.$row2['Pago'].'*'.'Abono a Doc. '.$row2['Cop_Num'])),$obBD_conexion);
//                            if($detener) break;
                        } 
                }
		/**
...
<HTML>
	<HEAD>
		...
		<?Php require_once("../../mascaras/model1/estilos/basic.php"); ?>
                ...
                <style>
                    #list .no_padding{padding: 0 !important;}
                    #list .no_padding input[type="number"]{height: 23px;font-size: 14px;font-weight: bold; -moz-appearance:textfield !important;}
                    #list .no_padding input[type="number"]::-webkit-outer-spin-button,
                    #list .no_padding input[type="number"]::-webkit-inner-spin-button {
                        -webkit-appearance: none !important;
                        margin: 0 !important;
                    }
                    .jqgrow .columnDisabled {
...
<div id="set1">
	...
	<tr class="BarraTitulo">
            <td colspan="2" height="10">&raquo; Cancelaci�n Por Lotes a Prooveedores</td>
        </tr>
        <tr>
            <td colspan="2" >
                <div id="comp1">
		...
		<LEGEND>
                    ...
                    <form id="provFormTemp">
                       ...
                       <div class="datasegmento">
                               ...
                               <input id="docu" name="search" maxlength="13" onkeydown='if (event.keyCode === 13) $.SearchOrDialog("#provDialog",selectProvee);' type="text" class="search ui-corner-all" placeholder="Ingrese Cedula/R.U.C. ..." title="Buscar Proveedor Por Documento o Descripci�n" autofocus />                               
                               <input type="text" name="op_opciones" value="c" style="display: none;" /> 
                               <input type="hidden" name="Prv_Cod" id="PrvCodBus" value="" />
                               <a onclick="$('#provDialog').dialog('open');/*$('#docu').removeAttr('readOnly');*/" title="B�squeda de Proveedores"  class="btn btn-success btn-mini"><i class=" icon-check icon-white" ></i></a>                               
                  ...
                  <form id="formCompTemp" action="javascript:if($('#PrvCodBus').val()===''){$.alert('Seleccione Proveedor');} else{$('#list').Search('#formCompTemp','ajaxComprobante');}">
                    ...
                    <div>
                        ...
                        <div class="datasegmento" style="text-align: left;" >
                            ...
                            <button type="button" onclick="this.form.submit()" class="btn btn-success" style="height: 27px;" title="Ejecutar B�squeda" >
          ...
          <td colspan="2" align="left" valign="top"> 
            <div id="grillaComp" style="padding:10px;">   
                        ...
                        <div style="padding-top:10px;">                            
                            <button type="button" class="btn btn-primary start" onclick="$('#list').stopGridEdit();if($('#list').jqGrid('getCol', 'Pago', false, 'sum')===0){$.alert('El valor del Pago es Inválido');updateSaldos($('#list'));}else{SelectFact();}" title="Gestionar el Pago Seleccionado"> <i class="icon-book icon-white"></i> <span>Pagar</span></button>
                        </div>
                </div> 
                 ...
                 <script type="text/javascript"> 
                        ...
                        function setPagoCellTotal(grid){
                            var pagoTotal=0;
                            $('input[id$="_Pago"]').each(function(){ pagoTotal+=this.value*1; });  
                            grid.jqGrid("footerData", "set", {'Pago':pagoTotal}); 
                        }
                        function setPagoCell(grid,row){
                            if(row.act==="Yes"){ 
                                var pago=parseFloat('0'+row.Pago),saldo=parseFloat(row.Saldo);                                        
                                if(!(pago>0&&pago<=saldo))
                                    grid.jqGrid("setCell", row.Cpp_Cod, "Pago", row.Saldo);                                 
                                grid.jqGrid('editRow',row.Cpp_Cod);
                            } else grid.jqGrid("setCell", row.Cpp_Cod, "Pago", null);   
                        }
                        function updateSaldos(grid,id){
                            if(typeof id!=='undefined'){
                                grid.jqGrid('saveRow', id, false, 'clientArray');
                                var row= grid.jqGrid('getRowData',id);
                                setPagoCell(grid,row);
                                setPagoCellTotal(grid);
                                return;
                            }
                            grid.stopGridEdit();                             
                            var rows= grid.jqGrid('getRowData'), aux=$.jgrid.inlineEdit; 
                            $.jgrid.inlineEdit={focusField:false};
                            for(var i=0;i<rows.length;i++){                                
                                setPagoCell(grid,rows[i]);
                        ...
                        function updatePagoTotal(grid){ 
                            
                        }
                        function validaVal(id,val){                            
                            var pos=id.split('_'),
                                GridF=$("#list"),
                                saldo=parseFloat(GridF.jqGrid('getCell',pos[0],'Saldo'));
                                val=parseFloat(val);
                            if(val>0&&val<=saldo)
                                $('#'+id).val(val.toFixed(2)); 
                            else
                                $('#'+id).val(saldo.toFixed(2));                            
                            setPagoCellTotal(GridF);
                        }
                    $(document).ready(function () {  
                        ...
                        compGrid.jqGrid({
                            ...
                            colModel: [                               
                                { label: 'C�d.Int.', name: 'Cpp_Cod', key: true, hidden:true,viewable:true },  
                                { label: 'C�d.Int.', name: 'Asi_Cod',  hidden:true },                                  
                                ...
                                { classes:'columnDisabled no_padding',label: 'A Pagar', name: 'Pago', width: 45, align: 'right', formatter:'currency', decimalPlaces: '2', summaryRound: 2,
                                        formatoptions: {prefix:'$ ', thousandsSeparator:',',decimalSeparator:'.',defaultValue:'0.00'},
                                        summaryTpl: "Total: {0}",summaryType: "sum", // set the formula to calculate the summary type 
                                        editable:true,edittype:'text',editoptions:{type:'number',style:'text-align: right;',min:0,onchange:"validaVal($(this).attr('id'),this.value);"}
                                }    
                            ...
                            loadComplete: function (data) { 
                                ...
                                for (i = 0; i < c; i += 1) {                                    
                                    $(rows[i].cells[iCol]).click(function (e) {                                         
                                        updateSaldos(grid,$(this).parent().attr('id'));//updateTotals(grid);    
                                    });
                                }  
                            ...
                            subGridRowExpanded: function(subgrid_id, row_id) {
                                ...
                                $("#"+subgrid_table_id).jqGrid({
                                        ...
                                        colModel: [
                                                ...
                                                {label:'Observaci�n',name:"Pag_Obs",width:100},
                        ...
                        <tr><td valign="top">
                            ...
                            <div class="row">
                                ...
                                <div  class="datasegmento" id="Benediv"><input id="Bene_Id" name="Bene_Id" type="hidden" />
                                    <input id="apellido" name="apellido" class="text medium ui-corner-all clearable x onX" type="text" size="32" placeholder="Apellidos" style="width:40%;text-transform:uppercase" /><input  id="nombre" name="nombre" class="text medium ui-corner-all clearable x onX" type="text" size="32" placeholder="Nombres" style="width:40%;text-transform:uppercase" />
...
<!--                                    <button id="btnBene" onclick="$('#beneDialog').dialog('open');return false;" title="Seleccionar Beneficiario" class="btn btn-success btn-mini" type="button"><i class=" icon-check icon-white"></i></button>-->
                                </div>
                            </div>
                            <div class="row">
                                <div class="segmento required">Valor:</div>
                                <div  class="datasegmento"><input  class="money text small ui-corner-all" name="Com_Val" id="Com_Val_Egre" onchange=" updateValores()" type="text" size="10" maxlength="12" style="text-align:right" onkeypress="return  validar_decimal(event)" readonly="" /><img src="../../mascaras/model1/imagenes/32x32/info.gif" class="imgMsg" /><label id="lblMsg" class="lblMsg" style="color:blue;"></label></div>
                            </div>               
                        </td></tr>
                        </table> 
                ...
                <div colspan="2" style="padding:10px;">
                    <button onclick="updateSaldos($('#list'));$('#comp2').prev('#comp1');" class="btn btn-inverse fileinput-button" title="Volver Atrás"><i class=" icon-arrow-left icon-white"></i><span>&nbsp;&nbsp;Atr&aacute;s&nbsp;&nbsp;</span></button><span style="width: 150px;"></span>
                ...
                $(document).ready(function () {  
                        ...
                        gridComp.jqGrid({
                            ...
                            colModel: [
                                { label: 'C�d.Int.', name: 'Pld_Cod', key: true, width: 15,align:"center", hidden:true },  
...
<!--INICIO DEL DIALOGO BUSCAR PROVEEDOR--> 
    <div id="provDialog" title="B�squeda de Proveedores">  
       <form> 
        <fieldset>
		<legend><label class="Titulos2">B�squeda de Proveedor</label></legend>
		<table border="0">
                   <tr>
			  ...
			  <td width="266"><input name="op_opciones" type="radio" value="c" onclick="setfocus(this.form.search)" alt="" />
				<span class="LetraNegra"><strong>C�dula/R.U.C.</strong></span></td>
                    </tr>
                </table>
            ...
            <ul>
              ...
              <li><a href="#chequeComp">Cheques</a></li>              
                  ...
                  <div id="detalleComp" style="clear: both;">
                        ...
                        <div class="row">
                            <div class="segmento">Observaci�n:</div><div  class="datasegmento"><input id="lblObsV2" type="text" class="label ui-widget-content ui-corner-all" readonly /></div>
                        </div>
                        <div class="row">
        ...
        $(document).ready(function() {
                ...
                $('#asiento').jqGrid({
                 ...
                 $('#cheque').jqGrid({
                    ...
                    cmTemplate: {sortable:false},colModel: [
                        { label: 'C�d.Int.', name: 'Che_Cod', key: true,hidden:true,viewable: true },                                
...
<!--INICIO DEL DIALOGO IMPRIMIR --> 
    <div id="successDialog"  title="Mensaje del Sistema">  
        ...
        <center> 
            ...
            <a id="impCompr" target="_blank" href=""  style="display: inline;" title="Imprimir Comprobante"><span  class="btn btn-primary start"> <i class="icon-print icon-white"></i> <span>Imprimir Comprobante</span></span> </a>
...
<script>
	...
	$.vv=function(v){return (v!==null && !$.isUnd(v));};
