<?php 
require_once(dirname(__file__)."/../DATA/libs/AbstractModel.php");
class manifiesto_vehiculo extends AbstractModel{
    protected $_name = 'manifiesto_vehiculo'; 
    protected $_primary = array('Cho_Cod','Pla_Cod');	
    protected $_state = '';
    //protected $_fields = array(); // se declara las filas que va a tener el arreglo en caso de insert o update //opcional
    
    /* crea una sql basica global para la tabla */
    public function _selectBasic($cond=null,$limits=false){         
       return $this->select(true,array('*'))
            ->join('vehiculo',"vehiculo.Veh_Cod = $this->_name.Veh_Cod", array('*'))
            ->joinLeft('manifiesto_transporte', "manifiesto_transporte.Mat_Cod = vehiculo.Mat_Cod", array('Mat_Des'))
            ->joinLeft('manifiesto_plantas', "manifiesto_plantas.Pla_Cod = $this->_name.Pla_Cod", array('Pla_Nom', 'Pla_Lic'))
            ->joinLeft('cliente', "cliente.Cli_Cod = manifiesto_plantas.Cli_Cod", array())
            ->joinLeft('persona', "persona.Prs_Cod = cliente.Prs_Cod", array('Prs_Ced'));             
    }
    /* crea una sql standart para jqgrid, condiciones incluidas */
    public function _selectBasicGrid($cond=null,$limits=false){ 
        $sel=$this->_selectBasic();        
        $this->sqlByNombre("setEmpCod", $sel);
         
        // Aplicar filtros si existen
        if (is_array($cond) && !empty($cond['op_opciones']) && !empty($cond['search'])) {
            $searchTerm = addslashes($cond['search']);
            if ($cond['op_opciones'] == "p") {
                // Búsqueda por placa
                $sel->where("vehiculo.Veh_Pla LIKE ?", "%$searchTerm%");
            } else if ($cond['op_opciones'] == "pn") {
                // Búsqueda por nombre de la planta
                $sel->where("manifiesto_plantas.Pla_Nom LIKE ?", "%$searchTerm%");
            } else if ($cond['op_opciones'] == "pl") {
                // Búsqueda por licencia de la planta
                $sel->where("manifiesto_plantas.Pla_Lic LIKE ?", "%$searchTerm%");
            } else if ($cond['op_opciones'] == "c") {
                // Búsqueda por cédula/RUC del cliente
                $sel->where("persona.Prs_Ced LIKE ?", "%$searchTerm%");
            }
        }
        
        return $sel; 
    }
    /* formatea el array para insert o update */
    public function formatData($data, $type, $allData=null){ 
        return ($type=='I')?$data:$data;
    }
    /* crea sentencia por id nombre sql */
    public function sqlByNombre($id,$Par_Sql,$cond=null){
        if(is_object($Par_Sql)){ $sql=$Par_Sql; $Par_Sql=$cond; }else $sql='';
        switch($id){
            case "setEmpCod":
                $sql->where("vehiculo.Emp_Cod=?",$_SESSION['Ses_Emp_Cod']);
                //echo $this->getSqlString($sql)."<br/>";
                break;
            case "":
                $sql="";
                //echo $this->getSqlString($sql)."<br/>";
                break;                        
            default: throw new Exception ("No existe la sql denominada $id!");
        }
        //echo $this->getSqlString($sql)."<br/>";
        return $sql;
    }
    /* crea sentencia por id numero sql */
    public function sqlByNumero($id,$Par_Sql,$cond=null){
        if(is_object($Par_Sql)){ $sql=$Par_Sql; $Par_Sql=$cond; }else $sql='';
        switch($id){
            case 0:
                $sql="";
                //echo $this->getSqlString($sql)."<br/>";
                break;
            default: throw new Exception ("No existe la sql numero $id!");
        }
        //echo $this->getSqlString($sql)."<br/>";
        return $sql;
    }
}