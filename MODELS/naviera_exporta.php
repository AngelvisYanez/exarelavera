...
class naviera_exporta extends AbstractModel{
    protected $_name = 'naviera_exporta'; 
    protected $_primary = array('Nav_Cod');	
    protected $_state = 'Nav_Est';
    ...
    public function _selectBasicGrid($cond=null,$limits=false){ 
        ...
        if(isset($cond['Nav_Tip'])&&!empty($cond['Nav_Tip'])) 
             $sel->where("$this->_name.Nav_Tip=?",$cond['Nav_Tip']);
        if(isset($cond['search'])&&!empty($cond['search'])) 
            $sel->where("UPPER($this->_name.Nav_Nom) LIKE UPPER(?)", "%{$cond['search']}%");        
        return $sel; 
    }
