...
class chofer extends AbstractModel{
    protected $_name = 'chofer';
    protected $_primary = array('Cho_Cod');
    protected $_state = 'Cho_Est';
    ...
    public function _selectBasic($cond=null){
        return $this->select()
            ->join('persona', "persona.Prs_Cod=$this->_name.Prs_Cod",array('Chofer'=>$this->expr($this->concat(array('persona.Prs_Ape','persona.Prs_Nom'))),'Ruc_Chofer'=>'Prs_Ced','Prs_Nom','Prs_Ape','Prs_Ced','Prs_Dir'))
        ;
    }
    ...
    public function sqlByNombre($id,$Par_Sql,$cond=null){
        ...
        switch($id){
            ...
            case "setEmpCod":
                ...
                //echo $this->getSqlString($sql)."<br/>";
                break;
            case "setLastVehiculo":
                $viaje=$this->select(false)->from('viaje',array('Veh_Cod'))->where("viaje.{$this->_primary[0]}=$this->_name.{$this->_primary[0]}")->order("Via_Cod DESC")->limit(1);
                $sql->joinLeft('vehiculo',"vehiculo.Veh_Cod=(".str_replace("\n",'',$this->toStr($viaje)).")",array('Veh_Cod','Veh_Pla','Veh_Col','Veh_Mar','Prv_Cod'));
                //echo $this->getSqlString($sql)."<br/>";
                break;
