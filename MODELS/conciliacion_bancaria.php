...
class conciliacion_bancaria extends AbstractModel{
    ...
    protected $_primary = array('Cob_Cod');
    ...
    public function _selectBasic($cond=null,$limits=false){
        return $this->select()
            ->join('perio_cont',"perio_cont.Pec_Cod=$this->_name.Pec_Cod",array('Pec_Fei','Pec_Year'=>$this->expr('YEAR(Pec_Fei)')))
            ->join('usuarios',"usuarios.Usu_Cod=$this->_name.Usu_Cod",array())
            ->join('persona',"persona.Prs_Cod=usuarios.Prs_Cod",array('Prs_Nom'))
            ->join('banco', "banco.Ban_Cod=$this->_name.Ban_Cod",array('Pld_Cod','Ban_Cue','Ban_Est','Ban_Est'))
            ->join('det_plan', "det_plan.Pld_Cod=banco.Pld_Cod",array('Pla_Cod','Pld_Cdc','Pld_Des','Pld_Tip'))
	    ->where("Cob_Est = 'A'");
        ;
    }
