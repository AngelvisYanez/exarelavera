...
$.extend($.jgrid,{
	...
	styleUI : {
		jQueryUI : {
			...
			inlinedit : {
				inputClass : "ui-widget-content ui-corner-all form-control input-xs",
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			loadComplete: null,
			gridComplete: null,
			loadError: function (jqXHR,stat,e) { $(this).clearGridData(true).msgMain('warning','El servidor fallo en responder!',true); },
			loadBeforeSend: null,
			afterInsertRow: null,
			beforeRequest: null,
		...
		$(this).empty().attr("tabindex",p.tabIndex||"0");
		this.p = p ;
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if (ts.p.viewrecords){
                    $(ts).msgMain('info','No se Encontraron Registros',(ts.p.reccount===0));//erik
					if(ts.p.reccount === 0) {
						...
						success:function(data,st, xhr) {
							...
							if( ts.p.loadonce || ts.p.treeGrid) {ts.p.datatype = "local";}
                                                        if(!data.success&& typeof data.error!=='undefined')
                                                            $(ts).msgMain('danger','&nbsp;Error al ejecutar la consulta!&nbsp;',true);//erik
							data=null;
							if (npage === 1) { endReq(); }
						},
		...
		for(i=0;i<this.p.colNames.length;i++){
			var tooltip = ts.p.headertitles ? (" title=\""+$.jgrid.stripHtml(ts.p.colModel[i]['labelLong']||ts.p.colNames[i])+"\"") :""; //erik
        ...
        $(grid.bDiv).append($('<div class="vcenter center" style="display:none;position:absolute;width:100%;height:100%;"><span><i class="fa fa-info-circle"></i>&nbsp;&nbsp;<strong></strong></span></div>')); //erik
	});
};
$.jgrid.extend({
    highlightRow : function(id,cell,color){ if(this.prop('tagName').toUpperCase()!=='TABLE'&&this.prop('tagName').toUpperCase()!=='TR') return this; var t=this,g=t.prop('id'),cols=(Array.isArray(cell))?cell:[cell],row=(typeof id!=='undefined'&&id!==null?'tr#'+id+' ':''),hl={color: typeof color==='string'?color:'#44faff'}; $.each(cols,function(i,c){ t.find(row+'td'+(typeof c==='string'?'[aria-describedby^="'+g+'_'+c+'"]':'')+':visible:not(.jqgrid-rownum)').effect('highlight', hl, 500); }); return this; }, //erik
    msgMain : function(clase,msg,show){ this.parent().next().find('span').attr('class','alert alert-'+clase).find('strong').html(msg).end().end()[show===true?'show':'hide']();  },
	...
	getCol : function (col, obj, mathopr) {
		var ret = [], val, sum=0, min, max, v, st;
		obj = typeof obj !== 'boolean' ? false : obj;
		if(mathopr === undefined) { mathopr = false; }else{
                    try{ st=$.isFunction(mathopr)?mathopr:(typeof mathopr==='string'&&!['sum','avg','count','min','max'].includes(mathopr)?eval(mathopr):undefined); }catch(e){}
                }
		this.each(function(){
			var $t=this, pos=-1;
			if(!$t.grid) {return;}
			...
			if(pos>=0) {
				var ln = $t.rows.length, i =0, dlen=0, aux=$.isFunction(st)?st(undefined,$t.p.colModel[pos]['name'],$($t).getRowData($t.rows[i].id)):undefined;
				if (ln && ln>0){
					while(i<ln){
						if($($t.rows[i]).hasClass('jqgrow')) {
							...
							if(mathopr){
								v = parseFloat(val);
								if(!isNaN(v)) {
									sum += $.isFunction(st)?st(v,$t.p.colModel[pos]['name'],$($t).getRowData($t.rows[i].id)):v;
		...
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function () {
					...
					$($t.grid.fbDiv).height($($t.grid.bDiv).height()-16-1);
		...
		return this.each(function(){
			...
			if(!!$t.p){ //erik
				...
				setTimeout(function(){
					...
					var gHead = $.extend([],$t.p.groupHeader);
					if (gHead != null && gHead.length) {
						for(var i=0,z=gHead.length;i<z;i++)
							$("#"+$.jgrid.jqID($t.p.id)).jqGrid("destroyGroupHeader").jqGrid("setGroupHeaders", gHead[i]); //erik
					}
				}, timeout);
			}
		});
...
//Helper functions
	...
	bindEv: function  (el, opt) {
		var $t = this;
		if(typeof opt.dataInit==='string'&&$.isFunction(eval(opt.dataInit)))opt.dataInit=eval(opt.dataInit);
		...
		return this.each(function(){
			...
			$.jgrid.viewModal("#"+$.jgrid.jqID(IDs.themodal),{
				...
				onHide :  function(h) {
					...
					h.w.remove();
					if(p.onClose) p.onClose.call(this, $(frm));
					if(h.o) {h.o.remove();}
				}
			});
	...
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			if (findnav) {
				...
				var tbd = $("<td></td>").addClass(p.classes);
			...
			var funcs = {
				...
				avg: function() {
					...
					return funcs.sum();
                        ...
                        var st;
                        try{ st=$.isFunction(fn)?fn:(typeof fn==='string'&&!['sum','avg','count','min','max'].includes(fn)?eval(fn):undefined); }catch(e){}

			if(!funcs[fn]&&st===undefined) {
				throw ("jqGrid Grouping No such method: " + fn);
			}
			var res = st===undefined?funcs[fn]():parseFloat(v||0)+parseFloat(st(rc[field],field,rc)||0);
...
//module begin
	...
	$.fn.fmatter.integer = function(cellval, opts) {
		...
		if($.fmatter.isEmpty(cellval)||(!isNaN(cellval)&&cellval*1===0)) { //erik
