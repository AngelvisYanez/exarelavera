...
b=this;void
0===a&&(a=500),setTimeout(function(){var winwidth=$(window).width(),parentwidth=$("#gbox_"+$.jgrid.jqID(b.p.id)).parent().width(),ww=b.p.width;if((winwidth-parentwidth)>3){ww=parentwidth}else{ww=winwidth}$("#"+$.jgrid.jqID(b.p.id)).jqGrid("setGridWidth",ww);var gHead=$.extend([],b.p.groupHeader);if(gHead!=null&&gHead.length>0){$("#"+$.jgrid.jqID(b.p.id)).jqGrid("destroyGroupHeader"); for(var i=0,z=gHead.length;i<z;i++){$("#"+$.jgrid.jqID(b.p.id)).jqGrid("setGroupHeaders",gHead[i])}}},a)})}})});!function(a){"use strict";"function"==typeof
...
0!==c.colModel&&void
0!==c.colModel.formatoptions&&(d=a.extend({},d,c.colModel.formatoptions)),a.fmatter.isEmpty(b)||(!isNaN(b)&&b*1===0)?d.defaultValue:a.fmatter.util.NumberFormat(b,d)},a.fn.fmatter.number=function(b,c){var
d=a.extend({},c.number);return void
0!==c.colModel&&void
0!==c.colModel.formatoptions&&(d=a.extend({},d,c.colModel.formatoptions)),a.fmatter.isEmpty(b)||(!isNaN(b)&&b*1===0)?d.defaultValue:a.fmatter.util.NumberFormat(b,d)},a.fn.fmatter.currency=function(b,c){var
d=a.extend({},c.currency);return void
0!==c.colModel&&void
0!==c.colModel.formatoptions&&(d=a.extend({},d,c.colModel.formatoptions)),a.fmatter.isEmpty(b)||(!isNaN(b)&&b*1===0)?d.defaultValue:a.fmatter.util.NumberFormat(b,d)},a.fn.fmatter.date=function(b,c,d,e){var
...
r=[];r=a.jgrid.findPos(f),d.left=($('body').width()-(d.width*1))/2/*r[0]+4*/,d.top=r[1]+4}q.top=d.top+"px",q.left=d.left}else(0!==d.left||0!==d.top)&&(q.left=d.left,q.top=d.top+"px");if(a("a.ui-jqdialog-titlebar-close",n).click(function(){var
...
d=this;if(typeof c.dataInit==='string'&&a.isFunction(eval(c.dataInit)))c.dataInit=eval(c.dataInit);(a.isFunction(c.dataInit))&&c.dataInit.call(d,b,c),c.dataEvents&&a.each(c.dataEvents,function(){void
